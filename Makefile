...
###############################################################################
################ EMR Serverless notebook image (SageMaker Studio) ##############
###############################################################################
# Custom EMR Serverless image consumed by SageMaker Unified Studio notebooks.
# This is the migration path for Databricks `cluster.custom_libraries`: EMR
# Serverless workers are ephemeral, so a baked image replaces cluster-installed
# libraries. See docs/platform/emr_serverless_notebook_image.md.
#
# EMR Serverless is amd64-only; PLATFORM is forced rather than inherited.

EMR_SERVERLESS_DOCKERFILE := .container/Dockerfile.emr-serverless
EMR_SERVERLESS_CONTEXT    := .container/emr-serverless
EMR_SERVERLESS_PLATFORM   := linux/amd64

# Source of the private wheels and JARs. Defaults to the prod artifacts bucket so
# a local build matches exactly what CI publishes. Override to validate against
# forno-published libraries before they are promoted:
...
.PHONY: fetch-emr-serverless-artifacts
## Downloads private wheels and JARs into .container/emr-serverless/ci-staged/.
## Requires AWS credentials in the environment. CI runs this via Woodpecker
## before plugins/docker; local builds call it automatically.
fetch-emr-serverless-artifacts:
	@ARTIFACTS_BUCKET=$(ARTIFACTS_BUCKET) sh .container/emr-serverless/fetch-artifacts.sh

.PHONY: build-emr-serverless-notebook
## Builds the base EMR Serverless notebook image (common library set).
build-emr-serverless-notebook: fetch-emr-serverless-artifacts
	@echo "Building EMR Serverless notebook image (base)"
	@echo "=========="
	@echo ""
	@DOCKER_BUILDKIT=1 docker build \
	  --platform $(EMR_SERVERLESS_PLATFORM) \
	  --target emr-serverless-base \
	  -t bi-etl-ejuice:emr-serverless-base-latest \
	  -f $(EMR_SERVERLESS_DOCKERFILE) \
	  .
	@echo ""
	@echo "-> Image ready: bi-etl-ejuice:emr-serverless-base-latest"

.PHONY: build-emr-serverless-notebook-heavy
## Builds the heavy EMR Serverless notebook image (base + geo/ML/NLP stack).
## Roughly 1 GB larger than base; only needed by Sedona / Presidio / LightGBM DAGs.
build-emr-serverless-notebook-heavy: fetch-emr-serverless-artifacts
	@echo "Building EMR Serverless notebook image (heavy)"
	@echo "=========="
	@echo ""
	@DOCKER_BUILDKIT=1 docker build \
	  --platform $(EMR_SERVERLESS_PLATFORM) \
	  --target emr-serverless-heavy \
	  -t bi-etl-ejuice:emr-serverless-heavy-latest \
	  -f $(EMR_SERVERLESS_DOCKERFILE) \
	  .
	@echo ""
	@echo "-> Image ready: bi-etl-ejuice:emr-serverless-heavy-latest"

.PHONY: lock-emr-serverless-notebook
## Regenerates the pinned requirements for the EMR Serverless notebook image.
## Run after editing requirements-base.in / requirements-heavy.in / constraints.txt,
## then commit the resulting .txt locks. Resolution targets Python 3.9 to match
## EMR 7.12's default interpreter.
lock-emr-serverless-notebook:
	@echo "Locking EMR Serverless notebook requirements (Python 3.9)"
	@echo "=========="
	@echo ""
	@uv pip compile $(EMR_SERVERLESS_CONTEXT)/requirements-base.in \
	  -c $(EMR_SERVERLESS_CONTEXT)/constraints.txt \
	  --python-version 3.9 \
	  --output-file $(EMR_SERVERLESS_CONTEXT)/requirements-base.txt
	@uv pip compile $(EMR_SERVERLESS_CONTEXT)/requirements-heavy.in \
	  -c $(EMR_SERVERLESS_CONTEXT)/constraints.txt \
	  --python-version 3.9 \
	  --output-file $(EMR_SERVERLESS_CONTEXT)/requirements-heavy.txt
	@echo ""
	@echo "-> Locks updated. Review the diff before committing."
