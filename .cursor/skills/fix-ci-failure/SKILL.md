...
| `invalid domain` | `domain:` is not in the allowed list | Use a valid domain from the SSOT `packages/bietlejuice-core/src/bietlejuice/governance/domains.yml` (see `fairness_metadata.mdc` / `governance_metadata.mdc` for guidance) |
...
| `F2-02 column_description_not_substantive` | Column `description` passes Yamale min length but fails TDQ heuristics | Rewrite using **SQL + declaration + `docs/llm_context/`**. For bulk remediation use skill **`fair-metadata`**. |
