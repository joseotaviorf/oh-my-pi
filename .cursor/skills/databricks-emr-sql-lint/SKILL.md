...
description: Lints SQL under dags/ for Databricks-only constructs that break, change results, or silently degrade performance on EMR Spark 3.5 — queries must run on both DBR 16.4 and plain EMR Spark 3.5. Detects QUALIFY, GROUP BY ALL, IFF, DECODE, 3-arg DATEDIFF, variant access (column:key), legacy DATE_FORMAT pattern letters, Databricks-only RANGE_JOIN/SKEW hints that EMR ignores, and joins with no extractable hash key that Spark can only plan as a BroadcastNestedLoopJoin. Reports severity, line numbers, and dual-runtime-safe rewrites; lint-only, never edits unless explicitly asked. Use when editing or creating any .sql file in dags/, or when the user mentions EMR migration, Spark 3.5 compatibility, dual-runtime, ParseException, nested loop, OR join, range join, BroadcastNestedLoopJoin, a query that is fast on Databricks but slow on EMR, or asks "is this query EMR-compatible?".
