...
from emr.aws_auth import call_with_aws_retry
...
from emr.result_fetch import fetch_json_from_s3, format_result_json_line
from emr.steps import build_spark_step

REUSABLE_CLUSTER_STATES = frozenset({"WAITING", "RUNNING"})
...
def submit_step_to_cluster(
    ...
    *,
    ...
    if wait_result and not wait:
        raise ValueError("--wait-result requires --wait")
    if wait_result and not result_s3_uri:
        raise ValueError("--wait-result requires --result-s3-uri")
    ...
    if wait_result and result_s3_uri:
        region = _resolve_region(cfg)
        payload = call_with_aws_retry(
            lambda: fetch_json_from_s3(
                result_s3_uri,
        ...
        if payload is None:
            print(
                f"Missing validation result at {result_s3_uri}",
                file=sys.stderr,
            )
            raise SystemExit(1)
        print(format_result_json_line(payload))


def _cluster_tags_dict(tags: list[dict[str, str]] | None) -> dict[str, str]:
    out: dict[str, str] = {}
    for tag in tags or []:
        key = tag.get("Key")
        if key is not None:
            out[str(key)] = str(tag.get("Value", ""))
    return out


def describe_cluster(*, cluster_id: str, region: str | None) -> dict[str, Any]:
    client = _emr_client(region)
    resp = client.describe_cluster(ClusterId=cluster_id)
    cluster = resp["Cluster"]
    status = cluster.get("Status") or {}
    return {
        "id": cluster.get("Id", cluster_id),
        "name": cluster.get("Name", ""),
        "state": status.get("State", "UNKNOWN"),
        "tags": _cluster_tags_dict(cluster.get("Tags")),
    }
...
def list_active_clusters(
    *,
    ...
    client = _emr_client(region)
    resp = client.list_clusters(ClusterStates=list(REUSABLE_CLUSTER_STATES))
    clusters: list[dict[str, Any]] = []
    for summary in resp.get("Clusters") or []:
        cluster_id = summary.get("Id")
        if not cluster_id:
            continue
        detail = describe_cluster(cluster_id=cluster_id, region=region)
        if tag_key is not None:
            if detail["tags"].get(tag_key) != tag_value:
                continue
        clusters.append(detail)
    return clusters


def print_clusters_json(clusters: list[dict[str, Any]]) -> None:
    print(json.dumps(clusters, indent=2))


def print_cluster_json(cluster: dict[str, Any]) -> None:
