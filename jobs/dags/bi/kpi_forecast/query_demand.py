query_demand = """select 
        f.sk_property,
        -- f.sk_booking, -- ribaldo said to use id_booking instead of sk_booking. there is a difference between the two when rescheduled (same id booking but different sk_booking). we are already removing rescheduled lines but for some other reason the count is different
        f.sk_owner,
        f.sk_user_affiliate,
        f.sk_user_agent,
        f.sk_user_visitor,
        ...
        -- booking --
        db.id_booking, -- use instead of sk_booking
        db.visit_follow_up,
        --db.visit_type? ...
        ...
        -- where db.reason_category != 'Reschedule' -- when a booking is cancelled AND rescheduled we want to count the new scheduling only. the status of the first one will be cancelled and the reason will be 'rescheduling'
        ;"""
