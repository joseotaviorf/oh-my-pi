    ...
    salaries_cte AS (
        SELECT
            ...
            CASE s.action_reason
                WHEN "Dissídio / Acordo Coletivo" THEN UPPER("Dissídio")
                WHEN "Contratar" THEN UPPER("Contratação")
                WHEN "Dissídio / Convenção Coletiva" THEN UPPER("Dissídio")
                WHEN "Outros Casos" THEN UPPER("Outros")
                WHEN "Readmitir para preencher posição vaga" THEN UPPER("Readmissão")
                WHEN "Salário Mínimo" THEN UPPER("Enquadramento")
                WHEN "Efetivação de Estagiário ou Aprendiz" THEN UPPER("Efetivação")
                WHEN "Movimentação de Centro de Custo" THEN UPPER("Outros")
                WHEN "Reorganização" THEN UPPER("Alteração de Função")
                WHEN "Futura contratação para preencher posição vaga" THEN UPPER("Contratação")
                WHEN "1º Emprego" THEN UPPER("Contratação")
            ELSE UPPER(s.action_reason)
