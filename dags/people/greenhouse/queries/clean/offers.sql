SELECT
    ...
    regexp_extract(keyed_custom_fields['salary']['value'], 'unit=([A-Z]+)', 1) AS salary_unit,
    regexp_extract(keyed_custom_fields['brasil_annual_salary_amount']['value'], 'unit=([A-Z]+)', 1) AS annual_salary_unit,
    keyed_custom_fields['employment_type']['value'] AS employment_type,
    keyed_custom_fields['workplace']['value'] AS workplace,
    regexp_extract(keyed_custom_fields['reporting_to_manager']['value'], 'name=([^,]+)', 1) AS reporting_to_manager_name,
    keyed_custom_fields['car_allowance']['value'] AS car_allowance,
    keyed_custom_fields['plan_m_dico']['value'] AS medical_plan,
    regexp_extract(keyed_custom_fields['sign_on_bonus']['value'], 'unit=([A-Z]+)', 1) AS hiring_bonus_unit,
    regexp_extract(keyed_custom_fields['stock_options_cash_amount_offered']['value'], 'unit=([A-Z]+)', 1) AS stock_options_cash_unit,
    opening['status'] AS opening_status,
    -- numeric
    version,
    CAST(regexp_extract(keyed_custom_fields['salary']['value'], 'value=([0-9.]+)', 1) AS DECIMAL(10,2)) AS salary_value,
    CAST(regexp_extract(keyed_custom_fields['brasil_annual_salary_amount']['value'], 'value=([0-9.]+)', 1) AS DECIMAL(10,2)) AS annual_salary_value,
    CAST(regexp_extract(keyed_custom_fields['sign_on_bonus']['value'], 'value=([0-9.]+)', 1) AS DECIMAL(10,2)) AS hiring_bonus_value,
    CAST(regexp_extract(keyed_custom_fields['stock_options_cash_amount_offered']['value'], 'value=([0-9.]+)', 1) AS DECIMAL(10,2)) AS stock_options_cash_value,
    -- boolean
