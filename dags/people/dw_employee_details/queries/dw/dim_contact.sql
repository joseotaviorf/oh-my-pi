...
personal_email_at_version AS (
    ...
    FROM (
        SELECT
            ...
            ROW_NUMBER() OVER (
                PARTITION BY cv.id_person, cv.dt_effective_started
                ORDER BY ea.dt_started DESC
            ) AS _rn
        FROM
...
github_at_version AS (
    ...
    FROM (
        SELECT
            ...
            ROW_NUMBER() OVER (
                PARTITION BY cv.id_person, cv.dt_effective_started
                ORDER BY pl.dt_effective_started DESC
            ) AS _rn
        FROM
            contact_versions AS cv
        INNER JOIN
...
address_at_version AS (
    ...
    FROM (
        SELECT
            ...
            ROW_NUMBER() OVER (
                PARTITION BY cv.id_person, cv.dt_effective_started
                ORDER BY a.dt_effective_started DESC
            ) AS _rn
        FROM
            contact_versions AS cv
        INNER JOIN
    ...
    SELECT
        ...
        COALESCE(
            LEAD(cv.dt_effective_started) OVER (
                PARTITION BY cv.id_person
                ORDER BY cv.dt_effective_started
            ) - INTERVAL '1 DAY',
            DATE('9999-12-31')
        ) AS dt_valid_to,
        (
        ...
        NOW() AS ts_load,
        cv.id_person AS _id_person,
        cv.dt_effective_started AS _dt_effective_started,
        cv.dt_effective_ended AS _dt_effective_ended
    FROM
        contact_versions AS cv
    LEFT JOIN
...
FROM (
    SELECT
        *,
        ROW_NUMBER() OVER (
            PARTITION BY _id_person, _dt_effective_started
            ORDER BY _dt_effective_ended DESC NULLS LAST
        ) AS _rn
    FROM final_joined
)
WHERE _rn = 1
