...
* **Effective workforce** : `is_effective_worker = TRUE` marks permanent/effective employees (CLT) and is `FALSE` for interns (Estagiário) and young apprentices (Jovem Aprendiz). The flag is stable for the life of each assignment; once non-effective, it stays non-effective. An intern or apprentice who converts to an effective role does so under a new assignment with `is_effective_worker = TRUE`. Official turnover and attrition scope to `is_effective_worker = TRUE`.
