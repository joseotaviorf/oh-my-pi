...
* **Hire date semantics** : The fact table exposes two hire dates that serve different purposes. `dt_hired` is the start date of the employee's most recent contract, regardless of any prior employment history. `dt_original_hire` is the start date of their very first contract at the company, which may predate a cross-entity transfer. For tenure calculations that should credit prior employment, use `dt_original_hire`; for calculating seniority within the current contract, use `dt_hired`. The flag `is_transfer_hire` indicates that the current contract began as the incoming side of a transfer from a previous one, meaning the person was already at the company before this assignment started.
