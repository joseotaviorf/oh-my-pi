...
assignments AS (
  SELECT
    ...
    assignments ['GradeLadderName'] AS comp_ladder_directorate,
    ...
    TO_TIMESTAMP(
      SUBSTR(
        REPLACE(assignments ['CreationDate'], 'T', ' '),
        0,
        19
      ),
      'yyyy-MM-dd HH:mm:ss'
    ...
    TO_TIMESTAMP(
      SUBSTR(
        REPLACE(assignments ['LastUpdateDate'], 'T', ' '),
        0,
        19
      ),
      'yyyy-MM-dd HH:mm:ss'
...
assignments_dff AS (
  SELECT
    ...
    assignments_dff ['trilha'] AS career_track,
    ...
    TO_TIMESTAMP(
      SUBSTR(
        REPLACE(assignments_dff ['CreationDate'], 'T', ' '),
        0,
        19
      ),
      'yyyy-MM-dd HH:mm:ss'
    ...
    TO_TIMESTAMP(
      SUBSTR(
        REPLACE(assignments_dff ['LastUpdateDate'], 'T', ' '),
        0,
        19
      ),
      'yyyy-MM-dd HH:mm:ss'
...
SELECT DISTINCT
  ...
  -- dates
  GREATEST (
    DATE(a.dt_effective_start),
    DATE(adff.dt_effective_start)
  ) AS dt_effective_start,
  GREATEST (
    DATE(a.dt_effective_end),
    DATE(adff.dt_effective_end)
  ) AS dt_effective_end,
...
WHERE
  ...
  AND (
    DATE(a.dt_effective_start) BETWEEN DATE_SUB(DATE('{load_start_date}'), 35)
    AND DATE('{load_end_date}')
    OR DATE(adff.dt_effective_start) BETWEEN DATE_SUB(DATE('{load_start_date}'), 35)
    AND DATE('{load_end_date}')
    OR DATE(a.ts_last_update) BETWEEN DATE_SUB(DATE('{load_start_date}'), 35)
    AND DATE('{load_end_date}')
    OR DATE(adff.ts_last_update) BETWEEN DATE_SUB(DATE('{load_start_date}'), 35)
    AND DATE('{load_end_date}')
  )
QUALIFY
  ROW_NUMBER() OVER (PARTITION BY a.id_assignment, COALESCE(a.dt_effective_start, a.ts_last_update) ORDER BY a.COALESCE(a.dt_effective_start, a.ts_last_update) DESC) = 1
