...
> Organizational reference tables covering cost centers and Codex team data, business units, and public job definitions: the structural dimensions used to classify and analyze headcount in People Analytics.
...
❌ **Employee records** : Individual employee data is not part of this schema. For public employee information, use `dw_people`; for detailed and sensitive employee data, use `dw_employee_details`.
...
* **Temporal coverage:** Mixed. `dim_cost_center` preserves the full history of attribute changes (validity window); `dim_business_unit` and `dim_job` reflect the current state only.
