WITH
  base_data AS (
    SELECT
      a.id_application,
      q.id_demographic_question AS id_question,
      ao.id_demographic_answer_option AS id_answer,
      q.name AS question,
      ...
      a.ts_updated
    FROM
      datalake_greenhouse_v3_clean.demographic_answers AS a
    LEFT JOIN 
      datalake_greenhouse_v3_clean.demographic_questions AS q
        ON a.id_demographic_question = q.id_demographic_question
    LEFT JOIN
      datalake_greenhouse_v3_clean.demographic_answer_options AS ao
        ON a.id_demographic_answer_option = ao.id_demographic_answer_option
  ),
  ...
  disability_type_enriched AS (
    ...
    FROM base_data
    WHERE id_question IN (4003824009, 4003867009, 4003874009)
    GROUP BY
      id_application
  ),
...
SELECT
  ...
  dt.id_answer AS answer_disability_type_list_ids,
