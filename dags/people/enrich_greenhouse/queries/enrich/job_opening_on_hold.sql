...
LEFT JOIN
  datalake_greenhouse.job_openings AS open
    ON open.id = start.id_job_opening
LEFT JOIN
  datalake_greenhouse_clean.close_reasons AS close
    ON close.id = start.id_close_reason
WHERE
