SELECT
    ...
    NULLIF(band, '') AS band,
    NULLIF(_job_, '') AS job,
    NULLIF(_job_classification_, '') AS job_classification,
    ...
    NULLIF(readiness_for_promotion, '') AS readiness_for_promotion,
    ...
    NULLIF(CAST(to_which_positionperson AS STRING), '') AS successor_target_position,
    ...
    NULLIF(CAST(no_official_on_going_performance_updates AS STRING), '') AS informal_performance_updates,
    NULLIF(CAST(last_editor AS STRING), '') AS last_editor,
    NULLIF(type_of_last_movementrecognition, '') AS last_movement_recognition_type,
    ...
    TO_DATE(NULLIF(CAST(deadline_mmddyyyy AS STRING), ''), 'MM/dd/yyyy') AS dt_plan_deadline,
    ...
    TO_DATE(NULLIF(date_of_last_movementrecognition, ''), 'dd/MM/yyyy') AS dt_last_movement_recognition,
    NOW() AS ts_load
FROM
