# `xray_job_history_edc` — reverse export governance


| Field | Value |
| --- | --- |
| **Metastore table** | `reverse_reports.xray_job_history_edc` |
| **Business owner** | People Insights |
| **Technical owner** | Enterprise Engineering and People Insights |
| **Domain** | People |
| **One-line summary** | Job (cargo) tenure windows per employee for X-Ray AppSheet. |
| **Business purpose** | Historical job changes for X-Ray / Employee Data Center. Migrated from Daily Pipeline notebook `xray_update` ([DBP-1447](https://quintoandar.atlassian.net/browse/DBP-1447)). |
...
| **Delivery channel** | Google Sheets tab **historico_cargos** in workbook [https://docs.google.com/spreadsheets/d/1SNGQmGyj9rMCUkfQgPg2T5TAbpre-3yMPEt-2_aPux4/edit?usp=sharing](https://docs.google.com/spreadsheets/d/1SNGQmGyj9rMCUkfQgPg2T5TAbpre-3yMPEt-2_aPux4/edit?usp=sharing). Service account editor: `gsheets-people-access@airflow-186119.iam.gserviceaccount.com`. |
| **Contract notes** | Grain: one row per assignment + job name tenure stint (min/max `dt_month_reference`). Stints use gaps-and-islands over the job name so a job revisited later gets a separate row instead of merging into the same window. |
