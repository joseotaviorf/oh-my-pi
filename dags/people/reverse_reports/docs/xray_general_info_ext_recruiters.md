...
| **Contract notes** | Grain is one row per employee (same as `xray_general_info`). Exported headers match the AppSheet General Information tab. Rows whose `banda` is not castable to integer are excluded. `structure = People` is out of scope. Anyone whose manager or L1–L7 name matches Abi Saber (spaces stripped) is out of scope, including Deborah herself. `dt_last_update` is load-time metadata from the parent export. |
