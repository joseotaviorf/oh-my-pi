-- P&T headcount roster with termination date (active + terminated).
-- Scope is the P&T domain defined by the L1 manager (source of truth): the
-- CTO (paulo braz golgher) and the CPO (rafael dantas de castro). No active
-- filter is applied so terminated employees keep their dt_desligamento.
WITH
    pnt_management_hierarchy AS (
        SELECT
            assignment_number,
            ...
            name_l8
        FROM
            dw_employee_details.dim_management_hierarchy
        WHERE
            is_current = TRUE
            AND LOWER(TRIM(name_l1)) IN (
                'paulo braz golgher',
...
SELECT
    ...
    CASE
        WHEN f.dt_terminated <= CURRENT_DATE() THEN 'desligado'
        ELSE 'ativo'
    END AS status,
