...
[Link to Catalog Row](https://quintoandar.atlassian.net/wiki/spaces/team162449f9cca34903915bfe1c1c6c507e/pages/5474320386/People+Data+Catalog)
...
| `fact_hours_bank_rule_totals` | One row per employee, date, and hourly-bank bucket, with closed/open balance status (API rule breakdown) | [DataHub](https://datahub.apps.data-prd.habitat.zone/dataset/urn:li:dataset:(urn:li:dataPlatform:trino,hive.dw_time.fact_hours_bank_rule_totals,PROD)/Schema) · [SQL](https://github.com/quintoandar/bi-etl-ejuice/blob/master/dags/people/dw_time/queries/dw/fact_hours_bank_rule_totals.sql) |
| `fact_hours_bank_daily_totals` | One row per employee and date with UI-aligned Banco Acumulado (last matured API balance carried forward) | [SQL](https://github.com/quintoandar/bi-etl-ejuice/blob/master/dags/people/dw_time/queries/dw/fact_hours_bank_daily_totals.sql) |
...
* **Hourly-bank balance line** : Each row is one employee on a calendar balance date within one hourly-bank bucket. The `is_closed` boolean separates official closed balances (`true`) from open current-period balances (`false`) that can still receive adjustments. Use `fact_hours_bank_rule_totals` for this API rule breakdown.

* **UI-aligned daily hours-bank total** : Each row in `fact_hours_bank_daily_totals` is one employee on a calendar day with `sum_minutes_running_balance` matching OiTchau Banco Acumulado. The series is bounded to the load month (opening stock = API balance on the first day in that month) and carries the last matured API balance forward — the trailing snapshot day arrives zeroed and is excluded until the 45-day ingestion lookback rewrites it; the matured API already embeds jornada debits. `amount_minutes_delta_day_over_day` is the day-over-day delta (OiTchau UI label Banco Dia). Use this table when the total must match the product UI; keep `fact_hours_bank_rule_totals` for segment slices and overtime cost estimates.
...
* **UI Banco Acumulado vs API** : `fact_hours_bank_rule_totals` mirrors the OiTchau API rule map per rule segment; matured API balances match the product UI (the trailing snapshot day arrives zeroed until the ingestion lookback rewrites it). Prefer `fact_hours_bank_daily_totals.sum_minutes_running_balance` for a gap-free daily UI-parity series.
