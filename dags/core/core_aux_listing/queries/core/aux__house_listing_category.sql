...
--------------------------------------------------------------------------------------------------------
    SELECT
        ...
        MAX(has_termination_canceled) AS has_termination_canceled,
        MAX(state_order) AS max_order_status
    FROM
        core_listing.aux__lbc_status_version_order
    GROUP BY
...
house_status_version_last_status AS (
    SELECT
        ...
        MAX(
            CASE
                WHEN lbc_vo.status IN ('despublicado', 'UNPUBLISHED') THEN ts_state_started
            END
        ) OVER(PARTITION BY lbc_vo.id_house, lbc_vo.listing_version) AS ts_last_unpublished,
        MAX(
            lbc_vo.status = 'alugado'
            OR (
                lbc_vo.status = 'SUSPENDED'
                AND lbc_vo.status_reason = 'RENTED'
            )
    ...
    FROM
        core_listing.aux__lbc_status_version_order AS lbc_vo
    LEFT JOIN
...
SELECT
    id_house,
    CAST(id_house || LPAD(version, 3, '0') AS BIGINT) AS id_house_listing,
    version,
    status,
    status_history,
    status_reason,
