...
class CoreListingSparkJob(BaseCoreModelSparkJob):
    ...
    def get_listing_config(self):
        ...
        return {
            ...
            "LISTING_BUSINESS_CONTEXT_TABLE": self.get_config(
                "LISTING_BUSINESS_CONTEXT_TABLE"
            ),
            "HOUSE_TABLE": self.get_config("HOUSE_TABLE"),
            "AUX_LBC_STATUS_VERSION_ORDER_TABLE": self.get_config(
                "AUX_LBC_STATUS_VERSION_ORDER_TABLE"
            ),
            "AUX_HOUSE_LISTING_CATEGORY_TABLE": self.get_config(
    ...
    def create_core_model(self, spark: SparkSession, args) -> DataFrame:
        ...
        # Load source data
        listing_business_context_df = spark.read.table(
            config["LISTING_BUSINESS_CONTEXT_TABLE"]
        )
        house_df = spark.read.table(config["HOUSE_TABLE"])
        aux_lbc_status_version_order_df = spark.read.table(
            config["AUX_LBC_STATUS_VERSION_ORDER_TABLE"]
        )
        aux_house_listing_category_df = spark.read.table(
            config["AUX_HOUSE_LISTING_CATEGORY_TABLE"]
        )
        ...
        result_df = SurrogateKeysHelper.generate_surrogate_key(
            result_df, config["ENTITY_TYPE"], id_column="id_listing"
        )
    ...
    def _process_rent_listings(
        ...
        lbc_rent_df = (
            listing_business_context_df.filter(col("business_context") == "RENT")
            .alias("lbc_base")
            .join(
                house_df.select(
                    col("id"),
        ...
        rent_base_df = (
            aux_lbc_status_version_order_df.select(
                col("id_house"),
                col("listing_version"),
        ...
        rent_joined_df = rent_base_df.alias("rb").join(
            lbc_rent_df.alias("lbc"), col("rb.id_house") == col("lbc.id_house"), "inner"
        )
        ...
        rent_df = rent_joined_df.select(
            ...
            .otherwise(col("hlc.listing_category"))
            .alias("category"),
            col("lbc.ownership"),
            col("lbc.business_context"),
            col("rb.is_extended_rental"),
            col("rb.has_termination_canceled"),
            (col("rb.max_listing_version") == col("rb.listing_version")).alias(
    ...
    def _process_sale_listings(
        self, listing_business_context_df: DataFrame, house_df: DataFrame
    ) -> DataFrame:
        """Process SALE listings from listing_business_context table."""

        # Expression for id_house_listing
        id_house_listing_expr = concat(
            col("lbc.id_house").cast("string"), lit("000")
        ).cast("bigint")
        ...
        sale_df = sale_joined_df.select(
            ...
            sha2(
                concat(
                    id_house_listing_expr.cast("string"), col("lbc.business_context")
                ),
                256,
            ).alias("id_listing"),
            lit(0).alias("version"),
