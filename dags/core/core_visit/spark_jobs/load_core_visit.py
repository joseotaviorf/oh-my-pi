...
class CoreVisitSparkJob(BaseCoreModelSparkJob):
    ...
    def create_core_model(self, spark: SparkSession, args) -> DataFrame:
        ...
        result_df = SurrogateKeysHelper.generate_surrogate_key(
            result_df, config["ENTITY_TYPE"], id_column="id_visit"
        )
        result_df = result_df.withColumnRenamed("surrogate_key", "sk_core_visit")
    ...
    def _process_last_event(self, visit_status_log_df):
        ...
        return (
            visit_status_log_df.withColumn("row_num", row_number().over(window_spec))
            .filter(col("row_num") == 1)
            .select(col("id_visit"), col("event_type").alias("last_event_type"))
        )
    ...
    def _process_all_events(self, visit_status_log_df):
        ...
        return visit_status_log_df.groupBy("id_visit").agg(
            spark_max(
                when(
                    col("event_type").isin("VISIT_REQUEST_CANCELED", "VISIT_CANCELED"),
                    col("reason"),
                )
            ...
            spark_max(when(col("event_type") == "VISIT_DONE", col("ts_created"))).alias(
                "ts_visit_done"
            ),
            spark_max(
                when(
    ...
    def _join_all_data(
        ...
        result_df = result_df.alias("v").join(
            vsl_last_event_df.alias("vsl"), col("v.id") == col("vsl.id_visit"), "left"
        )

        # Join with all events
        result_df = result_df.join(
            all_events_df.alias("ae"), col("v.id") == col("ae.id_visit"), "left"
        )
