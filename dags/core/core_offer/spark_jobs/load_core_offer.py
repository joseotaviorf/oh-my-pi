...
class CoreOfferSparkJob(BaseCoreModelSparkJob):
    ...
    def get_offer_config(self):
        ...
        return {
            ...
            "RENTAL_TRANSACT_OFFER_TABLE": self.get_config(
                "RENTAL_TRANSACT_OFFER_TABLE"
            ),
    ...
    def create_core_model(self, spark: SparkSession, args) -> DataFrame:
        ...
        if (
            ...
            self.logger.info(
                f"m=create_core_model, mode=INCREMENTAL, load_start_date={args.load_start_date}, load_end_date={args.load_end_date}"
            )
        else:
            self.logger.info(
        ...
        result_df = SurrogateKeysHelper.generate_surrogate_key(
            assembled_df, config["ENTITY_TYPE"], "id_offer"
        )
        ...
        self.logger.info(
            f"m=create_core_model, msg=Core offer model created with {result_df.count()} records"
        )
