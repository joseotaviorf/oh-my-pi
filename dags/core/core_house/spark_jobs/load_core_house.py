...
class CoreHouseSparkJob(BaseCoreModelSparkJob):
    ...
    def _load_house_data(self, spark, config, args):
        ...
        if (
            ...
            hlr_updated_ids = (
                spark.read.table(config["HOUSE_LISTING_RELATION_TABLE"])
                .filter(
                    (col("ts_database_transaction").cast("date") >= start_date)
                    & (col("ts_database_transaction").cast("date") <= end_date)
                )
            ...
            return house_df.join(
                broadcast(
                    all_house_ids
                ),  # Spark sends a complete copy to all executors
    ...
    def create_core_model(self, spark: SparkSession, args) -> DataFrame:
        ...
        house_listing_relation_df = spark.read.table(
            config["HOUSE_LISTING_RELATION_TABLE"]
        )
        ...
        latest_hlr_df = self._get_latest_house_listing_relation(
            house_listing_relation_df
        )

        # Build the core model
        result_df = self._build_core_house(filtered_houses_df, latest_hlr_df, user_df)

        # Generate surrogate key
        result_df = SurrogateKeysHelper.generate_surrogate_key(
            result_df, config["ENTITY_TYPE"], id_column="id_house"
        )
    ...
    def _get_latest_house_listing_relation(
        self, house_listing_relation_df: DataFrame
    ) -> DataFrame:
    ...
    def _build_core_house(
        ...
        result_df = filtered_houses_df.alias("h").join(
            latest_hlr_df.alias("hlr"), col("h.id") == col("hlr.id_house"), "left"
        )

        # Join with user table for owner from house_listing_relation
        result_df = result_df.join(
            user_df.alias("u1"), col("hlr.id_related") == col("u1.id"), "left"
        )

        # Join with user table for owner from house.id_user (fallback)
        result_df = result_df.join(
            user_df.alias("u2"), col("h.id_user") == col("u2.id"), "left"
        )
