WITH company_product AS (
  ...
  FROM datalake_company_clean.company_product AS cp
  GROUP BY ALL
), company_address AS (
  SELECT
    id_company,
  ...
  FROM (
    SELECT
      ca.id_company,
      ca.id_address,
      ROW_NUMBER() OVER (PARTITION BY ca.id_company ORDER BY ca.ts_updated DESC) AS _w,
      ca.ts_updated
    FROM datalake_company_clean.company_address AS ca
  ) AS _t
  WHERE
    _w = 1
), company_cnpj_aux AS (
  ...
  FROM (
    SELECT
      ...
      cd.ts_updated,
      ROW_NUMBER() OVER (PARTITION BY cd.id_company ORDER BY cd.ts_updated DESC) AS _w
    FROM datalake_company_clean.company_document AS cd
    INNER JOIN datalake_company_clean.document AS d
      ON cd.id_document = d.id AND d.document_type = 'CNPJ' AND d.status = 'ACTIVE'
  ) AS _t
  WHERE
    _w = 1
), company_cnpj AS (
  ...
  FROM (
    SELECT
      ...
      ca.ts_updated,
      ROW_NUMBER() OVER (PARTITION BY identification_number ORDER BY ts_updated DESC) AS _w
    FROM company_cnpj_aux AS ca
  ) AS _t
  WHERE
    _w = 1
), company_creci AS (
  ...
  FROM (
    ...
    FROM datalake_company_clean.company_document AS cd
    INNER JOIN datalake_company_clean.document AS d
      ON cd.id_document = d.id AND d.document_type = 'CRECI' AND d.status = 'ACTIVE'
  ) AS _t
  WHERE
    _w = 1
), company_members AS (
  SELECT
    ...
    cm.company_name,
    cm.extracted_3p_tag
  FROM datalake_hubspot.company_members AS cm
...
SELECT
  ...
  c.id AS id_company,
  ca.id_address,
  COALESCE(cmc.id_company, cmu.id_company) AS id_hubspot,
  ...
  c.company_name,
  c.trade_name,
  COALESCE(cmc.company_name, cmu.company_name) AS hubspot_company_name,
  COALESCE(cmc.extracted_3p_tag, cmu.extracted_3p_tag) AS extracted_3p_tag,
  cp.is_company_asp,
  cp.is_company_ciq,
  ...
  c.ts_created,
  c.ts_updated
FROM datalake_company_clean.company AS c
LEFT JOIN company_product AS cp
  ON c.id = cp.id_company
LEFT JOIN company_address AS ca
  ON c.id = ca.id_company
LEFT JOIN company_cnpj AS ccn
  ON c.id = ccn.id_company
LEFT JOIN company_creci AS ccr
  ON c.id = ccr.id_company
LEFT JOIN company_members AS cmu
  ON c.uuid_company = cmu.uuid_company
LEFT JOIN company_members AS cmc
