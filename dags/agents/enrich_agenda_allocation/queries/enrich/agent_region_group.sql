...
daily_region AS (
    SELECT
        DATE(TO_TIMESTAMP(ad.date, 'YYYY-MM-DD HH:mm:ss'))  AS ts_slot,
        ara.id_agent,
        ara.id_region,
        aux.region_code,
    ...
    WHERE
        ...
        AND TO_TIMESTAMP(ad.date, 'YYYY-MM-DD HH:mm:ss') > TO_TIMESTAMP(DATE('2018-01-31 00:00:00'), 'YYYY-MM-DD HH:mm:ss')  -- limit date, where aud started to be implemented
...
region_records_union AS (
    ...
    WHERE
        ...
        AND ad.date > DATE('2018-01-31') -- limit date, where aud started to be implemented
        AND ad.date BETWEEN '{load_start_date}' AND '{load_end_date}'
),
