...
SELECT
    ...
    GREATEST(CAST(date AS TIMESTAMP), ts_status_started) AS ts_validity_started,
    LEAST(CAST(date AS TIMESTAMP) + INTERVAL 1 DAY, ts_status_ended) AS ts_validity_ended,
    NOW() AS ts_load,
    year,
    month,
    day
