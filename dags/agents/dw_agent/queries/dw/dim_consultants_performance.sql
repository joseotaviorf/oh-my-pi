WITH base_p AS (
    SELECT
        DATE(dhl.ts_publication) AS dt_publication,
    ...
    LEFT JOIN 
        datalake_gsheets_clean.asp_consultants_allocation AS asp_alloc
            ON fhl.sk_owner % 100 BETWEEN asp_alloc.range_start AND asp_alloc.range_end
            AND dhl.ts_publication BETWEEN asp_alloc.dt_start AND asp_alloc.dt_end
            AND asp_alloc.team LIKE '%' || dhl.listing_category_start || '%'
    LEFT JOIN 
        dw_public.dim_user asp
            ON asp_alloc.id_user = asp.sk_user
    LEFT JOIN 
        dw_public.dim_user AS owner
            ON owner.sk_user = fhl.sk_owner
    LEFT JOIN 
        datalake_ebdb_clean.user_pro_owner AS pp_multi
            ON owner.sk_user = pp_multi.id_user
            AND dhl.ts_publication > DATE(pp_multi.ts_created)
    LEFT JOIN 
        dw_rent.fact_house_listings AS fhl_asp_previous
            ON fhl_asp_previous.sk_owner = fhl.sk_owner
            AND fhl_asp_previous.sk_house_listing <> fhl.sk_house_listing
    LEFT JOIN 
        dw_rent.dim_house_listing AS dhl_asp_previous
            ON dhl_asp_previous.sk_house_listing = fhl_asp_previous.sk_house_listing
            AND dhl_asp_previous.ts_publication < dhl.ts_publication
            AND dhl_asp_previous.consultant_type = 'ASP'
    WHERE 
        dhl.consultant_type = 'CIQ_FULL'
        AND dhl.ts_publication > DATE('2023-07-15')
        AND dhl.country_code IN ('BR', 'Undefined')
...
messages_raw AS (
    ...
    FROM 
        datalake_sirena.messages mes
    LEFT JOIN 
        datalake_sirena_clean.prospects sp
            ON sp.id = mes.id_prospect
    WHERE 
        template IN ('ab20b849-89d4-4d96-879c-cb12ad97d4d8',
...
messages AS (
    ...
    FROM 
        messages_raw AS mes
    INNER JOIN 
        datalake_sirena_clean.agents AS a
    ...
    INNER JOIN 
        dw_rent.dim_house_listing AS dhl
            ON fl.sk_house_listing = dhl.sk_house_listing
            AND mes.ts_created > dhl.ts_listing_version_start
            AND (mes.ts_created < dhl.ts_listing_version_end OR dhl.is_last_version)
            AND dhl.version > 0
    LEFT JOIN 
        datalake_offboarding.contract_termination ct
            ON ct.id_house_listing = dhl.sk_house_listing - 1
            AND dhl.is_early_demand
            AND ct.status <> 'CANCELED'
    WHERE 
        COALESCE(ct.ts_analyst_annulment_input, dhl.ts_publication) > DATEADD(DAY, -30, DATE_TRUNC('MM', CURRENT_DATE()))
...
predicted_price AS (
    ...
    FROM 
        datalake_ebdb_clean.house_predicted_price dpr
    LEFT JOIN 
        datalake_ebdb_clean.house_predicted_price_aud dpr_aud
...
house_price AS (
    ...
    FROM 
        datalake_ebdb_smart_price.rental_price_history
    WHERE 
...
exclusivity AS (
    ...
    INNER JOIN 
        datalake_ebdb_clean.special_condition_aud AS aud
...
revision AS (
    SELECT
        ...
        m.min_ts_created,
        pp.p_90,
        MAX_BY(ec.key_location, ec.rev) FILTER (
            WHERE ec.ts_revision < m.min_ts_created
        ) AS ef_rev_before,
        MIN_BY(ec.ts_revision, ec.rev) FILTER (
            WHERE ec.ts_revision BETWEEN m.min_ts_created AND DATEADD(DAY, 21, m.min_ts_created)
            AND ec.key_location IN ('KeysWithAgent', 'LockBox', 'FrontDoor', 'Password', 'KeysLocker')
        ) AS ef_rev_after,
        MAX_BY(hp.rent, hp.rev) FILTER (
            WHERE hp.ts_revision < m.min_ts_created
        ) AS pc_rev_before,
        MIN_BY(hp.ts_revision, hp.rev) FILTER (
            WHERE hp.ts_revision BETWEEN m.min_ts_created AND DATEADD(DAY, 21, m.min_ts_created)
            AND hp.rent <= pp.p_90
        ) AS pc_rev_after,
        MAX_BY(exc.is_exclusive, exc.rev) FILTER (
            WHERE exc.ts_revision < m.min_ts_created
        ) AS ex_rev_before,
        MIN_BY(exc.ts_revision, exc.rev) FILTER (
            WHERE exc.ts_revision BETWEEN m.min_ts_created AND DATEADD(DAY, 21, m.min_ts_created)
            AND exc.is_exclusive
        ) AS ex_rev_after
    FROM 
        messages AS m
    LEFT JOIN 
        entry_condition AS ec
            ON ec.id_house = m.id_house
    LEFT JOIN 
        predicted_price AS pp
            ON m.id_house = pp.id_house
            AND m.min_ts_created BETWEEN ts_start AND ts_end
    LEFT JOIN 
        house_price AS hp
            ON hp.id_house = m.id_house
    LEFT JOIN 
        exclusivity AS exc
            ON exc.id_house = m.id_house
    GROUP BY 
        1, 2, 3, 4, 5
),
base AS (
    ...
    LEFT JOIN 
        alocacao_imoveis_ciq_atuacao_asp AS asp_in_ciq_full
