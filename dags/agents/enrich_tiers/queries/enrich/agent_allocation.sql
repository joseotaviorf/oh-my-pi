...
agent_allocation AS (
    SELECT 
        ...
        mha.hub_name,
        mha.profile,
        ROW_NUMBER() OVER(PARTITION BY mp.id, u.id_main_user ORDER BY IF(mha.id_parent_user IS NOT NULL, 1, 0) DESC, mha.dt_reference DESC) = 1 AS is_last_update,
...
SELECT 
    al.id_metric_period,
    al.id_member_profile,
    al.id_user,
    al.id_agent,
    al.uuid_person,
    al.id_parent_user,
    al.id_parent_agent,
    al.uuid_parent_person,
    al.id_business_unit,
    al.hub_name,
    al.profile,
    al.dt_reference,
    ...
    al.day
FROM
    agent_allocation AS al
WHERE
    al.is_last_update IS TRUE