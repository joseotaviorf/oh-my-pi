...
paralel_relations_windows AS (
    SELECT
        id,
        ts_status_started,
        ts_status_ended,
        LAG(ts_status_ended) OVER(PARTITION BY id_paralel_relations ORDER BY ts_status_started, id) AS prev_ts_status_ended,
        LEAD(ts_status_started) OVER(PARTITION BY id_paralel_relations ORDER BY ts_status_started, id) AS next_ts_status_started
    FROM
...
paralel_relations_status_fix AS (
    SELECT
        id,
        CASE
            WHEN ts_status_started < prev_ts_status_ended THEN ts_status_started + INTERVAL '1' SECOND
            ELSE ts_status_started
        END AS ts_status_started_revised,
        CASE
            WHEN next_ts_status_started < COALESCE(ts_status_ended, NOW()) THEN next_ts_status_started - INTERVAL '1' SECOND
            ELSE ts_status_ended
        END AS ts_status_ended_revised
    FROM
        paralel_relations_windows
    WHERE
        ts_status_started < prev_ts_status_ended
        OR next_ts_status_started < COALESCE(ts_status_ended, NOW())
),
preferred_fixed_agent_history_rows AS (
    SELECT
...
preferred_fixed_agent_history_ranked AS (
    SELECT
        id_snapshot,
        id,
        id_user_visit_preferences,
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY ts_status_started, COALESCE(ts_status_ended, NOW()) DESC
        ) AS rn
    FROM
        preferred_fixed_agent_history_rows
)
SELECT
    id_snapshot,
    id,
    ...
    ts_status_ended
FROM
    preferred_fixed_agent_history_ranked
WHERE
    rn = 1
