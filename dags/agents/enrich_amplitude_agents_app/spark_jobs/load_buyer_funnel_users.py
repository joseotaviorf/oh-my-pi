...
def get_agents_search_events_state(dt_snapshot: date):
  ...
  df = (
    ...
    .where((F.col("year") == dt_snapshot.year) & (F.col("month") == dt_snapshot.month) & (F.col("day") == dt_snapshot.day))
  ...
  df = (
    ...
    .agg(
      ...
      F.min(F.when(F.col("id_user").isNotNull(), F.col("ts_event"))).alias("ts_first_logged_activity"),
      F.min(F.when(F.col("business_context") == "sale", F.col("ts_event"))).alias("ts_first_sale_activity"),
      F.min("ts_event").alias("ts_first_activity"),
      F.max("ts_event").alias("ts_latest_activity"),
      F.max(F.when(F.col("business_context") == "sale", F.col("ts_event"))).alias("ts_latest_sale_activity"),
    )
...
def get_minimum_timestamp(column_a, column_b):
  ...
  """
  return F.when(column_a.isNull(), column_b) \
...
def compute_new_state(df_current_state, df_previous_state):
    ...
    """
    joined = (
      df_current_state.alias("cs")
      .join(
        df_previous_state.alias("ps"), 
        on="id_amplitude", 
        how="left"
      )
    )
    ...
    df_new_state = (
      joined.select(
        ...
        get_minimum_timestamp(F.col("ps.ts_first_logged_activity"), F.col("cs.ts_first_logged_activity")).alias("ts_first_logged_activity"),
        get_minimum_timestamp(F.col("ps.ts_first_sale_activity"), F.col("cs.ts_first_sale_activity")).alias("ts_first_sale_activity"),
        get_minimum_timestamp(F.col("ps.ts_first_activity"), F.col("cs.ts_first_activity")).alias("ts_first_activity"),
        get_maximum_timestamp(F.col("ps.ts_latest_activity"), F.col("cs.ts_latest_activity")).alias("ts_latest_activity"),
        get_maximum_timestamp(F.col("ps.ts_latest_sale_activity"), F.col("cs.ts_latest_sale_activity")).alias("ts_latest_sale_activity"),
...
if __name__ == "__main__":
  ...
  db_info = DatalakeMetastoreService.get_db_info(environment, database_name, datalake_bucket)
  ...
  for dt_snapshot in dates_by_process:
    ...
    df_previous_state = get_previous_state_for_ids(df_state, full_table_name, dt_snapshot)
