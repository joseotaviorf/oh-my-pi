...
all_listings AS (
	...
	LEFT JOIN
		fact_listing_rent_flows AS rf
			...
			AND (rf.sk_contract_signed_date > 0 OR rf.sk_contract_created_date > 0)
	LEFT JOIN
		dim_house_listing AS dhl -- bring information about the listing
			ON rf.sk_house_listing = dhl.sk_house_listing
	LEFT JOIN
		dim_region AS dr -- bring information from city_group
			ON rf.sk_region = dr.sk_region
	LEFT JOIN
		datalake_ebdb_raw_prod.contratofull AS cfull -- bring administration fee
...
all_ended_re_rentals AS (
	...
	FROM all_next_contracts AS anc
	WHERE
