WITH listing_rent_flows AS (
    ...
    rent_flows_base AS (
        SELECT
            ...
            COALESCE(contract.id, -1) AS sk_contract,
            ...
            house_listing.ts_listing_version_start AS dt_house_listing,
            ...
            DATE_TRUNC('DAY', proposal.ts_tenant_first_doc_sent) AS dt_tenant_manual_first_doc_sent,
            proposal.ts_tenant_auto_first_doc_sent AS dt_tenant_auto_first_doc_sent,
            DATE_TRUNC('DAY', proposal.ts_tenant_first_doc_sent) AS dt_tenant_first_doc_sent,
            ...
            COALESCE(CAST(DATE_FORMAT(house_listing.ts_listing_version_start, "yyyyMMdd") AS BIGINT), -1) AS sk_house_listing_date,
            COALESCE(CAST(DATE_FORMAT(house_listing.ts_last_unpublished, "yyyyMMdd") AS BIGINT), -1) AS sk_house_listing_de_publication_date,
            COALESCE(
                CAST(
                    DATE_FORMAT(
        ...
        */
            datalake_ebdb_contract.contract
                ON contract.id = COALESCE(house_listing.id_contract, -1)
        LEFT JOIN 
