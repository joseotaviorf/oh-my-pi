...
rent_price_changes_clean AS (
    SELECT
        ...
        ts_revision
    FROM
        rent_price_changes_raw
    QUALIFY
        lag_price IS DISTINCT FROM rent_price OR lag_price IS NULL
),
sale_price_changes_clean AS (
    SELECT
        ...
        ts_revision
    FROM
        sale_price_changes_raw
    QUALIFY
        lag_price IS DISTINCT FROM sale_price OR lag_price IS NULL
),
...
sale_price_changes_enriched AS (
    SELECT
        ...
        lag_price,
        CASE
            WHEN sale_price - lag_price < 0 THEN 'PRICE_DECREASE'
            WHEN sale_price - lag_price > 0 THEN 'PRICE_INCREASE'
            ELSE 'FIRST_PRICE'
        END AS change_type,
        (sale_price - lag_price)/NULLIF(lag_price, 0) AS last_price_variation,
        (sale_price - MIN(IF(lag_price IS NULL, sale_price, NULL)) OVER (PARTITION BY id_house))/NULLIF(MIN(IF(lag_price IS NULL, sale_price, NULL)) OVER (PARTITION BY id_house), 0) AS first_price_variation,
        IF(lag_price IS NULL, TRUE, FALSE) AS is_first_price,
        IF(ROW_NUMBER() OVER (PARTITION BY id_house, dt_change ORDER BY ts_revision DESC) = 1, TRUE, FALSE) AS is_last_price_of_day,
        ts_revision AS ts_price_started,
        LEAD(ts_revision) OVER (PARTITION BY id_house ORDER BY ts_revision) AS ts_price_ended
...
LEFT JOIN
    rent_status_version_order AS rsvo
        ON rsvo.id_house = pc.id_house
        AND pc.ts_price_started BETWEEN rsvo.ts_listing_version_start AND COALESCE(rsvo.ts_listing_version_end, TO_TIMESTAMP(CURRENT_DATE))
...
LEFT JOIN
    sale_status_version_order AS ssvo
        ON ssvo.id_house = pc.id_house
        AND pc.ts_price_started BETWEEN ssvo.ts_listing_version_start AND COALESCE(ssvo.ts_listing_version_end, TO_TIMESTAMP(CURRENT_DATE))
