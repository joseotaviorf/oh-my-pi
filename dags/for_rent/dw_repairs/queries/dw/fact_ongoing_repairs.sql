WITH repair_request_chat_ranked AS (
  SELECT
    id_repair_request AS sk_repair_request,
    ts_started,
    ROW_NUMBER() OVER (PARTITION BY id_repair_request ORDER BY ts_started ASC) AS rn
  FROM
    datalake_repairs_clean.repair_request_chat
),
repair_request_chat AS (
  SELECT
    sk_repair_request,
    ts_started
  FROM
    repair_request_chat_ranked
  WHERE
    rn = 1
),
repair_request_budget_ranked AS (
  SELECT
    id_repair_request AS sk_repair_request,
    id_budget_sender,
    ROW_NUMBER() OVER (PARTITION BY id_repair_request ORDER BY ts_updated DESC) AS rn
  FROM
...
status_fup_ranked AS (
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY rrtnf.id_repair_request ORDER BY rrtnf.ts_updated ASC) AS rn
  FROM
...
tickets_whatsapp_ranked AS (
  SELECT
    tc.id_ticket,
    REPLACE(CAST(custom_fields['Ticket do contato'] AS STRING),'#','') AS id_contact_ticket,
    ROW_NUMBER() OVER(
      PARTITION BY REPLACE(CAST(custom_fields['Ticket do contato'] AS STRING),'#','')
      ORDER BY tc.ts_created DESC
    ) AS rn
  FROM
...
tickets_whatsapp AS (
  SELECT
    ...
    id_contact_ticket
  FROM
    tickets_whatsapp_ranked
  WHERE
    rn = 1
),
fact_ongoing_repairs_ranked AS (
  SELECT
    ...
    DATEDIFF(DATE(tc.ts_solved), DATE(tc.ts_created)) AS frt,
    DATEDIFF(
      to_timestamp(CAST(GET_JSON_OBJECT(rt.custom_fields, '$["[Data] Data do first reply "]') AS STRING),'dd/MM/yy HH'),
    ...
    IF(tc.ts_created IS NOT NULL AND tc.ts_solved IS NULL , TRUE, FALSE) AS is_ongoing,
    CASE
    ...
    to_timestamp(DATEADD(DAY, ROUND(((tc.reply_time_min_calendar/60)/24)),tc.ts_created - INTERVAL 3 HOUR)) AS ts_first_public_comment,
    te.ts_reflux,
    te.ts_contestation,
