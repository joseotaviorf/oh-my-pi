...
SELECT
  ...
  DATEDIFF(DAY, DATE(tc.ts_created) , DATE(tc.ts_solved) ) AS frt,
  DATEDIFF(DAY, tc.ts_created - INTERVAL 3 HOUR, to_timestamp(CAST(GET_JSON_OBJECT(rt.custom_fields, '$["[Data] Data do first reply "]') AS STRING),'dd/MM/yy HH')) AS days_to_first_reply,
