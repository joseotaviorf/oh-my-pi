...
,ongoing_repair_tickets_ranked AS (
SELECT
  tc.id_ticket,
  ...
  TRY_CAST(GET_JSON_OBJECT(TO_JSON(tc.custom_fields), '$["[Data] Definição do prestador"]') AS DATE) AS dt_definition,
  fi.dt_chat,
...
WHERE
  ...
  AND tc.ts_created >= DATE('{load_start_date}') - INTERVAL 3 YEAR
