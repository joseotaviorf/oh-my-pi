...
aux_ongoing_rentals AS (
    /** We only want to analyze the number of ongoing rentals on the last day of the month if it's a closed month.
        If the current month hasn't finished yet, we want to analyze the number of ongoing rentals for the most
        recent day.
    **/
    SELECT
...
drivers AS (
    ...
    LEFT JOIN
        contract_signed AS cs
            ON cs.dt_month = d.month_start
    LEFT JOIN
        ccv
            ON ccv.dt_month = d.month_start
    LEFT JOIN
        ongoing_rentals ors
            ON ors.dt_month = d.month_start
    LEFT JOIN
        ended_rentals ers
            ON ers.dt_month = d.month_start
    WHERE
        d.sk_date >= 20220101
),
...
SELECT
    ...
    (total_tickets_proportional/driver) AS ticket_rate_monthly
FROM
    total_tickets_prep
