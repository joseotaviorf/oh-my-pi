...
SELECT -- [ODS] This table was migrated from ODS flow and needs a future refactoring to remove castings and renamings
  ...
  COALESCE(hlc.id_previous_contract, LAG(hlc.id_contract) OVER(PARTITION BY hl.id_house ORDER BY hl.id_house_listing), -1) AS sk_previous_contract,
  COALESCE(hlc.id_next_contract, LEAD(hlc.id_contract) OVER(PARTITION BY hl.id_house ORDER BY hl.id_house_listing), -1) AS sk_next_contract,
