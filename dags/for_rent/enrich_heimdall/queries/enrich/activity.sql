SELECT
  ...
  ) AS TIMESTAMP) AS ts_transferred,
  COALESCE(
    CASE
    ...
    END,
    CAST(GET_JSON_OBJECT(dl.metadata, '$.requestedAt.$date') AS TIMESTAMP),
    TO_TIMESTAMP(GET_JSON_OBJECT(dl.metadata, '$.requested_date'), 'yyyy-MM-dd')
  ) AS ts_requested,
  CAST(GET_JSON_OBJECT(FROM_JSON(dl.transition_list, 'ARRAY<STRING>')[0], '$.createdAt.$date') AS TIMESTAMP) AS ts_transition_created,
  dl.ts_created,
  dl.ts_updated
FROM datalake_heimdall_clean.activity AS dl