WITH listing_contracts AS (
  SELECT
    hl.id_house, /* Relating each listing to its respective contract */
    hl.id_house_listing,
    c.id AS id_contract
  ...
  SELECT
    ...
    c.id AS id_contract,
    COALESCE(
      LAG(c.id) OVER (PARTITION BY lc.id_house ORDER BY lc.id_house_listing, c.ts_created, t.ts_created),
      NULL
    ) AS id_previous_contract,
    lc.id_house,
    hl.country_code,
    ...
    IF(
      c.status <> 'Finalizado'
      AND NOT t.status IS NULL
      AND NOT t.status IN ('CANCELED', 'DONE'),
      TRUE,
      FALSE
    ) AS is_on_termination,
  ...
  JOIN listing_contracts AS lc
    ON lc.id_contract = c.id
  JOIN datalake_ebdb_listing.house_listing AS hl
  ...
  FROM datalake_ebdb_listing.house_listing AS hl
  LEFT JOIN latest_contract AS lc
...
SELECT
  ...
  CAST((
    CAST(hl.ts_listing_version_end AS BIGINT) - CAST(CAST(hlc.dt_contract_annulment AS TIMESTAMP) AS BIGINT)
  ) / (
    86400
  ) AS INT) AS days_ended_rental_to_relisting,
  CAST((
    CAST(hlc.ts_next_contract_signed AS BIGINT) - CAST(hl.ts_listing_version_end AS BIGINT)
  ) / (
    86400
  ) AS INT) AS days_relisting_to_re_rental,
  CAST((
    CAST(hlc.ts_next_contract_signed AS BIGINT) - CAST(CAST(hlc.dt_contract_annulment AS TIMESTAMP) AS BIGINT)
  ) / (
    86400
  ) AS INT) AS days_ended_rental_to_re_rented,
  cb.is_early_relisting,
  cb.is_early_demand,
  cb.is_extended_rental,
  COALESCE(
    LAG(cb.is_on_termination) OVER (PARTITION BY cb.id_house ORDER BY cb.id_house_listing, cb.ts_contract_created, cb.ts_termination_created),
    NULL
  ) AS is_previous_contract_on_termination,
  ...
  cb.dt_termination,
  IF(
    cb.previous_contract_status IN ('Ativo', 'Finalizado'),
