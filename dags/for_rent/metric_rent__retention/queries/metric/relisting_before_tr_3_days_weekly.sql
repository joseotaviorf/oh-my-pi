...
ended_rentals_requested AS (
    SELECT
    	...
    	dc.sk_contract,
      dc.country_code,
    	dt.ts_created AS termination_request_date,
    	dt.dt_termination AS termination_date,
    	dc.value_segment,
    	DATE(COALESCE(dc.ts_analyst_annulment_input,dc.dt_annulment)) AS dt_ended_rental_confirmed,
    	ROW_NUMBER() OVER(PARTITION BY fct_or.sk_house_listing, DATE_TRUNC('MONTH',DATE(dt.ts_created)) ORDER BY DATE(dt.ts_created) DESC) AS rn
    FROM
    ...
    JOIN
      dw_retention.fact_contract_termination AS fct
        ON fct_or.sk_contract = fct.sk_contract
    JOIN
    ...
    WHERE
        ...
        AND DATE(dt.ts_created) >= DATE('2022-01-01')
...
listing_base AS (
  ...
  WHERE
    ...
    AND DATE_TRUNC('WEEK', dhl_2.ts_publication) <= DATE_TRUNC('WEEK', CURRENT_DATE)
)
SELECT 
  DATE_TRUNC('WEEK',dt_publication_nxt) AS dt_relisting_week,
  value_segment AS category,
  SUM(IF(leadtime_tr_rl <= 3, 1, 0)) AS tr_with_rl_3_days,
  SUM(IF(leadtime_tr_rl <= 3 AND dt_publication_nxt <= DATE_ADD(CURRENT_DATE(), -3), 1, 0)) AS tr_with_rl_matured_3_days,
  COUNT(DISTINCT nxt_sk_house_listing) AS relistings,
  COUNT(DISTINCT IF(dt_publication_nxt <= DATE_ADD(CURRENT_DATE(), -3), nxt_sk_house_listing, NULL)) AS relistings_matured_3_days,
  (CAST(SUM(IF(leadtime_tr_rl <= 3, 1, 0)) AS DOUBLE) / COUNT(DISTINCT nxt_sk_house_listing)) * 100.00 AS pct_tr_with_rl_3_days,
  (CAST(SUM(IF(leadtime_tr_rl <= 3 AND dt_publication_nxt <= DATE_ADD(CURRENT_DATE(), -3), 1, 0)) AS DOUBLE) 
    / COUNT(DISTINCT IF(dt_publication_nxt <= DATE_ADD(CURRENT_DATE(), -3), nxt_sk_house_listing, NULL))) * 100.00 AS pct_tr_with_rl_matured_3_days
FROM
  listing_base
GROUP BY
  1 ,2
