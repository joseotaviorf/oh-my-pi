...
metric_calculations AS (
  SELECT
    CAST(DATE_TRUNC('WEEK',erc.dt_ended_rental_confirmed) AS DATE) AS dt_reference_week,
    erc.country_code,
...
metric_calculations_maturation_4W AS (
  SELECT
    CAST(DATE_TRUNC('WEEK', DATE_ADD(erc.dt_ended_rental_confirmed, 28)) AS DATE) AS dt_reference_week,
    erc.country_code,
    erc.value_segment AS category,
    erc.rental_administrator,
    COUNT(DISTINCT
...
metric_calculations_maturation_12W AS (
  SELECT
    CAST(DATE_TRUNC('WEEK', DATE_ADD(erc.dt_ended_rental_confirmed, 84)) AS DATE) AS dt_reference_week,
    erc.country_code,
    erc.value_segment AS category,
    erc.rental_administrator,
    COUNT(DISTINCT
...
all_calculations AS (
  ...
  JOIN
    metric_calculations_maturation_4W AS mc4w
      ON mc4w.dt_reference_week = mc.dt_reference_week
  ...
  JOIN
    metric_calculations_maturation_12W AS mc12w
      ON mc12w.dt_reference_week = mc.dt_reference_week
