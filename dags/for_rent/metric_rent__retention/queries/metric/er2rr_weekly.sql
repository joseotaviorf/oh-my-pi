...
metric_calculations AS (
  SELECT
    CAST(DATE_TRUNC('WEEK',db.dt_ended_rental_confirmed) AS DATE) AS dt_reference_week,
    db.country_code,
    db.category,
...
metric_calculations_maturation_4W AS (
  SELECT
    CAST(DATE_TRUNC('WEEK', DATE_ADD(db.dt_ended_rental_confirmed, 28)) AS DATE) AS dt_reference_week,
    db.country_code,
    db.category,
    db.rental_administrator,
    COUNT(DISTINCT
...
metric_calculations_maturation_12W AS (
  SELECT
    CAST(DATE_TRUNC('WEEK', DATE_ADD(db.dt_ended_rental_confirmed, 84)) AS DATE) AS dt_reference_week,
    db.country_code,
    db.category,
    db.rental_administrator,
    COUNT(DISTINCT
