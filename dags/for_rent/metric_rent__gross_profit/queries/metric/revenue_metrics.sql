...
metrics_aux AS (
  SELECT
    ...
    SUM(addons_guarantee) AS addons_guarantee,
    ...
    SUM(addons_new_business_revenue) AS addons_new_business_revenue,
    SUM(addons_revenue_total) AS addons_revenue,
    SUM(agents_commission) AS agents_commission,
    SUM(affiliates_commission) AS affiliates_commission,
    SUM(revenue_share_total) AS revenue_share_total,
    SUM(gross_revenue) AS gross_revenue,
    SUM(net_revenue_pre_taxes) AS net_revenue_pre_taxes
  FROM
    dw_rental_contribution_margin.fact_house_listing_revenues
  WHERE
    country_code IN ('BR', 'Undefined')
