...
owner_version AS (
  SELECT
    id_owner,
    ROW_NUMBER() OVER (PARTITION BY id_owner ORDER BY year, month, day) AS owner_version,
    category,
    is_amateur,
    is_pp_multi_active,
    year, 
    month, 
    day,
    LEAD(dt_houses_owned) OVER (PARTITION BY id_owner ORDER BY year, month, day) AS dt_owner_version_ended
  FROM
...
SELECT
  ohc.id_owner || 0 || COALESCE(ov.owner_version, 1) AS id_owner_version,
  ohc.id_owner,
  ohc.category,
  COALESCE(ov.owner_version, 1) AS owner_version,
  ohc.is_amateur,
...
LEFT JOIN
  owner_version AS ov
    ...
    AND MAKE_DATE(ohc.year, ohc.month, ohc.day) < COALESCE(dt_owner_version_ended, CURRENT_DATE())
