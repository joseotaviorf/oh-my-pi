WITH rent_modifications AS (
	SELECT
	    ...
	    DATE(TIMESTAMP(ure.ts_revision/1000)) AS modification,
	...
	WHERE (mod_rent = True
	    OR DATE(TIMESTAMP(ure.ts_revision/1000)) = DATE(ha.dt_first_publication)
	    OR DATE(TIMESTAMP(ure.ts_revision/1000)) = DATE(dhl.ts_publication))
 ),
...
listings_info AS (
    SELECT
        hi.id_house,
        hi.sk_house_listing,
        hi.dt_first_publication,
        hi.ts_publication,
        hi.ts_house_first_publication,
        hi.first_rent,
        hi.last_rent,
        hi.last_rent_dim,
        hi.dt_last_rent,
        hi.dt_first_rent,
        hi.house_predicted_price,
        hi.house_total_area,
        hi.house_type,
        hi.is_house_furnished,
        DATE_TRUNC('month', hi.ts_publication) AS month_publication,
        hi.first_rent/hi.house_total_area AS first_price_m2,
        hi.last_rent/hi.house_total_area AS last_price_m2,
        hi.house_predicted_price/hi.house_total_area AS calculator_price_m2,
        reg.sk_region,
        reg.macro_name,
        reg.city_name,
        ltr.days_house_listing_to_contract_signed,
        COUNT(DISTINCT CASE WHEN DATEDIFF(dem.dt_event, hi.ts_publication) <= 7 THEN dem.sk_rf END) AS rent_flows_em_7_dias,
        COUNT(DISTINCT CASE WHEN DATEDIFF(dem.dt_event, hi.ts_publication) <= 7 THEN dem.sk_booking END) AS visits_booked_in_7_days,
        COUNT(DISTINCT dem.sk_contract) AS contracts_signed,
        AVG(dc.rent) AS avg_rent_contracts_signed,
        AVG(dc.rent)/hi.house_total_area AS avg_rent_contracts_signed_m2
	FROM
	    houses_infos hi
	LEFT JOIN
...
contract_base AS (
    SELECT
        c.sk_contract,
        f.sk_house_listing,
        f.sk_house_listing/1000 AS sk_house,
        f.sk_region,
        dr.city_name,
        dr.macro_name,
        c.rent,
...
contracts_full_match_avg AS (
    SELECT
        ...
        house_type,
        is_house_furnished,
        avg_contracts_price_m2,
        amount_contracts,
        'region_type_furniture' AS match_type,
...
contracts_region_furn_match_avg AS (
    SELECT
        ...
        avg_contracts_price_m2,
        amount_contracts,
        'region_furniture' AS match_type,
...
first_predicted_price AS (
    ...
    FROM
	 datalake_ebdb_clean.house_predicted_price_aud
    WHERE rev_type = 0
...
last_predicted_price AS (
    ...
    FROM
	 datalake_ebdb_clean.house_predicted_price
    WHERE business_context = 'RENT'
),
listing_pricing_flows AS (
    SELECT
        ...
        CASE
          WHEN listings_info.dt_last_rent = listings_info.dt_first_rent AND listings_info.first_rent = listings_info.last_rent THEN NULL
          ELSE DATEDIFF(listings_info.dt_last_rent, listings_info.ts_publication)
        ...
        (listings_info.first_price_m2-COALESCE(COALESCE(full_match.price_3mon, reg_furn_match.price_3mon), city_furn_match.price_3mon))*1.0/COALESCE(COALESCE(full_match.price_3mon, reg_furn_match.price_3mon), city_furn_match.price_3mon) AS diff_first_listing_contracts,
        ...
        CASE
          WHEN listings_info.dt_last_rent = listings_info.dt_first_rent AND listings_info.first_rent = listings_info.last_rent THEN NULL
          ELSE (listings_info.last_price_m2-COALESCE(COALESCE(full_match.price_3mon, reg_furn_match.price_3mon), city_furn_match.price_3mon))*1.0/COALESCE(COALESCE(full_match.price_3mon, reg_furn_match.price_3mon), city_furn_match.price_3mon)
...
SELECT
    ...
    dt_first_rent,
    dt_last_rent,
    days_publication_to_last_price_change,
    calculator_price,
    house_total_area,
    house_type,
    is_house_furnished,
    ...
    city_group,
    region_name,
    rent_flows_in_7_days,
    visits_booked_in_7_days,
    listing_contracts_signed,
    avg_rent_listing_contracts_signed,
    avg_rent_listing_contracts_signed_m2,
    days_house_listing_to_contract_signed,
    avg_price_contracts_signed_m2,
    total_contracts_compared,
    match_type,
    diff_first_listing_contracts,
    first_prediction_p_10,
    first_prediction_p_20,
    first_prediction_p_30,
    first_prediction_p_40,
    first_prediction_p_50,
    first_prediction_p_70,
    first_prediction_p_80,
    first_prediction_p_90,
    first_prediction_certainty,
    last_prediction_p_10,
    last_prediction_p_20,
    last_prediction_p_30,
    last_prediction_p_40,
    last_prediction_p_50,
    last_prediction_p_60,
    last_prediction_p_70,
    last_prediction_p_80,
    last_prediction_p_90,
    last_prediction_certainty,
    diff_last_listing_contracts,
    diff_first_listing_calculator,
    diff_last_listing_calculator,
    CASE
