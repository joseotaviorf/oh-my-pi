...
WITH visits AS (
  ...
  WHERE
    visit_intent = 'SALE' AND NOT ts_created IS NULL
),
offers AS (
  ...
  WHERE
    NOT ts_offer_submitted IS NULL
),
fact_listing_status AS (
  ...
  SELECT
    ...
    CASE
      WHEN h.sale_price IS NULL OR h.sale_price < 150000
      THEN NULL
      WHEN h.sale_price < 300000
      THEN '150-300k'
      WHEN h.sale_price < 500000
      THEN '300-500k'
      WHEN h.sale_price < 900000
      THEN '500-900k'
      WHEN h.sale_price < 1500000
      THEN '900-1500k'
      WHEN h.sale_price >= 1500000
      THEN '1500k+'
    END AS bedrooms_price_bins,
    CASE
      WHEN r.city_group = 'RMSP'
      THEN 'RMSP'
      WHEN r.city_group = 'Rio de Janeiro'
      THEN 'Rio de Janeiro'
      WHEN r.city_group = 'Porto Alegre'
      THEN 'Porto Alegre'
      ELSE 'Other City Group'
    END AS city_group,
    l.ts_status_started
  FROM datalake_sale_listings.sale_listing_status AS l
  LEFT JOIN datalake_ebdb_listing.house AS h
    ON BIGINT(LEFT(l.id_sale_listing, 9)) = h.id
  LEFT JOIN datalake_region.region AS r
    ON l.id_region = r.id AND r.level IN ('SubRegiao', 'Cidade')
  WHERE
    l.status_history = 'PUBLISHED'
),
ongoing_listings_ranked AS (
  /*
...
*/
  SELECT
    ...
    f.city_group,
    LAST_VALUE(v.id_booking_created_date) IGNORE NULLS OVER (PARTITION BY f.id_house ORDER BY date) AS lag_visits,
    LAST_VALUE(o.id_offer_submitted_date) IGNORE NULLS OVER (PARTITION BY f.id_house ORDER BY date) AS lag_offers,
    ...
    d.date,
    d.month_start,
    ROW_NUMBER() OVER (PARTITION BY f.id_sale_listing, d.date ORDER BY f.ts_status_started DESC) AS rn
  FROM fact_listing_status AS f
  JOIN datalake_quintoandar.aux_date AS d
    ON d.id_date BETWEEN NULLIF(f.id_status_start_date, -1) AND COALESCE(
      NULLIF(f.id_status_end_date, -1),
      CAST(REPLACE(CAST(CURRENT_DATE AS STRING), '-', '') AS BIGINT) - 1
    )
    AND d.date <= DATE_ADD(CURRENT_DATE, 1 * -1)
  LEFT JOIN datalake_ebdb_listing.house AS h
    ON BIGINT(LEFT(f.id_sale_listing, 9)) = h.id
  LEFT JOIN visits AS v
    ON h.id = v.id_house AND d.id_date = v.id_booking_created_date
  LEFT JOIN offers AS o
    ON h.id = o.id_house AND d.id_date = o.id_offer_submitted_date
),
ongoing_listings AS (
  /*
With the visits and offers, we will do the ongoing listings to understand the amount of days in publication each listing has.
...
*/
  SELECT
    ...
    date,
    month_start
  FROM ongoing_listings_ranked
  WHERE
    rn = 1
),
stranded_logic_with_threshold AS (
  /*
...
*/
  SELECT
    ...
    CASE
      WHEN ol.first_visit IS NULL AND ol.ongoing_days_published >= t.threshold_first_visit
      THEN TRUE
      WHEN NOT ol.first_visit IS NULL
      AND ol.id_date < ol.first_visit
      AND ol.ongoing_days_published >= t.threshold_first_visit
      THEN TRUE
      ELSE FALSE
    END AS visit_stranded_check,
    CASE
      WHEN ol.first_offer IS NULL AND ol.ongoing_days_published >= t.threshold_first_offer
      THEN TRUE
      WHEN NOT ol.first_offer IS NULL
      AND ol.id_date < ol.first_offer
      AND ol.ongoing_days_published >= t.threshold_first_offer
      THEN TRUE
      ELSE FALSE
    END AS offer_stranded_check,
    CASE
      WHEN NOT ol.first_visit IS NULL
      AND DATEDIFF(TO_DATE(date), TO_DATE(CAST(ol.lag_visits AS STRING), 'yyyyMMdd')) >= t.threshold_recurrence_visit
      THEN TRUE
      ELSE FALSE
    END AS visit_stranded_recurrence_check,
    CASE
      WHEN NOT ol.first_offer IS NULL
      AND DATEDIFF(TO_DATE(date), TO_DATE(CAST(ol.lag_offers AS STRING), 'yyyyMMdd')) >= t.threshold_recurrence_offer
      THEN TRUE
      ELSE FALSE
    END AS offer_stranded_recurrence_check,
    MAX(t.month_start) OVER (PARTITION BY 1) AS max_month_start,
    ol.month_start
  FROM ongoing_listings AS ol
  LEFT JOIN datalake_sale_stranded_listings.thresholds AS t
...
*/
  SELECT
    ...
    id_date,
    ongoing_days_published,
    visit_stranded_check,
    offer_stranded_check,
    visit_stranded_recurrence_check,
    offer_stranded_recurrence_check
  FROM stranded_logic_with_threshold
  WHERE
...
*/
  SELECT
    ...
    id_date,
    CASE
      WHEN offer_stranded_recurrence_check
      THEN 'RECURRENCE_OFFER_STRANDED'
      WHEN offer_stranded_check
      THEN 'OFFER_STRANDED'
      WHEN visit_stranded_recurrence_check AND NOT (
        offer_stranded_recurrence_check
      )
      THEN 'RECURRENCE_VISIT_STRANDED'
      WHEN visit_stranded_check AND NOT (
        offer_stranded_check
      )
      THEN 'VISIT_STRANDED'
      ELSE 'HEALTH_LISTING'
...
*/
  SELECT
    ...
    LEAD(stranded_status) OVER (PARTITION BY id_sale_listing ORDER BY id_date DESC) AS next_stranded_status,
    ROW_NUMBER() OVER (PARTITION BY id_sale_listing ORDER BY id_date ASC) AS rn
  FROM apply_hierarchy
),
status_changes AS (
  /*
CTE that will help create the status change columns for each listing
*/
  SELECT
    ...
    days_published,
    is_offer_and_visit_stranded
  FROM status_changes_ranked
  WHERE
    next_stranded_status <> stranded_status OR rn = 1
),
aux AS (
  SELECT
    id_sale_listing,
