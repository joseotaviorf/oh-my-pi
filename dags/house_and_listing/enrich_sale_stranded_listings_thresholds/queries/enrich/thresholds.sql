...
fact_listing_status AS (
  ...
  SELECT
    ...
    ts_status_started,
    status_history
  FROM datalake_sale_listings.sale_listing_status
),
...
*/
  ...
  JOIN datalake_quintoandar.aux_date AS d
    ON d.id_date BETWEEN NULLIF(f.sk_status_start_date, -1) AND COALESCE(
      NULLIF(f.sk_status_end_date, -1),
  ...
  LEFT JOIN visits AS v
    ON h.id = v.id_house AND d.id_date = v.sk_booking_created_date
  LEFT JOIN offers AS o
    ON h.id = o.id_house AND d.id_date = o.sk_offer_submitted_date
  LEFT JOIN datalake_region.region AS r
    ON f.id_region = r.id AND r.level IN ('SubRegiao', 'Cidade')
  WHERE
    1 = 1 AND f.status_history = 'PUBLISHED'
),
ongoing_listings AS (
  /*
With the visits and offers, we will do the ongoing listings to understand the amount of days in publication each listing has.
...
*/
  SELECT
    ...
    days_published,
    ongoing_days_published,
    CASE WHEN id_date = first_visit THEN ongoing_days_published ELSE NULL END AS days_published_to_first_visit,
    CASE WHEN id_date = first_offer THEN ongoing_days_published ELSE NULL END AS days_published_to_first_offer,
    month_start
...
*/
  ...
  FROM days_published_ongoing_listings_check
  GROUP BY
    1,
    2,
    3,
...
*/
  SELECT
    ...
    ongoing_days_published,
    IF(
      id_date = visits AND status_history = 'PUBLISHED',
      ROW_NUMBER() OVER (PARTITION BY id_sale_listing, status_history ORDER BY date ASC),
      NULL
    ) AS days_published_to_visits,
    IF(
      id_date = offers AND status_history = 'PUBLISHED',
      ROW_NUMBER() OVER (PARTITION BY id_sale_listing, status_history ORDER BY date ASC),
      NULL
    ) AS days_published_to_offers,
    month_start
  FROM ongoing_listings
),
...
*/
  SELECT
    ...
    LAST(days_published_to_visits) IGNORE NULLS OVER (PARTITION BY id_sale_listing ORDER BY id_date) AS days_published_to_visits,
    LAST(days_published_to_offers) IGNORE NULLS OVER (PARTITION BY id_sale_listing ORDER BY id_date) AS days_published_to_offers
  FROM days_published_ongoing_listings_for_recurrence_check
),
...
*/
  SELECT
    ...
    check,
    NULLIF(
      days_published_to_visits - LAG(days_published_to_visits) OVER (PARTITION BY id_sale_listing ORDER BY days_published_to_visits),
      0
    ) AS diff_days_between_visits,
    NULLIF(
      days_published_to_offers - LAG(days_published_to_offers) OVER (PARTITION BY id_sale_listing ORDER BY days_published_to_offers),
      0
    ) AS diff_days_between_offers
  FROM days_between_events
),
...
*/
  SELECT
    ...
    id_sale_listing,
    ROUND(
      AVG(avg_days_between_visits) OVER (PARTITION BY id_sale_listing ORDER BY month_start ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
    ) AS avg_days_between_visits,
    ROUND(
      AVG(avg_days_between_offers) OVER (PARTITION BY id_sale_listing ORDER BY month_start ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
    ) AS avg_days_between_offers
  FROM recurrence_by_month
),
aux AS (
  ...
  WHERE
    NOT bedrooms_price_bins IS NULL
  GROUP BY
    1,
    2,
    3,
...
SELECT
  ...
  MONTH(TO_DATE(month_start)) AS month,
