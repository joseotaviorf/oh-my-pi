WITH key_location_aud AS (
  ...
  SELECT
    ...
    ts_revision,
    access_type_name,
    MAX(ts_revision) OVER (PARTITION BY id_house, CAST(ts_revision AS DATE)) = ts_revision AS is_last_status_of_day,
    CAST(ts_revision AS DATE) AS dt_key_location_started,
    COALESCE(
      LEAD(CAST(ts_revision AS DATE)) OVER (PARTITION BY id_house ORDER BY ts_revision),
      '2100-01-01'
    ) AS dt_key_location_ended
  ...
  SELECT
    haa.id_house,
    CASE haa.relation_type WHEN 'KEY_HOLDER' THEN TRUE ELSE FALSE END AS is_keys_with_agent_eligible,
    TIMESTAMP_MILLIS(ure.ts_revision) AS ts_revision
  FROM datalake_ebdb_clean.user_revision_entity AS ure
  JOIN datalake_ebdb_clean.house_agent_aud AS haa
    ON haa.rev = ure.id
  WHERE
    CAST(TIMESTAMP_MILLIS(ure.ts_revision) AS DATE) < DATE '2025-11-01'
), new AS (
  SELECT
    heh.id_house,
  ...
  WHERE
    CAST(ts_entrance_started AS DATE) >= '2025-11-01' AND key_location = 'AGENT'
), agent_aud AS (
  SELECT
    id_house,
  ...
  SELECT
    ...
    CASE
      WHEN hakt.status = 'NOT_DELIVERED'
      THEN 'KEY_HOLDER_ALLOCATED'
      WHEN hakt.status = 'DELIVERED'
      THEN 'KEY_HOLDER_VALIDATED'
      WHEN hakt.status = 'RETURNED'
      THEN 'KEY_HOLDER_DEALLOCATED'
      ELSE NULL
    END AS status,
    TIMESTAMP_MILLIS(ure.ts_revision) AS ts_revision,
    ure.ts_revision AS ts_revision_unix
  FROM datalake_ebdb_clean.house_agent_key_tracking_aud AS hakt
  JOIN datalake_ebdb_clean.user_revision_entity AS ure
    ON hakt.rev = ure.id
  WHERE
    CAST(TIMESTAMP_MILLIS(ure.ts_revision) AS DATE) < DATE '2025-06-17'
), new_attrib AS (
  SELECT
  ...
  WHERE
    CAST(heh.ts_entrance_started AS DATE) >= '2025-06-17'
  ...
  SELECT
    ...
    CASE WHEN has_opted_keys_with_agent = TRUE THEN 'OPT_IN' ELSE 'NO_OPT_IN' END AS status,
    ts_revision,
    ts_revision_unix
  FROM key_location_aud
  WHERE
    mod_has_opted_keys_with_agent = TRUE AND NOT has_opted_keys_with_agent IS NULL
  UNION
  SELECT
    id,
    id_agent,
  ...
  SELECT
    ...
    LEAST(
      CASE
        WHEN NOT ts_listing_version_start IS NULL
        THEN ABS(1000 * UNIX_TIMESTAMP(ts_listing_version_start) - ts_revision_unix)
      END,
      CASE
        WHEN NOT ts_listing_version_end IS NULL
        THEN ABS(
          1000 * UNIX_TIMESTAMP(ts_listing_version_end - INTERVAL '7' DAYS) - ts_revision_unix
        )
      END
    ) AS min_diff_listing,
    CASE WHEN RIGHT(dhl.id_house_listing, 3) = '000' THEN TRUE ELSE FALSE END AS is_first_listing,
    ts_listing_version_start,
    ts_revision
  FROM merged_key_status AS mks
  JOIN datalake_ebdb_listing.house_listing AS dhl
    USING (id_house)
), keys_closer_listings /* keep closer listing */ AS (
  SELECT DISTINCT
    FIRST_VALUE(id_agent) IGNORE NULLS OVER (PARTITION BY id_house_listing ORDER BY ts_revision DESC) AS id_agent,
  ...
  SELECT
    ...
    MAX(CASE WHEN status = 'OPT_IN' THEN TRUE WHEN status = 'NO_OPT_IN' THEN FALSE END) AS is_keys_with_agent_opt_in,
  ...
  FROM datalake_ebdb_listing.house_listing AS hl
  JOIN datalake_ebdb_listing.house AS h
    ON hl.id_house = h.id
  LEFT JOIN key_location_changes AS klc
    ON klc.id_house = hl.id_house
    AND (
      (
        klc.dt_key_location_started >= CAST(COALESCE(ts_listing_version_start, '1900-01-01') AS DATE)
        AND klc.dt_key_location_started < CAST(COALESCE(ts_listing_version_end, '2100-01-01') AS DATE)
      )
      OR (
        CAST(COALESCE(ts_listing_version_start, '1900-01-01') AS DATE) >= klc.dt_key_location_started
        AND CAST(COALESCE(ts_listing_version_start, '1900-01-01') AS DATE) < klc.dt_key_location_ended
      )
    )
    AND klc.is_last_status_of_day = TRUE
  LEFT JOIN agent_aud AS aa
...
SELECT DISTINCT
  ...
  ndl.who_is_living,
  CASE
    WHEN NOT ndl.ts_contract_signed IS NULL AND ka.has_keys_with_agent_delivered
    THEN CAST((
      UNIX_TIMESTAMP(ka.ts_returned) - UNIX_TIMESTAMP(ndl.ts_contract_signed)
    ) / 86400 AS DECIMAL(7, 2))
  END AS days_cs_to_return,
  CAST((
    UNIX_TIMESTAMP(ka.ts_attributed) - UNIX_TIMESTAMP(ka.ts_publicated)
  ) / 86400 AS DECIMAL(7, 2)) AS days_publication_to_attribution,
...
FROM non_doorman_listing AS ndl
LEFT JOIN keys_attributions AS ka
  USING (id_house_listing)
LEFT JOIN datalake_ebdb_country.user
