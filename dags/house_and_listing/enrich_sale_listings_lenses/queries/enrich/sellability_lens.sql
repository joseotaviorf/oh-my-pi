WITH region_cluster_groups AS (
  SELECT
    r.id AS id_region,
    CASE
      WHEN NOT c.cluster_name IS NULL AND c.cluster_name <> 'Sem Cluster'
      THEN c.cluster_name
      WHEN r.city_group IN ('RMSP') AND city_name = 'São Paulo'
      THEN 'São Paulo'
      WHEN r.city_group IN ('RMSP') AND city_name <> 'São Paulo'
      THEN 'Grande São Paulo'
      WHEN r.city_group IN ('Rio de Janeiro') AND city_name = 'Rio de Janeiro'
      THEN 'Rio de Janeiro'
      WHEN r.city_group IN ('Rio de Janeiro') AND city_name <> 'Rio de Janeiro'
      THEN 'Grande Rio de Janeiro'
      WHEN r.city_group IN ('Porto Alegre') AND city_name = 'Porto Alegre'
      THEN 'Porto Alegre'
      WHEN r.city_group IN ('Porto Alegre') AND city_name <> 'Porto Alegre'
      THEN 'Grande Porto Alegre'
      WHEN r.city_group IN ('Belo Horizonte') AND city_name = 'Belo Horizonte'
      THEN 'Belo Horizonte'
      WHEN r.city_group IN ('Belo Horizonte') AND city_name <> 'Belo Horizonte'
      THEN 'Grande Belo Horizonte'
      WHEN r.city_group IN ('Campinas') AND city_name = 'Campinas'
      THEN 'Campinas'
      WHEN r.city_group IN ('Campinas') AND city_name <> 'Campinas'
      THEN 'Grande Campinas'
      ELSE city_name
    END AS region_group
  FROM datalake_region.region AS r
  LEFT JOIN datalake_gsheets_clean.for_sale_marketplace_region_clusters AS c
    ON r.id = c.id_region
),
publications AS (
  SELECT
  ...
  FROM datalake_ebdb_clean.listing_business_context AS l
  LEFT JOIN datalake_ebdb_listing.house AS h
    ON l.id_house = h.id
  WHERE
    l.business_context = 'SALE' AND NOT l.ts_first_publication IS NULL
),
ccvs AS (
  SELECT
    id_house,
  ...
  WHERE
    NOT ts_sale_agreement_signed IS NULL AND id_house > 0
),
events AS (
  SELECT
    *
...
events_enriched AS (
  ...
  LEFT JOIN datalake_ebdb_listing.house AS h
    ON h.id = e.id_house
  LEFT JOIN datalake_sale_listings_lenses.segmentation_bins AS p
    ON p.bin = 'price' AND (
      e.price >= p.lower_band AND e.price < p.upper_band
    )
  LEFT JOIN datalake_sale_listings_lenses.segmentation_bins AS pm2
    ON pm2.bin = 'price_m2'
    AND (
      (
        e.price / h.total_area
      ) >= pm2.lower_band
      AND (
        e.price / h.total_area
      ) < pm2.upper_band
    )
  LEFT JOIN datalake_sale_listings_lenses.segmentation_bins AS ta
    ON ta.bin = 'total_area'
    AND (
      h.total_area >= ta.lower_band AND h.total_area < ta.upper_band
    )
  LEFT JOIN region_cluster_groups AS rg
...
timeline AS (
  SELECT
    ...
    total_area_bin,
    price_m2_bin
  FROM datalake_quintoandar.aux_date, all_combinations
  WHERE
    date BETWEEN TO_DATE('2013-01-01') AND CURRENT_DATE
),
filling_timeline_forward_filled AS (
  SELECT
    date,
    region_group,
    house_type,
    ...
    total_area_bin,
    price_m2_bin,
    LAST_VALUE(region_factor) IGNORE NULLS OVER (PARTITION BY region_group, house_type, bedrooms, price_bin, total_area_bin, price_m2_bin ORDER BY date) AS region_factor,
    LAST_VALUE(house_type_factor) IGNORE NULLS OVER (PARTITION BY region_group, house_type, bedrooms, price_bin, total_area_bin, price_m2_bin ORDER BY date) AS house_type_factor,
    LAST_VALUE(bedrooms_factor) IGNORE NULLS OVER (PARTITION BY region_group, house_type, bedrooms, price_bin, total_area_bin, price_m2_bin ORDER BY date) AS bedrooms_factor,
    LAST_VALUE(price_factor) IGNORE NULLS OVER (PARTITION BY region_group, house_type, bedrooms, price_bin, total_area_bin, price_m2_bin ORDER BY date) AS price_factor,
    LAST_VALUE(total_area_factor) IGNORE NULLS OVER (PARTITION BY region_group, house_type, bedrooms, price_bin, total_area_bin, price_m2_bin ORDER BY date) AS total_area_factor,
    LAST_VALUE(price_m2_factor) IGNORE NULLS OVER (PARTITION BY region_group, house_type, bedrooms, price_bin, total_area_bin, price_m2_bin ORDER BY date) AS price_m2_factor
  FROM timeline
  LEFT JOIN factors
...
filling_timeline AS (
  SELECT
    ...
    total_area_factor,
    price_m2_factor
  FROM filling_timeline_forward_filled
  WHERE
    NOT region_factor IS NULL
    AND NOT house_type_factor IS NULL
    AND NOT bedrooms_factor IS NULL
    AND NOT price_factor IS NULL
    AND NOT total_area_factor IS NULL
    AND NOT price_m2_factor IS NULL
),
sellability_factor AS (
  SELECT
    ...
    price_m2_bin,
    COALESCE(
      house_type_factor * bedrooms_factor * price_factor * total_area_factor * price_m2_factor,
      0
    ) AS factor,
    filling_timeline.date
  FROM events_enriched
  LEFT JOIN filling_timeline
    USING (region_group, house_type, bedrooms, price_bin, total_area_bin, price_m2_bin)
  WHERE
    event = 'PUBLICATION' AND filling_timeline.date >= events_enriched.date
),
creating_tiers AS (
  SELECT
    id_house,
    ...
    CASE
      WHEN factor <= 0.3
      THEN 'S1'
      WHEN factor > 0.3 AND factor <= 1
      THEN 'S2'
      WHEN factor > 1 AND factor <= 2
      THEN 'S3'
      WHEN factor > 2 AND factor <= 3
      THEN 'S4'
      WHEN factor > 3
      THEN 'S5'
    END AS tier,
    date AS ts_tier_started
  FROM sellability_factor
),
...
tier_status AS (
  SELECT
    ...
    CASE
      ...
      THEN 'Quite Similar'
      WHEN tier = 'S3'
      THEN 'Moderately Similar'
      WHEN tier = 'S2'
      THEN 'Somewhat Similar'
      WHEN tier = 'S1'
      THEN 'Not Very Similar'
    END AS tier_name,
    ts_tier_started,
    DATE_ADD(
