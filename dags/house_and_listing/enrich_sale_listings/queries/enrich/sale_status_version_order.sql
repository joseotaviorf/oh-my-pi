...
lbc_aud AS (
  SELECT
    ...
    status_closing,
    previous_status,
    previous_status_closing,
    status_reason,
    ts_first_publication
  FROM lbc_aud_with_lag
  WHERE
...
house_aud AS (
  SELECT
    ...
    rev.reason,
    lbc_aud.rev,
    CAST(rev.ts_revision AS DATE) AS status_date,
    lbc_aud.ts_first_publication,
    rev.ts_revision AS revision_time
  FROM lbc_aud
  JOIN datalake_ebdb_clean.house AS h
    ON h.id = lbc_aud.id_house
  JOIN datalake_ebdb_user.user_revision_entity AS rev
...
house_new_status_new_date AS (
  SELECT
    ...
    IF(
      ...
      DATEDIFF(
        TO_DATE(CAST(COALESCE(ts_status_changed_next, NOW()) AS DATE)),
        TO_DATE(CAST(ts_status_changed AS DATE))
      ),
      NULL
    ) AS days_unpublished,
    order_status,
    ...
    IF(status_history IS NULL, ts_first_publication, ts_status_changed) AS ts_status_changed_new
  FROM house_status_history
),
-- Although a rented status is a trigger to a new version, the new version will only start when there is a new published status.
house_status_version_first_publi AS (
  SELECT
    ...
    status_reason_detail,
    status_history_new,
    MIN(CASE WHEN status_history_new = 'PUBLISHED' THEN ts_status_changed_new END) OVER (PARTITION BY id_house) AS first_publication_change_version,
    days_unpublished,
    order_status,
    max_order_status,
    ts_first_publication,
  ...
  FROM house_new_status_new_date
),
...
house_status_version_order_not_null_publi_date AS (
  ...
  FROM house_status_version_first_publi
  WHERE
    NOT first_publication_change_version IS NULL
),
aux AS (
  SELECT
    id_house,
    ...
    ts_status_changed_next,
    ts_status_changed_new
  FROM house_status_version_order_null_publi_date
  UNION ALL
  SELECT
    id_house,
    id_user_revision,
    ...
    ts_status_changed_next,
    ts_status_changed_new
  FROM house_status_version_order_not_null_publi_date
)
SELECT
  id_house,
  id_user_revision,
