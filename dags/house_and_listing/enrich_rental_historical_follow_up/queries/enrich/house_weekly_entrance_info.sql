WITH date_series AS (
  ...
  FROM datalake_quintoandar.aux_date AS dd
  WHERE
    CAST(week_start AS DATE) < DATE_TRUNC('WEEK', CURRENT_DATE)
    AND date >= '2021-01-01'
),
house_date_series AS (
  ...
  CROSS JOIN date_series AS dd
  WHERE
    NOT h.dt_first_publication IS NULL
),
entry_aud AS (
  SELECT
    id_house,
  ...
  WHERE
    mod_type OR mod_occupant OR mod_authorization
),
entry_ordered_ranked AS (
  SELECT
    id_house,
    key_type_history,
    ...
    COALESCE(
      LEAD(week_end) OVER (PARTITION BY id_house ORDER BY ts_event),
      DATE_ADD(DATE_TRUNC('WEEK', CURRENT_DATE), 6)
    ) AS week_end_key_type_end,
    ROW_NUMBER() OVER (PARTITION BY id_house, week_end ORDER BY ts_event DESC) AS rn
  FROM entry_aud
),
entry_ordered AS (
  SELECT
...
house_entrance_ordered AS (
  SELECT
    ...
    COALESCE(
      LEAD(week_end) OVER (PARTITION BY id_house ORDER BY ts_event),
      DATE_ADD(DATE_TRUNC('WEEK', CURRENT_DATE), 6)
    ) AS week_end_house_entrance_end
  FROM house_entrance_aud
),
...
FROM house_date_series AS hds
JOIN entry_base AS entry
  ON hds.id_house = entry.id_house AND hds.week_end = entry.week_end
JOIN house_entrance_base AS heb
  ON hds.id_house = heb.id_house AND hds.week_end = heb.week_end
