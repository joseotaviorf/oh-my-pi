...
JOB_NAME = "load_fact_price_suggested"
...
def build_fact_price_suggested(spark, affected_suggestions):
    """
    Build fact_price_suggested for affected suggestions.
    Contains measures (prices, bounds, anchors).
    """
    ...
    return filtered_df.select(
        ...
        F.col("id_house").alias("sk_house"),
        F.col("id_prediction").alias("sk_prediction"),
...
if __name__ == "__main__":
    ...
    if affected_count == 0:
        ...
        result_df = build_fact_price_suggested(spark, affected_suggestions)
        ...
        MetastoreServiceFactory.create_loader_metastore_service(
            spark_client
        ).refresh_table(write_database_name, write_table_name)
        logger.info(
