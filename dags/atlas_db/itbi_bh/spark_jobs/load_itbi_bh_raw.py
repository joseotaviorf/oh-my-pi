...
from pyspark.sql.functions import coalesce, lit, month, to_date, to_timestamp, year
...
def get_data(
    ...
    for url in urls:
        ...
        df = pd.read_csv(
            csv_content, sep=";", thousands=".", decimal=",", skip_blank_lines=True
        )
        ...
        if columns_rename_mapped:
            df = rename_columns(
                columns_rename_mapped=columns_rename_mapped, dataframe=df
            )
...
def extract_urls(
    ...
    """
    try:
        pattern = rf'<a\s+(?:[^>]*?\s+)?href="([^"]*itbi.*?\{source_format})"'
        ...
        logger.info(
            f"m=extract_urls, msg=Extracted {len(filtered_urls)} URLs from {source_download_page_url}"
        )
