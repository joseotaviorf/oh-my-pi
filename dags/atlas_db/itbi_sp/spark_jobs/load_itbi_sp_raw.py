...
def extract_urls(
    ...
    try:
        ...
        excel_links = [
            ...
            and "Excel/xlsx" in a.text
            and (
                (base_url + a["href"])
                if not a["href"].startswith("https")
                else a["href"]
            )
            not in blocklist_urls
        ]
...
def rename_columns(
    ...
    problematic_columns = set(dataframe.columns) & set(
    ...
    if problematic_columns:
        for column in problematic_columns:
            columns_rename_mapped = {
                **columns_rename_mapped,
                **problematic_columns_rename_mapped[column],
            }
...
def process_url(
    ...
    month_expr = F.create_map(
        *sum([[F.lit(k), F.lit(v)] for k, v in month_map.items()], [])
    )
    ...
    for sheet in list_of_sheets:
        ...
        logger.info(
            f"m=process_url, msg=Converted sheet {sheet_name} from URL {url} to Spark DataFrame with {df.count()} rows and {len(df.columns) - 1} columns."
        )
        ...
        df = (
            ...
            .withColumn(
                "year",
                F.expr("substring(source_tab, 5, length(source_tab))").cast(
                    IntegerType()
                ),
            )
            .withColumn(
                "month",
                month_expr[F.expr("substring(source_tab, 0, 3)")].cast(IntegerType()),
            )
        )

        logger.info(
            f"m=process_url, msg=Added new columns for {sheet_name} from URL {url}."
        )
...
def get_data(
    ...
    logger.info(
        f"m=get_data, msg=Downloaded {len(dataframes)} sheets from {source_download_page_url} and merged them into a single dataframe."
    )
...
def main():
    ...
    problematic_columns_rename_mapped = config_service.get_config(
        "problematic_columns_to_rename"
    )
