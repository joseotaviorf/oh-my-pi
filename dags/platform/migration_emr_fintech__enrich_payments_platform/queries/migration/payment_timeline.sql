WITH subscription_invoices AS (
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY id, charge_status ORDER BY rev DESC) AS _w
  ...
  FROM (
    SELECT
      ...
      ts_canceled,
      ts_updated,
      ROW_NUMBER() OVER (PARTITION BY id_charge, payment_method, status ORDER BY ts_updated DESC) AS _w
    FROM datalake_checkout_clean.charge_status_log
  ) AS _t
  WHERE
    _w = 1
), charge_created AS (
  ...
  SELECT
    ...
    cat.timeline_order
  FROM unique_ws_charge AS c
  LEFT JOIN charge_aud_timeline AS cat
    ON cat.id = c.id
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY COALESCE(
        ...
        COALESCE(cc.id_finance_entity, wsc.id_finance_entity, si.id_finance_entity),
        COALESCE(cc.id_finance_entity, wsc.id_finance_entity, si.id_finance_entity)
      ), COALESCE(cc.id_charge, wsc.id) ORDER BY COALESCE(cca.ts_created, wsc.ts_updated_timeline, cc.ts_created) DESC) AS _w,
      cc.id_charge,
      wsc.id,
      cca.ts_created,
      wsc.ts_updated_timeline
  ...
  SELECT
    ...
    csl.timeline_order
  FROM unique_credit_card_payments AS uccp
  LEFT JOIN charge_aud_timeline AS csl
  ...
  FROM unique_checkout AS uc
  LEFT JOIN charge_status_log_timeline AS csl
...
SELECT DISTINCT
  id_business_key, /* id_payment_timeline BIGINT GENERATED ALWAYS AS IDENTITY (created directly on Databricks CREATE TABLE) */
  datasource,
  CASE
    WHEN timeline_status = ''
    THEN NULL
    WHEN timeline_status = '52'
    THEN NULL
    WHEN timeline_status = 'PAID'
    THEN 'PAID'
    WHEN timeline_status = 'PROCESSING'
    THEN 'PROCESSING'
    WHEN timeline_status = 'ERROR'
    THEN 'ERROR'
    WHEN timeline_status = 'CANCELED'
    THEN 'CANCELED'
    WHEN timeline_status = 'CHARGEBACK'
    THEN 'CHARGEBACK'
    WHEN timeline_status = 'CHARGEBACK_CAPTURED'
    THEN 'CHARGEBACK_CAPTURED'
    WHEN timeline_status = 'REFUNDED'
    THEN 'REFUNDED'
    WHEN timeline_status = 'PROCESSING_REFUND'
    THEN 'PROCESSING_REFUND'
    WHEN timeline_status = 'PENDING_CAPTURE'
    THEN 'PENDING_CAPTURE'
    WHEN timeline_status = 'CAPTURE_NOT_PROCESSED'
    THEN 'CAPTURE_NOT_PROCESSED'
    WHEN timeline_status = 'CAPTURE_DENIED'
    THEN 'CAPTURE_DENIED'
    WHEN timeline_status = 'CAPTURE_BLOCKED'
    THEN 'CAPTURE_BLOCKED'
    WHEN timeline_status = 'CAPTURED'
    THEN 'CAPTURED'
    WHEN timeline_status = 'PENDING_CANCELATION'
    THEN 'PENDING_CANCELATION'
    WHEN timeline_status = 'CANCELATION_NOT_PROCESSED'
    THEN 'CANCELATION_NOT_PROCESSED'
    WHEN timeline_status = 'OPEN'
    THEN 'OPEN'
    WHEN timeline_status = 'REQUESTED'
    THEN 'REQUESTED'
    WHEN timeline_status = 'PENDING_REGISTER_PAYMENT'
    THEN 'PENDING_REGISTER_PAYMENT'
    WHEN timeline_status = ':boleto.status/paid'
    THEN 'PAID'
    WHEN timeline_status = ':boleto.status/scheduled'
    THEN 'SCHEDULED'
    WHEN timeline_status = ':boleto.status/processed'
    THEN 'PROCESSED'
    WHEN timeline_status = ':boleto.status/error'
    THEN 'ERROR'
    WHEN timeline_status = ':boleto.status/chargeback'
    THEN 'CHARGEBACK'
    WHEN timeline_status = ':boleto.status/requested'
    THEN 'REQUESTED'
    WHEN timeline_status = ':boleto.status/canceled'
    THEN 'CANCELED'
    WHEN timeline_status = ':boleto.status/write-down-requested'
    THEN 'PENDING_WRITE_DOWN'
    WHEN timeline_status = ':boleto.status/created'
    THEN 'CREATED'
    WHEN timeline_status = ':boleto.status/written-down'
    THEN 'WRITTEN_DOWN'
    WHEN timeline_status = ':boleto.status/written-down-error'
    THEN 'WRITE_DOWN_ERROR'
    WHEN timeline_status = ':boleto.status/write-down-paid-requested'
    THEN 'PENDING_WRITE_DOWN_PAID'
    WHEN timeline_status = ':boleto.status/changed'
    THEN 'CHANGED'
