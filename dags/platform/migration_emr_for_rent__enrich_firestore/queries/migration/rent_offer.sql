...
SELECT
  ...
  CAST(GET_JSON_OBJECT(roa.updated_message, hasDraftTopics) AS BOOLEAN) AS has_draft_topics,
  CAST(GET_JSON_OBJECT(roa.updated_message, instantOffer) AS BOOLEAN) AS is_instant_offer,
  CAST(GET_JSON_OBJECT(roa.updated_message, visualized) AS BOOLEAN) AS is_visualized,
  CAST(FROM_UNIXTIME(
    BIGINT(GET_JSON_OBJECT(updated_message, notifications.emailSentToOwnerAt._seconds))
  ) AS TIMESTAMP) AS ts_email_sent_to_owner,
  CAST(BIGINT(
    COALESCE(
      GET_JSON_OBJECT(roa.updated_message, firstSentAt._seconds),
      GET_JSON_OBJECT(roa.updated_message, firstSentAt[*]._seconds)
    )
  ) AS TIMESTAMP) AS ts_first_sent,
  CAST(BIGINT(
    COALESCE(
      GET_JSON_OBJECT(roa.updated_message, lastSentDate._seconds),
      GET_JSON_OBJECT(roa.updated_message, lastSentDate[*]._seconds)
    )
  ) AS TIMESTAMP) AS ts_last_sent,
  CAST(BIGINT(
    COALESCE(
      GET_JSON_OBJECT(roa.updated_message, deadline._seconds),
      GET_JSON_OBJECT(roa.updated_message, deadline[*]._seconds)
    )
  ) AS TIMESTAMP) AS ts_deadline,
  CAST(BIGINT(
    COALESCE(
      GET_JSON_OBJECT(roa.updated_message, visualizedAt._seconds),
      GET_JSON_OBJECT(roa.updated_message, visualizedAt[*]._seconds)
    )
  ) AS TIMESTAMP) AS ts_visualized,
  CAST(BIGINT(
    COALESCE(
      GET_JSON_OBJECT(roa.updated_message, createdDate._seconds),
      GET_JSON_OBJECT(roa.updated_message, createdDate[*]._seconds)
    )
  ) AS TIMESTAMP) AS ts_created,
  CAST(BIGINT(
    COALESCE(
      GET_JSON_OBJECT(roa.updated_message, updated._seconds),
      GET_JSON_OBJECT(roa.updated_message, updated[*]._seconds)
    )
  ) AS TIMESTAMP) AS ts_updated,
  roa.ts_updated AS ts_updated_message,
  NOW() AS ts_load
FROM datalake_firestore_clean.rent_offer AS roa
INNER JOIN last_update AS lup
  ON roa.id = lup.id
