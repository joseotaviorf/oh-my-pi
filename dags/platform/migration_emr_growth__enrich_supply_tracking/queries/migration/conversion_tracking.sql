WITH taxonomy AS (
  SELECT
    ...
    ct.funnel_step,
    ct.id_region,
    COALESCE(mr_leads.ops_objective, mr_houses.ops_objective, ct.ops_objective, ops.ops_objective) AS ops_objective, /* We are considering the application of the reprocessing and in null cases, we get it from the conversion directly */
    ...
    ct.ops_approach, /* These fields are not present in the manual reprocessing table */
  ...
  FROM datalake_supply_flows.conversion_taxonomy AS ct
  LEFT JOIN datalake_supply_flows.operations_agents AS ops
    ON (
      ops.id_user = ct.id_user_registrant
    )
  LEFT JOIN datalake_supply_flows.manual_reprocessing_events AS mr_leads
    ON (
      ct.id_lead = mr_leads.id_entity
    )
    AND (
      ct.business_context = mr_leads.business_context
    )
    AND (
      mr_leads.reprocessing_entity_type = 'lead'
    )
  LEFT JOIN datalake_supply_flows.manual_reprocessing_events AS mr_houses
    ON (
      ct.id_house = mr_houses.id_entity
    )
    AND (
      ct.business_context = mr_houses.business_context
    )
  ...
  LEFT JOIN taxonomy AS ct
    ON (
      ce.id_entity = ct.id_house
    )
    AND (
      ce.business_context = ct.business_context
    )
    AND (
      ct.source <> 2
    )
  ...
  SELECT
    ...
    CASE
      ...
      WHEN NOT cf.application IS NULL
      THEN cf.application
      WHEN NOT cf.id_user_registrant IS NULL
  ...
  SELECT
    ...
    CASE
      WHEN aux_group = 'T6.0'
      THEN FIRST_VALUE(application_staging) OVER (PARTITION BY id_entity, business_context ORDER BY aux_group ASC, ts_event_adjusted ASC)
  ...
  SELECT
    ...
    business_context,
    CASE
      WHEN ownership = 'THIRD_PARTY'
      THEN '3P'
      WHEN ownership = 'STANDARD'
      THEN '1P'
  ...
  SELECT
    ...
    NOW() AS ts_load
  FROM conversion_final AS h
  LEFT JOIN house_supply_source AS hsc
    ON (
      h.id_entity = hsc.id_house
    )
    AND (
      h.business_context = hsc.business_context
    )
  LEFT JOIN datalake_supply_flows.conversion_lookup AS cl
    ON (
      h.id_lead = cl.id_lead
    )
    AND (
      h.business_context = cl.business_context
    )
    AND (
      h.supply_source = cl.supply_source
    )
)
SELECT
  id_lead_ebdb,
...
FROM (
  SELECT
    ...
    ts_reprocessing_event,
    ts_load,
    ROW_NUMBER() OVER (PARTITION BY id_lead_ebdb, id_house, business_context, funnel_step, business_event ORDER BY ts_event_adjusted) AS _w
  FROM aux_tab
) AS _t
WHERE
  _w = 1