WITH all_listings AS (
  SELECT
    ...
    total_area,
    bedrooms
  FROM datalake_atlas_pricing_report.on_market_house_listings
  UNION ALL
  SELECT
    id_house,
    id_region,
  ...
  FROM (
    SELECT
      ...
      total_area,
      bedrooms,
      ROW_NUMBER() OVER (PARTITION BY id_house, business_context ORDER BY ts_status_started DESC) AS _w,
      ts_status_started
  ...
  SELECT
    ...
    6371 * 2 * ASIN(
      SQRT(
        POWER(SIN(RADIANS(sim.lat - ref.lat) / 2), 2) + COS(RADIANS(ref.lat)) * COS(RADIANS(sim.lat)) * POWER(SIN(RADIANS(sim.lng - ref.lng) / 2), 2)
      )
    ) AS distance_km
  FROM datalake_atlas_pricing_report.on_market_house_listings AS ref
  LEFT JOIN all_listings AS sim
  ...
  FROM (
    SELECT
      *,
      ROW_NUMBER() OVER (PARTITION BY id_house, business_context, house_status ORDER BY distance_km) AS _w
    FROM neighborhood_similar_houses
    WHERE
      distance_km <= 2
  ) AS _t
  WHERE
  ...
  FROM similar_houses_recovery AS rec
  INNER JOIN datalake_atlas_pricing_report.on_market_house_listings AS ref
    ON rec.id_house = ref.id_house AND rec.business_context = ref.business_context
  LEFT JOIN all_listings AS sim
    ON ref.business_context = sim.business_context
    AND ref.house_type = sim.house_type
  ...
  FROM (
    SELECT
      *,
      MAX(similar_order) OVER (PARTITION BY id_house, business_context, house_status) AS _w
    FROM nearest_recovered_houses
  ) AS _t
  WHERE
    _w >= 5
  ...
  FROM (
    SELECT
      nh.*,
      COUNT(nh.similar_id_house) OVER (PARTITION BY id_house, business_context, house_status) AS _w
    FROM nearest_houses AS nh
  ) AS _t
  WHERE
    _w >= 5
  UNION ALL
  SELECT
    ...
    distance_km
  FROM nearest_recovered_houses_fix
)
SELECT
  result.id_house,
...
FROM all_similar_houses AS result
INNER JOIN datalake_atlas_pricing_report.on_market_house_listings AS reference
  ON reference.id_house = result.id_house
  AND reference.business_context = result.business_context
INNER JOIN all_listings AS similar
