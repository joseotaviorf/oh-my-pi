WITH similar_houses AS (
  ...
  LEFT JOIN datalake_atlas_pricing_report.on_market_house_listings AS sim
    ...
    AND (
      NOT sim.iptu IS NULL OR NOT sim.condo IS NULL
    )
  ...
  FROM (
    SELECT
      *,
      ...
      ROW_NUMBER() OVER (PARTITION BY id_house ORDER BY distance_km) AS _w
    FROM similar_houses
    WHERE
      distance_km <= 2 AND rn_condo = 1 AND NOT similar_condo IS NULL
  ) AS _t
  WHERE
    _w <= 30
  ...
  FROM (
    SELECT
      *,
      MAX(similar_order) OVER (PARTITION BY id_house) AS _w
    FROM nearest_houses_condo
  ) AS _t
  WHERE
    _w >= 5
  ...
  FROM (
    ...
    FROM similar_houses
    WHERE
      distance_km <= 2 AND rn_iptu = 1 AND NOT similar_iptu IS NULL
  ) AS _t
  WHERE
    _w <= 30
  ...
  SELECT
    ...
    similar_id_house,
    distance_km
  FROM nearest_houses_condo_fix
  UNION
  SELECT
    id_house,
    similar_id_house,
    distance_km
  FROM nearest_houses_iptu_fix
), auxiliar_info AS (
  SELECT DISTINCT
    id_house,
    house_type,
