WITH amplitude_events AS (
  ...
  WHERE
    CAST(CAST(year AS STRING) || CAST(month AS STRING) || CAST(day AS STRING) AS DATE) >= (
  ...
  FROM dw_datamarts.lead_listing_flows AS llf
  ...
  INNER JOIN dw_public.dim_region AS dr
    ON fhl.sk_region = dr.sk_region AND dr.country_code <> 'MX'
  GROUP BY
    1,
    2,
    3,
  ...
  SELECT DISTINCT
    ...
    CAST(a.mdape_city AS FLOAT)
  FROM datalake_atlas_pricing_report.region_metrics AS a
  LEFT JOIN house_views AS m
