WITH listing_base AS (
  ...
  FROM datalake_atlas_pricing_report.status_history_house_listing AS lst
  UNION ALL
  SELECT
    chg.id_house,
  ...
  FROM datalake_atlas_pricing_report.status_history_house_price_change AS chg
  UNION ALL
  SELECT
    neg.id_house,
  ...
  FROM datalake_atlas_pricing_report.status_history_house_negotiation AS neg
  ...
  FROM listing_base AS lb
  LEFT JOIN datalake_atlas_pricing_report.status_history_house_negotiation AS neg
  ...
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY nw.id_house, nw.status, nw.next_negotiation, nw.business_context ORDER BY nw.ts_status_started) AS first_status,
    ROW_NUMBER() OVER (PARTITION BY nw.id_house, nw.status, nw.next_negotiation, nw.business_context ORDER BY nw.ts_status_started DESC) AS last_status,
    ROW_NUMBER() OVER (PARTITION BY nw.id_house, nw.next_negotiation, nw.business_context ORDER BY nw.ts_status_started) AS window_order
  FROM negotiation_windows AS nw
  ...
  WHERE
    (
      ...
      OR (
        status = 'PRICE_CHANGE' AND last_status = 1 AND window_order > 1
      )
      OR (
        status = 'UNPUBLISHED'
        AND next_negotiation IS NULL
        AND last_status = 1
        AND COALESCE(next_status, 'N/A') <> 'PUBLISHED'
      )
      OR (
        status = 'NEGOTIATED' AND window_order > 1
      )
    )
)
SELECT
...
FROM avoiding_redundancy
WHERE
  1 = CASE
    WHEN status = 'PRICE_CHANGE'
    AND previous_price = price
    AND previous_status = 'PUBLISHED'
    THEN 0
    WHEN status = 'PRICE_CHANGE' AND next_price = price AND next_status = 'PUBLISHED'
    THEN 0
    ELSE 1
  END
