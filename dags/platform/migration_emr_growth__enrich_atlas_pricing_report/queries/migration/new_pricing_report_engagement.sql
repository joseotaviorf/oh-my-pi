WITH house_metrics AS (
  SELECT
    ...
    MEDIAN(
      CASE WHEN similar_house_status = 'off-market' THEN similar_days_in_the_market END
    ) AS off_market_days_to_contract_sign
  ...
  FROM datalake_atlas_pricing_report.house_condo_metrics
  GROUP BY ALL
)
SELECT
...
FROM house_metrics
LEFT JOIN condo_metrics
  ON house_metrics.id_house = condo_metrics.id_house
LEFT JOIN datalake_atlas_pricing_report.house_listing_views_metrics AS hlvm
