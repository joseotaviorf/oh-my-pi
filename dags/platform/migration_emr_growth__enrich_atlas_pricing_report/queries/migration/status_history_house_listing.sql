WITH sale AS (
  WITH aux_fs AS (
    ...
    INNER JOIN aux_fs AS afs
      ON fl.sk_sale_listing = afs.sk_sale_listing AND afs.min_load = fl.ts_load
  )
  SELECT
  ...
  WITH aux_fr AS (
    ...
    SELECT
      di.id_house_listing,
      di.rent
    FROM datalake_rental_historical_follow_up.house_listings_daily_info AS di
    INNER JOIN aux_fr AS a
      ON a.id_house_listing = di.id_house_listing AND a.min_day = di.dt_day
    WHERE
      country_code = 'BR'
  )
  SELECT
  ...
  FROM union_listing AS ul
  LEFT JOIN datalake_atlas_pricing_report.status_history_house_price_change AS hist_price
  ...
  FROM status_published AS sp
  LEFT JOIN datalake_rental_historical_follow_up.house_listings_daily_info AS dailyinfo
    ON dailyinfo.id_house = sp.id_house
    AND dailyinfo.year = YEAR(TO_DATE(sp.ts_status_started))
    AND dailyinfo.month = MONTH(TO_DATE(sp.ts_status_started))
    AND dailyinfo.day = DAY(TO_DATE(sp.ts_status_started))
  WHERE
    sp.price IS NULL AND sp.business_context = 'RENT'
  ...
  FROM (
    SELECT
      ...
      sp.ts_status_started,
      ROW_NUMBER() OVER (PARTITION BY sp.id_house, sp.ts_status_started ORDER BY FROM_UNIXTIME(r.ts_revision / 1000) DESC) AS _w,
      r.ts_revision
    FROM status_published AS sp
    INNER JOIN datalake_ebdb_clean.house_aud AS h_aud
      ON h_aud.id_house = sp.id_house
    INNER JOIN datalake_ebdb_clean.user_revision_entity AS r
      ON h_aud.rev = r.id AND sp.ts_status_started >= FROM_UNIXTIME(r.ts_revision / 1000)
    WHERE
      sp.price IS NULL AND sp.business_context = 'SALE'
  ) AS _t
  WHERE
    _w = 1
)
...
FROM rent_price_fallback AS rpf
UNION ALL
SELECT
  spf.id_house,
...
FROM sale_price_fallback AS spf
UNION ALL
SELECT
  ul.id_house,
  ul.business_context,
