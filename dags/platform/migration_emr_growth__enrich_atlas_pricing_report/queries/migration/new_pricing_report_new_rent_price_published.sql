WITH dim_region AS (
  ...
  FROM dw_rent.fact_house_listings AS f
  INNER JOIN dw_rent.dim_house_listing AS d
    USING (sk_house_listing)
  INNER JOIN dw_public.dim_region AS r
    USING (sk_region)
  WHERE
    d.status = 'PUBLISHED' AND r.country_code = 'BR'
  GROUP BY
    1
), all_transactions AS (
  SELECT
    ...
    CAST(DATE_TRUNC('QUARTER', dc.ts_signature) AS DATE) AS dt_quarter
  ...
  INNER JOIN dim_region AS r
    USING (sk_region)
  INNER JOIN active_for_rent_city_groups AS c
    USING (city_group)
  WHERE
    NOT sk_contract IS NULL
  ...
  WHERE
    price_per_m2 < 50000 AND dt_quarter >= CAST('2019-01-01' AS DATE)
  GROUP BY
    1,
    2
), price_published AS (
  ...
  FROM median_price_by_region AS p
  INNER JOIN dim_region AS r
    USING (sk_region)
  WHERE
    sk_region <> -1
