WITH contract_features AS (
  SELECT
    ...
    COUNT(
      CASE
        ...
        ) AND order_invoice_wallet_risk = 1
        THEN sk_invoice
      END
    ) AS n_anchor_invoices_not_negativable,
    ...
    SUM(
      IF(
        payment_status = 'open' AND has_bill_item_condominio,
        ABS(balance_bill_item_condominio),
        0
      )
    ) AS open_condominio_balance,
    ...
    SUM(
      IF(
        payment_status = 'open' AND has_bill_item_multa_recisoria,
        ABS(balance_bill_item_multa_recisoria),
        0
      )
    ) AS open_multa_recisoria_balance,
    ...
    SUM(
      IF(payment_status = 'open' AND has_bill_item_acordo, ABS(balance_bill_item_acordo), 0)
    ) AS open_acordo_balance,
    ...
    SUM(
      IF(
        payment_status = 'open' AND has_bill_item_rental_core,
        ABS(balance_bill_item_rental_core),
        0
      )
    ) AS open_rental_core_balance,
    ...
    SUM(
      IF(payment_status = 'open' AND has_bill_item_reparos, ABS(balance_bill_item_reparos), 0)
    ) AS open_reparos_balance,
    ...
    SUM(
      IF(
        payment_status = 'open' AND has_bill_item_multas_ongoing,
        ABS(balance_bill_item_multas_ongoing),
        0
      )
    ) AS open_multas_ongoing_balance,
    ...
    SUM(
      IF(
        payment_status = 'open' AND has_bill_item_utilidades,
        ABS(balance_bill_item_utilidades),
        0
      )
    ) AS open_utilidades_balance,
    ...
    SUM(
      IF(payment_status = 'open' AND has_bill_item_outros, ABS(balance_bill_item_outros), 0)
    ) AS open_outros_balance,
    COUNT(
      CASE
        WHEN is_first_invoice_contract AND payment_status = 'open'
        THEN sk_invoice
        ELSE NULL
      END
    ) AS n_first_invoices_open,
    COUNT(CASE WHEN is_first_invoice_contract THEN sk_invoice ELSE NULL END) AS n_first_invoices,
    COUNT(
      CASE
        WHEN invoice_type IN ('monthly', 'onboarding') AND invoice_delay_t1 > 0
        THEN sk_invoice
      END
    ) AS n_overdue_monthlys_t1,
    COUNT(
      CASE
        WHEN NOT invoice_type IN ('monthly', 'onboarding')
        AND is_child_negotiation
        AND invoice_delay_t1 > 0
        THEN sk_invoice
      END
    ) AS n_overdue_deals_t1,
    COUNT(
      CASE
        WHEN NOT invoice_type IN ('monthly', 'onboarding')
        AND NOT is_child_negotiation
        AND invoice_delay_t1 > 0
        THEN sk_invoice
      END
    ) AS n_overdue_others_t1,
    COUNT(
      CASE
        WHEN invoice_type IN ('monthly', 'onboarding') AND invoice_delay_t2 > 0
        THEN sk_invoice
      END
    ) AS n_overdue_monthlys_t2,
    COUNT(
      CASE
        WHEN NOT invoice_type IN ('monthly', 'onboarding')
        AND is_child_negotiation
        AND invoice_delay_t2 > 0
        THEN sk_invoice
      END
    ) AS n_overdue_deals_t2,
    COUNT(
      CASE
        WHEN NOT invoice_type IN ('monthly', 'onboarding')
        AND NOT is_child_negotiation
    ...
    SUM(
      CASE
        WHEN is_invoice_overdue_t1
        AND overdue_recovered_amount_t1 > 0
        AND invoice_type IN ('monthly')
        THEN invoice_delay_t1
        ELSE 0
      END
    ...
    COLLECT_SET(CASE WHEN payment_status = 'open' THEN sk_invoice ELSE NULL END) AS array_open_invoices,
    COLLECT_SET(CASE WHEN payment_status = 'paid' THEN sk_invoice ELSE NULL END) AS array_paid_invoices,
    COLLECT_SET(CASE WHEN payment_status = 'written-down' THEN sk_invoice ELSE NULL END) AS array_negotiated_invoices,
    COUNT(CASE WHEN invoice_type IN ('monthly') THEN sk_invoice ELSE NULL END) AS n_monthly_invoices,
    COUNT(
      CASE WHEN payment_status IN ('paid', 'written-down') THEN sk_invoice ELSE NULL END
    ) AS n_invoices_paid,
    COUNT(
      CASE
        WHEN payment_status IN ('paid', 'written-down') AND invoice_delay_t2 <= 0
        THEN sk_invoice
        ELSE NULL
      END
    ) AS n_invoices_paid_ontime_t2,
    COUNT(
      CASE
        WHEN payment_status IN ('paid', 'written-down') AND invoice_delay_t2 > 0
        THEN sk_invoice
        ELSE NULL
      END
    ) AS n_overdue_invoices_paid_t2,
    COUNT(
      CASE
        WHEN payment_status IN ('paid', 'written-down') AND invoice_delay_t1 <= 0
        THEN sk_invoice
        ELSE NULL
      END
    ) AS n_invoices_paid_ontime_t1,
    COUNT(
      CASE
        WHEN payment_status IN ('paid', 'written-down') AND invoice_delay_t1 > 0
        THEN sk_invoice
        ELSE NULL
      END
    ) AS n_overdue_invoices_paid_t1,
    COUNT(
      CASE
        WHEN payment_status IN ('paid', 'written-down')
        AND invoice_type IN ('monthly')
        AND invoice_delay_t2 <= 0
        THEN sk_invoice
        ELSE NULL
      END
    ) AS n_monthly_invoices_paid_ontime_t2,
    COUNT(
      CASE
        WHEN payment_status IN ('paid', 'written-down')
        AND invoice_type IN ('monthly')
    ...
    COUNT(
      CASE
        ...
        ELSE NULL
      END
    ) AS n_monthly_overdue_invoices_paid_t2,
    COUNT(
      CASE
        WHEN payment_status IN ('paid', 'written-down')
        AND invoice_type IN ('monthly')
        ...
        ELSE NULL
      END
    ) AS n_monthly_overdue_invoices_paid_t1,
    COUNT(
      CASE
        WHEN invoice_type IN ('monthly') AND dt_reference = dt_begin
        THEN sk_invoice
        ELSE NULL
      END
    ...
    SUM(
      CASE
        WHEN is_child_negotiation AND invoice_delay_t1 <= 0 AND payment_status = 'open'
        THEN ABS(due_amount)
        ELSE 0
      END
    ) AS open_wallet_to_due_deals,
    SUM(
      CASE
        WHEN is_child_negotiation
        AND DATE_TRUNC('MONTH', dt_due) = DATE_TRUNC('MONTH', dt_reference)
        AND payment_status = 'open'
        THEN ABS(due_amount)
        ELSE 0
      END
    ) AS open_wallet_to_due_deals_at_reference_month,
    SUM(
      CASE
        WHEN is_child_negotiation
        AND DATE_TRUNC('MONTH', dt_due) > DATE_TRUNC('MONTH', dt_reference)
        ...
        ELSE 0
      END
    ) AS open_wallet_to_due_deals_at_future_month,
    SUM(
      CASE
        WHEN invoice_type IN ('monthly', 'onboarding') AND invoice_delay_t1 > 0
        THEN ABS(due_amount)
    ...
    SUM(
      CASE
        ...
        ELSE 0
      END
    ) AS sum_overdue_others_t1
  FROM dw_collections_segmentation.fact_invoice_wallet_timeline
  WHERE
    dt_reference BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
  GROUP BY
    1,
    2
  ...
  SELECT
    ...
    dt_contract_start,
    dt_contract_end,
    GREATEST(dt_first_invoice, CAST('{load_start_date}' AS DATE)) AS dt_start_interval,
    IF(
      NOT dt_contract_end IS NULL,
      LEAST(
  ...
  WHERE
    dt_end_interval BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
  ...
  FROM get_date_array
  LATERAL VIEW
  EXPLODE(dt_reference_array) AS dt_reference
), invoices AS (
  SELECT
    CAST(ts_due AS DATE) AS dt_pipe,
    CAST(ts_created AS DATE) AS dt_invoicing,
    id_external AS id_invoice
  FROM datalake_retsuko.invoice
  WHERE
    purpose = 'monthly'
    AND due_amount < 0
    AND CAST(DATE_TRUNC('MONTH', ts_due) AS DATE) BETWEEN CAST(DATE_TRUNC('MONTH', CAST('{load_start_date}' AS DATE)) AS DATE) AND LAST_DAY(CAST('{load_end_date}' AS DATE))
), pipeline_dates AS (
  SELECT
    CAST(DATE_TRUNC('MONTH', dt_pipe) AS DATE) AS dt_month,
    dt_pipe,
  ...
  SELECT
    CAST(DATE_TRUNC('MONTH', dt_invoicing) AS DATE) AS dt_month,
    dt_invoicing,
    COUNT(DISTINCT id_invoice) AS invoices
  FROM invoices
  GROUP BY
  ...
  FROM (
    SELECT
      *,
      ROW_NUMBER() OVER (PARTITION BY dt_month ORDER BY invoices DESC) AS _w
    FROM pipeline_dates
  ) AS _t
  WHERE
    _w = 1
  ...
  SELECT /*+ RANGE_JOIN(d, 1) */
    ...
    p.dt_pipe,
    b.dt_invoicing
  FROM deduplicate_pipeline_dates AS p
  FULL OUTER JOIN deduplicate_billing_dates AS b
    ON p.dt_month = b.dt_month
  LEFT JOIN dw_public.dim_date AS d
    ON d.date = COALESCE(p.dt_month, b.dt_month)
), contract_timeline AS (
  SELECT /*+ RANGE_JOIN(f, 1),
    RANGE_JOIN(d, 1) */
    m.*,
    CASE
      WHEN m.dt_contract_end IS NULL
      OR m.dt_contract_end > (
        DATE_TRUNC('MONTH', m.dt_reference) + INTERVAL '1' MONTH
      )
      THEN 'Ativo'
      ELSE 'Finalizado'
    END AS contract_status_beginning_of_month,
    CASE
      WHEN m.dt_contract_end IS NULL OR m.dt_contract_end > m.dt_reference
      THEN 'Ativo'
      ELSE 'Finalizado'
    ...
    COALESCE(f.overdue_recovered_amount_t1, 0) > 0
    AND COALESCE(f.open_wallet_overdue_t1, 0) = 0 AS is_quitacao_t1,
    COALESCE(f.overdue_recovered_amount_t2, 0) > 0
    AND COALESCE(f.open_wallet_overdue_t2, 0) = 0 AS is_quitacao_t2,
    COALESCE(f.overdue_recovered_amount_t3, 0) > 0
    AND COALESCE(f.open_wallet_overdue_t3, 0) = 0 AS is_quitacao_t3,
    COALESCE(f.open_wallet_overdue_t1, 0) = 0 AS is_regular_t1,
    COALESCE(f.open_wallet_overdue_t2, 0) = 0
    AND COALESCE(f.overdue_recovered_amount_t2, 0) = 0
    AND DATE_TRUNC('MONTH', m.dt_reference) = m.dt_reference AS is_current_at_month_start_t1,
  ...
  LEFT JOIN contract_features AS f
    ON m.sk_contract = f.sk_contract AND m.dt_reference = f.dt_reference
  LEFT JOIN get_dates_billing AS d
    ON d.dt_month = DATE_TRUNC('MONTH', m.dt_reference)
), fact_collection_base AS (
  SELECT
    CAST(dt_occurrence AS DATE) AS dt_reference,
    sk_contract,
  ...
  WHERE
    CAST(dt_occurrence AS DATE) >= CAST('{load_start_date}' AS DATE)
    AND CAST(dt_occurrence AS DATE) <= CAST('{load_end_date}' AS DATE)
  GROUP BY
    1,
    2
  ...
  SELECT
    CAST(ts_event AS DATE) AS dt_reference,
    id_contract,
    ...
    COUNT(
      DISTINCT CASE
        ...
        AND NOT id_invoice IS NULL
        THEN id_amplitude
      END
    ) AS qnt_app_events_overdue,
    ...
    CAST(COUNT(
      DISTINCT CASE
        ...
        THEN id_amplitude
      END
    ) > 0 AS BOOLEAN) AS has_app_events_overdue
  FROM datalake_collections_quintoandar.delinquency_app_events
  WHERE
    NOT id_contract IS NULL
    AND NOT funnel_step IN ('Home Page')
    AND CAST(ts_event AS DATE) >= CAST('{load_start_date}' AS DATE)
    AND CAST(ts_event AS DATE) <= CAST('{load_end_date}' AS DATE)
  GROUP BY
    1,
    2
), base_evictions AS (
  SELECT
    ...
    CAST(dt_registered AS DATE) AS dt_registered,
    CAST(dt_arbitral_distribution_start AS DATE) AS dt_arbitral_distribution,
    CAST(dt_closure AS DATE) AS dt_elaw_closure,
    LEAST(CAST(dt_registered AS DATE), CAST(dt_arbitral_distribution_start AS DATE)) AS dt_begin
  ...
  SELECT
    *,
    CASE
      WHEN NOT dt_elaw_closure_timeline IS NULL
      THEN 'EVENDED'
      WHEN dt_registered_timeline IS NULL
      AND NOT dt_arbitral_distribution_timeline IS NULL
      AND dt_elaw_closure_timeline IS NULL
      THEN 'EVEX'
      WHEN NOT dt_registered_timeline IS NULL
      AND NOT dt_arbitral_distribution_timeline IS NULL
      AND dt_elaw_closure_timeline IS NULL
      THEN 'EVEX'
      WHEN NOT dt_registered_timeline IS NULL
  ...
  FROM (
    SELECT
      ...
      dt_reference,
      ROW_NUMBER() OVER (PARTITION BY id_contract, dt_reference ORDER BY id_process) AS _w
    FROM status_timeline_evic
    WHERE
      status_time IN ('EVEX', 'EVPD')
  ) AS _t
  WHERE
    _w = 1
), base_negotiations AS (
  SELECT
    CAST(dt_promisse AS DATE) AS dt_reference,
    sk_contract,
    COUNT(DISTINCT sk_negotiation) AS promessas,
    COUNT(DISTINCT CASE WHEN NOT dt_down_payment IS NULL THEN sk_negotiation ELSE NULL END) AS acordos,
    COUNT(
      DISTINCT CASE
        WHEN number_of_installments > 1 AND promisse_payment_method = 'BOLETO'
        THEN sk_negotiation
        ELSE NULL
      END
    ) AS promessas_neg_parcelada_boleto,
    COUNT(
      DISTINCT CASE
        WHEN number_of_installments > 1
        AND promisse_payment_method = 'BOLETO'
        AND NOT dt_down_payment IS NULL
        THEN sk_negotiation
        ELSE NULL
      END
    ) AS acordos_neg_parcelada_boleto,
    COUNT(
      DISTINCT CASE WHEN discount_to_original_amount > 1 THEN sk_negotiation ELSE NULL END
    ) AS promessas_desconto_principal,
    COUNT(
      DISTINCT CASE
        WHEN discount_to_original_amount > 1 AND NOT dt_down_payment IS NULL
        THEN sk_negotiation
        ELSE NULL
      END
    ) AS acordo_desconto_principal,
    COUNT(DISTINCT CASE WHEN discount_to_fees_amount > 1 THEN sk_negotiation ELSE NULL END) AS promessas_desconto_multas,
    COUNT(
      DISTINCT CASE
        WHEN discount_to_fees_amount > 1 AND NOT dt_down_payment IS NULL
        THEN sk_negotiation
        ELSE NULL
      END
    ) AS acordo_desconto_multas,
    COUNT(DISTINCT sk_negotiation) FILTER(WHERE
      DATEDIFF(TO_DATE(dt_cancellation), TO_DATE(dt_promisse)) >= 5
      AND dt_down_payment IS NULL) AS qt_promessa_quebrada_fp,
    COUNT(DISTINCT sk_negotiation) FILTER(WHERE
      NOT dt_down_payment IS NULL
      AND (
        number_of_installments = 1 OR promisse_payment_method = 'CARTÃO DE CRÉDITO'
      )) AS qt_acordo_a_vista,
    COUNT(DISTINCT sk_negotiation) FILTER(WHERE
      NOT dt_down_payment IS NULL
    ...
    COUNT(DISTINCT sk_negotiation) FILTER(WHERE
      discount_to_original_amount > 50 AND NOT dt_down_payment IS NULL) AS qt_aco_desconto
  FROM dw_collection_recovery_quintoandar.fact_negotiation
  WHERE
    negotiation_status IN ('broken-requested-by-client', 'started', 'offset', 'broken', 'finished', 'canceled')
    AND CAST(dt_promisse AS DATE) <= CAST('{load_end_date}' AS DATE)
  GROUP BY
    1,
    2
), negotiations AS (
  ...
  FROM (
    SELECT
      ...
      CASE
        WHEN UPPER(dpdc.guarantee) = 'SEGUROFAIRFAX'
        THEN 'FAIRFAX'
        WHEN UPPER(dpdc.guarantee) = 'PRO_GUARANTOR'
        THEN 'PRO_GUARANTOR'
        WHEN UPPER(dpdc.guarantee) = 'RENTALGUARANTEE'
        THEN 'RENTAL_GUARANTEE'
        WHEN UPPER(dpdc.guarantee) = 'RENTALDEPOSIT' OR dpdc.guarantee = 'DEPOSITO'
        THEN 'RENTAL_DEPOSIT'
        WHEN UPPER(dpdc.guarantee) IN ('STANDALONE', 'THIRDPARTYGUARANTEE')
        THEN 'BROKERAGE_ONLY'
        ELSE 'OTHERS'
      END AS contract_guarantee,
      ROW_NUMBER() OVER (PARTITION BY dpdc.sk_contract ORDER BY dpdc.ts_updated DESC) AS _w,
      dpdc.ts_updated
    FROM dw_rent.dim_contract AS dpdc
  ) AS _t
  WHERE
    _w = 1
), contract_info AS (
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY c.sk_contract ORDER BY f.sk_proposal DESC) AS _w,
      f.sk_proposal
    FROM dw_rent.dim_contract AS c
    LEFT JOIN dw_rent.fact_listing_rent_flows AS f
      ON c.sk_contract = f.sk_contract AND f.sk_contract <> -1
    LEFT JOIN datalake_sorting_hat_clean.proponent AS p
      ON p.id_proposal = f.sk_proposal
    GROUP BY
      c.sk_contract,
      f.sk_proposal,
      f.sk_proposal_approved_date,
      c.ts_canceled,
      c.guarantee
  ) AS _t
  WHERE
  ...
  LEFT JOIN evictions_timeline AS e
    ON e.dt_reference = m.dt_reference AND CAST(e.id_contract AS BIGINT) = m.sk_contract
  LEFT JOIN collections_efforts AS be
    ON be.dt_reference = m.dt_reference AND be.sk_contract = m.sk_contract
  LEFT JOIN app_events_features AS app
    ON app.id_contract = m.sk_contract AND app.dt_reference = m.dt_reference
  LEFT JOIN negotiations AS d
    ON d.sk_contract = m.sk_contract AND d.dt_reference = m.dt_reference
  LEFT JOIN contract_info AS c
...
SELECT
  ...
  CASE
    ...
    THEN CAST(DATEDIFF(TO_DATE(CAST(dt_reference AS DATE)), TO_DATE(dt_contract_end)) AS BIGINT)
    ELSE NULL
  END AS days_since_ending,
  CAST(DATEDIFF(TO_DATE(CAST(dt_reference AS DATE)), TO_DATE(dt_contract_start)) AS BIGINT) AS days_since_contract_start,
