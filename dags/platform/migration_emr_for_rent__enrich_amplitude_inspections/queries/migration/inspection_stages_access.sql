WITH filter_type_tenant AS (
  ...
  FROM (
    SELECT DISTINCT
      ...
      month,
      day,
      MAX(ia.ts_updated) OVER (PARTITION BY ia.id_inspection) AS _w
  ...
  SELECT
    ...
    COALESCE(
      SUM(
        CAST(UPPER(insp.user_type) IN (
          SELECT
            val
          FROM filter_type_owner
        ) AS SMALLINT)
      ),
      0
    ) AS total_owner_access_review,
    COALESCE(MAX(UPPER(insp.user_type) IN (
      SELECT
        val
      FROM filter_type_owner
    )), FALSE) AS has_owner_access_review,
    COALESCE(
      SUM(
        CAST(UPPER(insp.user_type) IN (
          SELECT
            val
          FROM filter_type_tenant
        ) AS SMALLINT)
      ),
      0
    ) AS total_tenant_access_review,
    COALESCE(
      MAX(UPPER(insp.user_type) IN (
        SELECT
          val
        FROM filter_type_tenant
      )),
      FALSE
    ) AS has_tenant_access_review,
    MIN(
      CASE
        WHEN UPPER(insp.user_type) IN (
          SELECT
            val
          FROM filter_type_owner
        )
    ...
    ia.month,
    ia.day
  FROM inspections AS ia
  LEFT JOIN datalake_amplitude_clean.170698_inspection_review_home_page_viewed_events AS insp
  ...
  SELECT
    ...
    COALESCE(
      ...
      0
    ) AS total_owner_access_budget_approval,
    COALESCE(MAX(UPPER(ba.user_type) IN (
      SELECT
        val
      FROM filter_type_owner
    )), FALSE) AS has_owner_access_budget_approval,
    COALESCE(
      SUM(
        CAST(UPPER(ba.user_type) IN (
          SELECT
      ...
      0
    ) AS total_tenant_access_budget_approval,
    COALESCE(MAX(UPPER(ba.user_type) IN (
      SELECT
        val
      FROM filter_type_tenant
    )), FALSE) AS has_tenant_access_budget_approval,
    MIN(
      CASE
        WHEN UPPER(ba.user_type) IN (
          SELECT
            val
          FROM filter_type_owner
        )
    ...
    ia.day
  FROM inspections AS ia
  LEFT JOIN datalake_amplitude_clean.170698_inspection_budgetapproval_home_page_viewed_events AS ba
...
JOIN budget_approval_events AS ba
