WITH credit_analysis_ordered AS (
  ...
  SELECT
    ...
    LAST(id_variant) IGNORE NULLS AS id_last_variant_not_null,
    FIRST(category) AS first_category,
    LAST(category) AS last_category,
    LAST(category) IGNORE NULLS AS last_category_not_null,
    MAX(bypass) AS max_bypass
  FROM credit_analysis_ordered
  WHERE
    NOT id_proposal IS NULL
  GROUP BY
    1
  ...
  FROM datalake_docx.credit_evaluation
  ...
  SELECT
    ...
    MAX(
      p.rent_value + COALESCE(p.condo_value, 0) + COALESCE(p.iptu_value, 0) + COALESCE(p.home_insurance_value, 0)
    ) AS package,
    CASE
      WHEN SUM(p2.monthly_income) <> 0
      THEN CAST(MAX(
        p.rent_value + COALESCE(p.condo_value, 0) + COALESCE(p.iptu_value, 0) + COALESCE(p.home_insurance_value, 0)
      ) / SUM(p2.monthly_income) AS DECIMAL(9, 2))
      ELSE NULL
    END AS dti
  FROM datalake_sorting_hat_clean.proposal AS p
  LEFT JOIN datalake_sorting_hat_clean.proponent AS p2
  ...
  FROM datalake_sorting_hat_clean.proposal AS p
  LEFT JOIN datalake_sorting_hat_clean.proposal_version AS pv
  ...
  FROM sorting_hat_proposals_prev
  WHERE
    rn = 1
...
SELECT
  ...
  shp.ts_processed
FROM sorting_hat_proposals AS shp
LEFT JOIN credit_analysis_proposals AS cap
  ON shp.id_proposal = cap.id_proposal
LEFT JOIN credit_evaluations AS ceval
  ON shp.id_proposal = ceval.id_proposal
LEFT JOIN dti
  ON shp.id_proposal = dti.id_proposal
LEFT JOIN latest_screening_result AS cr
  ON shp.id_proposal = cr.id_proposal
LEFT JOIN datalake_rental_guarantee.guarantee AS rg
