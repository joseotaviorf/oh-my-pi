WITH credit_analysis_versions AS (
  ...
  FROM (
    SELECT
      ...
      risk_category_canon,
      score,
      ROW_NUMBER() OVER (PARTITION BY id_proposal ORDER BY ts_database_transaction DESC) AS _w,
      ts_database_transaction
    FROM datalake_sorting_hat_clean.screening_result
  ) AS _t
  WHERE
    _w = 1
), credit_analysis AS (
  SELECT
    ...
    cav.max_ca_category,
    ca.result,
    IF(ca.category IS NULL AND NOT ca.bypass IS NULL, TRUE, FALSE) AS is_bypass,
    cap.id_proposal_from_rg,
    cap.last_category,
    IF(ca.id_analyst IS NULL, FALSE, TRUE) AS is_manual_analysis,
    CASE
      WHEN cap.guarantee_source = 'CRM_DOCUMENTATION_ANALYSIS'
      THEN TRUE
      ELSE FALSE
  ...
  LEFT JOIN datalake_credit_analysis.credit_analysis_proposals AS cap
    ON ca.id_proposal = cap.id_proposal
  LEFT JOIN credit_analysis_versions AS cav
    ON ca.id_credit_analysis = cav.id_credit_analysis
  LEFT JOIN latest_screening_result AS sr
...
SELECT
  ...
  ca.last_category,
  CASE
    WHEN rsc.category_level = 40 AND rsc.standalone_factor = 0.96
    THEN 'Standalone Low'
    WHEN rsc.category_level = 41 AND rsc.standalone_factor = 1.20
    THEN 'Standalone High'
  END AS standalone_factor,
  CASE
    WHEN rsc.is_standalone_allowed = TRUE
    THEN 'STANDALONE'
    WHEN rsc.is_third_party_guarantee_allowed = TRUE
    THEN 'THIRD_PARTY_GUARANTEE'
    WHEN (
    ...
    THEN 'DEPOSIT'
    WHEN (
      rsc.category_level = 0
    )
    OR (
      ca.is_bypass = TRUE AND ca.id_proposal_from_rg IS NULL
    )
    THEN 'FREE'
    WHEN (
      ca.is_bypass = TRUE AND NOT ca.paid_guarantee_type IS NULL
    )
    THEN ca.paid_guarantee_type
    WHEN (
      ca.category IS NULL AND ca.is_bypass = FALSE AND ca.reason IN ('CLEAR_NO')
    )
    THEN 'CLEAR_NO'
    WHEN ca.category = -1
    THEN 'UNDEFINED'
  ...
  CASE
    ...
    THEN 'FREE'
    WHEN (
      ca.last_category IS NULL AND ca.is_bypass = FALSE
    )
    THEN 'CLEAR_NO'
    WHEN (
      NOT ca.last_category IS NULL AND NOT ca.id_proposal_from_rg IS NULL
    )
    OR (
      ca.is_bypass = TRUE
      AND ca.last_category IS NULL
      AND NOT ca.id_proposal_from_rg IS NULL
    )
    THEN ca.paid_guarantee_type
    WHEN (
      ca.id_proposal_from_rg IS NULL AND NOT ca.last_category IS NULL
    )
    THEN 'NOT_ACCEPTED'
