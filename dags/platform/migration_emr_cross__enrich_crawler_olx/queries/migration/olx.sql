 WITH json_select AS (
  ...
  SELECT 
    ...
    id_house,
    platform,
    COALESCE(GET_JSON_OBJECT(house_info,'$.external_id'),
      NULLIF(REGEXP_EXTRACT(GET_JSON_OBJECT(house_info, '$.description'), '[Cc][óo]digo.*?[\\-:]\\s*(.*?)<br>', 1), ''),
      NULLIF(REGEXP_EXTRACT(GET_JSON_OBJECT(house_info, '$.description'), '(?i)C[óo]digo[:.\s]*(.*?)<br>', 1), ''),
      NULLIF(REGEXP_EXTRACT(GET_JSON_OBJECT(house_info, '$.description'), '(?i)C[óo]d[:.\s]*(.*?)<br>', 1), '')
    ) AS advertiser_listing_id,
    ...
    GET_JSON_OBJECT(advertiser,'$.addresses.location.state') AS advertiser_address_state,
    GET_JSON_OBJECT(advertiser,'$.addresses.location.city') AS advertiser_address_city,
    GET_JSON_OBJECT(advertiser,'$.addresses.location.neighbourhood') AS advertiser_address_neighborhood,
    COALESCE(GET_JSON_OBJECT(advertiser,'$.isProfessional'), GET_JSON_OBJECT(advertiser,'$.isprofessional')) AS advertiser_is_professional,
