WITH fl_events AS (
  ...
  FROM (
    SELECT
      ...
      ts_event,
      rev,
      ROW_NUMBER() OVER (PARTITION BY id_entity, business_context ORDER BY rev, ts_event) AS _w
    FROM datalake_supply_flows.conversion_staging
    WHERE
      step = 'FIRST_LISTING'
  ) AS _t
  WHERE
    _w = 1 /* First Listing Event */
), opp_events AS (
  SELECT
    id_lead,
    id_entity,
  ...
  FROM (
    ...
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY lbca.id_house, lbca.business_context ORDER BY ure.ts_revision) AS _w,
      lbca.id_house,
      ure.ts_revision
    FROM datalake_ebdb_clean.listing_business_context_aud AS lbca
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY id_entity, business_context ORDER BY rev, ts_event DESC) AS _w
    FROM datalake_supply_flows.conversion_staging
    WHERE
      step = 'PROSPECT' AND (
        drop_step = 'AQ2O'
  ...
  WHERE
    _w = 1 /* The last drop AQ2O */
), t1 /* FIRST LISTING EVENTS */ AS (
  SELECT
    id_lead,
    id_entity,
    id_user_registrant,
  ...
  FROM fl_events
), opp_ongoing /* OPPORTUNITY EVENTS */ AS (
  SELECT
    opp.id_lead,
    opp.id_entity,
    ...
    DATE_ADD(DAY, scs.ongoing_window, opp.ts_event) AS ts_ongoing, /* This window was setup in config table */
    opp.ts_event AS ts_event_adjusted, /* The minor data between the event and the first listing */
    opp.rev,
    scs.ongoing_window
  FROM opp_events AS opp
  ...
  FROM (
    SELECT
      ...
      'T2.0' AS aux_group,
      FALSE AS aux_data_event,
      ROW_NUMBER() OVER (PARTITION BY id_entity, business_context ORDER BY cs.rev, cs.ts_event) AS _w,
      cs.ts_event
    FROM opp_events AS cs
    JOIN fl_events AS fl /* Cases with listing */
      USING (id_entity, business_context)
  ) AS _t
  WHERE
    _w = 1
  ...
  FROM (
    SELECT
      ...
      DATE_ADD(DAY, scs.ongoing_window, qual.ts_event) AS ts_ongoing,
      d.ts_event AS ts_event_discard,
      d.ts_first_discard,
      d.ts_last_discard,
      scs.ongoing_window,
      ROW_NUMBER() OVER (PARTITION BY qual.id_entity, qual.business_context, qual.step ORDER BY qual.rev, qual.ts_event) AS _w,
      qual.ts_event
    FROM datalake_supply_flows.conversion_staging AS qual
  ...
  FROM (
    SELECT
      ...
      'T3.0' AS aux_group,
      FALSE AS aux_data_event,
      ROW_NUMBER() OVER (PARTITION BY qual.id_entity, qual.business_context ORDER BY qual.rev, qual.ts_event) AS _w,
      qual.ts_event
    FROM datalake_supply_flows.conversion_staging AS qual
  ...
  SELECT
    *
  FROM t3_3
), discards_events_q2aq /* PROSPECTS Q2AQ EVENTS */ AS (
  SELECT
    id_lead,
    id_entity,
    id_user_registrant,
  ...
  FROM (
    SELECT
      ...
      ts_first_discard,
      ts_last_discard,
      RANK() OVER (PARTITION BY id_lead, business_context ORDER BY rev, ts_event DESC) AS _w
    FROM discards_events_q2aq
  ) AS _t
  WHERE
    _w = 1 /* ÚLTIMO EVENTO DE DESCARTE */
  UNION ALL
  SELECT
    id_lead,
    id_entity,
  ...
  JOIN last_discards_q2aq AS d
    ON (
      p.id_lead_ebdb = d.id_lead
    )
    AND (
      p.business_context = d.business_context
    )
    AND (
  ...
  FROM (
    SELECT
      ...
      MAX(ts_event) OVER (PARTITION BY id_entity, business_context) AS ts_last_discard,
      ROW_NUMBER() OVER (PARTITION BY id_lead, business_context ORDER BY rev, ts_event DESC) AS _w
    FROM datalake_supply_flows.conversion_staging
    LEFT ANTI JOIN fl_events AS fl
      USING (id_lead, business_context)
    LEFT ANTI JOIN bob_events AS qual
      USING (id_lead, business_context)
    LEFT ANTI JOIN t4 AS discards_q2aq
      USING (id_lead, business_context)
    WHERE
      step = 'PROSPECT' AND drop_step = (
        'P2Q'
  ...
  SELECT
    ...
    5 AS weight,
    p.ts_event,
    DATE_ADD(DAY, scs.ongoing_window, p.ts_event) AS ts_ongoing, /* FAÇO A SOMA DE 30 DIAS NA DATA DO EVENTO DE OPP */
    p.ts_event AS ts_first_discard,
    DATE_ADD(DAY, scs.ongoing_window, p.ts_event) AS ts_last_discard,
    ongoing_window
  FROM datalake_supply_flows.acquisition_tracking AS p
  ...
  LEFT ANTI JOIN fl_events AS fl
    ON (
      fl.id_lead = p.id_lead_ebdb
    )
    AND (
      fl.business_context = p.business_context
    )
  LEFT ANTI JOIN bob_events AS qual
    ON (
      qual.id_lead = p.id_lead_ebdb
    )
    AND (
      qual.business_context = p.business_context
    )
  LEFT ANTI JOIN t2 AS opp /* Removing opportunities */
    ON (
      opp.id_lead = p.id_lead_ebdb
    )
    AND (
      opp.business_context = p.business_context
    )
  LEFT ANTI JOIN t4 AS discards_q2aq /* Removing discards Q2AQ */
    ON (
      discards_q2aq.id_lead = p.id_lead_ebdb
    )
    AND (
      discards_q2aq.business_context = p.business_context
    )
  LEFT ANTI JOIN t3 AS discards_aq2o /* Removing discards AQ2O */
    ON (
      discards_aq2o.id_lead = p.id_lead_ebdb
    )
    AND (
      discards_aq2o.business_context = p.business_context
    )
  LEFT ANTI JOIN 3p_events AS 3pe /* Removing 3P events */
    ON (
      3pe.id_lead = p.id_lead_ebdb
    )
    AND (
      3pe.business_context = p.business_context
    )
    AND (
      3pe.supply_source = p.supply_source
    )
  LEFT JOIN datalake_supply_flows.supply_conversion_settings AS scs
    ON (
      scs.step = p.funnel_step
    )
  WHERE
    p.funnel_step = 'PROSPECT'
), t5_1 AS (
