WITH rent_sale_flow AS (
  ...
  SELECT
    rs.id_user,
    rs.id_house,
    CONCAT(rs.id_user, '__', rs.id_house) AS id_user_house,
    MD5(
      CONCAT(
        rs.id_user,
        '__',
    ...
    COALESCE(DATEDIFF(TO_DATE(sia.ts_visit_booked), TO_DATE(sia.ts_search)) <= 14, FALSE) AS is_search_click_and_visit_booked_within_14_days,
    COALESCE(DATEDIFF(TO_DATE(sia.ts_direct_offer), TO_DATE(sia.ts_search)) <= 14, FALSE) AS is_search_click_and_direct_offer_within_14_days,
    COALESCE(
      DATEDIFF(TO_DATE(sia.ts_visit_booked), TO_DATE(sia.ts_search)) <= 14
      OR DATEDIFF(TO_DATE(sia.ts_direct_offer), TO_DATE(sia.ts_search)) <= 14,
      FALSE
    ) AS is_search_click_and_vb_or_do_within_14_days,
    COALESCE(DATEDIFF(TO_DATE(sia.ts_offer), TO_DATE(sia.ts_search)) <= 14, FALSE) AS is_search_click_and_offer_within_14_days,
    rs.ts_first_flow_action,
