WITH rent_sale_flow AS (
  ...
  WHERE
    ...
    AND (
      ...
      OR ts_contract_signed BETWEEN DATE_ADD(CAST('{start_date}' AS DATE), STRUCT(days_past_30 AS days_past_30) * -1) AND CAST('{end_date}' AS DATE)
    )
  GROUP BY ALL
  UNION ALL
  SELECT
  ...
  WHERE
    ...
    AND (
      ...
      OR dt_sale_agreement_signed BETWEEN DATE_ADD(CAST('{start_date}' AS DATE), STRUCT(days_past_30 AS days_past_30) * -1) AND CAST('{end_date}' AS DATE)
    )
  GROUP BY ALL
  ...
  WHERE
    MAKE_DATE(year, month, day) BETWEEN DATE_ADD(CAST('{start_date}' AS DATE), STRUCT(days_past_45 AS days_past_45) * -1) AND CAST('{end_date}' AS DATE)
  ...
  SELECT
    ...
    COALESCE(DATEDIFF(TO_DATE(ri.ts_visit_booked), TO_DATE(ri.ts_recommendation)) <= 14, FALSE) AS is_rec_click_and_visit_booked_within_14_days,
    COALESCE(DATEDIFF(TO_DATE(ri.ts_direct_offer), TO_DATE(ri.ts_recommendation)) <= 14, FALSE) AS is_rec_click_and_direct_offer_within_14_days,
    COALESCE(
      DATEDIFF(TO_DATE(ri.ts_visit_booked), TO_DATE(ri.ts_recommendation)) <= 14
      OR DATEDIFF(TO_DATE(ri.ts_direct_offer), TO_DATE(ri.ts_recommendation)) <= 14,
      FALSE
    ) AS is_rec_click_and_vb_or_do_within_14_days,
    COALESCE(DATEDIFF(TO_DATE(ri.ts_offer), TO_DATE(ri.ts_recommendation)) <= 14, FALSE) AS is_rec_click_and_offer_within_14_days,
    rs.ts_first_flow_action,
    rs.ts_contract_signed AS ts_contract_signed,
    YEAR(TO_DATE(rs.ts_first_flow_action)) AS year,
    MONTH(TO_DATE(rs.ts_first_flow_action)) AS month,
    DAY(TO_DATE(rs.ts_first_flow_action)) AS day
  FROM rent_sale_flow AS rs
  LEFT JOIN recs_impressions_actions AS ri
