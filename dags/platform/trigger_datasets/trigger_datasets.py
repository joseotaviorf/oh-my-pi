...
def trigger_dataset_based_on_input(session: Session = NEW_SESSION, **context):
    ...
    if not (source_dag_id and source_task_id and source_run_id):
        raise ValueError(
            "dag_id, task_id and source_run_id must be provided in dag trigger forms"
        )

    logging.info(
        ""
        f"Triggering dataset for DAG: {source_dag_id}, Task: {source_task_id}, Source Run ID: {source_run_id}"
    )
    ...
    if is_first_run:
        logging.info(
            f"First run detected, adding dataset URI: {dataset_uri_first_run} to the list"
        )
