WITH pre_proposal_aud AS (
  SELECT DISTINCT
    ...
    MAX(CASE WHEN is_tenant_edition AND mod_rent THEN pp_aud.rent END) OVER w AS last_rent_value_tenant,
    COALESCE(
      MIN(CASE WHEN is_landlord_edition AND mod_rent THEN pp_aud.rent END) OVER w,
      MIN(pp_aud.original_rent) OVER w
    ) AS last_rent_value_landlord,
    MAX(CASE WHEN is_tenant_edition AND mod_rent THEN pp_aud.rent END) OVER w + MAX(pp_aud.original_condo) OVER w AS total_rent_value
  FROM datalake_ebdb_proposal.pre_proposal AS pp
  JOIN datalake_ebdb_proposal.pre_proposal_aud AS pp_aud
    ON pp.id = pp_aud.id_pre_proposal
  WINDOW w AS (PARTITION BY pp.id)
  ...
  SELECT
    ...
    aos.id_firestore,
    aos.ts_event,
    IF(NOT acc.id_firestore IS NULL, acc.final_attribution_app_type, aos.app_type) AS app_type,
    aos.version_name,
    IF(NOT acc.id_firestore IS NULL, acc.final_attribution_source, aos.utm_source) AS utm_source,
    IF(NOT acc.id_firestore IS NULL, acc.final_attribution_medium, aos.utm_medium) AS utm_medium,
    IF(NOT acc.id_firestore IS NULL, acc.final_attribution_campaign, aos.utm_campaign) AS utm_campaign,
    IF(NOT acc.id_firestore IS NULL, acc.final_attribution_content, aos.utm_content) AS utm_content,
    IF(NOT acc.id_firestore IS NULL, acc.final_attribution_term, aos.utm_term) AS utm_term,
    IF(NOT acc.id_firestore IS NULL, acc.final_attribution_origin, 'old_attribution') AS final_attribution_origin,
    IF(
      NOT acc.id_firestore IS NULL,
      acc.final_attribution_branded = 'Branded',
      COALESCE(
        (
          (
          ...
          AND NOT UPPER(utm_campaign) LIKE '%NON-BRANDED%'
        ),
        FALSE
      )
    ) AS flg_branded
  FROM datalake_amplitude_offer.offer_submitted_events AS aos
  LEFT JOIN datalake_tracked_events.attribution_cross_channel AS acc
    ON aos.id_firestore = acc.id_firestore AND acc.event_name = 'offer_submitted'
), offer_submitted_events AS (
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY id_user, id_house ORDER BY id_user, id_house, ts_event) AS rn_without_id_firestore,
    ROW_NUMBER() OVER (PARTITION BY id_firestore ORDER BY id_firestore, ts_event) AS rn_id_firestore,
    (
      NOT id_firestore IS NULL
    ) AS has_id_firestore
  FROM enrich_attribution
), old_pre_proposal AS (
  SELECT
    ...
    CAST(NULL AS STRING) AS id_firestore,
    country_code,
    ...
    NULL AS tenant_type,
    CAST('Other' AS STRING) AS type,
    NULL AS has_pets,
    FALSE AS is_instant_offer,
    NULL AS ts_email_sent_to_owner
  FROM datalake_ebdb_proposal.pre_proposal AS pp
  LEFT JOIN pre_proposal_aud AS pp_aud
    ON pp.id = pp_aud.id
  LEFT JOIN datalake_ebdb_clean.user AS tenant
  ...
  SELECT
    ...
    CAST(id_firestore AS STRING) AS id_firestore,
    country_code,
  ...
  SELECT
    o.*,
    IF(
      NOT ose_id_firestore.id_firestore IS NULL,
      ose_id_firestore.app_type,
      ose_wo_id_firestore.app_type
    ) AS app_type,
    COALESCE(ose_id_firestore.version_name, ose_wo_id_firestore.version_name) AS version_name,
    IF(
      NOT ose_id_firestore.id_firestore IS NULL,
      ose_id_firestore.utm_source,
      ose_wo_id_firestore.utm_source
    ) AS utm_source,
    IF(
      NOT ose_id_firestore.id_firestore IS NULL,
      ose_id_firestore.utm_medium,
      ose_wo_id_firestore.utm_medium
    ) AS utm_medium,
    IF(
      NOT ose_id_firestore.id_firestore IS NULL,
      ose_id_firestore.utm_campaign,
      ose_wo_id_firestore.utm_campaign
    ) AS utm_campaign,
    IF(
      NOT ose_id_firestore.id_firestore IS NULL,
      ose_id_firestore.utm_content,
      ose_wo_id_firestore.utm_content
    ) AS utm_content,
    IF(
      NOT ose_id_firestore.id_firestore IS NULL,
      ose_id_firestore.utm_term,
      ose_wo_id_firestore.utm_term
    ) AS utm_term,
    IF(
      NOT ose_id_firestore.id_firestore IS NULL,
      ose_id_firestore.flg_branded,
      ose_wo_id_firestore.flg_branded
    ) AS flg_branded,
    IF(
      NOT ose_id_firestore.final_attribution_origin IS NULL,
      ose_id_firestore.final_attribution_origin,
      ose_wo_id_firestore.final_attribution_origin
    ) AS final_attribution_origin
  FROM all_offers AS o
  LEFT JOIN offer_submitted_events AS ose_wo_id_firestore
  ...
  LEFT JOIN offer_submitted_events AS ose_id_firestore
  ...
  FROM (
    ...
    */
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY LOWER(app_type), LOWER(utm_source), LOWER(utm_medium), branded = 'Branded' ORDER BY LOWER(app_type), LOWER(utm_source), LOWER(utm_medium), branded = 'Branded', id) AS _w,
      branded
    FROM datalake_gsheets_clean.taxonomy_demand
    WHERE
      first_update_source = 'Inquilinos' AND flg_via_reschedule = 0
  ) AS _t
  WHERE
    _w = 1
)
