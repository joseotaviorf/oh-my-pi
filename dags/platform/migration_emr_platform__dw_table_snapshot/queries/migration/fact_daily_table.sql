...
FROM (
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY COALESCE(ltc.table_name, tu.table_name), COALESCE(COALESCE(ltc.year, tu.year), COALESCE(ltc.year, tu.year)), COALESCE(COALESCE(ltc.month, tu.month), COALESCE(ltc.month, tu.month)), COALESCE(COALESCE(ltc.day, tu.day), COALESCE(ltc.day, tu.day)) ORDER BY cd.ts_last_scheduler_ran DESC) AS _w,
    ltc.table_name,
    cd.ts_last_scheduler_ran
  FROM datalake_databricks.load_table_costs AS ltc
  FULL OUTER JOIN datalake_trino_table_usage.total_table_usage_information AS tu
    USING (table_name, year, month, day)
  JOIN datalake_dag_inventory_clean.table AS dit
  ...
  JOIN datalake_airflow.dag AS cd
    ON cd.id_dag = dit.dag
  JOIN datalake_pipeline.dag AS pd
