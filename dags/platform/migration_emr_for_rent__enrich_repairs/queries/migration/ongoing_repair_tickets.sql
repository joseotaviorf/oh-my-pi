WITH ticket_history_base AS (
  ...
  WHERE
    tc.year >= YEAR(TO_DATE(CURRENT_DATE - INTERVAL '1' YEAR))
    AND tc.group_name IN ('FullService [Back]', 'Prestadores Parceiros [SO]', 'Reparos [BACK]', 'Triagem Reparos [Back]', 'FullService [BACK]', 'ReparAção (Piloto Urgente)', 'Reparos PP Multi [BACK]', 'ReparAção Comum [BACK]', 'Triagem [Porto]', 'Atendimento [Porto]', 'ReparAção Emergencial [BACK]')
...
FROM (
  SELECT
    ...
    CAST(tc.custom_fields['[Data] Definição do prestador'] AS DATE) AS dt_definition,
    fi.dt_chat,
  ...
  WHERE
    tc.group_name IN ('Reparos [BACK]', 'Triagem Reparos [Back]', 'FullService [BACK]', 'Autosserviço Reparos [BACK]', 'ReparAção (Piloto Urgente)', 'Reparos PP Multi [BACK]', 'ReparAção Comum [BACK]', 'Reembolso de Reparos [Back]', 'Triagem [Porto]', 'Atendimento [Porto]', 'ReparAção Emergencial [BACK]')
    AND tc.ts_created >= CURRENT_DATE - INTERVAL '3' YEAR
    AND NOT tc.channel IN ('call', 'whatsapp')
    AND NOT tc.status IN ('deleted')
    AND NOT tc.tags LIKE '%caso_ticket_agregador%'
    AND (
      tc.client_type <> 'prestador_de_serviço' OR tc.client_type IS NULL
    )
    AND NOT tc.tags LIKE '%reembolso_sem_autorização%'
    AND NOT tc.tags LIKE '%whatsapp_reparos%'
) AS _t
WHERE
  _w = 1
