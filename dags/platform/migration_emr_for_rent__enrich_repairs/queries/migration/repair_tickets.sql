WITH ticket_history_base AS (
  SELECT
    ...
    MIN(
      CASE
        ...
        THEN t.ts_updated
      END
    ) AS ts_resolution_contestation
  FROM datalake_zendesk.tickets AS t
  WHERE
    MAKE_DATE(t.year, t.month, t.day) >= CAST('{load_start_date}' AS DATE) - INTERVAL '1' YEAR
    AND t.ts_updated >= CAST('{load_start_date}' AS DATE) - INTERVAL '1' YEAR
  GROUP BY ALL
), repair_tickets AS (
  SELECT DISTINCT
    tc.id_ticket,
    CASE
      WHEN (
        EXISTS(
          ('%iq_pp_autosserviço_contestou%', '%pp_autosserviço_contestou%'),
          x -> tc.tags LIKE x
        )
      )
      THEN 'PWA'
      ...
      AND NOT tc.tags LIKE '%ticket_acompanhamento%'
      THEN 'CX'
    END AS contestation_task_origin,
    IF(tc.tags LIKE '%closed_by_merge%', TRUE, FALSE) AS is_closed_by_merge,
    IF(
      ww.dt_end_1 > (
        CAST('{load_start_date}' AS DATE) - INTERVAL '1' DAY
      )
      AND ts_resolution_contestation IS NULL
      AND CAST(tc.ts_solved - INTERVAL '3' HOUR AS DATE) IS NULL,
      TRUE,
      FALSE
    ) AS is_contestation_backlog,
  ...
  INNER JOIN ticket_history_base AS th
    ON th.id_ticket = tc.id_ticket
  LEFT JOIN datalake_date.workday_window AS ww
    ON ww.dt_Ref = CAST(th.ts_contestation AS DATE) AND id_city = 39
  WHERE
    tc.year >= YEAR(TO_DATE(CAST('{load_start_date}' AS DATE) - INTERVAL '1' YEAR))
    AND tc.group_name IN ('FullService [Back]', 'Reparos [BACK]', 'Triagem Reparos [Back]', 'FullService [BACK]', 'ReparAção (Piloto Urgente)', 'ReparAção Comum [BACK]', 'Reparos PP Multi [BACK]', 'Reembolso de Reparos [Back]', 'Triagem [Porto]', 'Atendimento [Porto]')
    AND NOT tc.channel IN ('call')
    AND NOT tc.status IN ('deleted')
    AND NOT tc.tags LIKE '%caso_ticket_agregador%'
), relisting_db AS (
  SELECT
    rf.id_contract,
  ...
  LEFT JOIN datalake_ebdb_listing.house AS h
    ON rf.id_house = h.id
  WHERE
    hl.ts_listing_version_start BETWEEN CAST('{load_start_date}' AS DATE) - INTERVAL '3' YEAR AND CAST('{load_start_date}' AS DATE) - 1
    AND (
      hl.listing_category = 'Re-Listing'
    )
    AND (
      (
        h.country_code <> 'MX'
      ) OR (
        h.country_code IS NULL
      )
    )
  ...
  FROM (
    SELECT
      ...
      rr.id_contract,
      rr.ts_updated,
      ROW_NUMBER() OVER (PARTITION BY rr.id, rr.id_third_party_crm_ticket_external ORDER BY rr.ts_updated ASC) AS _w,
      rr.id,
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY sr.id_repair_request, ri.id_contract ORDER BY sr.id_third_party_crm_ticket_external ASC) AS _w,
      ri.id_contract
    FROM datalake_repairs_clean.service_request AS sr
    LEFT JOIN repairs_interaction AS ri
      ON ri.id_request = sr.id_repair_request
    WHERE
      CAST(sr.ts_created AS DATE) >= CAST('{load_start_date}' AS DATE) - INTERVAL '1' YEAR
      AND NOT sr.id_third_party_crm_ticket_external IS NULL
  ) AS _t
  WHERE
    _w = 1
), service_provider AS (
  ...
  FROM (
    SELECT
      ...
      rr.service_provider,
      rr.ts_created,
      ROW_NUMBER() OVER (PARTITION BY rr.id_third_party_crm_ticket_external ORDER BY rr.ts_updated DESC) AS _w,
      rr.id_third_party_crm_ticket_external,
      rr.ts_updated
    FROM datalake_repairs_clean.repair_request AS rr
  ) AS _t
  ...
  SELECT
    sr.id_third_party_crm_ticket_external,
    IF(NOT rc.ts_started IS NULL, TRUE, FALSE) AS has_chat_negociation,
    CASE
      WHEN ri.ts_updated <= rc.ts_started
      THEN (
        ri.ts_updated - INTERVAL '3' HOUR
      )
      WHEN ri.ts_updated > rc.ts_started
      THEN (
        rc.ts_started - INTERVAL '3' HOUR
      )
      ELSE COALESCE(ri.ts_updated - INTERVAL '3' HOUR, rc.ts_started - INTERVAL '3' HOUR)
    END AS ts_first_interaction,
    CAST(rc.ts_started - INTERVAL '3' HOUR AS DATE) AS dt_chat
  FROM datalake_repairs_clean.service_request AS sr
  LEFT JOIN datalake_repairs_clean.repair_request AS rr
    ON rr.id = sr.id_repair_request
  LEFT JOIN datalake_repairs_clean.repair_request_chat AS rc
...
FROM (
  SELECT
    ...
    CAST(tc.id_contract AS INT) AS id_contract,
    tc.group_name,
    tc.client_type,
    tc.status,
    ...
    CASE
      WHEN tc.analyst_organization = 'atn'
      THEN 'atento'
      WHEN tc.analyst_organization = 'atento'
      THEN 'atento'
      WHEN tc.analyst_organization = 'webhelp'
      THEN 'webhelp'
      WHEN tc.analyst_organization = 'webhelpbr'
      THEN 'webhelp'
      WHEN tc.analyst_organization = 'quintoandar.com'
      THEN 'quintoandar'
      WHEN tc.analyst_organization = 'quintoandar'
      THEN 'quintoandar'
      WHEN tc.analyst_organization = 'contractors'
      THEN 'webhelp'
    ...
    CASE WHEN tc.status = 'closed' THEN tc.ts_updated - INTERVAL '3' HOUR ELSE NULL END AS ts_closed_local,
    ...
    YEAR(TO_DATE(tc.ts_updated - INTERVAL '3' HOUR)) AS year,
    MONTH(TO_DATE(tc.ts_updated - INTERVAL '3' HOUR)) AS month,
    DAY(TO_DATE(tc.ts_updated - INTERVAL '3' HOUR)) AS day,
    ROW_NUMBER() OVER (PARTITION BY tc.id_ticket ORDER BY tc.ts_updated DESC) AS _w,
    tc.ts_updated
  FROM datalake_zendesk.tickets AS tck
  ...
  LEFT JOIN datalake_repairs_clean.service_request AS sr
    ON sr.id_third_party_crm_ticket_external = tc.id_ticket
  LEFT JOIN service_provider AS rr
    ON sr.id_repair_request = rr.id_repair_request
  LEFT JOIN repairs_interaction AS ri
    ON ri.id_request = rr.id_repair_request
  LEFT JOIN relisting_distinct AS rd
    ON rd.id_contract = tc.id_contract
  LEFT JOIN datalake_survicate.repairs_surveys AS csat
    ON csat.id_ticket = tc.id_ticket
  LEFT JOIN datalake_repairs_clean.repair_request AS rr_first_interaction
    ON tc.id_ticket = rr_first_interaction.id_third_party_crm_ticket_external
    AND NOT rr_first_interaction.id_third_party_crm_ticket_external IS NULL
  LEFT JOIN first_interaction AS fi
    ON fi.id_third_party_crm_ticket_external = tc.id_ticket
  LEFT JOIN repair_interaction_pp AS rpp
    ON rpp.id_third_party_crm_ticket_external = tc.id_ticket
  LEFT JOIN repair_tickets AS th
  ...
  WHERE
    MAKE_DATE(tck.year, tck.month, tck.day) >= CAST('{load_start_date}' AS DATE) - INTERVAL '1' YEAR
    AND tck.ts_updated >= CAST('{load_start_date}' AS DATE) - INTERVAL '1' YEAR
    AND tck.group_name IN ('Reparos [BACK]', 'Triagem Reparos [Back]', 'FullService [BACK]', 'Autosserviço Reparos [BACK]', 'ReparAção (Piloto Urgente)', 'Reparos PP Multi [BACK]', 'ReparAção Comum [BACK]', 'Reembolso de Reparos [Back]', 'Triagem [Porto]', 'Atendimento [Porto]', 'ReparAção Emergencial [BACK]')
    AND (
      tc.ts_created >= CAST('{load_start_date}' AS DATE) - INTERVAL '6' MONTH
      OR tc.ts_solved >= CAST('{load_start_date}' AS DATE) - INTERVAL '2' YEAR
