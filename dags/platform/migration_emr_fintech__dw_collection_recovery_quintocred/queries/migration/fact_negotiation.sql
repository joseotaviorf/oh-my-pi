WITH trato_feito_dt_paid AS (
  SELECT
    i.id_negotiation,
    CAST(MIN(p.dt_paid) AS DATE) AS dt_paid
  FROM datalake_trato_feito_clean.payment AS p
  LEFT JOIN datalake_debt_recovery.installment AS i
  ...
  FROM (
    SELECT
      ...
      id_contract,
      CASE
        WHEN debtor = 'rental_contract_landlord'
        THEN 'PP QuintoAndar'
        WHEN debtor = 'rental_contract_tenant'
        THEN 'IQ QuintoAndar'
        WHEN debtor = 'velo_delinquency_tenant'
        THEN 'IQ QuintoCred'
      END AS creditor,
      is_contract_recurrent_debtor,
      status,
      ...
      IF(NOT ts_first_payment IS NULL, TRUE, FALSE) AS is_down_payment_paid,
      qt_installments,
      negotiation_original_amount,
      interest_fee_amount,
      fine_fee_amount,
      ...
      ROUND(
        negotiation_original_amount + fine_fee_amount + interest_fee_amount + credit_card_fee_amount,
        2
      ...
      GREATEST(
        ROUND(
          (
            negotiation_original_amount + fine_fee_amount + interest_fee_amount
          ) - total_expected_amount,
          2
        ),
        0
      ) AS discount_amount_without_adm_fee, /* Total debt without credit card fee (debt_amount_without_adm_fee = original + fine + fee) - Negotiated amount (total_expected_amount) */
      total_expected_amount AS negotiated_amount,
      down_payment_amount,
      paid_amount,
      CAST(qt_installments_paid AS INT) AS qt_installments_paid,
      IF(
        NOT breached_installment IS NULL,
        CAST(qt_installments AS INT) - CAST(qt_installments_paid AS INT),
        0
      ) AS breached_installments,
      dt_expected_end,
      CAST(ts_created_at AS DATE) AS dt_promisse,
      CAST(ts_first_payment AS DATE) AS dt_first_payment,
      CAST(ts_paid_all AS DATE) AS dt_paid_all,
      CAST(ts_breach AS DATE) AS dt_breach,
      ROW_NUMBER() OVER (PARTITION BY id_contract, id_negotiation_external ORDER BY ts_created_at DESC) AS _w,
      id_negotiation_external,
      ts_created_at
    FROM datalake_debt_recovery.negotiation
  ) AS _t
  WHERE
    _w = 1 /* removes the exception in which 1 Trato-Feito negotiation ID has more than one Recupera negotiation ID. Ex: 97080 */
), creditor_pending AS (
  SELECT
    id_negotiation,
    id_invoice,
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY id_installment, installment_code ORDER BY dt_table_insertion DESC, year DESC, month DESC, day DESC) AS _w,
      id_installment,
      installment_code,
      dt_table_insertion,
      year,
  ...
  FROM (
    SELECT
      ...
      customer_document,
      CASE
        WHEN negotiation_status = 'ACORDO_LIQUIDADO'
        THEN 'finished'
        WHEN negotiation_status = 'ACORDO_CANCELADO' AND down_payment IS TRUE
        THEN 'broken'
        WHEN negotiation_status = 'ACORDO_CANCELADO' AND down_payment IS FALSE
        THEN 'canceled'
        WHEN negotiation_status = 'ACORDO_EM_ANDAMENTO' AND down_payment IS TRUE
        THEN 'offset'
      ...
      GREATEST(
        ROUND(
          expense_amount - original_debt_amount - interest_fee_amount - adm_fee_amount - negotiation_discount_amount,
          2
        ),
        0
      ) AS fine_fee_amount,
      ...
      GREATEST(ROUND((
        expense_amount - adm_fee_amount
      ) - negotiated_amount, 2), 0) AS discount_amount_without_adm_fee, /* Negotiated amount (negotiated_amount) - debt amount without credit card fee (debt_amount_without_adm_fee = original + fine + fee = expense_amount - adm_fee_amount) */
      negotiated_amount,
      down_payment_amount,
      down_payment_amount_without_fees,
      total_amount_paid,
      ...
      dt_down_payment,
      dt_paid_all,
      ROW_NUMBER() OVER (PARTITION BY id_negotiation ORDER BY ts_snapshot DESC, id_contract DESC) AS _w,
      ts_snapshot
  ...
  SELECT
    id_negotiation,
    is_special_installment,
    MIN(id_contract) AS id_contract,
    CASE
      WHEN COUNT(DISTINCT id_contract) > 1
      THEN TRUE
      WHEN is_special_installment IS TRUE
      THEN TRUE
      ELSE FALSE
    END AS is_invalid_negotiation,
    CASE
      WHEN COUNT(DISTINCT id_contract) > 1
      THEN 'Multiple contracts included in negotiation'
      WHEN is_special_installment IS TRUE
      THEN 'Special Installment'
      ELSE NULL
  ...
  FROM (
    SELECT DISTINCT
      ...
      CAST(ad.dt_emission AS DATE) AS dt_promisse,
      ROW_NUMBER() OVER (PARTITION BY d.id_contract_quintoandar, ad.id_installment ORDER BY ad.ts_update DESC) AS _w,
      d.id_contract_quintoandar,
      ad.id_installment,
      ad.ts_update
  ...
  SELECT
    CAST(COALESCE(tfn.id_negotiation_recupera, rn.id_negotiation) AS STRING) AS sk_negotiation,
    ...
    COALESCE(
      tfn.creditor,
      CASE
        WHEN rn.id_creditor IN (1, 4, 7, 8, 9)
        THEN 'IQ QuintoAndar'
        WHEN rn.id_creditor IN (3, 5)
        THEN 'IQ QuintoCred'
        WHEN rn.id_creditor IN (2, 6)
        THEN 'PP QuintoAndar'
      END
    ) AS creditor,
    ...
    CASE
      WHEN COALESCE(tfn.status, rn.negotiation_status) = 'canceled'
      THEN 'PROMESSA QUEBRADA'
      WHEN oi.total_invoices_negotiated >= 1 AND number_of_installments > 1
      THEN 'ACORDO'
      WHEN oi.total_invoices_negotiated > 1 AND number_of_installments = 1
      THEN 'QUITAÇÃO'
      WHEN oi.total_invoices_negotiated = 1 AND number_of_installments = 1
      THEN 'SUBSTITUIÇÃO'
      WHEN COALESCE(tfn.status, rn.negotiation_status) = 'started'
      THEN 'PROMESSA'
      ELSE 'INDEFINIDO'
    ...
    COALESCE(
      tfn.fine_fee_amount,
      IF(invn.is_invalid_negotiation IS TRUE, rn.original_fine_amount, rn.fine_fee_amount),
      0
    ) AS fine_fee_amount,
    ...
    NOW() AS ts_load
  FROM recupera_negotiation AS rn
  FULL OUTER JOIN trato_feito_negotiation AS tfn
  ...
  LEFT JOIN trato_feito_dt_paid AS p
...
SELECT
  ...
  CASE
    WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(dt_due_invoice_anchor)) <= 0
    THEN 'Current'
    WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(dt_due_invoice_anchor)) <= 30
    THEN '1-30'
    WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(dt_due_invoice_anchor)) <= 60
    THEN '31-60'
    WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(dt_due_invoice_anchor)) <= 90
    THEN '61-90'
    WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(dt_due_invoice_anchor)) <= 120
    THEN '91-120'
    WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(dt_due_invoice_anchor)) <= 180
    THEN '121-180'
    WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(dt_due_invoice_anchor)) IS NULL
    THEN NULL
    ELSE 'over 180'
  ...
  ROUND(
    ...
    2
  ) AS discount_to_original_amount,
  negotiated_amount,
  negotiated_to_be_due_amount,
  negotiated_overdue_amount,
  down_payment_amount,
  ...
  paid_amount,
  total_next_due,
  DATEDIFF(TO_DATE(dt_down_payment), TO_DATE(dt_due_invoice_anchor)) AS sla_debt_anchor_to_promisse_payment,
  DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(dt_due_invoice_anchor)) AS sla_debt_anchor_to_promisse,
  DATEDIFF(TO_DATE(dt_ending), TO_DATE(dt_down_payment)) AS sla_promisse_payment_to_negotiation_ending,
...
LEFT JOIN paschoalotto_operator AS po
  ON UPPER(u.id_operator) LIKE '%PASCH%'
