WITH deduplicate_creditor_pending AS (
  ...
  FROM (
    SELECT DISTINCT
      ...
      ROW_NUMBER() OVER (PARTITION BY id_installment ORDER BY dt_table_insertion DESC, ts_load DESC) AS _w,
      id_installment,
      dt_table_insertion,
      ts_load
  ...
  FROM (
    SELECT DISTINCT
      ...
      id_contract,
      id_installment AS id_invoice,
      ROW_NUMBER() OVER (PARTITION BY id_installment ORDER BY ts_last_debt_update DESC) AS _w,
      id_installment,
      ts_last_debt_update
  ...
  FROM (
    ...
    WHERE
      NOT IF(ASCII(TRIM(installment_code)) = 0, NULL, installment_code) IS NULL
  ) AS _t
  WHERE
    _w = 1
  ...
  FROM (
    SELECT DISTINCT
      ...
      CASE
        ...
        THEN 'IQ QuintoAndar'
        WHEN n.debtor = 'velo_delinquency_tenant'
        THEN 'IQ QuintoCred'
      END AS creditor,
      ...
      ROW_NUMBER() OVER (PARTITION BY d.id_external ORDER BY d.ts_created DESC) AS _w,
      d.id_external,
      d.ts_created
  ...
  SELECT DISTINCT
    ...
    CASE
      WHEN COALESCE(cp.id_creditor, cr.id_creditor, crwd.id_creditor) IN (1, 4, 7, 8, 9)
      THEN 'IQ QuintoAndar'
      WHEN COALESCE(cp.id_creditor, cr.id_creditor, crwd.id_creditor) IN (3, 5)
      THEN 'IQ QuintoCred'
      WHEN COALESCE(cp.id_creditor, cr.id_creditor, crwd.id_creditor) IN (2, 6)
      THEN 'PP QuintoAndar'
    END AS creditor,
  ...
  FULL OUTER JOIN deduplicate_complementary_records AS cr
    ON cp.id_contract = cr.id_contract AND cp.id_invoice = cr.id_invoice
  FULL OUTER JOIN deduplicate_complementary_records_written_down AS crwd
    ON cp.id_contract = crwd.id_contract AND cp.id_invoice = crwd.id_invoice
), debts AS (
  SELECT
  ...
  FULL OUTER JOIN recupera_debts AS rd
    ON rd.id_contract = tfd.id_contract AND rd.id_invoice = tfd.id_invoice
  ...
  SELECT
    ...
    CAST(COALESCE(o.dt_due, o.dt_due_legacy) AS DATE) AS dt_invoice_due,
    CAST(o.ts_paid AS DATE) AS dt_invoice_paid
...
LEFT JOIN union_quintoandar_quintocred AS u
  ON d.id_invoice = u.id_invoice AND d.creditor = u.creditor
