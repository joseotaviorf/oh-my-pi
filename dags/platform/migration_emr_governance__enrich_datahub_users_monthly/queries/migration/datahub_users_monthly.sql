...
WITH bounds AS (
  ...
  SELECT
    ...
    dt_terminated,
    CASE
      WHEN chapter = 'Data' OR directorate = 'Dados'
      THEN 'Tech - Data'
  ...
  WHERE
    NOT e.dt_hired IS NULL
    AND e.dt_hired <= b.month_end
    AND (
      e.dt_terminated IS NULL OR e.dt_terminated >= b.month_start
    )
  ...
  WHERE
    ...
    AND (
      dt_deleted IS NULL OR CAST(dt_deleted AS DATE) >= b.month_start
    )
