WITH partner_agencies_aux AS (
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY REPLACE(
        ...
        ' ',
        ''
      ) ORDER BY ts_updated DESC) AS _w
    FROM datalake_hubspot_clean.company
  ) AS _t
  WHERE
    _w = 1 AND NOT extracted_3p_tag IS NULL
), partner_agencies AS (
  SELECT
    ...
    paa.partner_state,
    paa.ts_updated
  FROM partner_agencies_aux AS paa
  LEFT JOIN partner_agencies_aux AS current_paa
  ...
  SELECT
    ...
    COALESCE(
      pa.current_tag,
      NULLIF(REGEXP_EXTRACT(wc.contract_name, '(?i)(?<=\\[3P\\-)(.+?)(?=\\])'), '')
    ) AS 3p_partner,
    wc.contract_name LIKE '%[3P-%]%' OR wc.contract_name LIKE '%[3p-%]%' AS is_3p_contract,
    wc.contract_name LIKE '%[3P-%]%'
    OR wc.contract_name LIKE '%[3p-%]%'
    AND partner_state IS DISTINCT FROM 'MG' AS is_3p_5a_contract,
    wc.contract_name LIKE '%[3P-%]%'
    OR wc.contract_name LIKE '%[3p-%]%'
    AND partner_state IS NOT DISTINCT FROM 'MG' AS is_3p_bh_contract,
    wc.ts_created,
    wc.ts_updated
  FROM datalake_ebdb_clean.work_contract AS wc
  LEFT JOIN datalake_gsheets_clean.sale_business_unit_standardization AS bus /* kept this source only for historical purpose. */
    ON bus.hub_name_wc = wc.contract_name
  LEFT JOIN partner_agencies AS pa
    ON REPLACE(
      UPPER(NULLIF(REGEXP_EXTRACT(wc.contract_name, '(?i)(?<=\\[3P\\-)(.+?)(?=\\])'), '')),
      ' ',
      ''
    ) = pa.extracted_3p_tag
...
LEFT JOIN hubspot_company_name_history AS ch
