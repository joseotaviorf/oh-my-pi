WITH bill_items_validation AS (
  ...
  WHERE
    ...
    AND (
      (
        LOWER(bill_item_description) LIKE '%ação boletos%'
      )
      OR (
        LOWER(bill_item_description) LIKE '%liminar judicial%'
      )
    )
  ...
  SELECT
    ...
    CAST(i.ts_created AS DATE) AS dt_invoice_creation,
  ...
  SELECT
    ...
    id_invoice,
    status,
    dt_due_oldest_invoice,
    dt_oldest_month_write_off,
    is_invoice_fraudulent
  FROM (
    SELECT
      i.id_contract_external AS id_contract,
      i.id_external AS id_invoice,
      ...
      IF(NOT fi.id_invoice IS NULL, TRUE, FALSE) AS is_invoice_fraudulent,
      ROW_NUMBER() OVER (PARTITION BY i.id_contract_external ORDER BY i.dt_due_adjusted) AS _w,
    ...
    WHERE
      i.status = 'open' OR NOT fi.id_invoice IS NULL
  ) AS _t
  WHERE
    _w = 1
)
SELECT
  ...
  i.due_amount,
  i.accrual_year_month,
  DATEDIFF(TO_DATE(c.dt_oldest_month_write_off), TO_DATE(c.dt_due_oldest_invoice)) AS delay_contamined_days,
  CASE
    WHEN DATEDIFF(TO_DATE(c.dt_oldest_month_write_off), TO_DATE(c.dt_due_oldest_invoice)) IS NULL
    THEN NULL
    WHEN DATEDIFF(TO_DATE(c.dt_oldest_month_write_off), TO_DATE(c.dt_due_oldest_invoice)) <= 0
    THEN 'a. Current'
    WHEN DATEDIFF(TO_DATE(c.dt_oldest_month_write_off), TO_DATE(c.dt_due_oldest_invoice)) <= 30
    THEN 'b. 1-30'
    WHEN DATEDIFF(TO_DATE(c.dt_oldest_month_write_off), TO_DATE(c.dt_due_oldest_invoice)) <= 60
    THEN 'c. 31-60'
    WHEN DATEDIFF(TO_DATE(c.dt_oldest_month_write_off), TO_DATE(c.dt_due_oldest_invoice)) <= 90
    THEN 'd. 61-90'
    WHEN DATEDIFF(TO_DATE(c.dt_oldest_month_write_off), TO_DATE(c.dt_due_oldest_invoice)) <= 120
    THEN 'e. 91-120'
    WHEN DATEDIFF(TO_DATE(c.dt_oldest_month_write_off), TO_DATE(c.dt_due_oldest_invoice)) <= 150
    THEN 'f. 121-150'
    WHEN DATEDIFF(TO_DATE(c.dt_oldest_month_write_off), TO_DATE(c.dt_due_oldest_invoice)) <= 180
    THEN 'g. 151-180'
    ELSE 'h. over 180'
