WITH opa_filtered AS (
  ...
  FROM (
    SELECT
      ...
      day,
      hour,
      ROW_NUMBER() OVER (PARTITION BY id_request, app ORDER BY ts_event DESC) AS _w
    FROM datalake_access_logs_clean.istio
    WHERE
      MAKE_DATE(year, month, day) BETWEEN '{load_start_date}' AND '{load_end_date}'
  ) AS _t
  WHERE
...
SELECT
  ...
  opa.day,
  opa.hour
FROM opa_filtered AS opa
LEFT JOIN istio_filtered AS istio
  ON opa.id_request = istio.id_request AND opa.app = istio.app
