WITH records_distributed_channels AS (
  SELECT
    id_creditor,
    id_customer,
    id_digital_channel,
    count_multiples_records
  FROM (
    SELECT
      id_creditor,
      id_customer,
      id_digital_channel,
      COUNT(*) OVER (PARTITION BY id_creditor, id_customer) AS count_multiples_records,
      ROW_NUMBER() OVER (PARTITION BY id_creditor, id_customer ORDER BY CAST(ts_current_registration AS DATE) DESC) AS _w,
      ts_current_registration
  ...
  FROM (
    SELECT
      ...
      REPLACE(
        REPLACE(REPLACE(hr.occurence_description, CHR(124), ' - '), CHR(13), ''),
        CHR(10),
        ''
      ) AS observation,
      hr.ts_occurrence,
      ...
      CASE
        WHEN rdc.count_multiples_records > 1
        THEN 'AMBOS'
        ELSE rdc.id_digital_channel
      END AS digital_channel,
      ROW_NUMBER() OVER (PARTITION BY hr.id_creditor, hr.id_customer, cr.id_installment ORDER BY hr.ts_occurrence DESC) AS _w,
      cr.id_installment
    FROM operational_records AS opr
    INNER JOIN datalake_recupera_clean.complementary_records AS cr
      ON opr.id_creditor = cr.id_creditor AND opr.id_customer = cr.id_customer
    ...
    LEFT JOIN datalake_recupera_clean.historical_records AS hr
      ON opr.id_creditor = hr.id_creditor AND opr.id_customer = hr.id_customer
    LEFT JOIN datalake_recupera_clean.installments_indicators AS ii
...
SELECT DISTINCT
  ...
  COALESCE(i.amount_to_pay, 0) AS deal_amount,
...
SELECT DISTINCT
  ...
  DAY(TO_DATE(NOW())) AS day
FROM collection_base AS cb
LEFT JOIN datalake_recupera_clean.installment AS i
  ON i.id_installment = cb.installment_code
WHERE
  cb.id_creditor IN ('1', '3', '5') AND cb.installment_code IS NULL
