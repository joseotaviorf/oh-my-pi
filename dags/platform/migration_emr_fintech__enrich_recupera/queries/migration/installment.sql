WITH negotiation_status AS (
  ...
  FROM (
    SELECT
      ...
      id_installment,
      dt_expiration_installment_agreement,
      ROW_NUMBER() OVER (PARTITION BY id_installment, id_customer, id_contract, dt_expiration_installment_agreement ORDER BY MAKE_DATE(year, month, day) DESC) AS _w,
      year,
      month,
      day
  ...
  FROM (
    SELECT
      id_installment,
      ts_canceled_installment,
      ROW_NUMBER() OVER (PARTITION BY id_installment ORDER BY MAKE_DATE(year, month, day) DESC) AS _w,
      year,
      month,
      day
...
SELECT
  CAST(i.id_installment AS STRING) AS id_negotiation,
  COALESCE(isd.id_contract, cd.id_contract) AS id_contract,
  i.id_operator,
  i.id_customer AS customer_document,
  CASE
    WHEN CAST(i.installment_number AS INT) = 0
    AND LOWER(i.installment_situation) = 'parcela paga'
    THEN CAST(i.id_installment AS INT)
    ELSE NULL
  END AS id_first_installment_paid,
  CASE
    WHEN LOWER(i.installment_situation) = 'parcela paga'
    THEN i.id_customer
    ELSE NULL
  END AS id_paid_customer_document,
  i.receipt_code AS id_receipt,
  CASE
    WHEN i.id_creditor IN (1, 4, 7, 8, 9)
    THEN 'IQ QuintoAndar'
    WHEN i.id_creditor IN (3, 5)
    THEN 'IQ QuintoCred'
    WHEN i.id_creditor IN (2, 6)
    THEN 'PP QuintoAndar'
  END AS creditor,
  i.is_special_installment,
  ci.indicator_content AS contract_status,
  CASE
    WHEN LOWER(i.installment_situation) = 'parcela em aberta'
    THEN 'Em aberto'
    WHEN LOWER(i.installment_situation) = 'parcela paga'
    THEN 'Pago'
    WHEN LOWER(i.installment_situation) = 'acordo cancelado' AND dm.dt_paid IS NULL
    THEN 'Quebrado'
    WHEN LOWER(i.installment_situation) = 'acordo cancelado' AND NOT dm.dt_paid IS NULL
    THEN 'Pago'
    ELSE NULL
  ...
  (
    i.installments_amount + 1
  ) AS adjusted_number_of_installments,
  i.installment_number + 1 AS installment_number,
  DATEDIFF(TO_DATE(i.dt_installment), TO_DATE(cp.dt_due)) AS delay_days,
  i.main_amount,
  ...
  i.dt_due,
  dm.dt_paid,
  CAST(cd.ts_canceled_installment AS DATE) AS dt_canceled,
  CURRENT_DATE - 1 AS dt_base
FROM datalake_recupera_clean.installment AS i
LEFT JOIN deduplicate_installment_detail AS isd
  ON i.id_installment = isd.id_installment
  AND i.dt_due = isd.dt_expiration_installment_agreement
LEFT JOIN contract_data AS cd
  ON i.id_creditor = cd.id_creditor AND i.id_customer = cd.id_customer
LEFT JOIN negotiation_status AS ns
...
LEFT JOIN deduplicated_installment_canceled AS cd
  ON cd.id_installment = i.id_installment
LEFT JOIN creditor_pending AS cp
  ON cp.id_installment = i.id_installment
LEFT JOIN datalake_recupera_clean.indicator_contracts AS ci
...
WHERE
  i.dt_installment <= CURRENT_DATE - 1 AND i.is_installment_active IS TRUE
