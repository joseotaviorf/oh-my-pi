WITH repair_exempted AS (
  ...
  FROM (
    SELECT
      ...
      re.is_exempted,
      re.ts_granted,
      MAX(re.ts_granted) OVER (PARTITION BY re.id_repair_request) AS _w
...
SELECT
  ...
  COUNT(DISTINCT rr.id_repair_request) FILTER(WHERE
    r.reviewer_type = 'ADMIN') AS total_repair_request_by_analyst,
  COUNT(DISTINCT rr.id_repair_request) FILTER(WHERE
    r.reviewer_type = 'OWNER') AS total_repair_request_by_owner,
  COUNT(DISTINCT c.id_repair_request) AS total_repair_contested,
  COUNT(DISTINCT re.id_repair_request) FILTER(WHERE
    re.is_exempted IS TRUE) AS total_repair_exempted,
...
LEFT JOIN datalake_inspection_services_clean.repair_request AS rr
  ON rr.id_reviewer = r.id_reviewer
LEFT JOIN datalake_inspection_services_clean.contestation AS c
  ON c.id_reviewer = r.id_reviewer
LEFT JOIN repair_exempted AS re
  ON re.id_granted_by = r.id_reviewer
LEFT JOIN datalake_inspections.report_approval AS ra
  ON ra.id_assessment = r.id_assessment
LEFT JOIN datalake_amplitude_inspections.inspection_stages_access AS ira
  ON ira.id_inspection = r.id_inspection
LEFT JOIN datalake_inspections.inspection_status_change AS is_status
