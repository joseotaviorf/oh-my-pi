/* a channel maps to many sessions over time; pick the one whose active window */ /* contains the message timestamp so each id_message resolves to a single session */ /* and the fan-out across all of a channel's sessions no longer duplicates messages */
WITH whatsapp_channel_session AS (
  ...
  FROM (
    SELECT
      ce.id AS id_message,
      c.id_session,
      ROW_NUMBER() OVER (PARTITION BY ce.id ORDER BY CASE WHEN ce.ts_created BETWEEN c.ts_created AND c.ts_updated THEN 0 ELSE 1 END, c.ts_created DESC) AS session_rank
    FROM datalake_quinto_messenger_clean.channel_event AS ce
    INNER JOIN datalake_quinto_messenger_clean /* The addition of this source and date validation was due to a migration by the engineering team. */ /* In the future, we will no longer need the channel base as a source. */.channel AS c
      ON c.id_channel = ce.id_channel AND ce.ts_created < '2026-02-23T14:00:00.000+00:00'
    WHERE
  ...
  FROM (
    SELECT
      ce.id AS id_message,
      chat.id_session,
      ROW_NUMBER() OVER (PARTITION BY ce.id ORDER BY CASE
        WHEN ce.ts_created BETWEEN chat.ts_created AND chat.ts_updated
        THEN 0
        ELSE 1
      END, chat.ts_created DESC) AS session_rank
    FROM datalake_quinto_messenger_clean.channel_event AS ce
    INNER JOIN datalake_quinto_messenger_clean.chat AS chat
  ...
  FROM (
    SELECT
      ...
      chat2.id_session,
      ROW_NUMBER() OVER (PARTITION BY ce.id ORDER BY CASE
        WHEN ce.ts_created BETWEEN chat2.ts_created AND chat2.ts_updated
        THEN 0
        ELSE 1
      END, chat2.ts_created DESC) AS session_rank
    FROM datalake_quinto_messenger_clean.channel_event AS ce
    INNER JOIN datalake_quinto_messenger_clean.chat AS chat2
  ...
  SELECT DISTINCT
    ...
    END AS user_type
  FROM datalake_quinto_messenger_clean.channel_event AS ce
  LEFT JOIN whatsapp_channel_session AS wchannel
    ON wchannel.id_message = ce.id
  LEFT JOIN whatsapp_sauron_session AS wsauron
  ...
  LEFT JOIN datalake_quinto_messenger_clean.chat AS c
    ON c.id_channel = icm.id_channel AND c.source = 'sauron'
  LEFT JOIN datalake_quinto_messenger_clean.chat AS css
    ON css.id_channel = icm.id_channel AND css.source = 'support_session'
  WHERE
  ...
  SELECT DISTINCT
    ...
    m.ts_created
  FROM messages AS m
  LEFT JOIN datalake_sauron_clean.session AS s_num
    ON s_num.id = TRY_CAST(m.id_sauron_session AS BIGINT)
  LEFT JOIN datalake_sauron_clean.session AS s_hash
    ON s_hash.public_id = m.id_sauron_session
  LEFT JOIN datalake_support_session_service_clean.support_session AS ss
  ...
  WHERE
    NOT COALESCE(m.id_sauron_session, m.id_sss_session) IS NULL
), spoc_session AS (
  SELECT
    c.id_session,
    c.id_sss_session,
  ...
  FROM (
    SELECT
      ...
      mw.ts_created,
      CASE
        WHEN NOT LAG(mw.ts_created) OVER (PARTITION BY COALESCE(mw.id_sauron_session, mw.id_sss_session) ORDER BY mw.ts_created) IS NULL
        ...
        THEN TIMESTAMPDIFF(
          SECOND,
          TO_DATE(
            LAG(mw.ts_created) OVER (PARTITION BY COALESCE(mw.id_sauron_session, mw.id_sss_session) ORDER BY mw.ts_created)
          ),
          TO_DATE(mw.ts_created)
        )
        ELSE NULL
      END AS reply_time,
      ...
      ROW_NUMBER() OVER (PARTITION BY mw.id_message ORDER BY COALESCE(c_cast.ts_created, c_hash.ts_created, sss.ts_created) DESC) AS _w
    FROM messages_w_users AS mw
    LEFT JOIN datalake_customer_support.chats AS c_cast
    ...
    LEFT JOIN datalake_customer_support.chats AS c_hash
    ...
    LEFT JOIN datalake_customer_support.chats AS sss
    ...
    LEFT JOIN spoc_session AS ss
      ON ss.id_session = mw.id_sauron_session OR ss.id_sss_session = mw.id_sss_session
    WHERE
  ...
  FROM (
    SELECT DISTINCT
      ...
      m.message,
      CASE
        WHEN m.role LIKE 'HUMAN'
        THEN 'User'
        WHEN m.role LIKE 'ANALYST'
        THEN 'Analyst'
        WHEN m.role LIKE 'SYSTEM'
        THEN 'Bot'
        ELSE m.role
      END AS user_type,
      m.conversation_type,
      ...
      ROW_NUMBER() OVER (PARTITION BY m.id_message ORDER BY COALESCE(c_cast.ts_created, c_hash.ts_created, c_sss.ts_created) DESC) AS _w
    FROM datalake_chatbot.messages AS m
    LEFT JOIN datalake_customer_support.chats AS c_cast
    ...
    LEFT JOIN datalake_customer_support.chats AS c_hash
      ...
      AND MAKE_DATE(c_hash.year, c_hash.month, c_hash.day) >= '{load_start_date}'
    LEFT JOIN datalake_customer_support.chats AS c_sss
...
FROM (
  SELECT
    ...
    ts_task_twilio_created,
    NOW() AS ts_load,
    ROW_NUMBER() OVER (PARTITION BY id_message ORDER BY ts_created DESC) AS _w,
    id_message
  FROM all_messages_with_tasks
) AS _t
WHERE
  _w = 1