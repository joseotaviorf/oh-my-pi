WITH greenseer_uniques_ranked AS (
  ...
  FROM datalake_greenseer_clean.session
  WHERE
    ts_started >= CAST('{load_start_date}' AS DATE) - INTERVAL '3' MONTH
  ...
  FROM datalake_journey_flow_clean.journey_flow
  WHERE
  ...
  SELECT
    ...
    g.ts_started,
    g.ts_ended
  FROM greenseer_uniques AS g
  LEFT JOIN journeys_uniques AS j
  ...
  SELECT
    ...
    NOT COALESCE(
      ...
      has_emma_try,
      has_emma_flow
    ) IS NULL AS tried_retention
  FROM greenseer_sessions
), sauron_ranked AS (
  SELECT
    id,
    source,
  ...
  SELECT
    g.id_session,
    (
      NOT s.agent IS NULL AND s.agent <> 'QuintoAndar'
    ) AS is_retention
  FROM greenseer_tried_retention AS g
  LEFT JOIN sauron_uniques AS s
  ...
  SELECT
    ...
    ticket_origin,
    ROW_NUMBER() OVER (PARTITION BY id_session ORDER BY ts_updated DESC) AS rn
  FROM datalake_customer_support.tickets
  WHERE
    front_or_back = 'front'
  ...
  SELECT
    ...
    id_ticket,
    ticket_origin
  FROM sessions_and_tickets_ranked
  WHERE
    rn = 1
  ...
  SELECT
    gs.id_session,
    CASE
      WHEN gs.response_key = LAG(gs.response_key, 1) OVER (PARTITION BY gs.id_user ORDER BY ts_started)
      THEN TRUE
      ELSE FALSE
    ...
    CASE
      WHEN (
        (
          UNIX_TIMESTAMP(gs.ts_started) - UNIX_TIMESTAMP(LAG(gs.ts_started) OVER (PARTITION BY gs.id_user ORDER BY gs.ts_started))
        ) / (
          3600
        )
      ) <= 12
  ...
  LEFT JOIN datalake_journey_flow_clean.journey_flow AS j
    ON gs.id_session = j.id_correlation AND j.journey_name IS NULL
), sessions_with_recontact AS (
  SELECT
    id_session,
    has_same_previous_theme,
    recontact_time
  FROM sessions_with_recontact_ranked
  WHERE
    rn = 1
  ...
  SELECT
    ...
    gs.memory,
    (
      NOT gs.fired_response IS NULL
    ) AS has_journey_flow_response,
    ...
    IF(
      ...
      AND gr.is_retention = TRUE,
      TRUE,
      FALSE
    ) AS is_recontact,
    sr.has_same_previous_theme,
    CASE WHEN gs.ts_ended > gs.ts_started + INTERVAL '4' HOUR THEN TRUE ELSE FALSE END AS has_exceeded_session_timeout,
    gs.has_fallback,
    st.ticket_origin,
    gs.ts_started,
    gs.ts_ended,
    gs.ts_updated,
    YEAR(TO_DATE(gs.ts_started)) AS year,
    MONTH(TO_DATE(gs.ts_started)) AS month,
    DAY(TO_DATE(gs.ts_started)) AS day
  FROM greenseer_sessions AS gs
  LEFT JOIN greenseer_retentions AS gr
    ON gr.id_session = gs.id_session
  LEFT JOIN sessions_and_tickets AS st
    ON st.id_session = gs.id_session
  LEFT JOIN sessions_with_recontact AS sr
  ...
  SELECT DISTINCT
    ...
    g.ts_started,
    'chat' AS channel
  FROM greenseer_enriched AS g
  WHERE
    g.ts_started >= CAST('{load_start_date}' AS DATE) - INTERVAL '3' MONTH
  ...
  JOIN greenseer_enriched AS g
    ON CAST(t.id_user_main AS STRING) = g.id_user
    AND g.ts_started < t.ts_created - INTERVAL '3' HOUR
  WHERE
    t.channel = 'call'
    AND g.ts_started >= CAST('{load_start_date}' AS DATE) - INTERVAL '3' MONTH
), assistances AS (
  SELECT
    id_user,
    channel,
    ...
    (
      UNIX_TIMESTAMP(ts_started) - UNIX_TIMESTAMP(LAG(ts_started, 1) OVER (PARTITION BY id_user ORDER BY ts_started ASC))
    ) / (
      3600
    ) AS hours_to_next_session
  FROM base_churn
), wrangling AS (
  SELECT
    id_user,
    ...
    CASE
      WHEN channel = 'call' AND previews_channel = 'chat' AND hours_to_next_session <= 96
      THEN TRUE
      WHEN channel = 'chat' AND previews_channel = 'chat' AND hours_to_next_session <= 96
      THEN FALSE
  ...
  FROM greenseer_enriched
  WHERE
    ts_started BETWEEN CAST('{load_start_date}' AS DATE) - INTERVAL '3' MONTH AND CAST('{load_end_date}' AS DATE)
    AND NOT response_key IS NULL
  ...
  SELECT
    ...
    raw_answer_last,
    CASE
      WHEN raw_answer_last IS NULL
      THEN NULL
      WHEN LENGTH(raw_answer_last) = 1
      THEN CAST(raw_answer_last AS INT)
      ELSE 10
  ...
  SELECT
    ...
    gs.is_retention,
    CASE WHEN gs.is_retention <> TRUE AND gs.id_ticket IS NULL THEN TRUE ELSE FALSE END AS is_abandonmet,
    gs.is_recontact,
    CASE WHEN gs.is_retention = TRUE AND gs.has_fallback = TRUE THEN TRUE ELSE FALSE END AS is_retained_session_with_fallback,
    COALESCE(w.is_churn_chat, FALSE) AS is_churn_chat,
    NOT nop.user_response IS NULL
    AND NOT nop.n_options IS NULL
    AND nop.user_response = n_options + 1 AS is_other_option,
  ...
  FROM greenseer_enriched AS gs
  LEFT JOIN surveys_fup AS sf
    ON gs.id_session = sf.id_session
  LEFT JOIN wrangling AS w
    ON gs.id_user = w.id_user AND w.previews_contact_date = gs.ts_started
  LEFT JOIN normalize_options AS nop
    ON gs.id_session = nop.id_session
  WHERE
    gs.ts_started >= CAST('{load_start_date}' AS DATE) - INTERVAL '3' MONTH
)
SELECT
  sk_session,
  sk_ticket,
