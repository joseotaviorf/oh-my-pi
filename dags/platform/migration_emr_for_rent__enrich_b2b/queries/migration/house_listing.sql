WITH house_portability AS (
  ...
  JOIN datalake_ebdb_clean.portability AS por
...
SELECT DISTINCT
  ...
  IF(
    (
      (
        ...
        OR (
          NOT partner_agent.id_partner IS NULL AND partner.type = 'PRIME'
        )
      )
  ...
  OR NOT por.id_house IS NULL AS is_b2b,
  IF(NOT por.id_house_listing IS NULL, TRUE, FALSE) AS is_portability
FROM datalake_ebdb_clean.house AS h
...
LEFT JOIN datalake_lead.reprocessed_lead AS rl
  ON rl.id = l.id
LEFT JOIN datalake_lead.lead AS lo
  ON lo.id = rl.id_origin_lead
LEFT JOIN datalake_ebdb_clean.partner_agent
  ON partner_agent.id_user = h.id_user
LEFT JOIN datalake_ebdb_clean.partner
...
LEFT JOIN house_portability AS por
