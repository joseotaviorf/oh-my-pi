WITH lead_origin_rene_descartes AS (
...
), affiliate_type_fix /* Rule to fix affiliate type, we get the original affiliate type */ AS (
  SELECT
    lo_base.id_lead,
  ...
  SELECT
    ...
    ) AS database_tracking_campaign, /* We're tracking how database is the source of our UTMs */
    NVL2(
      r.medium,
      'rene_descartes',
      NVL2(
  ...
  LEFT JOIN lead_origin_amplitude AS a
    ON (
      r.id_lead_ebdb = a.id_lead_ebdb
    )
  LEFT JOIN affiliate_type_fix AS atf
    ON (
      r.id_lead = atf.id_lead
    )
