WITH lead_origin_rene_descartes AS (
  ...
  WHERE
    CAST(amd.ts_created AS DATE) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
  ...
  JOIN lead_origin_rene_descartes AS lo_fix
    ON (
      lo_base.original_lead = lo_fix.id_lead_ebdb
    )
  WHERE
    lo_base.affiliate_type IS NULL AND NOT lo_fix.affiliate_type IS NULL
  ...
  FROM (
    SELECT
      ...
      ts_event,
      ROW_NUMBER() OVER (PARTITION BY id_lead ORDER BY ts_event) AS _w,
      id_lead
    FROM datalake_amplitude_lead.lead_origin AS lo
    WHERE
      CAST(ts_event AS DATE) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
  ) AS _t
  WHERE
    _w = 1
), inbound_leads AS (
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY ia.id_lead_ebdb ORDER BY fm.dt_cost DESC) AS _w,
      fm.dt_cost
    FROM datalake_supply_flows.inbound_attribution AS ia
    LEFT JOIN datalake_growth_media_platform.facebook_metrics AS fm
  ...
  SELECT
    ...
    COALESCE(oa.utm_campaign, a.campaign, inbound_leads.utm_campaign) AS campaign,
    COALESCE(oa.utm_medium, a.medium, inbound_leads.utm_medium) AS medium,
    COALESCE(oa.utm_source, a.source, inbound_leads.utm_source) AS source,
    COALESCE(oa.utm_content, a.content, inbound_leads.utm_content) AS content,
    COALESCE(oa.utm_term, a.term, inbound_leads.utm_term) AS term,
    r.ops_agent,
    r.ops_partner,
    r.application,
    ...
    NVL2(
      ...
      NVL2(
        a.campaign,
        'amplitude',
        NVL2(inbound_leads.utm_campaign, 'facebook_api', 'lost_tracking')
      )
    ) AS database_tracking_campaign,
    NVL2(
      oa.utm_medium,
      'cdp',
      NVL2(
        a.medium,
        'amplitude',
        NVL2(inbound_leads.utm_medium, 'facebook_api', 'lost_tracking')
      )
    ) AS database_tracking_medium,
    NVL2(
      oa.utm_source,
      'cdp',
      NVL2(
        a.source,
        'amplitude',
        NVL2(inbound_leads.utm_source, 'facebook_api', 'lost_tracking')
      )
    ) AS database_tracking_source,
    NVL2(
      oa.utm_content,
      'cdp',
      NVL2(
        a.content,
        'amplitude',
        NVL2(inbound_leads.utm_content, 'facebook_api', 'lost_tracking')
      )
    ) AS database_tracking_content,
    NVL2(
      oa.utm_term,
      'cdp',
      NVL2(a.term, 'amplitude', NVL2(inbound_leads.utm_term, 'facebook_api', 'lost_tracking'))
    ) AS database_tracking_term,
    NVL2(
      r.ctwa_clid,
      'rene_descartes',
      NVL2(inbound_leads.ctwa_clid, 'sauron', 'lost_tracking')
    ) AS database_tracking_ctwa,
    COALESCE(r.ts_event, a.ts_event) AS ts_event
  FROM lead_origin_rene_descartes AS r
  LEFT JOIN datalake_attribution.online_attribution AS oa
...
SELECT DISTINCT
  ...
  CASE
    WHEN ops_agent = 'CAPTA_AI'
    THEN 'acquisition'
    WHEN NOT ops_agent IS NULL
    THEN 'conversion'
    ELSE NULL
  ...
  IF(NOT original_lead IS NULL, 'automatically_reprocessed', NULL) AS reprocessed,
