WITH house_aud AS (
  /* ------------------------------------------------------------------------------------------------------ */ /* Bring to IMOVEL_AUD datetime for each revision made                                                -- */ /* Also creates previous_status column so we can identify status changes                              -- */ /* (status_MOD = 1 may not work sometimes)                                                            -- */ /* ------------------------------------------------------------------------------------------------------ */
  SELECT
  ...
  INNER JOIN datalake_ebdb_clean.user_revision_entity AS rev
    ON rev.id = h.rev
  ...
  FROM datalake_ebdb_listing.house_listing_category AS hlf
  JOIN house_rent_history AS rh
    ON hlf.id_house = rh.id_house
    AND (
      (
        rh.ts_rent_price_changed BETWEEN hlf.ts_listing_version_start AND COALESCE(hlf.ts_listing_version_end - INTERVAL '1' SECOND, CURRENT_TIMESTAMP())
      )
      OR (
        COALESCE(rh.ts_next_rent_price_change, CURRENT_TIMESTAMP()) BETWEEN hlf.ts_listing_version_start AND COALESCE(hlf.ts_listing_version_end - INTERVAL '1' SECOND, CURRENT_TIMESTAMP())
      )
      OR (
        rh.max_ts_rent_price_changed <= hlf.ts_listing_version_start
      )
    )
  ...
  SELECT
    id_house_listing,
    MAX(
      CASE WHEN max_order_status_version = order_rent_price THEN rent_price_history END
    ) AS rent
  FROM house_rent_max
  GROUP BY
    1
  ...
  SELECT
    ...
    dt_opted_in,
    dt_opted_out
  FROM datalake_ebdb_listing.house_listing_special_conditions
  WHERE
  ...
  FROM last_opt AS lo
  JOIN first_opt AS fo
    ON lo.id_house_listing = fo.id_house_listing
    AND (
      CASE
        WHEN lo.special_condition_type LIKE 'Originals%'
        THEN 'Originals'
        WHEN lo.special_condition_type LIKE '%Rent'
        THEN 'ioRent'
        ELSE lo.special_condition_type
      END
    ) = (
      CASE
        WHEN fo.special_condition_type LIKE 'Originals%'
        THEN 'Originals'
        WHEN fo.special_condition_type LIKE '%Rent'
        THEN 'ioRent'
  ...
  FROM (
    SELECT
      sale.*,
      ROW_NUMBER() OVER (PARTITION BY sale.id_house ORDER BY sale.ts_updated DESC) AS _w
    FROM sale
    LEFT JOIN datalake_ebdb_listing.lbc_status_version_order AS lbc_version
  ...
  SELECT
    ...
    ) AS INT)) AS BOOLEAN) AS is_for_sale,
    CAST(MAX(CAST((
      business_context = 'RENT'
    ) AS INT)) AS BOOLEAN) AS is_for_rent
  ...
  SELECT
    ...
    NOT lsc_exclusivity.dt_first_opted_in IS NULL AS is_exclusive,
    hl.is_extended_rental,
    hl.has_termination_canceled,
    (
      (
        NOT lsc_originals.dt_last_opted_in IS NULL
        AND lsc_originals.dt_last_opted_out IS NULL
      )
      OR (
        lsc_originals.dt_last_opted_in > lsc_originals.dt_last_opted_out
      )
    ) AS is_originals_active,
    (
      (
        NOT lsc_iorent.dt_last_opted_in IS NULL AND lsc_iorent.dt_last_opted_out IS NULL
      )
      OR (
        lsc_iorent.dt_last_opted_in > lsc_iorent.dt_last_opted_out
      )
    ) AS is_iorent_active,
  ...
  JOIN datalake_ebdb_clean.contract AS c
  ...
  FROM house_listing_latest_contracts
  ...
  SELECT
    ...
    hllc.order_renting
  FROM house_listing AS hl
  LEFT JOIN house_listing_latest_contracts AS hllc
    ON hl.id_house_listing = hllc.id_house_listing
  LEFT JOIN next_listing_contract AS nlc
  ...
  FROM house_listing
  WHERE
    has_termination_canceled = FALSE AND is_for_rent
  ...
  SELECT
    ...
    COALESCE(hls.ts_status_ended, (
      CURRENT_TIMESTAMP() - INTERVAL '1' DAY
    )) AS ts_status_ended,
    hl.ts_listing_version_start,
    (
      hl.ts_listing_version_start + INTERVAL '8' WEEK
    ) AS ts_to_be_stranded,
    CASE
      WHEN (
        hl.ts_listing_version_start + INTERVAL '8' WEEK
      ) <= hls.ts_status_started
    ...
    LAG(hls.status_history) OVER (PARTITION BY hls.id_house_listing ORDER BY hls.ts_status_started, COALESCE(hls.ts_status_ended, (
      CURRENT_TIMESTAMP() - INTERVAL '1' DAY
    ))) AS previous_status_history,
    LAG(hls.status_change_reason) OVER (PARTITION BY hls.id_house_listing ORDER BY hls.ts_status_started, COALESCE(hls.ts_status_ended, (
      CURRENT_TIMESTAMP() - INTERVAL '1' DAY
  ...
  SELECT
    ...
    MIN(CASE WHEN type_stranded = 'stranded' THEN ts_status_started END) OVER (PARTITION BY id_house_listing) AS min_ts_all_status, /* calculate min date of all status, because if it is a valid status that is the date that will be used */
    MIN(
      CASE
        WHEN type_stranded = 'stranded'
        AND status_history IN ('publicado', 'suspenso', 'edicao', 'aguardando_publicacao', 'PUBLISHED', 'SUSPENDED', 'EDITING')
        AND (
          (
            previous_status_history <> 'alugado'
            AND (
      ...
      END
    ) OVER (PARTITION BY id_house_listing) AS min_ts_valid_status, /* calculate min date of valid status to define stranded */
  ...
  WHERE
    NOT type_stranded IS NULL
), house_listing_stranded_status AS (
  SELECT
    *,
    CASE
      WHEN rn = 1
      AND status_history = 'alugado'
      OR (
        status_history = 'SUSPENDED' AND status_reason = 'RENTED'
      )
      THEN NULL
  ...
  SELECT DISTINCT
    ...
    CASE
      WHEN GREATEST(
        CAST(COALESCE(min_ts_stranded, '3000-01-01') AS TIMESTAMP),
        CAST(min_ts_to_be_stranded AS TIMESTAMP)
      ) = '3000-01-01'
      THEN NULL
      ELSE GREATEST(
        CAST(COALESCE(min_ts_stranded, '3000-01-01') AS TIMESTAMP),
  ...
  JOIN house_listing AS hl
    ...
    AND (
      heh.ts_entrance_started BETWEEN COALESCE(hl.ts_listing_version_start, CAST('1922-01-01' AS DATE)) AND COALESCE(hl.ts_listing_version_end, CAST('2100-01-01' AS DATE))
      OR COALESCE(hl.ts_listing_version_start, CAST('1922-01-01' AS DATE)) BETWEEN heh.ts_entrance_started AND COALESCE(heh.ts_entrance_ended, CAST('2100-01-01' AS DATE))
    )
  ...
  FROM datalake_ebdb_listing.lbc_status_version_order
  WHERE
...
SELECT
  ...
  CASE
    ...
    WHEN (
      ...
      AND status_reason = 'RENTED'
      AND (
        hl.revision_reason <> 'TERMINATION_CANCELED' OR hl.revision_reason IS NULL
      )
    )
  ...
  IF(NOT hled.id_house_listing IS NULL, TRUE, FALSE) AS is_early_relisting,
  ...
  CASE
    WHEN hl.version = 1
    THEN fp.ts_first_publication
    WHEN hl.version > 0
    THEN hl.ts_listing_version_start
    ELSE NULL
  END AS ts_publicated,
...
FROM house_listing AS hl
LEFT JOIN house_listing_contracts AS hl_c
  ON hl.id_house_listing = hl_c.id_house_listing
LEFT JOIN house_listing_not_cancelled AS hlnc
...
LEFT JOIN datalake_ebdb_listing.house_listing_early_demand AS hled
  ON hled.id_house_listing = hl.id_house_listing
LEFT JOIN first_publication AS fp
  ON fp.id_house = hl.id_house
LEFT JOIN datalake_consumption_bill.consumption_bill_history AS cbh
  ON cbh.id_house = hl.id_house
  AND CAST(hl.ts_listing_version_start AS DATE) >= cbh.dt_status_started
  AND CAST(hl.ts_listing_version_start AS DATE) < cbh.dt_status_ended
