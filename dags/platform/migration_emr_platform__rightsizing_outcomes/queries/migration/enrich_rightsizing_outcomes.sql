/* ============================================================================ */ /* enrich_rightsizing_outcomes.sql */ /* Prod vs __validation twin comparison for cluster right-sizing feedback. */ /* Grain: one row per validation Airflow dag run (validation_airflow_run_id). */ /* Reference prod pairing ladder: (1) reference_prod_dag_run_id persisted in the */ /* validation run's conf by trigger_cluster_validation_dags.py, (2) candidate */ /* whose derived load window equals the validation's own conf window, (3) */ /* fastest successful prod run in 14d with wall >= 480s. Conf is a pickled dict */ /* stored as bytea-hex text; fields are extracted via unhex+regexp. */ /* Promotion thresholds align with scripts/promote_rightsizing_validations.py: */ /* REGRESSION_TOLERANCE_PCT = 5.0, STRONG_POSITIVE_COST_PCT = -15.0, MEM_WARN = 82 */ /* is_decisive_validation_run mirrors decisive_row_for_dag() newest-first scan. */ /* outcome/promotion_action CASE order matches decide_promotion_action(): runtime */ /* fail, incomplete pairing (extend), cost, wall, mem warn, pass/promote. */ /* ============================================================================ */
WITH validation_runs_raw AS (
  SELECT
    ...
    CAST(ts_started AS DATE) AS val_dt,
    state AS val_state,
    CASE
      WHEN NOT configuration IS NULL AND SUBSTRING(configuration, 1, 2) = '\\x'
      THEN DECODE(UNHEX(SUBSTRING(configuration, 3)), 'ISO-8859-1')
    END AS conf_text,
    ROW_NUMBER() OVER (PARTITION BY id_dag, id_run ORDER BY ts_updated DESC) AS dedup_rn
  FROM datalake_astro_clean.dag_run
  WHERE
    MAKE_DATE(year, month, day) >= CAST('{load_start_date}' AS DATE)
    AND ts_started >= CAST('{load_start_date}' AS TIMESTAMP)
    AND ts_started < CAST('{load_end_date}' AS TIMESTAMP) + INTERVAL '1' DAY
    AND (
      id_dag LIKE 'bietlejuice.%' OR id_dag LIKE 'quintoml.wonka.%'
    )
    AND id_dag RLIKE '__validation$'
  ...
  SELECT
    ...
    TO_DATE(
      NULLIF(
        REGEXP_EXTRACT(
          conf_text,
          'load_start_date.{{1,5}}?(\\d{{4}}-\\d{{2}}-\\d{{2}})'
        ),
        ''
      )
  ...
  FROM (
    ...
    FROM datalake_astro_clean.dag_run
    WHERE
      MAKE_DATE(year, month, day) >= DATE_ADD(CAST('{load_start_date}' AS DATE), 16 * -1)
      AND (
        id_dag LIKE 'bietlejuice.%' OR id_dag LIKE 'quintoml.wonka.%'
      )
      AND NOT id_dag RLIKE '__validation$'
      AND state = 'success'
      AND NOT ts_ended IS NULL
      AND NOT ts_started IS NULL
  )
  WHERE
    dedup_rn = 1
  ...
  SELECT
    ...
    (
      NOT p.prod_conf_load_start_date IS NULL AND NOT p.prod_conf_load_end_date IS NULL
    ) AS is_ref_window_from_conf,
    COALESCE(p.prod_conf_load_start_date, CAST(p.ts_data_interval_started AS DATE)) AS ref_derived_start,
    COALESCE(
      p.prod_conf_load_end_date,
      DATE_ADD(CAST(p.ts_data_interval_ended AS DATE), 1 * -1)
  ...
  SELECT
    c.*,
    CASE
      WHEN NOT c.ref_derived_start IS NULL
      AND NOT c.ref_derived_end_raw IS NULL
    ...
    COALESCE(
      NOT c.val_conf_load_start_date IS NULL
      AND NOT c.val_conf_load_end_date IS NULL
      AND c.val_conf_load_start_date = c.ref_derived_start
      AND c.val_conf_load_end_date IN (c.ref_derived_end_raw, CASE
    ...
    ROW_NUMBER() OVER (PARTITION BY c.validation_airflow_run_id ORDER BY CASE WHEN c.is_conf_run_id_match THEN 0 ELSE 1 END, CASE
      WHEN NOT c.val_conf_load_start_date IS NULL
      AND c.val_conf_load_start_date = c.ref_derived_start
      AND c.val_conf_load_end_date IN (c.ref_derived_end_raw, CASE
        WHEN c.ref_derived_start >= c.ref_derived_end_raw
        THEN DATE_ADD(c.ref_derived_end_raw, 1)
      ...
      THEN 0
      ELSE 1
    END, c.ref_duration_seconds ASC, c.ref_ts_started DESC) AS ref_rank
  ...
  SELECT
    ...
    CASE
      WHEN r.reference_prod_airflow_run_id IS NULL
      THEN NULL
      WHEN r.is_ref_window_from_conf
      THEN 'conf'
      ELSE 'data_interval'
    ...
    CASE
      WHEN r.reference_prod_airflow_run_id IS NULL
      THEN NULL
      WHEN r.is_conf_run_id_match
      THEN 'conf_run_id'
      WHEN r.is_window_match
      THEN 'conf_window'
      ELSE 'heuristic'
  ...
  WHERE
    dt_dag_run_started >= DATE_ADD(CAST('{load_start_date}' AS DATE), 16 * -1)
    AND dt_dag_run_started <= CAST('{load_end_date}' AS DATE)
    AND (
      airflow_dag_id LIKE 'bietlejuice.%' OR airflow_dag_id LIKE 'quintoml.wonka.%'
    )
  ...
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY v.validation_airflow_run_id ORDER BY ABS(UNIX_TIMESTAMP(fd.ts_logical_run_started) - UNIX_TIMESTAMP(v.val_ts_started)), fd.total_cost_usd DESC) AS fd_rank
  ...
  LEFT JOIN fd_runs AS fd
    ...
    AND fd.ts_logical_run_started <= COALESCE(v.val_ts_ended, v.val_ts_started) + INTERVAL '30' MINUTES
), val_fd AS (
  SELECT
    *
  FROM val_fd_ranked
  ...
  SELECT
    ...
    fd.worker_count,
    fd.primary_dbr_version,
    ROW_NUMBER() OVER (PARTITION BY v.validation_airflow_run_id ORDER BY ABS(UNIX_TIMESTAMP(fd.ts_logical_run_started) - UNIX_TIMESTAMP(v.ref_ts_started)), fd.total_cost_usd DESC) AS fd_rank
  FROM validation_with_reference AS v
  INNER JOIN fd_runs AS fd
    ON NOT v.reference_prod_airflow_run_id IS NULL
    AND fd.airflow_dag_id = v.prod_airflow_dag_id
    AND fd.ts_logical_run_started >= v.ref_ts_started - INTERVAL '5' MINUTES
    AND fd.ts_logical_run_started <= COALESCE(v.ref_ts_ended, v.ref_ts_started) + INTERVAL '5' MINUTES
  ...
  SELECT
    CASE
      WHEN airflow_dag_id RLIKE '__validation$'
  ...
  FROM fd_runs
  WHERE
    NOT airflow_dag_id RLIKE '__validation$'
  ...
  SELECT
    ...
    CASE WHEN NOT v.reference_prod_airflow_run_id IS NULL THEN 1 ELSE 0 END AS prod_run_count,
    1 AS val_run_count,
    CASE
    ...
    CASE
      WHEN NOT val_fd.validation_airflow_run_id IS NULL
  ...
  SELECT
    ...
    CASE
      WHEN NOT p.prod_avg_cost_usd IS NULL
      AND p.prod_avg_cost_usd > 0
      AND NOT p.val_avg_cost_usd IS NULL
      THEN ROUND((
        p.val_avg_cost_usd - p.prod_avg_cost_usd
      ) / p.prod_avg_cost_usd * 100.0, 1)
    END AS delta_cost_pct,
    CASE
      WHEN NOT p.prod_wall_p50_min IS NULL
      AND p.prod_wall_p50_min > 0
      AND NOT p.val_wall_p50_min IS NULL
      THEN ROUND((
        p.val_wall_p50_min - p.prod_wall_p50_min
      ) / p.prod_wall_p50_min * 100.0, 1)
    END AS delta_wall_p50_pct,
    CASE
      WHEN NOT p.prod_wall_p95_min IS NULL AND NOT p.val_wall_p95_min IS NULL
    ...
    (
      ...
      AND NOT p.val_avg_cost_usd IS NULL
      AND NOT p.prod_avg_cost_usd IS NULL
      AND p.prod_avg_cost_usd > 0
      AND (
        p.val_avg_cost_usd - p.prod_avg_cost_usd
      ) / p.prod_avg_cost_usd * 100.0 <= 5.0
      AND (
        (
          COALESCE(c.schedule_interval_minutes, 1440.0) > 120
          AND NOT p.val_wall_p50_min IS NULL
          AND NOT p.prod_wall_p50_min IS NULL
          AND p.val_wall_p50_min <= p.prod_wall_p50_min * 1.05
        )
        OR (
          COALESCE(c.schedule_interval_minutes, 1440.0) <= 120
          AND NOT p.val_wall_p95_min IS NULL
          AND NOT p.prod_wall_p95_min IS NULL
          AND p.val_wall_p95_min <= GREATEST(0.8 * COALESCE(c.schedule_interval_minutes, 60.0), p.prod_wall_p95_min * 1.05)
        )
      )
    ) AS is_promote_eligible,
    CASE
      WHEN p.val_failure_count > 0
      THEN 'fail'
      WHEN p.val_clean_run_count < 1
      THEN 'extend'
      WHEN NOT p.val_avg_cost_usd IS NULL
      AND NOT p.prod_avg_cost_usd IS NULL
      AND p.prod_avg_cost_usd > 0
      AND (
        p.val_avg_cost_usd - p.prod_avg_cost_usd
  ...
  SELECT
    ...
    (
      ...
      AND (
        (
          NOT s.delta_cost_pct IS NULL AND s.delta_cost_pct <= -15.0
        )
        OR (
          NOT s.delta_cost_pct IS NULL
          AND s.delta_cost_pct <= 5.0
          AND (
            (
              COALESCE(s.schedule_interval_minutes, 1440.0) > 120
              AND NOT s.val_wall_p50_min IS NULL
              AND NOT s.prod_wall_p50_min IS NULL
              AND s.val_wall_p50_min <= s.prod_wall_p50_min * 1.05
            )
            OR (
              COALESCE(s.schedule_interval_minutes, 1440.0) <= 120
              AND NOT s.val_wall_p95_min IS NULL
              AND NOT s.prod_wall_p95_min IS NULL
              AND s.val_wall_p95_min <= GREATEST(0.8 * COALESCE(s.schedule_interval_minutes, 60.0), s.prod_wall_p95_min * 1.05)
            )
          )
        )
      )
  ...
  SELECT
    w.*,
    ROW_NUMBER() OVER (PARTITION BY w.prod_airflow_dag_id ORDER BY w.val_ts_started ASC) AS validation_attempt_number,
    ROW_NUMBER() OVER (PARTITION BY w.prod_airflow_dag_id ORDER BY w.val_ts_started DESC) = 1 AS is_latest_validation_run,
    LAG(w.outcome) OVER (PARTITION BY w.prod_airflow_dag_id ORDER BY w.val_ts_started) AS prior_outcome,
    LAG(w.val_failure_count) OVER (PARTITION BY w.prod_airflow_dag_id ORDER BY w.val_ts_started) AS prior_val_failure_count,
    ROW_NUMBER() OVER (PARTITION BY w.prod_airflow_dag_id ORDER BY CASE
      WHEN w.promotion_action IN ('promote', 'reject') OR w.outcome = 'warn'
      THEN 0
      WHEN w.has_strong_positive_signal
      THEN 1
      ELSE 2
    END, w.val_ts_started DESC) = 1 AS is_decisive_validation_run,
    (
      (
        LAG(w.outcome) OVER (PARTITION BY w.prod_airflow_dag_id ORDER BY w.val_ts_started) = 'fail'
        AND w.outcome IN ('pass', 'warn')
      )
      OR (
        LAG(w.val_failure_count) OVER (PARTITION BY w.prod_airflow_dag_id ORDER BY w.val_ts_started) = 1
        AND w.val_failure_count = 0
        AND (
          (
            NOT w.val_avg_cost_usd IS NULL
            AND NOT LAG(w.val_avg_cost_usd) OVER (PARTITION BY w.prod_airflow_dag_id ORDER BY w.val_ts_started) IS NULL
            AND w.val_avg_cost_usd < LAG(w.val_avg_cost_usd) OVER (PARTITION BY w.prod_airflow_dag_id ORDER BY w.val_ts_started)
          )
          OR (
            NOT w.val_wall_p50_min IS NULL
            AND NOT LAG(w.val_wall_p50_min) OVER (PARTITION BY w.prod_airflow_dag_id ORDER BY w.val_ts_started) IS NULL
            AND w.val_wall_p50_min < LAG(w.val_wall_p50_min) OVER (PARTITION BY w.prod_airflow_dag_id ORDER BY w.val_ts_started)
          )
        )
      )
