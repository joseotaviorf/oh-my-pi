WITH ranked_hr_system_workers AS (
  SELECT
    ...
    ethnicities,
    religions,
    DENSE_RANK() OVER (PARTITION BY id_person ORDER BY dt_effective) AS worker_rank
  ...
  SELECT
    ...
    ethnicities,
    religions
  FROM ranked_hr_system_workers
  WHERE
    worker_rank = 1
), external_identifiers_step1 AS (
  SELECT
    id_person,
    EXPLODE(external_identifiers) AS external_identifiers
  ...
  SELECT
    id_person,
    EXPLODE(legislative_info) AS legislative_info
  FROM hr_system_workers
), legislative_info AS (
  SELECT
    id_person,
    ...
    TO_TIMESTAMP(
      SUBSTRING(REPLACE(legislative_info['CreationDate'], 'T', ' '), 0, 19),
      'yyyy-MM-dd HH:mm:ss'
    ) AS ts_created,
    TO_TIMESTAMP(
      SUBSTRING(REPLACE(legislative_info['LastUpdateDate'], 'T', ' '), 0, 19),
      'yyyy-MM-dd HH:mm:ss'
    ) AS ts_last_update,
    CAST(legislative_info['EffectiveStartDate'] AS DATE) AS dt_effective_start,
    CAST(legislative_info['EffectiveEndDate'] AS DATE) AS dt_effective_end
  ...
  SELECT
    ...
    TO_TIMESTAMP(
      SUBSTRING(REPLACE(ethnicities['CreationDate'], 'T', ' '), 0, 19),
      'yyyy-MM-dd HH:mm:ss'
    ) AS ts_created,
    TO_TIMESTAMP(
      SUBSTRING(REPLACE(ethnicities['LastUpdateDate'], 'T', ' '), 0, 19),
      'yyyy-MM-dd HH:mm:ss'
    ) AS ts_last_update,
  ...
  SELECT
    ...
    ts_created,
    ts_last_update
  FROM ethnicities_primary
  WHERE
    last_update_raw = max_last_update_raw
), religions_step1 AS (
  SELECT
    id_person,
    EXPLODE(religions) AS religions
  FROM hr_system_workers
  ...
  SELECT
    ...
    TO_TIMESTAMP(
      SUBSTRING(REPLACE(religions['CreationDate'], 'T', ' '), 0, 19),
      'yyyy-MM-dd HH:mm:ss'
    ) AS ts_created,
    TO_TIMESTAMP(
      SUBSTRING(REPLACE(religions['LastUpdateDate'], 'T', ' '), 0, 19),
      'yyyy-MM-dd HH:mm:ss'
    ) AS ts_last_update,
...
SELECT
  ...
  ) AS sk_demographic_information, /* non-ids */
  li.id_person_legislative,
  eth.id_ethnicity,
  rel.id_religion,
  li.marital_status, /* non-metrics */
  li.gender,
  ...
  IF(
    CAST(lidff.neurodiversity AS STRING) RLIKE '^[0-9]+$',
    lidff.neurodiversity_name,
  ...
  rel.religion,
  CASE
    WHEN lidff.has_disability = 'Sim'
    THEN TRUE
    WHEN lidff.has_disability = 'Não'
    THEN FALSE
    ELSE NULL
  END AS has_disability, /* metrics */
  CASE
    WHEN lidff.has_salary_advance = 'S'
    THEN TRUE
    WHEN lidff.has_salary_advance = 'N'
    THEN FALSE
    ELSE NULL
  END AS has_salary_advance,
  li.dt_effective_start AS dt_effective_start, /* dates */
  li.dt_effective_end AS dt_effective_end,
  LEAST(li.ts_created, eth.ts_created, rel.ts_created) AS ts_created, /* timestamps */
  GREATEST(li.ts_last_update, eth.ts_last_update, rel.ts_last_update) AS ts_last_update,
  NOW() AS ts_load
...
LEFT JOIN legislative_info_dff AS lidff
  ON li.id_person = lidff.id_person
  AND li.id_person_legislative = lidff.id_person_legislative
LEFT JOIN ethnicities AS eth
  ON li.id_person = eth.id_person AND li.legislation_code = eth.legislation_code
LEFT JOIN religions AS rel
  ON li.id_person = rel.id_person AND li.legislation_code = rel.legislation_code
