WITH raw_predicted_durations AS (
  SELECT
    ...
    BIGINT(CAST(input_data.id_event AS DOUBLE)) AS id_event,
    input_data.event_type AS event_type,
    MAX_BY(
      output_data.prediction / /* Convert from days to months */ 30,
      STRUCT(model_version AS model_version, ts_inference AS ts_inference)
    ...
    MAX_BY(
      CAST(FROM_UNIXTIME(input_data.ts_event / 1000) AS TIMESTAMP),
      STRUCT(model_version AS model_version, ts_inference AS ts_inference)
    ) AS ts_event,
    CAST(MAX_BY(ts_inference, STRUCT(model_version AS model_version, ts_inference AS ts_inference)) AS DATE) AS dt_inference
  FROM (
    SELECT
      ts_inference,
      model_version,
  ...
  SELECT
    ...
    CAST(input_data.id_tenant_prospect AS BIGINT) AS id_tenant_prospect,
    BIGINT(CAST(input_data.id_event AS DOUBLE)) AS id_event,
    MAX_BY(prediction, STRUCT(model_version AS model_version, ts_inference AS ts_inference)) AS predicted_conversion
  FROM (
    SELECT
      ts_inference,
      model_version,
  ...
  SELECT
    ...
    table1.dt_inference,
    table1.ts_event
  FROM raw_predicted_durations AS table1
  JOIN raw_predicted_conversions
    USING (id_rent_flow, id_house, id_tenant_prospect, id_event)
), exploded_table AS (
  SELECT
    *,
    CASE
  ...
  FROM (
    SELECT
      ...
      id_event,
      event_type,
      EXPLODE(SEQUENCE(1, CAST(CEIL(predicted_duration) AS INT))) AS months_since_contract_start,
      predicted_duration,
      predicted_conversion,
      rent,
      dt_inference,
      ts_event
    FROM raw_predictions_table
  )
  ...
  SELECT
    ...
    (
      CASE
        WHEN months_since_contract_start = 1
        THEN rent /* Brokerage */ + rent * 0.02 /* Reserves */ + rent * 0.016 /* Brokerage Financing */
        ELSE rent * 0.004 /* credit card payment */ + rent * 0.002 /* Long-Term Rental Antecipation */ + rent * 0.001 /* Monthly Rental Antecipation */ + rent * 0.02 /* other revenues */ + rent * 0.007 /* interest payments */
      END + rent * 0.095 /* Management fee */ + rent * 0.023 /* Service fee */ + rent * 0.007 /* Guarantee */
    ...
    (
      CASE
        ...
        THEN (
          1 - 0.2893321011
        ) * rent /* Brokerage */ + rent * 0.02 /* Reserves */ + rent * 0.016 /* Brokerage Financing */
        ELSE rent * 0.004 /* credit card payment */ + rent * 0.002 /* Long-Term Rental Antecipation */ + rent * 0.001 /* Monthly Rental Antecipation */ + rent * 0.02 /* other revenues */ + rent * 0.007 /* interest payments */
      END + rent * 0.095 /* Management fee */ + rent * 0.023 /* Service fee */ + rent * 0.007 /* Guarantee */
    ) * (
      1 - 0.0847624411
    ...
    (
      (
        ...
        1 - 0.0847624411
      ) - COALESCE(pre_rental_costs, 0.0) - COALESCE(onboarding_costs, 0.0) - COALESCE(ongoing_costs, 0.0) - COALESCE(offboarding_costs, 0.0) - COALESCE(cost_of_guarantee, 0.0)
    ) * predicted_conversion AS estimated_contribution_margin,
    dt_inference,
    CAST(ts_event AS DATE) AS dt_event_time
  FROM exploded_table
  LEFT JOIN contract_onboarding_costs
    USING (id_rent_flow, id_house, id_tenant_prospect, id_event, months_since_contract_start)
  LEFT JOIN contract_ongoing_costs
    USING (id_rent_flow, id_house, id_tenant_prospect, id_event, months_since_contract_start)
  LEFT JOIN contract_offboarding_costs
    USING (id_rent_flow, id_house, id_tenant_prospect, id_event, months_since_contract_start)
)
SELECT
  id_flow,
