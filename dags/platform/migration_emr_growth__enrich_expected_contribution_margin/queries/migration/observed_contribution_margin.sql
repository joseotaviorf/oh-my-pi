WITH contracts_table AS (
  ...
  WHERE
    id_event = id_contract AND id_event_type = 9
), revenue_table AS (
  SELECT
    id_rent_flow,
    id_house,
    ...
    gross_revenue
  FROM dw_rental_contribution_margin.fact_house_listing_revenues
  LEFT JOIN contracts_table
    USING (id_contract)
  JOIN rent_flows_table
    USING (id_contract, id_house)
  WHERE
  ...
  FROM VALUES
    ...
    ('202403', -8.73),
    ('202404', -8.55),
    ('202405', -8.16) AS _t0(accrual_year_month, tax_rate)
  ...
  WHERE
    contract_lifetime > 0
    AND NOT (
      NOT dt_termination IS NULL AND dt_month_start = dt_termination
    )
  ...
  SELECT
    ...
    CASE
      ...
      ELSE provision_balance - LAG(provision_balance) OVER (PARTITION BY id_contract ORDER BY dt_month_start)
    END AS losses
  FROM provision_table
  LEFT JOIN contracts_table
    USING (id_contract)
  WHERE
  ...
  LEFT JOIN taxes_table
    USING (accrual_year_month)
  LEFT JOIN contract_onboarding_costs
    USING (id_contract, contract_lifetime)
  LEFT JOIN contract_ongoing_costs
    USING (id_contract, contract_lifetime)
  LEFT JOIN contract_offboarding_costs
    USING (id_contract, contract_lifetime)
  LEFT JOIN losses_table
    USING (id_contract, dt_month_start)
  WHERE
    (
      dt_termination IS NULL
    ) OR (
      dt_month_start <= dt_termination
    )
)
SELECT
  id_rent_flow,
  ...
  (
    net_revenue_pre_taxes * (
      1 + tax_rate / 100
    ) + losses - COALESCE(pre_rental_costs, 0.) - COALESCE(onboarding_costs, 0.) - COALESCE(ongoing_costs, 0.) - COALESCE(offboarding_costs, 0.)
