WITH collections AS (
  SELECT
    ...
    c.ts_request,
    ROW_NUMBER() OVER (PARTITION BY c.id_trace, LOWER(c.tool_name) ORDER BY c.ts_request ASC, c.id_request ASC) AS mcp_seq
  FROM datalake_request_logging_clean.collections AS c
  WHERE
    MAKE_DATE(c.year, c.month, c.day) >= CAST('{load_start_date}' AS DATE) - INTERVAL '1' DAY
    AND c.ts_request >= CAST('{load_start_date}' AS TIMESTAMP)
    AND NOT c.tool_name IS NULL
  ...
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY obs.id_trace, LOWER(obs.name) ORDER BY obs.ts_started ASC, obs.id_observation ASC) AS obs_seq
  FROM datalake_langfuse_clean.observations AS obs
  WHERE
    MAKE_DATE(obs.year, obs.month, obs.day) >= CAST('{load_start_date}' AS DATE) - INTERVAL '1' DAY
    AND obs.ts_started >= CAST('{load_start_date}' AS TIMESTAMP)
    AND obs.type = 'TOOL'
    AND NOT (
      obs.level = 'ERROR' AND obs.output IS NULL
    )
    AND LOWER(obs.name) IN ('get_financial_context_v1', 'get_annual_tax_report_v1', 'get_debt_breakdown_v1', 'get_negotiation_options_v1', 'get_ongoing_negotiation_information_v1', 'get_last_paid_invoices_v1', 'simulate_negotiation_v1', 'create_negotiation_v1', 'get_original_invoices_by_status_v1')
), mcp_with_obs AS (
  SELECT
    m.id_request,
    m.id_trace,
    ...
    m.ts_request,
    (
      m.mcp_http_status_code BETWEEN 200 AND 299
    ) AS is_mcp_success,
    t.obs_input AS mcp_input,
    CASE
      WHEN m.mcp_http_status_code IS NULL
      THEN NULL
      WHEN m.mcp_http_status_code BETWEEN 200 AND 299
      THEN NULL
      ELSE CAST(t.obs_output AS STRING)
    END AS mcp_tool_error_message
  FROM collections AS m
  LEFT JOIN tool_observations AS t
    ON t.id_trace = m.id_trace
    AND t.obs_tool_name_lower = LOWER(m.mcp_tool_name)
    AND t.obs_seq = m.mcp_seq
  LEFT JOIN datalake_langfuse_clean.traces AS trc
...
SELECT
  ...
  YEAR(TO_DATE(p.ts_request)) AS year,
  MONTH(TO_DATE(p.ts_request)) AS month,
  DAYOFMONTH(TO_DATE(p.ts_request)) AS day
FROM mcp_with_obs AS p
LATERAL VIEW
POSEXPLODE(p.calls_array) AS pos, call
WHERE
  NOT p.id_langfuse_session IS NULL AND p.environment = 'prod'
