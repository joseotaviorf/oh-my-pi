WITH counts AS (
  SELECT
    ...
    id_tenant_prospect,
    id_owner,
    COUNT(DISTINCT id_booking) FILTER(WHERE
      NOT ts_booking_created IS NULL) AS nbr_bookings_created,
    COUNT(DISTINCT id_booking) FILTER(WHERE
      NOT ts_visit_completed IS NULL) AS nbr_visits_completed,
    COUNT(DISTINCT id_booking) FILTER(WHERE
      NOT ts_visit_performed IS NULL) AS nbr_visits_performed,
    COUNT(DISTINCT id_booking) FILTER(WHERE
      NOT ts_booking_canceled IS NULL) AS nbr_visits_canceled,
    COUNT(DISTINCT id_offer) FILTER(WHERE
      NOT ts_direct_offer_submitted IS NULL) AS nbr_direct_offers_submitted,
    COUNT(DISTINCT id_offer) FILTER(WHERE
      NOT ts_offer_submitted IS NULL) AS nbr_offers_submitted,
    COUNT(DISTINCT id_offer) FILTER(WHERE
      NOT ts_offer_approved IS NULL) AS nbr_offers_approved,
    COUNT(DISTINCT id_reservation) FILTER(WHERE
      NOT ts_reservation_created IS NULL) AS nbr_reservations_created,
    COUNT(DISTINCT id_proposal) FILTER(WHERE
      NOT ts_proposal_created IS NULL) AS nbr_proposals_created,
    COUNT(DISTINCT id_proposal) FILTER(WHERE
      NOT ts_guarantee_paid IS NULL) AS nbr_guarantees_paid,
    COUNT(DISTINCT id_contract) FILTER(WHERE
      NOT ts_contract_created IS NULL) AS nbr_contracts_created,
    COUNT(DISTINCT id_contract) FILTER(WHERE
      NOT ts_contract_signed IS NULL) AS nbr_contracts_signed,
    COUNT(DISTINCT id_contract) FILTER(WHERE
      NOT dt_contract_terminated IS NULL) AS nbr_contracts_terminated,
    COUNT(DISTINCT id_advance_payment) FILTER(WHERE
      NOT ts_advance_payment_created IS NULL) AS nbr_advance_payments_created,
    COUNT(DISTINCT id_advance_payment) FILTER(WHERE
      NOT ts_advance_payment_paid IS NULL) AS nbr_advance_payments_paid,
...
SELECT
  ...
  rf.country_code,
  DATEDIFF(TO_DATE(ts_last_event), TO_DATE(ts_first_event)) AS days_first_touchpoint_to_last,
  DATEDIFF(TO_DATE(NOW()), TO_DATE(ts_last_event)) AS days_since_last_event,
...
JOIN datalake_rent_flows.rent_flows_types AS rt
  ON rf.country_code IS NOT DISTINCT FROM rt.country_code
  AND rf.first_touchpoint IS NOT DISTINCT FROM rt.first_touchpoint
  AND rf.status IS NOT DISTINCT FROM rt.status
  AND rf.is_step_rejected IS NOT DISTINCT FROM rt.is_step_rejected
  AND rf.is_valid_rent_flow IS NOT DISTINCT FROM rt.is_valid_rent_flow
  AND rf.has_visit_flow IS NOT DISTINCT FROM rt.has_visit_flow
  AND rf.has_offer_flow IS NOT DISTINCT FROM rt.has_offer_flow
  AND rf.has_direct_offer_flow IS NOT DISTINCT FROM rt.has_direct_offer_flow
  AND rf.has_tta_flow IS NOT DISTINCT FROM rt.has_tta_flow
  AND IF(c.nbr_contracts_signed > 0, TRUE, FALSE) IS NOT DISTINCT FROM rt.had_contract_signed
LEFT JOIN datalake_company.company_sks AS cs
  ON (
    NOT rf.uuid_company IS NULL AND rf.uuid_company = cs.uuid_company
  )
  OR (
    rf.uuid_company IS NULL
    AND NOT rf.id_company_hubspot IS NULL
