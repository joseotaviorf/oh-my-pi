WITH get_first_process_closure AS (
  SELECT
    id_case,
    closure_reason,
    closure_result,
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY id_case ORDER BY dt_status_changed) AS _w,
      dt_status_changed
  ...
  PIVOT(SUM(COALESCE(expense_amount, 0)) AS expense_amount, MAX(dt_start) AS dt_start, MAX(dt_end) AS dt_end FOR stage_description IN ('Distribuicao Arbitral' AS stage_distribuicao_arbitral, 'Citacao Arbitral' AS stage_citacao_arbitral, 'Sentenca Arbitral' AS stage_sentenca_arbitral, 'Distribuicao Judicial' AS stage_distribuicao_judicial, 'Decisao de Citacao Judicial' AS stage_decisao_citacao_judicial, 'Citacao Judicial' AS stage_citacao_judicial, 'Decisao Coercitivo' AS stage_decisao_coercitivo, 'Emissao Coercitivo' AS stage_emissao_coercitivo, 'Peticao Judicial' AS stage_peticao_judicial, 'Decisao Imissao na Posse' AS stage_decisao_imissao_posse, 'Emissao Imissao na Posse' AS stage_emissao_imissao_posse, 'Requerimento da Execucao' AS stage_requerimento_execucao, 'Arresto Cautelar' AS stage_arresto_cautelar, 'Citacao do Executado' AS stage_citacao_execucao, 'Penhora' AS stage_penhora, 'Avaliacao dos Bens' AS stage_avaliacao_bens, 'Satisfacao do Credito' AS stage_satisfacao_credito))
)
SELECT DISTINCT
  ...
  CASE
    WHEN COUNT(*) OVER (PARTITION BY BIGINT(id_contract)) > 1
    THEN TRUE
    ELSE FALSE
  END AS is_reincident,
  IF(p.case_status = 'Active' AND NOT fpc.dt_first_closure IS NULL, TRUE, FALSE) AS is_reopened,
  ...
  p.ldt_stock,
  CASE
    WHEN p.ldt_stock BETWEEN 0 AND 120
    THEN '<120D'
    WHEN p.ldt_stock BETWEEN 121 AND 240
    THEN '120-240D'
    WHEN p.ldt_stock BETWEEN 241 AND 360
    THEN '240-360D'
    WHEN p.ldt_stock BETWEEN 361 AND 5000
    THEN '>360D'
    ELSE NULL
  END AS stock_range,
  CASE
    WHEN CAST(p.dt_case_acceptance AS DATE) > CAST(fpc.dt_first_closure AS DATE)
    THEN 0
    WHEN NOT fpc.dt_first_closure IS NULL
    THEN DATEDIFF(
      TO_DATE(LAST_DAY(ADD_MONTHS(fpc.dt_first_closure, 0))),
      TO_DATE(p.dt_case_acceptance)
    )
    ELSE NULL
  END AS ldt_resolution,
