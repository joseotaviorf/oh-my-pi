WITH get_external_id_contract AS (
  SELECT DISTINCT
  ...
  FROM (
    SELECT
      ...
      ts_last_update,
      ts_last_activity,
      ROW_NUMBER() OVER (PARTITION BY id_contract ORDER BY ts_last_update DESC) AS _w
...
SELECT
  ...
  crt.court_name,
  CASE
    WHEN c.process_type = 'Despejo'
    THEN 'EVICTIONS'
    WHEN c.process_type = 'Execucao'
    THEN 'EXECUTION'
    WHEN c.process_type = 'Reivindicatoria'
    THEN 'CLAIM'
    ELSE c.process_type
  END AS process_type,
  ...
  c.dt_status_changed,
  CASE
    WHEN CAST(CASE WHEN c.case_status = 'Completed' THEN c.dt_status_changed ELSE NULL END AS DATE) IS NULL
    AND c.dt_case_accepted > LAST_DAY(ADD_MONTHS(CURRENT_DATE, 0))
    THEN 0
    WHEN CAST(CASE WHEN c.case_status = 'Completed' THEN c.dt_status_changed ELSE NULL END AS DATE) IS NULL
    THEN DATEDIFF(TO_DATE(LAST_DAY(ADD_MONTHS(CURRENT_DATE, 0))), TO_DATE(c.dt_case_accepted))
    ELSE NULL
  END AS ldt_stock,
  CASE
    WHEN CAST(c.dt_case_accepted AS DATE) > CAST(CASE WHEN c.case_status = 'Completed' THEN c.dt_status_changed ELSE NULL END AS DATE)
    THEN 0
    WHEN NOT IF(c.case_status = 'Completed', c.dt_status_changed, NULL) IS NULL
    THEN DATEDIFF(
      TO_DATE(
        LAST_DAY(ADD_MONTHS(IF(c.case_status = 'Completed', c.dt_status_changed, NULL), 0))
      ),
      TO_DATE(c.dt_case_accepted)
    )
    ELSE NULL
  ...
  ct.ts_contract_end,
  c.ts_updated
FROM datalake_cyber_legal_clean.case AS c
LEFT JOIN datalake_cyber_legal_clean.text_content AS ec
