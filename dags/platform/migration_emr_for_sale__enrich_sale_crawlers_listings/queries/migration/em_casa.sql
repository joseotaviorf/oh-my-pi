WITH em_casa_listings_raw AS (
  SELECT
    ...
    REGEXP_REPLACE(
      LOWER(CAST(address.street AS STRING)),
      'avenida|rua|av.|#|&|1|2|3|4|5|6|7|8|9| da | de | do | das | dos ',
      ''
    ) AS address_for_join,
    ...
    CONCAT(
      UPPER(SUBSTRING(CAST(house_info.unit_type AS STRING), 1, 1)),
      LOWER(SUBSTRING(CAST(house_info.unit_type AS STRING), 2, 15))
    ) AS unit_type,
    CAST(NULL AS STRING) AS usage_type,
    CAST(price.sale.price AS DOUBLE) AS price,
    (
      CAST(price.sale.price AS DOUBLE) / CAST(house_info.area AS INT)
    ) AS price_m2,
    ...
    ) OVER (PARTITION BY id ORDER BY CAST(CONCAT(CAST(year AS STRING), '-', CAST(month AS STRING), '-', CAST(day AS STRING)) AS DATE) DESC) AS ts_last_publication,
    FIRST_VALUE(
      CAST(CONCAT(CAST(year AS STRING), '-', CAST(month AS STRING), '-', CAST(day AS STRING)) AS DATE)
    ) OVER (PARTITION BY 1 ORDER BY CAST(CONCAT(CAST(year AS STRING), '-', CAST(month AS STRING), '-', CAST(day AS STRING)) AS DATE) DESC) AS ts_last_extraction
  FROM datalake_crawlers_listings_clean.em_casa
), em_casa_listings AS (
  SELECT
    id_house,
    id_house_platform,
  ...
  FROM (
    SELECT
      ...
      rm.neighborhood,
      r.city_group,
      ROW_NUMBER() OVER (PARTITION BY id_house_platform ORDER BY rm.ts_updated DESC) AS _w,
      rm.ts_updated
    FROM datalake_crawlers_listings.em_casa_region_mapping AS rm
...
SELECT
  ...
  CAST(NULL AS INT) AS nearby_other_platform_houses,
