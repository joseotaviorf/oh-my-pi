WITH houses AS (
  ...
  FROM datalake_crawlers_listings.em_casa_region_mapping
  ...
  FROM (
    SELECT
      ...
      id_neighborhood,
      neighborhood,
      CASE WHEN ts_updated = ad.date THEN 'Publicado' ELSE 'Despublicado' END AS status_history,
      ts_last_extraction,
      ad.date AS ts_started_date,
      LAG(CASE WHEN ts_updated = ad.date THEN 'Publicado' ELSE 'Despublicado' END) OVER (PARTITION BY id_house ORDER BY ad.date) AS _w,
      ad.date
    FROM houses AS h
    JOIN datalake_quintoandar.aux_date AS ad
      ON ad.date >= ts_updated
      AND (
        ad.date < COALESCE(dt_next_updated, ts_last_extraction + 1)
      )
