WITH ivr AS (
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY id_task ORDER BY ts_created DESC) AS _w,
      ts_created
...
SELECT
  ...
  ts_ivr_started,
  YEAR(TO_DATE(ts_ivr_started)) AS year,
  MONTH(TO_DATE(ts_ivr_started)) AS month,
  DAY(TO_DATE(ts_ivr_started)) AS day
