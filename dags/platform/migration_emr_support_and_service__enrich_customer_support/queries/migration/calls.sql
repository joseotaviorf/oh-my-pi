WITH sss_and_sauron_call_sessions AS (
  ...
  WHERE
    source IN ('call_in_app', 'call')
    AND MAKE_DATE(year, month, day) BETWEEN '{load_start_date}' - INTERVAL '30' DAY AND '{load_end_date}'
    AND ts_created >= CAST('2025-11-10' AS DATE)
), call_sessions AS (
  SELECT
    source_identity,
    id_session,
  ...
  FROM (
    SELECT
      ...
      id_user,
      id_sss_session,
      ROW_NUMBER() OVER (PARTITION BY source_identity ORDER BY id_session) AS _w
    FROM sss_and_sauron_call_sessions
  ) AS _t
  WHERE
    _w = 1
), call_events AS (
  SELECT
    ...
    CASE
      WHEN LOWER(direction) = 'outbound'
      THEN outbound_to_phone_number
  ...
  FROM (
    SELECT DISTINCT
      ...
      id_queue,
      queue_name,
      ROW_NUMBER() OVER (PARTITION BY id_reservation ORDER BY ts_task_created) AS _w,
      ts_task_created
    FROM call_events
  ) AS _t
  WHERE
  ...
  FROM (
    SELECT
      ...
      id_queue,
      queue_name,
      ROW_NUMBER() OVER (PARTITION BY id_task ORDER BY ts_task_created) AS _w,
      ts_task_created
    FROM call_events
  ) AS _t
  WHERE
  ...
  SELECT
    ...
    COUNT(
      CASE
        WHEN event_type = 'reservation.accepted' OR event_type = 'reservation.completed'
        THEN id_reservation
      END
    ) > 0 AS is_reservation_answered,
    COUNT(CASE WHEN event_type = 'reservation.timeout' THEN id_reservation END) > 0 AS is_reservation_timeout,
    COUNT(CASE WHEN event_type = 'reservation.rejected' THEN id_reservation END) > 0 AS is_reservation_rejected,
    COUNT(CASE WHEN event_type = 'reservation.canceled' THEN id_reservation END) > 0 AS is_reservation_canceled,
    MIN(CASE WHEN event_type = 'reservation.created' THEN ts_created END) AS ts_reservation_created,
    MIN(CASE WHEN event_type = 'reservation.accepted' THEN ts_created END) AS ts_reservation_accepted,
  ...
  FROM call_events AS ce
  LEFT JOIN reservations AS r
    ON r.id_task = ce.id_task
  WHERE
    NOT ce.direction IS NULL
    AND NOT ce.task_cancelation_reason IS NULL
  ...
  SELECT
    ...
    COALESCE(
      CAST(SUM(CAST(is_reservation_answered AS INT)) OVER (PARTITION BY r.id_task ORDER BY r.ts_reservation_created ROWS BETWEEN 1 FOLLOWING AND UNBOUNDED FOLLOWING) AS BOOLEAN),
      FALSE
  ...
  LEFT JOIN queues_per_reservation AS qpr
    ON qpr.id_reservation = r.id_reservation
  LEFT JOIN call_answered_flag AS caf
  ...
  SELECT
    ...
    CAST(SUM(
      CAST(CASE
        WHEN r.has_following_answered_reservation IS FALSE AND r.total_queues > 1
        THEN TRUE
        ELSE FALSE
      END AS INT)
    ) OVER (PARTITION BY r.id_task ORDER BY r.ts_reservation_created ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS BOOLEAN) AS ends_in_abandon,
    r.is_reservation_answered,
    r.is_reservation_timeout,
    r.is_reservation_rejected,
    r.is_reservation_canceled,
...
SELECT DISTINCT
  ...
  c.id_worker,
  CASE
    WHEN c.channel_type = 'call-in-app' OR c.direction = 'outbound-api'
    THEN 'in app'
    ELSE c.direction
  END AS origin,
  c.direction,
  ...
  YEAR(TO_DATE(ts_task_created)) AS year,
  MONTH(TO_DATE(ts_task_created)) AS month,
  DAY(TO_DATE(ts_task_created)) AS day
FROM calls AS c
LEFT JOIN reservation_queues AS rq
  ON rq.id_reservation = c.id_reservation
LEFT JOIN task_queues AS tq
  ON tq.id_task = c.id_task AND c.id_reservation IS NULL
LEFT JOIN call_sessions AS cs
  ON cs.source_identity = c.id_call OR cs.source_identity = c.id_task
