...
WITH zendesk_email AS (
  SELECT
    ...
    custom_fields,
    CASE
      WHEN CAST(requester_wait_time_min_business AS INT) / (
        60.0 * COALESCE(CAST(replies AS INT), 1)
      ) < 6
      THEN TRUE
      WHEN CAST(requester_wait_time_min_business AS INT) / (
        60.0 * COALESCE(CAST(replies AS INT), 1)
  ...
  FROM datalake_zendesk.tickets_current
  WHERE
    id_session IS NULL
    AND id_call IS NULL
    AND ts_created >= CAST('{load_start_date}' AS DATE) - INTERVAL '6' MONTH
), csat AS (
  SELECT
    id_ticket,
  ...
  SELECT
    ...
    csat_score,
    NOT COALESCE(CAST(user_comment AS STRING), CAST(csat_score AS STRING), CAST(is_solved AS STRING)) IS NULL AS is_answered,
    is_solved,
    ts_first_response AS ts_response
  FROM datalake_survicate.zendesk_email_surveys
  WHERE
    NOT id_ticket IS NULL
    AND NOT COALESCE(CAST(user_comment AS STRING), CAST(csat_score AS STRING), CAST(is_solved AS STRING)) IS NULL
), csat_answers AS (
  SELECT
    id_ticket,
    csat_comment,
  ...
  FROM csat_answers AS ca
  INNER JOIN csat_answers AS ca2
    ON ca2.id_ticket = ca.id_ticket AND ca2.rw_number_asc = 1
  WHERE
    ca.rw_number_desc = 1
), back_tickets AS (
  SELECT
    ...
    COALESCE(
      NULLIF(
        REGEXP_EXTRACT(custom_fields['Ticket do contato'], '([0-9]{{8}})'),
        ''
      ),
      REGEXP_EXTRACT(
        SUBSTRING(
          SPLIT(
            REGEXP_REPLACE(description, 'WT[a-z0-9]{{20,40}}', ''),
            'Ticket do contato'
          )[1],
          1,
          18
  ...
  WHERE
    ...
    AND (
      NOT tags LIKE '%bot_end_conversation%' AND NOT tags LIKE '%closed_by_merge%'
    )
    AND COALESCE(
      NULLIF(
        REGEXP_EXTRACT(custom_fields['Ticket do contato'], '([0-9]{{8}})'),
  ...
  FROM (
    SELECT
      ...
      lt.ts_first_created,
      CAST((
        UNIX_TIMESTAMP(lt.ts_last_solved) - UNIX_TIMESTAMP(lt.ts_first_created)
      ) / 60.0 AS DOUBLE) AS total_backoffice_minutes_time,
      ROW_NUMBER() OVER (PARTITION BY bt.front_ticket ORDER BY lt.ts_first_created DESC) AS _w
    FROM back_tickets AS bt
    INNER JOIN last_and_first_back_tickets_timestamps AS lt
      ON lt.front_ticket = bt.front_ticket AND lt.ts_last_solved = bt.ts_ticket_solved
  ) AS _t
  WHERE
    _w = 1
)
SELECT DISTINCT
  ...
  ze.id_contract,
  ze.agent_email,
  CASE WHEN ze.tags LIKE '%"ticket_ativo"%' THEN 'outbound' ELSE 'inbound' END AS direction,
  ze.tags,
  ze.status,
  ze.department,
  ze.ticket_type,
  ...
  dc.team,
  dc.area,
  NOT bt.front_ticket IS NULL AS has_back_tickets,
  ze.tags LIKE '%bot_end_conversation%' AS is_bot,
  ze.tags LIKE '%closed_by_merge%' AS is_closed_by_merge,
  CASE
    WHEN dc.front_or_back = 'Front'
    THEN 'front'
    WHEN dc.front_or_back = 'Back' OR ze.tags LIKE '%tarefa_atendimento_escalado%'
    THEN 'back'
    ELSE 'undefined'
  END AS front_or_back,
  NOT bt.back_ticket IS NULL AS has_back_ticket,
  CAST((
    UNIX_TIMESTAMP(COALESCE(bt.ts_ticket_solved, ze.ts_ticket_solved)) - UNIX_TIMESTAMP(ze.ts_ticket_started)
  ) / 60.0 AS DOUBLE) AS frt,
  bt.is_open_back_ticket,
  bt.back_ticket_list,
  bt.back_ticket AS last_back_ticket,
  bt.total_backoffice_minutes_time,
  CAST((
    UNIX_TIMESTAMP(bt.ts_first_created) - UNIX_TIMESTAMP(ze.ts_ticket_solved)
  ) / 60.0 AS DOUBLE) AS total_minutes_front_to_open_back_ticket_time,
  ...
  ze.ts_ticket_solved,
  ze.ts_ticket_ended
FROM zendesk_email AS ze
LEFT JOIN datalake_gsheets_clean.department_control AS dc
  ON dc.department = ze.department
LEFT JOIN csat_first_and_last_ts AS cs
  ON ze.id_ticket = cs.id_ticket
LEFT JOIN last_back_ticket AS bt
  ON bt.front_ticket = ze.id_ticket
WHERE
  ze.channel IN ('email', 'form_faq', 'web', 'other', 'whatsapp')
  AND /* emails with the tags below are not new demands or automatically closed, therefore, they should not be considered */ NOT ze.tags LIKE '%resolve_ticket_acompanhamento%'
  AND NOT ze.tags LIKE '%fechado_automaticamente_noreply%'
  AND NOT ze.tags LIKE '%redirecionado_atendimento_2%'
  AND NOT ze.tags LIKE '%closed_by_merge%'
  AND NOT ze.tags LIKE '%zapdesk%'
  AND NOT ze.tags LIKE '%ticket_via_call%'
  AND NOT ze.tags LIKE '%call_contato_receptivo%'
  AND NOT ze.tags LIKE '%call_contato_ativo%'
  AND NOT ze.tags LIKE '%redirecionado_adm_v1%'
