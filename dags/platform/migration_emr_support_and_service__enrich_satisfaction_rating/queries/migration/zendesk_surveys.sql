WITH zendesk_users_contact AS (
  ...
  FROM (
    SELECT
      ...
      zuc.role,
      zuc.id_user_main,
      MAX(zuc.id_user_main) OVER (PARTITION BY zuc.id_user_zendesk) AS _w
    FROM datalake_support_users.zendesk_users AS zuc
    WHERE
...
LEFT JOIN zendesk_users_contact AS zuc
  ON zuc.id_user_zendesk = tfm.id_requester
WHERE
  CAST(sr.ts_updated AS DATE) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
