SELECT
  ...
  cmm.total_cost,
  CAST(cmm.impressions AS FLOAT) AS impressions,
  CAST(cmm.clicks AS FLOAT) AS clicks
FROM datalake_consolidated_growth_metrics.consolidated_media_metrics AS cmm
LEFT JOIN datalake_growth_taxonomy.media_setup AS msp
  ON cmm.campaign_name_convention_media_setup = msp.naming_convention_sufix
WHERE
  cmm.id_date BETWEEN CAST(REPLACE('{load_start_date}', '-', '') AS INT) AND CAST(REPLACE('{load_end_date}', '-', '') AS INT)
  AND (
    cmm.total_cost <> 0 OR cmm.impressions <> 0 OR cmm.clicks <> 0
  )
