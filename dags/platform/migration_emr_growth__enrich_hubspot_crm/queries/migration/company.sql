WITH merged_companies_aux AS (
  ...
  FROM (
    SELECT
      ids_merged_companies,
      ROW_NUMBER() OVER (PARTITION BY id_company ORDER BY ts_updated DESC) AS _w,
      id_company,
      ts_updated
    FROM datalake_hubspot.company_history
  ) AS _t
  WHERE
    _w = 1
), merged_companies AS (
  ...
  SELECT
    ...
    rfc,
    ts_updated,
    LAST(sale_lead_status) OVER (PARTITION BY id_company ORDER BY ts_updated ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS current_sale_status,
    LAST(rent_lead_status) OVER (PARTITION BY id_company ORDER BY ts_updated ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS current_rent_status,
    LAST(tag_real_estate_agency) OVER (PARTITION BY id_company ORDER BY ts_updated ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS current_tag,
    LAST(is_archived) OVER (PARTITION BY id_company ORDER BY ts_updated ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS is_currently_archived
  FROM datalake_hubspot.company_history
), listing_ownership /* We're repeating part of the logic applied to datalake_company.company */ /* We can't use it directly here because it would delay the start of the query, */ /* impacting enrich_ebdb_listing */ AS (
  SELECT
  ...
  WHERE
    hlr.related_as = 'LISTING_OWNER' AND hlr.source_type = 'COMPANY_REF'
  GROUP BY
    1
), company_document AS (
  SELECT
  ...
  FROM (
    SELECT
      ...
      CASE
        WHEN a.country IN ('Brasil', 'Brazil', 'BR') OR a.country IS NULL
        THEN NULLIF(REGEXP_REPLACE(identification_number, '[^0-9]', ''), '')
      END AS cnpj,
      CASE
        WHEN a.country IN ('México', 'Mexico', 'MX')
        THEN NULLIF(REGEXP_REPLACE(identification_number, '[^0-9A-Za-z]', ''), '')
      END AS rfc,
      NULLIF(REGEXP_REPLACE(identification_number, '[^0-9A-Za-z]', ''), '') AS document,
      c.status AS company_status,
      CASE d.status WHEN 'ACTIVE' THEN 0 ELSE 1 END AS document_status_preference,
      ROW_NUMBER() OVER (PARTITION BY COALESCE(c.uuid_company, d.uuid_company) ORDER BY CASE d.status WHEN 'ACTIVE' THEN 0 ELSE 1 END, d.ts_updated DESC) AS _w,
      d.ts_updated
    FROM datalake_company_clean.document AS d
    LEFT JOIN datalake_company_clean.company_document AS cd
  ...
  WHERE
    _w = 1
), company_matches_aux /* Select the best match for each hubspot company. */ AS (
  SELECT
    uuid_company,
    id_company_hubspot,
    id_merged_company,
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY ch.id_company ORDER BY lo.houses_currently_owned DESC /* First, companies with houses */, cd.company_status IS NOT DISTINCT FROM 'ACTIVE' DESC /* Then, active companies */) AS _w,
      ch.id_company,
      lo.houses_currently_owned,
      cd.company_status
    FROM company_document AS cd
    LEFT JOIN listing_ownership AS lo
      ON lo.uuid_company = cd.uuid_company
    JOIN hubspot_companies AS ch
      ON cd.document = ch.document OR cd.cnpj = ch.cnpj OR cd.rfc = ch.rfc
    LEFT JOIN merged_companies AS mc
      ON ch.id_company = mc.id_merged_company
  ) AS _t
  WHERE
    _w = 1
), company_matches /* Sometimes multiple hubspot companies match the same uuid_company. Then, here we select the best match for each uuid_company */ AS (
  SELECT
    uuid_company,
    id_company_hubspot
  FROM (
    SELECT
      uuid_company,
      id_company_hubspot,
      ROW_NUMBER() OVER (PARTITION BY uuid_company ORDER BY NOT is_currently_archived AND id_merged_company IS NULL DESC /* First, not archived nor merged */, is_current_hubspot_member DESC /* Then, the ones that are currently members */, NOT current_tag IS NULL DESC /* Then, the ones with tags */, ts_hubspot_updated DESC /* Otherwise, most recent */) AS _w,
      is_current_hubspot_member,
      ts_hubspot_updated,
      is_currently_archived,
      id_merged_company,
      current_tag
    FROM company_matches_aux
  ) AS _t
  WHERE
    _w = 1
)
...
FROM (
  SELECT
    ...
    ch.member_type,
    ch.member_category,
    ch.sale_member_category, /* The row below will be duplicated with the row above until June 7th, so we give time for people to update their queries */ /* After that, member_category will be deprecated and we will remove the row above */
    ch.rent_member_category,
    ch.company_cluster,
    ch.member_category_history,
    ch.sale_member_category_history, /* The row below will be duplicated with the row above until June 7th, so we give time for people to update their queries */ /* After that, member_category_history will be deprecated and we will remove the row above */
    ...
    CASE
      WHEN NOT mc.id_merged_company IS NULL OR ch.is_archived
      THEN 'Archived'
      ELSE ch.lead_status
    END AS lead_status,
    CASE
    ...
    END AS sale_lead_status, /* The row below will be duplicated with the row above until June 7th, so we give time for people to update their queries */ /* After that, lead_status will be deprecated and we will remove the row above */
    CASE
      WHEN NOT mc.id_merged_company IS NULL OR ch.is_archived
      THEN 'Archived'
    ...
    ch.sale_lead_status_history, /* The row below will be duplicated with the row above until June 7th, so we give time for people to update their queries */ /* After that, lead_status will be deprecated and we will remove the row above */
    ...
    NOT mc.id_merged_company IS NULL OR ch.is_archived AS is_archived,
    NOT mc.id_merged_company IS NULL AS is_merged_into_other_company,
    ...
    IF(NOT mc.id_merged_company IS NULL, ch.ts_updated, ch.ts_archived) AS ts_archived,
    ch.ts_created,
    ch.ts_updated,
    ch.year,
    ch.month,
    ch.day,
    ROW_NUMBER() OVER (PARTITION BY ch.id_company ORDER BY ch.ts_updated DESC) AS _w
  FROM datalake_hubspot.company_history AS ch
  LEFT JOIN company_matches AS cm
    ON ch.id_company = cm.id_company_hubspot
  LEFT JOIN merged_companies AS mc
    ON ch.id_company = mc.id_merged_company
  LEFT JOIN datalake_hubspot.company_contact AS cc
