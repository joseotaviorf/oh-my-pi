SELECT
  ...
  NULLIF(GET_JSON_OBJECT(properties, '$.categoria_do_membro'), '') AS sale_member_category, /* The row below will be duplicated with the row above until June 7th, so we give time for people to update their queries */ /* After that, lead_status will be deprecated and we will remove the row above */
  ...
  FROM_JSON(
    GET_JSON_OBJECT(properties_with_history, '$.categoria_do_membro'),
    'array<struct<\n            value:string,\n            timestamp:timestamp,\n            sourceType:string,\n            sourceId:string,\n            sourceLabel:string,\n            updatedByUserId:string\n        >>'
  ) AS sale_member_category_history, /* The row below will be duplicated with the row above until June 7th, so we give time for people to update their queries */ /* After that, lead_status will be deprecated and we will remove the row above */
  FROM_JSON(
  ...
  NULLIF(
    GET_JSON_OBJECT(properties, '$.hs_analytics_first_touch_converting_campaign'),
    ''
  ) AS hs_analytics_first_touch_converting_campaign,
  NULLIF(GET_JSON_OBJECT(properties, '$.qual_'), '') AS crm,
  SPLIT(
    NULLIF(GET_JSON_OBJECT(properties, '$.com_quais_imobiliarias_tem_parceria_'), ''),
    ';'
  ) AS partner_agencies,
  ...
  SPLIT(
    NULLIF(
      GET_JSON_OBJECT(
        properties,
        '$.qual_a_solucao_empresa_de_garantia_locaticia_oferece_para_os_clientes_de_locacao_'
      ),
      ''
    ),
    ';'
  ) AS rental_guarantee_solutions,
  NULLIF(GET_JSON_OBJECT(properties, '$.qual_o_foco_da_imobiliaria_'), '') AS real_estate_agency_focus,
  SPLIT(
    NULLIF(
      GET_JSON_OBJECT(properties, '$.trabalha_com_financiamento__se_sim__quais_bancos_'),
      ''
    ),
    ';'
  ) AS financing_banks,
  CASE
    ...
    OR NULLIF(GET_JSON_OBJECT(properties, '$.country'), '') IS NULL
    THEN NULLIF(REGEXP_REPLACE(GET_JSON_OBJECT(properties, '$.cnpj'), '[^0-9]', ''), '')
  END AS cnpj,
  NULLIF(
    REGEXP_REPLACE(GET_JSON_OBJECT(properties, '$.cnpj_unico'), '[^0-9A-Za-z]', ''),
    ''
  ) AS cnpj_unique,
  CASE
    WHEN NULLIF(GET_JSON_OBJECT(properties, '$.country_code'), '') = 'MX'
    THEN NULLIF(REGEXP_REPLACE(GET_JSON_OBJECT(properties, '$.cnpj'), '[^0-9A-Za-z]', ''), '')
  END AS rfc,
  ...
  NULLIF(GET_JSON_OBJECT(properties, '$.hs_lead_status'), '') AS sale_lead_status, /* The row below will be duplicated with the row above until June 7th, so we give time for people to update their queries */ /* After that, lead_status will be deprecated and we will remove the row above */
  ...
  FROM_JSON(
    GET_JSON_OBJECT(properties_with_history, '$.hs_lead_status'),
    'array<struct<\n            value:string,\n            timestamp:timestamp,\n            sourceType:string,\n            sourceId:string,\n            sourceLabel:string,\n            updatedByUserId:string\n        >>'
  ) AS sale_lead_status_history, /* The row below will be duplicated with the row above until June 7th, so we give time for people to update their queries */ /* After that, lead_status will be deprecated and we will remove the row above */
  FROM_JSON(
  ...
  NULLIF(
    GET_JSON_OBJECT(properties, '$.trabalha_em_parceria_com_outras_imobiliarias_'),
    ''
  ) = 'Sim' AS has_partnerships_with_other_agencies,
  ...
  month,
  day
FROM datalake_hubspot_clean.company
WHERE
