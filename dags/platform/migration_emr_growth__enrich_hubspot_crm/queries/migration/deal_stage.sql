WITH exploded_stages AS (
  SELECT
    CAST(id_deal AS BIGINT),
    EXPLODE(id_stage_history) AS stage_struct
  FROM datalake_hubspot.deal
...
SELECT
  ...
  id_user_updated_by,
  source_type,
  DATEDIFF(TO_DATE(COALESCE(cet.ts_stage_ended, NOW())), TO_DATE(cet.ts_stage_started)) AS days_in_stage,
  cet.ts_stage_started,
  cet.ts_stage_ended
FROM calculated_end_timestamps AS cet
JOIN datalake_hubspot.stage AS s
