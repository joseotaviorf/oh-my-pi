WITH exploded_history_sale AS (
  ...
  SELECT
    ...
    LEAD(lead_status_struct.timestamp) OVER (PARTITION BY id_company ORDER BY lead_status_struct.timestamp) AS ts_status_ended
  FROM exploded_history_sale
  UNION ALL
  SELECT
    id_company,
    lead_status_struct['updatedByUserId'] AS id_user_updated_by,
...
SELECT
  ...
  business_context,
  lead_status,
  DATEDIFF(TO_DATE(COALESCE(ts_status_ended, NOW())), TO_DATE(ts_status_started)) AS days_in_status,
