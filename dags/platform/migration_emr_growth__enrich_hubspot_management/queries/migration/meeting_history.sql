SELECT
  ...
  CAST(NULLIF(GET_JSON_OBJECT(properties, '$.hs_meeting_start_time'), '') AS TIMESTAMP) AS ts_meeting_start,
  CAST(NULLIF(GET_JSON_OBJECT(properties, '$.hs_meeting_end_time'), '') AS TIMESTAMP) AS ts_meeting_end,
  CAST(NULLIF(GET_JSON_OBJECT(properties, '$.hs_timestamp'), '') AS TIMESTAMP) AS ts_meeting,
  ts_created,
  ts_updated,
  year,
  month,
  day
FROM datalake_hubspot_clean.meeting
WHERE
