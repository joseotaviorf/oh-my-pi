WITH lpv AS (
  ...
  SELECT
    ...
    sc.id_house,
    sc.id_region,
    DATEDIFF(TO_DATE(d.date), TO_DATE(CAST(sc.ts_last_publication AS DATE))) + 1 AS days_published,
  ...
  JOIN datalake_quintoandar.aux_date AS d
    ON d.date >= CAST(sc.ts_status_started AS DATE)
    AND d.date < COALESCE(CAST(sc.ts_status_ended AS DATE), NOW())
  WHERE
...
FROM (
  ...
  LEFT JOIN datalake_ebdb_listing.house AS h
    ON dol.id_house = h.id AND h.is_sale_3p_supply
  LEFT JOIN datalake_company.company_sks AS cs_supply
    ON (
      NOT h.uuid_company IS NULL AND h.uuid_company = cs_supply.uuid_company
    )
    OR (
      h.uuid_company IS NULL
      AND NOT h.id_company_hubspot IS NULL
