WITH amplitude_data AS (
  ...
  FROM (
    SELECT
      ...
      dd.month,
      dd.day,
      ROW_NUMBER() OVER (PARTITION BY sc.sk_house, dd.`date` ORDER BY sc.ts_status_started DESC) AS _w,
      dd.`date`,
      sc.ts_status_started
    FROM status_changes AS sc
    JOIN dw_public.dim_date AS dd
      ON dd.`date` BETWEEN CAST(sc.ts_status_started AS DATE) AND COALESCE(CAST(sc.ts_status_ended AS DATE), NOW())
    WHERE
      MAKE_DATE(dd.year, dd.month, dd.day) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
