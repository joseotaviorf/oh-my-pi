WITH historical_prices AS (
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY lbc.id_house, DATE_TRUNC('DAY', r.ts_revision) ORDER BY rev DESC) AS _w,
      r.ts_revision,
      rev
  ...
  FROM (
    SELECT
      ...
      has_great_sale_price_tag,
      ts_change,
      LAG(has_great_sale_price_tag) OVER (PARTITION BY id_house ORDER BY ts_change ASC) AS _w
    FROM great_price_tag_status_by_day
  ) AS _t
  WHERE
    _w IS DISTINCT FROM has_great_sale_price_tag
  ...
  FROM (
    SELECT
      ...
      d.is_actual,
      d.ts_price_started,
      ROW_NUMBER() OVER (PARTITION BY d.id_house, d.ts_price_started ORDER BY t.ts_change ASC) AS _w,
      t.ts_change
  ...
  FROM (
    SELECT
      ...
      LAG(tier) OVER (PARTITION BY id_house ORDER BY ts_price_started) AS _w,
      ts_price_started
    FROM score_business_logic
  ) AS _t
  WHERE
    tier IS DISTINCT FROM _w OR is_actual = TRUE
