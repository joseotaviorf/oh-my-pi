...
segments_perspective AS (
  ...
  WHERE fcc.channel IN ('chat', 'call')
...
started_and_ended_chat_time AS (
  SELECT
    ...
    DATE_FORMAT(
      NVL(DATE_ADD(SECOND, CAST(mq.talk_time AS INTEGER), fcc.ts_reservation_created), fcc.ts_reservation_created),
      'HH:mm:ss'
    ) AS chat_ended_time,
...
final AS (
  SELECT 
    ...
    IF(
      ...
      AND DAYOFWEEK(DATE(mq.ts_created)) BETWEEN 2 AND 6,
      TRUE,
      FALSE
    ) workday_chat
  FROM segments_perspective mq
