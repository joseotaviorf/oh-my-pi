...
recontact_check AS (
  SELECT
    ...
    rc.theme_detail,
    CASE
      WHEN DATE_DIFF(DAY, LAG(DATE(rc.ts_started)) OVER(PARTITION BY rc.sk_user, rc.team ORDER BY rc.ts_started), DATE(rc.ts_started)) <= 3 
  ...
  QUALIFY 
    ROW_NUMBER() OVER(PARTITION BY rc.sk_ticket, rc.sk_user, rc.team, rc.theme ORDER BY rc.ts_started DESC) = 1
),
demand_back_FRC AS (
  SELECT DISTINCT
    ft.ts_created AS ts_started,
