...
inspections AS (
  ...
  WHERE di.inspection_type IN ('offboarding', 'verification')
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY fi.sk_contract, fi.ts_synced 
    ORDER BY fi.ts_synced ASC
  ) = 1
),
...
terminator AS (
  SELECT
    ...
    CASE 
      WHEN MIN(DATE(ins.ts_synced)) > MIN(DATE(t.ts_termination_request)) THEN MIN(DATE(ins.ts_synced))
      ELSE NULL
    END inspection_date,
    ins.inspection_type,
...
new_mediation AS (
  ...
  QUALIFY ROW_NUMBER() OVER (
    ...
    ORDER BY t.ts_termination_request DESC
  ) = 1
),
...
offboarding AS (
  ...
  QUALIFY ROW_NUMBER() OVER (
    ...
    ORDER BY inspection_date ASC, termination_request DESC
  ) = 1
),
...
repair_resolution_terminations AS (
  ...
  QUALIFY ROW_NUMBER() OVER (
    ...
    ORDER BY ft.ts_termination_request DESC
  ) = 1
),
...
SELECT DISTINCT
  ...
  spc.spoc_wave,
  CASE 
    WHEN ofb.termination_request < DATE('2025-06-02') 
      AND spc.is_spoc_contract = TRUE 
      AND (spc.is_spoc_control_group = FALSE OR spc.is_spoc_control_group IS NULL) 
      THEN 'before_wave_6_lab_test'
    WHEN ofb.termination_request < DATE('2025-06-02') 
      AND spc.is_spoc_contract = TRUE 
      AND spc.is_spoc_control_group = TRUE 
      THEN 'before_wave_6_lab_control'
    WHEN ofb.termination_request >= DATE('2025-05-22') 
      AND spc.is_spoc_contract = TRUE 
      AND (spc.is_spoc_control_group = FALSE OR spc.is_spoc_control_group IS NULL) 
      AND (spc.team = 'ROLLOUT' OR spc.team IS NULL) 
      THEN 'rollout'
    WHEN ofb.termination_request BETWEEN DATE('2025-06-02') AND DATE('2025-07-29') 
      AND spc.is_spoc_contract = TRUE 
      AND (spc.is_spoc_control_group = FALSE OR spc.is_spoc_control_group IS NULL) 
      AND spc.team = 'LAB' 
      THEN 'wave_6_lab_test'
    WHEN ofb.termination_request BETWEEN DATE('2025-07-30') AND DATE('{load_start_date}') 
      AND spc.is_spoc_contract = TRUE 
      AND (spc.is_spoc_control_group = FALSE OR spc.is_spoc_control_group IS NULL) 
      AND spc.team = 'LAB' 
