...
FROM (
  SELECT
    ...
    c.month,
    c.day,
    ROW_NUMBER() OVER (PARTITION BY c.id_user, c.ts_concierge_contact, c.concierge_flow_type ORDER BY p.ts_prospect_event DESC) AS _w
  FROM datalake_search.concierge_messages AS c
  JOIN prospects AS p
    ON (
      p.id_user = c.id_user OR p.prospect_phone = c.user_phone
    )
