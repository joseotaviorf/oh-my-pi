WITH visits AS (
  ...
  SELECT
    ...
    CASE
      WHEN p.event_name = 'USER FIRST ACTIVATION'
      THEN 'new_prospect'
      WHEN p.event_name IN ('USER RECOVERY', 'USER RECOVERY IN OTHER CITY GROUP')
      THEN 'recovered_prospect'
  ...
  SELECT
    ...
    NOT m.user_phone IS NULL
    AND (
      p.id_user IS NULL
      OR p.prospect_event_type = 'prospect_churn'
      OR (
        p.prospect_event_type <> 'prospect_churn'
        AND CAST(p.ts_prospect_event AS DATE) = CAST(m.ts_concierge_contact AS DATE)
      )
    ...
    NOT pe.sk_prospect IS NULL AS is_concierge_prospect,
    ...
    COALESCE(m.ts_concierge_contact, CAST('1970-01-01' AS TIMESTAMP)) AS ts_concierge_contact,
    COALESCE(pe.ts_prospect_event, p.ts_prospect_event) AS ts_prospect_event,
    COALESCE(v.ts_concierge_visit_event, CAST('1970-01-01' AS TIMESTAMP)) AS ts_concierge_visit_event,
    v.ts_visit_created,
    COALESCE(m.year, v.year) AS year,
    COALESCE(m.month, v.month) AS month,
    COALESCE(m.day, v.day) AS day,
    ROW_NUMBER() OVER (PARTITION BY COALESCE(m.id_user, v.id_user), COALESCE(m.id_notification, CAST(-1 AS BIGINT)), COALESCE(m.id_phone_session, '-'), COALESCE(v.visit_code, '-'), COALESCE(m.concierge_flow_type, 'Unknown'), COALESCE(m.ts_concierge_contact, CAST('1970-01-01' AS TIMESTAMP)), COALESCE(v.ts_concierge_visit_event, CAST('1970-01-01' AS TIMESTAMP)) ORDER BY COALESCE(pe.ts_prospect_event, p.ts_prospect_event) DESC, v.id_visit DESC, pe.sk_prospect DESC) AS _row_num /* Dedup on the merge_on key: the joins above can fan out multiple rows that share the */ /* merge_on key but differ in non-key columns (e.g. several prospect activation events */ /* per visit), which breaks the Delta MERGE (DELTA_MULTIPLE_SOURCE_ROW_MATCHING_TARGET_ROW_IN_MERGE). */ /* QUALIFY is not EMR-compatible, so we rank here and filter _row_num = 1 in the outer query, */ /* keeping one row per merge_on key preferring the most recent prospect event. */
  FROM datalake_search.concierge_messages AS m
  LEFT JOIN datalake_search.concierge_prospects_aux AS p
    ON (
      m.id_user = p.id_user OR m.user_phone = p.prospect_phone
    )
