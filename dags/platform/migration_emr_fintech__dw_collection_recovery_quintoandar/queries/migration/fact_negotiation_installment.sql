WITH nexxera_confirmation AS (
  ...
  FROM (
    SELECT
      ...
      SUBSTRING(our_number, 1, 8) AS our_number,
      net_amount AS paid_amount,
      due_amount,
      ROW_NUMBER() OVER (PARTITION BY SUBSTRING(our_number, 1, 8), occurrence_code ORDER BY dt_occurrence_code DESC) AS _w,
      occurrence_code,
      dt_occurrence_code
  ...
  SELECT DISTINCT
    ...
    i.source,
    CASE
      WHEN i.installment_status IN ('pending', 'registered') AND NOT nx.paid_amount IS NULL
      THEN 'paid' /* Only use nexxera if others call it 'registered' */
      WHEN ri.status = 'paid'
      THEN 'paid'
      ELSE i.installment_status
    END AS installment_status,
    CASE
      WHEN UPPER(i.payment_method) IN ('CREDIT-CARD', 'CARTÃO', 'CARTÃO DE CRÉDITO')
    ...
    CASE
      ...
      THEN (
        i.main_amount + i.debts_fee_amount
      ) - i.amount_to_pay
    ...
    CASE
      WHEN i.installment_status IN ('pending', 'registered') AND NOT nx.paid_amount IS NULL
      THEN nx.paid_amount /* Only use nexxera if others call it 'registered' */
      ELSE i.paid_amount
    END AS paid_amount,
    i.dt_creation,
    i.dt_due,
    CASE
      WHEN i.installment_status IN ('pending', 'registered') AND NOT nx.paid_amount IS NULL
      THEN nx.dt_paid /* Only use nexxera if others call it 'registered' */
      ELSE COALESCE(CAST(ri.ts_paid AS DATE), i.dt_paid)
    END AS dt_paid,
    IF(i.installment_status IN ('canceled', 'expired'), i.dt_canceled, NULL) AS dt_canceled
  FROM datalake_collections_quintoandar.installment AS i
  LEFT JOIN nexxera_confirmation AS nx
    ON nx.our_number = i.our_number AND nx.dt_due = i.dt_due
  ...
  CAST(COALESCE(
    ...
    CASE
      WHEN discount_amount >= (
        debts_fee_amount + credit_card_fee_amount
      )
      ...
      ELSE 0
    END
  ) AS DECIMAL(14, 2)) AS discount_to_original,
  CAST(main_amount - COALESCE(
    discount_to_original_amount,
    CASE
      WHEN discount_amount >= (
        debts_fee_amount + credit_card_fee_amount
