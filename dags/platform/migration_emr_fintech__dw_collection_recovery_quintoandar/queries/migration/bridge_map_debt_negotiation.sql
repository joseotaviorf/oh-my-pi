WITH deduplicate_creditor_pending AS (
  ...
  FROM (
    SELECT
      ...
      CONCAT(
        n.id_contract,
        COALESCE(CAST(n.id_negotiation_external AS BIGINT), n.id_negotiation_external)
      ) AS id_negotiation,
      n.id_contract,
      'Trato Feito' AS source,
      1 AS priority,
      ROW_NUMBER() OVER (PARTITION BY n.id_contract, d.id_external, CONCAT(
        n.id_contract,
        COALESCE(CAST(n.id_negotiation_external AS BIGINT), n.id_negotiation_external)
      ) ORDER BY d.ts_created DESC) AS _w,
      d.id_external,
      d.ts_created
...
FROM (
  SELECT
    ...
    source,
    NOW() AS ts_load,
    ROW_NUMBER() OVER (PARTITION BY sk_debt, sk_negotiation ORDER BY priority) AS _w,
    priority
  FROM union_sources
) AS _t
WHERE
