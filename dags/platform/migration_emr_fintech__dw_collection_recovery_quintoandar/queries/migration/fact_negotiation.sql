WITH original_invoices AS (
  ...
  SELECT
    ...
    CASE
      ...
      AND u.negotiation_status = 'started'
      THEN 'PROMESSA'
      WHEN COALESCE(i.dt_down_payment, u.dt_down_payment) IS NULL
      AND u.negotiation_status = 'canceled'
      THEN 'PROMESSA QUEBRADA'
      WHEN NOT COALESCE(i.dt_down_payment, u.dt_down_payment) IS NULL
      AND oi.total_invoices_negotiated >= 1
      AND u.number_of_installments > 1
      THEN 'ACORDO'
      WHEN NOT COALESCE(i.dt_down_payment, u.dt_down_payment) IS NULL
      AND oi.total_invoices_negotiated > 1
      AND u.number_of_installments = 1
    ...
    CASE
      WHEN UPPER(COALESCE(i.promisse_payment_method, u.promisse_payment_method)) IN ('CREDIT-CARD', 'CARTÃO', 'CARTÃO DE CRÉDITO')
    ...
    CASE
      WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(oi.dt_due_invoice_anchor)) <= 0
      THEN 'Current'
      WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(oi.dt_due_invoice_anchor)) <= 30
      THEN '1-30'
      WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(oi.dt_due_invoice_anchor)) <= 60
      THEN '31-60'
      WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(oi.dt_due_invoice_anchor)) <= 90
      THEN '61-90'
      WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(oi.dt_due_invoice_anchor)) <= 120
      THEN '91-120'
      WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(oi.dt_due_invoice_anchor)) <= 180
      THEN '121-180'
      WHEN DATEDIFF(TO_DATE(u.dt_promisse), TO_DATE(oi.dt_due_invoice_anchor)) IS NULL
      THEN NULL
      ELSE 'over 180'
    END AS delay_contamined_range,
    oi.total_invoices_negotiated,
    u.number_of_installments,
    IF(u.paid_installments = 0 AND NOT i.dt_down_payment IS NULL, 1, u.paid_installments) AS paid_installments,
    ...
    CASE
      ...
      AND (
        source LIKE '%Recupera%' OR source LIKE '%Migração%'
      )
    ...
    IF(
      NOT COALESCE(i.dt_down_payment, u.dt_down_payment) IS NULL,
      i.down_payment_net_amount,
      0
    ) AS down_payment_net_amount_paid,
    u.paid_amount,
    oi.dt_due_invoice_anchor,
    u.dt_promisse,
    u.dt_due_promisse,
    CAST(IF(u.negotiation_status <> 'finished', u.dt_cancellation, NULL) AS DATE) AS dt_cancellation,
  ...
  LEFT JOIN renegotiation AS r
    ON u.id_negotiation = r.id_negotiation AND u.id_negotiation = r.id_contract
  ...
  LEFT JOIN installments_data AS i
    ON u.id_negotiation = i.id_negotiation AND u.id_contract = sk_contract
  ...
  SELECT
    ...
    COALESCE(
      discount_to_credit_fee_amount,
      CASE
        WHEN credit_card_fee_amount <> 0
        AND total_discount_amount = (
          fine_fee_amount + interest_fee_amount + credit_card_fee_amount
        )
        THEN credit_card_fee_amount
        ELSE 0
      END
    ) AS discount_to_credit_fee_amount,
    COALESCE(
...
SELECT
  ...
  CAST(CASE
    WHEN number_of_installments = 1 AND NOT dt_down_payment IS NULL
    THEN original_debt_amount - discount_to_original_amount
    ELSE down_payment_net_amount_paid
  END AS DECIMAL(14, 2)) AS down_payment_net_amount_paid,
  CAST(CASE
    WHEN (
      promisse_payment_method = 'CARTÃO DE CRÉDITO' OR number_of_installments = 1
    )
