WITH negotiation_data AS (
  ...
  FROM (
    SELECT DISTINCT
      ...
      ROW_NUMBER() OVER (PARTITION BY ip.id_invoice, ip.id_contract ORDER BY ABS(DATEDIFF(TO_DATE(n.dt_down_payment), TO_DATE(ip.ts_paid)))) AS _w,
      n.dt_down_payment,
      ip.ts_paid
  ...
  SELECT
    ...
    IF(
      o.invoice_type = 'extra',
      CONCAT(COALESCE(o.id_contract, 0), CAST(o.id_negotiation_parent AS STRING)),
      NULL
    ) AS sk_origin_negotiation,
    ...
    ROUND(
      CASE
        ...
        WHEN o.payment_status = 'written-down' AND n.sk_negotiation IS NULL
        THEN ABS(o.recovered_amount)
    ...
    COALESCE(
      ...
      rc.partner,
      LAG(rc.partner) IGNORE NULLS OVER (PARTITION BY o.id_contract ORDER BY o.dt_reference)
    ) AS advisory,
    ...
    o.dt_reference,
    NOW() AS ts_load
  FROM datalake_collections_quintoandar.overdue_portfolio_timeline AS o
  LEFT JOIN datalake_recupera.contract_advisory_distribution AS rc
    ...
    AND rc.dt_snapshot BETWEEN DATE_TRUNC('MONTH', DATE_ADD(CAST('{load_start_date}' AS DATE), -90)) AND DATE_ADD(CAST('{load_end_date}' AS DATE), 1)
  LEFT JOIN datalake_cyber.agency_timeline AS cad
    ON o.id_contract = cad.id_contract
    AND o.dt_reference = cad.dt_reference
    AND cad.dt_reference BETWEEN DATE_TRUNC('MONTH', DATE_ADD(CAST('{load_start_date}' AS DATE), -90)) AND DATE_ADD(CAST('{load_end_date}' AS DATE), 1)
  LEFT JOIN datalake_cyber.queue_timeline_2 AS q
    ON o.id_contract = q.id_contract
    AND o.dt_reference = q.dt_reference
    AND q.dt_reference BETWEEN DATE_TRUNC('MONTH', DATE_ADD(CAST('{load_start_date}' AS DATE), -90)) AND DATE_ADD(CAST('{load_end_date}' AS DATE), 1)
  LEFT JOIN negotiation_data AS n
    ON o.id_invoice = n.id_invoice AND o.id_contract = n.id_contract
  WHERE
    o.dt_reference BETWEEN DATE_TRUNC('MONTH', DATE_ADD(CAST('{load_start_date}' AS DATE), -90)) AND DATE_ADD(CAST('{load_end_date}' AS DATE), 1)
  ...
  FROM (
    ...
    SELECT
      ...
      eviction_queue,
      eviction_queue_description,
      ROW_NUMBER() OVER (PARTITION BY id_contract, id_invoice ORDER BY dt_reference DESC) AS _w,
      dt_reference
    FROM add_all_dimensions
    WHERE
      dt_reference >= DATE_ADD(max_dt_invoice_paid, -1)
      AND dt_reference <= max_dt_invoice_paid
      AND (
        advisory IS NULL OR NOT advisory IN ('DBAIXAS', 'DCARGA')
      ) /* ignore DBAIXAS, because it references to the paid invoices, and we want the last valid partner before the invoice payment. */
  ) AS _t
  WHERE
    _w = 1
)
SELECT
  ...
  a.paid_via,
  CASE
    WHEN NOT a.has_app_action_event IS NULL
    AND NOT a.reason IN ('negotiation-recupera', 'negotiation-cyber', 'agreement')
    AND a.payment_status = 'paid'
    THEN TRUE
    WHEN NOT a.is_ssn_boletao IS NULL
    AND a.is_ssn_boletao IS TRUE
    AND a.payment_status = 'written-down'
    THEN TRUE
    ELSE FALSE
  END AS is_ssn,
  ...
  IF(
    a.dt_reference >= a.dt_invoice_paid,
    g.segmentation_queue_description,
    a.segmentation_queue_description
  ) AS segmentation_queue_description,
  IF(a.dt_reference >= a.dt_invoice_paid, g.agreement_queue, a.agreement_queue) AS agreement_queue,
  IF(
    a.dt_reference >= a.dt_invoice_paid,
    g.agreement_queue_description,
    a.agreement_queue_description
  ) AS agreement_queue_description,
  IF(a.dt_reference >= a.dt_invoice_paid, g.eviction_queue, a.eviction_queue) AS eviction_queue,
  IF(
    a.dt_reference >= a.dt_invoice_paid,
    g.eviction_queue_description,
    a.eviction_queue_description
  ) AS eviction_queue_description,
  IF(
    ROW_NUMBER() OVER (PARTITION BY a.id_contract, a.id_invoice, a.dt_month_start ORDER BY a.dt_reference DESC) = 1,
    TRUE,
    FALSE
  ) AS is_most_recent_record_month,
  a.dt_invoice_paid,
  a.dt_invoice_due,
  ...
  a.dt_month_end,
  a.dt_reference,
  YEAR(TO_DATE(a.dt_reference)) AS year,
  MONTH(TO_DATE(a.dt_reference)) AS month,
  DAY(TO_DATE(a.dt_reference)) AS day,
  a.ts_load
FROM add_all_dimensions AS a
LEFT JOIN get_last_valid_partner AS g
  ON a.id_contract = g.id_contract AND a.id_invoice = g.id_invoice
LEFT JOIN datalake_cyber.queue_timeline AS q
  ON a.id_contract = q.id_contract_external AND a.dt_reference = q.dt_reference
WHERE
  sk_origin_negotiation IS NULL
  OR (
    NOT sk_origin_negotiation IS NULL AND negotiation_installment_number <> 1
  )
  AND a.dt_reference BETWEEN DATE_TRUNC('MONTH', DATE_ADD(CAST('{load_start_date}' AS DATE), -90)) AND DATE_ADD(CAST('{load_end_date}' AS DATE), 1)
