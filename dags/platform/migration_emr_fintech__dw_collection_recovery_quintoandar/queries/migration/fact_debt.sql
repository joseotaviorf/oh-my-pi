WITH deduplicate_creditor_pending AS (
  ...
  FROM (
    ...
    WHERE
      NOT installment_code IS NULL AND NOT id_creditor IN (3, 5)
  ) AS _t
  WHERE
    _w = 1
  ...
  FROM (
    SELECT
      ...
      1 AS priority,
      ROW_NUMBER() OVER (PARTITION BY id_invoice ORDER BY dt_debt_created DESC) AS _w,
      dt_debt_created
  ...
  SELECT
    id_invoice,
    id_contract,
    CASE WHEN creditor = 'QuintoAndar' THEN 'IQ QuintoAndar' ELSE creditor END AS creditor,
    invoice_status,
    due_amount,
    interest_fee_amount,
    fine_fee_amount,
...
FROM (
  SELECT
    ...
    NOW() AS ts_load,
    ROW_NUMBER() OVER (PARTITION BY d.id_invoice, COALESCE(d.id_contract, r.id_contract) ORDER BY d.priority) AS _w,
    d.priority
  FROM debts AS d
