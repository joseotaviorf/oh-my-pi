...
FROM (
  SELECT
    ...
    aux_date.month,
    aux_date.day,
    ROW_NUMBER() OVER (PARTITION BY mp.id_member_profile, aux_date.date ORDER BY mp.ts_relationship_started DESC, mp.ts_relationship_ended DESC) AS _w,
    aux_date.date,
    mp.ts_relationship_started,
    mp.ts_relationship_ended
  FROM datalake_hub_services.member_profile AS mp
  JOIN datalake_quintoandar.aux_date
    ON aux_date.date BETWEEN CAST(mp.ts_relationship_started AS DATE) AND CAST(mp.ts_relationship_ended AS DATE)
  WHERE
    MAKE_DATE(aux_date.year, aux_date.month, aux_date.day) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
) AS _t
WHERE
  1 = _w