WITH member_profile_aud AS (
  SELECT DISTINCT
    ...
    COALESCE(
      rev.ts_created,
      LEAD(mpa.ts_created) OVER (PARTITION BY mpa.id ORDER BY mpa.ts_updated)
    ) AS ts_relationship_ended,
    mpa.ts_created,
    mpa.ts_updated,
    mpa.year,
    mpa.month,
    mpa.day
  FROM datalake_hub_services_clean.member_profile_aud AS mpa
  LEFT JOIN datalake_hub_services_clean.rev_info AS rev
    ON rev.id = mpa.rev_end AND rev.ts_created >= mpa.ts_created
  ...
  SELECT
    ...
    u.cpf
  FROM datalake_hub_services_clean.member_profile AS mpa
  ...
  SELECT
    mpa.id_revision,
    CASE
      WHEN mpa.ts_relationship_ended IS NULL AND mpa.is_active IS FALSE
      THEN mpa.ts_updated
      WHEN mpa.ts_relationship_ended IS NULL AND mpa.is_active IS TRUE
      THEN NOW()
      ELSE mpa.ts_relationship_ended
    END AS ts_relationship_ended
  FROM member_profile_aud AS mpa
)
SELECT
  id_revision,
...
FROM (
  SELECT
    ...
    NOW() AS ts_load,
    ROW_NUMBER() OVER (PARTITION BY mpa.id_revision ORDER BY mre.ts_relationship_ended DESC) AS _w
  FROM member_profile_aud AS mpa
  JOIN relationship_ended AS mre
    ON mre.id_revision = mpa.id_revision
  LEFT JOIN member_profile_last_user AS mpu
    ON mpu.id = mpa.id
  LEFT JOIN member_profile_last_user AS mpu_parent
    ON mpu_parent.id = mpa.id_parent_member_profile
  LEFT JOIN datalake_ebdb_clean.agent_data AS ad
    ON ad.id = mpu.id_agent
  LEFT JOIN datalake_hub_services.business_unit AS bu
