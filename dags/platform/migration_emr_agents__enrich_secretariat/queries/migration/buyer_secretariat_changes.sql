WITH users_5a AS (
  ...
  WHERE
    ...
    AND NOT TRY_CAST(SUBSTRING(main_phone, 6, 9) AS INT) IN (111111111, 222222222, 333333333, 444444444, 555555555, 666666666, 777777777, 888888888, 999999999)
    AND NOT TRY_CAST(SUBSTRING(main_phone, 6, 8) AS INT) IN (11111111, 22222222, 33333333, 44444444, 55555555, 66666666, 77777777, 88888888, 99999999)
), visitor AS (
  SELECT
    id,
    id_external,
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY COALESCE(
        COALESCE(v.id_external, u_email.id_user, u_phone.id_user),
        u_email.id_user,
        u_phone.id_user
      ), v.id ORDER BY version DESC) AS _w,
      u_email.id_user,
      version
    FROM datalake_hub_services_clean.visitor AS v
    LEFT JOIN users_5a AS u_email
      ON COALESCE(v.id_external, 0) = 0
      AND UPPER(TRIM(u_email.email)) = UPPER(TRIM(v.email))
    LEFT JOIN users_5a AS u_phone
      ON COALESCE(v.id_external, 0) = 0
  ...
  SELECT
    ...
    visitor_name,
    visitor_email
  FROM datalake_hub_services_clean.lead AS le
  LEFT JOIN visitor AS vi
  ...
  SELECT
    ...
    re.ts_assigned,
    re.ts_unassigned
  FROM datalake_hub_services_clean.responsible AS re
  LEFT JOIN leads AS le
    ON le.id_lead = re.id_lead
  LEFT JOIN datalake_hub_services_clean.users AS resp_u
    ON re.id_responsible = resp_u.id
  LEFT JOIN datalake_hub_services_clean.users AS dist_u
