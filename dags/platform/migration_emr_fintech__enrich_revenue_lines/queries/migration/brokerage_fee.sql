WITH contract_partnership_data AS (
  ...
  FROM (
    SELECT
      ...
      partner_type,
      brokerage_split_percentage,
      ROW_NUMBER() OVER (PARTITION BY id_contract ORDER BY id DESC) AS _w,
      id
    FROM datalake_ebdb_clean.contract_partnership_data
  ) AS _t
  WHERE
    _w = 1
), rental_brokerage_fee AS (
  SELECT
    ...
    ROUND(
      c.rent * c.first_rental_commission * (
        1.00 - COALESCE(c.agent_brokerage_share, 0.00) - COALESCE(p.brokerage_split_percentage, 0.00) - COALESCE(pp.brokerage_split_percentage, 0.00)
      ),
      2
    ) AS prod_theorical_amount,
    ...
    CAST(di.ts_due AS DATE) AS dt_due,
    CAST(di.ts_paid AS DATE) AS dt_paid,
    c.dt_start AS dt_contract_start
  FROM datalake_invoice.invoice_entries AS fie
  ...
  LEFT JOIN contract_partnership_data AS p
    ON c.sk_contract = p.id_contract AND p.partner_type = 'AUTONOMOUS_AGENT'
  LEFT JOIN contract_partnership_data AS pp
    ON c.sk_contract = pp.id_contract AND pp.partner_type = 'EXECUTIVE_FOR_RENT'
  WHERE
    NOT die.from_account_type IN ('quinto andar', 'contract expenses')
  ...
  SELECT
    ...
    pp.brokerage_split_percentage AS select_agent_brokerage_split_percentage,
    ROUND(
      c.rent * c.first_rental_commission * COALESCE(p.brokerage_split_percentage, 0.00),
      2
    ) AS prod_theorical_amount,
    ROUND(SUM(fie.brl_entry_due_amount), 2) AS invoice_theorical_amount,
    ROUND(SUM(fie.brl_entry_paid_amount), 2) AS invoice_paid_amount,
  ...
  WHERE
    ...
    AND NOT die.description IN ('Taxa de corretagem - Consultor Imobiliário')
  GROUP BY
    1,
    2,
    3,
  ...
  SELECT
    ...
    pp.brokerage_split_percentage AS select_agent_brokerage_split_percentage,
    ROUND(
      c.rent * c.first_rental_commission * COALESCE(pp.brokerage_split_percentage, 0.00),
      2
    ) AS prod_theorical_amount,
    ROUND(SUM(fie.brl_entry_due_amount), 2) AS invoice_theorical_amount,
    ROUND(SUM(fie.brl_entry_paid_amount), 2) AS invoice_paid_amount,
  ...
  FROM (
    SELECT
      die.id AS id_invoice_entry,
      ROW_NUMBER() OVER (PARTITION BY fie.id_invoice, die.description ORDER BY fie.ts_created) AS _w,
      fie.id_invoice,
      die.description,
      fie.ts_created
    FROM datalake_invoice.invoice_entries AS fie
  ...
  SELECT
    die.id AS id_invoice_entry
  FROM datalake_retsuko.invoice_entry AS die
  WHERE
  ...
  FROM (
    ...
    INNER JOIN datalake_retsuko.invoice_entry AS die
      ON fie.id = die.id
    WHERE
      die.description LIKE '%Crédito - %' AND fie.id_invoice > 0
  ) AS _t
  WHERE
    _w = 1
), brokerage_fee AS (
  ...
  WHERE
    NOT id_invoice_entry IN (
      SELECT
        id_invoice_entry
      FROM rental_brokerage_fee_discount
    )
    AND NOT id_invoice_entry IN (
      SELECT
        id_invoice_entry
      FROM rental_brokerage_fee_credit
    )
    AND NOT id_invoice_entry IN (
      SELECT
        id_invoice_entry
      FROM rental_brokerage_fee_installment
    )
  UNION
  SELECT
    *
  ...
  WHERE
    NOT id_invoice_entry IN (
      SELECT
        id_invoice_entry
      FROM credit_fix_partner
    )
  UNION
  SELECT
    *
  FROM rental_ciq_select_commission
)
SELECT
  id_contract_ebdb,
  id_invoice_entry,
