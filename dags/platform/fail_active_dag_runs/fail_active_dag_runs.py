...
from airflow.utils.state import DagRunState, TaskInstanceState  # <-- Import TaskInstanceState
...
def find_and_fail_active_runs_and_tis(session=None):
    ...
    logger.warning(f"Found {len(active_runs)} active DAG runs. Marking them and their tasks as FAILED...")
    ...
    for run in active_runs:
        ...
        try:
            ...
            active_tis = session.query(TaskInstance).filter(
                TaskInstance.dag_id == run.dag_id,
                TaskInstance.run_id == run.run_id,
                TaskInstance.state.in_([TaskInstanceState.RUNNING, TaskInstanceState.QUEUED, TaskInstanceState.SCHEDULED, TaskInstanceState.UP_FOR_RETRY, TaskInstanceState.UP_FOR_RESCHEDULE])
            ).all()

            if active_tis:
                logger.warning(f"Found {len(active_tis)} unfinished tasks. Marking them as FAILED...")
                for ti in active_tis:
                    try:
                        logger.info(f"  - Marking TI as FAILED: {ti.task_id} | State: {ti.state}")
                        # Set task instance state to FAILED
                        ti.set_state(TaskInstanceState.FAILED, session=session)
                    except Exception as ti_e:
                        logger.error(f"    Error marking TI {ti.task_id} as FAILED: {ti_e}")
            else:
