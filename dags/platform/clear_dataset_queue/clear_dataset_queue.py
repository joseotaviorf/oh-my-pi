...
def clear_dataset_data(dag: DAG, execution_date: datetime, session=None):
    ...
    deleted_queue_entries = (
        session.query(DatasetDagRunQueue)
        .filter(DatasetDagRunQueue.target_dag_id == dag.dag_id)
        .delete()
    )
    logging.info(
        f"Deleted {deleted_queue_entries} dataset dag run queue entries for DAG ID: {dag.dag_id}"
    )
    ...
    logging.info(
        f"Created a successful dataset-triggered DAG run for DAG ID: {dag.dag_id}"
    )

    dag_run.note = (
        "This DAG run was created automatically by the DAG airflow.clear_dataset_queue as a way "
...
with DAG(
    ...
    def run_clear_dataset_events_from_queue(**context):
        ...
        for dag_id in dag_ids:
            ...
            clear_dataset_data(
                dag_bag.dags[dag_id], execution_date=context["execution_date"]
            )
