WITH booking_status AS (
  ...
  FROM (
    SELECT
      id_schedule,
      id_author_user,
      ROW_NUMBER() OVER (PARTITION BY id_schedule ORDER BY ts_created) AS _w,
      ts_created
  ...
  LEFT JOIN status_log AS sl
    ON b.id = sl.id_schedule
  LEFT JOIN first_booking_author_sc AS fbasc
  ...
  FROM (
    SELECT
      ...
      MIN(b_aud.REV) OVER (PARTITION BY b_aud.id) AS _w,
      b_aud.REV,
      b_aud.id
    FROM datalake_ebdb_clean.booking_aud AS b_aud
    WHERE
  ...
  SELECT
    ...
    CAST(FROM_UNIXTIME(ure.ts_revision / 1000) AS TIMESTAMP) /* TODO [ODS] check if milliseconds is really needed for this column */ + (
      ure.ts_revision % 1000
    ) * INTERVAL '1' MILLISECONDS AS ts_first_canceled
  FROM min_canceled_date AS mcd
  ...
  FROM (
    SELECT
      *,
      ROW_NUMBER() OVER (PARTITION BY id_appointment ORDER BY ts_updated DESC) AS _w
    FROM datalake_schedules_clean.appointment_history
  ) AS _t
  WHERE
    _w = 1
), creation_users AS (
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY id_appointment ORDER BY ts_updated ASC) AS _w,
      ts_updated
    FROM datalake_schedules_clean.appointment_history
    WHERE
  ...
  SELECT
    ...
    IF(
      a.status = 'CANCELLED',
      FIRST(a.ts_updated) OVER (PARTITION BY a.id_inspection ORDER BY a.ts_updated),
      NULL
    ) AS ts_first_appointment_cancelled_utc,
    IF(
      a.status = 'CANCELLED',
      FIRST(a.ts_updated - INTERVAL '3' HOURS) OVER (PARTITION BY a.id_inspection ORDER BY a.ts_updated),
      NULL
    ) AS ts_first_appointment_cancelled_local_tz,
    a.year,
    a.month,
    a.day
  ...
  LEFT JOIN datalake_schedules_clean.appointment AS s
    ON COALESCE(a.id_schedules, b.id_schedule) = s.id
  LEFT JOIN appointment_history AS ah
    ON b.id_schedule = ah.id_appointment
  LEFT JOIN creation_users
    ON a.id_appointment = creation_users.id_appointment
  LEFT JOIN canceled_users
    ON a.id_appointment = canceled_users.id_appointment
  WHERE
    CAST(a.ts_updated AS DATE) BETWEEN '{load_start_date}' AND '{load_end_date}'
  ...
  SELECT
    ...
    CASE
      WHEN LOWER(b.type) IN ('vistoria', 'vistoriaquarteirizada')
      THEN 'INSPECTION'
    END AS type,
    CASE
      WHEN LOWER(b.status) = 'agendado'
      THEN 'DONE'
      WHEN LOWER(b.status) = 'cancelado'
      THEN 'CANCELLED'
      WHEN LOWER(b.status) = 'marcado'
      THEN 'SCHEDULED'
      WHEN LOWER(b.status) = 'aguardandoconfirmacao'
    ...
    b.is_confirmed,
    CAST(b.dt_booking AS TIMESTAMP) + FLOOR((
      b.slot_day * 15 / 60
    ) + 8) * INTERVAL '1' HOURS + ABS(b.slot_day * 15 % 60) * INTERVAL '1' MINUTES AS ts_appointment_inspected_local_tz,
    ...
    YEAR(TO_DATE(b.ts_updated)) AS year,
    MONTH(TO_DATE(b.ts_updated)) AS month,
    DAY(TO_DATE(b.ts_updated)) AS day
  FROM datalake_ebdb_clean.booking AS b
  LEFT JOIN datalake_inspection_services_clean.appointment AS a
    ON b.id = a.id_external_appointment
  LEFT JOIN datalake_ebdb_clean.inspection AS i
    ON b.id = i.id_booking
  LEFT JOIN datalake_inspection_services_clean.inspection AS is
    ON i.id = is.id_external
  LEFT JOIN booking_status AS bs
    ON b.id = bs.id_booking AND b.status = bs.status
  LEFT JOIN canceled_date AS cd
    ON b.id = cd.id_booking
  LEFT JOIN first_booking_author AS fba
    ON b.id = fba.id_booking
  WHERE
    b.type IN ('Vistoria', 'VistoriaQuarteirizada')
    AND CAST(b.ts_updated AS DATE) BETWEEN '{load_start_date}' AND '{load_end_date}'
  ...
  SELECT
    ...
    COALESCE(
      iad.ts_first_appointment_cancelled_local_tz,
      md.ts_first_appointment_cancelled_local_tz
    ) AS ts_first_appointment_cancelled_local_tz,
    COALESCE(iad.year, md.year) AS year,
    COALESCE(iad.month, md.month) AS month,
    COALESCE(iad.day, md.day) AS day
  FROM inspection_appointment_data AS iad
  FULL OUTER JOIN main_appointment_data AS md
  ...
  FROM (
    SELECT
      ...
      ts_updated,
      ROW_NUMBER() OVER (PARTITION BY id_inspection ORDER BY ts_updated DESC) AS _w
    FROM datalake_inspection_services_clean.inspection
  ) AS _t
  WHERE
    _w = 1
)
...
FROM (
  SELECT
    MD5(
      COALESCE(CONCAT(cas.id_is_appointment, 'IS'), CONCAT(cas.id_main_appointment, 'PWA'))
    ) AS id_appointment,
    ...
    cas.is_confirmed,
    CASE
      WHEN FIRST(i.id_inspection) OVER (PARTITION BY i.id_contract, i.inspection_type ORDER BY i.ts_updated) = cas.id_inspection
      THEN TRUE
      ELSE FALSE
    END AS is_first_schedule,
    CASE WHEN cas.id_user_who_created = 194233 THEN TRUE ELSE FALSE END AS is_first_schedule_auto,
    CASE
      WHEN CAST(cas.ts_first_appointment_cancelled_utc AS DATE) = CAST(TO_UTC_TIMESTAMP(CAST(cas.ts_appointment_inspected_local_tz AS TIMESTAMP), 'UTC') AS DATE)
      THEN TRUE
      ELSE FALSE
    END AS is_d0_canceled,
    CASE
      WHEN CAST(cas.ts_first_appointment_cancelled_utc AS DATE) = DATE_ADD(
        CAST(TO_UTC_TIMESTAMP(CAST(cas.ts_appointment_inspected_local_tz AS TIMESTAMP), 'UTC') AS DATE),
        1 * -1
      )
      THEN TRUE
      ELSE FALSE
    ...
    CASE
      WHEN NOT cas.cancellation_reason IN ('INSPECTOR_BLOCKED_SCHEDULE', 'CANCELED_PROBLEM_INSPECTOR', 'CANCELED_INSPECTOR_NOT_ATTEND', 'CANCELED_INSPECTOR_CAN_NOT_ATTEND_INSPECTION')
      THEN TRUE
      WHEN cas.cancellation_reason IS NULL
      THEN NULL
      ELSE FALSE
    END AS is_not_canceled_by_inspector,
    TO_UTC_TIMESTAMP(CAST(cas.ts_appointment_inspected_local_tz AS TIMESTAMP), 'UTC') AS ts_appointment_inspected_utc,
    ...
    cas.month,
    cas.day,
    ROW_NUMBER() OVER (PARTITION BY MD5(
      COALESCE(CONCAT(cas.id_is_appointment, 'IS'), CONCAT(cas.id_main_appointment, 'PWA'))
    ) ORDER BY cas.ts_appointment_updated_utc DESC) AS _w
  FROM coalesce_appointment_sources AS cas
  LEFT JOIN inspection_data AS i
