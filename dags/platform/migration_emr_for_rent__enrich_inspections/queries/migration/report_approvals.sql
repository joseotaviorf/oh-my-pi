WITH owner_approvals AS (
  ...
  FROM (
    SELECT
      *,
      ROW_NUMBER() OVER (PARTITION BY id_assessment ORDER BY ts_updated DESC) AS _w
  ...
  FROM (
    ...
    FROM datalake_inspection_services_clean.assessment
  ) AS _t
  WHERE
    _w = 1
  ...
  FROM (
    SELECT
      ...
      NULL AS id_budget,
      CASE WHEN ra_owner.is_approved = TRUE THEN ra_owner.id_reviewer END AS owner_approved_by,
      CASE WHEN ra_tenant.is_approved = TRUE THEN ra_tenant.id_reviewer END AS tenant_approved_by,
      ...
      ROW_NUMBER() OVER (PARTITION BY a.id_inspection ORDER BY COALESCE(
        COALESCE(ra_owner.ts_updated, ra_tenant.ts_updated),
        COALESCE(ra_owner.ts_updated, ra_tenant.ts_updated)
      ) DESC) AS _w
    FROM owner_approvals AS ra_owner
    FULL OUTER JOIN tenant_approvals AS ra_tenant
      ON ra_owner.id_assessment = ra_tenant.id_assessment
    FULL OUTER JOIN assessment AS a
      ON a.id_assessment = ra_owner.id_assessment
      OR a.id_assessment = ra_tenant.id_assessment
    WHERE
      CAST(COALESCE(ra_owner.ts_updated, ra_tenant.ts_updated) AS DATE) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
  ) AS _t
  WHERE
    _w = 1
  UNION ALL
  ...
  FROM (
    SELECT
      ...
      b.month,
      b.day,
      ROW_NUMBER() OVER (PARTITION BY b.id_inspection ORDER BY b.ts_updated DESC) AS _w
    FROM datalake_inspection_services_clean.budget AS b
    LEFT JOIN assessment AS a
      ON a.id_inspection = b.id_inspection
    WHERE
      MAKE_DATE(b.year, b.month, b.day) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
  ) AS _t
  WHERE
    _w = 1
)
...
FROM (
  SELECT
    ...
    ura.month,
    ura.day,
    ROW_NUMBER() OVER (PARTITION BY ura.id_inspection ORDER BY ura.ts_updated DESC) AS _w
  FROM union_reviewer_approvals AS ura
