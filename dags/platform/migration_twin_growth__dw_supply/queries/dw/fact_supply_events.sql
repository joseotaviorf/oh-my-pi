WITH stg_fact_supply AS (
  SELECT
    ...
    ses.detailed_route AS nm_detailed_route,
    COALESCE(ses.id_task, '-1') AS sk_task,
    COALESCE(ses.id_chat_session, '-1') AS sk_chat_session,
    ses.quinto_andar_phone_number,
    supply_source,
...
SELECT 
  ...
  sfs.nm_detailed_route,
  sfs.sk_task,
  sfs.sk_chat_session,
  sfs.quinto_andar_phone_number,
  sfs.utm_medium,
  sfs.utm_source,
  sfs.utm_campaign,
  sfs.utm_term,
