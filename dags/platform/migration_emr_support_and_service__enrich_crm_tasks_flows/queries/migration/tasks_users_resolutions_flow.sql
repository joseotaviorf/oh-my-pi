WITH tasks AS (
  ...
  SELECT DISTINCT
    ...
    COALESCE(
      DATE_FORMAT(
        LAG(tr.ts_action) OVER (PARTITION BY tr.id_task ORDER BY DATE_TRUNC('SECOND', tr.ts_action)),
        'yyyyMMdd'
      ),
      -1
    ) AS id_task_user_start_date,
    ...
    ROUND(
      (
        UNIX_TIMESTAMP(tr.ts_action, 'yyyy-MM-dd hh:mm:ss') - UNIX_TIMESTAMP(
          LAG(tr.ts_action) OVER (PARTITION BY tr.id_task ORDER BY DATE_TRUNC('SECOND', tr.ts_action)),
          'yyyy-MM-dd hh:mm:ss'
        )
      ) / 3600.0,
    ...
    tr.type,
    CAST(tr.ts_action AS DATE) AS dt_partition,
    tr.ts_action,
    tr.ts_start,
    ...
    tr.month,
    tr.day
  FROM datalake_crm_tasks_resolution.tasks_resolution AS tr
  ...
  WHERE
    (
      action_type = 'REALIZE' AND id_user_action <> -1
  ...
  SELECT
    ...
    t.month,
    t.day
  FROM prev_tasks AS t
  INNER JOIN prev_max_realized_by_user AS prev_max
...
SELECT DISTINCT
  ...
  CASE
    WHEN NOT t.action_user_name IS NULL
    THEN CAST(COALESCE(mrbu.task_user_resolve_hours, t.task_user_resolve_hours) AS DECIMAL)
...
FROM prev_tasks AS t
LEFT JOIN max_realized_by_user AS mrbu
  ON t.id_task = mrbu.id_task
  AND t.id_user_action = mrbu.id_user_action
  AND t.action_type = mrbu.action_type
LEFT JOIN datalake_heimdall.activity AS act
  ON t.id_origin = act.id
WHERE
  NOT (
    t.id_user_action = -1 AND t.action_type = 'REALIZE'
  )
