WITH snap_recent AS (
  ...
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY sk_propose ORDER BY month_reference DESC) AS rn
  FROM snap AS tl
  WHERE
  ...
  LEFT JOIN modalidade_cobranca AS mod
    ON tl.sk_propose = mod.sk_propose AND mod.rn = 1
  LEFT JOIN mensalidades_renewal_dedup AS m
