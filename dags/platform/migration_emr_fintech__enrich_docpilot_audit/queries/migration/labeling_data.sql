WITH resend_events AS (
  ...
  WHERE
    p.tenant_documentation_status = 'ReenvioDocumentos'
    AND p.mod_tenant_documentation_status = TRUE
    AND hl.country_code = 'BR'
  GROUP BY
    1
), first_docs_sent AS (
  SELECT
  ...
  FROM dw_rent.fact_listing_rent_flows AS f
  LEFT JOIN dw_rent.dim_house_listing AS hl
    ON f.sk_house_listing = hl.sk_house_listing
  WHERE
    hl.country_code = 'BR'
  ...
  WHERE
    ds.dt_doc_sent >= DATE_TRUNC('MM', ADD_MONTHS(CURRENT_DATE, -3))
  ...
  SELECT
    ...
    reason,
    ts_updated,
    ROW_NUMBER() OVER (PARTITION BY id_proposal ORDER BY ts_updated DESC) AS rn
  ...
  FROM datalake_sorting_hat_clean.analysis_request AS ar
  INNER JOIN datalake_sorting_hat_clean.checklist AS c
    ON c.id_analysis_request = ar.id
  INNER JOIN datalake_sorting_hat_clean.checklist_group AS cg
    ON cg.id_checklist = c.id
  INNER JOIN datalake_sorting_hat_clean.checklist_item AS ci
    ON ci.id_checklist_group = cg.id
  WHERE
    is_current_checklist AND input_type = 'MANUAL'
  GROUP BY
    1,
    2
  ...
  WHERE
    ...
    AND NOT result IN ('NEW', 'ADVISABLE', 'INADVISABLE')
    AND NOT analyst IS NULL
  ORDER BY
    ts_updated ASC
  ...
  SELECT
    ...
    c.population_proportion * (
      1 - C.population_proportion
    ) AS population_proportion_factor,
    c.resend_multiplication_factor,
    c.days_in_month,
    c.days_in_week
  FROM population_data AS p, constants_calculator AS c
), intermediate_calculations AS (
  SELECT
    days_in_month,
    days_in_week,
    z_squared * population_proportion_factor / error_squared AS infinite_pop_sample_size,
    resend_multiplication_factor * (
      1 - resend_share
    ) AS resent_docs_adj_factor,
    (
      z_squared * population_proportion_factor
    ) / (
      error_squared * first_doc_sent_avg
    ) AS finite_pop_factor
  ...
  SELECT DISTINCT
    ...
    GREATEST(
      ROUND(
        ga.group_size_before_sampling / (
          SELECT
            total_population_size
          FROM grouped_analysis
          LIMIT 1
        ) * wss.weekly_sample_size
      ),
      1
    ) AS num_samples_for_group
  FROM grouped_analysis AS ga, weekly_sample_size AS wss
  ...
  SELECT
    ...
    psr.reason,
    psr.analyst
  FROM proportional_samples_raw AS psr
  ...
  FROM (
    SELECT
      ...
      reason,
      analyst,
      ROW_NUMBER() OVER (PARTITION BY analyst ORDER BY RAND()) AS _w
    FROM latest_week_analyses
    WHERE
      result = 'REJECTED'
  ) AS _t
  ...
  SELECT
    ...
    ora.reason,
    ora.analyst
  FROM one_rejection_per_analyst AS ora
  ...
  SELECT DISTINCT
    *
  FROM proportional_samples
  UNION ALL
  SELECT DISTINCT
    *
  FROM missing_rejections
  ...
  SELECT
    ...
    id_context_external
  FROM datalake_docx_clean.document AS d
  INNER JOIN datalake_docx_clean.folder_reference AS fr
    ON d.id_folder = fr.id_source_folder
  WHERE
    NOT id_folder IS NULL AND id_folder_reference_type = 7
  ...
  FROM (
    SELECT
      ...
      statements_json_str,
      payslips_json_str,
      ROW_NUMBER() OVER (PARTITION BY id_proposal, id_proponent ORDER BY ts_updated DESC) AS _w,
      ts_updated
    FROM income_document_processed
  ) AS _t
  WHERE
    _w = 1
  ...
  SELECT
    ...
    cas.reason,
    cas.analyst
  FROM credit_audit_sample AS cas
  LEFT JOIN income_document_deduped AS ide
    ON ide.id_proposal = cas.id_proposal
), data_joined AS (
  SELECT
    idj.id_proposal,
    idj.id_proponent,
    ...
    idj.reason,
    idj.analyst
  FROM income_document_joined AS idj
  INNER JOIN id_document_deduped AS idd
    ON idj.id_folder = idd.id_folder
  INNER JOIN filtered_documents AS fd
    ON idj.id = fd.document_id
), data_deduped AS (
  SELECT
    id_proposal,
    id_proponent,
  ...
  FROM (
    SELECT
      ...
      reason,
      analyst,
      ROW_NUMBER() OVER (PARTITION BY id_proposal, id_proponent ORDER BY ts_updated_for_dedup DESC) AS _w,
      ts_updated_for_dedup
    FROM data_joined
  ) AS _t
  WHERE
    _w = 1
  ...
  SELECT
    ...
    name,
    CASE
      WHEN SIZE(FROM_JSON(statements_json_str, 'ARRAY<MAP<STRING, STRING>>')) = 0
      THEN NULL
      ELSE TRANSFORM(FROM_JSON(statements_json_str, 'ARRAY<MAP<STRING, STRING>>'), x -> x['fileName'])
    END AS statements_array,
    CASE
      WHEN SIZE(FROM_JSON(payslips_json_str, 'ARRAY<MAP<STRING, STRING>>')) = 0
      THEN NULL
      ELSE TRANSFORM(FROM_JSON(payslips_json_str, 'ARRAY<MAP<STRING, STRING>>'), x -> x['fileName'])
  ...
  WHERE
    NOT COALESCE(statements_array, payslips_array) IS NULL
)
SELECT
  id_proposal,
  name,
