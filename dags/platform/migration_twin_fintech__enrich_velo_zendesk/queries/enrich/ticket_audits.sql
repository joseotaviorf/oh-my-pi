WITH base_ticket_audits AS (
    SELECT
        ...
        id_author,
        event_via,
        EXPLODE(FROM_JSON(events, 'array<struct<id: string, type: string, field_name: string, previous_value: string, value: string, public: string>>')) AS parsed_events,
        sequence_number,
        table_version,
        dt_extracted,
        ts_batched,
...
SELECT
  ...
  ta.parsed_events.public AS is_public,
  ta.dt_extracted,
  ta.ts_batched,
  ta.ts_received,
  ta.ts_created,
  FROM_UTC_TIMESTAMP(ta.ts_created, 'Brazil/East') AS ts_created_local,
  ta.ts_load,
  ta.year,
  ta.month,
  ta.day
