...
dim_date AS (
  SELECT 
    DISTINCT month_start, month_end, year
  FROM 
    dw_public.dim_date
),
snap AS (
  ...
  LEFT JOIN 
    dim_date dt
      ON dt.month_start = add_months(DATE_TRUNC('month', s.ts_snapshot), -1)
  JOIN 
    snap_recent r
...
propose_timeline_renewal AS (
  SELECT 
    *,
    CASE 
      WHEN 
        TRY_CAST(CONCAT(CAST(YEAR(CURRENT_DATE) AS VARCHAR(10)), '-', CAST(MONTH(dt_contract_started) AS VARCHAR(10)), '-', CAST(DAY(dt_contract_started) AS VARCHAR(10))) AS DATE) IS NULL 
        THEN 
          DATE_TRUNC('MONTH', DATE(CONCAT(CAST(YEAR(CURRENT_DATE) AS VARCHAR(10)), '-', CAST(MONTH(dt_contract_started) AS VARCHAR(10)), '-', CAST(DAY(dt_contract_started) - 1 AS VARCHAR(10))))) 
      ELSE 
        DATE_TRUNC('MONTH', DATE(CONCAT(CAST(YEAR(CURRENT_DATE) AS VARCHAR(10)), '-', CAST(MONTH(dt_contract_started) AS VARCHAR(10)), '-', CAST(DAY(dt_contract_started) AS VARCHAR(10)))))
    END AS month_renewal_backup,
    CASE 
      WHEN 
        is_corrected_robot = TRUE AND monthly_timeline_renewal_corrected = 0 
...
mensalidades_renewal AS (
  SELECT DISTINCT 
    tl.sk_propose,
    IF(
      (tl.month_propose_life < COALESCE(r.month_renewal, tl.month_renewal_backup) OR r.sk_propose IS NULL), 
  ...
  WHERE month_propose_life >= DATE(CONCAT(CAST(YEAR(CURRENT_DATE) - 1 AS VARCHAR(10)), '-12-01'))
),
mensalidades_renewal_dedup AS (
  SELECT
    sk_propose,
...
mensalidades_propose_aud AS (
  SELECT
    ...
    LEAD(monthly_timeline_propose_aud) OVER (
      PARTITION BY sk_propose 
      ORDER BY month_reference
    ) AS monthly_guarantee_after_aud
  FROM snap
  WHERE month_chargeble = TRUE
),
...
modalidade_cobranca AS (
  ...
  FROM snap tl
  WHERE month_propose_life IS NOT NULL 
...
base_final AS (
  SELECT 
    ...
    CASE 
      ...
      WHEN ABS(mod.monthly_timeline_renewal - mod.monthly_value_propose) < 0.2 
        AND ABS(mod.monthly_timeline_renewal - mod.value_payment) < 0.2 
        AND mod.delinquency_monthly_value IS NOT NULL 
        THEN 'Match parcial - D<>PAY=R=PR'
      WHEN ABS(mod.monthly_timeline_renewal - mod.monthly_value_propose) < 0.2 
        AND ABS(mod.monthly_timeline_renewal - mod.delinquency_monthly_value) < 0.2 
        AND mod.value_payment IS NOT NULL 
    ...
    COUNT(
      IF(tl.gateway_payment = 'BILLING DIRETO', tl.monthly_timeline_renewal, NULL)
    ) AS months_billing_direto,
    COUNT(
      IF(tl.gateway_payment NOT IN ('BILLING DIRETO', 'sem cobrança'), tl.monthly_timeline_renewal, NULL)
    ) AS months_quintocred_cobra,
    COALESCE(r.month_renewal, tl.month_renewal_backup) AS month_renewal,
    COUNT(
      IF(tl.month_chargeble = TRUE, tl.month_propose_life, NULL)
    ) AS total_months_of_service,
  ...
  WHERE 
    ...
    AND tl.month_propose_life = ADD_MONTHS(DATE_TRUNC('month', tl.dt_snapshot), -1)
  GROUP BY 
