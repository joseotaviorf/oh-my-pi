WITH propose_canceled_date AS (
  ...
  FROM datalake_rental_guarantee_platform_clean.fiancavelo_proposecompany_legacy
), cte_values AS (
  SELECT
    id_propose,
    pi.value,
  ...
  FROM datalake_rental_guarantee_platform_clean.plan AS pl
  LEFT JOIN datalake_rental_guarantee_platform_clean.company_plan AS cp
    ON pl.id = cp.id_plan
  GROUP BY
    1,
    2
), old_prop_values /* cte to get propose_values from propose_legacy proposes */ AS (
  SELECT
    p.id AS id_propose,
  ...
  LEFT JOIN old_duplicated_company_plan AS odcp
    ON pl.id = odcp.id_plan AND p.id_quintocred_company = odcp.id_company
  LEFT JOIN datalake_rental_guarantee_platform_clean.company_plan AS cp
  ...
  FROM datalake_rental_guarantee_platform_clean.fiancavelo_object_legacy AS o
  LEFT JOIN datalake_rental_guarantee_platform_clean.fiancavelo_property_legacy AS py
    ON py.id = o.id_property AND py.is_active
  WHERE
    o.is_active
), old_main_client AS (
  SELECT
  ...
  FROM datalake_rental_guarantee_platform_clean.fiancavelo_proposeperson_legacy AS pp
  WHERE
    pp.is_active AND pp.id_type = 1 /* bringing only main IQ */
), main_person AS (
  SELECT
    id_person,
    id_propose,
    declared_income
  FROM datalake_velo.propose_person_legacy
  WHERE
  ...
  FROM datalake_velo.propose_person_legacy AS pp
  ...
  FROM datalake_velo.payment_legacy
  GROUP BY
    1
), persons_metrics AS (
  SELECT
...
SELECT DISTINCT
  ...
  (
    mc.id_primary_person + 100
  ) * -1 AS id_primary_person,
  ...
  IF(NOT pc.id_propose IS NULL, 'PJ/company', 'PF') AS origin,
  ...
  CAST(NULL AS DECIMAL(38, 18)) AS total_declared_income,
  ...
  NOT old.id_contract IS NULL AS is_contract,
  FALSE AS is_direct_billing,
  COALESCE(
    DATEDIFF(TO_DATE(old.dt_ended), TO_DATE(CAST(old.dt_contract_started AS DATE))) <= 10,
    FALSE
  ...
  IF(r.id_propose IS NULL, old.dt_ended, old.dt_analyst_annulment_input) AS dt_ended_official,
...
LEFT JOIN old_house AS h
  ON h.id_propose = p.id AND h.rn = 1 /* Gets only the latest updated id */
LEFT JOIN old_main_client AS mc
  ON mc.id_propose = p.id AND mc.rn = 1 /* Gets only the latest updated id */
LEFT JOIN old_payments_metrics AS pym
  ON pym.id_propose = p.id
LEFT JOIN propose_company AS pc
  ON pc.id_propose = p.id AND pc.rn = 1
LEFT JOIN datalake_velo.junk AS jk1
  ON jk1.id_lvl_1 = IF(NOT pc.id_propose IS NULL, 1, 2)
...
LEFT JOIN old_prop_values AS pv
...
WHERE
  ...
  AND NOT p.id IN (
    SELECT
      id_propose
    FROM datalake_gsheets_clean.quintocred_excluded_proposals
  )
