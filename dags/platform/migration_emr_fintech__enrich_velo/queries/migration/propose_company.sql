WITH address AS (
  ...
  FROM (
    ...
    FROM datalake_rental_guarantee_platform_clean.tenant_company AS c
...
SELECT DISTINCT
  ...
  tc.ts_created,
  tc.ts_updated
FROM datalake_rental_guarantee_platform_clean.tenant_company AS tc
LEFT JOIN company_info AS comp
  ON tc.uuid_company = comp.uuid_company
LEFT JOIN address AS a
  ON tc.uuid_company = a.uuid_company
LEFT JOIN datalake_company_clean.document AS d
  ON tc.uuid_company = d.uuid_company AND d.document_type = 'CNPJ'
