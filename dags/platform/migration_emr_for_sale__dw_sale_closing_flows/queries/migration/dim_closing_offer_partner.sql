...
SELECT DISTINCT
  ...
  ca.partner_type,
  CASE
    WHEN NOT ca.first_pre_analysis IS NULL
    AND ca.partner_type IN ('EXTERNAL', 'FRANCHISE')
    THEN 'PARTNER'
    ELSE 'INTERNAL'
  END AS partner_type_group,
  CASE
    WHEN NOT ca.first_credit_approved_date IS NULL
    THEN 'Approved'
  ...
  CASE
    WHEN NOT fo.ts_offer_dismissed IS NULL
    AND fo.ts_offer_rescued IS NULL
    OR NOT dsa.ts_sale_agreement_cancelled IS NULL
    THEN TRUE
    ELSE FALSE
  ...
  CASE
    WHEN NOT fo.ts_offer_dismissed IS NULL AND fo.ts_offer_rescued IS NULL
    THEN TRUE
    ELSE FALSE
  END AS is_offer_canceled,
  CASE WHEN NOT fo.ts_offer_accepted IS NULL THEN TRUE ELSE FALSE END AS is_offer_accepted,
  CASE WHEN ca.proposal_approved > 0 THEN TRUE ELSE FALSE END AS is_credit_approved,
  CASE WHEN NOT ca.last_credit_ended IS NULL THEN TRUE ELSE FALSE END AS is_credit_ended,
  CASE
    WHEN NOT COALESCE(ca.last_credit_ended, ca.credit_ended_isolve) IS NULL
    THEN TRUE
    ELSE FALSE
  END AS is_credit_ended_general,
  CASE WHEN fo.sk_sale_agreement_signed_date > 0 THEN TRUE ELSE FALSE END AS is_ccv_signed,
  CASE WHEN NOT ca.last_financing_ended IS NULL THEN TRUE ELSE FALSE END AS is_financing_ended,
  NOW() AS ts_load
