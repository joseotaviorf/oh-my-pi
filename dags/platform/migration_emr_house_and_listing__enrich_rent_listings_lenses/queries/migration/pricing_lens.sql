WITH historical_prices AS (
  SELECT
    ...
    (
      price / calculator_rent_price
    ) - 1 AS diff_calculator_price,
    ts_price_started
  FROM datalake_ebdb_pricing.rent_listing_price_changes
), actual_prices AS (
  SELECT
    h.id AS id_house,
    hl.id_house_listing,
  ...
  LEFT JOIN datalake_ebdb_clean.house_predicted_price AS p
    ON h.id = p.id_house AND p.business_context = 'RENT'
  WHERE
    NOT h.rent IS NULL
), prices_dataset AS (
  SELECT
  ...
  SELECT
    ...
    CASE
      WHEN d.calculator_price IS NULL OR d.calculator_price = 0
      THEN 'T- Undefined'
