WITH goal_category AS (
  SELECT
    g.id_goal,
    CASE
      WHEN g.goal_category IN ('ProjetoOuProcessoCriticoProjec', 'Projeto ou Processo Crítico')
        THEN 'project'
      WHEN g.goal_category IN ('Indicador/KPI', 'IndicadorKpi')
        THEN 'kpi'
...
goal_base AS (
  SELECT
    ...
    CASE
      WHEN gc.category <> 'kpi' THEN NULL
      WHEN g.attribute2_text = 'Quanto maior, melhor/The bigger the better/Cuanto más grande, mejor'
        THEN 'maximize'
      WHEN g.attribute2_text = 'Quanto menor, melhor/The smaller the better/Cuanto más pequeño mejor'
        THEN 'minimize'
  ...
  INNER JOIN
    datalake_pin_core_clean.all_people AS ap
      ON ap.id_person = g.id_person
        AND ap.dt_effective_ended >= DATE('9999-12-31')
  WHERE
...
goal_values AS (
  SELECT
    ...
    CASE
      ...
      WHEN b.goal_orientation = 'minimize'
        THEN GREATEST(CAST(b.maximum_value AS DOUBLE), CAST(b.minimum_value AS DOUBLE))
      WHEN b.goal_orientation = 'maximize'
        THEN LEAST(CAST(b.maximum_value AS DOUBLE), CAST(b.minimum_value AS DOUBLE))
      ELSE NULL
    END AS minimum_value,
    b.target_value,
    CASE
...
kpi_full_achievements AS (
  SELECT
    ...
    CASE
      WHEN b.result_value IS NULL THEN NULL
      WHEN CAST(b.result_value AS DOUBLE) <= v.target_value
        THEN (TRY_DIVIDE(
          (CAST(b.result_value AS DOUBLE) - v.minimum_value),
          (v.target_value - v.minimum_value)
        ) * (1 - 0.7)) + 0.7
      WHEN CAST(b.result_value AS DOUBLE) > v.target_value
        THEN (TRY_DIVIDE(
          (CAST(b.result_value AS DOUBLE) - v.target_value),
          (v.maximum_value - v.target_value)
        ) * (1.2 - 1)) + 1
      ELSE NULL
    END AS achievement_percentage_raw
  FROM
...
kpi_partial_achievements AS (
  SELECT
    ...
    CASE
      ...
      WHEN b.goal_orientation = 'minimize'
        THEN TRY_DIVIDE(
          v.target_value - CAST(b.result_value AS DOUBLE),
          ABS(v.target_value)
        ) + 1
      ELSE NULL
    END AS achievement_percentage_raw
...
SELECT
  ...
  ga.achievement_percentage * b.goal_weight / 100.0 AS achievement_percentage_weighted,
  CASE
    WHEN b.result_value IS NOT NULL
    THEN b.goal_weight / NULLIF(
      SUM(
        CASE WHEN b.result_value IS NOT NULL THEN b.goal_weight END
      ) OVER (
        PARTITION BY b.person_number, b.review_period_name, b.goal_plan_name
      ),
      0
    ) * 100.0
  END AS effective_goal_weight,
  SUM(
    CASE WHEN b.result_value IS NOT NULL
      THEN ga.achievement_percentage * b.goal_weight
    END
  ) OVER (
    PARTITION BY b.person_number, b.review_period_name, b.goal_plan_name
  ) / NULLIF(
  ...
  b.result_value IS NOT NULL AS is_calculation_registered,
  b.dt_started,
