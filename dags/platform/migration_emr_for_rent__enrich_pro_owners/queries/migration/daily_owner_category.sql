WITH pp_multi_cluster AS (
  ...
  FROM (
    SELECT
      ...
      cluster,
      ts_load,
      ROW_NUMBER() OVER (PARTITION BY id_owner ORDER BY ts_load DESC) AS _w
    FROM datalake_gsheets_clean.pp_multi_cluster
  ) AS _t
  WHERE
    _w = 1
  ...
  SELECT
    ...
    CASE
      WHEN ohqh.id_owner = pmc.id_owner
      THEN pmc.cluster
      WHEN ohqh.ongoing_houses > 15
      THEN 'Investors (15+)'
      WHEN ohqh.ongoing_houses >= 10
      THEN 'Investors (10-15)'
      WHEN ohqh.ongoing_houses >= 5
      THEN 'Long-tail (5-10)'
      ELSE 'Amateur'
    ...
    ohqh.month,
    ohqh.day
  FROM datalake_pro_owners.daily_owner_houses_quantity_history AS ohqh
  ...
  FROM owner_house_category_changes
  WHERE
...
LEFT JOIN current_category AS cc
