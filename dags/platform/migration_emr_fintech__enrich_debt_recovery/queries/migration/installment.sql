WITH deduplicate_payment AS (
  ...
  SELECT
    ...
    p.our_number,
    DATEDIFF(TO_DATE(p.ts_updated), TO_DATE(i.dt_due)) AS ts_paid_diff
...
SELECT
  ...
  COALESCE(
    external_index + 1,
    DENSE_RANK() OVER (PARTITION BY cp.id_negotiation ORDER BY cp.ts_created, cp.id_external)
  ...
  CASE
    ...
    OR (
      LOWER(c.name) LIKE '%cyber%' AND BIGINT(n.id_collector_external) < 10000000
    ) /* migration from recupera to cyber */
    OR LOWER(c.name) = '5a-collector'
    THEN cp.debts_fee_amount
    WHEN LOWER(c.name) LIKE '%cyber%'
    THEN (
      cp.installment_interest + cp.installment_fee_amount + cp.installment_costs + cp.installment_lawyers_fee
    )
  END AS debts_fee_amount,
  cp.discount_amount,
  cp.total_amount,
  cp.dt_due,
  CAST(IF(cp.status IN ('canceled', 'expired'), COALESCE(cp.ts_expired, cp.ts_updated), NULL) AS DATE) AS dt_canceled,
