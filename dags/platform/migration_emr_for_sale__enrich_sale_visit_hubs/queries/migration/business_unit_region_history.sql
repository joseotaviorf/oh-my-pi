...
FROM datalake_hub_services.business_unit AS bu
UNION ALL
SELECT
  ...
  CAST(bur.dt_start AS TIMESTAMP) AS ts_start_coverage,
  LEAST(
    CAST(NULLIF(bur.dt_end, '2022-05-23T17:25:17') AS TIMESTAMP),
    TO_TIMESTAMP('2022-05-23 17:25:17', 'yyyy-MM-dd HH:mm:ss')
  ) AS ts_end_coverage
FROM datalake_gsheets_clean.business_unit_region AS bur
WHERE
