WITH reservations_events AS (
  SELECT
    ...
    month,
    day
  FROM datalake_bigfone_events.events
  WHERE
    provider = 'twilio'
    AND (
  ...
  SELECT
    ...
    COUNT(
      CASE
        WHEN re1.event = 'reservation.accepted' OR re1.event = 'reservation.completed'
        THEN re1.id_reservation
      END
    ) > 0 AS is_answered,
    COUNT(CASE WHEN re1.event = 'reservation.timeout' THEN re1.id_reservation END) > 0 AS is_timeout,
    COUNT(CASE WHEN re1.event = 'reservation.rejected' THEN re1.id_reservation END) > 0 AS is_rejected,
    MIN(re1.ts_event_unix) AS ts_created_unix,
    MAX(re1.ts_event_unix) AS ts_ended_unix,
    re1.year,
    re1.month,
    re1.day
  FROM reservations_events AS re1
  LEFT JOIN reservations_events AS re2
    ON re1.id_task = re2.id_task
    AND re1.id_reservation = re2.id_reservation
    AND NOT re2.queue_name IS NULL
  GROUP BY
    1,
    2,
    3,
  ...
  SELECT
    re.*
  FROM reservations_events AS re
  JOIN call_flex_reservations AS cr
  ...
  SELECT
    *
  FROM tasks_events
), answered_time_calculations /* The logic of the CTE below is highly dependent of the order of the events (now we are sorting using */ /* the unix timestamp and untie by the alphabetical of the event). */ /* Changing these event names may impact on the results of this CTE and consequently the whole call model */ AS (
  SELECT
    fe.id_task,
    fe.id_reservation,
    CASE
      ...
      THEN LEAD(ts_event_unix, 1) OVER (PARTITION BY fe.id_task ORDER BY ts_event_unix, event DESC) - COALESCE(
        LAG(ts_event_unix, 1) OVER (PARTITION BY fe.id_task ORDER BY ts_event_unix, event DESC),
        ts_event_unix
      )
      WHEN event = 'reservation.created'
      AND LEAD(event, 1) OVER (PARTITION BY fe.id_task ORDER BY ts_event_unix, event DESC) = 'reservation.completed'
      THEN LEAD(ts_event_unix, 2) OVER (PARTITION BY fe.id_task ORDER BY ts_event_unix, event DESC) - ts_event_unix
    END AS seconds_queue_time,
    CASE
      WHEN event = 'reservation.accepted'
      THEN LEAD(ts_event_unix, 1) OVER (PARTITION BY fe.id_task ORDER BY ts_event_unix) - ts_event_unix
...
SELECT
  ...
  CAST(FROM_UNIXTIME(ts_created_unix) AS TIMESTAMP) AS ts_created,
  FROM_UTC_TIMESTAMP(CAST(FROM_UNIXTIME(ts_created_unix) AS TIMESTAMP), 'Brazil/East') AS ts_created_local,
  CAST(FROM_UNIXTIME(ts_ended_unix) AS TIMESTAMP) AS ts_ended,
  FROM_UTC_TIMESTAMP(CAST(FROM_UNIXTIME(ts_ended_unix) AS TIMESTAMP), 'Brazil/East') AS ts_ended_local,
  ts_created_unix,
  year,
  month,
  day
FROM call_flex_reservations AS cr
LEFT JOIN answered_time_calculations AS atc
  ON cr.id_task = atc.id_task AND cr.id_reservation = atc.id_reservation
GROUP BY
  1,
  2,
  3,
