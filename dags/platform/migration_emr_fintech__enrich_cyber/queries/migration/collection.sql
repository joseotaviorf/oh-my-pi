WITH codes_descriptions AS (
  ...
  FROM (
    SELECT DISTINCT
      UPPER(code) AS code,
      code_type,
      REGEXP_REPLACE(code_description, '(F\\)\\s*)', '') AS code_description,
      ROW_NUMBER() OVER (PARTITION BY UPPER(code), code_type ORDER BY group) AS _w,
      group
  ...
  FROM (
    SELECT
      ...
      cpc,
      promessa,
      ROW_NUMBER() OVER (PARTITION BY action_code, result_code, complement_code ORDER BY ts_load) AS _w,
      ts_load
  ...
  SELECT
    ...
    CASE
      WHEN action = '..'
      THEN LAG(IF(action = '..', NULL, action)) IGNORE NULLS OVER (PARTITION BY id_contract, contract_group, id_user, phone_number, ts_activity ORDER BY sequence_number)
      ELSE action
    END AS action,
    CASE
      WHEN action = '..'
      THEN LAG(result) IGNORE NULLS OVER (PARTITION BY id_contract, contract_group, id_user, phone_number, ts_activity ORDER BY sequence_number)
      ELSE result
    END AS result,
    CASE
      WHEN action = '..'
      THEN LAG(complement) IGNORE NULLS OVER (PARTITION BY id_contract, contract_group, id_user, phone_number, ts_activity ORDER BY sequence_number)
      ELSE complement
    END AS complement,
    sequence_number,
    comment,
    CASE
      WHEN action = '..'
      THEN LAG(phone_number) IGNORE NULLS OVER (PARTITION BY id_contract, contract_group, id_user, phone_number, ts_activity ORDER BY sequence_number)
      ELSE phone_number
    END AS phone_number,
    CASE
      WHEN action = '..'
      THEN LAG(phone_extension) IGNORE NULLS OVER (PARTITION BY id_contract, contract_group, id_user, phone_number, ts_activity ORDER BY sequence_number)
...
SELECT
  ...
  l.id_contract,
  l.contract_group,
  COALESCE(c.id_contract_external, SPLIT(l.id_contract, '\\.')[0]) AS id_contract_external,
  ...
  CASE
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) LIKE 'PASCH%'
    THEN 'PASCHOALOTTO'
    WHEN UPPER(l.id_user) LIKE 'PSC%'
    THEN 'PASCHOALOTTO'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) LIKE '%TRC%'
    THEN 'TRC'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) LIKE '%GRB%'
    THEN 'BULGARELLI'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) LIKE '%BULGARELLI%'
    THEN 'BULGARELLI'
    WHEN UPPER(l.id_user) LIKE '%BGR%'
    THEN 'BULGARELLI'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) LIKE '%MEETC%'
    THEN 'MEETCALL'
    WHEN UPPER(l.id_user) LIKE '%MTC%'
    THEN 'MEETCALL'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) LIKE '%MONES%'
    THEN 'MONEST'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) LIKE '%PELLON%'
    THEN 'PELLON'
    WHEN UPPER(l.id_user) LIKE 'PLL%'
    THEN 'PELLON'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) LIKE '%PLC%'
    THEN 'PLC'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) LIKE '%GONDIM%'
    THEN 'GONDIM'
    WHEN UPPER(l.id_user) LIKE 'GAN%'
    THEN 'GONDIM'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) LIKE '%NOVAQ%'
    THEN 'NOVAQUEST'
    WHEN UPPER(l.id_user) LIKE '%NVQ%'
    THEN 'NOVAQUEST'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) LIKE '%PORTAL%'
    THEN 'PORTAL_QUINTOANDAR'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) = 'WEBHELP'
    THEN 'WEBHELP'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, l.id_user)) LIKE '%WHELP%'
    THEN 'WEBHELP'
  ...
  l.phone_extension,
  CASE
    WHEN UPPER(l.id_user) IN ('SISTEMA', 'HOST', 'RCVRY')
    THEN 0
    WHEN UPPER(l.result) IN ('UU', 'BD')
    THEN 0
    WHEN UPPER(l.result) IN ('AG', 'AE', 'AF', 'AD', 'AB', 'AC', 'BA', 'BC', 'XX', 'YY')
    THEN 1
  ...
  CASE
    WHEN UPPER(l.id_user) IN ('SISTEMA', 'HOST', 'RCVRY')
    THEN 0
    WHEN UPPER(l.result) IN ('UU', 'BD', 'AG', 'AD')
    THEN 0
    WHEN UPPER(l.result) IN ('AE', 'AF', 'AB', 'AC', 'BA', 'BC', 'XX', 'YY')
    THEN 1
  ...
  CASE
    WHEN UPPER(l.id_user) IN ('SISTEMA', 'HOST', 'RCVRY')
    THEN 0
    WHEN UPPER(l.result) IN ('UU', 'AG', 'AE', 'AD', 'AB', 'BA', 'XX', 'BD')
    THEN 0
    WHEN UPPER(l.result) IN ('AF', 'AC', 'BC', 'YY')
    THEN 1
