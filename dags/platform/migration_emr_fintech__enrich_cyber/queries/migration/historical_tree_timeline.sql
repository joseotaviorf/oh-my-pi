WITH deduplicate_queues AS (
  ...
  SELECT
    h.id,
    SPLIT(h.id_contract, '\\.')[0] AS id_contract_external,
    h.id_contract,
    h.contract_group,
  ...
  FROM datalake_cyber_clean.historical_tree_decision AS h
  WHERE
    MAKE_DATE(h.year, h.month, h.day) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
), stacked AS (
  SELECT
    id_contract_external,
    'segmentation' AS dimension_key,
    COALESCE(dt_segmentation_queue_update, CAST(ts_record_insertion AS DATE)) AS dt_from,
    ...
    dt_due_date
  FROM historical_filtered
  UNION ALL
  SELECT
    id_contract_external,
    'commission',
    COALESCE(dt_commission_queue_update, CAST(ts_record_insertion AS DATE)),
  ...
  SELECT
    id_contract_external,
    'agreement',
    COALESCE(dt_agreement_queue_update, CAST(ts_record_insertion AS DATE)),
  ...
  SELECT
    id_contract_external,
    'digital_channel',
    COALESCE(dt_digital_channel_queue_update, CAST(ts_record_insertion AS DATE)),
  ...
  SELECT
    id_contract_external,
    'eviction',
    COALESCE(dt_eviction_queue_update, CAST(ts_record_insertion AS DATE)),
  ...
  SELECT
    id_contract_external,
    'credit_denial',
    COALESCE(dt_credit_denial_queue_update, CAST(ts_record_insertion AS DATE)),
  ...
  SELECT
    id_contract_external,
    'olos_dialer_label',
    COALESCE(dt_olos_dialer_label_update, CAST(ts_record_insertion AS DATE)),
  ...
  SELECT
    id_contract_external,
    'evictions_reason',
    COALESCE(dt_evictions_reason_update, CAST(ts_record_insertion AS DATE)),
  ...
  SELECT
    id_contract_external,
    'campaign_label',
    COALESCE(dt_campaign_label_update, CAST(ts_record_insertion AS DATE)),
  ...
  SELECT
    ...
    COALESCE(dt_serasa_limpa_nome_label_update, CAST(ts_record_insertion AS DATE)),
  ...
  SELECT
    id_contract_external,
    'evictions_label',
    COALESCE(dt_evictions_label_update, CAST(ts_record_insertion AS DATE)),
  ...
  SELECT
    id_contract_external,
    'label_6',
    COALESCE(dt_not_collection_label_update, CAST(ts_record_insertion AS DATE)),
  ...
  FROM (
    SELECT
      ...
      total_delayed_amount,
      dt_due_date,
      ROW_NUMBER() OVER (PARTITION BY id_contract_external, dimension_key, dt_from ORDER BY ts_record_insertion DESC, id DESC) AS _w
    FROM stacked
  ) AS _t
  WHERE
    _w = 1
  ...
  SELECT
    ...
    sd.dimension_key,
    sd.dt_from,
    LEAD(sd.dt_from) OVER (PARTITION BY sd.id_contract_external, sd.dimension_key ORDER BY sd.dt_from, sd.ts_record_insertion, sd.id) AS dt_next,
    sd.dim_value,
  ...
  LEFT JOIN deduplicate_queues AS dq
    ...
    AND (
      ...
      OR (
        si.dimension_key = 'agreement' AND dq.queue_type = 'Acordo'
      )
      OR (
        si.dimension_key = 'digital_channel' AND dq.queue_type = 'Canais Digitais'
      )
      OR (
        si.dimension_key = 'eviction' AND dq.queue_type = 'Eviction'
      )
      OR (
        si.dimension_key = 'credit_denial' AND dq.queue_type = 'Negativação'
      )
      OR (
        si.dimension_key = 'olos_dialer_label' AND dq.queue_type = 'Discador Olos'
      )
      OR (
        si.dimension_key = 'campaign_label' AND dq.queue_type = 'Campaign'
      )
      OR (
        si.dimension_key = 'serasa_limpa_nome_label'
        AND dq.queue_type = 'Label Serasa Limpa Nome'
      )
      OR (
        si.dimension_key = 'evictions_label' AND dq.queue_type = 'Label Evictions'
      )
    )
), calendar_dim AS (
  SELECT
  ...
  INNER JOIN intervals AS i
    ON CAST(d.date AS DATE) >= i.dt_from
    AND IF(
      i.dt_next IS NULL,
      CAST(d.date AS DATE) <= CURRENT_DATE,
      CAST(d.date AS DATE) < i.dt_next
    )
  WHERE
    CAST(d.date AS DATE) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
)
SELECT
  ...
  MAX(
    CASE WHEN cd.dimension_key = 'serasa_limpa_nome_label' THEN cd.dim_description END
  ) AS serasa_limpa_nome_label_description,
  ...
  CAST(cd.aux_date AS DATE) AS dt_reference,
  COALESCE(
  ...
  YEAR(TO_DATE(cd.aux_date)) AS year,
  MONTH(TO_DATE(cd.aux_date)) AS month,
  DAY(TO_DATE(cd.aux_date)) AS day
