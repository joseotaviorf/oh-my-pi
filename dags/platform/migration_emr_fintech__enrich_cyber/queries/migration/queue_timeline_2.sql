WITH deduplicate_queues AS (
  ...
  SELECT
    ...
    queue_type,
    CASE
      WHEN queue_type = 'agreement'
      THEN agreement_queue
      WHEN queue_type = 'segmentation'
      THEN segmentation_queue
      WHEN queue_type = 'eviction'
      THEN eviction_queue
    END AS queue,
    CASE
      WHEN queue_type = 'agreement'
      THEN agreement_queue_description
      WHEN queue_type = 'segmentation'
      THEN segementation_queue_description
      WHEN queue_type = 'eviction'
      THEN eviction_queue_description
    END AS queue_description,
    dt_updated,
    ts_last_activity
  FROM base_union
  UNPIVOT(dt_updated FOR queue_type IN (ts_last_update_agreement_queue AS `agreement`, ts_last_update_segmentation_queue AS `segmentation`, ts_last_update_eviction_queue AS `eviction`))
  ORDER BY
    queue_type,
    dt_updated,
    ts_last_activity
  ...
  FROM (
    SELECT
      *,
      ROW_NUMBER() OVER (PARTITION BY id_contract, queue_type, dt_updated ORDER BY ts_last_activity DESC, priority) AS _w
    FROM unpivot_table
    ORDER BY
      dt_updated
  ) AS _t
  ...
  SELECT
    ...
    COALESCE(
      DATE_ADD(
        LEAD(dt_updated) OVER (PARTITION BY id_contract, queue_type ORDER BY dt_updated),
        -1
      ),
      CURRENT_DATE
    ) AS dt_next_update
  ...
  LATERAL VIEW
  EXPLODE(SEQUENCE(dt_updated, dt_next_update)) AS dt_reference
  ORDER BY
    queue_type,
    dt_reference
)
...
FROM create_records_for_date
PIVOT(MAX(queue) AS queue, MAX(queue_description) AS queue_description FOR queue_type IN ('agreement' AS agreement, 'segmentation' AS segmentation, 'eviction' AS eviction))
ORDER BY
  dt_reference
