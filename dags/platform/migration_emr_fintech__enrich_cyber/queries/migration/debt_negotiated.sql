WITH invoices AS (
  ...
  FROM (
    SELECT
      ...
      ts_due,
      ts_limit_pause,
      ROW_NUMBER() OVER (PARTITION BY id_invoice, contract_group ORDER BY COALESCE(ts_update, MAKE_DATE(year, month, day)) DESC) AS _w,
      contract_group,
      ts_update,
      year,
      month,
  ...
  FROM (
    SELECT
      ...
      COALESCE(
        REGEXP_REPLACE(h.id_client, '\\.|\\-', ''),
        REGEXP_REPLACE(c.id_client, '\\.|\\-', '')
      ) AS id_client,
      ...
      ROW_NUMBER() OVER (PARTITION BY COALESCE(COALESCE(h.id_invoice, c.id_invoice), COALESCE(h.id_invoice, c.id_invoice)) ORDER BY COALESCE(h.id_agreement, c.id_offer) ASC) AS _w,
      h.id_agreement,
      c.id_offer
...
SELECT
  ...
  COALESCE(c.id_contract_external, SPLIT(COALESCE(b.id_contract, i.id_contract), '\\.')[0]) AS id_contract,
