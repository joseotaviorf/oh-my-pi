...
SELECT DISTINCT
  ...
  COALESCE(
    ...
    SPLIT(COALESCE(b.id_contract, ha.id_contract, cc.id_contract), '\\.')[0]
  ) AS id_contract,
...
FULL OUTER JOIN datalake_cyber_clean.campaign_contracts AS cc
  ON ha.id_invoice = cc.id_invoice
  AND COALESCE(b.id_contract, ha.id_contract) = cc.id_contract
