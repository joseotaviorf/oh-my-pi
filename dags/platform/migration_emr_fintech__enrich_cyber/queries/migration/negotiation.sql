WITH campaign_discount AS (
  SELECT
    id_offer AS id_agreement,
    SUM(
      IF(field_name IN ('Juros Residuais', 'Juros Acordo'), amount_without_discount, 0)
    ) AS contract_interest_fees_amount,
    SUM(
      IF(field_name IN ('Multa Residuais', 'Multa Acordo'), amount_without_discount, 0)
    ) AS fine_amount,
    SUM(
      IF(field_name IN ('Parcelas Vencidas', 'Parcelas a Vencer'), amount_without_discount, 0)
    ) AS original_amount,
    SUM(
      IF(
        field_name IN ('Custas Residuais', 'Custas Acordo', 'Honorários Residuais', 'Despesas judiciais'),
        amount_without_discount,
        0
      )
    ) AS eviction_costs_amount,
    SUM(IF(field_name IN ('Parcelas Vencidas', 'Parcelas a Vencer'), discount, 0)) AS discount_to_original_amount,
    SUM(IF(field_name IN ('Juros Residuais', 'Juros Acordo'), discount, 0)) AS discount_to_fees_amount,
    SUM(IF(field_name IN ('Multa Residuais', 'Multa Acordo'), discount, 0)) AS discount_to_fine_amount,
    SUM(
      IF(
        field_name IN ('Custas Residuais', 'Custas Acordo', 'Honorários Residuais', 'Despesas judiciais'),
        discount,
  ...
  SELECT
    ...
    COALESCE(ad.discount_to_fine_amount, cd.discount_to_fine_amount) AS discount_to_fine_amount,
    COALESCE(ad.discount_to_eviction_costs, cd.discount_to_eviction_costs) AS discount_to_eviction_costs,
    COALESCE(ad.debt_amount, cd.debt_amount) AS debt_amount,
    COALESCE(ad.negotiated_amount, cd.negotiated_amount) AS negotiated_amount,
  ...
  SELECT
    ai.id_agreement,
    MIN(IF(ai.status = 'Quebrado', CAST(ai.ts_due_installment AS DATE), NULL)) AS dt_cancelation,
    ...
    MAX(IF(NOT p.id_agreement_installment IS NULL, ai.installment_number + 1, 0)) AS paid_installments,
    ...
    SUM(
      IF(
        NOT p.id_agreement_installment IS NULL AND ai.installment_number = 0,
        ai.amount_to_pay,
        0
      )
    ) AS down_payment_amount,
    CASE
      WHEN COUNT(DISTINCT p.id_payment) = COUNT(DISTINCT ai.id_agreement_installment)
      THEN MAX(CAST(p.ts_payment AS DATE))
    END AS dt_paid_all,
    MIN(IF(ai.installment_number = 0, CAST(ai.ts_due_installment AS DATE), NULL)) AS dt_due_promisse,
    MAX(
      IF(
        NOT p.id_agreement_installment IS NULL AND ai.installment_number = 0,
        CAST(p.ts_payment AS DATE),
        NULL
      )
    ) AS dt_down_payment,
    MAX(CAST(ai.ts_due_installment AS DATE)) AS dt_expected_end
  ...
  SELECT
    id_agreement_type,
    MAX(max_discount_level_1) FILTER(WHERE
      label = 'Juros Acordo') AS max_interest_discount_percentage,
    MAX(max_discount_level_1) FILTER(WHERE
      label = 'Multa Acordo') AS max_fine_discount_percentage,
    MAX(max_discount_level_1) FILTER(WHERE
      label = 'Parcelas Vencidas') AS max_main_amount_discount_percantage
  FROM datalake_cyber_clean.agreement_type_discount
  WHERE
    LOWER(field_name) IN ('u1vlrprcag', 'u1vlrmuag', 'u1vlrjuag')
  GROUP BY
    1
  ...
  FROM (
    SELECT
      agency_group,
      id_agency,
      ROW_NUMBER() OVER (PARTITION BY agency_group ORDER BY percentage_remuneration DESC) AS _w,
  ...
  SELECT
    ...
    UPPER(a.authorized_by_manager) AS id_user_authorized,
    c.id_agency,
    ...
    COALESCE(CAST(a.ts_agreement_creation AS DATE), CAST(c.ts_boletagem_sent AS DATE)) AS dt_promisse,
    COALESCE(i.dt_due_promisse, CAST(c.ts_due_down_payment AS DATE)) AS dt_due_promisse,
    i.dt_down_payment,
    i.dt_paid_all,
    i.dt_expected_end,
    CASE
      WHEN a.status = 'Cancelado'
      THEN CAST(COALESCE(a.ts_agreement_breach, a.ts_canceled, a.ts_status_update) AS DATE)
      WHEN c.campaign_status = 'Vencida'
      THEN CAST(c.ts_due_boletagem AS DATE)
...
SELECT
  ...
  COALESCE(c.id_contract_external, SPLIT(a.id_contract, '\\.')[0]) AS id_contract,
  ...
  CASE
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, a.id_user)) LIKE 'PASCH%'
    THEN 'PASCHOALOTTO'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, a.id_user)) LIKE '%TRC%'
    THEN 'TRC'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, a.id_user)) LIKE '%GRB%'
    THEN 'BULGARELLI'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, a.id_user)) LIKE '%BGR%'
    THEN 'BULGARELLI'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, a.id_user)) LIKE '%MEETC%'
    THEN 'MEETCALL'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, a.id_user)) LIKE '%MONES%'
    THEN 'MONEST'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, a.id_user)) LIKE '%PELLON%'
    THEN 'PELLON'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, a.id_user)) LIKE '%PLC%'
    THEN 'PLC'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, a.id_user)) LIKE '%GONDIM%'
    THEN 'GONDIM'
    WHEN UPPER(COALESCE(agg.agency_name, ag.agency_name, a.id_user)) LIKE '%NOVAQ%'
    THEN 'NOVAQUEST'
    WHEN UPPER(a.id_user) LIKE 'PSC%'
    THEN 'PASCHOALOTTO'
    WHEN UPPER(a.id_user) LIKE 'NVQ%'
    THEN 'NOVAQUEST'
    WHEN UPPER(a.id_user) LIKE '%SERASA%'
    THEN 'SERASA'
    WHEN COALESCE(u.id_agency, a.id_agency) = 'QUINTO'
    THEN 'COBRANÇA_INTERNA_QA'
  ...
  CASE
    WHEN UPPER(a.contact_type) IN ('IAPORTAL', 'IAWPP')
    THEN 'Matthew'
    WHEN NOT a.id_campaign IS NULL OR UPPER(a.agreement_type) LIKE 'CAM%'
    THEN 'Boletagem'
    WHEN UPPER(at.agreement_type_description) LIKE '%SELFSERVICE%'
    THEN 'Portal Auto Negociação'
    WHEN UPPER(ag.agency_type) = 'PORTAL'
    THEN 'Portal Auto Negociação'
    WHEN UPPER(a.id_user) = 'PORTALQA'
    THEN 'Portal Auto Negociação'
    WHEN UPPER(ag.agency_type) = 'CYBER CREDIT'
    THEN 'Operador Interno'
    WHEN UPPER(a.agreement_type) LIKE '%SERASA%'
    THEN 'Serasa Digital'
    WHEN UPPER(ag.agency_type) IN ('ASSESSORIA CONVENCIONAL', 'ASSESSORIA JURÍDICA')
    THEN 'Assessoria'
    WHEN UPPER(a.id_user) = 'MIGRACAO'
    THEN 'Migração'
    ELSE ag.agency_type
  END AS origin_agreement,
  a.status AS original_negotiation_status,
  CASE
    WHEN a.status = 'Cancelado' AND NOT a.dt_down_payment IS NULL
    THEN 'broken'
    WHEN a.status = 'Cancelado' AND a.dt_down_payment IS NULL
    THEN 'canceled'
    WHEN a.status = 'Finalizado'
    THEN 'finished'
    WHEN a.status = 'Autorizado' AND NOT a.dt_down_payment IS NULL
    THEN 'offset'
    WHEN a.status = 'Autorizado' AND a.dt_down_payment IS NULL
    THEN 'started'
    WHEN a.status = 'Pendente'
    THEN 'pending'
    ELSE a.status
  END AS negotiation_status,
  a.campaign_status,
  a.description_broken_agreement,
  IF(NOT a.dt_down_payment IS NULL, TRUE, FALSE) AS is_down_payment_paid,
