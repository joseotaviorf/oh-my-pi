...
SELECT
  ...
  COALESCE(c.id_contract_external, SPLIT(dm.id_contract, '\\.')[0]) AS id_contract_external,
  c.id_client,
...
LEFT JOIN deduplicate_queues AS qdtpcd
  ON dm.previous_digital_channel_queue = qdtpcd.queue
  AND qdtpcd.queue_type = 'Canais Digitais'
...
LEFT JOIN deduplicate_queues AS qdtpn
  ON dm.previous_credit_denial_queue = qdtpn.queue
  AND qdtpn.queue_type = 'Negativação'
...
LEFT JOIN deduplicate_queues AS qdtlsln
  ON dm.serasa_limpa_nome_label = qdtlsln.queue
  AND qdtlsln.queue_type = 'Label Serasa Limpa Nome'
