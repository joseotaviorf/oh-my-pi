WITH deduplicate_records_delinquent_master AS (
  ...
  FROM (
    SELECT
      ...
      olos_dialer_label,
      id_agency,
      ROW_NUMBER() OVER (PARTITION BY id_contract, contract_group ORDER BY MAKE_DATE(year, month, day) DESC) AS _w,
      year,
      month,
      day
...
FROM deduplicate_records_delinquent_master AS dm
...
LEFT JOIN datalake_cyber_clean.bill AS b
  ON dm.id_contract = b.id_contract
  AND dm.contract_group = b.contract_group
  AND b.invoice_or_entry = 'Invoice'
