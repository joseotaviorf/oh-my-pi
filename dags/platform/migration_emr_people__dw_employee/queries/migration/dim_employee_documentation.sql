WITH hr_system_workers_ranked AS (
  SELECT
    id_person,
    national_identifiers,
    DENSE_RANK() OVER (PARTITION BY id_person ORDER BY dt_effective) AS dense_rank_effective
  FROM datalake_hr_system_clean.workers
), hr_system_workers AS (
  SELECT
    id_person,
    national_identifiers
  FROM hr_system_workers_ranked
  WHERE
    dense_rank_effective = 1
), national_identifiers_step1 AS (
  SELECT
    id_person,
    EXPLODE(national_identifiers) AS national_identifiers
  ...
  SELECT
    ...
    TO_TIMESTAMP(
      SUBSTRING(REPLACE(national_identifiers['LastUpdateDate'], 'T', ' '), 0, 19),
      'yyyy-MM-dd HH:mm:ss'
    ) AS ts_last_update
...
SELECT
  ...
  da.polling_station,
  GREATEST(
    CAST(ni2_cpf.ts_last_update AS DATE),
    CAST(ni2_rg.ts_last_update AS DATE),
    CAST(ni2_pis.ts_last_update AS DATE),
    da.dt_effective_start
  ) AS ts_last_update,
  NOW() AS ts_load
FROM hr_system_workers AS w
LEFT JOIN national_identifiers_step2 AS ni2_cpf
  ON w.id_person = ni2_cpf.id_person AND ni2_cpf.national_identifier_type = 'CPF'
LEFT JOIN national_identifiers_step2 AS ni2_rg
  ON w.id_person = ni2_rg.id_person AND ni2_rg.national_identifier_type = 'RG'
LEFT JOIN national_identifiers_step2 AS ni2_pis
  ON w.id_person = ni2_pis.id_person AND ni2_pis.national_identifier_type = 'PIS'
LEFT JOIN national_identifiers_dff AS nidff
  ON w.id_person = nidff.id_person
  AND ni2_rg.id_national_identifier = nidff.id_national_identifier
LEFT JOIN datalake_hr_system.demographic_attributes AS da
  ON w.id_person = da.id_person AND da.legislation_code = 'BR'
