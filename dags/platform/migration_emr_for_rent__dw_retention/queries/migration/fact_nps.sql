WITH union_customer_conversions AS (
  SELECT
    ...
    status,
    ts_dispatch_created
  FROM datalake_tracksale.customer_conversions
  UNION ALL
  SELECT
    id_dispatch,
  ...
  FROM datalake_casa_mineira_tracksale.customer_conversions
)
SELECT
  sk_nps,
  sk_termination,
...
FROM (
  SELECT
    ...
    NOW() AS ts_load,
    ROW_NUMBER() OVER (PARTITION BY ct.id_termination, cc.id_customer ORDER BY nps.ts_answer_sent_utc DESC) AS _w,
    ct.id_termination,
    cc.id_customer,
    nps.ts_answer_sent_utc
  FROM (
    SELECT
      *
    ...
    SELECT
      *
    FROM datalake_casa_mineira_tracksale.answer
  ) AS nps
  JOIN union_customer_conversions AS cc
    ON cc.id_answer = nps.id
  JOIN datalake_nps_answer_drivers.answer_drivers AS ad
    ON ad.id_answer = nps.id
  JOIN datalake_offboarding.contract_termination AS ct
  ...
  JOIN (
    ...
    SELECT
      *
    FROM datalake_casa_mineira_tracksale.dispatch
  ) AS d
  ...
  JOIN (
    ...
    SELECT
      *
    FROM datalake_casa_mineira_tracksale.campaign
  ) AS c
