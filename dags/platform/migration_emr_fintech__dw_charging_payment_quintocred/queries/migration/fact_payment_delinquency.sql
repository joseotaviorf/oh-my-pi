WITH base AS (
  SELECT
    ...
    dt_created,
    dt_due
  FROM dw_charging_payment_quintocred.fact_direct_billing
  UNION ALL
  SELECT
    id_propose,
    id,
    ...
    dt_created,
    dt_due
  FROM dw_charging_payment_quintocred.fact_charging_payment
), base_payment AS (
  SELECT
    *
  FROM (
    SELECT
      *,
      ROW_NUMBER() OVER (PARTITION BY id_propose, DATE_TRUNC('MONTH', dt_due) ORDER BY order_status ASC, origin_table ASC) AS _w
    FROM base
  ) AS _t
  WHERE
    _w = 1
...
SELECT
  ...
  p.value_paid,
  CASE
    WHEN NOT d.id_propose IS NULL AND p.id_propose IS NULL
    THEN FALSE
    WHEN NOT d.id_propose IS NULL AND NOT p.id_propose IS NULL
    THEN TRUE
  ...
  NOW() AS ts_load
FROM base_payment AS p
FULL OUTER JOIN dw_charging_payment_quintocred.fact_delinquecy AS d
