WITH ranked AS (
    SELECT
        ...
        day,
        NOW() AS ts_load,
        ROW_NUMBER() OVER (PARTITION BY id_dag, dt_execution ORDER BY ts_execution DESC) AS rn
    FROM
...
SELECT
    sk_snapshot, sk_dag, sk_line, sk_cluster_config, sk_execution_date, sk_spark_updated_date,
    cluster_name, job_name, dbus, price, daily_executions, spark_version_number,
    is_dag_builder_migrated, ts_bietlejuice_first_execution, ts_bietlejuice_last_execution,
    year, month, day, ts_load
FROM ranked
WHERE rn = 1
