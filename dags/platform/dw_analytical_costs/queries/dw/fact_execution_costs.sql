WITH ranked AS (
    SELECT
        ...
        day,
        NOW() AS ts_load,
        ROW_NUMBER() OVER (PARTITION BY id_cluster, ts_execution ORDER BY ts_execution DESC) AS rn
    FROM
...
SELECT
    sk_snapshot, sk_cluster, sk_cluster_config, sk_execution_date,
    cluster_name, dbus, price, spark_version_number, ts_execution,
