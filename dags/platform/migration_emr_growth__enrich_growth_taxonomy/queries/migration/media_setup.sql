WITH base AS (
  SELECT
    ...
    ts_load
  FROM datalake_growth_taxonomy.media_setup_staging
  UNION ALL
  SELECT
    id_media_setup,
    naming_convention_sufix,
    ...
    ts_combination_created,
    ts_load
  FROM datalake_growth_taxonomy.ad_hoc_rules
)
SELECT
  *
FROM (
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY naming_convention_sufix ORDER BY ts_load DESC) AS _w
  FROM base
) AS _t
WHERE
  _w = 1