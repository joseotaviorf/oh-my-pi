WITH amenities_aud AS (
...
), removed_redundancies /* Particularly in the first few years of operation, instead of updating existing rows, we would delete every single */ /* row and then create new ones after. Here, we're removing changes from the aud that are replaced by another change in less than a minute */ AS (
  SELECT
    *
  FROM (
    SELECT
      *,
      LEAD(has_feature) OVER (PARTITION BY id_house, NOT rev IS NULL, id_amenity, is_condo_amenity ORDER BY ts_change, rev_type DESC) AS _w,
      LEAD(ts_change) OVER (PARTITION BY id_house, NOT rev IS NULL, id_amenity, is_condo_amenity ORDER BY ts_change, rev_type DESC /* rev_type = 2 (deletion) will come first, so its row would be removed if the deletion is simultaneous with an insert */) AS _w_2
    FROM amenities_full_log
  ) AS _t
  WHERE
    COALESCE(UNIX_TIMESTAMP(_w_2) - UNIX_TIMESTAMP(ts_change), 61) > 60
    OR has_feature IS NOT DISTINCT FROM _w
  ...
  FROM (
    SELECT
      *,
      LAG(has_feature) OVER (PARTITION BY id_house, id_amenity, is_condo_amenity ORDER BY rev NULLS LAST, rev_type DESC) AS _w
    FROM removed_redundancies
  ) AS _t
  WHERE
    _w IS DISTINCT FROM has_feature
)
SELECT
  da.id_house,
  da.id_user,
