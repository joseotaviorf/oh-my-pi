WITH claims_omie AS (
  SELECT
    ...
    CASE
      ...
      THEN 'Assinatura'
      WHEN te.sk_category IN ('1.05.99', '2.11.99', '1.05.95', '2.11.94', '1.05.97', '2.11.97', '1.05.96', '2.11.96', '1.05.98', '2.11.98')
      THEN 'Garantia'
      ELSE 'Outros'
    ...
    IF(
      ...
      AND EXTRACT(DAY FROM (
        COALESCE(dt_paid, CURRENT_DATE) - dt_due
      )) > 0,
      1,
      0
    ) AS is_over,
  ...
  SELECT
    ...
    CASE
      WHEN j.desc_lvl_1 = 'GUARANTEE'
      THEN 'Garantia'
      WHEN j.desc_lvl_1 = 'TERMINATION'
      THEN 'Rescisao'
      ELSE NULL
    END AS invoice_type,
    IF(
      dt_due < CURRENT_DATE
      AND EXTRACT(DAY FROM (
        COALESCE(d.ts_paid, CURRENT_DATE) - dt_due
      )) > 0,
      1,
      0
    ) AS is_over,
  ...
  FROM dw_velo.fact_velo_occurrence AS d
  LEFT JOIN dw_velo.dim_velo_junk AS j
    ON d.sk_occurrence_type = j.sk_junk
  LEFT JOIN qtd_bill AS qtd
    ON d.sk_occurrence = qtd.id
  WHERE
    d.is_legacy IS FALSE AND j.desc_lvl_1 IN ('GUARANTEE', 'TERMINATION')
), claims_final AS (
  SELECT
    *,
    2 AS system
  ...
  SELECT DISTINCT
    b1.sk_propose,
    CAST(b1.ts_propose_started AS DATE) AS dt_created,
    CAST(b1.ts_evaluation_started AS DATE) AS dt_es,
    CAST(b1.ts_sign_started AS DATE) AS dt_ca,
    b1.dt_contract_started AS dt_cs
  FROM dw_velo.fact_velo_propose AS b1
  WHERE
  ...
  SELECT
    ...
    COLLECT_LIST(DISTINCT cm.mob) AS mob_array,
  ...
  LEFT JOIN cte_claims_by_month AS cm
  ...
  SELECT
    ...
    TIMESTAMPDIFF(
      MONTH,
      TO_DATE(DATE_TRUNC('MONTH', b1.dt_cs)),
      TO_DATE(DATE_TRUNC('MONTH', b2.dt_first_claim))
    ) AS mob,
    b2.mob_array
  FROM cte_contracts AS b1
  LEFT JOIN cte_claims AS b2
...
SELECT DISTINCT
  ...
  TIMESTAMPDIFF(
    MONTH,
    TO_DATE(DATE_TRUNC('MONTH', f.dt_cs)),
    TO_DATE(DATE_TRUNC('MONTH', CURRENT_DATE))
  ) AS cohort_age_mob_actual,
  ...
  IF(NOT f.mob IS NULL, 1, 0) AS claim_mob31_more,
  ...
  IF(NOT f.mob IS NULL, 1, 0) AS is_claim,
  f.dt_created,
  f.dt_es,
  f.dt_ca,
  f.dt_cs,
