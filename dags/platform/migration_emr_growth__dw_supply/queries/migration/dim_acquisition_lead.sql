WITH acq_lead AS (
  ...
  FROM (
    SELECT
      ...
      NOW() AS ts_updated,
      ROW_NUMBER() OVER (PARTITION BY sk_supply_lead, supply_source, business_context ORDER BY ts_event_adjusted DESC) AS _w,
      supply_source,
      business_context,
      ts_event_adjusted
    FROM datalake_supply_flows.supply_events_tracking
    WHERE
      funnel_step = 'PROSPECT' AND NOT sk_supply_lead IS NULL
  ) AS _t
  WHERE
    _w = 1
)
SELECT
  ...
  acq_lead.tp_lead,
  acq_lead.ts_updated,
  CONCAT_WS('#', acq_lead.sk_supply_lead, acq_lead.cd_supply_source, acq_lead.nm_business_context) AS bk_acq_lead
FROM acq_lead