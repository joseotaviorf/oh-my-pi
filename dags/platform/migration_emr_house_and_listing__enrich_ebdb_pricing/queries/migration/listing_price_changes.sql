WITH house_aud AS (
  SELECT
    ...
    r.reason,
    ROW_NUMBER() OVER (PARTITION BY h_aud.id_house, IF(lbc.ts_first_publication >= r.ts_revision, 'UNPUBLISHED', 'PUBLISHED') ORDER BY h_aud.rev DESC) AS threshold, /* The purpose of creating this threshold is to take the last line before the listing is published, so that we have the first price. */
    LAG(h_aud.rent) OVER (PARTITION BY h_aud.id_house ORDER BY r.ts_revision, h_aud.rev) /* We can't trust the mod_sale_price flag in 100% of cases, so we need to check if the price has changed manually. */ IS DISTINCT FROM h_aud.rent
  ...
  FROM (
    ...
    WHERE
      (
        ...
        OR (
          status_threshold = 'PUBLISHED' AND has_price_changed = TRUE
        )
  ...
  SELECT
    ...
    ) / NULLIF(lag_price, 0) AS last_price_variation,
    (
      rent_price - MIN(IF(lag_price IS NULL, rent_price, NULL)) OVER (PARTITION BY id_house)
    ) / NULLIF(MIN(IF(lag_price IS NULL, rent_price, NULL)) OVER (PARTITION BY id_house), 0) AS first_price_variation,
  ...
  SELECT
    ...
    lag_sale_price,
    CASE
      WHEN sale_price - lag_sale_price < 0
      THEN 'price down'
      WHEN sale_price - lag_sale_price > 0
      THEN 'price up'
      ELSE 'first price'
    ...
    ) / NULLIF(lag_sale_price, 0) AS last_price_variation,
    (
      sale_price - MIN(IF(lag_sale_price IS NULL, sale_price, NULL)) OVER (PARTITION BY id_house)
    ) / NULLIF(MIN(IF(lag_sale_price IS NULL, sale_price, NULL)) OVER (PARTITION BY id_house), 0) AS first_price_variation,
    ...
    LEAD(ts_price_started) OVER (PARTITION BY id_house ORDER BY ts_price_started) AS ts_price_ended,
    ts_first_publication
  FROM sale_price_changes_clean
), rent_price_changes AS (
  SELECT
    id_house,
    CASE WHEN is_first_price = TRUE THEN id_owner ELSE id_user_revision END AS id_user_revision,
    id_owner,
    id_region,
    id_revision,
    CASE WHEN is_first_price = TRUE THEN 'PUBLISHED' ELSE NULL END AS status,
    rent_price,
    lag_price,
    ...
    ROUND(
      CASE
        WHEN is_first_price
        THEN NULLIF(first_price_variation, 0)
    ...
    DATEDIFF(TO_DATE(COALESCE(ts_price_ended, CURRENT_DATE)), TO_DATE(ts_price_started)) AS days_with_pricing_scheme,
    ts_price_ended IS NULL AS is_last_price,
    is_first_price,
    ts_price_started,
    ts_price_ended,
    ts_first_publication
  FROM rent_price_changes_enriched
  WHERE
    (
  ...
  SELECT
    ...
    ts_price_ended,
    ts_first_publication
  FROM sale_price_changes_enriched
  WHERE
    (
  ...
  SELECT
    id_house,
    status_history,
    ROW_NUMBER() OVER (PARTITION BY id_house, CAST(ts_status_changed AS DATE) ORDER BY ts_status_changed DESC) AS order_status,
    CAST(ts_status_changed AS DATE) AS dt_change,
    ts_status_changed
  FROM datalake_sale_listings.sale_status_version_order
  ...
  SELECT
    ...
    CAST(DATE_ADD(
      COALESCE(
        LEAD(dt_change) OVER (PARTITION BY id_house ORDER BY ts_status_changed),
        CURRENT_DATE
      ),
      -1
    ) AS DATE) AS dt_status_ended_date
  FROM sale_version_order
  WHERE
    order_status = 1
  ...
  SELECT
    ...
    id_house_listing,
    status_history,
    CAST(ts_status_started AS DATE) AS dt_status_started_date,
    CAST(DATE_ADD(
      COALESCE(
        LEAD(CAST(ts_status_started AS DATE)) OVER (PARTITION BY id_house ORDER BY CAST(ts_status_started AS DATE)),
        CURRENT_DATE
      ),
      -1
    ) AS DATE) AS dt_status_ended_date
  ...
  SELECT
    ROW_NUMBER() OVER (PARTITION BY hpp_aud.id_house, CAST(r.ts_revision AS DATE) ORDER BY hpp_aud.rev DESC) AS order_status,
    CAST(r.ts_revision AS DATE) AS dt_change,
    ...
    hpp_aud.p_90,
    hpp_aud.certainty
  FROM datalake_ebdb_clean.house_predicted_price_aud AS hpp_aud
  INNER JOIN datalake_ebdb_user.user_revision_entity AS r
    ON hpp_aud.rev = r.id
  WHERE
    business_context = 'RENT'
  ...
  SELECT
    ...
    CAST(DATE_ADD(
      COALESCE(LEAD(dt_change) OVER (PARTITION BY id_house ORDER BY ts_change), CURRENT_DATE),
      -1
    ) AS DATE) AS dt_calculator_result_ended
  FROM house_predicted_rent_price_aud
  WHERE
    order_status = 1
  ...
  FROM house_predicted_sale_price_aud
  WHERE
    order_status = 1
)
SELECT
  ...
  pc.is_first_price,
  pc.is_last_price,
  NOT a.id IS NULL AS is_smart_price_change,
  pc.ts_price_started,
  pc.ts_price_ended
FROM rent_price_changes AS pc
LEFT JOIN rent_status_version_order AS rls
  ON rls.id_house = pc.id_house
  AND CAST(pc.ts_price_started AS DATE) BETWEEN rls.dt_status_started_date AND COALESCE(rls.dt_status_ended_date, CURRENT_DATE)
LEFT JOIN rent_calculator_changes AS cc
  ON cc.id_house = pc.id_house
  AND CAST(pc.ts_price_started AS DATE) BETWEEN cc.dt_calculator_result_started AND cc.dt_calculator_result_ended
LEFT JOIN datalake_ebdb_clean.dynamic_pricing_house_aud AS a
...
SELECT
  ...
  pc.ts_price_started,
  pc.ts_price_ended
FROM sale_price_changes AS pc
LEFT JOIN sale_status_version_order AS sls
...
LEFT JOIN sale_calculator_changes AS cc
  ON cc.id_house = pc.id_house
