WITH json_select AS(
  SELECT
    ...
    day
  FROM
    datalake_crawler_zap_imoveis_clean.zap_imoveis
  WHERE
...
SELECT
  ...
  GET_JSON_OBJECT(address,'$.number') AS street_number,
  ...
  COALESCE(
    CAST(GET_JSON_OBJECT(house_info, '$.unit_type[0]') AS STRING),
    CAST(GET_JSON_OBJECT(house_info, '$.unit_type') AS STRING)
  ) AS unit_type,
  COALESCE(
  ...
  GET_JSON_OBJECT(advertiser,'$.publisher_url') AS advertiser_url,  
  GET_JSON_OBJECT(advertiser, '$.licenseNumber') AS advertiser_creci,
  GET_JSON_OBJECT(advertiser,'$.address.billing.state') AS advertiser_address_state,
  GET_JSON_OBJECT(advertiser,'$.address.billing.city') AS advertiser_address_city,
  GET_JSON_OBJECT(advertiser,'$.address.billing.zipCode') AS advertiser_address_zip_code,
  GET_JSON_OBJECT(advertiser,'$.address.billing.neighborhood') AS advertiser_address_neighborhood,
  GET_JSON_OBJECT(advertiser,'$.address.billing.street') AS advertiser_address_street,
  GET_JSON_OBJECT(advertiser,'$.address.billing.streetNumber') AS advertiser_address_street_number,
