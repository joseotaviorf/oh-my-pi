WITH issues AS (
  ...
  FROM comment_author
  WHERE
    NOT id_update_author_account IS NULL
  UNION ALL
  /* Attachment author */
  SELECT
    id_account,
    ...
    is_active,
    ts_updated
  FROM attachment_author
  WHERE
    NOT id_account IS NULL
)
SELECT
  id_account,
  name,
...
FROM (
  SELECT
    ...
    is_active,
    ts_updated,
    ROW_NUMBER() OVER (PARTITION BY id_account ORDER BY ts_updated DESC) AS _w
  FROM union_account
) AS _t
WHERE
  1 = _w