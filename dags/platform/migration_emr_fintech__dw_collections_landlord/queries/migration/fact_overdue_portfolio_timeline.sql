WITH deal_deduplication AS (
  SELECT
    ...
    dt_end_deal,
    rn_deal_order
  FROM dw_collections_landlord.fact_landlord_deal_installment_match
  WHERE
    rn_deal_order = 1
  ...
  SELECT
    CASE
      WHEN m.tipo_fat = 'original_invoice'
      THEN m.tipo_fat
      WHEN NOT li.id_invoice_deal IS NULL AND m.tipo_fat = 'deal_invoice'
      THEN 'embedded in original row'
      WHEN li.id_invoice_deal IS NULL AND m.tipo_fat = 'deal_invoice'
      THEN 'deal invoice with no-match'
      ELSE NULL
    END AS flag_deal_installment_match,
    CASE WHEN NOT f.id_invoice_deal IS NULL THEN TRUE ELSE FALSE END AS flag_deal_existence,
    m.id_invoice,
    m.invoice_user,
    m.id_contract,
    m.id_account,
    ...
    f.dt_end_deal,
    f.rn_deal_order
  FROM dw_collections_landlord.fact_invoice_landlord_portfolio AS m
  LEFT JOIN deal_deduplication AS f
    ON f.id_invoice_original = m.id_invoice
  LEFT JOIN list_invoices_installments AS li
    ON li.id_invoice_deal = m.id_invoice
  WHERE
    1 = 1
    AND (
      (
        m.tipo_fat = 'original_invoice'
      )
      OR (
        m.tipo_fat = 'deal_invoice' AND li.id_invoice_deal IS NULL
      )
    )
  ORDER BY
    m.dt_begin DESC
  ...
  SELECT
    CASE
      WHEN flag_deal_existence
      THEN COALESCE(dt_end_deal, CURRENT_DATE)
      WHEN NOT (
        flag_deal_existence
      )
      THEN COALESCE(dt_end, CURRENT_DATE)
  ...
  SELECT
    ...
    CASE WHEN asdt.dt_reference > rdb.dt_ended_official THEN TRUE ELSE FALSE END AS flag_days_extended_outside_invoice,
    CASE
      WHEN NOT (
        rdb.flag_deal_existence
      )
      AND rdb.status = 'canceled'
      AND asdt.dt_reference >= rdb.dt_canceled
      THEN 'canceled'
      WHEN NOT (
        rdb.flag_deal_existence
      )
      AND rdb.status IN ('paid', 'written-down')
      AND asdt.dt_reference >= rdb.dt_paid
      THEN rdb.status
      WHEN NOT (
        rdb.flag_deal_existence
      ) AND rdb.status IN ('open')
      THEN 'open'
      WHEN NOT (
        rdb.flag_deal_existence
      )
      AND rdb.status IN ('not-payable')
      AND asdt.dt_reference >= rdb.dt_END
      THEN 'written-down-by-encontro-contas'
      WHEN NOT (
        rdb.flag_deal_existence
      )
      THEN 'open'
      ...
      AND rdb.status = 'written-down'
      AND asdt.dt_reference < rdb.dt_ended_official
      THEN CONCAT('deal', '-', 'open')
      WHEN (
        rdb.flag_deal_existence
      )
      AND rdb.status = 'written-down'
      AND asdt.dt_reference >= rdb.dt_ended_official
      AND rdb.status_deal <> 'not-payable'
      THEN CONCAT('deal', '-', rdb.status_deal)
      WHEN (
        rdb.flag_deal_existence
      )
      AND rdb.status = 'written-down'
      AND asdt.dt_reference >= rdb.dt_ended_official
      AND rdb.status_deal = 'not-payable'
      THEN CONCAT('deal', '-', 'written-down-by-encontro-contas')
      ELSE 'undefined'
    ...
    CASE
      ...
      AND rdb.status = 'written-down'
      AND asdt.dt_reference >= rdb.dt_paid
      THEN CONCAT('deal-extended-life')
      ELSE 'undefined'
  ...
  LEFT JOIN add_end_date_official AS rdb
    ON rdb.id_invoice = asdt.id_invoice AND rdb.id_contract = asdt.id_contract
), add_recovery_and_lagging AS (
  SELECT
    CASE WHEN LEAST(dt_reference, dt_ended_official) > dt_due THEN TRUE ELSE FALSE END AS flag_delay,
    CASE
      WHEN LEAST(dt_reference, dt_ended_official) > dt_due
      AND invoice_life_status_timeline = 'original-life'
      THEN TRUE
      WHEN LEAST(dt_reference, dt_ended_official) > dt_due_deal
      AND invoice_life_status_timeline <> 'original-life'
      THEN TRUE
      ELSE FALSE
    END AS flag_delay_t1,
    DATEDIFF(TO_DATE(LEAST(dt_reference, dt_ended_official)), TO_DATE(dt_due)) AS delay_t2,
    dt_reference,
    id_invoice,
    id_contract,
    dt_begin,
    ...
    rn_deal_order,
    CASE
      WHEN invoice_status_timeline IN ('paid', 'deal-paid', 'written-down-by-encontro-contas', 'deal-written-down-by-encontro-contas')
      THEN due_amount_adjustment
      ELSE 0
  ...
  SELECT
    ...
    CASE
      WHEN dd.total_working_days_in_month_fintech <= am.business_day
      AND dd.total_working_days_in_month_fintech = dd.working_days_in_month_fintech
      THEN 'atual'
      WHEN dd.working_days_in_month_fintech < am.business_day
      THEN 'passado'
      WHEN dd.working_days_in_month_fintech = am.business_day
      THEN 'atual'
      ELSE 'futuro'
    END AS class_business_day,
    COALESCE(NOT amdu.max_dt_business_day IS NULL, FALSE) AS is_ultima_dt_du,
    (
      dd.date = dd.month_end OR dd.date = DATE_ADD(CURRENT_DATE, -1)
    ) AS is_full_month,
  ...
  CROSS JOIN (
    ...
    FROM dw_public.dim_date AS dd
    WHERE
      dd.date = DATE_ADD(CURRENT_DATE, -1)
  ) AS am
  LEFT JOIN (
    SELECT
    ...
    FROM dw_public.dim_date AS dd
    WHERE
      dd.date BETWEEN ADD_MONTHS(TRUNC(CURRENT_DATE, 'MM'), -24) AND DATE_ADD(CURRENT_DATE, -1)
    GROUP BY
      dd.month_start,
      dd.working_days_in_month_fintech
  ) AS amdu
    ON amdu.max_dt_business_day = dd.date AND amdu.month_start = dd.month_start
  ...
  SELECT
    ...
    CASE
      WHEN COALESCE(g.max_delay_t2_contract_level, 0) <= 0
      THEN 'Current'
      WHEN COALESCE(g.max_delay_t2_contract_level, 0) <= 30
      THEN '1-30'
      WHEN COALESCE(g.max_delay_t2_contract_level, 0) <= 60
      THEN '31-60'
      WHEN COALESCE(g.max_delay_t2_contract_level, 0) <= 90
      THEN '61-90'
      ELSE '90+'
    END AS delay_range_contract_t2
  FROM add_recovery_and_lagging AS m
  LEFT JOIN aux_calendar AS f
    ON f.dt_base = m.dt_reference
  LEFT JOIN contract_level_delay AS g
    ON g.dt_reference = m.dt_reference AND g.id_contract = m.id_contract
)
SELECT
  ...
  ts_retsuko_updated,
  LAST_VALUE(
    CASE WHEN dt_reference = month_end THEN max_delay_t2_contract_level ELSE NULL END
  ) IGNORE NULLS OVER (PARTITION BY id_invoice ORDER BY dt_reference ASC) AS last_contract_delay_at_closing
FROM add_aux_calendar
