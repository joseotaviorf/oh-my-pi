WITH sale_offers AS (
  SELECT DISTINCT
    ...
    SUBSTRING(CAST(sr.id_house AS STRING), 4) AS sk_house,
    CAST(s.ts_created AS DATE) AS ts_created,
    ROW_NUMBER() OVER (PARTITION BY sr.id_house ORDER BY s.ts_created DESC) AS rn
  FROM datalake_monopoly_clean.sale AS s
  LEFT JOIN datalake_monopoly_clean.sale_revision AS sr
  ...
  SELECT
    ...
    CASE
      WHEN LENGTH(REGEXP_REPLACE(p.document_number, '[^0-9]', '')) = 11
      THEN CONCAT(
        '***.',
        SUBSTRING(REGEXP_REPLACE(p.document_number, '[^0-9]', ''), 4, 3),
        '.',
        SUBSTRING(REGEXP_REPLACE(p.document_number, '[^0-9]', ''), 7, 3),
        '-',
        '**'
      )
      WHEN LENGTH(REGEXP_REPLACE(p.document_number, '[^0-9]', '')) = 14
      THEN CONCAT(
        '**.',
        SUBSTRING(REGEXP_REPLACE(p.document_number, '[^0-9]', ''), 5, 3),
        '.',
        SUBSTRING(REGEXP_REPLACE(p.document_number, '[^0-9]', ''), 8, 3),
        '/',
        SUBSTRING(REGEXP_REPLACE(p.document_number, '[^0-9]', ''), 11, 4),
        '-**'
      )
      ELSE REGEXP_REPLACE(p.document_number, '[0-9]', '*')
    END AS document_number_mask,
    CAST(s.ts_created AS DATE) AS ts_created,
    ROW_NUMBER() OVER (PARTITION BY CONCAT('***', SUBSTRING(p.document_number, 4, LENGTH(p.document_number) - 5), '**'), p.person_name ORDER BY s.ts_created DESC) AS rn
  FROM datalake_monopoly_clean.person AS p
  LEFT JOIN datalake_monopoly_clean.person_sale AS p_sale
    ON p_sale.id_person = p.id
  LEFT JOIN datalake_monopoly_clean.sale AS s
    ON s.id = p_sale.id_sale
  LEFT JOIN datalake_monopoly_clean.sale_revision AS sr
    ON s.id = sr.id AND s.current_revision = sr.revision
  LEFT JOIN datalake_checkout_clean.order AS o
  ...
  LEFT JOIN datalake_checkout_clean.pix AS pix
    ON c.id = pix.id_charge AND pix.id_requester = 8 AND NOT pix.id_bank_payment IS NULL
  WHERE
    TRUE AND person_type IN ('buyer', 'seller')
  ...
  FROM datalake_monopoly_clean.sale_transaction AS st
  LEFT JOIN datalake_monopoly_clean.sale AS s
    ON s.id = st.id_sale
  LEFT JOIN datalake_monopoly_clean.sale_revision AS sr
    ON s.id = sr.id AND s.current_revision = sr.revision
  ...
  FROM datalake_nexxera.cnab_charges
  WHERE
    TRUE
  ...
  WHERE
    ...
    AND bank_account IN ('04525', '09846')
    AND occurrence_code IN ('06')
    AND NOT UPPER(document_number) LIKE '%FS%'
), bank_statement AS (
  SELECT
    ...
    CASE
      WHEN LOCATE('*', ext.counterpart_document) > 0
      THEN ext.counterpart_document
      ELSE CASE
        WHEN LENGTH(REGEXP_REPLACE(ext.counterpart_document, '[^0-9]', '')) = 11
        THEN CONCAT(
          '***.',
          SUBSTRING(REGEXP_REPLACE(ext.counterpart_document, '[^0-9]', ''), 4, 3),
          '.',
          SUBSTRING(REGEXP_REPLACE(ext.counterpart_document, '[^0-9]', ''), 7, 3),
          '-',
          '**'
        )
        WHEN LENGTH(REGEXP_REPLACE(ext.counterpart_document, '[^0-9]', '')) = 14
        THEN CONCAT(
          '**.',
          SUBSTRING(REGEXP_REPLACE(ext.counterpart_document, '[^0-9]', ''), 5, 3),
          '.',
          SUBSTRING(REGEXP_REPLACE(ext.counterpart_document, '[^0-9]', ''), 8, 3),
          '/',
          SUBSTRING(REGEXP_REPLACE(ext.counterpart_document, '[^0-9]', ''), 11, 4),
          '-**'
        )
        ELSE REGEXP_REPLACE(ext.counterpart_document, '[0-9]', '*')
      END
    END AS counterpart_document_mask,
    ext.counterpart_person,
    ext.origin_identifier,
    CASE
      WHEN ext.origin_operation IN ('PIX_RECEPCAO')
      THEN 'PIX'
      WHEN ext.origin_operation IN ('TED_CASHIN')
      THEN 'TED'
      WHEN ext.origin_operation IN ('TEF_CC_CC', 'TEF_CP_CC', 'TEF_SISPAG_PJ')
      THEN 'TEF'
    END AS origin_operation
  FROM datalake_itau_statements_clean.statement_879200452586 AS ext
  WHERE
    TRUE
  ...
  SELECT
    ...
    CASE
      ...
      THEN 'TEF'
    END AS origin_operation
  FROM datalake_itau_statements_clean.statement_879200984646 AS ext
  WHERE
    TRUE
    AND ext.operation IN ('C')
    AND ext.origin_operation IN ('PIX_RECEPCAO', 'TED_CASHIN', 'TEF_CC_CC', 'TEF_CP_CC', 'TEF_SISPAG_PJ')
  ...
  WHERE
    TRUE AND account_number IN ('110360X', '110901')
), franc_sale_offer AS (
  SELECT
    f.bank_account,
  ...
  FROM francesinha AS f
  LEFT JOIN sale_offers AS so
    ON so.sk_house = f.sk_house AND so.ts_created <= f.dt_paid AND so.rn = 1
  ORDER BY
    f.dt_paid
), bank_sale_person AS (
  SELECT
  ...
  LEFT JOIN sale_persons AS sp
    ON (
      bs.origin_identifier = sp.id_bank_payment
  ...
  SELECT
    ...
    fso.company_use,
    CAST(NULL AS STRING) AS counterpart_document,
    CAST(NULL AS STRING) AS counterpart_name
  FROM franc_sale_offer AS fso
  UNION ALL
  SELECT
    bsp.bank_account,
    bsp.sk_house,
  ...
  SELECT
    ...
    CASE WHEN (
      NOT st.sk_offer IS NULL
    ) THEN TRUE ELSE FALSE END AS monopoly_is_reconcilied,
    CASE
      WHEN (
        NOT s.sk_offer IS NULL
      ) AND (
        ABS(s.paid_amount) = ABS(ufb.paid_amount)
      )
      THEN TRUE
      ELSE FALSE
    END AS sap_is_reconcilied
  FROM union_franc_bank AS ufb
  LEFT JOIN sale_transactions AS st
    ON st.sk_offer = ufb.sk_offer
    AND CAST(st.amount AS DECIMAL(10, 2)) = CAST(ufb.paid_amount AS DECIMAL(10, 2))
    AND YEAR(TO_DATE(st.dt_accounting)) = YEAR(TO_DATE(ufb.dt_paid))
    AND MONTH(TO_DATE(st.dt_accounting)) = MONTH(TO_DATE(ufb.dt_paid))
  ...
  LEFT JOIN sap AS s
    ...
    AND YEAR(TO_DATE(s.dt_paid)) = YEAR(TO_DATE(ufb.dt_paid))
    AND MONTH(TO_DATE(s.dt_paid)) = MONTH(TO_DATE(ufb.dt_paid))
    AND s.dt_paid <= ufb.dt_paid
    AND s.rn = 1
  WHERE
    ufb.dt_paid >= CAST('2025-01-01' AS DATE)
), final_bypass AS (
  SELECT
    fb.bank_account,
    fb.company_use,
    ...
    CAST(NULL AS STRING) AS counterpart_name,
    fb.sk_house,
    CAST(NULL AS STRING) AS sk_offer,
    ...
    CASE WHEN NOT s.id_transaction IS NULL THEN TRUE ELSE FALSE END AS sap_is_reconcilied
  FROM francesinha_bypass AS fb
  LEFT JOIN sap AS s
    ON ABS(s.paid_amount) = fb.paid_amount
    AND (
      YEAR(TO_DATE(s.dt_paid)) = YEAR(TO_DATE(fb.dt_paid))
      AND MONTH(TO_DATE(s.dt_paid)) = MONTH(TO_DATE(fb.dt_paid))
    )
    AND s.id_sale_transaction IS NULL
  LEFT JOIN dw_sale.dim_listing AS sl
    ON SUBSTRING(CAST(sl.sk_house AS STRING), 4) = fb.sk_house
), final_all AS (
  SELECT
    *,
...
FROM (
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY CASE WHEN bank_type_transaction = 'BOLETO' THEN sk_house ELSE company_use END, CASE WHEN bank_type_transaction = 'BOLETO' THEN NULL ELSE counterpart_document END, CASE WHEN bank_type_transaction = 'BOLETO' THEN NULL ELSE counterpart_name END, bank_paid_amount, bank_dt_paid ORDER BY bank_dt_paid DESC) AS _w,
    bank_dt_paid
  FROM final_all
) AS _t
WHERE
