WITH
sap_entity AS (
    SELECT id_finance_entity, event, status, failed_reason, ts_created, id_sap_gateway_feature
    FROM (
        SELECT
            id_finance_entity,
            e.event,
            ...
            ROW_NUMBER() OVER (PARTITION BY id_finance_entity ORDER BY ts_created DESC) AS _rn
...
seu_barriga_base AS (
    SELECT id_invoice, id_original_invoice_external, company_use, payment_status, status, paid_via, reason, amount, dt_paid
    FROM (
        SELECT
            ...
            ROW_NUMBER() OVER (PARTITION BY id_external, payment_company_use_number ORDER BY ts_created DESC) AS _rn
        FROM
...
vans_checkout_union AS (
    ...
    SELECT company_use, id_invoice, ts_paid, payment_method, payment_status, paid_amount, our_number, payment_source
    FROM (
        SELECT
            COALESCE(
                CASE
            ...
            ROW_NUMBER() OVER (PARTITION BY b.your_number ORDER BY b.ts_paid DESC) AS _rn
        FROM
...
pre_vans_checkout AS (
    ...
    SELECT company_use, id_invoice, payment_method, payment_status, payment_source, dt_paid, paid_amount, our_number
    FROM (
        SELECT
            company_use,
            id_invoice,
...
vans_checkout_matched AS (
    SELECT join_key, company_use, id_invoice, payment_method, payment_status, dt_paid, pay_year, pay_month, amount, rn
    FROM (
        SELECT
            join_key,
            company_use,
