WITH sap_entity AS (
  ...
  SELECT DISTINCT
    hash,
    id_external_payment
  FROM datalake_sap_gateway.sap_payload
), sap AS (
  SELECT DISTINCT
  ...
  FROM datalake_accounting_funnel.ledger AS l
  LEFT JOIN de_para_company_use AS dp
    ON l.hash = dp.hash
  WHERE
    (
  ...
  SELECT
    ...
    bank_account,
    CASE
      WHEN bank_account = 426887
      THEN '11010X'
      WHEN bank_account = 79952
      THEN '11118X'
      WHEN bank_account = 433065
      THEN '11016X'
      WHEN bank_account = 502307
      THEN '11035X'
      WHEN bank_account = 502331
      THEN '11057X'
      ELSE 'unknown'
    END AS sap_account,
    dt_paid,
  ...
  FROM datalake_nexxera.cnab_payments
  WHERE
    occurrence_code IN ('00', 'DV')
    AND /* AND */ /*     is_latest_attempt IS TRUE */ bank_account IN (426887, 79952, 433065, 502307, 502331)
), cap AS (
  SELECT
    dt_paid,
  ...
  FROM datalake_accounting_funnel.payment_platforms
  WHERE
    payment_platform = 'vans_cap'
    AND payment_status IN ('paid', 'chargeback')
    AND (
      CAST(SPLIT(reference_5, ':')[0] AS INT) IN (1, 2, 7, 10, 13)
      OR reference_5 IS NULL
    )
  GROUP BY
    1,
    2,
  ...
  SELECT DISTINCT
    ...
    CASE
      WHEN cap.payment_status = 'chargeback' AND sap.paid_amount = 0
      THEN TRUE
      WHEN ABS(f.paid_amount) = ABS(cap.paid_amount)
      THEN TRUE
      ELSE FALSE
    END AS is_vans_compliance,
    CASE
      WHEN f.sap_account <> sap.account_number
      THEN FALSE
      WHEN (
        ABS(f.paid_amount) = ABS(sap.paid_amount)
      )
      THEN TRUE
      WHEN last_occurrence_code = 'DV'
      AND (
        sap.paid_amount = 0 OR sap.paid_amount IS NULL
      )
      THEN TRUE
      ELSE FALSE
  ...
  LEFT JOIN francesinha AS f
    ON f.company_use = cs.company_use AND cs.dt_paid = f.dt_paid
  LEFT JOIN cap
    ON cs.company_use = cap.company_use AND cs.dt_paid = cap.dt_paid
  LEFT JOIN sap
    ON cs.company_use = sap.company_use AND cs.dt_paid = sap.dt_paid
  LEFT JOIN seu_barriga AS r
    ON cs.company_use = r.company_use AND cs.dt_paid = r.dt_paid
  WHERE
    NOT cs.company_use IS NULL
    AND NOT (
      NOT cap.company_use IS NULL AND f.company_use IS NULL AND sap.company_use IS NULL
    )
  ...
  SELECT
    ...
    dt_paid,
    CASE
      WHEN vf = vs AND dt_bank_paid = dt_sap_paid
      THEN TRUE
      WHEN last_occurrence_code = 'DV'
      AND (
        sap_paid_amount = 0 OR sap_paid_amount IS NULL
      )
      THEN TRUE
      WHEN id_company_use LIKE '%MLRA%'
      THEN FALSE
      WHEN id_company_use LIKE '%MIP%'
      THEN FALSE
      WHEN id_company_use LIKE '%MCI%'
      THEN FALSE
      WHEN id_company_use LIKE '%!%' OR hash LIKE 'm%'
      THEN FALSE
      WHEN NOT dt_sap_tax IS NULL AND dt_sap_tax < dt_sap_paid
      THEN FALSE
      WHEN bank_paid_amount IS NULL AND vs >= 0
      THEN FALSE
      WHEN vf > 0 AND sap_paid_amount IS NULL
      THEN FALSE
      WHEN vf > 0 AND vc = 0 AND sap_paid_amount = 0 AND num_entries_sap > 0
      THEN TRUE
      WHEN vs % vf = 0
      THEN FALSE
      WHEN vf <> vs
    ...
    CASE
      ...
      THEN 'Concilied - Refund'
      WHEN id_company_use LIKE '%MLRA%'
      THEN 'Not Concilied - LRA single entry'
      WHEN id_company_use LIKE '%MIP%'
      THEN 'Not Concilied - MPI single entry'
      WHEN id_company_use LIKE '%MCI%'
      THEN 'Not Concilied - MCI single entry'
      WHEN id_company_use LIKE '%!%' OR hash LIKE 'm%'
      THEN 'Not Concilied - manual accounting'
      WHEN NOT dt_sap_tax IS NULL AND dt_sap_tax < dt_sap_paid
      THEN 'Not Concilied - retroactive adjustment'
      WHEN bank_paid_amount IS NULL AND vs >= 0
      THEN 'Not Concilied - Fracesinha missing'
      WHEN vf > 0 AND sap_paid_amount IS NULL
      THEN 'Not Concilied - SAP missing'
      WHEN vf > 0 AND vc = 0 AND sap_paid_amount = 0 AND num_entries_sap > 0
      THEN 'Concilied - SAP zeroed by chargeback'
      WHEN vs % vf = 0
      THEN 'Not Concilied - SAP duplicated'
      WHEN vf <> vs
      THEN 'Not Concilied - SAP <> Francesinha'
...
SELECT
  ...
  aux.hash,
  id_invoice,
  CAST(bank_account_number AS STRING) AS account_number,
  bank_account_number,
  sap_account_number,
  num_entries_sap,
  last_occurrence_code,
  ...
  CASE
    WHEN is_bank_concilied_detail = 'Not Concilied - SAP missing'
    AND aux.id_invoice IS NULL
    THEN 'retsuko not found'
    WHEN is_bank_concilied_detail = 'Not Concilied - SAP missing'
    AND e.id_finance_entity IS NULL
    THEN 'sap entity not found'
    WHEN is_bank_concilied_detail = 'Not Concilied - SAP missing' AND e.status = 'failed'
    THEN CONCAT('sap_entity failed:', e.failed_reason)
    WHEN is_bank_concilied_detail = 'Not Concilied - SAP missing'
    AND sg.id_feature IS NULL
    THEN 'gateway not found'
    WHEN is_bank_concilied_detail = 'Not Concilied - SAP missing'
    AND sg.sync_sap_job_status = 'error'
