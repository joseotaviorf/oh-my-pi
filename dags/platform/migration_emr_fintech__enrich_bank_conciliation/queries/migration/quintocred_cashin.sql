WITH bank_statement AS (
  SELECT
    CASE
      WHEN LENGTH(TRIM(ext.origin_complement)) > 26
      THEN ext.origin_complement
  ...
  FROM datalake_itau_statements_clean.statement_879200494588 AS ext
  WHERE
    ext.operation IN ('C') AND ext.origin_operation IN ('PIX_RECEPCAO')
), francesinha AS (
  SELECT
    CASE
      WHEN LENGTH(TRIM(ccrv2.document_number)) > 0
      THEN TRIM(REPLACE(ccrv2.document_number, 'C!', ''))
      ELSE ccrv2.our_number
    END AS company_use,
    CASE
      WHEN LENGTH(TRIM(ccrv2.document_number)) > 0
      THEN 'BOLETO_DIRECT_BILLING'
      ELSE 'BOLETO_COLLECTION'
    END AS type_transaction,
    ccrv2.net_amount AS paid_amount,
    ccrv2.dt_credit AS dt_paid
  FROM datalake_nexxera_clean.cnab_charges_recupera_velo_2 AS ccrv2
  WHERE
    ccrv2.occurrence_code IN ('06') AND ccrv2.bank_account IN ('04945')
), bank_union AS (
  SELECT
    *
  FROM bank_statement
  ...
  SELECT
    ...
    SUBSTRING(c.id_transaction, 1, 30) AS company_use,
    c.amount AS paid_amount,
    CAST(c.ts_updated AS DATE) AS dt_paid
  FROM datalake_pixar_clean.charge AS c
  WHERE
    c.status IN ('paid')
  UNION ALL
  SELECT
    i.id_external AS our_number,
    SUBSTRING(n.id, 1, 30) AS company_use,
  ...
  SELECT
    id_transaction,
    CASE
      WHEN REPLACE(accounting_rule, '+', '-') RLIKE 'baixa-contas-receber:conta-banco-cash-in|baixa-conta-receber:valores-receber-imobiliaria-conta-banco-cash-in'
  ...
  WHERE
    account_number IN ('11035X') AND source_client IN ('rental-guarantee-pla')
  GROUP BY
    1,
    2,
    3,
...
SELECT
  ...
  IF(
    NOT p.our_number IS NULL
    AND p.paid_amount = bu.paid_amount
    AND p.dt_paid = bu.dt_paid,
    TRUE,
    FALSE
  ) AS is_charge_concilied,
  IF(
    NOT s.company_use IS NULL
    AND s.paid_amount = bu.paid_amount
    AND s.dt_paid = bu.dt_paid,
    TRUE,
    FALSE
  ) AS is_sap_concilied,
...
FROM bank_union AS bu
LEFT JOIN payment_systems AS p
