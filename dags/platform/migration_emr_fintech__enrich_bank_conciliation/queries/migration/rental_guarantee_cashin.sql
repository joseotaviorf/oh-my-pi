WITH rental_guarantee_and_bank AS (
  ...
  FROM datalake_rental_guarantee_clean.sap AS s
  LEFT JOIN datalake_rental_guarantee_clean.charge AS c
    ON c.id = s.id_finance_entity AND c.id_guarantee = s.id_business_entity
  LEFT JOIN datalake_checkout_clean.pix AS p
    ON p.id_transaction = c.id_external
  LEFT JOIN datalake_checkout_clean.charge AS checkout_charge
    ON checkout_charge.id = p.id_charge
  LEFT JOIN datalake_itau_statements_clean.statement_879200429691 AS itau
...
SELECT
  ...
  rg.checkout_amount,
  sl.sap_amount
FROM rental_guarantee_and_bank AS rg
LEFT JOIN sap_gateway AS sg
  ON rg.id_feature = sg.id_feature
LEFT JOIN sap_ledger AS sl
  ON sg.hash = sl.hash AND rg.id_finance_entity = sl.id_finance_entity
