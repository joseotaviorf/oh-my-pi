WITH house AS (
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY h.id_external ORDER BY h.id DESC) AS _w
    FROM datalake_ebdb_clean.house AS h
    WHERE
      NOT h.id_external IS NULL
  ) AS _t
  WHERE
    _w = 1
), bc_sale AS (
  ...
  FROM (
    SELECT
      bcd.id_lead,
      bcd.ts_created,
      ROW_NUMBER() OVER (PARTITION BY bcd.id_lead ORDER BY bcd.ts_created DESC) AS _w
    FROM datalake_brokers_supply_processor_clean.business_context_detail AS bcd
    WHERE
...
SELECT
  ...
  l.cnpj,
  l.lead_hash,
  NULLIF(TRIM(GET_JSON_OBJECT(l.location, '$.address')), '') AS address, /* location */
  ...
  CAST(NULLIF(TRIM(GET_JSON_OBJECT(l.location, '$.regionId')), '') AS BIGINT) AS region_id,
  ...
  CAST(NULLIF(TRIM(GET_JSON_OBJECT(l.pricing, '$.rent')), '') AS DOUBLE) AS rent, /* pricing */
  ...
  CAST(NULLIF(TRIM(GET_JSON_OBJECT(l.blueprint, '$.totalArea')), '') AS INT) AS total_area, /* blueprint */
  ...
  NULLIF(TRIM(GET_JSON_OBJECT(l.owner, '$.email')), '') AS owner_email, /* owner */
  ...
  NULLIF(TRIM(GET_JSON_OBJECT(l.owner_agent, '$.personUuid')), '') AS owner_agent_person_uuid, /* owner_agent */
  ...
  NULLIF(TRIM(GET_JSON_OBJECT(l.access, '$.accessType')), '') AS access_type, /* access */
  ...
  NULLIF(TRIM(GET_JSON_OBJECT(l.brokers, '$.condominium')), '') AS condominium, /* brokers */
  ...
  NULLIF(TRIM(GET_JSON_OBJECT(l.details, '$.description')), '') AS house_description, /* details */
  l.details, /* raw JSON columns */
  l.photos,
  l.is_sent_to_main, /* booleans */
  ...
  FROM_UTC_TIMESTAMP(CAST(bs.ts_created AS TIMESTAMP), 'America/Sao_Paulo') AS ts_sale_business_context_created,
  FROM_UTC_TIMESTAMP(CAST(br.ts_created AS TIMESTAMP), 'America/Sao_Paulo') AS ts_rent_business_context_created,
  ...
  YEAR(TO_DATE(l.ts_updated)) AS year,
  MONTH(TO_DATE(l.ts_updated)) AS month,
  DAY(TO_DATE(l.ts_updated)) AS day
FROM datalake_brokers_supply_processor_clean.lead_3p AS l
LEFT JOIN house AS h
  ON h.id_external = l.uuid_lead
LEFT JOIN bc_sale AS bs
  ON l.id = bs.id_lead
LEFT JOIN bc_rent AS br
  ON l.id = br.id_lead