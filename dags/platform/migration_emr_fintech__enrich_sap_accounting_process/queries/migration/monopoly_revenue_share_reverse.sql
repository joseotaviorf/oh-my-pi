WITH monopoly AS (
  ...
  LEFT JOIN datalake_monopoly_clean.sale_transaction AS st
    ON st.id_sale = s.id
    AND st.event IN ('estate-agents-revenue-share', 'estate-agents-revenue-share-reversion')
  ...
  WHERE
    1 = 1
    AND dt_reference >= CAST('2025-01-01' AS DATE)
...
SELECT
  (
    'RE-MNPL-RS-' || sl.id_finance_entity_entry || '-' || COALESCE(sl.account_number, '')
  ...
  sl.account_number,
  CASE
    WHEN sl.account_number = '700013'
    THEN 'Gross Revenue - Brokarage (For Sale)'
    WHEN sl.account_number = '700006'
    THEN 'Revenue Share - Brokarage - AGENT (For Sale)'
    WHEN sl.account_number = '700007'
  ...
  MIN(
    CASE
      WHEN m.id_feature IS NULL AND sg.id_feature IS NULL
      THEN 'manual transaction'
      WHEN m.id_feature IS NULL
      AND NOT sg.id_feature IS NULL
      AND sl.source_client <> 'monopoly'
      THEN CONCAT('source', '-', sl.source_client)
      WHEN m.id_feature IS NULL AND NOT sg.id_feature IS NULL
...
FROM sap_ledger AS sl
LEFT JOIN sap_gateway AS sg
  ON sl.id_feature = sg.id_feature
LEFT JOIN monopoly AS m
  ON sl.id_feature = m.id_feature AND sl.account_number = m.account_number
WHERE
  m.id_feature IS NULL
GROUP BY
  1,
  2,
  3,
