WITH aux_business_days AS (
  SELECT
    date
  FROM datalake_quintoandar.aux_date
  WHERE
    is_brz_business_day = TRUE
), sap AS (
  SELECT
    l.id_business_entity,
    REGEXP_REPLACE(l.id_finance_entity, '[^0-9]', '') AS id_finance_entity,
    l.id_finance_entity_entry,
    l.hash,
    l.account_number,
    l.account_name,
    SUM(l.debit_credit) OVER (PARTITION BY REGEXP_REPLACE(l.id_finance_entity, '[^0-9]', '')) AS sap_amount,
    l.dt_reference AS dt_sap_reference,
    l.dt_created AS dt_sap_created
  FROM datalake_accounting_funnel.ledger AS l
  WHERE
    account_number = '513020'
), sap_gateway AS (
  SELECT
    id_finance_entity,
    id_feature,
    hash,
    sync_sap_job_status,
    sap_send_status,
    sap_processed_status,
    webhook_error
  FROM (
    SELECT
      f.id_finance_entity,
      s.id_feature,
      s.hash,
      s.status AS sync_sap_job_status,
      w.status AS sap_send_status,
      w.webhook_status AS sap_processed_status,
      w.errors AS webhook_error,
      ROW_NUMBER() OVER (PARTITION BY f.id_finance_entity, s.id_feature ORDER BY s.ts_updated) AS _w,
      s.ts_updated
    FROM datalake_sap_gateway_clean.feature AS f
    LEFT JOIN datalake_sap_gateway_clean.sync_sap_job AS s
      ON f.id_feature = s.id_feature
    LEFT JOIN datalake_sap_gateway_clean.webhook_log AS w
      ON s.idoc = w.idoc
    WHERE
      s.erp_solution IN ('S4')
      AND s.type = 'LCM'
      AND NOT s.status IN ('ignore', 'ignored')
  ) AS _t
  WHERE
    _w = 1
), retsuko AS (
  SELECT
    id_business_entity,
    id_finance_entity,
    id_sap_gateway_feature,
    version,
    event,
    status,
    failed_status,
    failed_reason,
    accrual_year_month,
    retsuko_amount,
    dt_source_trigger
  FROM (
    SELECT DISTINCT
      ct.id_external AS id_business_entity,
      i.id_external AS id_finance_entity,
      se.id_sap_gateway_feature,
      se.version,
      se.event,
      se.status,
      se.failed_status,
      se.failed_reason,
      i.accrual_year_month,
      CAST(i.due_amount AS DECIMAL(12, 2)) AS retsuko_amount,
      CAST(i.ts_write_off AS DATE) AS dt_source_trigger,
      ROW_NUMBER() OVER (PARTITION BY i.id_external, se.event ORDER BY se.ts_updated DESC) AS _w,
      se.ts_updated
    FROM datalake_retsuko.entry AS e
    INNER JOIN datalake_retsuko_clean.account AS af
      ON e.id_from_account = af.id
    INNER JOIN datalake_retsuko_clean.account AS at
      ON e.id_to_account = at.id
    LEFT JOIN datalake_retsuko.invoice AS i
      ON e.id_invoice = i.id
    LEFT JOIN datalake_retsuko_clean.contract AS ct
      ON ct.id = e.id_contract
    LEFT JOIN datalake_retsuko_clean.sap_entity AS se
      ON se.id_finance_entity = i.id_external
    WHERE
      TRUE
      AND i.is_write_off = TRUE
      AND ct.country_code = 'BR'
      AND af.type IN ('contract', 'tenant', 'landlord')
      AND at.type IN ('contract', 'tenant', 'landlord')
      AND NOT i.reason IN ('write-off-negotiation-cyber', 'write-off-negotiation-5A')
  ) AS _t
  WHERE
    _w = 1
), df AS (
  SELECT
    'RTSK-WO' || '-' || r.id_finance_entity || '-' || COALESCE(account_number, '') AS id_accounting_process,
    r.id_business_entity,
    r.id_finance_entity,
    CAST(NULL AS STRING) AS id_finance_entity_entry,
    r.version,
    s.account_number,
    r.retsuko_amount,
    CAST(s.sap_amount AS DECIMAL(12, 2)) AS sap_amount,
    r.accrual_year_month,
    MIN(
      CASE
        WHEN NOT s.hash IS NULL
        THEN 'success'
        WHEN s.hash IS NULL
        AND (
          r.id_finance_entity IS NULL OR r.status = 'failed' OR NOT r.failed_reason IS NULL
        )
        THEN 'source failure'
        WHEN s.hash IS NULL
        AND (
          sg.id_feature IS NULL
          OR sg.sync_sap_job_status = 'error'
          OR NOT sg.webhook_error IS NULL
        )
        THEN 'gateway failure'
        ELSE 'unknown failure'
      END
    ) AS accounting_process_status,
    MIN(
      CASE
        WHEN s.hash IS NULL AND r.id_finance_entity IS NULL
        THEN 'source not found'
        WHEN s.hash IS NULL AND r.status = 'failed'
        THEN r.failed_reason
        WHEN s.hash IS NULL AND sg.id_feature IS NULL
        THEN 'gateway not found'
        WHEN s.hash IS NULL AND sg.sync_sap_job_status = 'error'
        THEN sg.webhook_error
        WHEN s.hash IS NULL AND NOT r.id_finance_entity IS NULL AND NOT sg.id_feature IS NULL
        THEN 'sap not found'
        ELSE NULL
      END
    ) AS error_description,
    r.dt_source_trigger,
    MAX(s.dt_sap_reference) AS dt_sap_reference,
    MAX(s.dt_sap_created) AS dt_sap_created,
    IF(s.hash IS NULL, FALSE, TRUE) AS is_completeness,
    IF(
      (
        ABS(retsuko_amount) - ABS(sap_amount)
      ) >= 0.05
      OR (
        ABS(retsuko_amount) - ABS(sap_amount)
      ) <= -0.05
      OR sap_amount IS NULL,
      FALSE,
      TRUE
    ) AS is_correctness,
    CASE
      WHEN dt_sap_reference IS NULL OR dt_source_trigger IS NULL
      THEN FALSE
      WHEN TRUNC(CAST(dt_source_trigger AS DATE), 'MM') = TRUNC(CAST(dt_sap_reference AS DATE), 'MM')
      THEN TRUE
      WHEN TRUNC(CAST(dt_sap_reference AS DATE), 'MM') = ADD_MONTHS(TRUNC(CAST(dt_source_trigger AS DATE), 'MM'), 1)
      AND (
        SELECT
          COUNT(*)
        FROM aux_business_days AS ad
        WHERE
          ad.date >= LEAST(CAST(dt_source_trigger AS DATE), CAST(dt_sap_reference AS DATE))
          AND ad.date <= GREATEST(CAST(dt_source_trigger AS DATE), CAST(dt_sap_reference AS DATE))
      ) <= 3
      THEN TRUE
      ELSE FALSE
    END AS is_temporality
  FROM retsuko AS r
  LEFT JOIN sap_gateway AS sg
    ON sg.id_feature = r.id_sap_gateway_feature
  LEFT JOIN sap AS s
    ON s.hash = sg.hash OR s.id_finance_entity = CAST(r.id_finance_entity AS STRING)
  WHERE
    TRUE
  GROUP BY ALL
)
SELECT
  id_accounting_process || '-' || ROW_NUMBER() OVER (PARTITION BY id_accounting_process ORDER BY dt_sap_created) AS id_accounting_process,
  id_business_entity,
  id_finance_entity,
  id_finance_entity_entry,
  version,
  'for rent' AS business_unit,
  'seu barriga' AS source_name,
  'write-off' AS accounting_type,
  account_number,
  'write-off' AS accounting_name,
  retsuko_amount AS source_amount,
  sap_amount,
  is_completeness,
  is_correctness,
  is_temporality,
  IF(
    is_completeness IS TRUE AND is_correctness IS TRUE AND is_temporality IS TRUE,
    TRUE,
    FALSE
  ) AS is_compliance,
  accounting_process_status,
  error_description,
  accrual_year_month,
  dt_source_trigger,
  dt_sap_reference,
  dt_sap_created
FROM df