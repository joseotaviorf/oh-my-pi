WITH monopoly AS (
  SELECT
    ...
    ne.error_type,
    CASE
      WHEN st.event = 'nf-provision-reversion'
      THEN SUM(ae.credit) * -1
      WHEN st.event = 'nf-provision'
      THEN SUM(ae.credit)
      ELSE 0
  ...
  WHERE
    ...
    AND NOT st.id_external_sync IS NULL
    AND ae.credit <> 0
  GROUP BY
    st.id,
    ae.id,
  ...
  INNER JOIN sap_gateway_base AS sg
    ON l.hash = sg.hash
  WHERE
    1 = 1 AND dt_reference >= CAST('2024-01-01' AS DATE) AND account_number = '420032'
  GROUP BY
    1,
    2,
    3
  ...
  SELECT
    (
      'MNPL-P' || '-' || COALESCE(id_finance_entity_entry, id_finance_entity) || '-' || COALESCE(account_number, '')
