WITH monopoly AS (
  ...
  INNER JOIN sap_gateway_base AS sg
    ON l.hash = sg.hash
  WHERE
    1 = 1 AND dt_reference >= CAST('2025-01-01' AS DATE) AND account_number = '420032'
  GROUP BY
    1,
    2,
    3,
...
SELECT
  (
    'RE-MNPL-P-' || sl.id_finance_entity_entry || '-' || COALESCE(sl.account_number, '')
  ...
  CASE
    ...
    THEN 'Brokerage For Sale (Platform)'
