WITH aux_business_days AS (
  ...
  INNER JOIN datalake_retsuko_clean.contract AS ct
    ON ct.id = e.id_contract
  WHERE
    CAST(e.ts_created AS DATE) > CAST('2025-01-01' AS DATE)
    AND e.bill_item IN ('entry.bill-item/adjustment-agreement-rental', 'entry.bill-item/between-contracts', 'entry.bill-item/condominium-reserves-funds', 'entry.bill-item/condominium-usage', 'entry.bill-item/condominium', 'entry.bill-item/early-termination-fee', 'entry.bill-item/igpm-rental', 'entry.bill-item/improvement-work', 'entry.bill-item/ipca-rental', 'entry.bill-item/iptu-adjustment', 'entry.bill-item/iptu', 'entry.bill-item/light-water-or-gas', 'entry.bill-item/others', 'entry.bill-item/rental', 'entry.bill-item/repair-work', 'entry.bill-item/residential-protection-5A-fund-transfer')
  ...
  WHERE
    NOT id_finance_entity IS NULL AND event IN ('new-accounting-entries')
), sap_gateway AS (
  SELECT
    id_finance_entity,
    id_feature,
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY f.id_finance_entity, s.id_feature, s.hash ORDER BY w.ts_updated DESC) AS _w,
      w.ts_updated
  ...
  WHERE
    1 = 1 AND account_number IN (700004)
  GROUP BY
    1,
    2,
    3,
  ...
  SELECT
    ...
    MIN(
      CASE
        WHEN NOT sl_hash.hash IS NULL
        THEN 'success'
        WHEN sl_hash.hash IS NULL
        AND (
    ...
    MIN(
      CASE
        WHEN sl_hash.hash IS NULL AND se.id_finance_entity IS NULL
        THEN 'source not found'
        WHEN sl_hash.hash IS NULL AND se.status = 'failed'
        THEN se.failed_reason
        WHEN sl_hash.hash IS NULL AND sg.id_feature IS NULL
        THEN 'gateway not found'
        WHEN sl_hash.hash IS NULL AND sg.sync_sap_job_status = 'error'
        THEN sg.webhook_error
        WHEN sl_hash.hash IS NULL
        AND NOT se.id_finance_entity IS NULL
