WITH payments_in_retsuko AS (
  ...
  FROM (
    SELECT
      ...
      CAST(CASE
        WHEN LOCATE('|', i.payment_company_use_number) > 0
        THEN SUBSTRING_INDEX(i.payment_company_use_number, '|', -1)
        ELSE i.payment_company_use_number
      END AS STRING) AS company_use,
      i.paid_amount AS billing_amount,
      i.accrual_year_month,
      CAST(i.ts_paid AS DATE) AS dt_billing_source,
      dd.next_brz_fintech_business_day AS dt_billing,
      ...
      se.ts_created,
      se.ts_synced,
      ROW_NUMBER() OVER (PARTITION BY i.id_external, i.payment_company_use_number ORDER BY i.ts_created DESC) AS _w,
      i.id_external,
    ...
    WHERE
      NOT i.payment_company_use_number IS NULL
  ...
  SELECT
    CAST(SUBSTRING(UPPER(our_number), 1, LENGTH(our_number) - 1) AS INT) AS company_use,
  ...
  SELECT
    ...
    END AS company_use,
    CAST(ext.date_accounting AS DATE) AS dt_bank_paid,
    '45268-8' AS bank_account_number,
  ...
  WHERE
    ...
    AND CAST(ext.date_accounting AS DATE) >= CURRENT_DATE - 180
  UNION
  SELECT
  ...
  WHERE
    bank_account = '03922'
    AND occurrence_code = '06'
    AND NOT document_number IS NULL
  ...
  SELECT
    ...
    CAST(ts_paid AS DATE) AS dt_payment_source
  FROM datalake_checkout_clean.boleto
  WHERE
    id_requester = 5
  UNION
  SELECT
  ...
  INNER JOIN dw_collection_recovery_quintoandar.fact_negotiation_installment AS fni
  ...
  SELECT
    ...
    dt_sap_job_synced
  FROM datalake_accounting_funnel.erp_gateway_stages_status
  ...
  WHERE
    account_number IN ('11016X', '11004X', '11010X', '11036X', '11118X', '11035X', '11036X', '11035X', '11057X', '11307X')
  ...
  SELECT
    ...
    f.bank_amount,
    l.sap_amount,
    IF(NOT l.hash IS NULL, TRUE, FALSE) AS is_completeness_compliance,
    IF((
      ABS(r.billing_amount) - ABS(l.sap_amount) = 0
    ), TRUE, FALSE) AS is_correctness_compliance,
    IF(
      (
        l.dt_sap_reference BETWEEN r.dt_billing_source AND DATE_ADD(r.dt_billing, 3)
      ),
      TRUE,
      FALSE
    ) AS is_temporality_compliance,
    r.dt_billing,
    pcv.dt_payment_source,
    f.dt_bank_paid,
    l.dt_sap_created,
    l.dt_sap_reference
  FROM payments_in_retsuko AS r
  LEFT JOIN payments_vans_checkout AS pcv
    ON r.id_invoice = pcv.id_invoice
  LEFT JOIN francesinha AS f
    ON r.company_use = CAST(f.company_use AS STRING)
  LEFT JOIN payments_in_gateway AS g
    ON r.id_sap_gateway_feature = g.id_feature
  LEFT JOIN payments_in_ledger AS l
    ON g.hash = l.hash
)
SELECT
  id_business_entity,
  ...
  IF(
    is_completeness_compliance = TRUE
    AND is_correctness_compliance = TRUE
    AND is_temporality_compliance = TRUE,
    TRUE,
    FALSE
  ) AS is_compliance,
  dt_billing,
  ...
  dt_sap_created,
  dt_sap_reference
FROM create_compliance_columns