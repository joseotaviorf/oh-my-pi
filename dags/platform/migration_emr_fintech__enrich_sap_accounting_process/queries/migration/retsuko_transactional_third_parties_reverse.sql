WITH retsuko AS (
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY id_finance_entity, id_finance_entity_entry ORDER BY dt_created DESC) AS _w,
      dt_created
    FROM datalake_accounting_funnel.ledger
    WHERE
      dt_reference >= '2025-01-01' AND account_number IN (700004)
  ) AS _t
  WHERE
    _w = 1
), base AS (
  SELECT
    (
      'RE-RTSK-T-' || COALESCE(sl_hash.id_finance_entity, '') || '-' || COALESCE(sl_hash.account_number, '')
    ...
    CASE
      WHEN sl_hash.source_client <> 'seubarriga'
      THEN CONCAT('source', '-', sl_hash.source_client)
      WHEN r.id_finance_entity IS NULL
      AND se.id_sap_gateway_feature IS NULL
      AND sg.id_finance_entity IS NULL
      THEN 'manual transaction'
      WHEN r.id_finance_entity IS NULL
      AND se.id_sap_gateway_feature IS NULL
      AND NOT sg.id_finance_entity IS NULL
      THEN 'transaction missing in sap entity'
      WHEN r.id_finance_entity IS NULL
      AND NOT se.id_sap_gateway_feature IS NULL
      AND NOT sg.id_finance_entity IS NULL
      THEN 'wrong account number or postponed entry'
      ELSE NULL
  ...
  LEFT JOIN retsuko AS r
    ON (
      (
        COALESCE(sl_hash.id_finance_entity_entry, se.id_finance_entity) = r.id_finance_entity_entry
        AND (
          r.account_number = sl_hash.account_number
        )
      )
      OR (
        (
          sl_hash.id_finance_entity = r.id_finance_entity
        )
        AND (
          r.account_number = sl_hash.account_number
        )
...
FROM base
WHERE
  id_finance_entity_entry_r IS NULL
