WITH sap AS (
  ...
  WHERE
    ...
    AND NOT source_client IN ('rental-guarantee-pla')
), sap_gateway AS (
  SELECT
    id_finance_entity,
    id_feature,
  ...
  SELECT
    ...
    MIN(
      CASE
        ...
        AND r.id_sap_gateway_feature IS NULL
        ...
        WHEN r.id_finance_entity IS NULL
        AND NOT r.id_sap_gateway_feature IS NULL
        AND NOT sg.id_finance_entity IS NULL
        THEN 'wrong account number'
        ELSE NULL
      END
  ...
  WHERE
    r.id_finance_entity IS NULL AND sap_amount <> 0
  GROUP BY ALL
)
SELECT
