WITH sap_gateway AS (
  ...
  WHERE
    dt_reference >= CAST('2025-01-01' AS DATE) AND account_number IN ('420006')
  GROUP BY
    1,
    2,
    3,
...
SELECT
  (
    'KK-I-' || sl.id_transaction || '-' || COALESCE(sl.account_number, '')
  ...
  CASE
    WHEN r.id_finance_entity IS NULL AND sg.id_finance_entity IS NULL
    THEN 'manual transaction'
    WHEN r.id_finance_entity IS NULL AND NOT sg.id_finance_entity IS NULL
