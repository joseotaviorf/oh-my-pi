WITH aux_business_days AS (
  ...
  SELECT DISTINCT
    ...
    CASE
      ...
      AND LOWER(e.description) LIKE '%consultor imobiliário%'
      THEN '700007'
      WHEN e.bill_item IN ('entry.bill-item/adm-fee-adm-partner')
      THEN '700009'
      WHEN e.bill_item IN ('entry.bill-item/brokerage-partner-select')
      THEN '700010'
      WHEN e.bill_item IN ('entry.bill-item/brokerage-adm-partner')
      THEN '700011'
    ...
    CASE
      WHEN e.bill_item IN ('entry.bill-item/brokerage-estate-agent')
      THEN 'brokerage agent'
      WHEN e.bill_item IN ('entry.bill-item/brokerage-adm-partner')
      AND LOWER(e.description) LIKE '%consultor imobiliário%'
      THEN 'brokerage CIQ'
      WHEN e.bill_item IN ('entry.bill-item/adm-fee-adm-partner')
      THEN 'adm fee partner'
      WHEN e.bill_item IN ('entry.bill-item/brokerage-partner-select')
      THEN 'brokerage select'
      WHEN e.bill_item IN ('entry.bill-item/brokerage-adm-partner')
      THEN 'B2B brokerage partner'
    END AS accounting_name,
    i.accrual_year_month,
    CAST(e.ts_created AS DATE) AS dt_source_trigger,
    CASE
      WHEN e.bill_item IN ('entry.bill-item/adm-fee-adm-partner')
      THEN CAST(-ABS(amount) AS DECIMAL(12, 2))
      ELSE CAST(amount AS DECIMAL(12, 2))
    END AS source_amount
  FROM datalake_retsuko.entry AS e
  INNER JOIN datalake_retsuko_clean.account AS af
    ON e.id_from_account = af.id
  INNER JOIN datalake_retsuko_clean.account AS at
  ...
  LEFT JOIN datalake_retsuko_clean.contract AS ct
  ...
  WHERE
    ...
    AND CAST(e.ts_created AS DATE) >= '2024-01-01'
    AND af.type IN ('contract', 'tenant', 'landlord')
    AND at.type IN ('contract', 'tenant', 'landlord')
    AND (
      i.is_write_off = FALSE OR i.is_write_off IS NULL
    )
  ...
  SELECT DISTINCT
    ...
    'seu barriga' AS source_name,
    CASE
      WHEN e.bill_item IN ('entry.bill-item/brokerage-adm-partner', 'entry.bill-item/brokerage-partner-select', 'entry.bill-item/brokerage-estate-agent')
      THEN '700005'
      WHEN e.bill_item IN ('entry.bill-item/adm-fee-adm-partner')
      THEN '700008'
    END AS account_number,
    CASE
      WHEN e.bill_item IN ('entry.bill-item/brokerage-adm-partner', 'entry.bill-item/brokerage-partner-select', 'entry.bill-item/brokerage-estate-agent')
      THEN 'brokerage partners'
      WHEN e.bill_item IN ('entry.bill-item/adm-fee-adm-partner')
      THEN 'B2B adm partner'
    END AS accounting_name,
    i.accrual_year_month,
    CAST(e.ts_created AS DATE) AS dt_source_trigger,
  ...
  FROM (
    SELECT
      ...
      failed_status,
      failed_reason,
      ROW_NUMBER() OVER (PARTITION BY id_finance_entity, event ORDER BY ts_updated DESC) AS _w,
      ts_updated
    FROM datalake_retsuko_clean.sap_entity
    WHERE
      NOT id_finance_entity IS NULL
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY f.id_finance_entity, f.id_feature ORDER BY f.ts_updated) AS _w,
      f.ts_updated
    ...
    WHERE
      ...
      AND CAST(f.ts_created AS DATE) >= CAST('2024-01-01' AS DATE)
  ) AS _t
  WHERE
    _w = 1
), sap_ledger AS (
  SELECT
    ...
    CAST(dt_created AS DATE) AS dt_sap_created,
    CAST(dt_reference AS DATE) AS dt_sap_reference
  FROM datalake_pas.ledger
  WHERE
    dt_reference >= CAST('2024-01-01' AS DATE)
  ...
  SELECT
    ...
    MIN(
      CASE
        WHEN NOT sl.hash IS NULL
        THEN 'success'
        WHEN sl.hash IS NULL
        AND (
          se.id_finance_entity IS NULL
          OR se.status = 'failed'
          OR NOT se.failed_reason IS NULL
        )
        THEN 'source failure'
        WHEN sl.hash IS NULL
        AND (
          sg.id_feature IS NULL
          OR sg.sync_sap_job_status = 'error'
          OR NOT sg.webhook_error IS NULL
        )
        THEN 'gateway failure'
        ELSE 'unknown failure'
      END
    ) AS accounting_process_status,
    MIN(
      CASE
        WHEN sl.hash IS NULL AND se.id_finance_entity IS NULL
        THEN 'source not found'
        WHEN sl.hash IS NULL AND se.status = 'failed'
        THEN se.failed_reason
        WHEN sl.hash IS NULL AND sg.id_feature IS NULL
        THEN 'gateway not found'
        WHEN sl.hash IS NULL AND sg.sync_sap_job_status = 'error'
        THEN sg.webhook_error
        WHEN sl.hash IS NULL AND NOT se.id_finance_entity IS NULL AND NOT sg.id_feature IS NULL
        THEN 'sap not found'
        ELSE NULL
      END
  ...
  FROM retsuko_final AS r
  LEFT JOIN sap_entity AS se
    ON r.id_finance_entity_entry = se.id_finance_entity
  LEFT JOIN sap_gateway AS sg
    ON se.id_sap_gateway_feature = sg.id_feature
  LEFT JOIN sap_ledger AS sl
    ON r.account_number = sl.account_number AND sl.hash = sg.hash
  GROUP BY
    1,
    2,
    3,
  ...
  SELECT
    (
      'RTSK-RRS' || '-' || COALESCE(id_finance_entity_entry, id_finance_entity) || '-' || COALESCE(account_number, '')
    ) AS id_accounting_process,
    id_business_entity,
    id_finance_entity,
    id_finance_entity_entry,
    version,
    ...
    IF(
      (
        ABS(source_amount) - ABS(sap_amount)
      ) >= 0.05
      OR (
        ABS(source_amount) - ABS(sap_amount)
      ) <= -0.05
    ...
    CASE
      WHEN dt_sap_reference IS NULL OR dt_source_trigger IS NULL
      THEN FALSE
      WHEN TRUNC(CAST(dt_source_trigger AS DATE), 'MM') = TRUNC(CAST(dt_sap_reference AS DATE), 'MM')
      THEN TRUE
      WHEN TRUNC(CAST(dt_sap_reference AS DATE), 'MM') = ADD_MONTHS(TRUNC(CAST(dt_source_trigger AS DATE), 'MM'), 1)
      AND (
        SELECT
          COUNT(*)
        FROM aux_business_days AS ad
        WHERE
...
SELECT
  ...
  is_temporality,
  IF(
    is_completeness IS TRUE AND is_correctness IS TRUE AND is_temporality IS TRUE,
    TRUE,
    FALSE
  ) AS is_compliance,
  accounting_process_status,
