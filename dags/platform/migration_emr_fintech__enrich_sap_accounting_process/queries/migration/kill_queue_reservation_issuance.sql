WITH sap_gateway AS (
  ...
  FROM (
    SELECT
      ...
      s.status,
      s.hash,
      RANK() OVER (PARTITION BY f.id_source ORDER BY s.ts_updated DESC) AS _w,
      s.ts_updated
    ...
    WHERE
      f.source = 'kill-queue/reservation' AND s.type = 'NF'
  ) AS _t
  WHERE
    _w = 1
), sap AS (
  SELECT
    id_finance_entity,
    accrual_year_month,
    CASE WHEN account_number = '420006' THEN 'reservation fee' END AS accounting_name,
    account_number,
    MAX(CAST(dt_created AS DATE)) AS dt_sap_created,
    MAX(CAST(dt_reference AS DATE)) AS dt_sap_reference,
    CAST(SUM(debit_credit) AS DECIMAL(12, 2)) AS sap_amount
  FROM datalake_accounting_funnel.ledger
  WHERE
    account_number = '420006' AND dt_reference >= '2024-01-01'
  GROUP BY
    1,
    2,
    3,
  ...
  FROM (
    SELECT
      ...
      CAST(r.ts_created AS DATE) AS dt_source_trigger,
      r.ts_updated,
      RANK() OVER (PARTITION BY id ORDER BY ts_updated DESC) AS _w,
      id
    FROM datalake_kill_queue_clean.reservation AS r
    WHERE
      r.status IN ('FINISHED', 'CANCELED') AND r.ts_created >= '2024-01-01'
  ) AS _t
  WHERE
    _w = 1
), df AS (
  SELECT
    ...
    CASE
      WHEN NOT s.id_finance_entity IS NULL
      THEN 'SUCCESS'
      WHEN s.id_finance_entity IS NULL AND NOT sg.id_source IS NULL
      THEN 'SG FAILURE'
      WHEN s.id_finance_entity IS NULL AND sg.id_source IS NULL
      THEN 'SOURCE FAILURE'
    END AS status,
  ...
  SELECT
    (
      'KK-I' || '-' || COALESCE(df.id_finance_entity, df.id_business_entity || df.accrual_year_month) || '-' || COALESCE(df.account_number, '')
    ...
    IF(
      (
        ABS(df.source_amount) - ABS(df.sap_amount)
      ) >= 0.05
      OR (
        ABS(df.source_amount) - ABS(df.sap_amount)
      ) <= -0.05
      ...
      FALSE,
      TRUE
    ) AS is_correctness_compliance,
    IF(
      df.dt_sap_reference BETWEEN df.dt_source_trigger AND DATE_ADD(df.dt_source_trigger, 30),
      TRUE,
      FALSE
...
SELECT DISTINCT
  ...
  IF(
    is_completeness_compliance IS TRUE
    AND is_correctness_compliance IS TRUE
    AND is_temporality_compliance IS TRUE,
    TRUE,
    FALSE
  ) AS is_compliance,
  dt_source_trigger,
