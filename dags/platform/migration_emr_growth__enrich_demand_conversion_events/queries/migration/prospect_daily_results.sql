WITH prospect_status_events AS (
  SELECT
    ...
    tps.month,
    tps.day
  FROM datalake_demand_flows.tenant_prospect_status AS tps
  WHERE
    CAST(tps.ts_status_started AS DATE) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
  UNION ALL
  SELECT
  ...
  WHERE
    CAST(bps.ts_status_started AS DATE) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
), prospect_results AS (
  SELECT
    ...
    pse.month,
    pse.day
  FROM prospect_status_events AS pse
  LEFT JOIN datalake_demand_flows.prospect_results AS pcr
  ...
  FROM (
    SELECT
      ...
      pcr.month,
      pcr.day,
      ROW_NUMBER() OVER (PARTITION BY id_demand_prospect_conversion_event ORDER BY ts_event ASC) AS _w
    FROM datalake_demand_flows.prospect_results AS pcr
    WHERE
      CAST(pcr.ts_event AS DATE) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
  ) AS _t
  WHERE
    _w = 1
)
SELECT
  ...
  entrance_uri,
  is_cqa_demand,
  CAST(DATE_FORMAT(ts_event, 'yyyyMMdd') AS INT) AS id_event_date,
  ts_event,
  year,
  month,
  day
