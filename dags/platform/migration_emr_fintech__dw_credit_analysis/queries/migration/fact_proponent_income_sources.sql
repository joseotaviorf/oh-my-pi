WITH get_credit_evaluations_count AS (
  ...
  FROM datalake_docx_clean.credit_evaluation_proponent AS cep
  LEFT JOIN datalake_sorting_hat_clean.proponent AS pt
    ON pt.cpf = cep.cpf
  LEFT JOIN datalake_docx_clean.credit_evaluation AS ce
    ON ce.id = cep.id_credit_evaluation AND ce.id_proposal = pt.id_proposal
  LEFT JOIN get_credit_evaluations_count AS cec
    ON cec.id_proposal = ce.id_proposal
  /* Get the latest credit evaluation */
  WHERE
    ce.ts_created = cec.ts_max_created
    AND /* Remove records with null proponent_type (legacy data) */ NOT cep.proponent_type IS NULL
  ...
  FROM (
    SELECT DISTINCT
      ...
      ROW_NUMBER() OVER (PARTITION BY pd.id_context_external, pd.cpf ORDER BY pd.ts_updated DESC) AS _w,
      pd.id_context_external,
      pd.cpf,
      pd.ts_updated
    FROM datalake_docx.personal_documentation AS pd
    LEFT JOIN datalake_docx.income_documentation AS id
      ON id.id_folder = pd.id_folder AND id.id_context_external = pd.id_context_external
    WHERE
      pd.document_context /* Get only tenant's data */ = 'Tenant'
  ) AS _t
  WHERE
    _w = 1
  ...
  FROM datalake_arquivo_confidencial_clean.presumed_income_report
  WHERE
    source = 'NEOWAY'
  ...
  SELECT DISTINCT
    COALESCE(ni.cpf, ti.cpf) AS proponent_cpf,
    ni.presumed_income_neoway,
    ti.presumed_income_transunion
  FROM get_neoway_presumed_income AS ni
  LEFT JOIN get_transunion_presumed_income AS ti
    ON ti.cpf = ni.cpf
  ...
  FROM proponent_incomes AS pi
  LEFT JOIN get_proponent_income_data AS pid
    ON pid.proponent_cpf = pi.proponent_cpf AND pid.id_proposal = pi.id_proposal
  LEFT JOIN join_presumed_income AS pib
  ...
  FROM get_calculated_income AS ci
  LEFT JOIN sum_proposal_incomes AS pi
  ...
  SELECT DISTINCT
    ...
    IF(
      ei.ratio_calculated_income >= 0.8,
      ei.sum_proponent_gross_income,
      ei.calculated_income
    ) AS proposal_elected_income,
    pis.resident AS is_resident,
    pis.total_credit_evaluations,
    pis.is_main_user,
    pis.ts_last_credit_evaluation,
    NOW() AS ts_load
  FROM join_income_sources AS pis
  LEFT JOIN get_elected_income AS ei
