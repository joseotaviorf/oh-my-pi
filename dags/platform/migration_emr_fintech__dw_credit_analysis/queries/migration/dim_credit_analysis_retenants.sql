WITH tenant_contract AS (
  ...
  FROM datalake_ebdb_contract.contract_person AS cp
  INNER JOIN datalake_ebdb_contract.contract AS dc
    ON dc.id = cp.id_contract AND dc.status <> 'Cancelado'
  INNER JOIN dw_rent.fact_listing_rent_flows AS flrf
    ON dc.id = flrf.sk_contract
  WHERE
    cp.contract_role IN ('tenant') AND NOT cp.id_user_contract_person IS NULL
), proposal_tenant AS (
  SELECT
    pl.id AS id_proposal,
    pl.ts_created AS proposal_ts_created,
    pt.cpf AS proposal_cpf
  FROM datalake_sorting_hat_clean.proposal AS pl
  INNER JOIN datalake_sorting_hat_clean.proponent AS pt
  ...
  FROM (
    SELECT
      ...
      tc.sk_client_flrf,
      ROW_NUMBER() OVER (PARTITION BY plt.id_proposal, plt.proposal_cpf ORDER BY tc.contract_dt_start DESC) AS _w
    FROM proposal_tenant AS plt
    INNER JOIN tenant_contract AS tc
  ...
  SELECT
    ...
    tc.contract_id_proposal,
    tc.sk_client_flrf,
    CASE WHEN pt.proposal_cpf = tc.contract_cpf THEN 1 ELSE 0 END AS equal_cpf_sk_personal_document,
    CASE
      WHEN pt.proposal_cpf = tc.contract_cpf AND tc.sk_user_fcp = tc.sk_client_flrf
      THEN 1
      ELSE 0
    END AS equal_main_contract_proposal
  FROM proposal_tenant AS pt
  LEFT JOIN proposal_last_contract AS plc
    ON plc.id_proposal = pt.id_proposal AND pt.proposal_cpf = plc.proposal_cpf
  FULL OUTER JOIN tenant_contract AS tc
    ON plc.id_contract = tc.id_contract AND plc.proposal_cpf = tc.contract_cpf
  WHERE
    NOT pt.id_proposal IS NULL
), proposal_retenant_count_p AS (
  SELECT
    id_proposal,
    COUNT(proposal_cpf) AS count_cpf_proposal,
    COUNT(DISTINCT proposal_cpf) AS distinct_cpf_proposal
  FROM proposal_retenant
  GROUP BY
    id_proposal
  ...
  FROM proposal_retenant_count_p AS prcp
  LEFT JOIN proposal_retenant_count_c AS prcc
  ...
  SELECT
    ...
    CASE
      WHEN prc.contract_dt_annulment < prc.proposal_ts_created
      THEN TIMESTAMPDIFF(
        MONTH,
        TO_DATE(DATE_TRUNC('MONTH', prc.contract_dt_annulment)),
        TO_DATE(DATE_TRUNC('MONTH', CAST(prc.proposal_ts_created AS DATE)))
      )
      ELSE NULL
    END AS contract_age,
    (
      CASE WHEN prc.count_cpf_contract <> 0 THEN 'retenant' ELSE 'newcustomer' END
    ) AS any_proponent,
    (
      CASE
        WHEN prc.is_main_proponent_contract_proposal <> 0 AND prc.count_cpf_contract <> 0
        THEN 'retenant'
        ELSE 'newcustomer'
      END
    ) AS main_proponent,
    ...
    CASE
      WHEN CAST(p.ts_created AS DATE) > '2023-12-05'
      THEN 'new_retenant_policy'
  ...
  FROM proposal_retenant_count AS prc
  LEFT JOIN datalake_sorting_hat_clean.proposal AS p
  ...
  FROM datalake_sorting_hat.policy_report
  ...
  FROM (
    SELECT
      ...
      CASE
        ...
        WHEN pcl.main_proponent = 'retenant' AND pcl.equal_proponents_in_proposal = 'retenant'
      ...
      NOW() AS ts_load,
      ROW_NUMBER() OVER (PARTITION BY pcl.sk_credit_analysis ORDER BY pcl.ts_last_contract_created DESC) AS _w
    FROM proposal_contract_label AS pcl
    LEFT JOIN get_active_contract_per_credit_analysis AS cpca
      ON cpca.sk_credit_analysis = pcl.sk_credit_analysis
      AND NOT pcl.sk_credit_analysis IS NULL
  ) AS _t
  WHERE
    _w = 1
)
SELECT
  ...
  CASE
    WHEN NOT pr.is_retenant_policy_report IS NULL
    THEN pr.is_retenant_policy_report
    ELSE ht.is_retenant
  END AS is_retenant,
  ...
  ht.ts_load
FROM add_retenant_historical_tags AS ht
LEFT JOIN get_retenant_policy_report AS pr
