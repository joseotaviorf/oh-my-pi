WITH trato_feito_negotiation AS (
  SELECT
    id_negotiation,
  ...
  FROM (
    SELECT
      ...
      n.id_contract,
      REGEXP_REPLACE(cl.document, '\\.|\\-', '') AS id_customer,
      CASE
        WHEN LOWER(n.collector) LIKE '%cyber%'
        AND BIGINT(n.id_negotiation_external) < 10000000
        THEN 'Trato Feito - Cyber (Migração)'
        WHEN LOWER(n.collector) LIKE '%cyber%'
        AND BIGINT(n.id_negotiation_external) >= 10000000
        THEN 'Trato Feito - Cyber'
        WHEN LOWER(n.collector) LIKE '%recupera%'
        THEN 'Trato Feito - Recupera'
        WHEN UPPER(n.collector) LIKE '%5A%'
        THEN 'Trato Feito - Self Service'
      END AS source,
      COUNT(n.id_contract) OVER (PARTITION BY n.id_negotiation_external) AS contracts_by_negotiation,
      CASE
        WHEN n.debtor = 'rental_contract_landlord'
        THEN 'PP QuintoAndar'
        WHEN n.debtor = 'rental_contract_tenant'
        THEN 'IQ QuintoAndar'
      END AS creditor,
      n.consultancy_name AS advisory,
      n.contact_type,
      CASE
        WHEN UPPER(n.contact_type) IN ('IAPORTAL', 'IAWPP')
        THEN 'Matthew'
        WHEN n.consultancy_name = 'PORTAL_QUINTOANDAR'
        THEN 'Portal Auto Negociação'
        WHEN UPPER(n.consultancy_name) IN ('PASCHOALOTTO', 'MEETCALL', 'TRC', 'GRB', CONCAT('BULGARELLI', 'MONEST'), 'PELLON', 'PLC', 'GONDIM', 'NOVAQUEST')
        THEN 'Assessoria'
        WHEN n.consultancy_name = 'SERASA'
        THEN 'Serasa Digital'
        WHEN n.consultancy_name = 'COBRANÇA_INTERNA_QA'
      ...
      n.paid_amount,
      CAST(n.qt_installments_paid AS INT) AS paid_installments,
      IF(
        NOT n.breached_installment IS NULL,
        CAST(n.qt_installments AS INT) - CAST(n.qt_installments_paid AS INT),
        0
      ) AS breached_installments,
      n.dt_expected_end,
      CAST(n.ts_created_at AS DATE) AS dt_promisse,
      CAST(n.ts_first_payment AS DATE) AS dt_down_payment,
      CAST(n.ts_paid_all AS DATE) AS dt_paid_all,
      CAST(n.ts_breach AS DATE) AS dt_cancellation,
      CAST(COALESCE(n.ts_breach, n.ts_paid_all) AS DATE) AS dt_ending,
      n.ts_created_at AS ts_promisse,
      ROW_NUMBER() OVER (PARTITION BY n.id_contract, n.id_negotiation_external ORDER BY n.ts_created_at DESC, cl.ts_created DESC) AS _w,
  ...
  WHERE
    _w = 1 /* removes the exception in which 1 Trato-Feito negotiation ID has more than one Recupera negotiation ID. Ex: 97080, and when 1 contract has more then 1 main-tenant living in the house. */
  ...
  SELECT
    ...
    CASE
      ...
      THEN discount_amount - (
        discount_to_original_amount + discount_to_fees_amount + discount_to_fine_amount + discount_to_eviction_costs
      )
      ELSE 0
  ...
  FROM (
    SELECT
      ...
      CASE WHEN rn.id_creditor IN (2, 6) THEN 'PP QuintoAndar' ELSE 'IQ QuintoAndar' END AS creditor,
      CASE
        WHEN rn.negotiation_status = 'ACORDO_LIQUIDADO'
        THEN 'finished'
        WHEN rn.negotiation_status = 'ACORDO_CANCELADO' AND rn.down_payment IS TRUE
        THEN 'broken'
        WHEN rn.negotiation_status = 'ACORDO_CANCELADO' AND rn.down_payment IS FALSE
        THEN 'canceled'
        WHEN rn.negotiation_status = 'ACORDO_EM_ANDAMENTO' AND rn.down_payment IS TRUE
        THEN 'offset'
      ...
      CASE
        WHEN REPLACE(rn.origin_agreement, '_', ' ') = 'Portal Autonegociação'
        THEN 'Portal Auto Negociação'
        WHEN REPLACE(rn.origin_agreement, '_', ' ') = 'Operador'
        THEN 'Operador Interno'
        WHEN REPLACE(rn.origin_agreement, '_', ' ') = 'Carta Campanha'
        THEN 'Boletagem'
      ...
      IF(
        rn.is_special_installment IS TRUE,
        rn.fine_fee_amount,
        GREATEST(
          rn.expense_amount - rn.original_debt_amount - rn.interest_fee_amount - rn.adm_fee_amount - rn.negotiation_discount_amount,
          0
        )
      ) AS fine_amount,
      ...
      'Recupera' AS source,
      2 AS priority,
      ROW_NUMBER() OVER (PARTITION BY CAST(rn.id_negotiation AS BIGINT) ORDER BY rn.ts_snapshot DESC, COALESCE(n.id_contract, rn.id_contract) DESC) AS _w,
      rn.ts_snapshot
    FROM datalake_recupera.negotiation AS rn
    LEFT JOIN trato_feito_negotiation AS n
      ON CAST(rn.id_negotiation AS BIGINT) = n.id_negotiation
      AND n.source = 'Trato Feito - Recupera'
    WHERE
      NOT id_creditor IN (3, 5)
  ) AS _t
  WHERE
    _w = 1
)
SELECT DISTINCT
  ...
  UPPER(
    COALESCE(tfn.promisse_payment_method, cn.promisse_payment_method, rn.promisse_payment_method)
  ) AS promisse_payment_method,
  cn.payment_method,
  CASE
    WHEN COALESCE(
      tfn.contracts_by_negotiation,
      cn.contracts_by_negotiation,
      rn.contracts_by_negotiation
    ) > 1
    THEN TRUE
    WHEN NOT COALESCE(cn.exception, rn.exception) IS NULL
    THEN TRUE
    ELSE FALSE
...
FULL OUTER JOIN cyber_negotiation AS cn
  ON tfn.id_negotiation = cn.id_negotiation AND tfn.id_contract = cn.id_contract
FULL OUTER JOIN recupera_negotiation AS rn
  ON tfn.id_negotiation = rn.id_negotiation AND tfn.id_contract = rn.id_contract
