WITH deduplicate_trato_feito_negotiation AS (
  SELECT DISTINCT
    id_contract,
    id_negotiation,
    qt_installments AS number_of_installments,
    COALESCE(CAST(id_negotiation_external AS BIGINT), id_negotiation_external) AS id_negotiation_external,
    CASE
    ...
    END AS creditor,
    CASE
      WHEN LOWER(collector) LIKE '%cyber%' AND BIGINT(id_negotiation_external) < 10000000
      THEN 'Trato Feito - Cyber (Migração)'
      WHEN LOWER(collector) LIKE '%cyber%' AND BIGINT(id_negotiation_external) >= 10000000
      THEN 'Trato Feito - Cyber'
      WHEN LOWER(collector) LIKE '%recupera%'
      THEN 'Trato Feito - Recupera'
      WHEN UPPER(collector) LIKE '%5A%'
  ...
  SELECT
    CONCAT(n.id_negotiation_external, LPAD(CAST(i.installment_number AS INT), 3, '0')) AS id_negotiation_installment,
    n.id_contract,
    ...
    i.debts_fee_amount,
    COALESCE(i.adm_fee_amount, 0) AS credit_card_fee_amount,
    i.installment_costs,
    i.installment_lawyers_fee,
    i.discount_amount,
    i.total_amount AS amount_to_pay,
    IF(NOT i.ts_paid IS NULL, i.total_amount, NULL) AS paid_amount,
    CAST(i.ts_created AS DATE) AS dt_creation,
    i.dt_due,
    CAST(i.ts_paid AS DATE) AS dt_paid,
    i.dt_canceled,
    n.source
  FROM datalake_debt_recovery.installment AS i
  INNER JOIN deduplicate_trato_feito_negotiation AS n
    ON i.id_negotiation = n.id_negotiation
), tf_negotiation_with_all_amounts_in_first_installment AS (
  SELECT
    id_negotiation_external,
    number_of_installments,
    SUM(debts_fee_amount) FILTER(WHERE
      installment_number = 1) AS debts_fee_amount_first_installment,
    SUM(debts_fee_amount) FILTER(WHERE
      installment_number > 1) AS debts_fee_amount_other_installments,
    SUM(discount_amount) FILTER(WHERE
      installment_number = 1) AS discount_amount_first_installment,
    SUM(discount_amount) FILTER(WHERE
      installment_number > 1) AS discount_amount_other_installments
  FROM trato_feito_installment
  WHERE
  ...
  HAVING
    (
      debts_fee_amount_first_installment > 0
      AND debts_fee_amount_other_installments = 0
    )
    OR (
      discount_amount_first_installment > 0 AND discount_amount_other_installments = 0
    )
), tf_divide_amount_by_installments AS (
  SELECT
    id_negotiation_external,
    FLOOR(debts_fee_amount_first_installment / number_of_installments, 2) AS debt_fees,
    debts_fee_amount_first_installment - FLOOR(debts_fee_amount_first_installment / number_of_installments, 2) * (
      number_of_installments - 1
    ) AS last_debt_fees,
    FLOOR(discount_amount_first_installment / number_of_installments, 2) AS discount,
    discount_amount_first_installment - FLOOR(discount_amount_first_installment / number_of_installments, 2) * (
      number_of_installments - 1
    ) AS last_discount
  FROM tf_negotiation_with_all_amounts_in_first_installment
  ...
  SELECT
    ...
    CASE
      ...
      AND i.installment_number = i.number_of_installments
      THEN f.last_debt_fees
      WHEN NOT f.id_negotiation_external IS NULL
      AND i.installment_number <> i.number_of_installments
      THEN f.debt_fees
    ...
    CASE
      WHEN NOT f.id_negotiation_external IS NULL
      AND i.installment_number = i.number_of_installments
      THEN f.last_discount
      WHEN NOT f.id_negotiation_external IS NULL
      AND i.installment_number <> i.number_of_installments
      THEN f.discount
      ELSE i.discount_amount
    END AS discount_amount
  FROM trato_feito_installment AS i
  LEFT JOIN tf_divide_amount_by_installments AS f
  ...
  SELECT DISTINCT
    ...
    'Cyber' AS source
  FROM datalake_cyber.installment AS i
  WHERE
    creditor = 'QuintoAndar'
), recupera_installments AS (
  SELECT DISTINCT
    CONCAT(
      CAST(ri.id_negotiation AS BIGINT),
      LPAD(CAST(ri.installment_number AS INT), 3, '0')
    ) AS id_negotiation_installment,
    ...
    ri.creditor,
    CASE
      WHEN ri.installment_status = 'Pago'
      THEN 'paid'
      WHEN ri.installment_status = 'Quebrado'
      THEN 'canceled'
      WHEN ri.installment_status = 'Em aberto'
      THEN 'pending'
    ...
    ri.dt_canceled,
    'Recupera' AS source
  FROM datalake_recupera.installment AS ri
  LEFT JOIN tf_all_fields_fix AS tf
    ON tf.id_negotiation_installment = CONCAT(
      CAST(ri.id_negotiation AS BIGINT),
      LPAD(CAST(ri.installment_number AS INT), 3, '0')
    )
    AND tf.source = 'Trato Feito - Recupera'
  WHERE
    NOT LOWER(ri.creditor) LIKE '%quintocred%'
)
SELECT
  COALESCE(
    tf.id_negotiation_installment,
    ci.id_negotiation_installment,
    ri.id_negotiation_installment
  ) AS id_negotiation_installment,
