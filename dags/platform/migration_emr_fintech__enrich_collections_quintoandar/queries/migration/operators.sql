WITH union_all_sources AS (
  SELECT
    ...
    CASE
      WHEN UPPER(id_operator_registration) = 'PASCHOAL'
      THEN 'PASCHOALOTTO'
      WHEN UPPER(id_operator_registration) LIKE '%PORTAL%'
      THEN 'PORTAL_QUINTOANDAR'
      WHEN LOWER(operator_login_code) LIKE '%@quintoandar.com.br'
      THEN 'COBRANÇA_INTERNA'
      WHEN UPPER(id_operator_registration) = 'QUINTO'
      THEN 'COBRANÇA_INTERNA'
      WHEN LOWER(operator_login_code) LIKE '%@sysopen.com.br'
      THEN 'SYSOPEN'
      WHEN UPPER(id_operator_registration) LIKE 'SERASA%'
      THEN 'SERASA'
  ...
  FROM datalake_recupera_clean.operators
  UNION ALL
  SELECT DISTINCT
  ...
  SELECT
    ...
    3 AS priority,
    NULL AS ts_user_created
  FROM datalake_webhelp_clean.login
  WHERE
    context = 'COBRANCA'
  UNION ALL
  SELECT DISTINCT
  ...
  SELECT DISTINCT
    ...
    'BULGARELLI' AS source,
    3 AS priority,
    CAST(dt_admission AS TIMESTAMP) AS ts_user_created
  FROM datalake_grb_clean.operators
  UNION ALL
  SELECT DISTINCT
    CASE
      WHEN NOT TRY_CAST(id_operator_cyber AS INT) IS NULL
...
FROM (
  SELECT DISTINCT
    ...
    ts_user_created,
    NOW() AS ts_load,
    ROW_NUMBER() OVER (PARTITION BY id_operator ORDER BY priority, ts_user_created DESC) AS _w,
    priority
