WITH base AS (
  ...
  WHERE
    i.due_amount < 0 AND c.country_code = 'BR' AND ii.invoice_user = 'tenant'
  ...
  FROM (
    SELECT
      ...
      n.dt_promisse,
      ROW_NUMBER() OVER (PARTITION BY dn.id_invoice ORDER BY n.dt_promisse DESC) AS _w
    FROM datalake_collections_quintoandar.debt_negotiated AS dn
    LEFT JOIN datalake_collections_quintoandar.negotiation AS n
      ON dn.id_negotiation = n.id_negotiation AND dn.id_contract = n.id_contract
    WHERE
      NOT n.dt_down_payment IS NULL
  ...
  FROM (
    SELECT
      ...
      CAST(i.ts_due AS DATE) AS dt_due_parent,
      i.dt_due_adjusted AS dt_due_adjusted_parent,
      n.dt_promisse,
      ROW_NUMBER() OVER (PARTITION BY ni.id_invoice_extra ORDER BY CAST(i.ts_due AS DATE), ni.id_invoice_extra) AS _w,
      ni.id_invoice_extra,
      i.ts_due
    FROM datalake_collections_quintoandar.installment AS ni
    LEFT JOIN datalake_collections_quintoandar.negotiation AS n
      ON ni.id_negotiation = n.id_negotiation AND ni.id_contract = n.id_contract
    LEFT JOIN datalake_collections_quintoandar.debt_negotiated AS d
      ON d.id_negotiation = ni.id_negotiation AND d.id_contract = ni.id_contract
    LEFT JOIN datalake_retsuko.invoice AS i
      ON d.id_invoice = i.id_external AND d.id_contract = i.id_contract_external
  ) AS _t
  WHERE
    _w = 1
), base_negotiation AS (
  ...
  WHERE
    NOT parent.id_negotiation_parent IS NULL
    OR NOT child.id_negotiation_child IS NULL
), contract_write_off AS (
  SELECT DISTINCT
    id_contract_external
  FROM base
  WHERE
    is_write_off IS TRUE
), first_payment AS (
  SELECT
    id_contract_external,
    id_external,
  ...
  FROM base
  WHERE
    LOWER(status) <> 'canceled' AND LOWER(contract_status) <> 'cancelado'
  ...
  SELECT
    ...
    IF(NOT bn.id_negotiation_parent IS NULL, TRUE, FALSE) AS is_child_negotiation,
    IF(NOT bn.id_negotiation_child IS NULL, TRUE, FALSE) AS has_child,
    bn.child_negotiation_agency,
    IF(NOT ssn.id_invoice IS NULL, TRUE, FALSE) AS has_app_action_event,
    b.id_account,
    b.id_audit,
    b.id_checkout_order,
    ...
    CASE
      WHEN b.status = 'paid'
      AND NOT ssn.id_invoice IS NULL
      AND NOT bn.id_negotiation_parent IS NULL
      ...
      AND (
        NOT bn.id_negotiation_parent IS NULL
        OR (
          b.purpose = 'extra'
          AND (
            b.reason LIKE '%negotiation%' OR b.reason LIKE '%agreement%'
          )
        )
      )
      THEN 'Paid Installment outside App'
      WHEN b.status = 'paid' AND ssn.id_invoice IS NULL AND bn.id_negotiation_parent IS NULL
      THEN 'Paid outside App'
      WHEN b.status = 'written-down' AND bn.id_negotiation_child IS NULL
      THEN 'Manual Written Down'
      WHEN b.status = 'written-down'
      AND NOT bn.id_negotiation_child IS NULL
      AND NOT bn.id_negotiation_parent IS NULL
      ...
      WHEN b.status = 'written-down' AND NOT bn.id_negotiation_child IS NULL
    ...
    COALESCE(b.is_write_off, FALSE) AS is_write_off,
    IF(NOT cwo.id_contract_external IS NULL, TRUE, FALSE) AS is_contract_write_off,
    COALESCE(fp.is_first_payment, FALSE) AS is_first_payment,
    COALESCE(fic.is_first_invoice_contract, FALSE) AS is_first_invoice_contract,
    b.dt_due_adjusted,
  ...
  LEFT JOIN base_negotiation AS bn
    ON bn.id_invoice = b.id_external AND bn.id_contract = b.id_contract_external
  LEFT JOIN paid_by_ssn AS ssn
    ON ssn.id_invoice = b.id_external
    AND ssn.id_contract = b.id_contract_external
    AND CAST(ssn.ts_event AS DATE) <= CAST(b.ts_paid AS DATE)
    AND CAST(ssn.ts_event AS DATE) >= CAST(b.ts_paid AS DATE) - INTERVAL '5' DAY
...
SELECT
  ...
  recovery_channel,
  CASE
    WHEN recovery_channel IN ('Negotiation - Advisory', 'Paid Installment in App', 'Paid Installment outside App')
    THEN 'BPO'
    WHEN recovery_channel IN ('Negotiation - SSN', 'Paid in App', 'Paid outside App', 'Negotiation - Matthew', 'Negotiation of Installment - SSN')
    THEN 'DIGITAL'
    WHEN recovery_channel IN ('Negotiation - Serasa')
    THEN 'DIGITAL EXTERNAL PARTNERS'
