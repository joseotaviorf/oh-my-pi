WITH deduplicate_creditor_pending AS (
  ...
  FROM (
    SELECT
      ...
      1 AS priority,
      ROW_NUMBER() OVER (PARTITION BY n.id_contract, d.id_external, COALESCE(CAST(n.id_negotiation_external AS BIGINT), n.id_negotiation_external) ORDER BY d.ts_created DESC) AS _w,
      d.id_external,
      d.ts_created
    ...
    WHERE
      NOT d.id_negotiation IS NULL AND n.debtor <> 'velo_delinquency_tenant'
  ) AS _t
  WHERE
    _w = 1
), cyber_debts AS (
  ...
  SELECT
    COALESCE(t.id_contract, c.id_contract, r.id_contract) AS id_contract,
    COALESCE(t.id_invoice, c.id_invoice, r.id_invoice) AS id_invoice,
    CAST(COALESCE(t.id_negotiation, c.id_negotiation, r.id_negotiation) AS STRING) AS id_negotiation,
...
FROM (
  SELECT
    ...
    source,
    NOW() AS ts_load,
    ROW_NUMBER() OVER (PARTITION BY id_contract, id_invoice, id_negotiation ORDER BY priority) AS _w,
    priority
  FROM union_sources
) AS _t
WHERE
