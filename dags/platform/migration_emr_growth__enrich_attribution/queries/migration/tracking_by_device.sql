WITH tracking AS (
  SELECT
    ...
    FROM_UNIXTIME(GET_JSON_OBJECT(user_properties, '$.egw_referrer_ts') / 1000) AS ts_referral_attribution,
    CASE
      WHEN ts_utm_attribution >= ts_referral_attribution
      THEN egw_utm_medium
      WHEN ts_referral_attribution IS NULL
      THEN egw_utm_medium
      WHEN EXISTS(('%google%', '%bing%', '%yahoo%'), x -> referrer_domain LIKE x)
      THEN 'seo'
      WHEN EXISTS(
        ('%chatgpt.%', '%gemini.%', '%copilot.%', '%perplexity.%'),
        x -> referrer_domain LIKE x
      )
      THEN 'llm'
      WHEN EXISTS(
        ('%claude.%', '%you.com%', '%poe.com%', '%deepseek.%'),
        x -> referrer_domain LIKE x
      )
      THEN 'other_llm'
    END AS utm_medium, /* UTM Medium */
    CASE
      WHEN utm_medium = 'seo' AND referrer_domain LIKE '%google%'
      THEN 'google'
      WHEN utm_medium = 'seo' AND referrer_domain LIKE '%bing%'
      THEN 'bing'
      WHEN utm_medium = 'seo' AND referrer_domain LIKE '%yahoo%'
      THEN 'yahoo'
      WHEN utm_medium = 'llm' AND referrer_domain LIKE '%chatgpt.%'
      THEN 'chatgpt'
      WHEN utm_medium = 'llm' AND referrer_domain LIKE '%gemini.%'
      THEN 'gemini'
      WHEN utm_medium = 'llm' AND referrer_domain LIKE '%copilot.%'
      THEN 'copilot'
      WHEN utm_medium = 'llm' AND referrer_domain LIKE '%perplexity.%'
      THEN 'perplexity'
      WHEN utm_medium = 'other_llm'
      THEN 'other_llm'
      ELSE egw_utm_source
    END AS utm_source, /* UTM Source */
    CASE
      WHEN utm_medium = 'seo'
      AND entrance_uri IN ('https://www.quintoandar.com.br/', 'https://www.quintoandar.com.br')
      THEN 'branded'
      WHEN utm_medium = 'seo'
      THEN 'non-branded'
      WHEN utm_medium IN ('llm', 'other_llm')
      THEN 'llm'
      ELSE egw_utm_campaign
    END AS utm_campaign, /* UTM Campaign */
    CASE
      WHEN utm_medium = 'seo'
      THEN NULL
      WHEN utm_medium IN ('llm', 'other_llm')
    ...
    END AS utm_content, /* UTM Content */
    CASE
      WHEN utm_medium = 'seo'
      THEN NULL
      WHEN utm_medium IN ('llm', 'other_llm')
  ...
  With the union, tracking_by_device after batch 2:
    ...
    */
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY id_device ORDER BY ts_utm_attribution_start DESC) AS _w,
  ...
  SELECT
    *,
    CASE
      WHEN LAG(utm_hash) OVER (PARTITION BY id_device ORDER BY ts_tracking ASC) = utm_hash
      THEN 0
      ELSE 1
    END AS is_new_group
