...
*/
    SELECT
      ...
      MAX(t.month_start) OVER (PARTITION BY 1) AS _w,
      ol.month_start
...
*/
    SELECT
      ...
      is_offer_and_visit_stranded,
      LEAD(stranded_status) OVER (PARTITION BY id_sale_listing ORDER BY id_date DESC) AS _w,
      ROW_NUMBER() OVER (PARTITION BY id_sale_listing ORDER BY id_date ASC) AS _w_2
    FROM apply_hierarchy
  ) AS _t
  WHERE
    _w <> stranded_status OR _w_2 = 1
), aux AS (
  SELECT
    id_sale_listing,
    id_house,
