SELECT
    ...
    NULLIF(GET_JSON_OBJECT(event_properties, '$.cta_position'),'') as ep_cta_position,
    NULLIF(GET_JSON_OBJECT(event_properties, '$.button_label'),'') as ep_button_label,
    NULLIF(GET_JSON_OBJECT(event_properties, '$.whatsapp'),'') as ep_whatsapp,
    NULLIF(GET_JSON_OBJECT(event_properties, '$.entry_point'),'') as ep_entry_point,
    NULLIF(GET_JSON_OBJECT(event_properties, '$.destination_url'),'') as ep_destination_url,
    NULLIF(GET_JSON_OBJECT(event_properties, '$.platform'),'') as ep_platform,
    NULLIF(GET_JSON_OBJECT(event_properties, '$.promo_origin'),'') as ep_promo_origin,
    NULLIF(GET_JSON_OBJECT(event_properties, '$.promo_channel'),'') as ep_promo_channel,
    NULLIF(GET_JSON_OBJECT(event_properties, '$.promo_keyword'),'') as ep_promo_keyword,
    NULLIF(GET_JSON_OBJECT(event_properties, '$.promo_offer'),'') as ep_promo_offer,
    NULLIF(GET_JSON_OBJECT(event_properties, '$.promo_creative'),'') as ep_promo_creative,
...
WHERE
    id_app = '170698' AND event_type = 'consorcio.cta_clicked'
    AND (
