...
supply_2_0_turning_point AS (
    SELECT
        *,
        CASE
            WHEN DATE(COALESCE(ts_prospect_rent, ts_lead)) >= DATE('2022-10-01') -- turning point date for funnel 2.0
                THEN ts_prospect_rent
            ELSE ts_prospect_old
        END AS ts_prospect,
        CASE
            WHEN DATE(COALESCE(ts_qualified_rent, ts_lead)) >= DATE('2022-10-01') -- turning point date for funnel 2.0
                THEN ts_qualified_rent
            ELSE ts_qualified_old
        END AS ts_qualified,
        CASE
            WHEN DATE(COALESCE(ts_available_qualified_rent, ts_lead)) >= DATE('2022-10-01') -- turning point date for funnel 2.0
        ...
        *,
        CASE
            ...
            WHEN (ts_opportunity IS NOT NULL
                ...
                THEN 'NotListedYet'
            WHEN (ts_opportunity IS NOT NULL
                AND ts_opt_out_rent >= ts_opportunity
                AND ts_first_listing IS NULL)
                AND photo_job_status in ('FotosTiradas','Completado', 'NaoListado')
            ...
            WHEN (ts_opportunity IS NULL
                AND ts_qualified IS NOT NULL
                AND ts_opt_out_rent >= ts_qualified)
        ...
        CASE
            ...
            WHEN (ts_opportunity IS NULL
                    AND ts_available_qualified IS NOT NULL
                    AND ts_opt_out_rent >= ts_available_qualified)
