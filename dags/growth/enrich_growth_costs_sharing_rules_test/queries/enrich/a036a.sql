...
fall_back AS (
  SELECT 
      ...
      mm.city_group,
      mm.business_context,
      FLOAT(SUM(mm.metric))/NULLIF(SUM(SUM(mm.metric)) OVER(PARTITION BY mm.id_date), 0) AS share
...
actual AS ( 
  SELECT 
      ...
      cm.city_group, 
      cm.business_context,
      FLOAT(SUM(cm.metric))/NULLIF(SUM(SUM(cm.metric)) OVER(PARTITION BY cm.id_date), 0) AS share
  FROM conversion_metrics AS cm
  GROUP BY 1,2,3,4
), 
-----------------------------------------------------------------------------------------------
