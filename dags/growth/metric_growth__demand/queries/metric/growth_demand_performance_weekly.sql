WITH qac_user AS (
  SELECT 
    DATE(DATE_TRUNC('week', fdtof.dt_event)) AS week_start,
    fdtof.sk_tof_user,
  ...
  LEFT JOIN
    qac_user
      ON qac_user.sk_tof_user = fdtof.sk_tof_user
        AND qac_user.week_start = dd.week_start
  WHERE 
    DATE(fdtof.dt_event) BETWEEN DATE_TRUNC('week', DATE('{load_start_date}')) AND DATE('{load_end_date}')
  GROUP BY ALL
), 
...
SELECT
  COALESCE(t.week_start, p.week_start) AS dt_week_start,
  ...
  NOW() AS ts_load
FROM 
  weekly_tof_metrics AS t 
  FULL OUTER JOIN weekly_prospect_metrics AS p
