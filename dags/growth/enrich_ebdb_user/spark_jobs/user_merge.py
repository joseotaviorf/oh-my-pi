...
def get_df_user_merge():
    ...
    user_merge_df = user_merge_df.select(
        col("id_loser_account"), col("id_winner_account")
    )

    # Creating edge and vertice dfs to use on GraphFrame
    edge = user_merge_df.withColumnRenamed("id_loser_account", "src").withColumnRenamed(
        "id_winner_account", "dst"
    )
    ...
    user_merge = user_components.withColumnRenamed("id", "id_user").withColumnRenamed(
    ...
    df = components.join(
        user_merge,
        (components.component == user_merge.id_component)
        & (components.id != user_merge.id_user),
        "inner",
    )

    # Collecting the users with collect_set function and renaming the column to 'predecessor_user_list'
    df = df.groupBy(col("id")).agg(
        collect_set(col("id_user")).alias("predecessor_user_list")
    )
...
if __name__ == "__main__":
    ...
    if df:
        ...
        logger.error(
            """msg=Error while getting data for user_merge table.
