...
latest_event_ranked AS (
    SELECT
        rde.id_tenant_prospect AS id_user,
        CASE
            WHEN rde.id_event_type IN (1, 2, 11, 12, 13, 14) THEN 'VISIT'
            WHEN rde.id_event_type IN (3, 4) THEN 'OFFER'
            WHEN rde.id_event_type IN (5, 6, 7, 8) THEN 'PROPOSAL'
            WHEN rde.id_event_type IN (9, 10) THEN 'CONTRACT'
            ELSE NULL
        END AS journey_step,
        ROW_NUMBER() OVER (
            PARTITION BY rde.id_tenant_prospect
            ORDER BY rde.ts_event DESC
        ) AS rn
    FROM
        datalake_rent_demand_events.rent_demand_events AS rde
    JOIN
