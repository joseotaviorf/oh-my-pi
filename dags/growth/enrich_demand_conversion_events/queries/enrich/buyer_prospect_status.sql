WITH sale_flows AS (
  SELECT
    ...
    dpce.ts_event
  FROM
    datalake_demand_flows.demand_prospect_conversion_events_staging AS dpce
...
sale_agreements AS (
  ...
  WHERE
    sde.sk_event_type = 6 AND sde.id_buyer IS NOT NULL
),
...
status_dates AS (
  SELECT
    ...
    FIRST_VALUE(ts_prospect_churned, TRUE) OVER (
      PARTITION BY id_buyer_prospect, city_group ORDER BY ts_event ROWS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) 
    AS ts_next_churn
  FROM
    prospect_churn_rule
),
...
status_rules AS (
  SELECT
    ...
    city_group,
    FIRST_VALUE(CASE WHEN ts_status_started = ts_became_new_buyer_prospect THEN city_group END) OVER (
      PARTITION BY id_buyer_prospect ORDER BY ts_status_started ROWS UNBOUNDED PRECEDING) 
    ...
    CASE
      ...
      WHEN status = 'ACTIVE' AND LAG(status) OVER(PARTITION BY id_buyer_prospect, city_group ORDER BY ts_status_started) = 'CHURNED'
          THEN 'RECOVERED AFTER CHURN'
      WHEN status = 'ACTIVE' AND LAG(status) OVER(PARTITION BY id_buyer_prospect, city_group ORDER BY ts_status_started) = 'SIGNED CCV'
