...
-- either a numeric `user.id` or a `user.uuid_person`, so the original wrote this as one
-- join with a disjunctive predicate:
...
-- and plans a BroadcastNestedLoopJoin over the whole `user` table. On EMR that never
-- completed (j-08413162U8TCNFF0AB8Q built a ~634 MiB broadcast and stalled).
--
