...
SOURCE = "gsheets"
CONTEXT = "growth"
DAG_NAME = f"{SOURCE}_{CONTEXT}"
DAG_ID = f"{SOURCE}.{CONTEXT}"

dag_factory = GsheetsDAGFactory(
    source=SOURCE, source_with_context=DAG_NAME, task_pool="gsheets_pool"
)
DAG_CONF = ConfigurationService(dag_name=DAG_NAME).get_config("dags")
execution_date = "{{ds}}"
