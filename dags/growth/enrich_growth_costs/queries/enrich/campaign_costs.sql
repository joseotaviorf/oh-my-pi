WITH cost AS (
  SELECT
    ...
    media_metrics.month,
    media_metrics.day
  FROM dw_growth.fact_media_platform_metrics AS media_metrics
  LEFT JOIN dw_growth.dim_sharing_rules AS sharing
    ON sharing.bk_sharing_rules = media_metrics.bk_sharing_rules
    AND utm_campaign_modified = SUBSTRING(media_metrics.utm_campaign, LOCATE('.', media_metrics.utm_campaign) + 1)
  LEFT JOIN dw_public.dim_region AS region
    ON region.sk_region = media_metrics.sk_region
  LEFT JOIN dw_growth.dim_media_setup AS setup
    ON setup.naming_convention_sufix = media_metrics.naming_convention_sufix
  JOIN dw_public.dim_date AS dates
    ON dates.sk_date = media_metrics.sk_cost_date
  WHERE
    CAST(media_metrics.dt_cost AS DATE) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
  GROUP BY ALL
), campaign_google_total /* Google Ads campaign totals aggregated by campaign and date */ AS (
  SELECT
    id_campaign,
  ...
  FROM datalake_google_ads_clean.campaigns_performance
  GROUP BY ALL
), campaign_google_geo_total /* Google campaign data with geo-targeting breakdown */ AS (
  SELECT
    date,
    week_start,
    year_month,
...
SELECT
  ...
  google_geo.month,
  google_geo.day
FROM campaign_google_geo_total AS google_geo
INNER JOIN campaign_google_total AS google_total
  ON google_geo.sk_campaign = google_total.id_campaign
  AND google_geo.sk_date = google_total.sk_date
WHERE
  CAST(google_geo.dt_cost AS DATE) BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)