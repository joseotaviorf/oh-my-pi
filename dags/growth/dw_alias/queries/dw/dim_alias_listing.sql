...
parsed AS (
  SELECT
    ...
    -- installations: 16 distinct values → individual boolean flags (source enum values remain PT)
    ...
    -- top amenities (≥ 1,000 listings in validated data) → individual boolean flags
    ...
    -- tail amenities (values not covered by top-17 flags) serialized as pipe-separated string
    array_join(from_json(GET_JSON_OBJECT(lf.text2filter_prediction, '$.amenities'), 'ARRAY<STRING>'), '|') AS amenities_other,
    -- cached_location: lat/lon extracted from JSON
