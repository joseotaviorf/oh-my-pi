...
def _create_dataframe_with_standard_columns(
    response_data: dict, display_date: str
) -> DataFrame:
...
def _soft_union_all(df1: DataFrame, df2: DataFrame) -> DataFrame:
    ...
    common_cols = [col for col in df1.columns if col in df2.columns] + [
        col for col in df2.columns if col in df1.columns
    ]
    df1_full = df1.select(
        *df1.columns,
        *[
            lit(None).alias(col)
            for col in [col for col in df2.columns if col not in common_cols]
        ],
    )
    df2_full = df2.select(
        *df2.columns,
        *[
            lit(None).alias(col)
            for col in [col for col in df1.columns if col not in common_cols]
        ],
    )
...
## Logger
JOB_NAME = "load_semrush_classified_raw"
...
if __name__ == "__main__":
    ...
    responses_data = [
        json.loads(s3_service.read_file(file_path))
        for file_path in s3_service.list_objects(responses_folder_path)
    ]
    responses_df = [
        _create_dataframe_with_standard_columns(data, display_date)
        for data in responses_data
    ]

    df = reduce(_soft_union_all, responses_df)
