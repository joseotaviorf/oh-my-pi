...
def _format_requests(endpoint: str, configs: dict, display_limit: int) -> list:
    ...
    for i in range(startRow, endRow, maxRows):
        ...
        url_requests.extend(
            [endpoint + params + displayOffsetParam + displayLimitParam]
        )
...
def _verify_request(api_key: str, url_test_request: str) -> Exception:
    ...
    try:
        api_units_before = int(
            requests.get(
                f"http://www.semrush.com/users/countapiunits.html?key={api_key}"
            ).text
        )
...
## Logger
JOB_NAME = "load_semrush_classified_transient"
...
if __name__ == "__main__":
    ...
    try:
        ...
        if url_requests:
            url_responses = [
                urllib.parse.unquote(basename(url)).replace(".json", "")
                for url in s3_service.list_objects(responses_folder_path)
            ]
            url_requests = [req for req in url_requests if req not in url_responses]

        else:
            raise Exception("The file 'requests.json' is not created yet.")
        ...
        requests_nested_list = [
            _format_requests(
                endpoint,
                report_list[report]["configs"],
                report_list[report]["display_limit"],
            )
    ...
    if url_requests:
        ...
        for url_request in url_requests:
            ...
            s3_service.upload_file(
                json.dumps(response_data), f"{responses_folder_path}/{file_name}.json"
            )
