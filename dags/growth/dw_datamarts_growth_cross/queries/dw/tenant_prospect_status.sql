...
contract_person AS (
    ...
    WHERE
        ...
        AND dc.ts_signature < COALESCE(DATEADD(SECOND, 86399, CAST(dt_annulment AS TIMESTAMP)), CURRENT_DATE)
),
contract_flows AS (
    SELECT
        *
...
aux_ap AS (
  SELECT
      ap.*,
      MIN(ts_start) OVER(PARTITION BY ap.sk_client) AS ts_ntp, -- Get first activetion by client,
      ROW_NUMBER() OVER(PARTITION BY ap.sk_client, ap.city_group ORDER BY ts_start) AS activation_order -- Get first activetion by client and city group
  FROM
      active_periods AS ap
),
base AS (
    SELECT
        *
