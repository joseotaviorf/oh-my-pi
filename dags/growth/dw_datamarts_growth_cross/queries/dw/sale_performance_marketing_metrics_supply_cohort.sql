...
costs_targets_results_combined AS (
  ...
  SELECT
    ...
    SUM(0) AS available_qualifieds_target,
  ...
  SELECT
    ...
    SUM(CAST(NULLIF(REPLACE(available_qualifieds,',',''), '') AS FLOAT)) AS available_qualifieds_target,
...
SELECT
  ...
  SUM(available_qualifieds_target) AS available_qualifieds_target,
