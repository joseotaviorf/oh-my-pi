...
WITH holidays_by_city_name AS (
	...
	CROSS JOIN dw_public.dim_region dr
	LEFT JOIN datalake_gsheets_clean.local_holidays lh
	  ON dd.date = DATE(REPLACE(lh.date,'-',''))
	  AND (lh.short_region_name = dr.short_region_name
	  	OR lh.city_group = dr.city_group)
	LEFT JOIN datalake_gsheets_clean.city_share cs
	  ON COALESCE(NULLIF(lh.city_name, ''), dr.city_name) = NULLIF(cs.city_name, '')
	LEFT JOIN datalake_gsheets_clean.weekday_holiday_share_supply hs
...
LEFT JOIN datalake_gsheets_clean.weekday_supply_channel_share wscs
...
JOIN datalake_gsheets_clean.week_volumes_supply gwvs
...
FROM diff_w_and_wo_holiday_share 
	...
	WITH
	supply_targets AS (
		SELECT * FROM datalake_static_files.rental_supply_targets
	)
	SELECT
	  	dt_target AS date,
