...
def transform_payload(database_name, table_name):
    ...
    df = df.select(['sk_house', 'classified_score_rent', 'classified_score_sale','dt_snapshot'])

    df = df.withColumnRenamed('sk_house', 'houseId') \
            .withColumnRenamed('classified_score_rent', 'rentClassifiedsRankingScore') \
            .withColumnRenamed('classified_score_sale', 'saleClassifiedsRankingScore') \
    ...
    payload = [{"eventDate": row["eventDate"], "payload": {"houseId": row["houseId"], "rentClassifiedsRankingScore": row["rentClassifiedsRankingScore"], "saleClassifiedsRankingScore": row["saleClassifiedsRankingScore"]}} for row in message_contents]
