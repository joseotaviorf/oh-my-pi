...
reservation AS (
    ...
    -- either a numeric `user.id` or a `user.uuid_person`, so this was originally one join with
    -- a disjunctive predicate:
    --     ON (u.id = hl.id_related OR u.uuid_person = hl.id_related)
    -- An OR across two different columns gives Spark no hash key, so it plans a
    -- BroadcastNestedLoopJoin over the whole `user` table -- O(N x M).
    --
    -- Measured on prod cluster j-08692581I0TJSPJF0DWC: `user` was broadcast at ~634 MiB
    -- (159 pieces, against a 10 MB autoBroadcastJoinThreshold that only BNLJ ignores). Of the
    -- 724 tasks in the MERGE materialize-source stage, the 688 holding no reservation rows
    -- finished in ~1s each; the 8 that did read the broadcast, compiled the join, and never
    -- returned. The exchange could never materialise, so the whole DAG hung.
    --
