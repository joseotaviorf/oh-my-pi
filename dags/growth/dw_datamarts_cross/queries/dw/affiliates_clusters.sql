...
lead_agg AS (
    SELECT
        llf.sk_user_lead_affiliate AS sk_user,
        CAST(DATE_TRUNC('month', dtl.date) AS DATE) AS month_start,
        COUNT(CASE WHEN llf.context_lead = 'Rent' THEN 1 ELSE NULL END) AS rent_lead,
        COUNT(CASE WHEN llf.context_lead = 'Sale' THEN 1 ELSE NULL END) AS sale_lead,
        COUNT(CASE WHEN llf.context_lead = 'Hybrid' THEN 1 ELSE NULL END)/2 AS hybrid_lead,
        COUNT(*) AS total_lead,
        COUNT(CASE WHEN llf.sk_prospect_date > 0 AND DATE_TRUNC('month', dtp.date) = DATE_TRUNC('month', dtl.date) AND llf.context_prospect = 'Rent' THEN 1 ELSE NULL END) AS rent_prospect,
        COUNT(CASE WHEN llf.sk_prospect_date > 0 AND DATE_TRUNC('month', dtp.date) = DATE_TRUNC('month', dtl.date) AND llf.context_prospect = 'Sale' THEN 1 ELSE NULL END) AS sale_prospect,
        COUNT(CASE WHEN llf.sk_prospect_date > 0 AND DATE_TRUNC('month', dtp.date) = DATE_TRUNC('month', dtl.date) AND llf.context_prospect = 'Hybrid' THEN 1 ELSE NULL END)/2 AS hybrid_prospect,
        COUNT(CASE WHEN llf.sk_prospect_date > 0 AND DATE_TRUNC('month', dtp.date) = DATE_TRUNC('month', dtl.date) THEN 1 ELSE NULL END) AS total_prospect
    FROM
        dw_datamarts.lead_listing_flows AS llf
    JOIN dw_public.dim_date AS dtl
        ON dtl.sk_date = llf.sk_lead_date
    LEFT JOIN dw_public.dim_date AS dtp
        ON dtp.sk_date = llf.sk_prospect_date
    WHERE llf.sk_lead_date > 0
    GROUP BY 1,2
),
metrics AS(
    SELECT
        ...
        dua.sk_user,
        dua.type,
        SUM(COALESCE(la.rent_lead, 0)) AS rent_lead,
        SUM(COALESCE(la.sale_lead, 0)) AS sale_lead,
        SUM(COALESCE(la.hybrid_lead, 0)) AS hybrid_lead,
        SUM(COALESCE(la.total_lead, 0)) AS total_lead,
        SUM(COALESCE(la.rent_prospect, 0)) AS rent_prospect,
        SUM(COALESCE(la.sale_prospect, 0)) AS sale_prospect,
        SUM(COALESCE(la.hybrid_prospect, 0)) AS hybrid_prospect,
        SUM(COALESCE(la.total_prospect, 0)) AS total_prospect
    FROM
    ...
    LEFT JOIN lead_agg AS la
        ON dua.sk_user = la.sk_user
        AND dt.month_start = la.month_start
    GROUP BY 1, 2, 3, 4
),
calculated_metrics AS (
    SELECT
        ...
        hybrid_prospect,
        total_prospect,
        LAG(total_lead,1) OVER (PARTITION BY sk_user ORDER BY month_start) AS lead_prev_month,
        LAG(total_prospect,1) OVER (PARTITION BY sk_user ORDER BY month_start) AS prospect_prev_month,
        SUM(total_lead) OVER (PARTITION BY sk_user ORDER BY month_start ROWS UNBOUNDED PRECEDING) AS acum_lead,
        SUM(total_prospect) OVER (PARTITION BY sk_user ORDER BY month_start ROWS UNBOUNDED PRECEDING) AS acum_prospect
    FROM
        metrics
),
clusters AS(
