...
translation_dictionary_ranked AS (
    SELECT
        account_name,
        campaign_name,
        origin,
        ...
        ROW_NUMBER() OVER (
            PARTITION BY account_name, campaign_name, origin
            ORDER BY campaign_name_convention
        ) AS rn
    FROM
...
region_ranked AS (
    SELECT
        ...
        city_group,
        ROW_NUMBER() OVER (
            PARTITION BY SF_NORMALIZE_STRING(city_name)
            ORDER BY id
        ) AS rn
    FROM
...
base_costs AS (
    SELECT
        ...
        am.country_code,
        CASE
            WHEN td.campaign_name_convention IS NOT NULL THEN 'sufix_from_dictionary'
            WHEN NULLIF(CONCAT_WS('.', SLICE(SPLIT(am.utm_campaign, '[.]'), 2, 7)), '') IS NOT NULL THEN 'sufix_from_campaign'
            ELSE 'not_mapped'
        END AS type_flow_media_setup,
        COALESCE(
            td.campaign_name_convention,
            NULLIF(CONCAT_WS('.', SLICE(SPLIT(am.utm_campaign, '[.]'), 2, 7)), '')
...
rateio_unknown_costs AS (
    SELECT
        ...
        CASE
            WHEN COALESCE(total_costs_without_unknown_costs, 0.0) = 0 OR COALESCE(unknown_costs, 0.0) = 0 THEN 0
            ELSE (SUM(total_cost) / total_costs_without_unknown_costs) * unknown_costs
        END AS unknown_costs_city_group,
        CASE
            WHEN COALESCE(impressions_without_unknown_costs, 0.0) = 0 OR COALESCE(unknown_impressions, 0.0) = 0 THEN 0
            ELSE (SUM(impressions) / impressions_without_unknown_costs) * unknown_impressions
        END AS unknown_impressions_city_group,
        CASE
            WHEN COALESCE(clicks_without_unknown_costs, 0.0) = 0 OR COALESCE(unknown_clicks, 0.0) = 0 THEN 0
            ELSE (SUM(clicks) / clicks_without_unknown_costs) * unknown_clicks
        END AS unknown_clicks_city_group
    FROM
        base_costs AS bcn
    LEFT JOIN
        criteo_unknown_costs AS uc
