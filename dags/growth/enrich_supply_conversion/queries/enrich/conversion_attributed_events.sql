WITH fl_events AS (
  ...
  FROM (
    SELECT
      ...
      ts_event,
      rev,
      ROW_NUMBER() OVER (PARTITION BY id_entity, business_context ORDER BY rev, ts_event) AS rn
    FROM datalake_supply_flows.conversion_staging
    WHERE
      step = 'FIRST_LISTING'
  ) AS ranked_events
  WHERE
    rn = 1 /* First Listing Event */
), opp_events AS (
  SELECT
    id_lead,
    id_entity,
  ...
  FROM (
    ...
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY lbca.id_house, lbca.business_context ORDER BY ure.ts_revision) AS rn
  ...
  GROUP BY id_lead, business_context, supply_source
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY id_entity, business_context ORDER BY rev, ts_event DESC) AS rn
    FROM datalake_supply_flows.conversion_staging
    WHERE
      step = 'PROSPECT' AND
      drop_step = 'AQ2O' /* Only AQ20 events */
  ) AS ranked_events
  WHERE
    rn = 1 /* The last drop AQ2O */
  ...
  JOIN bob_events AS qual /* Only qualified events */
    USING (id_entity, business_context)
  LEFT ANTI JOIN fl_events AS fl /* Filter only the ones that doesn't have first listing events */
    USING (id_entity, business_context)
  ...
  FROM (
    SELECT
      ...
      'T2.0' AS aux_group,
      FALSE AS aux_data_event,
      ROW_NUMBER() OVER (PARTITION BY id_entity, business_context ORDER BY cs.rev, cs.ts_event) AS rn
  ...
  FROM (
    SELECT
      ...
      d.ts_last_discard,
      scs.ongoing_window,
      ROW_NUMBER() OVER (PARTITION BY qual.id_entity, qual.business_context, qual.step ORDER BY qual.rev, qual.ts_event) AS rn
  ...
  FROM (
    SELECT
      ...
      'T3.0' AS aux_group,
      FALSE AS aux_data_event,
      ROW_NUMBER() OVER (PARTITION BY qual.id_entity, qual.business_context ORDER BY qual.rev, qual.ts_event) AS rn
    FROM datalake_supply_flows.conversion_staging AS qual
  ...
  FROM (
    SELECT
      ...
      ts_first_discard,
      ts_last_discard,
      RANK() OVER (PARTITION BY id_lead, business_context ORDER BY rev, ts_event DESC) AS rn
    FROM discards_events_q2aq
  ) AS ranked_events
  WHERE
    rn = 1 /* Last discard event */
  UNION ALL
  SELECT
    id_lead,
    id_entity,
  ...
  FROM (
    SELECT
      ...
      'conversion_p2q' AS business_event,
      weight,
      NULL AS reason, /* Column cleaning */
      drop_step,
      ts_event,
      TRUE AS aux_data_event,
      NULL AS ts_first_discard,
  ...
  FROM (
    SELECT
      ...
      rev,
      step,
      IF(drop_step = 'Q2AQ', 4, 5) AS weight, /* If Q2AQ event, comes before (step 4) */
      reason,
      drop_step,
      ts_event,
      MIN(ts_event) OVER (PARTITION BY id_entity, business_context) AS ts_first_discard, /* Getting first and last discard dates */
      MAX(ts_event) OVER (PARTITION BY id_entity, business_context) AS ts_last_discard,
      ROW_NUMBER() OVER (PARTITION BY id_lead, business_context ORDER BY rev, ts_event DESC) AS rn
    FROM datalake_supply_flows.conversion_staging
    LEFT ANTI JOIN fl_events AS fl
      USING (id_lead, business_context)
    LEFT ANTI JOIN bob_events AS qual
  ...
  SELECT
    ...
    5 AS weight,
    p.ts_event,
    DATE_ADD(DAY, scs.ongoing_window, p.ts_event) AS ts_ongoing, /* Sum of 30 days since the OPP event date*/
  ...
  SELECT
    ...
    weight,
    ts_event AS ts_event_original,
    ts_ongoing AS ts_event_adjusted, /* Last discard date */
    ts_first_discard,
    ts_last_discard,
    NULL AS rev,
