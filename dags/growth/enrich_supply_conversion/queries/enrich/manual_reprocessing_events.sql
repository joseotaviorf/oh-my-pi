WITH all_entities AS (
  ...
  FROM (
    SELECT
      ...
      application,
      ts_created,
      ROW_NUMBER() OVER (PARTITION BY id_lead, business_context ORDER BY weight ASC, ts_created DESC) AS rn
    FROM join_mailing
  ) AS ranked_mailing
  WHERE
    rn = 1 /* Getting the last reprocessing event from */
), events AS (
  SELECT
    id_lead,
    business_context,
  ...
  FROM (
    SELECT
      ...
      id_user_registrant,
      ts_event,
      ROW_NUMBER() OVER (PARTITION BY id_lead, business_context ORDER BY ts_event DESC) AS rn
    FROM datalake_olos_dialer.reprocessing_events
  ) AS ranked_reprocessing_events
  WHERE
    rn = 1 /* Getting the last event for each lead and business context */
)
SELECT
  ae.id_entity,
  ae.business_context,
