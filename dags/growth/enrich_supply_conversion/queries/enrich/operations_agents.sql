WITH wololo AS (
  ...
  FROM (
    SELECT
      ...
      'WOLOLO' AS source,
      'NSS' AS journey,
      attendance_info IS NOT NULL AS has_ops_info,
      ...
      ROW_NUMBER() OVER (PARTITION BY id_user ORDER BY attendance_info IS NOT NULL, ts_created DESC) AS rn
    FROM datalake_wololo_clean.context_discard
    WHERE
      NOT id_user IS NULL
  ) AS ranked_operations
  WHERE
    rn = 1 /* PEGANDO ÚLTIMO REGISTRO DE CADA ANALISTA */
  ...
  FROM (
    SELECT
      id_agent,
      email,
      ROW_NUMBER() OVER (PARTITION BY id_agent ORDER BY MAKE_DATE(year, month, day) DESC) AS rn
  ...
  FROM (
    SELECT
      id_campaign,
      description,
      ROW_NUMBER() OVER (PARTITION BY id_campaign ORDER BY MAKE_DATE(year, month, day) DESC) AS rn
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY users.id_agent ORDER BY MAKE_DATE(year, month, day) DESC) AS rn
  ...
  FROM (
    SELECT
      ...
      d.ops_team,
      d.ts_created,
      ROW_NUMBER() OVER (PARTITION BY r.id_main ORDER BY ts_created DESC) AS rn
...
and needs to be updated when the context of the CIQ create a new model to capture this info */ AS (
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY r.id_main ORDER BY d.ts_updated DESC) AS rn
...
FROM (
  SELECT
    ...
    ts_register,
    ROW_NUMBER() OVER (PARTITION BY id_user ORDER BY has_ops_info DESC, order) AS rn
