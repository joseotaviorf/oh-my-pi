...
def _fetch_filtered_paths(paths: list, is_backfill: str) -> list[str]:
  ...
  if is_backfill == "true":
    ...
    sorted_paths = sorted(
        paths,
        key=lambda path: int(
            path.rstrip("/").split("sync_run_id=")[-1]
        ),
        reverse=True
    )[0:100]
    return sorted_paths
...
def _discover_parquet_paths(
    ...
    sync_runs_paths = [
        _fetch_filtered_paths(run_dir, is_backfill) for run_dir in [
          _fetch_parquet_paths_modified_in_window(
              sync_path, start_ts, end_ts
    ...
    logger.info(
        "m=_discover_parquet_paths, sync_dir_count={}"
        "parquet_file_count={}, msg=discovered source files".format(
            len(sync_dirs), len(paths)
        )
    )
    return paths
...
def _read_sync_runs_input(
    ...
    """
    paths = _discover_parquet_paths(base_path, is_backfill, load_start_date, load_end_date)
    if not paths:
        logger.info(
            "m=_read_sync_runs_input, base_path={}, is_backfill={}, load_start_date={}, load_end_date={}, "
    ...
    logger.info(
        "m=_read_sync_runs_input, path_count={}, msg=reading parquet files".format(
            len(paths)
        )
    )
    try:
...
def main():
    ...
    logger.info(
        "m=main, environment={}, datalake_bucket={}, source={}, table_name={}, is_backfill={}, "
        "load_start_date={}, load_end_date={}, incremental_column={}, input_path={}, "
        "msg=starting spark job".format(
            args.environment,
