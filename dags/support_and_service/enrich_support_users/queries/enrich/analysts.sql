WITH zendesk_tickets AS (
  ...
  SELECT
    ...
    CASE
      ...
      WHEN zu.organization IS NULL AND NOT zu.email LIKE '%ext%'
  ...
  SELECT
    ...
    organization,
    ts_created
  FROM quinto_messenger_analysts
), analysts AS (
  SELECT
    ...
    ts_updated
  FROM zendesk_analysts AS za
  FULL OUTER JOIN twilio_analysts AS td
...
FROM (
  SELECT
    ...
    ts_created,
    ts_updated,
    ROW_NUMBER() OVER (PARTITION BY email ORDER BY ts_created DESC) AS _w
  FROM analysts
) AS _t
WHERE
  _w = 1
