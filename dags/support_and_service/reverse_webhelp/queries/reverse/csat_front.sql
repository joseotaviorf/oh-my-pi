SELECT DISTINCT
  ...
  YEAR(ft.ts_csat_response) AS year,
  MONTH(ft.ts_csat_response) AS month,
  DAY(ft.ts_csat_response) AS day,
  NOW() AS ts_load
FROM
...
WHERE
  ft.ts_csat_response BETWEEN DATE_TRUNC('month', DATE('{load_start_date}') - INTERVAL '3' months) AND DATE('{load_end_date}')
