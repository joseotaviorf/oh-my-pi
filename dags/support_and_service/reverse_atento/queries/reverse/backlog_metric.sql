...
SELECT
  ...
  CASE
      WHEN LEAST(bmt.days_worked,tickets.days_elapsed_business) <= tickets.sla_target THEN true
      WHEN LEAST(bmt.days_worked,tickets.days_elapsed_business) > tickets.sla_target THEN false
      ELSE NULL
...
LEFT JOIN
  dw_customer_support.fact_tickets AS tickets
    ON tickets.sk_ticket = bmt.sk_task
LEFT JOIN
  time_solved AS ts 
