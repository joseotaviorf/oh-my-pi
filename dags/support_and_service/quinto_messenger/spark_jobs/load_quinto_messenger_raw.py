...
def _load_table_data(db_table_name, raw_table_name, table_details):
    ...
    if table_details.get("is_incremental"):
        ...
        df = postgres_consumer.get_data_from_table(table_name=db_table_name)

        _load_dataframe_in_datalake(df=df, table_name=raw_table_name)


DATABRICKS_SCOPE = "quintoandar"
JOB_NAME = "load_quinto_messenger_raw"
...
if __name__ == "__main__":
    ...
    logger.info(
        ...
        table_name={db_table_name}, raw_table_name={raw_table_name}"""
    ...
    conn_config_json = dbutils.secrets.get(
        scope=DATABRICKS_SCOPE, key=DatabaseEnum.QUINTO_MESSENGER
    )
