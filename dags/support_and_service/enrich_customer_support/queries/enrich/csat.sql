WITH csat_zendesk AS (
  ...
  WHERE
    MAKE_DATE(year, month, day) BETWEEN '{load_start_date}' AND '{load_end_date}'
    AND ts_first_response >= '2024-01-01'
...
WHERE
  MAKE_DATE(e.year, e.month, e.day) BETWEEN '{load_start_date}' AND '{load_end_date}'
...
WHERE
  sa.id IS NOT NULL
  AND DATE(c.ts_attended) BETWEEN '{load_start_date}' AND '{load_end_date}'
