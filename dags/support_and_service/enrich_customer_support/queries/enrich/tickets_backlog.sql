...
tickets AS (
  SELECT
    ...
    CAST(dt_budgeted AS TIMESTAMP) AS ts_budgeted
  FROM
    datalake_zendesk.tickets_current
  WHERE
    year >= YEAR(DATE(DATE('{load_start_date}') - INTERVAL 3 MONTH))
    AND ts_created - INTERVAL 3 HOUR >= DATE('{load_start_date}') - INTERVAL 3 MONTH
    AND tags NOT LIKE '%robotserviceaccount02%'
    AND (
      (
        group_name = 'Offboarding Reparos [OFF] [POS] [BACK]'
        AND tags LIKE '%orçamentação_realizada%'
      )
      OR group_name != 'Offboarding Reparos [OFF] [POS] [BACK]'
    )
),
ticket_sla_target AS (
  ...
  MAX(
    CASE
      WHEN ts_budgeted IS NOT NULL
        AND ts_budgeted >= t.ts_created
        AND ts_budgeted < COALESCE(t.ts_solved,TIMESTAMP('{load_end_date}')) THEN ts_budgeted
      ELSE t.ts_created
    END
  ) AS ts_sla_started,
  t.ts_solved AS ts_solved
  FROM
    tickets AS t
  LEFT JOIN
  ...
  LEFT JOIN
    datalake_gsheets_clean.tag_sla_target AS tgs
      ...
      AND t.ts_created BETWEEN tgs.dt_start AND COALESCE(tgs.dt_end, TIMESTAMP('{load_end_date}') + INTERVAL 1 DAY)
  LEFT JOIN
    datalake_customer_support.sla_theme_detail AS tds
      ...
      AND DATE(t.ts_created) BETWEEN tds.dt_start AND COALESCE(tds.dt_end, TIMESTAMP('{load_end_date}') + INTERVAL 1 DAY)
  LEFT JOIN
    datalake_customer_support.sla_theme AS ths
      ON dc.journey_step = ths.journey_step
      AND t.contact_theme_tag = ths.theme
      AND DATE(t.ts_created) BETWEEN ths.dt_start AND COALESCE(ths.dt_end, TIMESTAMP('{load_end_date}') + INTERVAL 1 DAY)
  LEFT JOIN
