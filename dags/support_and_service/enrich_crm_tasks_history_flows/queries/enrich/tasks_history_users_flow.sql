WITH actions AS (
  SELECT
    ...
    COALESCE(
      ...
      CAST(ROUND(
        (
          UNIX_TIMESTAMP(
            LEAD(trh.ts_action) OVER (PARTITION BY trh.id_task ORDER BY DATE_TRUNC('SECOND', trh.ts_action))
          ) - UNIX_TIMESTAMP(trh.ts_action)
        ) / 3600.0,
        1
      ) AS DECIMAL(10, 1))
  ...
  FROM actions AS a
  WHERE
    a.action_type IN ('REALIZE', 'FINISH') AND a.id_user_action <> -1
...
SELECT DISTINCT
  ...
  CAST(ROUND(
    (
      UNIX_TIMESTAMP(csa.ts_started_task) - UNIX_TIMESTAMP(csa.ts_created_task)
    ) / 60.0,
    1
  ) AS DECIMAL(10, 1)) AS minutes_task_created_to_started,
...
FROM actions AS a
LEFT JOIN create_start_actions AS csa
  ON a.id_task = csa.id_task
LEFT JOIN most_recent_completed_task_by_user AS mrbu
...
WHERE
  NOT (
    a.id_user_action = -1 AND a.action_type = 'REALIZE'
  )