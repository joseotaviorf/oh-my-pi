--SELECIONA USERS COM CONTRATO ATIVO, INQUILINO/MORADOR
WITH pos_list AS (
  SELECT DISTINCT
...
base_order AS (
  SELECT DISTINCT
    bo.*,
    ROW_NUMBER() OVER(PARTITION BY id_user ORDER BY ts_started ASC) AS session_ordem
  FROM
