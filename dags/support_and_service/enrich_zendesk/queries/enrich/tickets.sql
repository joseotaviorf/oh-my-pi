WITH groups AS (
  ...
  FROM (
    SELECT DISTINCT
      id_group,
      name,
      ROW_NUMBER() OVER (PARTITION BY id_group ORDER BY ts_updated DESC) AS _w,
      ts_updated
    FROM datalake_zendesk_clean.groups
  ) AS _t
  WHERE
    _w = 1
), tickets AS (
  SELECT
    ...
    CASE
      ...
      AND (
        tags LIKE '%call_contato_ativo%' OR tags LIKE '%call_contato_receptivo%'
      )
      THEN 'call'
      WHEN via_channel IN ('api') AND tags LIKE '%form%'
      THEN 'form_faq'
      WHEN via_channel IN ('whatsapp', 'native_messaging')
      THEN 'whatsapp'
      WHEN via_channel IN ('web', 'email', 'chat')
      THEN via_channel
      ELSE 'other'
    END AS channel,
    MAP_FILTER(
  ...
  WHERE
    (
      raw_subject <> 'scrubbed' AND NOT via_channel IS NULL
    )
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY cf.id_ticket, cf.ts_updated, tf.title ORDER BY tf.ts_updated DESC) AS _w,
      tf.title
    FROM splitted_cf AS cf
    INNER JOIN datalake_zendesk_clean.ticket_fields AS tf
      ON tf.id_ticket_field = cf.key
    WHERE
      NOT cf.value IS NULL AND cf.value <> '' AND cf.value <> ' '
  ) AS _t
  WHERE
    _w = 1
  ...
  SELECT
    ...
    COALESCE(
      ...
      REPLACE(
        REPLACE(
          REPLACE(t.cf_map['360032588792'] /* custom_field '[CC] - Tipo de Cliente' */, 'cc_', ''),
          'er_',
          'er'
        ),
        'serviços',
    ...
    SPLIT(
      t.cf_map['360047178772'],
      '__' /* custom_field 'Classificação do atendimento (Tags)' */
    ...
    t.day
  FROM tickets AS t
  LEFT JOIN custom_fields_with_title AS cfwt
    ON cfwt.id_ticket = t.id_ticket AND cfwt.ts_updated = t.ts_updated
  WHERE
    t.via_channel <> 'api' OR (
      t.via_channel = 'api' AND NOT t.tags LIKE '%hsm%'
    )
), analyst_assignment AS (
  SELECT DISTINCT
    t.id_ticket,
    CASE
      WHEN t.id_assignee = '5124274148'
      THEN COALESCE(a.id_agent, 5124274148) /* 5124274148 is bot id */
      ELSE t.id_assignee
    END AS id_agent,
    ...
    t.ts_updated
  FROM tickets_with_fields AS t
  LEFT JOIN datalake_support_users.analysts AS a
    ON t.analyst_email = a.email
  LEFT JOIN datalake_support_users.analysts AS a2
    ON t.id_assignee = a2.id_agent
  WHERE
    NOT t.id_assignee IS NULL
  UNION ALL
  SELECT DISTINCT
    id_ticket,
...
SELECT
  ...
  t.id_problem_ticket,
  t.id_house,
  CAST(t.id_contract AS INT) AS id_contract,
  t.id_session,
  t.id_call,
  t.id_job,
  t.id_house_so,
...
LEFT JOIN analyst_assignment AS aa
  ON t.id_ticket = aa.id_ticket AND t.ts_updated = aa.ts_updated
