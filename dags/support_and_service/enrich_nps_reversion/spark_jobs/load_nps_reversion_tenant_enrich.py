...
JOB_NAME = "load_nps_reversion_tenant_enrich"
...
if __name__ == "__main__":
    ...
    output_table_name = config_service.get_config("offboarding_tenant_output_table_name")
    input_table_name = config_service.get_config("offboarding_tenant_input_table_name")
    partition_cols = config_service.get_config("output_partitions")
    model_filename = config_service.get_config("offboarding_tenant_model_filename")
    input_query = config_service.get_config("offboarding_input_query")
    input_query_columns = config_service.get_config("offboarding_tenant_input_columns")
    ...
    output_columns = config_service.get_config("offboarding_output_columns")
    ...
    reglog_model = pickle.load(open(model_filename, "rb"))
    ...
    df_predict[model_output_column] = reglog_model.predict_proba(df_predict[input_query_columns])[:,1]
    df_predict[model_predicted_date_column] = datetime.strptime(execution_date, "%Y-%m-%d").date()
    df_predict = df_predict[output_columns]
