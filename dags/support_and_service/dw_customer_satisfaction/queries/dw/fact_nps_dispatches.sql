WITH
  ...
  SELECT
    ...
    d.ts_dispatch_created AS ts_created,
...
attributes_union AS (
  SELECT 
    ...
    da.status,
    da.survey_opened,
    MAX(COALESCE(cci_e.cpf,cci_p.cpf)) AS cpf
  FROM
    datalake_tracksale.dispatch_attributes da
  LEFT JOIN datalake_ebdb_customer_contact_identification.customer_contact_identification AS cci_p
      ON REGEXP_REPLACE(REGEXP_REPLACE(cci_p.customer_contact,'\\D+',''),'^55','') = COALESCE(REGEXP_REPLACE(COALESCE(da.phone),'\\D+',''),'')
      AND cci_p.channel = 'phone'
    LEFT JOIN datalake_ebdb_customer_contact_identification.customer_contact_identification cci_e
      ON LOWER(cci_e.customer_contact) = LOWER(da.email)
...
customer_last_rent_event AS (
  SELECT
    ...
    MAX(rd.ts_event) FILTER(WHERE rd.ts_event < au.dispatch_time) AS ts_rent_last_event
  FROM
...
SELECT
  ...
  COALESCE(at.cpf, -1) AS sk_personal_document,
