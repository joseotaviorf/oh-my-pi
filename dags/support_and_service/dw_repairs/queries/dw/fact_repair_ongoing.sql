...
SELECT
  ...
  COALESCE(tfm.id_user, -1) AS sk_user,
  ...
  COALESCE(CAST(REPLACE(SUBSTRING(DATE(DATEADD(DAY, ROUND(((tfm.minutes_reply_calendar/60)/24)),rt.ts_created_local)),1, 10),'-','') AS BIGINT), -1) AS sk_first_public_comment,
  rt.reopens,
  rt.relisting,
  rt.replies,
  ...
  tfm.minutes_reply_calendar AS ldt_fr_minutes,
  NOW() AS ts_load,
  rt.year AS year,
  rt.month AS month,
  rt.day AS day
