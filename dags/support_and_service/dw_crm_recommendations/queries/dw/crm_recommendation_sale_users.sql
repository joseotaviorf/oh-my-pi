-- The source 30-day window is a single partition-pruned scan. The original Trino
-- query hand-rolled a three-branch UNION ALL over year/month/day to force
-- pushdown; Spark folds MAKE_DATE(year, month, day) >= <literal> into
-- PartitionFilters on its own, so the branches are unnecessary here.
WITH listing_page_view_users AS (
    ...
    WHERE
        ...
        AND GET_JSON_OBJECT(tracking.event_properties, '$.business_context') = 'sale'
    GROUP BY
        tracking.id_user
    HAVING
        COUNT(*) > 2
...
signed_buyers AS (
    SELECT DISTINCT
        sale_events.id_buyer AS id_user
    FROM
        datalake_sale_demand_events.sale_demand_events AS sale_events
    WHERE
        sale_events.sk_event_type = 6
        AND sale_events.dt_event >= ADD_MONTHS(CURRENT_DATE(), -12)
)
SELECT
...
LEFT ANTI JOIN
    signed_buyers AS recently_bought
    ON listing_viewers.id_user = recently_bought.id_user
WHERE
