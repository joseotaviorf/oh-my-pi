WITH persona_type_pivot AS (
  SELECT
    ...
    month,
    day,
    MAX(pt.tenant_journey_step) AS tenant_journey_step,
    MAX(pt.tenant_persona_step) AS tenant_persona_step,
    MAX(pt.is_tenant_offboarding) AS is_tenant_offboarding,
    MAX(pt.is_tenant_ongoing) AS is_tenant_ongoing,
    MAX(pt.is_tenant_onboarding) AS is_tenant_onboarding,
    MAX(pt.is_tenant_contract_to_entrance) AS is_tenant_contract_to_entrance,
    MAX(pt.is_tenant_visits_to_offer) AS is_tenant_visits_to_offer,
    MAX(pt.is_tenant_listing_and_search) AS is_tenant_listing_and_search,
    MAX(pt.is_tenant_pre_contract) AS is_tenant_pre_contract,
    MAX(pt.is_tenant_post_contract) AS is_tenant_post_contract,
    ...
    MAX(CASE WHEN pt.client_type = 'property_owner' THEN TRUE ELSE FALSE END) AS is_property_owner,
