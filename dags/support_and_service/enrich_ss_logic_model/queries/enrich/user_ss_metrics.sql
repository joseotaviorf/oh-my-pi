WITH persona_type_pivot AS (
  SELECT
    ...
    month,
    day,
    MAX(pt.journey_step) AS journey_step,
    MAX(pt.persona_step) AS persona_step,
    MAX(pt.is_offboarding) AS is_offboarding,
    MAX(pt.is_ongoing) AS is_ongoing,
    MAX(pt.is_onboarding) AS is_onboarding,
    MAX(pt.is_contract_to_entrance) AS is_contract_to_entrance,
    MAX(pt.is_visits_to_offer) AS is_visits_to_offer,
    MAX(pt.is_listing_and_search) AS is_listing_and_search,
    MAX(pt.is_pre_contract) AS is_pre_contract,
    MAX(pt.is_post_contract) AS is_post_contract,
...
SELECT
  ...
  ptp.is_ongoing,
  ptp.is_onboarding,
  ptp.is_contract_to_entrance,
  ptp.is_visits_to_offer,
  ptp.is_listing_and_search,
  ptp.is_pre_contract,
  ptp.is_post_contract,
  ptp.is_active,
  ptp.is_blocked,
