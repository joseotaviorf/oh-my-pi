WITH consultant_prep AS (
    SELECT
        id_offer,
        CASE
            WHEN SPLIT(REPLACE(id_consultant, ']', ''),',')[1] = '11422665'
                THEN SPLIT(REPLACE(id_consultant, '[', ''),',')[0]
            WHEN id_consultant NOT LIKE '%,%'
                THEN REPLACE(REPLACE(id_consultant, ']', ''), '[', '')
            ELSE SPLIT(REPLACE(id_consultant, ']', ''),',')[1]
        END AS id_consultant,
        id_closing_specialist
    FROM
        datalake_firestore.monday AS dfm
),
consultant_prep_rules AS (
    ...
    FROM
        consultant_prep AS cp
    LEFT JOIN
        datalake_gsheets_clean.monday_users AS mu
            ON mu.id_monday = cp.id_consultant
    LEFT JOIN
        datalake_gsheets_clean.monday_users AS ms
...
offer_rules AS (
    SELECT
        ...
        CASE
            ...
            WHEN LENGTH(dfm.drop_reason) > 4 THEN dfm.drop_reason
