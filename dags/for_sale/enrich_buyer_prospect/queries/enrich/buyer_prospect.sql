...
SELECT
  ...
  CAST(sf.sale_flows AS INTEGER) AS sale_flows,
  CAST(sf.regions_with_sale_flows AS INTEGER) AS regions_with_sale_flows,
  CAST(sf.weeks_with_sale_flows AS INTEGER) AS weeks_with_sale_flows,
  CAST(COALESCE(v.bookings_created,0) AS INTEGER) AS bookings_created,
  CAST(COALESCE(v.bookings_created_by_agent,0) AS INTEGER) AS bookings_created_by_agent,
  CAST(COALESCE(v.bookings_canceled,0) AS INTEGER) AS bookings_canceled,
  CAST(COALESCE(v.visits_completed,0) AS INTEGER) AS visits_completed,
  CAST(COALESCE(o.offers_submitted,0) AS INTEGER) AS offers_submitted,
  CAST(COALESCE(o.offers_accepted,0) AS INTEGER) AS offers_accepted,
  CAST(COALESCE(o.offers_dismissed,0) AS INTEGER) AS offers_dismissed,
  CAST(COALESCE(o.sale_agreements_signed,0) AS INTEGER) AS sale_agreements_signed,
  sf.avg_sale_price,
  o.avg_offer_price,
  o.avg_offer_discount,
  CAST(COALESCE(fo.houses_owned,0) AS INTEGER) AS houses_owned,
  CAST(COALESCE(fo.listings_for_sale,0) AS INTEGER) AS listings_for_sale,
  CAST(COALESCE(fo.listings_for_rent,0) AS INTEGER) AS listings_for_rent,
