WITH visit_rent_flow AS (
  ...
  FROM dw_rent.fact_rent_demand_events AS frde
  INNER JOIN dw_rent.dim_rent_event_type AS dret
    ON dret.sk_event_type = frde.sk_event_type
  WHERE
    LOWER(dret.abbreviation) IN ('os', 'oa', 'cs') AND NOT sk_visit IS NULL
  GROUP BY
    1,
    2,
    3
  ...
  INNER JOIN dw_sale.dim_sale_event_type AS dset
    ON dset.sk_event_type = fsde.sk_event_type
  WHERE
    LOWER(dset.abbreviation) IN ('os', 'oa', 'ccv') AND NOT sk_visit IS NULL
  GROUP BY
    1,
    2,
    3
...
SELECT
  UNHEX(MD5(sk_visit || event_code || business_context)) AS sk_visit_funnel,
  sk_visit,
  event_code,
  business_context,
  ts_first_event,
  ts_last_event
FROM visit_rent_flow
UNION ALL
SELECT
  UNHEX(MD5(sk_visit || event_code || business_context)) AS sk_visit_funnel,
  sk_visit,
