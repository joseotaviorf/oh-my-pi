...
WHERE 
    ...
    AND ((TRIM(iptu_standard_description) = 'COMERCIAL HORIZONTAL' AND TRIM(iptu_use_description) IN ('RESIDÊNCIA','RESIDÊNCIA COLETIVA, EXCLUSIVE CORTIÇO (MAIS DE UMA RESIDÊNCIA NO LOTE)', 'RESIDÊNCIA E OUTRO USO (PREDOMINÂNCIA RESIDENCIAL)'))
        ...
        OR (TRIM(iptu_standard_description) = 'RESIDENCIAL VERTICAL' AND TRIM(iptu_use_description) IN ('APARTAMENTO EM CONDOMÍNIO (EXIGE FRAÇÃO IDEAL)','RESIDÊNCIA','RESIDÊNCIA COLETIVA, EXCLUSIVE CORTIÇO (MAIS DE UMA RESIDÊNCIA NO LOTE)','RESIDÊNCIA E OUTRO USO (PREDOMINÂNCIA RESIDENCIAL)')))
