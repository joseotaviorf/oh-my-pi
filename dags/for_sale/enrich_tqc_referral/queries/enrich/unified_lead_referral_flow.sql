...
gsheets_tqc_agents_join_raw AS (
  SELECT
    tqc.id,
    ...
    -- Raw (unclamped) day distance between the matched agent's ts_created and the appointment.
    DATEDIFF(COALESCE(daa1.ts_created, daa2.ts_created, dua1.ts_created,dua2.ts_created),tqc.ts_appointment) AS raw_agent_days_diff,
    tqc.lead_name,
    tqc.lead_phone,
    tqc.lead_email,
    ah.hub_name,
  ...
  SELECT
    ...
    ROW_NUMBER() OVER(
      PARTITION BY
        id
      ORDER BY
        GREATEST(raw_agent_days_diff, 0), -- We prioritize the ones that were created before the appointment (which are set to 0). Otherwise, the most recent ones after the appointment.
        ABS(raw_agent_days_diff) -- If more than one is from before, we prioritize the ones closer to the appointment.
    ) AS rn
  FROM
    gsheets_tqc_agents_join_raw
),
gsheets_tqc_agents_join AS (
  SELECT
    ...
    ts_created
  FROM
    gsheets_tqc_agents_join_ranked
  WHERE
    rn = 1
),
gsheets_tqc_final_raw AS (
  SELECT
    tqc.id,
    ...
    tqc.lead_phone,
    DATEDIFF(COALESCE(du1.ts_created, du2.ts_created),tqc.ts_created) AS raw_lead_days_diff,
    tqc.hub_name,
    tqc.city_group,
    tqc.macro_region,
    tqc.status,
...
gsheets_tqc_final_ranked AS (
  SELECT
    ...
    id_user_lead,
    lead_phone,
    GREATEST(raw_lead_days_diff, 0) AS aux_deduplicate,
    hub_name,
    city_group,
    macro_region,
    status,
    ...
    origin,
    ts_created,
    ROW_NUMBER() OVER(PARTITION BY id ORDER BY GREATEST(raw_lead_days_diff, 0), ABS(raw_lead_days_diff)) AS rn
  FROM
    gsheets_tqc_final_raw
),
...
new_leads_ranked AS (
  SELECT
    ...
    -- In some cases, there is more than one hub active for the same agent at a given day. We're getting the newest one.
    ROW_NUMBER() OVER(PARTITION BY lr.id ORDER BY ah.dt_reference DESC) AS rn
  FROM
