WITH sales_flow_users AS (
  SELECT
    ...
    ts_created,
    ts_updated
  FROM datalake_sales_flow_clean.sales_flow_users
), first_invite AS (
  SELECT
    id_sales_flow,
    id_inviter,
  ...
  FROM (
    SELECT
      ...
      type,
      ts_created,
      ROW_NUMBER() OVER (PARTITION BY id_sales_flow, id_invitee, type ORDER BY ts_created ASC) AS _w
    FROM datalake_sales_flow_clean.sales_flow_invite_aud
  ) AS _t
  WHERE
    _w = 1
  ...
  FROM (
    SELECT
      ...
      type,
      ts_updated,
      ROW_NUMBER() OVER (PARTITION BY id_sales_flow, id_inviter, id_invitee, type ORDER BY ts_updated DESC) AS _w
    FROM datalake_sales_flow_clean.sales_flow_invite_aud
    WHERE
      status = 'ACCEPTED'
  ) AS _t
  ...
  FROM (
    SELECT
      ...
      sfiaud.ts_created,
      sfiaud.ts_updated,
      ROW_NUMBER() OVER (PARTITION BY sfiaud.id_sales_flow, sfiaud.id_inviter, sfiaud.id_invitee, sfiaud.type ORDER BY sfiaud.ts_updated DESC) AS _w
    FROM datalake_sales_flow_clean.sales_flow_invite_aud AS sfiaud
    LEFT JOIN first_invite AS fi
    ...
    LEFT JOIN last_invite_update AS liu
  ...
  FROM (
    SELECT
      id,
      id_external,
      ROW_NUMBER() OVER (PARTITION BY id ORDER BY ts_updated) AS _w,
      ts_updated
  ...
  SELECT
    ...
    GET_JSON_OBJECT(reference_properties, '$.offerId') AS id_offer,
    GET_JSON_OBJECT(reference_properties, '$.isCCVBuyer') AS is_ccv_buyer,
    GET_JSON_OBJECT(reference_properties, '$.isCCVFlow') AS is_ccv_flow,
    GET_JSON_OBJECT(reference_properties, '$.isIncomePartner') AS is_income_partner,
    GET_JSON_OBJECT(reference_properties, '$.hasJointBuyer') AS has_joint_buyer,
    GET_JSON_OBJECT(reference_properties, '$.inviteeList') AS invitee_list,
    ts_documentation_sent
  FROM datalake_docx_clean.folder_reference AS fr
  LEFT JOIN datalake_docx_clean.folder AS fo
...
SELECT
  ...
  sfu.ts_updated
FROM sales_flow_users AS sfu
LEFT JOIN users_info AS ui
  ON ui.id_user = sfu.id_user
LEFT JOIN sales_flow_invites AS sfi
  ON sfi.id_sales_flow = sfu.id_sales_flow AND sfi.id_invitee = sfu.id_user
LEFT JOIN users_info AS inv_ui
...
LEFT JOIN sales_flow_house AS sfh
  ON sfh.id = sf.id_house
LEFT JOIN folder_reference_docx AS frd
  ON frd.id_offer = off.id_firestore AND frd.id_external = ui.id_external
LEFT JOIN house_folder_reference AS hfr
  ON hfr.id_external = sfh.id_external AND hfr.id_offer = off.id_firestore
LEFT JOIN buyer_data AS byd
  ON byd.email = ui.email AND byd.id_sales_flow = sfu.id_sales_flow
LEFT JOIN seller_data AS sld
  ON sld.email = ui.email AND sld.id_sales_flow = sfu.id_sales_flow
