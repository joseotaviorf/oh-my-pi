...
WITH last_sf_entry AS (
  ...
  SELECT
    ...
    sf.flow_type
  FROM datalake_sales_flow_clean.offer_aud AS o
  LEFT JOIN last_sf_entry AS sf
    ON sf.id = o.id_sales_flow AND sf.row = 1
  GROUP BY
    o.id_firestore,
    sf.flow_type
), first_offer_entry AS (
  SELECT
    *
  FROM (
    SELECT
      *,
      ROW_NUMBER() OVER (PARTITION BY id_firestore ORDER BY ts_updated) AS _w
  ...
  SELECT
    ...
    l.ts_updated,
    l.ts_discarded
  FROM last_offer_entry AS l
  LEFT JOIN first_offer_entry AS f
    ON l.id_firestore = f.id_firestore
  LEFT JOIN last_reason_entry AS lre
    ON l.id_reject_reason = lre.id
), last_update_sales_flow /* SALES FLOW CTE */ AS (
  SELECT
    sf.*,
  ...
  SELECT
    ...
    sf.is_canceled,
    sf.ts_canceled
  FROM last_update_sales_flow AS sf
  INNER JOIN offers AS sfo
  ...
  SELECT
    ...
    *
  FROM datalake_sales_flow_clean.ccv_flow
), offers_ccvs AS (
  SELECT
  ...
  FROM offers_specialists AS o
  LEFT JOIN last_specialist AS dm
    ON dm.id_sales_flow = o.id_sales_flow AND dm.kind = 'DEAL_MAKER' AND dm.row = 1
  LEFT JOIN last_specialist AS tl
    ON tl.id_sales_flow = o.id_sales_flow AND tl.kind = 'TEAM_LEAD' AND tl.row = 1
), ccvs /* CCVS CTE */ AS (
  SELECT
    o.id_offer,
    o.id_sales_flow,
    lce.ts_signed,
    lce.ts_created
  FROM offers_ccvs AS o
  LEFT JOIN specialists_ccvs AS s
    ON s.id_offer = o.id_offer
  INNER JOIN last_ccv_entry AS lce
    ON lce.id_sales_flow = o.id_sales_flow
    AND lce.row = 1
    AND (
      (
        UNIX_TIMESTAMP(lce.ts_signed) - UNIX_TIMESTAMP(lce.ts_created)
      ) > 0
      OR (
        NOT s.id_consultant IS NULL AND lce.status = 'SIGNED'
      )
    )
), last_update_ccv_flow /* CCV FLOW CTE */ AS (
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY id_sales_flow ORDER BY ts_updated DESC) AS ROW
  FROM datalake_sales_flow_clean.ccv_flow
  ...
  SELECT
    ...
    CAST(ts_confection_started AS DATE) AS dt_confection_started,
    CAST(ts_signed AS DATE) AS dt_sale_agreement_signed
  FROM last_update_ccv_flow AS ccv_flow
  WHERE
    ROW = 1
), last_update_details /* SALES FLOWS DETAILS CTE */ AS (
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY id_sales_flow ORDER BY ts_updated DESC) AS ROW
  ...
  SELECT
    ...
    ts_buyer_fup,
    ts_updated
  FROM last_update_details AS sfd
  WHERE
    ROW = 1
), last_update_pendency /* PENDENCY CTE */ AS (
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY id_sales_flow ORDER BY ts_updated DESC) AS ROW
  ...
  SELECT
    ...
    type,
    ts_updated
  FROM last_update_pendency AS sfp
  WHERE
    ROW = 1
), last_update_mortgage /* MORTGAGE CTE */ AS (
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY id_mortgage ORDER BY ts_updated DESC) AS ROW
  FROM datalake_sales_flow_clean.mortgage
), mortgage AS (
  SELECT
    mg.id_mortgage,
    mg.id_sales_flow,
  ...
  FROM last_update_mortgage AS mg
  INNER JOIN offers AS sfo
    ON sfo.id_sales_flow = mg.id_sales_flow
  WHERE
    ROW = 1
), last_update_payment /* PAYMENT CTE */ AS (
  SELECT
    *
  FROM (
    SELECT
  ...
  FROM (
    SELECT
      *,
      ...
      ROW_NUMBER() OVER (PARTITION BY id_sales_flow ORDER BY ts_updated ASC) AS _w
  ...
  FROM last_update_payment AS p
  INNER JOIN first_update_payment AS fpm
    ON p.id = fpm.id
  INNER JOIN offers AS sfo
    ON sfo.id_sales_flow = p.id_sales_flow
), last_update_cash_payment /* CASH PAYMENT CTE */ AS (
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY id_cash_payment ORDER BY ts_updated DESC) AS ROW
  FROM datalake_sales_flow_clean.cash_payment
), cash_payment AS (
  SELECT
    cp.id_cash_payment,
    cp.id_payment,
    ...
    cp.crn_details,
    CAST(cp.ts_crn_started AS DATE) AS dt_crn_started,
    CAST(cp.ts_crn_ended AS DATE) AS dt_crn_ended
  FROM last_update_cash_payment AS cp
  WHERE
    ROW = 1
), last_update_notary /* NOTARY CTE */ AS (
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY id_notary ORDER BY ts_updated DESC) AS ROW
  FROM datalake_sales_flow_clean.notary
), notary AS (
  SELECT
    n.id_notary,
    n.id_sales_flow,
  ...
  FROM last_update_diligence AS d
  INNER JOIN offers AS sfo
    ON sfo.id_sales_flow = d.id_sales_flow
  WHERE
    ROW = 1
), last_update_diligence_appointment /* DILIGENCE APPOINTMENT */ AS (
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY id_diligence_appointment ORDER BY ts_updated DESC) AS ROW
  FROM datalake_sales_flow_clean.diligence_appointment
), diligence_appointment AS (
  SELECT
    da.id_diligence,
    CONCAT_WS(' - ', COLLECT_SET(appointment)) AS appointment
  FROM last_update_diligence_appointment AS da
  WHERE
    ROW = 1
  GROUP BY
    id_diligence
), last_update_house /* HOUSE CTE */ AS (
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY id ORDER BY ts_updated DESC) AS ROW
  ...
  FROM last_update_house AS h
  WHERE
    ROW = 1
), last_update_offer_monopoly /* MONOPOLY CTE */ AS (
  SELECT
    *,
  ...
  FROM last_update_offer_monopoly
  WHERE
    ROW = 1
), status_closing_changes /* ONBOARDING CTE */ AS (
  SELECT
    id,
    ts_updated,
    LAG(status_closing) OVER (PARTITION BY id ORDER BY ts_updated) AS previous_status
  ...
  SELECT
    id AS id_sales_flow,
    CAST(MIN(ts_updated) AS DATE) AS dt_ended
  FROM status_closing_changes
  WHERE
    previous_status = 'ONBOARDING' AND ts_updated < '2022-04-12'
  GROUP BY
    id_sales_flow
), onboarding_status /* ONBOARDING FROM STATUS ENRICH CTE */ AS (
  SELECT
    id_sales_flow,
    id_firestore AS id_offer,
    CAST(ts_macro_status_end AS DATE) AS dt_onboarding_ended
  FROM datalake_sale_offer_flows.sale_offer_status
  WHERE
    macro_status_name = 'ONBOARDING'
    AND CAST(ts_macro_status_end AS DATE) > CAST('2022-04-12' AS DATE)
), credit_analysis_status AS (
  SELECT
    id_sales_flow,
    id_offer,
    credit_analysis_status_name
  FROM (
    SELECT
      id_sales_flow,
      id_firestore AS id_offer,
      last_micro_status_name AS credit_analysis_status_name,
      ROW_NUMBER() OVER (PARTITION BY id_sales_flow, macro_status_name ORDER BY ts_micro_status_last_update DESC) AS _w,
      macro_status_name,
  ...
  WHERE
    _w = 1
), last_update_tag /* TAG CTE */ AS (
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY id ORDER BY ts_updated DESC) AS ROW
  FROM datalake_sales_flow_clean.tag
), tag AS (
  SELECT
    id_sales_flow,
    CONCAT_WS('; ', COLLECT_SET(CONCAT('#', label))) AS label
  FROM datalake_sales_flow_clean.sales_flow_tag AS sftag
  INNER JOIN last_update_tag AS tag
    ON sftag.id_tag = tag.id
  WHERE
    ROW = 1
  GROUP BY
    id_sales_flow
), base_offers_history AS (
  SELECT
    id_sales_flow,
    id_firestore AS id_offer,
  ...
  SELECT
    ...
    O.ts_discarded,
    CASE
      WHEN SF.is_canceled = FALSE AND NOT SF.ts_canceled IS NULL
      THEN TRUE
      ELSE FALSE
  ...
  SELECT
    ...
    closing_canceled_reason,
    is_canceled,
    CASE WHEN is_canceled = FALSE AND last_cancelation_status = TRUE THEN TRUE END AS is_a_rescue,
    CASE
    ...
    ts_updated,
    ccvs.ts_signed
  FROM cancelation_history AS ch
  LEFT JOIN ccvs
  ...
  FROM (
    SELECT
      id_sales_flow,
      CASE WHEN NOT closing_canceled_reason IS NULL THEN ts_updated ELSE NULL END AS ts_sale_agreement_canceled,
      ts_signed,
      ts_rescued,
      ROW_NUMBER() OVER (PARTITION BY id_sales_flow ORDER BY ts_updated DESC) AS _w,
      ts_updated
  ...
  FROM (
    SELECT
      ...
      rcs.ts_rescued,
      ROW_NUMBER() OVER (PARTITION BY rcs.id_sales_flow ORDER BY rcs.ts_updated DESC) AS _w,
      rcs.ts_updated
    FROM offers_history AS ofh
    INNER JOIN rescue_and_cancelation_status AS rcs
      ON rcs.id_sales_flow = ofh.id_sales_flow
      AND ofh.ts_last_offer_discarded <= rcs.ts_last_canceled
    WHERE
      rcs.is_a_rescue = TRUE AND closing_canceled_reason IS NULL
  ) AS _t
  WHERE
    _w = 1
), rescue_flow /* OFFER/SALE AGREEMENT RESCUE FLOW CTE */ AS (
  SELECT
    sf.id AS id_sales_flow,
    rof.id_offer,
    rof.is_a_rescued_offer,
    IF(NOT c_ccvs.ts_rescued IS NULL, TRUE, FALSE) AS is_a_rescued_ccv,
  ...
  FROM sales_flow AS sf
  LEFT JOIN rescued_offers AS rof
    ON rof.id_sales_flow = sf.id
  LEFT JOIN canceled_ccvs AS c_ccvs
    ON c_ccvs.id_sales_flow = sf.id
), brokerage /* BROKERAGE CTE */ AS (
  SELECT
    id_sales_flow,
...
SELECT
  ...
  CASE WHEN ft.flow_type = 'DEFAULT' THEN TRUE ELSE FALSE END AS has_used_negotiation_chat,
  sf.closing_canceled_reason AS sale_agreement_cancellation_reason,
  off.discard_reason AS drop_reason,
  CASE
    WHEN off.reject_source = 'BUYER'
    OR off.reject_source IS NULL
    AND LOWER(off.discard_reason) LIKE '%buyer%'
    THEN 'Buyer'
    WHEN off.reject_source = 'SELLER'
    OR off.reject_source IS NULL
    AND LOWER(off.discard_reason) LIKE '%seller%'
    THEN 'Seller'
    ELSE 'Other'
  ...
  p.fgts_value,
  p.down_payment_value,
  CASE WHEN p.payment_method LIKE '%USING_FGTS' THEN TRUE ELSE FALSE END AS has_used_fgts_in_payment,
  p.entry_amount,
  ...
  COALESCE(b.brokerage_fee, CAST(0.06 AS DECIMAL(5, 4))) AS brokerage_fee, /* 6% default, hard coded in sales-flow */
  b.quinto_andar_brokerage_split,
  CASE
    WHEN CAST(off.ts_accepted AS DATE) <= CAST(off.ts_discarded AS DATE)
    THEN DATEDIFF(TO_DATE(CAST(off.ts_discarded AS DATE)), TO_DATE(CAST(off.ts_accepted AS DATE)))
  END AS days_offer_accepted_to_offer_dismissed,
  DATEDIFF(TO_DATE(ccvf.dt_confection_started), TO_DATE(CAST(off.ts_accepted AS DATE))) AS days_offer_accepted_to_sale_agreement_created,
  DATEDIFF(TO_DATE(ccvf.dt_sale_agreement_signed), TO_DATE(CAST(off.ts_accepted AS DATE))) AS days_offer_accepted_to_sale_agreement_signed,
  DATEDIFF(TO_DATE(ccvf.dt_sale_agreement_signed), TO_DATE(ccvf.dt_confection_started)) AS days_sale_agreement_created_to_sale_agreement_signed,
  h.has_seller_debt_payments,
  COALESCE(
    sf.is_canceled,
    CASE
      WHEN NOT ccvf.dt_sale_agreement_signed IS NULL
  ...
  NULLIF(CAST(rf.ts_sale_agreement_canceled AS DATE), '0001-01-1') AS dt_sale_agreement_cancelled,
  NULLIF(CAST(rf.ts_offer_rescued AS DATE), '0001-01-1') AS dt_offer_rescued,
  NULLIF(CAST(rf.ts_sale_agreement_rescued AS DATE), '0001-01-1') AS dt_sale_agreement_rescued,
  NULLIF(COALESCE(os.dt_onboarding_ended, o.dt_ended), '0001-01-1') AS dt_onboarding_ended,
  NULLIF(CAST(d.ts_buyer_sent AS DATE), '0001-01-1') AS dt_diligence_buyer_sent_at,
  NULLIF(CAST(d.ts_seller_sent AS DATE), '0001-01-1') AS dt_diligence_seller_sent_at,
  NULLIF(CAST(d.ts_buyer_seller_ended AS DATE), '0001-01-1') AS dt_legal_analysis_ended,
  NULLIF(CAST(d.ts_partner_started AS DATE), '0001-01-1') AS dt_legaut_analysis_started,
  NULLIF(CAST(d.ts_partner_ended AS DATE), '0001-01-1') AS dt_legaut_analysis_ended,
  NULLIF(CAST(d.ts_legal_risk_started AS DATE), '0001-01-1') AS dt_legal_risk_started,
  NULLIF(CAST(d.ts_legal_risk_ended AS DATE), '0001-01-1') AS dt_legal_risk_ended,
  NULLIF(CAST(mg.dt_bank_started AS DATE), '0001-01-1') AS dt_bank_legal_analysis_started,
  NULLIF(CAST(mg.dt_credit_started AS DATE), '0001-01-1') AS dt_credit_analysis_started,
  NULLIF(CAST(mg.dt_credit_ended AS DATE), '0001-01-1') AS dt_credit_analysis_ended,
  NULLIF(CAST(mg.dt_started AS DATE), '0001-01-1') AS dt_financing_started,
  NULLIF(CAST(mg.dt_ended AS DATE), '0001-01-1') AS dt_financing_ended,
...
LEFT JOIN flow_type AS ft
  ON ft.id_offer = off.id_offer
LEFT JOIN ccvs AS ccv
...
LEFT JOIN credit_analysis_status AS cas
  ON cas.id_sales_flow = sf.id
LEFT JOIN brokerage AS b
  ON b.id_sales_flow = off.id_sales_flow
LEFT JOIN monopoly AS mpl
  ON off.id_offer = mpl.id_offer
WHERE
  tag.label IS NULL OR NOT tag.label LIKE '%#offertestedeproduto%'