WITH sale_listing_flows_adjust AS (
  SELECT
    ...
    lf.mkt_medium,
    lf.sales_company,
    CASE WHEN NOT rbh.id IS NULL THEN 1 ELSE 0 END AS is_3pbh_supply,
    ...
    CASE
      WHEN lf.mkt_origin = 'B2B' OR lf.mkt_origin = 'CIQ'
      THEN lf.mkt_origin
    ...
    CASE
      ...
      AND NOT lf.mkt_origin IN ('B2B', 'CIQ')
      THEN 'IS'
      ELSE sourcing_ops
    END AS lead_processing_operation,
    CASE
      WHEN lf.lead_context_origin = 'Organic'
      THEN 'Branded'
      ELSE lead_context_origin
    END AS mkt_campaign_context,
    CASE
      WHEN lf.mkt_origin IN ('Indica Aí - Agents', 'Indica Aí - General', 'Price Calculator', 'B2B', 'CIQ', 'Doorman')
      THEN 'Paid'
      WHEN lf.mkt_origin IN ('Other', 'Backend', 'Inbound')
      THEN 'Non Paid'
      WHEN lf.mkt_channel IN ('CRM/Notification', 'LeadEnrichment', 'Organic')
      AND lf.mkt_origin = 'Owner PWA'
      THEN 'Non Paid'
      WHEN lf.mkt_channel = 'Paid' AND lf.mkt_origin = 'Owner PWA'
      THEN 'Paid'
    END AS mkt_type,
    CASE
      WHEN NOT dr.city_group IN ('RMSP', 'Rio de Janeiro', 'Belo Horizonte', 'Porto Alegre', 'Campinas')
      THEN 'Out of coverage area'
      WHEN dr.city_group IN ('RMSP', 'Rio de Janeiro', 'Belo Horizonte', 'Porto Alegre', 'Campinas')
      THEN dr.city_group
    END AS city_group,
    ac.affiliate_volumetry
  FROM dw_datamarts.lead_listing_flows AS lf
  LEFT JOIN dw_datamarts.affiliates_clusters AS ac
    ON ac.sk_user = lf.sk_user_lead_affiliate
    AND ac.month_start = DATE_TRUNC('MONTH', (
      TO_DATE(CAST(lf.sk_lead_date AS STRING), 'yyyyMMdd')
    ))
  ...
  LEFT JOIN datalake_ebdb_listing.house AS rbh
    ON rbh.id = lf.sk_house_listing / 1000 AND rbh.is_3p_supply_bh
  WHERE
  ...
  SELECT
    ...
    mo.id_agent,
    CASE
      WHEN mo.payment_method IN ('1', '7')
      THEN 'Financiado'
      WHEN mo.payment_method = '2'
      THEN 'À Vista'
      WHEN mo.payment_method = '3'
      THEN 'À Vista + FGTS'
      WHEN mo.payment_method = '4'
      THEN 'Financiado + FGTS'
    ...
    (
      mo.listing_sale_price - mo.price_offered_by_buyer
    ) / mo.listing_sale_price AS offer_discount
  FROM datalake_firestore.monday AS mo
  ...
  SELECT
    ...
    TO_DATE(CAST(sk_booking_created_date AS STRING), 'yyyyMMdd') AS dt_created,
    TO_DATE(CAST(sk_visit_completed_date AS STRING), 'yyyyMMdd') AS dt_completed,
  ...
  LEFT JOIN datalake_hub_services_clean.business_unit AS hs
  ...
  LEFT JOIN dw_brokers.dim_broker AS cd
    ON cd.sk_broker = fo.sk_broker_demand
  LEFT JOIN dw_brokers.dim_broker AS cs
  ...
  FROM fact_os
  FULL OUTER JOIN fact_oa
    ON fact_os.sk_offer = fact_oa.sk_offer
  FULL OUTER JOIN fact_ccv
  ...
  SELECT
    ...
    dr.city_group
  FROM dw_sale.fact_sale_flows AS fsf
  JOIN dw_public.dim_region AS dr
  ...
  SELECT
    ...
    sc.ccv_date,
    sc.hub_offer
  FROM monday_adjusted AS offers
  FULL OUTER JOIN sale_closing AS sc
  ...
  SELECT
    ...
    db.hub_visit,
    sde.hub_offer
  FROM sale_demand_events AS sde
  FULL OUTER JOIN sale_bookings AS db
  ...
  SELECT
    ...
    rbh.partner_3p_supply AS supply_3pbh_partner,
    CASE
      WHEN NOT COALESCE(sdr.city_group, sdc.city_group) IN ('RMSP', 'Rio de Janeiro', 'Belo Horizonte', 'Porto Alegre', 'Campinas')
      THEN 'Out of coverage area'
    ...
    sdc.hub_offer,
    sdc.hub_visit
  FROM sale_demand_events_complete AS sdc
  LEFT JOIN dw_sale.fact_sale_flows AS fsf
    ON sdc.id_buyer = CAST(fsf.sk_buyer AS STRING)
    AND sdc.id_house = CAST(fsf.sk_house AS STRING)
  LEFT JOIN sale_demand_region AS sdr
    ON CAST(sdr.id_house AS STRING) = sdc.id_house
  LEFT JOIN datalake_ebdb_listing.house AS rbh
    ON rbh.id = sdc.id_house AND rbh.is_3p_supply_bh
), p2fc AS (
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', TO_DATE(CAST(slf.sk_prospect_date AS STRING), 'yyyyMMdd'))),
          TO_DATE(DATE_TRUNC('WEEK', TO_DATE(CAST(sk_first_contact_date AS STRING), 'yyyyMMdd')))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
    ...
    CAST(NULL AS BIGINT) AS q2aq,
    CAST(NULL AS BIGINT) AS aq2o,
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(
            DATE_TRUNC('WEEK', TO_DATE(CAST(slf.sk_first_contact_date AS STRING), 'yyyyMMdd'))
          ),
          TO_DATE(DATE_TRUNC('WEEK', TO_DATE(CAST(sk_qualified_date AS STRING), 'yyyyMMdd')))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', TO_DATE(CAST(slf.sk_qualified_date AS STRING), 'yyyyMMdd'))),
          TO_DATE(
            DATE_TRUNC('WEEK', TO_DATE(CAST(sk_available_qualified_date AS STRING), 'yyyyMMdd'))
          )
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
    ...
    CAST(NULL AS BIGINT) AS fc2q,
    CAST(NULL AS BIGINT) AS p2q,
    COUNT(slf.sk_qualified_date) AS q2aq,
    CAST(NULL AS BIGINT) AS aq2o,
    CAST(NULL AS BIGINT) AS q2o,
    CAST(NULL AS BIGINT) AS o2fl,
    CAST(NULL AS BIGINT) AS fl2ccv,
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(
            DATE_TRUNC('WEEK', TO_DATE(CAST(slf.sk_available_qualified_date AS STRING), 'yyyyMMdd'))
          ),
          TO_DATE(DATE_TRUNC('WEEK', TO_DATE(CAST(sk_opportunity_date AS STRING), 'yyyyMMdd')))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
    ...
    CAST(NULL AS BIGINT) AS p2q,
    CAST(NULL AS BIGINT) AS q2aq,
    COUNT(slf.sk_available_qualified_date) AS aq2o,
    CAST(NULL AS BIGINT) AS q2o,
    CAST(NULL AS BIGINT) AS o2fl,
    CAST(NULL AS BIGINT) AS fl2ccv,
    CAST(NULL AS BIGINT) AS vb2vc,
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', TO_DATE(CAST(slf.sk_opportunity_date AS STRING), 'yyyyMMdd'))),
          TO_DATE(DATE_TRUNC('WEEK', TO_DATE(CAST(sk_first_listing_date AS STRING), 'yyyyMMdd')))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(
            DATE_TRUNC('WEEK', TO_DATE(CAST(slf.sk_first_listing_date AS STRING), 'yyyyMMdd'))
          ),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_ccv_signed AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
    ...
    CAST(NULL AS BIGINT) AS ccv2pc
  FROM sale_listing_flows_adjust AS slf
  LEFT JOIN sale_demand_classification AS sde
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_created AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_completed AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_created AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_offer_sent AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_created AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_offer_accepted AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_offer_sent AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_deal_qualified AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_ccv_signed AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_diligence_started_legaut AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_diligence_started_legaut AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_diligence_ended_legaut AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  FROM sale_demand_classification
  WHERE
    NOT dt_diligence_started_legaut IS NULL
  GROUP BY
    1,
    2,
    3,
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_diligence_ended_legaut AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_diligence_started_legal AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  FROM sale_demand_classification
  WHERE
    NOT dt_diligence_ended_legaut IS NULL
  GROUP BY
    1,
    2,
    3,
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_diligence_started_legal AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_diligence_ended_legal AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  FROM sale_demand_classification
  WHERE
    NOT dt_diligence_started_legal IS NULL
  GROUP BY
    1,
    2,
    3,
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_diligence_ended_legal AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_diligence_ended AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  FROM sale_demand_classification
  WHERE
    NOT dt_diligence_ended_legal IS NULL
  GROUP BY
    1,
    2,
    3,
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_ccv_signed AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_credit_started AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_credit_started AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_credit_approved AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_ccv_signed AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_notes_registry_ended AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_credit_approved AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_finan_started AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  FROM sale_demand_classification
  WHERE
    NOT dt_credit_approved IS NULL
  GROUP BY
    1,
    2,
    3,
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_finan_started AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_finan_ended AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
    ...
    CAST(NULL AS BIGINT) AS credsent2finstart,
    CAST(NULL AS BIGINT) AS ccv2finstart,
    COUNT(DISTINCT id_offer) AS finstart2finended,
    CAST(NULL AS BIGINT) AS ccv2mi,
    CAST(NULL AS BIGINT) AS mi2ma,
    CAST(NULL AS BIGINT) AS ccv2ma,
    CAST(NULL AS BIGINT) AS ccv2pc
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_ccv_signed AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_matricula_inicio AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_matricula_inicio AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_matricula_atualizada AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  FROM sale_demand_classification
  WHERE
    NOT dt_matricula_inicio IS NULL
  GROUP BY
    1,
    2,
    3,
  ...
  SELECT
    ...
    CASE
      WHEN CAST((
        -DATEDIFF(
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_ccv_signed AS DATE))),
          TO_DATE(DATE_TRUNC('WEEK', CAST(dt_payment_concluded AS DATE)))
        ) / 7
      ) AS INT) < 20
      THEN 'W' || CAST((
        -DATEDIFF(
  ...
  RIGHT JOIN dw_public.dim_date AS dd
