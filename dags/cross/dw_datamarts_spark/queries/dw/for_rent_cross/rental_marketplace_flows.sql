...
-- RENT FLOWS --
  SELECT
    ...
    rf.sk_client,
    rf.sk_house_listing,
    COUNT(DISTINCT rf.sk_rf) AS rent_flows,
...
demand_funnel AS (
  ...
  SELECT
    ...
    COUNT(NULL) AS cum_rent_flows,
    COUNT(NULL) AS visits_booked,
    COUNT(DISTINCT flrf.sk_offer) AS offers_submitted,
    COUNT(NULL) AS offers_accepted,
    COUNT(NULL) AS credits_approved,
    COUNT(NULL) AS contracts_signed
  FROM
  ...
  SELECT
    ...
    COUNT(NULL) AS visits_booked,
    COUNT(NULL) AS offers_submitted,
    COUNT(DISTINCT flrf.sk_offer) AS offers_accepted,
    COUNT(NULL) AS credits_approved,
    COUNT(NULL) AS contracts_signed
  FROM
    datalake_listing_temp.fact_listing_rent_flows_house AS flrf
...
FROM
  ...
  LEFT JOIN tenant_prospect_interactions AS tpi
    ...
    AND DATEDIFF(ap.date, tpi.dt_event) BETWEEN 0 AND 28 --Tenant Prospects are considered "active" if their last rent flow occurred within 28 days
...
SELECT DISTINCT
  ...
  END AS is_active_tp,
  CASE
    WHEN GREATEST(lwd.rent_flows, lwd.visits_booked, lwd.offers_submitted, lwd.offers_accepted, lwd.credits_approved, lwd.contracts_signed) > 0
      THEN TRUE
    ELSE FALSE
  END AS had_demand,
