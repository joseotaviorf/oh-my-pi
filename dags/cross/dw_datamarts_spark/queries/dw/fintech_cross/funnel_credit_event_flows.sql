WITH
    ...
    rent_flows_base AS (
        ...
        FROM
            ...
            INNER JOIN dw_public.dim_date dd ON f.sk_offer_approved_date = dd.sk_date
            INNER JOIN max_contract mc ON f.sk_proposal = mc.sk_proposal
    ...
    events_dates AS (
        ...
        FROM
            ...
            INNER JOIN dw_public.dim_proposal AS dp ON rfb.sk_proposal = dp.sk_proposal
            INNER JOIN dw_public.dim_date AS dd_os ON rfb.sk_offer_submitted_date = dd_os.sk_date
            INNER JOIN dw_public.dim_date AS dd_oa ON rfb.sk_offer_approved_date = dd_oa.sk_date
            INNER JOIN dw_public.dim_date AS dd_es ON rfb.sk_last_credit_evaluation_init = dd_es.sk_date
            INNER JOIN dw_public.dim_date AS dd_evp ON rfb.sk_last_credit_evaluation_positive = dd_evp.sk_date
            INNER JOIN dw_public.dim_date AS dd_evn ON rfb.sk_last_credit_evaluation_negative = dd_evn.sk_date
            INNER JOIN dw_public.dim_date AS dd_ds ON rfb.sk_tenant_first_doc_sent_date = dd_ds.sk_date
            INNER JOIN dw_public.dim_date AS dd_dc ON rfb.sk_tenant_doc_complete_date = dd_dc.sk_date
            INNER JOIN dw_public.dim_date AS dd_da ON rfb.sk_credit_analysis_approved_date = dd_da.sk_date
            INNER JOIN dw_public.dim_date AS dd_guarantee ON rfb.sk_guarantee_paid_date = dd_guarantee.sk_date
            INNER JOIN dw_public.dim_date AS dd_cc ON rfb.sk_contract_created_date = dd_cc.sk_date
            INNER JOIN dw_public.dim_date AS dd_cs ON rfb.sk_contract_signed_date = dd_cs.sk_date
    ),
    days_diff_and_cohorts_maturation AS (
        SELECT
            ...
            ) AS days_evaluation_started_to_evaluation_approved,
            DATEDIFF (
                last_evaluation_started_date,
                contract_signed_date
            ) AS days_evaluation_started_to_contract_signed,
            DATEDIFF (
                evaluation_approved_date,
                document_first_sent_date
            ) AS days_evaluation_approved_to_document_first_sent,
            DATEDIFF (
                document_first_sent_date,
                last_document_approved_date
            ) AS days_document_first_sent_to_document_approved,
...
FROM
    rent_flows_base AS rfb
    INNER JOIN events_dates AS ed ON rfb.sk_proposal = ed.sk_proposal
    INNER JOIN days_diff_and_cohorts_maturation AS ddcm ON rfb.sk_proposal = ddcm.sk_proposal
    INNER JOIN dw_datamarts.funnel_demand_flows AS fdf ON rfb.sk_rent_flow = fdf.sk_rent_flow
