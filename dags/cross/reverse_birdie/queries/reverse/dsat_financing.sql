  ...
  SELECT
    ...
    ARRAY_JOIN(
      COLLECT_LIST(DISTINCT rc.answer_content) FILTER(WHERE
        rc.id_question IN ('2732164', '2718008')),
      ' | '
    ) AS reason_financ,
    ARRAY_JOIN(
      COLLECT_LIST(DISTINCT rc.answer_content) FILTER(WHERE
        rc.id_question = '2718009'),
      ' | '
    ) AS comment_csat
...
SELECT
  ...
  bf.rating,
  CASE
    WHEN NULLIF(bf.comment_csat, '') IS NULL AND NULLIF(bf.reason_financ, '') IS NULL
    THEN NULL
    ...
    AND NOT NULLIF(bf.reason_financ, '') IS NULL
    AND bf.rating <= 2
