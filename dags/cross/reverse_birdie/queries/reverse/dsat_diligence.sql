...
base_offers_internal_vendors AS (
    SELECT
        ...
        CASE 
            ...
            AND credit_model IN ('UNDEFINED','ATTA') THEN TRUE
            WHEN payment_method IN ('FINANCED_USING_FGTS', 'FINANCED') 
            AND credit_model = 'EXTERNAL' THEN FALSE
...
offer AS (
    SELECT 
        ...
        dsa.is_ccv_canceled
    FROM dw_sale.fact_offers fo
    JOIN dw_sale.dim_sale_agreement dsa 
        ON fo.sk_offer = dsa.sk_offer 
    LEFT JOIN dw_public.dim_region dim 
...
tickets AS (
    ...
    QUALIFY ROW_NUMBER() OVER (PARTITION BY em.id_user ORDER BY ts_sla_started DESC) = 1
),
...
fact_closing_flows AS (
    ...
    LEFT JOIN dw_sale.dim_sale_agreement dsa 
...
franchise_last_pre_analysis AS (
    ...
    LEFT JOIN dw_atta.dim_franchise_atta dfr 
...
nps AS (
    SELECT 
        ...
        ARRAY_JOIN(ARRAY_AGG(DISTINCT rc.answer_content) 
            FILTER(WHERE rc.id_question IN ('2732165','2718002')), ' | ') AS reason_dd,
        ARRAY_JOIN(ARRAY_AGG(DISTINCT rc.answer_content) 
            FILTER(WHERE rc.id_question = '2718003'), ' | ') AS comment_csat
    FROM datalake_survicate.response_content rc
    LEFT JOIN datalake_survicate.survey_questions sq 
        ON sq.id_question = rc.id_question
    LEFT JOIN datalake_survicate.survey_responses sr 
...
base_final as (
    SELECT
        ...
        fcf.city_group
    FROM nps
    LEFT JOIN tel_user n 
        ON nps.phone = n.tel
    LEFT JOIN post_specialist ps 
        ON ps.sk_offer = nps.offer_id
    LEFT JOIN fact_closing_flows fcf 
        ON fcf.sk_offer = nps.offer_id
    LEFT JOIN dw_sale.dim_sale_agreement a 
        ON a.sk_offer = nps.offer_id
    LEFT JOIN dim_user_sale dus 
        ON dus.sk_offer = nps.offer_id
    LEFT JOIN base_offers_internal_vendors iv 
        ON iv.sk_offer = nps.offer_id
    LEFT JOIN franchise_last_pre_analysis fran 
...
WHERE 
  rating IS NOT NULL
  AND bf.posted_at >= DATE('{load_start_date}')
