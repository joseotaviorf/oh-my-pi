      ...
      INNER JOIN (
        ...
        FROM (
          SELECT
            ...
            LAST_DAY(
              COALESCE(dc2.dt_annulment, CURRENT_DATE - INTERVAL '1' DAY)
            ) AS seq_end
          FROM
...
listings AS (
  SELECT
    ...
    hl.sk_house_listing,
    dc.sk_contract,
    DATE_FORMAT(dc.ts_signature, 'yyyy-mm-dd')                           AS ts_signature,
    DATE_FORMAT(COALESCE(dc.dt_start, dc.dt_entrance), 'yyyy-mm-dd')     AS inicio_contrato,
    DATE_FORMAT(dc.dt_annulment, 'yyyy-mm-dd')                           AS dt_annulment,
    DATE_FORMAT(hl.ts_early_demand_started, 'yyyy-mm-dd')                AS early_demand_started,
    rl.sk_house_listing                                                  AS sk_relisting,
    DATE_FORMAT(rl.ts_listing_version_start, 'yyyy-mm-dd')               AS dt_relisting
  ...
  WHERE
    ...
    AND DATE(dc.dt_annulment) >= '2022-01-01'
),
