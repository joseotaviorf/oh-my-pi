...
if __name__ == "__main__":
    ...
    if df is None:
        ...
        cols_structs = [
            c.name for c in df.schema if isinstance(c.dataType, StructType)
        ]  # get all struct columns
        for col_name in cols_structs:
            df = df.withColumn(
                col_name, F.to_json(F.col(col_name))
            )  # cast from struct to string keeping json format
