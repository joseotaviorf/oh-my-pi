...
SELECT
  ...
  COALESCE(CAST(REPLACE(SUBSTRING(vse.ts_first_reeschedule,1, 10),'-','') AS BIGINT), -1) AS sk_first_visit_reschedule_date,
  COALESCE(CAST(REPLACE(SUBSTRING(vse.ts_last_reeschedule,1, 10),'-','') AS BIGINT), -1) AS sk_last_visit_reschedule_date,
