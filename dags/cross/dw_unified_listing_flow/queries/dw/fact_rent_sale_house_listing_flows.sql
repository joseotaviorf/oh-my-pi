...
potential_listings AS (
    SELECT
        lfrl.listing_flow_business_context,
        ...
        CASE
          WHEN lfrl.listing_flow_business_context = 'RENT'
            THEN lfrl.is_b2b
          WHEN lfrl.listing_flow_business_context = 'SALE'
            THEN (plrl.is_b2b OR lfrl.is_b2b) -- Using business rules for both constraints of old b2b and new one
        END AS is_b2b,
      ...
      FROM datalake_unified_listing_flow.listing_flows_with_reprocessed_leads lfrl
      LEFT JOIN datalake_lead_tracking.lead_first_event_tracking lfet
        ON lfet.id_lead = lfrl.id_lead
      LEFT JOIN datalake_unified_potential_listing.potential_listing_lead_tasks pllt
        ON pllt.id = lfrl.id
            AND pllt.listing_flow_business_context = lfrl.listing_flow_business_context
      LEFT JOIN datalake_unified_potential_listing.potential_listing_reprocessed_leads plrl
        ON plrl.id = lfrl.id
            AND plrl.listing_flow_business_context = lfrl.listing_flow_business_context
      LEFT JOIN dw_public.dim_region dr
        ON dr.sk_region = lfrl.id_region
      LEFT JOIN datalake_unified_potential_listing.potential_listing_b2b plb2b
        ON plb2b.id = lfrl.id
          AND plb2b.listing_flow_business_context = lfrl.listing_flow_business_context
),
...
SELECT -- [ODS] This table was migrated from ODS flow and needs a future refactoring to remove castings and renamings
  atax.listing_flow_business_context,
