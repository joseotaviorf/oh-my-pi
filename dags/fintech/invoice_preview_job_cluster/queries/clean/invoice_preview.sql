SELECT
    ...
    CAST(
        NULLIF(REPLACE (REPLACE (amount, '.', ''), ',', '.'), '') AS double
    ) AS paid_amount,
    ...
    ELEMENT_AT (
        SPLIT (
            invoice_filename,
            '(invoice-preview-)|(-\\d{{2}}\\.csv)'
        ),
        -2
    ) AS invoice_accrual_year_month,
    ...
    TO_DATE (
        NULLIF(`landlord-invoice-created-at`, ''),
        'y-M-d'
    ) AS dt_landlord_invoice_created_at,
    year,
    month,
    day,
    ts_load
FROM
    datalake_invoice_preview_job_cluster_raw.invoice_preview
