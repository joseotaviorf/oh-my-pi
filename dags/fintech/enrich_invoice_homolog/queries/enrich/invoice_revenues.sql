...
rental_brokerage_fee_5A AS (
	WITH
        invoice_list AS (
          ...
          FROM
              datalake_invoice_homolog.invoice_all
          WHERE
        ...
        FROM datalake_invoice_homolog.invoice_all AS ia
...
rental_agents_commission AS (
	...
	invoice_list AS (
		SELECT
			*,
			ROW_NUMBER() OVER (PARTITION BY  id_contract, accrual_year_month ORDER BY accrual_year_month) AS seqnum
		FROM datalake_invoice_homolog.invoice_all
		WHERE
...
partner_revenue_share_brokerage AS (
    ...
    FROM
        datalake_invoice_homolog.invoice_all AS ia
    LEFT JOIN
        discount AS d
            ON ia.id_invoice_entry = d.id_invoice_entry
    WHERE
