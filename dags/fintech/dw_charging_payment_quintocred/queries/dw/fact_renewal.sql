...
get_rule_dt_renewal AS (
  SELECT 
    CASE 
      ...
      WHEN f.gateway_payment IN ('CREDIT_CARD', 'ANNUAL_CREDIT_CARD', 'PIXAR') OR (max_id_type = 4) THEN 'FLUXO B - OLD CHECKOUT'
...
class_building_lvl1 AS (
  SELECT *,
    CASE 
      ...
      WHEN flux_determination = 'FLUXO B - OLD CHECKOUT' AND status_payment IN ('SUCCESS', 'PROCESSING') AND gateway_payment IN ('CREDIT_CARD', 'PIXAR', 'ANNUAL_CREDIT_CARD') THEN 'B1'
      WHEN flux_determination = 'FLUXO B - OLD CHECKOUT' AND NOT(status_payment IN ('SUCCESS', 'PROCESSING') AND gateway_payment IN ('CREDIT_CARD', 'PIXAR', 'ANNUAL_CREDIT_CARD')) THEN 'B2'
      ...
      WHEN flux_determination = 'FLUXO D - NEW CHECKOUT' AND ((renewal_link_status = 'LINK_UNPAID' AND partial_link_status IS NULL) OR (renewal_link_status IS NULL AND partial_link_status = 'LINK_UNPAID')) AND (status_payment IN ('SUCCESS') AND gateway_payment IN ('CREDIT_CARD', 'PIXAR', 'ANNUAL_CREDIT_CARD')) AND ref_month_renewal_coalesce <= date('2024-9-1') THEN 'D5'
