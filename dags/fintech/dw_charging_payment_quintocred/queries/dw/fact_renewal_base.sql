...
agg_data AS (
    SELECT 
        ...
        ARRAY_AGG(DISTINCT CASE WHEN mob_delinquency_distance <= 0 THEN COALESCE(arr_id_type, ARRAY(0)) END) AS arr_id_type_valid,
        ARRAY_AGG(DISTINCT CASE WHEN mob_delinquency_distance <= 0 THEN COALESCE(arr_dt_created, ARRAY(DATE '1970-01-01')) END) AS arr_dt_created_valid,
        ARRAY_AGG(DISTINCT CASE WHEN mob_delinquency_distance <= 0 THEN COALESCE(arr_id_delinquency, ARRAY(0)) END) AS arr_id_delinquency_valid,
        ARRAY_AGG(DISTINCT CASE WHEN mob_delinquency_distance <= 0 THEN COALESCE(arr_dt_paid, ARRAY(DATE '1970-01-01')) END) AS arr_dt_paid_valid,
        ...
        ARRAY_AGG(DISTINCT CASE WHEN mob_delinquency_distance <= 0 THEN COALESCE(arr_id_type_active, ARRAY(0)) END) AS arr_id_type_valid_active,
        ARRAY_AGG(DISTINCT CASE WHEN mob_delinquency_distance <= 0 THEN COALESCE(arr_dt_created_active, ARRAY(DATE '1970-01-01')) END) AS arr_dt_created_valid_active,
        ARRAY_AGG(DISTINCT CASE WHEN mob_delinquency_distance <= 0 THEN COALESCE(arr_dt_due_active, ARRAY(DATE '1970-01-01')) END) AS arr_dt_due_valid_active,
        ARRAY_AGG(DISTINCT CASE WHEN mob_delinquency_distance <= 0 THEN COALESCE(arr_id_delinquency_active, ARRAY(0)) END) AS arr_id_delinquency_valid_active,
        ...
        ARRAY_AGG(DISTINCT CASE WHEN mob_delinquency_distance <= 0 THEN COALESCE(arr_id_type_archived, ARRAY(0)) END) AS arr_id_type_archived_valid,
        ARRAY_AGG(DISTINCT CASE WHEN mob_delinquency_distance <= 0 THEN COALESCE(arr_dt_created_archived, ARRAY(DATE '1970-01-01')) END) AS arr_dt_created_archived_valid,
        ARRAY_AGG(DISTINCT CASE WHEN mob_delinquency_distance <= 0 THEN COALESCE(arr_dt_due_archived, ARRAY(DATE '1970-01-01')) END) AS arr_dt_due_archived_valid,
        ARRAY_AGG(DISTINCT CASE WHEN mob_delinquency_distance <= 0 THEN COALESCE(arr_id_delinquency_archived, ARRAY(0)) END) AS arr_id_delinquency_archived_valid,
        ARRAY_AGG(DISTINCT COALESCE(arr_dt_due, ARRAY(DATE '1970-01-01'))) AS arr_arr_dt_due,
        ARRAY_AGG(DISTINCT COALESCE(arr_dt_paid, ARRAY(DATE '1970-01-01'))) AS arr_arr_dt_paid,
        ARRAY_AGG(DISTINCT COALESCE(arr_id_type, ARRAY(0))) AS arr_id_type,
        ARRAY_AGG(DISTINCT COALESCE(arr_dt_created, ARRAY(DATE '1970-01-01'))) AS arr_dt_created,
        ARRAY_AGG(DISTINCT COALESCE(arr_id_delinquency, ARRAY(0))) AS arr_id_delinquency,
        ARRAY_AGG(DISTINCT COALESCE(arr_id_delinquency_active, ARRAY(0))) AS arr_id_delinquency_active,
        ARRAY_AGG(DISTINCT COALESCE(arr_id_delinquency_archived, ARRAY(0))) AS arr_id_delinquency_archived,
