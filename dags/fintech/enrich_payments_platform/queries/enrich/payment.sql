WITH subscription_invoices AS (
  ...
  -- Webhook lookup keyed by payment_method. The original
  -- `ON pw.id_pix = pix.id OR pw.id_bolecode = bc.id` is a non-equi condition,
  -- which Spark can only plan as BroadcastNestedLoopJoin on EMR.
  LEFT JOIN (
    SELECT
      id_pix AS id_webhook_target,
      'PIX' AS webhook_kind,
      error_message
    FROM datalake_checkout_clean.pix_webhook
    WHERE
      NOT id_pix IS NULL
    UNION ALL
    SELECT
      id_bolecode AS id_webhook_target,
      'BOLECODE' AS webhook_kind,
      error_message
    FROM datalake_checkout_clean.pix_webhook
    WHERE
      NOT id_bolecode IS NULL
  ) AS pw
    ON pw.webhook_kind = ch.payment_method
    AND pw.id_webhook_target = CASE ch.payment_method
      WHEN 'PIX'
      THEN pix.id
      WHEN 'BOLECODE'
      THEN bc.id
