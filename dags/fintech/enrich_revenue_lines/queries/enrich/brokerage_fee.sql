WITH brokerage_fee AS (
    WITH rental_brokerage_fee AS (
        ...
        LEFT JOIN datalake_ebdb_clean.contract_partnership_data pp
    ...
    rental_ciq_select_commission AS (
        ...
        WHERE
            ...
            AND die.description IN ('Taxa de corretagem - Corretor Parceiro Select')
        GROUP BY
            1,2,3,4,5,6,7,8,9,12,13,14,15
),
rental_brokerage_base AS (
    SELECT
    ...
    INNER JOIN 
        datalake_retsuko.invoice_entry die
            ON fie.id = die.id
    WHERE
        fie.id_invoice > 0
    QUALIFY
...
WHERE 
    id_invoice_entry NOT IN (SELECT id_invoice_entry FROM rental_brokerage_base WHERE description LIKE '%Desconto por cadastro com link de indicação%')
    AND id_invoice_entry NOT IN (SELECT id_invoice_entry FROM rental_brokerage_base WHERE description LIKE '%Crédito - Parcelamento corretagem - QuintoAndar%')
    AND id_invoice_entry NOT IN (SELECT id_invoice_entry FROM rental_brokerage_base WHERE description LIKE 'Parcela%')
UNION
SELECT
    *
FROM 
