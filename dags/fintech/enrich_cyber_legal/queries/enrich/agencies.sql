...
agency AS (
    SELECT
        ...
        agency_type,
        ts_start
    FROM datalake_cyber_legal_homolog_clean.agency
),
agency_group_base AS (
    SELECT
        ...
        FIRST_VALUE(IF(UPPER(a.agency_type) = "ASSESSORIA JURÍDICA", a.agency_name, NULL)) OVER(PARTITION BY ag.agency_group ORDER BY ag.percentage_remuneration DESC) AS juridical_agency,
        FIRST_VALUE(IF(UPPER(a.agency_type) = "ASSESSORIA CONVENCIONAL", a.agency_name, NULL)) OVER(PARTITION BY ag.agency_group ORDER BY ag.percentage_remuneration DESC) AS conventional_agency,
    ...
    FROM datalake_cyber_legal_homolog_clean.agency_group AS ag
