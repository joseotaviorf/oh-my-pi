...
open_installment AS (
  SELECT
    id_customer,
    id_installment,
    ROUND(SUM(IF(DATE(dt_due) >= CURRENT_DATE , amount_to_pay, 0)),2) AS due_amount,
    ROUND(SUM(IF(DATE(dt_due) < CURRENT_DATE , amount_to_pay, 0)),2) AS overdue_amount,
    MIN(dt_due) AS dt_next_due,
    MAX(IF(DATE(dt_due) < CURRENT_DATE , "S", "N")) AS agreement_in_delay
...
agreements AS (
  SELECT
    ...
    IFNULL(o.agreement_in_delay, "N") AS agreement_in_delay,
    ...
    ROUND(SUM(i.discount_amount),2) AS agreement_discount_amount,
...
SELECT DISTINCT
  ...
  a.agreement_promise,
  INT(a.number_of_installments) AS number_of_installments,
  IFNULL(pd.original_debit_amount, 0) AS original_debit_amount,
  IFNULL(a.total_negotiated_amount, 0) AS total_negotiated_amount,
  IFNULL(a.agreement_discount_amount, 0) AS agreement_discount_amount,
  IFNULL(a.down_payment_amount, 0) AS down_payment_amount,
  IFNULL(a.due_amount, 0) AS due_amount,
  IFNULL(a.overdue_amount, 0) AS overdue_amount,
  IFNULL(a.total_amount_paid, 0) AS total_amount_paid,
  a.total_next_due,
  DATE(ic.ts_canceled_installment) AS dt_cancellation,
  a.dt_next_due,
  a.dt_promisse,
  a.dt_due_promisse,
  NOW() AS ts_snapshot,
