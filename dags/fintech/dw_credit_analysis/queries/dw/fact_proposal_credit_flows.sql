...
early_demand_ranked AS (
  SELECT
    id_house AS sk_house,
    ts_early_demand_started,
    ROW_NUMBER() OVER (PARTITION BY id_house ORDER BY sk_house_listing DESC) AS rn
  FROM
...
client_max_funnel_drop_step_ranked AS (
  SELECT
    sk_client,
    dt_reference,
    funnel_drop_step_ordered,
    ROW_NUMBER() OVER (
      PARTITION BY CASE WHEN sk_client IS NULL THEN 1 ELSE sk_client END, DATE_TRUNC('month', dt_reference)
      ORDER BY IF(funnel_drop_step_ordered IS NULL, "Z", funnel_drop_step_ordered) DESC
    ) AS rn
  FROM
    add_dt_reference
  WHERE
    ...
    AND is_last_credit_evaluation = TRUE
),
...
FROM
  client_max_funnel_drop_step_ranked
WHERE
  rn = 1
)
SELECT
