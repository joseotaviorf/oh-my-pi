...
JOB_NAME = "load_nazare_into_datalake"
...
if __name__ == "__main__":
    ...
    logger.info(
        f"""
                ...
                block_list={block_list}, msg=Starting spark job...
        """
    )
    ...
    conn_config_json = dbutils.secrets.get(scope="quintoandar", key=DatabaseEnum.NAZARE)
    ...
    for table in tables:
        if table.table_name not in block_list:
            ...
            if df:
                ...
                logger.warning(
                    f"""m=__main__, table_name={table.table_name}, msg=No data returned from Production database."""
                )
