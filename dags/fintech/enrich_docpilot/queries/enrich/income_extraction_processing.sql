WITH extract_errors AS (
  ...
  FROM (
    SELECT
      id_external,
      CASE
        WHEN error_type IN ('INVALID_DATE_RANGE', 'LESS_THAN_RECOMMENDED_PERIOD')
        THEN error_type
        ELSE NULL
  ...
  SELECT DISTINCT
    ...
    committed_income,
    SIZE(extract_errors) AS number_of_errors,
    SIZE(all_documents) AS number_of_processed_documents,
    ts_created,
    ts_updated
  ...
  LATERAL VIEW OUTER
  EXPLODE(payslip_entries) AS entrie
  GROUP BY
    id_external,
    file_path,
    document_date
  ...
  FULL OUTER JOIN extract_errors AS er
    ON ed.id_external = er.id_external AND ed.file_path = er.file_path
  LEFT JOIN processing_errors AS pe
  ...
  FROM (
    SELECT
      id AS id_user,
      cpf,
      ROW_NUMBER() OVER (PARTITION BY cpf ORDER BY ts_created DESC) AS _w,
      ts_created
...
SELECT DISTINCT
  ...
  IF(NOT u.id_user IS NULL, TRUE, FALSE) AS is_main_user,
  id.is_suspicious,
  id.is_dates_divergent,
  CAST(id.document_date AS DATE) AS dt_document_date,
  CAST(id.document_end_date AS DATE) AS dt_document_end_date,
  CAST(id.document_start_date AS DATE) AS dt_document_start_date,
  id.ts_created,
  id.ts_updated,
  id.ts_load
FROM transform_income_data AS id
