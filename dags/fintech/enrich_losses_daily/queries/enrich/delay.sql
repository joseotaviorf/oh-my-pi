...
base_step0_delay AS(
  ...
  FROM 
    datalake_losses_daily.closing fc
  LEFT JOIN 
  ...
  -- There`s an exception of deals on january balance due to the accounting window on the moment of the emission of this closing, there`s an alignment between MIS and controlling regarding this.
    --AND (is_paid_in_closing_day is FALSE or dt_closing = '2023-01-31' or dt_closing <=date('2021-12-31'))
    AND (is_paid_in_closing_day is FALSE or dt_closing <=date('2021-12-31'))
...
FROM 
  base_step4_delay
where payment_status <> 'written down' and payment_status <> 'written-down'