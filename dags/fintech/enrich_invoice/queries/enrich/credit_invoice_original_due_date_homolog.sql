...
debt_forgiveness AS (
  ...
  INNER JOIN 
    datalake_retsuko.debt_forgiveness AS d
    ON i.id_external = d.id_invoice
  WHERE 
    status IN ("not-payable", "canceled")
),
fraud AS ( 
  SELECT
    i.id_external,
...
invoices AS (
    ...
    LEFT JOIN
        ...
        ON inv.id_contract = acc.id_contract
    LEFT JOIN 
        debt_forgiveness df
    ...
    WHERE
        acc.type = 'tenant'
        AND (inv.due_amount < 0 OR df.id_external IS NOT NULL OR f.id_external IS NOT NULL)
), 
invoices_fixed AS (
  SELECT
        inv.id,
  ...
  LEFT JOIN
    ...
    ON hl.month = MONTH(inv.static_original_due_date)
...
LEFT JOIN
    ...
    ON inv.id_contract = rsk_cntrct.id
LEFT JOIN
    datalake_ebdb_clean.contract AS ebdb_cntrct
    ON ebdb_cntrct.id = rsk_cntrct.id_external
WHERE
