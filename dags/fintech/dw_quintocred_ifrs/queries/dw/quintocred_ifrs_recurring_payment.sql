...
repasse_direto AS (
    ...
    ,recebimento_por_contrato_boleto_billing AS (
        SELECT DISTINCT
            ...
            vp.total_package_amount AS valor_pacote,
            ROW_NUMBER() OVER (PARTITION BY COALESCE(sap.id_contract,e.propose),ADD_MONTHS(DATE(CONCAT(CAST(i.accrual_year AS VARCHAR(10)),'-',CAST((i.accrual_month) AS VARCHAR(10)),'-','01')),1 ) ORDER BY DATE(b.ts_paid) DESC, DATE(b.ts_created) DESC) AS rn
        FROM
