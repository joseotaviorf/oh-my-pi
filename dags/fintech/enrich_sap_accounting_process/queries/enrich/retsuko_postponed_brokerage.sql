...
sap_ledger_sibling AS (
  ...
  WHERE
    ...
    AND l.accounting_rule LIKE '%brokerage-quinto-andar-postponed%'
...
errors_base AS (
  SELECT
    ...
    MIN(CASE
      ...
      WHEN sib.sibling_accounts IS NOT NULL THEN CONCAT('accounted at payment time in ', sib.sibling_accounts, ' instead of ', r.account_number)
...
assertions_base AS (
  SELECT
    ('RTSK-PSTB'||'-'||COALESCE(id_finance_entity_entry, id_finance_entity)||'-'||COALESCE(account_number, '')) AS id_accounting_process,
    id_business_entity,
    id_finance_entity,
    id_finance_entity_entry,
    version,
