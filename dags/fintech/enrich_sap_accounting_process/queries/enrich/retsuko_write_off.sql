...
df AS (
    SELECT 
        ...
        MIN(CASE
          ...
          WHEN s.hash IS NULL AND (r.id_finance_entity IS NULL OR r.status = 'failed' OR r.failed_reason IS NOT NULL) THEN 'source failure'
          WHEN s.hash IS NULL AND (sg.id_feature IS NULL OR sg.sync_sap_job_status = 'error' OR sg.webhook_error IS NOT NULL) THEN 'gateway failure'
          ELSE 'unknown failure'
        END) AS accounting_process_status,
        MIN(CASE
          WHEN s.hash IS NULL AND r.id_finance_entity IS NULL THEN 'source not found'
          WHEN s.hash IS NULL AND r.status = 'failed' THEN r.failed_reason
          WHEN s.hash IS NULL AND sg.id_feature IS NULL THEN 'gateway not found'
          WHEN s.hash IS NULL AND sg.sync_sap_job_status = 'error' THEN sg.webhook_error
          WHEN s.hash IS NULL AND r.id_finance_entity IS NOT NULL AND sg.id_feature IS NOT NULL THEN 'sap not found'
          ELSE NULL
