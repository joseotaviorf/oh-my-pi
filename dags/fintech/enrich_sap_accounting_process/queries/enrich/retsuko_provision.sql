...
retsuko AS (
  ...
  WHERE
    ...
    (
      ...
      OR (DATE(e.ts_created) >= '2026-09-01' AND ct.landlord_legal_person = 'physical' AND e.bill_item IN ('entry.bill-item/adm-fee', 'entry.bill-item/igpm-adm-fee', 'entry.bill-item/ipca-adm-fee', 'entry.bill-item/adjustment-agreement-adm-fee', 'entry.bill-item/lockin'))
      OR (DATE(e.ts_created) >= '2026-09-01' AND e.bill_item IN ('entry.bill-item/service-fee'))
    )
