WITH francesinha AS (
    SELECT
        ...
        bank_account,
        dt_paid,
        SUM(IF(occurrence_code = '00', paid_amount, paid_amount * -1)) AS paid_amount
    FROM
...
SELECT DISTINCT
    ...
    CASE
        WHEN (payment_status = 'paid' OR payment_status IS NULL) AND ((ABS(f.paid_amount) = ABS(sap.paid_amount)) OR (ABS(cap.paid_amount) = ABS(sap.paid_amount))) THEN TRUE
