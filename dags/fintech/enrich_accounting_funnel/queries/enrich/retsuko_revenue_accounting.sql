...
errors_base AS (
  ...
  LEFT JOIN
    sap_ledger sl
      ON sl.hash = sg.hash
      AND r.accounting_number = sl.account_number
...
SELECT
  ('FR-RRA'||'-'||IF(id_finance_entity_entry IS NOT NULL, id_finance_entity_entry, id_finance_entity)||'-'|| accounting_number) AS id_accounting_process,
  id_business_entity,
  id_finance_entity,
  id_finance_entity_entry,
  version,
