...
contract_analyst_annulment_date AS (
    ...
    FROM (
        SELECT
            ...
            c_aud.id_house,
            c_aud.rev,
            ROW_NUMBER() OVER(PARTITION BY c_aud.id_contract ORDER BY ure.ts_revision) AS rn,
        ...
        FROM
            datalake_ebdb_clean.contract_aud AS c_aud
        INNER JOIN
...
get_contract_by_month AS (
    SELECT
        ...
        day
    FROM
        datalake_ebdb_transactional.contrato
    WHERE
