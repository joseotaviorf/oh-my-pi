...
JOB_NAME = "load_meetcall_into_datalake"
...
def _send_warning(dbutils, environment, table_name):
    ...
    message_content = (
        ...
        f"Table: `{table_name}`\n"
        f"Status: *FAILED*\n"
        f"Error: *There is no data in MEETCALL database (SQL SERVER) for table {table_name}*\n"
    )
...
if __name__ == "__main__":
    ...
    conn_config = _get_conn_config(dbutils, DatabaseEnum.MEETCALL)

    sqlserver_consumer = SQLServerConsumer(conn_config, spark_client)

    meetcall_table_name = table_name.upper()
    if extraction_type == "incremental" and date_filter_columns:
        df = sqlserver_consumer.get_incremental_data_from_table(meetcall_table_name, date_filter_columns, load_start_date, load_end_date)
    else:
        df = sqlserver_consumer.get_data_from_table(meetcall_table_name)
