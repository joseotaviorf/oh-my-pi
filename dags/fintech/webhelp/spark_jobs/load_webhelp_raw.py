...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "partition_folder",
        help="Indicates if the file is inside a partition folder or not (general_folder)",
    )
    ...
    spark.conf.set(
        "fs.azure.account.key." + azure_account_name + ".blob.core.windows.net",
        azure_account_key,
    )
    ...
    dates_to_ingest = (
        _generate_date_range(load_start_date, load_end_date)
        if extraction_type == "incremental"
        else _generate_date_range(load_end_date, load_end_date)
    )

    logger.info(
        f"Dates do process: load_start_date={load_start_date}, load_end_date = {load_end_date}"
    )
    ...
    for date_to_ingest in dates_to_ingest:
        ...
        try:
            ...
            logger.info(
                f"Original dataframe - Displaying a sample of 2 rows out of a total of {df.count()} rows"
            )
            df.show(2)
        ...
        logger.info(
            f"Final dataframe - Displaying a sample of 5 rows out of a total of {df.count()} rows"
        )
        df.show(5)

        logger.info(
            f"""Starting loading data into datalake database_name = {database_name}, table_name = {table_name}, extraction_type == {extraction_type}"""
        )
        if not df.isEmpty():
            if extraction_type == "incremental":
                df = (
