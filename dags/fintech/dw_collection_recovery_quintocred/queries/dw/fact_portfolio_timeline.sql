    ...
    SELECT
        ...
        CASE
            WHEN fn.id_operator IN (
                                ...
                                'LUCASGOM',
                                'AMANARDI',
                                'TALITAPE') THEN "TIME INTERNO"
            WHEN FN.ID_OPERATOR   = "IAFWS"     THEN "IAF"
            WHEN FN.ID_OPERATOR   = "BRBOTS"       THEN "BRBOTS"
            WHEN FN.ID_OPERATOR   IN ("DIGTECH","DIGITECH") THEN "DIGTECH"
        ELSE "TIME NAO LOCALIZADO"
        END AS team,
        ROUND(SUM(FNI.MAIN_AMOUNT),2) AS original_value,
        ROUND(SUM(FNI.paid_amount),2) AS paid_amount
    FROM
        dw_collection_recovery_quintocred.fact_negotiation_installment AS FNI
    LEFT JOIN
