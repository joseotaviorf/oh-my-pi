...
SELECT DISTINCT
  ...
  file_name,
  ROW_NUMBER() OVER (PARTITION BY TRIM(our_number) ORDER BY dt_received) AS payment_attempts,
  IF(ROW_NUMBER() OVER (PARTITION BY TRIM(our_number) ORDER BY dt_received DESC) = 1, TRUE, FALSE) AS is_latest_attempt,
  due_amount,
  paid_amount,
  dt_due,
  dt_paid,
