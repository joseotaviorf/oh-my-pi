...
segment_allocation AS (
    SELECT
        ranked.dt_last_appearance,
        ranked.segment_name,
        ranked.audience_name,
        ranked.id_contract
    FROM (
        SELECT
            sd.dt_last_appearance,
            sd.segment_name,
            sd.audience_name,
            sd.id_contract,
            ROW_NUMBER() OVER (
                PARTITION BY sd.id_contract, sd.dt_last_appearance
                ORDER BY sd.ts_entered_segment DESC, sd.ts_entered_audience DESC
            ) AS rn_allocation
        FROM
            datalake_debt_recovery.segmentation_distribution AS sd
        WHERE
            sd.is_active = TRUE
    ) AS ranked
    WHERE
        ranked.rn_allocation = 1
),
contract_features_normalized AS (
    SELECT
        sda.id_contract AS sk_contract,
        sda.dt_last_appearance AS dt_reference,
        COALESCE(sda.audience_name, 'Unsegmented') AS segmentation
    FROM
        segment_allocation AS sda
    WHERE
        sda.dt_last_appearance IS NOT NULL
),
