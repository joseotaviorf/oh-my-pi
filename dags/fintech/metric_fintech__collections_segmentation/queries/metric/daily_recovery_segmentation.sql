WITH month_view AS (
  SELECT
    DATE_TRUNC('month', m.dt_reference) AS dt_month_ref,
    f.segmentation AS segmentation,
    m.sk_invoice,
  ...
  WHERE (f.segmentation NOT IN ('active-current', 'ended-current') AND t.wallet > 0)
  GROUP BY 1,2,3
),
...
full_month_view AS (
  SELECT
    ...
    COUNT(DISTINCT CASE
        WHEN f2.segmentation = m.segmentation THEN f2.sk_contract
        ELSE NULL
    END) AS n_contracts_at_reference,
    COUNT(DISTINCT m.sk_contract) AS n_contracts_acc,
    COUNT(DISTINCT CASE
        WHEN f2.segmentation = m.segmentation THEN f.sk_invoice
        ELSE NULL
    END) AS n_invoices_at_reference,
    COUNT(DISTINCT m.sk_invoice) AS n_invoices_acc,
    SUM(ABS(m.due_amount)) AS total_due_amount_acc,
    SUM(CASE
        WHEN f2.segmentation = m.segmentation THEN ABS(m.due_amount)
        ELSE NULL
    END) AS total_due_amount_at_reference,
    SUM(CASE
        WHEN f2.segmentation = m.segmentation THEN f.recovered_amount
        ELSE NULL
    END) AS recovered_amount_at_reference,
    COUNT(DISTINCT CASE WHEN f2.segmentation = m.segmentation AND f.recovered_amount > 0 THEN f.sk_invoice ELSE NULL END) AS recovered_invoices_at_reference,
    COUNT(DISTINCT CASE
    ...
    COUNT(DISTINCT CASE WHEN f2.segmentation = m.segmentation AND f.recovered_amount > 0 THEN f.sk_contract ELSE NULL END) AS recovered_contracts_at_reference,
    COUNT(DISTINCT CASE
    ...
    COUNT(DISTINCT CASE
        WHEN f2.segmentation = m.segmentation
    ...
    SUM(CASE WHEN f2.segmentation = m.segmentation AND f.invoice_delay_t2 > 0 AND f.recovery_channel IN ('Negotiation - SSN', 'Paid in App','Paid outside App', 'Negotiation - Matthew', 'Negotiation of Installment - SSN') THEN f.recovered_amount ELSE NULL END) AS recovered_amount_DT2_Digital_at_reference,
    SUM(CASE
        WHEN f2.segmentation = m.segmentation
  ...
  LEFT JOIN month_view AS m
    ON dd.date >= m.dt_min_view
        AND dd.month_start = m.dt_month_ref
...
WHERE
    dd.month_start >= DATE('2025-01-01')
