...
contract_features AS (
    SELECT
        ...
        COUNT(CASE WHEN payment_status IN ('paid', 'written-down') THEN sk_invoice ELSE NULL END) AS n_invoices_paid,
        COUNT(CASE WHEN payment_status IN ('paid', 'written-down') AND invoice_delay_t2 <= 0 THEN sk_invoice ELSE NULL END) AS n_invoices_paid_ontime_t2,
        COUNT(CASE WHEN payment_status IN ('paid', 'written-down') AND invoice_delay_t2 > 0 THEN sk_invoice ELSE NULL END) AS n_overdue_invoices_paid_t2,
        COUNT(CASE WHEN payment_status IN ('paid', 'written-down') AND invoice_delay_t1 <= 0 THEN sk_invoice ELSE NULL END) AS n_invoices_paid_ontime_t1,
        COUNT(CASE WHEN payment_status IN ('paid', 'written-down') AND invoice_delay_t1 > 0 THEN sk_invoice ELSE NULL END) AS n_overdue_invoices_paid_t1,
        COUNT(CASE WHEN payment_status IN ('paid', 'written-down') AND invoice_type IN ('monthly') AND invoice_delay_t2 <= 0 THEN sk_invoice ELSE NULL END) AS n_monthly_invoices_paid_ontime_t2,
        COUNT(CASE WHEN payment_status IN ('paid', 'written-down') AND invoice_type IN ('monthly') AND invoice_delay_t1 <= 0 THEN sk_invoice ELSE NULL END) AS n_monthly_invoices_paid_ontime_t1,
