...
segmentation_features AS (
    SELECT
        f.*,
        c.monthly_income,
        COALESCE(avgd.avg_days_overdue_invoices_paid_t1, 0) AS avg_days_overdue_invoices_paid_t1,
        CASE
            WHEN f.reference_contract_status = 'Ativo'
                AND f.max_delay_contaminated_contract_t1 <= 1
            ...
            THEN 'af - BAIXA'
            WHEN f.reference_contract_status = 'Ativo'
                AND f.max_delay_contaminated_contract_t1 > 30
            THEN 'UNDEFINED'
            WHEN f.reference_contract_status = 'Finalizado'
                AND f.max_delay_contaminated_contract_t1 <= 29
            ...
            WHEN f.reference_contract_status = 'Finalizado'
                ...
                AND COALESCE(avgd.avg_days_overdue_invoices_paid_t1, 0) <= 11
                AND f.days_since_ending > 30
                AND COALESCE(c.monthly_income, 0) <= 8100
            THEN 'F1-30 - BAIXA'
            WHEN f.reference_contract_status = 'Finalizado'
                AND f.max_delay_contaminated_contract_t1 <= 29
                AND COALESCE(avgd.avg_days_overdue_invoices_paid_t1, 0) <= 11
            ...
            WHEN f.reference_contract_status = 'Finalizado'
                ...
                AND f.acc_broken_multiple_deals_lifetime = 0
            THEN 'F1-30 - MEDIA'
            WHEN f.reference_contract_status = 'Finalizado'
                AND f.max_delay_contaminated_contract_t1 <= 29
                AND COALESCE(avgd.avg_days_overdue_invoices_paid_t1, 0) > 11
            ...
            WHEN f.reference_contract_status = 'Finalizado'
                AND f.max_delay_contaminated_contract_t1 >= 30
                AND f.max_delay_contaminated_contract_t1 <= 90
                AND COALESCE(avgd.avg_days_overdue_invoices_paid_t1, 0) <= 25
                AND f.acc_cpc_l90 = 0
                AND f.has_overdue_balance_over5_t1_at_ending_ffill = 0
            THEN 'F30-90 - BAIXA'
            WHEN f.reference_contract_status = 'Finalizado'
                AND f.max_delay_contaminated_contract_t1 >= 30
                AND f.max_delay_contaminated_contract_t1 <= 90
                ...
                AND f.acc_cpc_l90 = 0
                AND f.has_overdue_balance_over5_t1_at_ending_ffill > 0
                AND f.max_delay_contaminated_contract_t1 <= 59
            THEN 'F30-90 - BAIXA'
            WHEN f.reference_contract_status = 'Finalizado'
                AND f.max_delay_contaminated_contract_t1 >= 30
                AND f.max_delay_contaminated_contract_t1 <= 90
                ...
                AND f.acc_cpc_l90 = 0
                AND f.has_overdue_balance_over5_t1_at_ending_ffill > 0
                AND f.max_delay_contaminated_contract_t1 >= 60
            ...
            WHEN f.reference_contract_status = 'Finalizado'
                ...
                AND COALESCE(avgd.avg_days_overdue_invoices_paid_t1, 0) <= 25
                AND f.acc_cpc_l90 > 0
                AND f.has_overdue_balance_over0_t1_at_ending_ffill = 0
            THEN 'F30-90 - MEDIA'
            WHEN f.reference_contract_status = 'Finalizado'
                AND f.max_delay_contaminated_contract_t1 >= 30
                AND f.max_delay_contaminated_contract_t1 <= 90
            ...
            WHEN f.reference_contract_status = 'Finalizado'
                ...
                AND f.max_delay_contaminated_contract_t1 <= 90
                AND COALESCE(avgd.avg_days_overdue_invoices_paid_t1, 0) > 25
                AND f.n_evictions_processes_lifetime <= 0
                AND f.acc_max_n_repairs = 0
            THEN 'F30-90 - ALTA'
            WHEN f.reference_contract_status = 'Finalizado'
                AND f.max_delay_contaminated_contract_t1 >= 30
        ...
        CASE
            ...
            WHEN f.max_delay_contaminated_contract_t2 <= 0 AND f.reference_contract_status = 'Ativo' THEN 'ACTIVE - CURRENT'
            WHEN f.max_delay_contaminated_contract_t2 <= 0 AND f.reference_contract_status = 'Finalizado' THEN 'ENDED - CURRENT'
            WHEN f.days_since_contract_start <= 35 THEN 'FPD'
            WHEN COALESCE(f.is_evictions, FALSE) = TRUE THEN 'EVEX'
            WHEN f.reference_contract_status = 'Finalizado' AND f.max_delay_contaminated_contract_t2 >= 90 THEN 'TITANIC ENDED'
            WHEN f.max_delay_contaminated_contract_t2 <= 15 AND f.n_anchor_invoices_not_negativable = 0 AND f.reference_contract_status = 'Ativo' THEN 'NEW DEFAULTER - ACTIVE'
            WHEN f.max_delay_contaminated_contract_t2 <= 15 AND f.n_anchor_invoices_not_negativable = 0 AND f.reference_contract_status = 'Finalizado' THEN 'NEW DEFAULTER - ENDED'
            WHEN f.max_delay_contaminated_contract_t2 <= 15 AND f.n_anchor_invoices_not_negativable > 0 THEN 'NEW DEFAULTER - NEFN'
            WHEN f.reference_contract_status = 'Ativo' AND f.max_delay_contaminated_contract_t2 > 15 AND f.n_anchor_invoices_not_negativable > 0 THEN 'OVER 15 - ACTIVE - NEFN'
            WHEN f.reference_contract_status = 'Finalizado' AND f.max_delay_contaminated_contract_t2 < 90 AND f.n_anchor_invoices_not_negativable = 0 THEN 'SINKING ENDED'
            WHEN f.reference_contract_status = 'Finalizado' AND f.max_delay_contaminated_contract_t2 < 90 AND f.n_anchor_invoices_not_negativable > 0 THEN 'SINKING ENDED - NEFN'
            ELSE NULL
