...
SELECT
    f.id_contract_ebdb,
    f.id_contract_retsuko,
    f.id_proposal,
    f.months_of_contract,
    f.contract_mob_number,
    f.contract_ever_number,
    er.invoice_anchor,
    ...
    f.ts_signature,
    f.dt_reference,
    f.dt_contract_updated
FROM 
  calculate_final_ever_fraud AS f
LEFT JOIN 
  calculate_final_ever AS er
  ON er.id_contract_retsuko = f.id_contract_retsuko
  AND er.contract_mob_number = f.contract_mob_number
  AND er.contract_ever_number = f.contract_ever_number
