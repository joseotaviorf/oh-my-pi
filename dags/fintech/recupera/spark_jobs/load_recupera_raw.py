...
def unzip_file(ingestion_date: str, zip_file_bytes: bytes, tmp_folder: str):
    ...
    zip_file_secret = dbutils.secrets.get(
        scope=DATABRICKS_SCOPE, key="RECUPERA_ZIP_FILE_PASSWORD"
    )
    with zipfile.ZipFile(BytesIO(zip_file_bytes)) as zip_file:
        ...
        zip_file.extractall(
            os.path.join("/dbfs", tmp_folder), pwd=zip_password.encode()
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "original_table_name",
        help="the name of the original table, as it is in the Recupera database",
    )
    parser.add_argument(
        "load_incremental",
        help="indicates wheter the load is incremental or not (full)",
    )
    ...
    partition_cols = json.loads(args.partition_columns)
    ...
    file_path_s3 = (
        f"{source_root_path}/{original_table_name}_{date_to_ingest_formatted}"
    )
    ...
    for s3_object in files_s3:
        print(
            f"File {os.path.basename(s3_object.key)} was last modified at {s3_object.last_modified}"
        )

    if not load_incremental:
        s3_object = sorted(files_s3, key=lambda obj: obj.last_modified, reverse=True)[0]
        list_files_s3 = [f"s3://{bucket_name}/{s3_object.key}"]
    else:
        list_files_s3 = [
            f"s3://{bucket_name}/{s3_object.key}" for s3_object in files_s3
        ]
    ...
    try:
        for file_path in list_files_s3:
            ...
            zip_file_content = (
                s3_resource.Bucket(bucket_name).Object(object_key).get()["Body"].read()
            )
            csv_file = unzip_file(
                date_to_ingest_formatted, zip_file_content, tmp_folder
            )

            if csv_file is None:
                logger.info(
                    f"""m=__main__, msg=No data was found at file {object_key}"""
                )
                continue
        ...
        logger.info(
            f"m=__main__, msg=Total of {total_files_df.count()} rows for table: {table_name} and date: {date_to_ingest}"
        )
        ...
        logger.info(
            f"m=__main__, msg=Folder {folder_to_remove} deleted: {folder_deleted}"
        )
