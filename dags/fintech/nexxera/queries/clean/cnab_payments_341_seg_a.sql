SELECT 
    ...
    REPLACE(SUBSTRING(metadata, 60, 20), '!', '|') AS company_use,
    SUBSTRING(metadata, 91, 8) AS ispb_code, 
    SUBSTRING(metadata, 99, 2) AS payment_account_pix,
    SUBSTRING(metadata, 121, 15) AS our_number,
    SUBSTRING(metadata, 164, 20) AS purpose_detail,
    SUBSTRING(metadata, 184, 6) AS document_number,
    SUBSTRING(metadata, 190, 14) AS cpf_cnpj,
