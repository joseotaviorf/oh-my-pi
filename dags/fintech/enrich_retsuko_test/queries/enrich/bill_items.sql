WITH cte_split_bill_item AS (
    ...
    FROM
    datalake_retsuko_test.entry AS e
)
...
FROM
    datalake_retsuko_test.invoice AS rci
    LEFT JOIN cte_split_bill_item AS bi
      ON bi.id_invoice = rci.id
    LEFT JOIN datalake_retsuko_test.entry AS rce
      ON bi.id = rce.id
    LEFT JOIN datalake_retsuko_test_clean.account AS rca
      ON rca.id = rce.id_from_account
    LEFT JOIN datalake_retsuko_test_clean.account AS rcab
      ON rcab.id = rce.id_to_account
    LEFT JOIN datalake_retsuko_test_clean.contract AS rcc
