...
overdue_invoices_events AS (
  ...
  SELECT
    ...
    id_session,
    id_user,
    ARRAY(ep_id_contracts) AS id_contract,
    FROM_JSON(ep_id_invoices, 'array<string>') AS id_invoice,
    device_family,
    "pending_invoices_overdue_card_viewed" AS event_name,
    event_properties,
    GET_JSON_OBJECT(event_properties, '$.summary_state') AS summary_state,
    "Pending Invoices" AS funnel_step,
    3 AS level,
    "Triggers when user views the 'Overdue Invoices' card on the 'Pending Invoices' page" AS event_description,
    "pix_cc" AS feature,
    TRUE AS is_active,
    year,
    month,
    ...
    ts_event
  FROM
    datalake_amplitude_clean.170698_pending_invoices_overdue_card_viewed_events
  WHERE
    MAKE_DATE(year, month, day) BETWEEN DATE('{load_start_date}') AND DATE('{load_end_date}')
