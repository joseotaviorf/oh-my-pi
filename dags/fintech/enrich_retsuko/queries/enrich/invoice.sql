WITH latest_audit AS (
  ...
  FROM (
    SELECT
      id_invoice,
      id_audit,
      ROW_NUMBER() OVER (PARTITION BY id_invoice ORDER BY ts_database_transaction DESC) AS _w,
      ts_database_transaction
    FROM datalake_retsuko.entry
  ) AS _t
  WHERE
    _w = 1
)
SELECT
  ...
  NOW() AS ts_load
FROM datalake_retsuko_clean.invoice AS inv
LEFT JOIN latest_audit AS la
...
LEFT JOIN datalake_quintoandar.aux_date AS dd
  ON dd.date = CAST(inv.ts_due AS DATE)
