...
from quintoandar_looker_api_client.consumers.folders_consumer import FoldersConsumer

JOB_NAME = "load_folders_to_raw"

logging.getLogger("py4j").setLevel(logging.ERROR)
logger = QuintoAndarLogger(JOB_NAME)


def get_folders_df(spark_client):
    client = LookerClient()
    consumer = FoldersConsumer(client)
    folders = consumer.consume_with_paths()

    folders_df = spark_client.create_dataframe(Row(**doc) for doc in folders)
...
if __name__ == "__main__":
    ...
    folders_df = get_folders_df(spark_client)
