...
JOB_NAME = "load_dashboard_elements_to_raw"

logging.getLogger("py4j").setLevel(logging.ERROR)
logger = QuintoAndarLogger(JOB_NAME)


def get_elements_df(spark_client):
    client = LookerClient()
    consumer = DashboardsConsumer(client)
    _, elements = consumer.consume_with_dashboard_elements()

    elements_df = spark_client.create_dataframe(Row(**doc) for doc in elements)
...
if __name__ == "__main__":
    ...
    elements_df = get_elements_df(spark_client)
    df = (
