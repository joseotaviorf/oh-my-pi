...
from quintoandar_looker_api_client.consumers.users_consumer import UsersConsumer

JOB_NAME = "load_users_to_raw"

logging.getLogger("py4j").setLevel(logging.ERROR)
logger = QuintoAndarLogger(JOB_NAME)


def get_users_df(spark_client):
    client = LookerClient()
    consumer = UsersConsumer(client)
    users = consumer.consume()

    users_df = spark_client.create_dataframe(Row(**doc) for doc in users)
...
if __name__ == "__main__":
    ...
    users_df = get_users_df(spark_client)
