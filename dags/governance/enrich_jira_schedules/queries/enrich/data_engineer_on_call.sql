...
FROM (
  SELECT
    ...
    p.month,
    p.day,
    ROW_NUMBER() OVER (PARTITION BY p.id_period ORDER BY p.dt_load DESC) AS _w
  FROM datalake_jira_schedules.periods AS p
  LEFT JOIN datalake_jira_schedules.periods AS p_base
    ON p_base.id_period = p.id_period AND p_base.timeline_version = 'base'
  WHERE
    p.id_schedule IN ('9e63f54a-de9c-4e0d-bf48-75c95a28450d', '2fe05f0a-46ac-4b1c-8c20-fce1a9e5667b')
    AND p.timeline_version = 'final'
    AND p.dt_load BETWEEN CAST('{load_start_date}' AS DATE) AND CAST('{load_end_date}' AS DATE)
) AS _t
WHERE
  _w = 1