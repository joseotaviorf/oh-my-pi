WITH table_info AS (
  ...
  SELECT
    ...
    task,
    layer,
    CAST(ts_event AS TIMESTAMP),
    dt_executed
  FROM table_info AS tb
  JOIN datalake_airflow.log AS lg
    ON tb.task = lg.id_task
    AND CAST(lg.ts_executed AS DATE) = CAST(tb.dt_executed - INTERVAL '1' DAY AS DATE)
    AND CAST(lg.ts_event AS DATE) = tb.dt_executed
  WHERE
  ...
  FROM all_table_executions AS al
  JOIN datalake_health_metrics.dag_historical_executions AS de
    ON al.dag = de.dag_name
...
SELECT
  ...
  ts_task_started,
  ts_task_ended,
  TIMESTAMPDIFF(MINUTE, TO_DATE(ts_task_started), TO_DATE(ts_task_ended)) AS duration_in_minutes,
  dt_executed
