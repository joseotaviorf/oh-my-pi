WITH max_sla_achieved AS (
  ...
  FROM datalake_health_metrics.daily_sla_metrics
  WHERE
  ...
  FROM (
    ...
    FROM datalake_health_metrics.daily_sla_metrics AS dsm
    JOIN max_sla_achieved AS msa
      ON dsm.dag_owner = msa.dag_owner
    WHERE
