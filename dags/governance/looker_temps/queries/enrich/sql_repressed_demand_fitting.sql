WITH house_available_hours AS (
  ...
  WHERE
    dd.date >= DATE_ADD(CURRENT_DATE, -182)
    AND dd.date <= DATE_ADD(CURRENT_DATE, 14)
    AND NOT dd.date IS NULL
), regions AS (
  SELECT DISTINCT
    dr.id AS region_id,
    dr.region_code,
    dr.city_group,
    dr.city_name
  FROM dw_public.dim_region AS dr
  WHERE
    dr.region_code <> '-1'
), slot_series AS (
  SELECT
    a.slot + b.slot * 10 AS slot
  FROM slot_0_9 AS a, slot_0_9 AS b
), dimensions AS (
  SELECT
    r.region_id,
    r.region_code,
    ...
    ss.slot,
    CASE
      WHEN ss.slot BETWEEN 0 AND 3
      THEN 8
      WHEN ss.slot BETWEEN 4 AND 7
      THEN 9
      WHEN ss.slot BETWEEN 8 AND 11
      THEN 10
      WHEN ss.slot BETWEEN 12 AND 15
      THEN 11
      WHEN ss.slot BETWEEN 16 AND 19
      THEN 12
      WHEN ss.slot BETWEEN 20 AND 23
      THEN 13
      WHEN ss.slot BETWEEN 24 AND 27
      THEN 14
      WHEN ss.slot BETWEEN 28 AND 31
      THEN 15
      WHEN ss.slot BETWEEN 32 AND 35
      THEN 16
      WHEN ss.slot BETWEEN 36 AND 39
      THEN 17
      WHEN ss.slot BETWEEN 40 AND 43
      THEN 18
    END AS hour,
    CASE
      WHEN ds.week_day BETWEEN 1 AND 5 AND ss.slot BETWEEN 0 AND 3
      THEN '1) Weekday 8-9h'
      WHEN ds.week_day BETWEEN 1 AND 5 AND ss.slot BETWEEN 4 AND 19
      THEN '2) Weekday 9-13h'
      WHEN ds.week_day BETWEEN 1 AND 5 AND ss.slot BETWEEN 20 AND 31
      THEN '3) Weekday 13-16h'
      WHEN ds.week_day BETWEEN 1 AND 5 AND ss.slot BETWEEN 32 AND 35
      THEN '4) Weekday 16-17h'
      WHEN ds.week_day BETWEEN 1 AND 5 AND ss.slot BETWEEN 36 AND 43
      THEN '5) Extended hours'
      WHEN ds.week_day = 6
      THEN '6) Saturday all hours'
      WHEN ds.week_day = 0
      THEN '7) Sunday all hours'
    END AS faixa
  FROM regions AS r
  CROSS JOIN date_series AS ds
  CROSS JOIN slot_series AS ss
  ...
  WHERE
    ...
    AND dt_scheduling >= CURRENT_DATE - INTERVAL '182' DAY
), booking_for_sale AS (
  SELECT DISTINCT
    id_visitor AS user_id,
    id_property AS house_id,
  ...
  SELECT
    ...
    CASE
      WHEN LENGTH(
        SPLIT(GET_JSON_OBJECT(event_properties, '$.alert_target_date'), CONCAT('\\Q', ','))[0]
      ) = 3
      THEN DATE(
        TO_TIMESTAMP(REPLACE(
          TRIM(
            SUBSTR(
              SPLIT(GET_JSON_OBJECT(event_properties, '$.alert_target_date'), CONCAT('\\Q', ','))[1],
              1,
              12
            )
          ),
      ...
      ELSE DATE(
        TO_TIMESTAMP(SUBSTR(TRIM(GET_JSON_OBJECT(event_properties, '$.alert_target_date')), 1, 10), 'yyyy-MM-dd')
      )
    END AS target_date,
    COALESCE(
      CAST(NULLIF(GET_JSON_OBJECT(event_properties, '$.alert_slot_from'), '') AS BIGINT),
      -1
    ) AS alert_slot_from, /* CASE
            ...
            END AS target_date, */
    COALESCE(
      CAST(NULLIF(GET_JSON_OBJECT(event_properties, '$.alert_slot_to'), '') AS BIGINT),
      -1
    ) AS alert_slot_to
  ...
  WHERE
    ...
    AND id_user <> ''
    AND ts_event >= CURRENT_DATE - INTERVAL '182' DAY
), encaixes_raw AS (
  SELECT
    evt.event_date,
    evt.user_id,
    ...
    evt.alert_slot_from,
    evt.alert_slot_to,
    CASE WHEN NOT etb.user_id IS NULL THEN 1 ELSE 0 END AS encaixe_realizado,
    RANK() OVER (PARTITION BY evt.user_id, evt.house_id ORDER BY evt.event_date DESC) AS rank_enc
  FROM visit_hoursalert_confirmed AS evt
  LEFT JOIN encaixe_to_booking AS etb
    ON etb.user_id = evt.user_id AND etb.house_id = evt.house_id
), encaixes_temp AS (
  SELECT DISTINCT
    user_id,
    house_id,
  ...
  FROM encaixes_raw AS enc
  JOIN slot_series AS ss
    ON ss.slot BETWEEN enc.alert_slot_FROM AND enc.alert_slot_to
  JOIN datalake_ebdb_clean.house AS h
    ON h.id = enc.house_id
  WHERE
    enc.rank_enc = 1
    AND NOT enc.user_id IS NULL
    AND NOT enc.target_date IS NULL
    AND NOT h.id_region IS NULL
), blocked_houses AS (
  SELECT
    *
  FROM (
    SELECT
      ...
      vs.status,
      CAST(CAST(FROM_UNIXTIME(CAST(r.ts_revision / 1000 AS BIGINT)) AS TIMESTAMP) AS TIMESTAMP) AS init,
      COALESCE(
        LEAD(
          CAST(CAST(FROM_UNIXTIME(CAST(r.ts_revision / 1000 AS BIGINT)) AS TIMESTAMP) AS TIMESTAMP)
        ) OVER (PARTITION BY vs.id_house ORDER BY r.ts_revision NULLS LAST),
        CURRENT_DATE
      ) AS `end`
    FROM datalake_ebdb_clean.house_visit_status_aud AS vs
    JOIN datalake_ebdb_clean.user_revision_entity AS r
      ON vs.rev = r.id AND mod_status = TRUE
  )
  WHERE
    status = 'BLOCKED'
), suspended_houses AS (
  ...
  FROM (
    SELECT
      ...
      COALESCE(
        LEAD(
          CAST(CAST(FROM_UNIXTIME(CAST(r.ts_revision / 1000 AS BIGINT)) AS TIMESTAMP) AS TIMESTAMP)
        ) OVER (PARTITION BY id_house ORDER BY r.ts_revision NULLS LAST),
        CURRENT_DATE
      ) AS `end`
    FROM datalake_ebdb_clean.house_aud AS h
    JOIN datalake_ebdb_clean.user_revision_entity AS r
      ON h.rev = r.id AND h.mod_status = TRUE
  )
  WHERE
    status = 'suspenso'
), cant_find_another_agent /* !! NEW canceled bookings that are considered repressed demand */ AS (
  SELECT DISTINCT
    id_visitor AS user_id,
    id_property AS house_id,
  ...
  WHERE
    ...
    AND DATE(dt_scheduling) >= CURRENT_DATE - INTERVAL '182' DAY
), encaixes_clean AS (
  SELECT
    region_id,
    ...
    CASE
      ...
      AND (
        t.event_date BETWEEN bh.init AND bh.`end`
      )
    ...
    CASE
      WHEN encaixe_realizado = 0
      AND (
        t.event_date BETWEEN sh.init AND sh.`end`
      )
      AND sh.status = 'suspenso'
      THEN slot_share_encaixe
    ...
    CASE
      ...
      AND (
        ...
        OR (
          CAST(slot AS BIGINT) BETWEEN 4 AND 7 AND hs.hours_available_09to10 = FALSE
        )
        OR (
          CAST(slot AS BIGINT) BETWEEN 8 AND 11 AND hs.hours_available_10to11 = FALSE
        )
        OR (
          CAST(slot AS BIGINT) BETWEEN 12 AND 15 AND hs.hours_available_11to12 = FALSE
        )
        OR (
          CAST(slot AS BIGINT) BETWEEN 16 AND 19 AND hs.hours_available_12to13 = FALSE
        )
        OR (
          CAST(slot AS BIGINT) BETWEEN 20 AND 23 AND hs.hours_available_13to14 = FALSE
        )
        OR (
          CAST(slot AS BIGINT) BETWEEN 24 AND 27 AND hs.hours_available_14to15 = FALSE
        )
        OR (
          CAST(slot AS BIGINT) BETWEEN 28 AND 31 AND hs.hours_available_15to16 = FALSE
        )
        OR (
          CAST(slot AS BIGINT) BETWEEN 32 AND 35 AND hs.hours_available_16to17 = FALSE
        )
        OR (
          CAST(slot AS BIGINT) BETWEEN 36 AND 39 AND hs.hours_available_17to18 = FALSE
        )
        OR (
          CAST(slot AS BIGINT) BETWEEN 40 AND 43 AND hs.hours_available_18to19 = FALSE
        )
      )
      THEN slot_share_encaixe
    ...
    CASE
      WHEN encaixe_realizado = 1 AND cfaa.status = 'Cancelado'
      THEN slot_share_encaixe
    END AS slot_share_encaixe_nao_realizado_cant_find_another_agent, /* NOVIDADE!! NOVIDADE!! NOVIDADE!! NOVIDADE!! NOVIDADE!! */
    CAST(hs.hours_available_08to09 AS BIGINT) + CAST(hs.hours_available_09to10 AS BIGINT) + CAST(hs.hours_available_10to11 AS BIGINT) + CAST(hs.hours_available_11to12 AS BIGINT) + CAST(hs.hours_available_12to13 AS BIGINT) + CAST(hs.hours_available_13to14 AS BIGINT) + CAST(hs.hours_available_14to15 AS BIGINT) + CAST(hs.hours_available_15to16 AS BIGINT) + CAST(hs.hours_available_16to17 AS BIGINT) + CAST(hs.hours_available_17to18 AS BIGINT) + CAST(hs.hours_available_18to19 AS BIGINT) AS slots_disponiveis_target_date,
    CASE
      WHEN encaixe_realizado = 0 AND visit_intent = 'SALE'
      THEN slot_share_encaixe
      ELSE NULL
    END AS slot_share_ocupado_por_visita_sale
  FROM encaixes_temp AS t
  LEFT JOIN house_available_hours AS hs
    ON CAST(t.house_id AS BIGINT) = hs.id_house
    AND CAST(hs.day_of_week AS BIGINT) = EXTRACT(dow FROM t.target_date)
    AND event_date BETWEEN hs.available_started_date AND COALESCE(hs.available_ended_date, (
      DATE_ADD(CURRENT_DATE, 2)
    ))
  LEFT JOIN blocked_houses AS bh
    ON (
      t.house_id = bh.house_id AND t.event_date BETWEEN bh.init AND bh.`end`
    )
  LEFT JOIN suspended_houses AS sh
    ON (
      t.house_id = sh.house_id AND t.event_date BETWEEN sh.init AND sh.`end`
    )
  LEFT JOIN booking_for_sale AS bfs
  ...
  SELECT
    ...
    SUM(
      COALESCE(slot_share_encaixe_realizado, 0) - COALESCE(slot_share_encaixe_nao_realizado_cant_find_another_agent, 0)
    ) AS share_encaixes_realizados,
    SUM(
      COALESCE(slot_share_encaixe_nao_realizado, 0) + COALESCE(slot_share_encaixe_nao_realizado_cant_find_another_agent, 0)
    ) AS share_encaixes_nao_realizados,
    ...
    SUM(
      COALESCE(slot_share_encaixe_nao_realizado, 0) + COALESCE(slot_share_encaixe_nao_realizado_cant_find_another_agent, 0) - (
        COALESCE(slot_share_nao_realizados_por_bloqueio_suspensao_agenda, 0) + COALESCE(slot_share_ocupado_por_visita_sale, 0)
      )
  ...
  SELECT
    CAST(CAST(FROM_UNIXTIME(CAST(ts_revision AS BIGINT) / 1000) AS TIMESTAMP) AS TIMESTAMP) AS date_time,
    hou.*
  FROM datalake_ebdb_clean.house_weekly_schedule_aud AS hou
...
SELECT
  ...
  d.week_start,
  CAST(d.date AS TIMESTAMP) + INTERVAL '8' HOUR + (
    INTERVAL '15' MINUTE
  ) * d.slot AS visit_hour,
  d.hour,
  d.faixa,
  enc.user_id,
  enc.house_id,
...
FROM encaixes_agg AS enc
LEFT JOIN dimensions AS d
  ON enc.region_id = d.region_id AND enc.target_date = d.date AND enc.slot = d.slot