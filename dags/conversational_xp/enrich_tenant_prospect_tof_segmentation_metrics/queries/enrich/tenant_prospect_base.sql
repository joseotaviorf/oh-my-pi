WITH tof_users AS (
    ...
    WHERE
        ...
        AND lower(s.business_context) = 'rent'
),
...
tp_users AS (
    SELECT
        ...
        CASE
            WHEN p.event_name = 'USER FIRST ACTIVATION' THEN 'ntp'
            WHEN p.event_name IN ('USER RECOVERY', 'USER RECOVERY IN OTHER CITY GROUP') THEN 'rtp'
        END AS event_name,
    ...
    LEFT JOIN
        ...
        ON p.id_prospect = c.id_user
            ...
            AND LOWER(c.business_context) = 'rent'
    WHERE
        ...
        AND p.id_prospect is not null
        AND LOWER(p.business_context) = 'rent'
...
SELECT
    ...
    event_name,
    CASE
        WHEN event_name in ('ntp', 'rtp') THEN 'tp'
