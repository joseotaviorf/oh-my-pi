...
def _extract_principal_service():
    ...
    _noise_services = [
        "default",
        "kong-serviceaccount",
        "kong-private-controller",
        "kong-public-controller",
    ]
...
def clean_cf(df):
    ...
    df = df.select(
        ...
        coalesce(col("direct.roles"), col("wrapped.roles")).alias(
            "principal_user_roles"
        ),
        coalesce(col("direct.sudoed_by_id"), col("wrapped.sudoed_by_id")).alias(
