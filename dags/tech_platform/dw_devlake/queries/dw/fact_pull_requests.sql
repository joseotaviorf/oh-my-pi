WITH primary_team AS (
  ...
  FROM datalake_devlake_clean.pull_request_team
  GROUP BY
    id_repo,
    pr_key
), commit_stats AS (
  ...
  FROM datalake_devlake_clean.pull_request_commits
  GROUP BY
    id_pr
  ...
  FROM VALUES
    ...
    ('github:GithubAccount:5:0') /* system/ghost account (conn 5) */ AS _values
), comment_stats AS (
  SELECT
    c.id_pr,
    ...
    MIN(CASE WHEN c.type = 'REVIEW' AND c.status = 'APPROVED' THEN c.created_date END) AS first_approval_ts,
    SUM(CASE WHEN NOT b.account_id IS NULL THEN 1 ELSE 0 END) AS bot_comment_count,
    SUM(CASE WHEN b.account_id IS NULL THEN 1 ELSE 0 END) AS human_comment_count,
    SUM(
      CASE
        WHEN b.account_id IS NULL AND c.type = 'REVIEW' AND c.status = 'APPROVED'
        THEN 1
        ELSE 0
      END
    ) AS human_approval_count,
    COUNT(
      DISTINCT CASE
        WHEN b.account_id IS NULL AND c.type IN ('REVIEW', 'DIFF')
        THEN c.account_id
      END
    ) AS human_reviewer_count,
    MIN(CASE WHEN b.account_id IS NULL THEN c.created_date END) AS first_human_comment_ts,
    MIN(
      CASE
        WHEN b.account_id IS NULL AND c.type = 'REVIEW' AND c.status = 'APPROVED'
        THEN c.created_date
      END
    ) AS first_human_approval_ts,
    SUM(CASE WHEN b.account_id IS NULL AND c.type = 'DIFF' THEN 1 ELSE 0 END) AS human_diff_comment_count,
    SUM(
      CASE
        WHEN b.account_id IS NULL AND c.type = 'DIFF' AND c.body LIKE '%```suggestion%'
        THEN 1
        ELSE 0
      END
    ) AS suggestion_count,
    SUM(
      CASE
        ...
        AND c.type = 'DIFF'
        AND (
          c.body LIKE '%```suggestion%'
          OR c.body LIKE '%nit:%'
          OR c.body LIKE '%Nit:%'
          OR c.body LIKE '%NIT:%'
          OR c.body LIKE '%should%'
          OR c.body LIKE '%could%'
          OR c.body LIKE '%consider%'
          OR c.body LIKE '%instead%'
          OR c.body LIKE '%better%'
          OR c.body LIKE '%prefer%'
          OR c.body LIKE '%bug%'
          OR c.body LIKE '%wrong%'
          OR c.body LIKE '%error%'
          OR c.body LIKE '%fix%'
          OR c.body LIKE '%TODO%'
          OR c.body LIKE '%FIXME%'
        )
        THEN 1
        ELSE 0
      END
    ) AS actionable_comment_count,
    SUM(
      CASE
        WHEN b.account_id IS NULL
        AND c.type = 'DIFF'
        ...
        AND NOT c.body LIKE '%```suggestion%'
        THEN 1
        ELSE 0
      END
    ) AS question_comment_count
  FROM datalake_devlake_clean.pull_request_comments AS c
  LEFT JOIN bot_accounts AS b
...
SELECT
  ...
  CAST(DATE_FORMAT(CAST(pr.ts_created AS DATE), 'yyyyMMdd') AS INT) AS sk_created_date,
  CAST(DATE_FORMAT(CAST(pr.ts_merged AS DATE), 'yyyyMMdd') AS INT) AS sk_merged_date,
  CAST(DATE_FORMAT(CAST(pcm.ts_released AS DATE), 'yyyyMMdd') AS INT) AS sk_released_date,
  CAST(DATE_FORMAT(CAST(cs.first_commit_ts AS DATE), 'yyyyMMdd') AS INT) AS sk_first_commit_date,
  CAST(DATE_FORMAT(CAST(cms.first_approval_ts AS DATE), 'yyyyMMdd') AS INT) AS sk_first_approval_date,
  pr.id_pr,
  pr.id_repo,
  pr.pr_key,
  pr.pr_title,
  ...
  CASE
    WHEN pr.pr_additions + pr.pr_deletions <= 10
    THEN 'XS'
    WHEN pr.pr_additions + pr.pr_deletions <= 50
    THEN 'S'
    WHEN pr.pr_additions + pr.pr_deletions <= 200
    THEN 'M'
    WHEN pr.pr_additions + pr.pr_deletions <= 500
    THEN 'L'
    ELSE 'XL'
  END AS pr_size_category,
  BIGINT(UNIX_TIMESTAMP(pr.ts_created) - UNIX_TIMESTAMP(cs.first_commit_ts)) AS coding_time_seconds,
  BIGINT(UNIX_TIMESTAMP(cms.first_comment_ts) - UNIX_TIMESTAMP(pr.ts_created)) AS review_wait_seconds,
  BIGINT(UNIX_TIMESTAMP(cms.first_approval_ts) - UNIX_TIMESTAMP(pr.ts_created)) AS first_approval_seconds,
  BIGINT(UNIX_TIMESTAMP(pr.ts_merged) - UNIX_TIMESTAMP(cms.first_approval_ts)) AS merge_after_approval_seconds,
  BIGINT(UNIX_TIMESTAMP(cms.first_human_comment_ts) - UNIX_TIMESTAMP(pr.ts_created)) AS human_review_wait_seconds,
  BIGINT(UNIX_TIMESTAMP(cms.first_human_approval_ts) - UNIX_TIMESTAMP(pr.ts_created)) AS human_first_approval_seconds,
...
FROM datalake_devlake_clean.pull_requests AS pr
LEFT JOIN datalake_devlake_clean.pr_custom_metrics AS pcm
  ON pr.id_pr = pcm.id_pr
LEFT JOIN primary_team AS pt
  ON pr.id_repo = pt.id_repo AND pr.pr_key = pt.pr_key
LEFT JOIN commit_stats AS cs
  ON pr.id_pr = cs.id_pr
LEFT JOIN comment_stats AS cms
