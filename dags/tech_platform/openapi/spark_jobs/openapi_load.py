...
def parse(df):
    ...
    processed_df = grouped_data.select(
        col("file_path"), parse_yaml_udf("yaml_content").alias("json_string")
    ...
    parsed_df = processed_df.select(
        col("file_path"),
        from_json(
            col("json_string"), StructType([StructField("paths", paths_schema, True)])
        ).alias("json_data"),
    )

    # Extract paths
    paths_df = parsed_df.select(
        col("file_path"), explode(col("json_data.paths")).alias("path", "path_details")
    )
...
def main():
    ...
    db_info = DatalakeMetastoreService.get_db_info(
        args.env, args.schema, args.datalake_bucket
    )
