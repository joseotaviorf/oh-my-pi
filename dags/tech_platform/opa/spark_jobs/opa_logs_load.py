...
# Outer envelope of each OPA log file. Only the fields actually consumed
...
def clean_cf(df):
    ...
    trace_id = when(
        traceparent.isNotNull() & traceparent.contains("-"),
    ...
    df = df.select(
        ...
        col("data.input.attributes.destination.principal").alias(
            "request_destination_principal"
        ),
        col("data.input.attributes.request.http.headers.x-request-id").alias(
            "id_request"
        ),
        col("data.input.attributes.request.http.headers.x-amz-cf-id").alias(
            "id_amz_cf"
        ),
        col("data.input.attributes.request.http.headers.x-real-ip").alias(
            "request_real_ip"
        ),
        col("data.result.dynamic_metadata.parameterized_path").alias(
        ...
        col("data.result.principalinfo.user.payload.email").alias(
            "principal_user_email"
        ),
        col("data.result.principalinfo.user.payload.main_user_id").alias(
            "id_main_principal_user"
        ),
        col("data.result.principalinfo.user.payload.personUUID").alias(
            "uuid_person_principal_user"
        ),
        col("data.result.principalinfo.user.provided_roles").alias(
            "principal_user_provided_roles"
        ),
        col("data.result.principalinfo.user.payload.providerId").alias(
            "principal_user_idp"
        ),
        col("data.result.principalinfo.user.payload.iss").alias(
            "principal_user_issuer"
        ),
        col("data.result.principalinfo.user.payload.sudoed_by_id").alias(
            "id_principal_user_impersonated_by"
        ),
        col("data.result.principalinfo.service.provided_roles").alias(
            ...
            StructField(
                ...
                StructType(
                    [
                        StructField(
                            "app",
                            StructType([StructField("revision", StringType(), True)]),
                            True,
                        ),
                        StructField(
                            "data",
