...
salesforce_relisting AS (
  SELECT
    *,
    CASE 
      WHEN is_relisting_eligible = TRUE AND is_early_relisting_active = FALSE THEN TRUE
      ELSE FALSE
    END has_relisting_flow
  FROM datalake_salesforce_clean.relisting_context
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY id_relisting_context 
    ORDER BY ts_last_modified DESC
  ) = 1
),
...
main_query AS (
  SELECT DISTINCT
    ...
    -- sc.id_case,
    CASE 
      WHEN ft.ts_termination_request < DATE('2025-06-02') 
      ...
      WHEN ft.ts_termination_request >= DATE('2025-05-22') 
        AND ft.is_spoc_contract = TRUE 
        AND (ft.is_spoc_control_group = FALSE OR ft.is_spoc_control_group IS NULL) 
        AND (dt.team = 'ROLLOUT' OR dt.team IS NULL) 
        THEN 'rollout'
      WHEN ft.ts_termination_request BETWEEN DATE('2025-06-02') AND DATE('2025-07-29') 
        AND ft.is_spoc_contract = TRUE 
        AND (ft.is_spoc_control_group = FALSE OR ft.is_spoc_control_group IS NULL) 
        AND dt.team = 'LAB' 
        THEN 'wave_6_lab_test'
      WHEN ft.ts_termination_request BETWEEN DATE('2025-07-30') AND DATE('{load_start_date}') 
        AND ft.is_spoc_contract = TRUE 
        AND (ft.is_spoc_control_group = FALSE OR ft.is_spoc_control_group IS NULL) 
        AND dt.team = 'LAB' 
    ...
    CASE 
      WHEN COALESCE(
        ...
        DATE_TRUNC('day', DATE_ADD(DAY, -1, DATE('{load_start_date}')))
      ) > t.ts_activity THEN TRUE
      ELSE FALSE
    END is_task_expired,
    CASE 
      WHEN t.ts_completed IS NOT NULL THEN 
        DATE_DIFF(day, DATE_TRUNC('day', t.ts_created), DATE_TRUNC('day', t.ts_completed))
      ELSE NULL
    END task_leadtime
  FROM tasks t
...
SELECT
  ...
  DATE(mq.ts_termination_request) dt_termination_request,
  DATE(mq.ts_termination_finished) dt_termination_finished,
  ...
  mq.task_leadtime,
  CASE 
    WHEN mq.status IS NULL THEN NULL
  ...
  CASE 
    WHEN mq.status IN ('Not Started', 'In Progress') 
      AND DATE(mq.ts_task_completed) IS NULL 
      AND DATE(mq.dt_task_expiration) < DATE(DATE('{load_start_date}')) THEN 1
    WHEN mq.status IN ('Not Started', 'In Progress') 
      AND DATE(mq.ts_task_completed) IS NULL 
