...
front_tickets_list AS (
  SELECT DISTINCT
    ...
    TIMESTAMPADD(DAY, -3, ft.ts_created) AS search_window_from,
...
recontact_check AS (
  SELECT
    ...
    TIMESTAMPDIFF(
      DAY,
      CASE WHEN rc.ts_started != LAG(rc.ts_started) OVER(PARTITION BY rc.sk_user, rc.team ORDER BY rc.ts_started)
           THEN LAG(DATE(rc.ts_started)) OVER(PARTITION BY rc.sk_user, rc.team ORDER BY rc.ts_started)
           ELSE NULL END, 
      DATE(rc.ts_started)
    ...
    rc.theme_detail,
    CASE
      WHEN TIMESTAMPDIFF(DAY, LAG(DATE(rc.ts_started)) OVER(PARTITION BY rc.sk_user, rc.team ORDER BY rc.ts_started), DATE(rc.ts_started)) <= 3 
    ...
    CASE
      ...
      WHEN TIMESTAMPDIFF(DAY, LAG(DATE(rc.ts_started)) OVER(PARTITION BY rc.sk_user, rc.team, rc.theme ORDER BY rc.ts_started), DATE(rc.ts_started)) <= 3 
...
demand_back_FRC AS (
  ...
  WHERE
    ...
    AND ft.ts_created >= DATE('{load_start_date}') - INTERVAL '6' month
)
SELECT
  rc.sk_ticket,
