...
csat AS ( 
    SELECT DISTINCT  
        ...
        type__c,
        CASE 
            WHEN type__c = 'Tenant' THEN 'Inquilino' 
            WHEN type__c IN ('Owner','Landlord') THEN 'Proprietário' 
            ELSE 'Sem Definição de Partes' 
        END AS client_type_csat,
        ROW_NUMBER() OVER (
            PARTITION BY sk_case, (CASE 
            WHEN type__c = 'Tenant' THEN 'Inquilino' 
            WHEN type__c IN ('Owner','Landlord') THEN 'Proprietário' 
            ELSE 'Sem Definição de Partes' 
        END) 
...
events_case as (
    SELECT 
        ...
        c.id_owner,
        c.type,
        fr.record_type_name, -- Herdado do join com fact_request
        id_parent,
        event_type,
        last_modified_date,
...
solved_date as (
    SELECT 
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY CAST(last_modified_date AS TIMESTAMP) - INTERVAL 3 HOURS ASC
        ) rn
...
first_open as (
    SELECT 
        cast(case_number as int) as case_number, 
        -- Spark SQL: Conversão direta para TIMESTAMP
...
csat_final as (
    SELECT DISTINCT
        ...
        COALESCE(sf.email_analista, u.email) as last_agent_email,
        CASE WHEN COALESCE(sf.email_analista, u.email) LIKE '%webhelp%' THEN 'webhelp'
             WHEN COALESCE(sf.email_analista, u.email) LIKE '%concentrix%' THEN 'webhelp'
             WHEN COALESCE(sf.email_analista, u.email) LIKE '%atento%' THEN 'atento'
             WHEN COALESCE(sf.email_analista, u.email) LIKE '%aec%' THEN 'aec'
             WHEN COALESCE(sf.email_analista, u.email) LIKE '%quintoandar%' THEN 'quintoandar'
             END as last_agent_organization,
        dc.ts_termination_requested,
        dc.dt_start,
        CASE
            WHEN dc.dt_start IS NULL THEN NULL
            -- Spark SQL: datediff(fim, inicio)
    ...
    LEFT JOIN solved_date as sd on CAST(sd.case_number AS int) = CAST(c.case_number AS int) and sd.rn = 1
