...
FROM (
  SELECT
    ...
    NOW() AS ts_load,
    ROW_NUMBER() OVER (
      PARTITION BY rt.id_ticket
      ORDER BY MAKE_DATE(rt.year, rt.month, rt.day) DESC
    ) AS _w
  ...
  WHERE
    MAKE_DATE(rt.year, rt.month, rt.day) BETWEEN CAST('{load_start_date}' AS DATE) - INTERVAL '1' YEAR AND CAST('{load_end_date}' AS DATE)
) AS _t
WHERE
  _w = 1
