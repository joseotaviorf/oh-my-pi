...
universo_tickets AS (
    SELECT
        ...
        CASE
            WHEN dd.department IN ('CX Pagamentos Ativo [POS] [BACK] [PAY]', 'Alteração de dados bancários [BACK]', 'Aditivos [REP] [POS] [BACK]') THEN
                CASE
                    WHEN dt.theme_detail IN (
                        'doubt_info_responsibility_condo_payment', 'request_change_responsibility_condo_payment',
                        'report_condo_value_different_announced', 'delete_fix_condo_expenses', 'add_condo_expenses',
                        'report_condo_paid_mistake', 'report_condo_default', 'confirm_condo_payment',
                        'send_condo_payment_receipt', 'report_informal_condo', 'request_refund_infraction_fine',
                        'report_expenses_prior_contract', 'doubt_info_guaranteed_condo', 'activate_guaranteed_condo',
                        'deactivate_guaranteed_condo'
                    ) THEN 'Condomínio'
                    WHEN dt.theme_detail IN (
                        'doubt_info_iptu_trash_fee', 'request_iptu_adjustment_value', 'request_refund_iptu_payment',
                        'report_discrepancy_contest_iptu_values', 'request_iptu_split'
                    ) THEN 'IPTU'
                    WHEN dt.theme_detail IN (
                        'doubt_info_due_date_boleto', 'report_discount_agreement_not_applied_wrong_boleto_tenant',
                        'request_negotiation_exemption_fine_interest_overdue_rent', 'request_negotiation_rent_boleto_not_overdue',
                        'doubt_info_boleto_values_tenant', 'request_resend_boleto_tenant', 'inform_boleto_payment_promise',
                        'request_refund_duplicate_rent_payment', 'report_issues_pay_boleto', 'report_issues_boleto_payment_credit_card',
                        'report_error_recurring_payment_credit_card', 'request_pause_wrong_charges', 'doubt_info_rent_adjustment',
                        'negotiate_rent_value', 'negotiate_rent_value_contract_anniversary', 'negotiate_change_adjustment_index',
                        'negotiate_temporary_rent_discount', 'report_extrajudicial_notice_rent_value', 'request_invoice_client_active_contract',
                        'request_invoice_fix_cancel', 'report_issues_one_rent_anticipation', 'report_issues_three_rent_anticipation_new_contracts',
                        'report_issues_six_rent_anticipation', 'report_issues_rent_anticipation_property_vacancy', 'doubt_info_brokerage_fee_installments',
                        'request_cancel_brokerage_fee_installments', 'request_exemption_brokerage_fee_active_contract', 'doubt_info_service_fee',
                        'doubt_info_administration_fee', 'doubt_info_owner_invoice_deadlines_values', 'contest_rent_transfer_values',
                        'request_bank_data_change_reversal_transfer', 'report_discount_agreement_not_applied_wrong_owner_transfer',
                        'request_send_statement_receipt_rent_transfer', 'request_resend_owner_invoice', 'report_extrajudicial_notice_owner_transfer'
                    ) THEN 'Aluguel'
                    WHEN dt.theme_detail IN ('delete_fix_condo_expenses', 'add_condo_expenses') THEN 'Reembolso de condomínio'
                    WHEN dt.theme_detail IN (
                        'request_report_income_tax', 'request_payment_statement_iq', 'report_income_report_error',
                        'request_income_split', 'request_final_country_exit_statement_pp_non_resident', 'request_carne_leao_income_statement'
                    ) THEN 'IR'
                    WHEN dt.theme_detail IN (
                        'report_inappropriate_behavior_tenants_residents', 'report_commercial_use_property', 'report_pets_staying_property',
                        'general_subjects_contracts_without_management', 'doubt_info_simple_bond', 'transfer_correct_journey_ongoing',
                        'request_repairs_ongoing', 'doubt_guidance_improvements', 'info_requested_repairs',
                        'reimbursement_completed_repair', 'disagree_repair_responsibility', 'repairs_responsibility_info',
...
flag_regra_2 AS (
    ...
    INNER JOIN (SELECT DISTINCT sk_user FROM universo_tickets WHERE sk_user > 0) u ON fhl.sk_owner = u.sk_user
...
flag_regra_2_5 AS (
    ...
    INNER JOIN dw_rent.fact_house_listings fhl ON CAST(fhl.sk_house_listing / 1000 AS BIGINT) = bte.id_house_texto
    WHERE fhl.sk_contract > 0
    GROUP BY 1
),
...
flag_regra_3 AS (
    ...
    INNER JOIN (SELECT DISTINCT sk_user FROM universo_tickets WHERE sk_user > 0) u ON fcp.sk_user = u.sk_user
...
SELECT
    ...
    CASE
        -- Context identifier - Pay e Aditivos
        WHEN (tb.department LIKE '%Alteração de dados bancários%' OR tb.department LIKE '%Pagamentos Ativo%') AND tb.is_pp_multi = TRUE THEN 'cx_back_pay_pp_multi'
        WHEN (tb.department LIKE '%Alteração de dados bancários%' OR tb.department LIKE '%Pagamentos Ativo%') AND tb.subdepartment = 'Condomínio' THEN 'cx_back_pay_cond_geral'
	   ...
	   WHEN (
        ...
        WHEN tb.department LIKE '%Atendimento Escalado%' THEN 'cx_offboarding_escalado'
        WHEN tb.department LIKE '%Offboarding Reparos%' THEN 'cx_offboarding_mediacao'
        ...
        WHEN tb.department LIKE '%Triagem [Porto]%' THEN 'reparos_porto'
        WHEN tb.department LIKE '%Atendimento [Porto]%' THEN 'reparos_porto'        
    ...
    CASE
        WHEN lower(da_last.agent_organization) IN ('atn', 'atento') THEN 'atento'
        WHEN lower(da_last.agent_organization) IN ('webhelp', 'webhelpbr', 'contractors') THEN 'webhelp'
        WHEN lower(da_last.agent_organization) IN ('quintoandar.com', 'quintoandar') THEN 'quintoandar'
    ...
    CASE
        WHEN tb.ts_started = MIN(tb.ts_started) OVER (PARTITION BY tb.sk_user, tb.department, COALESCE(tb.contrato_prioritario, r3.sk_contract)) THEN 'Encaiteirar'
        ELSE 'Duplicado'
    END AS flag,
