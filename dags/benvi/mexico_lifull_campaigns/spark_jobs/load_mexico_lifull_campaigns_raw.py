...
JOB_NAME = "load_mexico_lifull_campaigns_raw"
...
def _fetch_auth_token(session: requests.sessions.Session, url: str, credentials: dict, account_id: str) -> str:
    """
    ...
    try:
        ...
        logging.error(f"Fail to fetch auth token from Thribee plataform. error:{exception}")
        
        raise exception

def _fetch_campaign_data(login_url:str, credentials: dict, campaign_url: str, account: dict, execution_date: str) -> DataFrame:
    """
    ...
    auth_token = _fetch_auth_token(session, login_url, credentials, account["account_id"])
            ...
            try:
                ...
                if len(data) > 0:
                    return spark.createDataFrame([
                    {
                        ...
                        "curr_date": datetime.strptime(execution_date, "%Y-%m-%d"),
                    ...
                    logging.error(f"No data found for account {account['account_name']} to date {execution_date}.")

            except Exception as exception:
                logging.error(f"Fail to extract data. Schema error:{exception}")
                raise exception
        else:
            logging.error(f"Unsuccessful request to fetch campaign data from {campaign_url}. Status code: {response.status_code}")
...
if __name__ == "__main__":
    ...
    logger.info(
        f"""m=__main__, env={environment}, source={source}, datalake_bucket={datalake_bucket},
        start_date={start_date}, end_date={end_date}, raw_table_name={raw_table_name},
        raw_partition_cols={raw_partition_cols}, msg=Starting spark job..."""
    )
    ...
    credentials = json.loads(
        dbutils.secrets.get('quintoandar', APIEnum.MX_LIFULL_CAMPAIGNS)
    )

    """
    Fetch events data.
    """    
    times_range = [(start_date + timedelta(n)).strftime('%Y-%m-%d') for n in range((end_date - start_date).days + 1)]
    campaigns_data = [_fetch_campaign_data(login_url, credentials, campaign_url, accounts.get(acc), dt) for acc in accounts for dt in times_range]
    ...
    if len(filtered_campaign_data) > 0:
        ...
        logger.info(
            f"""m=raw_table_name={raw_table_name},len={len(filtered_campaign_data)}, found data. Starting load..."""
        )

        df = reduce(
            DataFrame.unionAll, filtered_campaign_data
        )
