    ...
    report_type_mapping AS (
        SELECT
            ...
            CASE
                ...
                WHEN ANY(report_type = 'VIDEO_PERFORMANCE_REPORT') THEN 'VIDEO_PERFORMANCE_REPORT'
                WHEN ANY(report_type = 'KEYWORDS_PERFORMANCE_REPORT') THEN 'KEYWORDS_PERFORMANCE_REPORT'
                WHEN ANY(report_type = 'AD_PERFORMANCE_REPORT') THEN 'AD_PERFORMANCE_REPORT'
                WHEN EVERY(EVERY(report_type = 'CAMPAIGN_PERFORMANCE_REPORT')) OVER (PARTITION BY campaign_name, id_campaign) THEN 'CAMPAIGN_PERFORMANCE_REPORT'
        END AS report_type
    FROM
        campaign_reports
    GROUP BY 1, 2, 3, 4, 5
...
keywords_metrics AS (
    ...
    FROM
        datalake_mexico_google_ads.keywords_performance AS gkpr
    JOIN
        report_type_mapping AS rtm
...
ad_type_flags AS (
    SELECT
        ...
        END AS ad_type
    FROM
        datalake_gsheets_clean.marketing_costs_google_ad_type_flags
),
ads_metrics AS (
    SELECT
        ...
        SUM(clicks) AS clicks
    FROM
        datalake_mexico_google_ads.ads_performance AS gapr
    JOIN
        report_type_mapping AS rtm
...
campaigns_metrics AS (
    SELECT
        ...
        SUM(clicks) AS clicks
    FROM
        datalake_mexico_google_ads.campaigns_performance AS gcpr
    JOIN
        report_type_mapping AS rtm
...
videos_metrics AS (
    SELECT
        ...
        SUM(clicks) AS clicks
    FROM
        datalake_mexico_google_ads.videos_performance AS gvpr
    JOIN
        report_type_mapping AS rtm
