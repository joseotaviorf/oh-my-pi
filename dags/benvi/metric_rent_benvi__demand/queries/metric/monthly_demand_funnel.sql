...
demand_funnel AS (
    SELECT  
        ...
        DATE(DATE_TRUNC('month', TO_DATE(STRING(sk_event_date), 'yyyyMMdd'))) AS dt_month_started
    FROM 
...
demand_actual_costs AS (
    SELECT
        ...
        DATE(DATE_TRUNC('MONTH', TO_DATE(STRING(id_date), 'yyyyMMdd'))) AS dt_month_started
    FROM
