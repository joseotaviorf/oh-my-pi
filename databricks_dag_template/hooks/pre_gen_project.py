...
if not dag_slug or not re.match(NAME_REGEX, dag_slug):
    logger.error(
        "msg=The dag_slug (%s) is invalid. It must match the regexp `%s`."
        % (dag_slug, NAME_REGEX)
    )
    sys.exit(1)

task_names = (
    "{{ cookiecutter.task_names }}".replace(" ", "_").replace("-", "_").split(",")
)
...
try:
    ...
    logger.error(
        "msg=The start date of the dag (%s) is invalid. Follow format YYYY-MM-DD."
        % dag_start_date
    )
    sys.exit(1)
...
try:
    ...
    logger.error(
        "msg=The value of max active runs (%s) is invalid. It must be an integer."
        % dag_max_active_runs
    )
    sys.exit(1)
...
if dag_catchup not in ("False", "True"):
    logger.error(
        "msg=The value of dag catchup (%s) is invalid. It must be True or False."
        % dag_catchup
    )
    sys.exit(1)
