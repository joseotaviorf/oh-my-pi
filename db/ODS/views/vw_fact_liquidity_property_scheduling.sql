...
with _offer as (
	select
		f.id_property_scheduling, f.id_scheduling, f.id_owner, f.id_user_affiliate, f.id_imovel, f.id_user_agent, f.id_user_visitor,
		f.id_user_visit_agent, b.id as b_id, f.id_visit, f.id_negotiation, null::bigint as id_pre_proposal, f.id_offer, f.id_proposal,
		f.id_contract, f.id_rental_flow, f.dt_contract_anullment, f.visit_created_from_app, f.visit_created_type, f.visit_last_updated_from_app,
  	f.visit_last_updated_type, p."version", p.first_publication_date, p.min_version_time, b."criadoEm" as b_criado_em, b."data",
		visitor.criado_em as visitor_criado_em, offer.criado_em as offer_criado_em, null::timestamp as pp_criado_em, agent.criado_em as agent_criado_em,
		i.regiao_id, contract."dataAssinado"
	from property_scheduling f
	left join booking b
	...
	left join usuario visitor
	  on visitor.id = f.id_user_visitor
	left join usuario agent
...
_pre_proposal as (
	select
		f.id_property_scheduling, f.id_scheduling, f.id_owner, f.id_user_affiliate, f.id_imovel, f.id_user_agent, f.id_user_visitor,
		f.id_user_visit_agent, b.id as b_id, f.id_visit, f.id_negotiation, f.id_pre_proposal, null::bigint as id_offer, f.id_proposal,
		f.id_contract, f.id_rental_flow, f.dt_contract_anullment, f.visit_created_from_app, f.visit_created_type, f.visit_last_updated_from_app,
  	f.visit_last_updated_type, p."version", p.first_publication_date, p.min_version_time, b."criadoEm" as b_criado_em, b."data",
		visitor.criado_em as visitor_criado_em, null::timestamp as offer_criado_em, pp."criadoEm" as pp_criado_em, agent.criado_em as agent_criado_em,
		i.regiao_id, contract."dataAssinado"
	from property_scheduling f
	left join booking b
	  on b.id = f.id_scheduling
...
_fact as (
	select
	  ...
	  coalesce((f.id_imovel || lpad(coalesce(f."version"::varchar(3), '1'), 3, '0'))::bigint, -1::bigint) as sk_property,
	  coalesce(f.regiao_id, -1)as sk_region,
	  f."version"::integer as listing_number,
	  coalesce(f.b_id, -1) as sk_booking,
	  coalesce(f.id_owner, -1) as sk_owner,
	  coalesce(f.id_user_affiliate, -1) as sk_user_affiliate,
	  ...
	  coalesce(to_char(f.first_publication_date,'YYYYMMDD')::integer, -1) as sk_first_listing_date,
	  coalesce(to_char(f.min_version_time,'YYYYMMDD')::integer, -1) as sk_listing_date,
	  coalesce(to_char(f.b_criado_em,'YYYYMMDD')::integer, -1) as sk_booking_created_date,
	  coalesce(to_char(f."data",'YYYYMMDD')::integer, -1) as sk_visit_date,
	  coalesce(to_char(f.visitor_criado_em,'YYYYMMDD')::integer, -1) as sk_visitor_user_signup_date,
	  coalesce(to_char(coalesce(f.offer_criado_em, f.pp_criado_em),'YYYYMMDD')::integer, -1) as sk_offer_created_date,
	  coalesce(to_char(f."dataAssinado",'YYYYMMDD')::integer, -1) as sk_contract_signed_date,
	  coalesce(to_char(f.agent_criado_em,'YYYYMMDD')::integer, -1) as sk_user_agent_date,
	  ...
	  h.vl_cost_marketing_campaigns_per_published_listings
	  	/ coalesce(nullif(count(1) over (partition by f.id_imovel, f."version"),0),1)
	...
	from offer_preproposal f
	left join vw_imovel_liquidity_marketing_costs h
	  on f.id_imovel = h.id
		  and f."version" = h."version"
	left join vw_imovel_liquidity_agents_costs ac
	...
	left join vw_imovel_liquidity_closing_costs c
	  on c.id_property_scheduling = f.id_property_scheduling
	left join vw_imovel_liquidity_visit_costs v
	...
	(
	  ...
	  from vw_imovel_agent_comission_slot s
	  group by id_property, "version"
	) acs
		on acs.id_property = f.id_imovel
			and acs."version" = f."version"
	    left join
	    (
	      select
	        id,
