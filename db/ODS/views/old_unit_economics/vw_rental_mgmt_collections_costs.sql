DROP VIEW IF EXISTS vw_rental_mgmt_collections_costs CASCADE;
CREATE VIEW vw_rental_mgmt_collections_costs AS
  WITH all_dates AS (
      SELECT DISTINCT
        cd.contract_id,
        cd.imovel_id,
      ...
      WHERE ((costs_dre."Category") :: TEXT = 'Collections' :: TEXT)
  ), cdre_dates AS (
      SELECT DISTINCT
        ct.contract_id,
        ct.imovel_id,
        ...
        ELSE date_part('days' :: TEXT,
                       ((date_trunc('month' :: TEXT, (cc.dre_date) :: TIMESTAMP WITHOUT TIME ZONE) +
                         '1 mon' :: INTERVAL)
                        - (cc.dre_date) :: TIMESTAMP WITHOUT TIME ZONE))
        END AS cost_days
      FROM (ct
        LEFT JOIN cdre_collections cc ON (((cc.dre_date) :: TIMESTAMP WITHOUT TIME ZONE = ct.date_range)))
  ), pre_final_result AS (
      SELECT DISTINCT
  ...
  SELECT
    ...
    OVER (
      ...
      ORDER BY pre_final_result.date_range )                            AS rn,
