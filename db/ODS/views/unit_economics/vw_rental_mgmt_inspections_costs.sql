...
CREATE VIEW vw_rental_mgmt_inspections_costs AS
  WITH all_dates AS (
      ...
      FROM (dim_date dd
        JOIN vw_rental_mgmt_contract cd ON (((dd.date = date_trunc('month' :: TEXT, cd.contract_init_date)) OR
                                             (dd.date = date_trunc('month' :: TEXT,
                                                                   (cd.termination_date) :: TIMESTAMP WITH TIME ZONE)))))
  ), ct AS (
      SELECT
