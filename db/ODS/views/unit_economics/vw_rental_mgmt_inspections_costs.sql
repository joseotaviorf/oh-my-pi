...
CREATE VIEW vw_rental_mgmt_inspections_costs AS WITH c_dates AS (
    SELECT
    ...
    FROM contract
    WHERE contract."dataAssinado" IS NOT NULL
          AND contract.tipo <> 'DealOnly'
    ...
    FROM (dim_date dd
      JOIN c_dates cd ON (((dd.date = date_trunc('month' :: TEXT, cd.contract_init_date)) OR
                           (dd.date = date_trunc('month' :: TEXT, (cd.termination_date) :: TIMESTAMP WITH TIME ZONE)))))
), ct AS (
    SELECT
      ...
      END                                                                                                   AS init_days,
      date_part('days' :: TEXT,
                ((ad.contract_date - date_trunc('month' :: TEXT, ad.contract_date)) - '1 mon' :: INTERVAL)) AS end_days,
    ...
    WHERE ((costs_dre."Category") :: TEXT = 'Inspections' :: TEXT)
), cdre_dates AS (
    SELECT DISTINCT
      ct.contract_id,
      ct.imovel_id,
      ...
      cc.dre_date,
      CASE
      WHEN ((date_part('month' :: TEXT, (cc.dre_date) :: TIMESTAMP WITHOUT TIME ZONE) >=
             date_part('month' :: TEXT, ct.created_date)) AND
            (date_part('year' :: TEXT, (cc.dre_date) :: TIMESTAMP WITHOUT TIME ZONE) >=
             date_part('year' :: TEXT, ct.created_date)))
        THEN ct.init_days
      ELSE NULL :: DOUBLE PRECISION
      END AS cost_days
    FROM (ct
      LEFT JOIN cdre_inspections cc ON (((cc.dre_date) :: TIMESTAMP WITHOUT TIME ZONE = ct.date_range)))
), pre_final_result AS (
    SELECT DISTINCT
