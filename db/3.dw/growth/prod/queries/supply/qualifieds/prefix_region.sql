with all_dates_prev as (
    ...
    row_number() over (partition by coalesce(dr.long_region_name, ''),
    	...
    	+ row_number() over (partition by coalesce(dr.long_region_name, ''),
			...
			- 1 as monthly_count,
    row_number() over (partition by coalesce(dr.long_region_name, ''),
                                    date_part('year', f.dt_qualified) order by f.dt_qualified asc)
    	+ row_number() over (partition by coalesce(dr.long_region_name, ''),
    	                                  date_part('year', f.dt_qualified) order by f.dt_qualified desc)
			- 1 as yearly_count
  ...
  order by coalesce(dr.long_region_name, ''), date_part('year', f.dt_qualified), date_part('month', f.dt_qualified), date_part('week', f.dt_qualified), date_part('day', f.dt_qualified)
),
all_dates as (
  select distinct
    _year,
...
all_dates_last_year as (
  ...
  where date_part('year', f.dt_qualified) = date_part('year', add_months(current_date, -12))
	...
	group by coalesce(dr.long_region_name, ''), date_part('year', f.dt_qualified)
	order by coalesce(dr.long_region_name, ''), date_part('year', f.dt_qualified)
),
