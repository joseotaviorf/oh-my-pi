with all_dates as (
    ...
    dense_rank() over (partition by coalesce(dr.city_name, ''),
                                    date_part('year', dp.dt_proposal_approved),
    																...
    																date_part('day', dp.dt_proposal_approved) order by f.sk_user_visitor asc)
    	+ dense_rank() over (partition by coalesce(dr.city_name, ''),
    	                                  date_part('year', dp.dt_proposal_approved),
    																		date_part('month', dp.dt_proposal_approved),
    																		date_part('week', dp.dt_proposal_approved),
    																		date_part('day', dp.dt_proposal_approved) order by f.sk_user_visitor desc)
			- 1 as daily_count,
    dense_rank() over (partition by coalesce(dr.city_name, ''),
                                    date_part('year', dp.dt_proposal_approved),
    																date_part('week', dp.dt_proposal_approved) order by f.sk_user_visitor asc)
    	+ dense_rank() over (partition by coalesce(dr.city_name, ''),
    	                                  date_part('year', dp.dt_proposal_approved),
    																		date_part('week', dp.dt_proposal_approved) order by f.sk_user_visitor desc)
			- 1 as weekly_count,
    dense_rank() over (partition by coalesce(dr.city_name, ''),
                                    date_part('year', dp.dt_proposal_approved),
    																date_part('month', dp.dt_proposal_approved) order by f.sk_user_visitor asc)
    	+ dense_rank() over (partition by coalesce(dr.city_name, ''),
    	                                  date_part('year', dp.dt_proposal_approved),
    																		date_part('month', dp.dt_proposal_approved) order by f.sk_user_visitor desc)
			- 1 as monthly_count,
    dense_rank() over (partition by coalesce(dr.city_name, ''),
                                    date_part('year', dp.dt_proposal_approved) order by f.sk_user_visitor asc)
    	+ dense_rank() over (partition by coalesce(dr.city_name, ''),
    	                                  date_part('year', dp.dt_proposal_approved) order by f.sk_user_visitor desc)
			- 1 as yearly_count
  ...
  order by coalesce(dr.city_name, ''), date_part('year', dp.dt_proposal_approved), date_part('month', dp.dt_proposal_approved), date_part('week', dp.dt_proposal_approved), date_part('day', dp.dt_proposal_approved)
),
