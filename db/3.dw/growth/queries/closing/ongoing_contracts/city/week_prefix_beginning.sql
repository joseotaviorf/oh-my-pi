with all_dates_city_week as (
  ...
  left join dim_region dr
	...
	group by coalesce(dr.city_name, ''), date_part('year', dd."date"), date_part('week', dd."date")
	order by coalesce(dr.city_name, ''), date_part('year', dd."date"), date_part('week', dd."date")
),
all_dates as (
	select
    w._year,
	...
	join growth.ongoing_contracts_city_day d
  	on d._year = w._year
      and d._week = w._week
      and d.city = w.city
