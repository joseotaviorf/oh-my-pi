with all_dates_city_week as (
  ...
  left join dim_region dr
	...
	group by coalesce(dr.city_name, ''), date_part('year', f.dt_lead_and_prospect), date_part('week', f.dt_lead_and_prospect)
	order by coalesce(dr.city_name, ''), date_part('year', f.dt_lead_and_prospect), date_part('week', f.dt_lead_and_prospect)
),
all_dates as (
	select
    w._year,
	...
	join growth.prospects_city_day d
  	on d._year = w._year
      and d._week = w._week
      and d.city = w.city
