with all_dates_city_week as (
  ...
  left join dim_region dr
	...
	group by coalesce(dr.city_name, ''), date_part('year', f.dt_opportunity), date_part('week', f.dt_opportunity)
	order by coalesce(dr.city_name, ''), date_part('year', f.dt_opportunity), date_part('week', f.dt_opportunity)
),
all_dates as (
	select
    w._year,
	...
	join growth.opportunities_city_day d
  	on d._year = w._year
      and d._week = w._week
      and d.city = w.city
