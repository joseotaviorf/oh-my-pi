with all_dates_prev as (
  select distinct
    ...
    row_number() over (partition by date_part('year', f.dt_opportunity),
    ...
    row_number() over (partition by date_part('year', f.dt_opportunity),
    	...
    	+ row_number() over (partition by date_part('year', f.dt_opportunity),
			...
			- 1 as monthly_count,
    row_number() over (partition by date_part('year', f.dt_opportunity) order by f.dt_opportunity asc)
    	+ row_number() over (partition by date_part('year', f.dt_opportunity) order by f.dt_opportunity desc)
			- 1 as yearly_count
