with all_dates_prev as (
  select distinct
    ...
    row_number() over (partition by date_part('year', f.dt_contact),
    ...
    row_number() over (partition by date_part('year', f.dt_contact),
    	...
    	+ row_number() over (partition by date_part('year', f.dt_contact),
			...
			- 1 as monthly_count,
    row_number() over (partition by date_part('year', f.dt_contact) order by f.dt_contact asc)
    	+ row_number() over (partition by date_part('year', f.dt_contact) order by f.dt_contact desc)
			- 1 as yearly_count
