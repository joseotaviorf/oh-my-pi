with all_dates_city_week as (
  ...
  left join dim_region dr
	...
	group by coalesce(dr.city_name, ''), date_part('year', dof.dt_first_sent), date_part('week', dof.dt_first_sent)
	order by coalesce(dr.city_name, ''), date_part('year', dof.dt_first_sent), date_part('week', dof.dt_first_sent)
),
all_dates as (
	select
    w._year,
	...
	join growth.offers_submitted_city_day d
  	on d._year = w._year
      and d._week = w._week
      and d.city = w.city
