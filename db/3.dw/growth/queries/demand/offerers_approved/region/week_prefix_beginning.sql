with all_dates_region_week as (
  ...
  left join dim_region dr
	...
	group by coalesce(dr.long_region_name, ''), date_part('year', dof.dt_approved), date_part('month', dof.dt_approved), date_part('week', dof.dt_approved)
	order by coalesce(dr.long_region_name, ''), date_part('year', dof.dt_approved), date_part('month', dof.dt_approved), date_part('week', dof.dt_approved)
),
all_dates as (
	select
    w._year,
	...
	join growth.offerers_approved_region_day d
  	on d.city = w.city
