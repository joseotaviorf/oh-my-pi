  ...
  select
	...
	join dim_proposal dp
		on f.sk_proposal = dp.sk_proposal
			...
			and f.sk_proposal != -1
	group by date_part('year', dp.dt_tenant_first_document_sent), date_part('week', dp.dt_tenant_first_document_sent)
	order by date_part('year', dp.dt_tenant_first_document_sent), date_part('week', dp.dt_tenant_first_document_sent)
),
all_dates as (
	select
    w._year,
	...
	join growth.documentation_sent_all_day d
  	on d._year = w._year
      and d._week = w._week
),
