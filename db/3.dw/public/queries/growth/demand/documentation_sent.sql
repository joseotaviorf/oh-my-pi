...
with all_dates_all as (
  select
		...
		date_part('week', dp.dt_tenant_first_document_sent) as _day,
...
all_dates_region as (
  select
	...
	group by dr.long_region_name, dr.city_name, date_part('year', dp.dt_tenant_first_document_sent), date_part('month', dp.dt_tenant_first_document_sent), date_part('week', dp.dt_tenant_first_document_sent), date_part('day', dp.dt_tenant_first_document_sent)
	order by dr.long_region_name, dr.city_name, date_part('year', dp.dt_tenant_first_document_sent), date_part('month', dp.dt_tenant_first_document_sent), date_part('week', dp.dt_tenant_first_document_sent), date_part('day', dp.dt_tenant_first_document_sent)
),
all_dates as (
	select *
	from all_dates_all
