...
with all_dates_all as (
	select
		...
		(date_part('year', f.dt_qualified)::varchar
			|| lpad(date_part('month', f.dt_qualified)::varchar, 2, '0')
			|| lpad(date_part('week', f.dt_qualified)::varchar, 2, '0'))::int  as concat_all,
		(date_part('year', f.dt_qualified)::varchar
			|| lpad(date_part('month', f.dt_qualified)::varchar, 2, '0'))::int  as concat_month,
	...
	group by date_part('year', f.dt_qualified), date_part('month', f.dt_qualified), date_part('week', f.dt_qualified)
	order by date_part('year', f.dt_qualified), date_part('month', f.dt_qualified), date_part('week', f.dt_qualified)
),
all_dates_region as (
  select
		date_part('year', f.dt_qualified) as _year,
	...
	group by r.long_region_name, r.city_name, date_part('year', f.dt_qualified), date_part('month', f.dt_qualified), date_part('week', f.dt_qualified)
	order by r.long_region_name, r.city_name, date_part('year', f.dt_qualified), date_part('month', f.dt_qualified), date_part('week', f.dt_qualified)
),
all_dates as (
	select *
	from all_dates_all
