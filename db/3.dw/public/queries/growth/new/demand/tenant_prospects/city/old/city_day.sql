drop table if exists growth.tenant_prospects_city_day;
create table growth.tenant_prospects_city_day as
with all_dates as (
	select
  ...
  group by coalesce(dr.city_name, ''), date_part('year', db.dt_created), date_part('month', db.dt_created), date_part('week', db.dt_created), date_part('day', db.dt_created)
