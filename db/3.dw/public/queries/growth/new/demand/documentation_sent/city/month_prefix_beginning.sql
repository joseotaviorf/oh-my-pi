with all_dates_city_month as (
  select
	...
	group by coalesce(dr.city_name, ''), date_part('year', dp.dt_tenant_first_document_sent), date_part('month', dp.dt_tenant_first_document_sent)
	order by coalesce(dr.city_name, ''), date_part('year', dp.dt_tenant_first_document_sent), date_part('month', dp.dt_tenant_first_document_sent)
),
all_dates as (
	select distinct
    d._year,
	...
	from growth.documentation_sent_all_day d
	join growth.documentation_sent_all_week w
  	on d._year = w._year
