...
def _get_cross_dependencies_path() -> str:
    return join(DAG_PACKAGES_ROOT, DAGS_CROSS_DEPENDENCIES_FILE_NAME)
...
class BietlejuiceDependencyHelper:
    ...
    def read_dependencies() -> dict:
        ...
        return FileService.get_dict_from_yaml_file(_get_cross_dependencies_path())
    ...
    def extract_dag_and_table_from_task_name(task_name: str) -> Tuple[str, str]:
        ...
        if ":create-external-table" in task_name:
            task_name_pattern = r"bietlejuice\.(.*):create-external-table-(enrich|raw|clean|core|dw)*-([^:]*)"
            table_group_number = 3
        else:
            task_name_pattern = r"bietlejuice\.(.*):(load|done)-(enrich|raw|clean|core|dw|metric|transactional)*-([^:]*)"
    ...
    def _extract_dag_and_table_from_redshift_task(task_name: str) -> Tuple[str, str]:
        ...
        """
        match = re.search(r"bietlejuice\.(.*):load-into-redshift-dw-([^:]*)", task_name)
