...
class SparkMetastoreLoader:
    ...
    def update_metastore(
        ...
        if not force_recreate and self.is_table_in_metastore(database_name, table_name):
            logger.info(
                f"m=update_metastore, db={database_name}, table={table_name}, msg=existing table in metastore, "
                "checking if it needs to be merged with dataframe schema"
            )
            merge_schema = self.create_merge_schema(database_name, table_name, df)
            if merge_schema:
                logger.info(
                    f"m=update_metastore, db={database_name}, table={table_name}, msg=existing table in metastore, "
                    "merging with dataframe schema"
                )
                self.recreate_table(
                    database_name,
                    table_name,
            ...
            logger.info(
                ...
                "or we are forcing table recreation: creating it with dataframe schema"
            )
            self.create_table(
        ...
        logger.info(
            f"m=update_metastore, db={database_name}, table={table_name}, msg=successfully loaded table in metastore."
        )
