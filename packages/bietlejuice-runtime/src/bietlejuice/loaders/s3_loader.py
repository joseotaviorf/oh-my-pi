...
from bietlejuice.services.containers import Services
...
class S3Loader:
    ...
    def load_full_table(
        ...
        *This method will be deprecated soon, you should use load_df.
        ...
        logger.info(
            f"m=load_full_table, table={database_name}.{table_name}, s3_path={s3_path}, "
            "msg=loaded table into S3."
        )
    ...
    def load_incremental_table(
        ...
        *This method will be deprecated soon, you should use load_df.
        ...
        if partition_overwrite_mode != "dynamic":
            raise RuntimeError(
                f"m=load_incremental_table, spark.sql.sources.partitionOverwriteMode={partition_overwrite_mode}, "
                "msg=partitionOverwriteMode have to be configured to 'dynamic'"
            )
        ...
        logger.info(
            f"m=load_incremental_table, path={s3_path}, "
            f"partitions={partition_cols}, msg=loaded partitions successfully"
        )
    ...
    def load_df(
        ...
        **options,
        ...
        desired_partitions = sc.defaultParallelism  # Almost all the times, this line will run before the cluster had scaled up, so we need to force this number to be different from the minimum.
        partition_overwrite_mode = spark.conf.get(
            "spark.sql.sources.partitionOverwriteMode"
        ).lower()
        if partition_overwrite_mode != "dynamic":
            logger.info(
                f"m=load_file, spark.sql.sources.partitionOverwriteMode={partition_overwrite_mode}, msg=partitionOverwriteMode have to be configured to 'dynamic'"
            )
