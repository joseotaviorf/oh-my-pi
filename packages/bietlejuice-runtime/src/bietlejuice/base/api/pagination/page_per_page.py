...
class PagePerPagePaginator(BasePaginator):
    ...
    Stops when the API ``metadata`` reports the last page (``page`` >= ``totalPages``), or when
    a page returns fewer rows than ``page_size``, or when a page is empty.
    """
    ...
    def _is_last_page(self, data: Any, results: List[Dict[str, Any]]) -> bool:
        if not results:
            return True
        if isinstance(data, dict):
    ...
    def fetch_all(self) -> Generator[List[Dict[str, Any]], None, None]:
        ...
        while True:
            ...
            if self._is_last_page(data, results):
                LOGGER.info(
