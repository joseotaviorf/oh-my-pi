import logging
import time
from typing import Any, Callable, Dict, Generator, List, Literal, Optional, Protocol
...
class CursorPaginator(BasePaginator):
    ...
    def __init__(
        ...
        context_info = (
            f", context_param={context_param} ({context_location})"
            if context_param
            else ""
        )
        page_size_info = f", page_size={page_size}" if page_size else ""
        delay_info = f", {page_delay}s delay" if page_delay else ""
