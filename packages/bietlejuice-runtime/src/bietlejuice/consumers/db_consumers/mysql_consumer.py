...
class MySqlConsumer(DBConsumer):
    ...
    def get_single_table_names_and_sizes(self, table):
        ...
        ORDER BY
          (data_length + index_length) DESC
        """.format(db=self.conn_config["db"], table=table)
    ...
    def get_data_from_table(self, table_name):
        ...
        df = self.spark_client.get_data_from_external_source(
            format="jdbc",
            options={**self.spark_common_options, "dbtable": "`" + table_name + "`"},
        )
    ...
    def get_data_from_table_in_parallel(
        ...
        if not partition_column:
            ...
            query = f"select max({partition_column}) from `{table_name}`"
            df = self.get_data_from_query(query)
            upper_bound = df.collect()[0][0]

            query = f"select min({partition_column}) from `{table_name}`"
            df = self.get_data_from_query(query)
    ...
    def get_table_schema(self, table_name):
        ...
        query = """
            ...
            """.format(db=self.conn_config["db"], table=table_name)
    ...
    def _get_partition_column_from_table(self, table_name):
        ...
        logger.warning(
            f"m=_get_partition_column_from_table, table={table_name}, msg=Partition column "
            "to parallelize the table read was not found."
        )
