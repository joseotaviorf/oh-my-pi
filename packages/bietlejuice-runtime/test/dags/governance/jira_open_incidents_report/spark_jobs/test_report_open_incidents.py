...
def test_gchat_card_uses_br_not_newlines():
    message = (
        "Open DEI incidents as of 2026-08-17 — 2 total:\n"
        "Data Fintech: 1 opened DEI incidents (newest: 0d, oldest: 0d)"
        " | on going 0 | unassigned 1 | description not filled 1\n"
        "no owner: 1 opened DEI incidents (newest: 1d, oldest: 1d)"
        " | on going 0 | unassigned 1 | description not filled 1"
    )
    card = _job._gchat_cards_v2(message)
    paragraph = card["cardsV2"][0]["card"]["sections"][0]["widgets"][0][
        "textParagraph"
    ]["text"]
    assert "\n" not in paragraph
    assert paragraph.startswith("<b>Open DEI incidents as of 2026-08-17 — 2 total:</b>")
    assert paragraph.count("<br>") == 2
    assert "Data Fintech:" in paragraph
    assert "no owner:" in paragraph


def test_generic_webhook_base_swaps_inmetro_path():
    assert (
        _job._generic_webhook_base(
            "https://notification-hub-internal-api.quintoandar.com.br/webhook/inmetro"
        )
        == "https://notification-hub-internal-api.quintoandar.com.br/webhook/generic"
    )
    already = "https://notification-hub-internal-api.quintoandar.com.br/webhook/generic"
    assert _job._generic_webhook_base(already) == already
