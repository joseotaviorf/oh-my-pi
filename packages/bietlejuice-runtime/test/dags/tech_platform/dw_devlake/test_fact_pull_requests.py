...
class TestCycleTimeBreakdown:
    ...
    def test_full_breakdown_available(self, fact_df):
        ...
        total = (
            row["coding_time_seconds"]
            + row["first_approval_seconds"]
            + row["merge_after_approval_seconds"]
        )
        assert total > 0
...
class TestBotHumanSplit:
    ...
    def test_bot_plus_human_equals_total(self, fact_df):
        ...
        for row in rows:
            assert (
                row["bot_comment_count"] + row["human_comment_count"]
                == row["comment_count"]
    ...
    def test_question_disjoint_from_suggestion(self, fact_df):
        ...
        for row in rows:
            if row["human_diff_comment_count"] > 0:
                assert (
                    row["suggestion_count"] + row["question_comment_count"]
                    <= row["human_diff_comment_count"]
                ), f"Double-count in {row['id_pr']}"
...
class TestPRSizeCategory:
    ...
    def test_all_prs_have_size_category(self, fact_df):
        ...
        rows = fact_df.collect()
        for row in rows:
            assert row["pr_size_category"] in (
                "XS",
                "S",
                "M",
                "L",
