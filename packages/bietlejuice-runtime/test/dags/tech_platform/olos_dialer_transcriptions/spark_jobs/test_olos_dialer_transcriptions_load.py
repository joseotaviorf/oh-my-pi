"""main() tests for the Unity Catalog secondary sync on Olos dialer transcriptions.

Run::

    uv run --directory packages/bietlejuice-runtime pytest \\
        test/dags/tech_platform/olos_dialer_transcriptions/spark_jobs/test_olos_dialer_transcriptions_load.py -q
"""
...
from dags.tech_platform.olos_dialer_transcriptions.spark_jobs import (  # noqa: E402
    olos_dialer_transcriptions_load as load_job,
)

_TABLE = "olos_dialer_transcriptions"
...
def _args(**overrides):
    args = Namespace(
        ...
        load_start_date=datetime(2026, 7, 1),
        load_end_date=datetime(2026, 7, 1),
        table_name=_TABLE,
        partition_cols=["year", "month", "day"],
        target_database_name=None,
        target_table_name=None,
        path="s3://monalisa-prod/transcriptions/{}/{}/{}/*.ndjson.gz",
    )
...
def _stub_pipeline(monkeypatch, *, records_count, clean_data_frame):
    ...
    day_frame.dropDuplicates.return_value = clean_data_frame

    monkeypatch.setattr(load_job, "load_data_frame", lambda path: raw_df)
    monkeypatch.setattr(load_job, "clean_df", lambda df, audio_date: day_frame)
...
class TestMainSecondaryCatalogSync:
    def test_syncs_after_delta_write_on_default_path(self, monkeypatch):
        ...
        _stub_pipeline(monkeypatch, records_count=3, clean_data_frame=clean_data_frame)
    ...
    def test_syncs_after_delta_write_on_validation_target(self, monkeypatch):
        ...
        args = _args(
            ...
            target_table_name="datalake_tech_platform_clean___olos_dialer_transcriptions",
        )
        clean_data_frame = MagicMock(name="clean_data_frame")
        monkeypatch.setattr(load_job, "parse_arguments", lambda: args)
