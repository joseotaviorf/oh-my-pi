"""Fixtures for the sfmc DAG module tests."""
...
DAG_MODULE_PATH = "dags.support_and_service.sfmc.sfmc"

# A one-extension team and a two-extension team, so the imported DAG exercises
# both the single-table case and the fan-out of one raw task into several
# clean tasks.
TEAMS_CONFIG = {
    "agents_supply_acquisition": {"de_types": ["send"]},
    "growth": {"de_types": ["send", "return"]},
}
...
def _install_dag_dependency_stubs(attach_engine_fn) -> None:
    ...
    # DatasetAdder transitively imports airflow.datasets / extra_link_plugin, which
    # are not available with the stubbed airflow package. The DAG only calls
    # attach_dataset_to_task, so stub the module directly.
    ...
    # parse_parameters is the contract under test, so keep the real one.
...
def sfmc_module():
    ...
    # A distinct task per call so the graph edges the DAG builds stay
    # distinguishable, and each task carries the task_id it was created with.
...
@pytest.fixture(scope="module")
def spark_task_calls(sfmc_module):
    """Every create_spark_python_task call the DAG made at import time."""
    return sfmc_module._mock_engine.create_spark_python_task.call_args_list
