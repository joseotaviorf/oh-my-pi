...
class TestGreenhouseAuditLogAPIPitIdResume(unittest.TestCase):
    ...
    @patch.object(GreenhouseAuditLogAPI, "_apply_authentication")
    @patch.object(GreenhouseAuditLogAPI, "_process_filters")
    def test_is_pit_id_expired_detects_expiration(
        self, mock_process_filters, mock_auth
    ):
        ...
        error = Exception(
            'Bad Request: {"error":"The Pit_Id has expired. Remove it from the subsequent search."}'
        )
        self.assertTrue(api_client._is_pit_id_expired(error))

    @patch.object(GreenhouseAuditLogAPI, "_apply_authentication")
    @patch.object(GreenhouseAuditLogAPI, "_process_filters")
    def test_is_pit_id_expired_rejects_other_errors(
        self, mock_process_filters, mock_auth
    ):
    ...
    @patch.object(GreenhouseAuditLogAPI, "_apply_authentication")
    @patch.object(GreenhouseAuditLogAPI, "_process_filters")
    def test_get_max_event_time_returns_none_for_empty(
        self, mock_process_filters, mock_auth
    ):
    ...
    @patch.object(GreenhouseAuditLogAPI, "_apply_authentication")
    @patch.object(GreenhouseAuditLogAPI, "_process_filters")
    def test_should_resume_on_error_invalid_cursor(
        self, mock_process_filters, mock_auth
    ):
    ...
    @patch.object(GreenhouseAuditLogAPI, "_apply_authentication")
    @patch.object(GreenhouseAuditLogAPI, "_process_filters")
    def test_should_resume_on_error_pit_id_expired(
        self, mock_process_filters, mock_auth
    ):
...
class TestGreenhouseAuditLogAPIProcessFilters(unittest.TestCase):
    ...
    def test_process_filters_with_date_placeholders(self, mock_auth):
        ...
        self.assertEqual(api_client.params["occurred_at[gte]"], "2025-01-15T00:00:00Z")
        # Verify end date placeholder was replaced
        self.assertEqual(api_client.params["occurred_at[lte]"], "2025-01-20T00:00:00Z")
    ...
    def test_process_filters_with_only_end_date(self, mock_auth):
        ...
        self.assertEqual(api_client.params["occurred_at[lte]"], "2025-01-31T00:00:00Z")
