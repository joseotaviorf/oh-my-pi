...
class TestEvaluationCacheExtraction:
    ...
    def test_nested_observability_data_populates_evaluation_cache_clean_json(
        self, spark, source_df
    ):
        ...
        inputs = json.dumps(
            {"planner_agent_request": {"business_context": "sale", "id_house": "1"}}
        )
...
class TestDeduplicate:
    ...
    def test_duplicate_uuid_ts_log_reduced_to_one(self, spark):
        ...
        assert (
            _deduplicate(self._dup_df(spark)).filter("uuid = 'uuid-dup'").count() == 1
        )
    ...
    def test_earliest_partition_is_kept(self, spark):
        ...
        row = _deduplicate(self._dup_df(spark)).filter("uuid = 'uuid-dup'").collect()[0]
...
class TestSchemaMismatches:
    ...
    def _call(self, actual_fields, expected_fields):
        ...
        return _schema_mismatches(
            StructType(actual_fields), StructType(expected_fields)
        )
