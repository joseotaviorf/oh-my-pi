...
_MODULE = "dags.core.core_house_history.spark_jobs.load_core_house_history"
...
def _run_create_core_model(
    spark_session, imovel_df, hlr_df, usuario_df, mock_cfg, args=None
):
...
class TestCoreHouseHistoryFieldEvents:
    ...
    def test_registrant_coalesce_uses_original_when_present(
        ...
        registrant_events = [
            r for r in result.collect() if r["event_name"] == "ev_id_user_registrant"
        ]
...
class TestMergeStrategyConfig:
    ...
    def test_when_matched_update_condition_is_false(self, mock_configuration_service):
        job = CoreHouseHistorySparkJob()
