...
sys.path.append(
    os.path.join(_find_project_root(), "dags", "growth", "hightouch_logs", "spark_jobs")
)
from load_hightouch_logs import (  # noqa: E402
...
class TestLoadHightouchLogs(unittest.TestCase):
    ...
    def test_load_table_into_datalake_incremental_success(
        self,
        mock_spark_client,
        mock_storage_format,
        ...
        # Assert
        mock_datalake_service.get_db_info.assert_called_once_with(
            "prod", "hightouch_logs", "5a-datalake-prod"
        )
        mock_metastore_instance.create_database.assert_called_once_with(
            "datalake_hightouch_logs_raw"
        )
    ...
    @patch("load_hightouch_logs.SparkTableStorageFormat")
    @patch("load_hightouch_logs.spark_client")
    def test_load_table_into_datalake_full_success(
        self,
        mock_spark_client,
        mock_storage_format,
        mock_datalake_service,
    ...
    @patch("load_hightouch_logs.SparkTableStorageFormat")
    @patch("load_hightouch_logs.spark_client")
    def test_load_table_with_unity_catalog_enabled(
        self,
        mock_spark_client,
        mock_storage_format,
        mock_datalake_service,
    ...
    def test_incremental_filtering_logic(self, mock_f):
        ...
        with (
            ...
            mock_db_info = {"db_raw_databricks": "test_db", "db_raw_path": "s3://test/"}
            mock_datalake_service.get_db_info.return_value = mock_db_info
            mock_unity_catalog.is_cluster_unity_catalog_enabled.return_value = False
    ...
    @patch("load_hightouch_logs.UnityCatalogHelper")
    @patch("load_hightouch_logs.DatalakeMetastoreService")
    def test_load_table_into_datalake_exception(
        self, mock_datalake_service, mock_unity_catalog, mock_spark
    ):
    ...
    def test_main_success(
        self, mock_logging, mock_parse_args, mock_read_input, mock_load_table
    ):
    ...
    @patch("load_hightouch_logs.read_input")
    @patch("load_hightouch_logs.parse_arguments")
    def test_main_with_environment_placeholder_replacement(
        self, mock_parse_args, mock_read_input
    ):
        ...
        with patch("load_hightouch_logs.load_table_into_datalake"):
            # Act
            main()
    ...
    def test_read_input_logging(self, mock_spark):
        ...
        with patch("load_hightouch_logs.logging") as mock_logging:
            ...
            mock_logging.info.assert_called_with(
                "Reading from S3 path: s3://test/path with format: delta"
            )
            ...
            # Assert
            mock_logging.info.assert_called_with(
                "Reading from catalog table: catalog.table"
            )
