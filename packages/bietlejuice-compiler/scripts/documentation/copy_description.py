...
def main():
    origin_dag, origin_table, destination_dag, destination_table, is_forward = (
        read_arguments()
    )
    ...
    updated_metadata = copy_from_origin_to_destination(
        origin_metadata, destination_metadata, is_forward
    )
...
def read_arguments() -> Tuple[str, str, str, str]:
    ...
    arg_parser.add_argument(
        "--destination-dag", "-dd", required=True, help="Destination DAG"
    )
    arg_parser.add_argument(
        "--destination-table", "-dt", required=True, help="Destination table"
    )
    arg_parser.add_argument(
        "--is-forward", "-f", action="store_true", help="Is forward?", default=False
    )
...
def read_metadata(dag_name: str, table_name: str) -> Dict:
    metadata_path = list(
        glob.iglob(
            f"{DAG_PACKAGES_ROOT}/**/{dag_name}/metadata/**/{table_name}.y*ml",
            recursive=True,
        )
    )[0]
...
def copy_from_origin_to_destination(
    origin_metadata: dict, destination_metadata: dict, is_forward: bool
) -> None:
    # If it is forward, the lineage we need to look at is the destination
    metadata_to_look_at_lineage = (
        destination_metadata if is_forward else origin_metadata
    )
    ...
    for column_name, column_data in metadata_to_look_at_lineage["columns"].items():
        ...
        if not lineage_column:
            print(
                f"Column {lineage_column} has no lineage on destination table metadata"
            )
            continue
        ...
        if table_in_lineage == other_metadata["table_name"]:
            ...
            destination_metadata = copy_description(
                origin_metadata,
...
def copy_description(
    ...
    try:
        ...
        if not description:
            print(
                f"There is no description to copy on origin table for column {origin_column_name}"
            )
            return destination_metadata
        ...
        if len(destination_metadata["columns"][destination_column_name]["lineage"]) > 1:
            print(
                f"Column {destination_column_name} has more than 1 column on lineage. Check it manually."
            )
        ...
        if "description" not in origin_metadata["columns"][origin_column_name]:
            print(
                f"Column {destination_column_name} has no description on origin table"
            )
            return destination_metadata
    except KeyError:
        print(
            f"Key 'lineage' probably does not exist on destination metadata for column {destination_column_name}"
        )

    print(
        f"Copying {origin_column_name} FROM {origin_metadata['table_name']} TO {destination_column_name} ON {destination_metadata['table_name']}"
    )
    destination_metadata["columns"][destination_column_name].update(
