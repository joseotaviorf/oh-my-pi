...
class SkipListFileUpdater:
    ...
    def __init__(
        self, skip_list_path: str, dag_list_path: str, last_updater: str
    ) -> None:
    ...
    def read_files(self) -> Tuple[Dict[str, str], List[str]]:
        ...
        with (
            open(self.skip_list_path) as skip_stream,
            open(self.dag_list_path) as dag_stream,
        ):
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "--json", "-j", required=True, help="Path to the skip_list json"
    )
    arg_parser.add_argument(
        "--dags",
        "-d",
        required=True,
        help="Path to the list of dags separated by line breaks",
    )
    arg_parser.add_argument(
        "--last_updater",
        "-l",
        action="store",
        required=True,
        help="Name of the person who is updating the file",
    )
    arg_parser.add_argument(
        "--today", "-t", required=False, help="Today' date in this format: yyyy-MM-dd"
    )

    args = arg_parser.parse_args()
    today = (
        args.today if args.today is not None else datetime.today().strftime("%Y-%m-%d")
    )
