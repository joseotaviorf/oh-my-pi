...
def test_main_syncs_with_separate_env_vars(
    ...
    monkeypatch.setenv(JIRA_OPS_TOKEN, "secret-token")
    monkeypatch.setenv(JIRA_OPS_CLOUD_ID, "cloud-123")
    monkeypatch.setattr(
        sync_module,
...
def test_main_returns_error_when_api_raises(
    ...
    # Arrange
    monkeypatch.delenv(JIRA_OPS_CREDENTIALS_JSON, raising=False)
    monkeypatch.setenv(JIRA_OPS_USERNAME, "u")
    monkeypatch.setenv(JIRA_OPS_TOKEN, "t")
    monkeypatch.setenv(JIRA_OPS_CLOUD_ID, "c")
    monkeypatch.setattr(
        sync_module,
        "BuildJiraOpsMuteCriteria",
        lambda: BuildJiraOpsMuteCriteria(mute_list_path=mute_list_yaml),
