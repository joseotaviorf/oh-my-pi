...
class SQLTranspiler:
    ...
    def apply_custom_replacements(self, sql_content: str) -> Tuple[str, int]:
        ...
        patterns = [
            ...
            (r'\bCURRENT_DATE\b(?!\()', "DATE('{load_start_date}')"),  # CURRENT_DATE (without parentheses)
        ]
    ...
    def print_summary(self):
        ...
        if self.current_date_replacements > 0:
            print("\n🔄 Custom Replacements:")
            print(f"   CURRENT_DATE → DATE('{{load_start_date}}'): {self.current_date_replacements}")
    ...
    def run(self):
        ...
        if self.args.mode == 'git-diff':
            ...
            if not sql_files:
                ...
                return 0
            logger.info(f"Found {len(sql_files)} SQL file(s) to transpile in directory.")
...
Examples:
  ...
  python sql_transcript.py --mode git-diff --from-branch origin/master --to-branch HEAD --dry-run
    ...
    parser.add_argument(
        '--to-branch',
        help='Target branch for git diff comparison (e.g., HEAD)'
    )
    
    parser.add_argument(
        '--file',
        help='Path to a single SQL file to transpile'
    )
    
    parser.add_argument(
        '--directory',
        help='Path to directory containing SQL files to transpile'
    )
