...
def get_doc_data_frame(table, option):
    ...
    os.system(f"databricks fs cp -r {source_doc_path} {destination_doc_path} --profile={get_databricks_profile(option)}")
    df = pd.read_parquet(f'{destination_doc_path}')
    os.system(f"databricks fs rm -r {source_doc_path} --profile={get_databricks_profile(option)}")
...
for dag_name, dag_infos in json_content.items():
    ...
    yml_path = os.path.join(BIETLEJUICE_ROOT, f"dags/{line}/{dag_name}/metadata/{layer}/{table}.yml")
    yml_file_to_read = open(yml_path, 'r', encoding='utf-8')
    ...
    for interator_sheet in range(doc_data_frame.shape[0]):
        yml_content['columns'][doc_data_frame['column'][interator_sheet]]['description'] = doc_data_frame['description'][interator_sheet].replace("\n", " ")

    with open(yml_path, 'w+') as yml_content_to_write:
        yaml.dump(yml_content, yml_content_to_write, Dumper=Dumper, sort_keys=False, allow_unicode=True)