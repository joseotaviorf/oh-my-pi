...
HOUSE_HISTORY_EVENT_CONFIGS = [
    ...
    {"tracked_col": "id_user_registrant", "target_col": "id_user_registrant", "target_type": "bigint"},
    ...
    {"tracked_col": "numeroBanheiros", "target_col": "total_bathrooms", "target_type": "int"},
    {"tracked_col": "numeroQuartos", "target_col": "total_bedrooms", "target_type": "int"},
    {"tracked_col": "numeroSuites", "target_col": "total_suites", "target_type": "int"},
    {"tracked_col": "andar", "target_col": "floor", "target_type": "int"},
    {"tracked_col": "dataCriacao", "target_col": "ts_created", "target_type": "timestamp"},
    {"tracked_col": "atualizadoEm", "target_col": "ts_updated", "target_type": "timestamp"},
]
...
CONFIG_MAP = {
    ...
    "merge_on_historical": [
        "id_house", "event_name", "ts_transaction", "year", "month", "day",
    ],
...
def transactional_imovel_df(spark_session):
    ...
    data = [
        (
            "42", "EXT-42", "10", "100", "50", None,
            "Rua A", "123", "Centro", "Apto 1", "01000-000",
            "São Paulo", "APARTMENT", 80.0, -23.55, -46.63,
            "2", "3", "1", "5",
            "2026-01-10", "2026-01-10",
            "c", datetime(2026, 1, 10, 8, 0, 0), datetime(2026, 1, 10, 8, 0, 1),
        ),
        (
            "42", "EXT-42", "10", "150", "50", None,
            "Rua A", "123", "Centro", "Apto 1", "01000-000",
            "São Paulo", "APARTMENT", 80.0, -23.55, -46.63,
            "2", "3", "1", "5",
            "2026-01-10", "2026-01-15",
            "u", datetime(2026, 1, 15, 9, 0, 0), datetime(2026, 1, 15, 9, 0, 1),
        ),
    ]
...
def transactional_hlr_df(spark_session):
    ...
    data = [
        (
            "42", "200", "PROPERTY_OWNER", "MAIN_USER",
            "c", datetime(2026, 1, 12, 10, 0, 0), datetime(2026, 1, 12, 10, 0, 1),
        ),
    ]
    return spark_session.createDataFrame(data, schema)
...
def transactional_hlr_with_delete_df(spark_session):
    ...
    data = [
        ...
        (
            "42", "200", "PROPERTY_OWNER", "MAIN_USER",
            "d", datetime(2026, 1, 14, 11, 0, 0), datetime(2026, 1, 14, 11, 0, 1),
        ),
    ]
    return spark_session.createDataFrame(data, schema)
...
def transactional_usuario_df(spark_session):
    ...
    data = [
        ("100", "uuid-aaa", "c", datetime(2026, 1, 1, 0, 0, 0), datetime(2026, 1, 1, 0, 0, 1)),
        ("150", "uuid-bbb", "c", datetime(2026, 1, 1, 0, 0, 0), datetime(2026, 1, 1, 0, 0, 1)),
        ("200", "uuid-ccc", "c", datetime(2026, 1, 12, 9, 0, 0), datetime(2026, 1, 12, 9, 0, 1)),
    ]
    return spark_session.createDataFrame(data, schema)
...
def transactional_usuario_late_df(spark_session):
    ...
    data = [
        ("100", "uuid-aaa", "c", datetime(2026, 1, 1, 0, 0, 0), datetime(2026, 1, 1, 0, 0, 1)),
        ("150", "uuid-bbb", "c", datetime(2026, 1, 1, 0, 0, 0), datetime(2026, 1, 1, 0, 0, 1)),
        ("200", "uuid-ccc", "c", datetime(2026, 2, 1, 0, 0, 0), datetime(2026, 2, 1, 0, 0, 1)),
    ]
    return spark_session.createDataFrame(data, schema)
