...
class TestCoreContractSparkJob:
    ...
    def test_create_core_model_success(
        ...
        with patch.object(
            job, "_load_contract_data", return_value=mock_tables_success["contract_df"]
        ), patch.object(
    ...
    def test_create_core_model_empty_input(
        ...
        with patch.object(
            job, "_load_contract_data", return_value=mock_tables_empty["contract_df"]
        ), patch.object(
    ...
    def test_create_core_model_owner_coalesce(
        ...
        # Mock the table loading methods
        with patch.object(
            job, "_load_contract_data", return_value=contract_df
        ), patch.object(job, "_load_house_data", return_value=house_df), patch.object(
            job,
            "_load_house_listing_relation_data",
            return_value=house_listing_relation_df,
        ), patch.object(
            job, "_load_user_data", return_value=user_df
        ):
