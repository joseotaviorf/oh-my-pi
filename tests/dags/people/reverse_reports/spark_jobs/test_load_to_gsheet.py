...
from tests.dags.people.reverse_reports.spark_jobs.conftest import MOCK_WORKSHEET_NOT_FOUND
...
class TestIsRetriableGsheetsError(unittest.TestCase):
    ...
    def test_429_is_retriable(self):
        self.assertTrue(job._is_retriable_gsheets_error(Exception("429 Too Many Requests")))

    def test_503_is_retriable(self):
        self.assertTrue(job._is_retriable_gsheets_error(Exception("503 Service Unavailable")))

    def test_resource_exhausted_is_retriable(self):
        self.assertTrue(job._is_retriable_gsheets_error(Exception("RESOURCE_EXHAUSTED")))
    ...
    def test_permission_denied_is_not_retriable(self):
        self.assertFalse(job._is_retriable_gsheets_error(Exception("PERMISSION_DENIED")))
...
class TestGetAuth(unittest.TestCase):
    ...
    def test_returns_credentials_and_scope(self):
        ...
        credentials, scope = job._get_auth(dbutils, "people", "GOOGLE_SERVICE_ACCOUNT_CREDENTIALS_PEOPLE")
        ...
        dbutils.secrets.get.assert_called_once_with(scope="people", key="GOOGLE_SERVICE_ACCOUNT_CREDENTIALS_PEOPLE")
...
class TestGetPayloadsFromTable(unittest.TestCase):
    ...
    def test_returns_header_and_rows_excluding_partition_cols(self, mock_spark_client_cls):
    ...
    def test_converts_none_to_empty_string(self, mock_spark_client_cls):
        ...
        _, rows = job._get_payloads_from_table(
            mock_spark_client, "jobs", "2025-03-12"
        )
...
class TestMain(unittest.TestCase):
    ...
    def test_main_success_prod_environment(
        self, mock_base_dbutils, mock_spark_cls, mock_gsheets_client_cls, mock_writer_cls
    ):
        ...
        with patch.object(job, "_ensure_worksheet_exists"):
            with patch("sys.argv", [
                "script", "jobs", "2025-03-12", "prod",
                "prod-sheet-id-123", "JobsTab"
            ]):
                job.main()

        mock_writer.write.assert_called_once_with("JobsTab", "prod-sheet-id-123", [
            ["id_user", "user_name"],
            ["1", "Alice"],
        ])
    ...
    def test_main_uses_forno_sheet_id_when_environment_forno(
        ...
        with patch.object(job, "_ensure_worksheet_exists"):
            with patch("sys.argv", [
                "script", "salary_tables", "2025-03-12", "forno",
                "ignored-sheet-id", "SalaryTab"
            ]):
                job.main()
