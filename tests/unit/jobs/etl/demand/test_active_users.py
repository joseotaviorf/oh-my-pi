import mock
import pandas as pd
from bietlejuice.jobs.etl.demand import ActiveUsersETL, DemandEnum


class TestActiveUsersETL(object):

    @mock.patch.object(ActiveUsersETL, '_move_to_datalake')
    def test_move_to_datalake(self, mock__move_to_datalake, active_users_etl):
        # arrange
        df = pd.DataFrame(data=[1], columns=['id'])
        period = mock.ANY
        expected_cols_length = 15
        expected_table_name = DemandEnum.ACTIVE_USERS.value

        # act
        active_users_etl.move_to_datalake(df=df, period=period)
    ...
    @mock.patch.object(ActiveUsersETL, '_extract_data')
    def test_extract_data(self, mock__extract_data, active_users_etl):
        # arrange
        period = mock.ANY
        expected_table_name = DemandEnum.ACTIVE_USERS.value

        # act
        active_users_etl.extract_data(period=period)
