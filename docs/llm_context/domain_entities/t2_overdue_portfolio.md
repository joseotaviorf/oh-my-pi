...
- **Target mapping — `sandbox.planning_performance_fact_daily_targets`** → this shared target table carries daily rate targets (`decimal(18,6)`, e.g. `0.5511` = 55.11%) for several for-rent business lines at once (Collections IQ/tenant — this entity —, Collections PP/landlord, AR, Evictions), joined by `dt_reference`. The Collections IQ (tenant) columns map onto this entity's fields as follows — confirmed by the requester, column by column:
  ...
  | `total_active_new_defaulter_recovery` | `portfolio = 'c) active-new-defaulter'` **alone** — despite the "total" prefix, this is not an aggregation of buckets `a`+`b`+`c`; it matches bucket `c` in isolation. Confirmed by the requester. |
  | `bpo_active_new_defaulter_recovery` | Same population as `total_active_new_defaulter_recovery` (`portfolio = 'c) active-new-defaulter'`), restricted to external collection agencies (BPO) only |
  ...
  | `net_recovery_active_stock` | Active debts only, excluding invoices from already-ended contracts — same population as `active_stock_recovery` (`portfolio = 'd) active-stock'`); the two target columns coexist under different names, kept as-is pending consolidation |
  | `net_recovery_atpj` | `is_atpj_portfolio = TRUE` |
  | `net_recovery_fpd` | Same population as `active_new_defaulter_first_payment_default_recovery` (`portfolio = 'a'`) |
...
**Do:**
- Apply `collectable_delinquent_portfolio = TRUE` for any analysis meant to match the official Net Recovery universe.
...
- Don't assume `net_recovery_active_stock` / `active_stock_recovery` or `net_recovery_fpd` / `active_new_defaulter_first_payment_default_recovery` are two independent targets — both pairs currently map to the same T2 population under different column names, pending consolidation.
