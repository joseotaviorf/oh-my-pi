...
- **Favorites** (outbound) → Concierge trigger that recommends similar listings based on the user's favorites. All variants share the generic comms-manager action `ConciergeOutboundNotification_whatsapp` and are identified by WhatsApp templates matching `concierge_favorites%` (presentation = first notification to that user; `_var1` / `_var2` = copy variants; `_3cards` / `_4cards` / `_5cards` = carousel size) (`concierge_flow_type = 'Favorites'`).
