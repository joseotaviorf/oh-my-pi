...
| Simulation detail (per-simulation rows) | `datalake_consorcio_clean.simulation` — only for raw rows; `deal` already carries `total_simulations` and the first / last / average simulation amounts |
| Business-day calendar (used in the table build) | `dw_public.dim_date` — `is_brz_business_day` |
...
The business input behind attribution lives in two team-maintained Google Sheets, materialized daily into `datalake_gsheets_clean`. Editing the sheet changes the table on the next build — treat them as source of truth for the mapping, never as an ad-hoc scratchpad.
...
### Query 2 — Audit the analyst attribution history

Use this to explain why a deal landed on a given supervisor, or to review the relationship history behind `deal.supervisor_name`. It reads the sheet directly; the deal columns are the resolved result.

```sql
-- One row per analyst relationship window, with the deals attributed to it.
-- dt_created / dt_ended are varchar 'YYYY-MM-DD' in the sheet -- CAST before comparing.
-- dt_ended empty/NULL means the relationship is still open.
SELECT
    op.id_hubspot_owner,
    op.analyst_name,
    op.role,
    op.supervisor,
    op.team,
    CAST(NULLIF(op.dt_created, '') AS DATE) AS dt_relationship_start,
    CAST(NULLIF(op.dt_ended,   '') AS DATE) AS dt_relationship_end,
    COUNT(d.id_deal)                        AS deals_created_in_window
FROM datalake_gsheets_clean.consorcio_inside_sales_operation op
LEFT JOIN datalake_consorcio.deal d
       ON CAST(d.id_hubspot_owner AS VARCHAR) = op.id_hubspot_owner
      AND d.dt_created >= CAST(NULLIF(op.dt_created, '') AS DATE)
      AND (op.dt_ended IS NULL OR op.dt_ended = ''
           OR d.dt_created <= CAST(op.dt_ended AS DATE))
      AND d.year = 2026
GROUP BY 1, 2, 3, 4, 5, 6, 7
ORDER BY op.analyst_name, dt_relationship_start
```
