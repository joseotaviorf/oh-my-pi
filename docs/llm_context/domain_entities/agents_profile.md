...
| **CIQ não-demanda aluguel** | Non-Demand Rent; Agent that can acquire supply on Rent with no visit capability. | `agent.profile = 'AUTONOMOUS_BROKERAGE_AGENT'` AND `agent.is_allow_demand_visit_management = FALSE` AND `agent.is_allow_supply_acquisition = TRUE` AND `prospect.business_context_applied = 'RENT'` |
| **CIQ não-demanda venda** | Non-Demand Sale; Agent that can acquire supply on Sale with no visit capability. | `agent.profile = 'AUTONOMOUS_BROKERAGE_AGENT'` AND `agent.is_allow_demand_visit_management = FALSE` AND `agent.is_allow_supply_acquisition = TRUE` AND `prospect.business_context_applied = 'SALE'` |
...
| **Non-Demand Rent** | `agent.profile = 'AUTONOMOUS_BROKERAGE_AGENT' AND agent.is_allow_demand_visit_management = FALSE AND agent.is_allow_supply_acquisition = TRUE AND prospect.business_context_applied = 'RENT'` | `agent` + `prospect` |
| **Non-Demand Sale** | `agent.profile = 'AUTONOMOUS_BROKERAGE_AGENT' AND agent.is_allow_demand_visit_management = FALSE AND agent.is_allow_supply_acquisition = TRUE AND prospect.business_context_applied = 'SALE'` | `agent` + `prospect` |
...
SELECT
    ...
    agent.is_allow_demand_visit_management,
