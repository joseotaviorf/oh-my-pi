...
| **CIQ não-demanda aluguel** | Non-Demand Rent; Agent that can acquire supply on Rent with no visit capability. | `agent.profile = 'AUTONOMOUS_BROKERAGE_AGENT'` AND `agent.is_allow_visit = FALSE` AND `agent.is_allow_supply_acquisition = TRUE` AND `prospect.business_context_applied = 'RENT'` |
| **CIQ não-demanda venda** | Non-Demand Sale; Agent that can acquire supply on Sale with no visit capability. | `agent.profile = 'AUTONOMOUS_BROKERAGE_AGENT'` AND `agent.is_allow_visit = FALSE` AND `agent.is_allow_supply_acquisition = TRUE` AND `prospect.business_context_applied = 'SALE'` |
...
- **Agents by business profile:** `COUNT(DISTINCT sk_agent)` on `dw_agent.dim_agent` using the [Classification rules](#classification-rules-source-of-truth); filter `status = 'ACTIVE'`.
