...
- **Demand**, **demanda**, **session**, **sessão**, **contact**, **contato** → an in-scope Isaias session (one row in `isaias_session_attribution`); the terms are used interchangeably here
...
**Included**: in-scope Isaias sessions across both hosts (`bot` = standalone Isaias or Isaias inside Wall-E / Mora) from `datalake_supply_flows.isaias_session_attribution`, and their **validly attributed** `opportunity` / `first_listing` conversions from `dw_growth.obt_supply`, on **coincident date** (`event_date`) within the analysis window. Segmentable by `bot`, `source_environment`, `nm_business_context` (RENT / SALE), and time grain (`event_date`).

**Excluded**: sessions not present in `isaias_session_attribution`; conversions credited to a session that is not the lead's last touching session (invalid attribution); conversions whose `event_date` falls outside the window; any signal that would require Langfuse behaviour flags (Langfuse is ad-hoc only, not a metric source).
...
```sql
-- session base
d.ts_session_start >= from_iso8601_timestamp('{start_date}T00:00:00Z')
-- obt lead lookback ({obt_start_date} = a few days before {start_date}, to catch leads created just before their attributing session)
o."date" >= DATE {obt_start_date}
```

**Data availability**: `datalake_supply_flows.isaias_session_attribution` has no historical backfill — it begins in **August 2026**, so `{start_date}` cannot be earlier than `2026-08-01` (earlier windows return no rows).
...
| `department` | From the Sauron / support-services join; `LOWER(department) LIKE '%is%'` marks escalation to Inside Sales. |
| `bot` / `source_environment` | Segmentation dimensions (host and entry channel). |
...
**Session identity**: sessions are counted by `COUNT(DISTINCT id_langfuse_session)`, not `id_sauron_session`. Every in-scope Isaias (chatbot) session always has a Langfuse session, so `id_langfuse_session` is a complete, non-lossy session key — no session is dropped from the denominator. Keep the same key in numerator-adjacent session counts (escalation) and the denominator.
...
FROM (
    -- ===== Session-supply ledger — trimmed to metric columns; full version in supply.md Query 2 =====
    WITH
    session_dim AS (
        SELECT
            ...
            ss.source_environment, ss.department,
            d.ts_session_start
    ...
    lead_session_candidates AS (
        ...
        UNION
        SELECT CAST(o.sk_lead AS VARCHAR), s.id_sauron_session, s.ts_session_start
        FROM dw_growth.obt_supply o
        JOIN session_dim s ON o.sk_chat_session = s.id_sauron_session
        WHERE o.sk_chat_session IS NOT NULL AND o.sk_lead IS NOT NULL
    ...
    conv_rows AS (
        ...
        JOIN session_dim sd ON va.id_session = sd.id_sauron_session
    ...
    valid_supply AS (
        SELECT id_sauron_session, sk_supply, nm_business_context,
