...
- **Effective worker / efetivo (excludes interns & apprentices)** → `is_effective_worker = TRUE` (permanent/effective employees; FALSE for Interns and Young Apprentices). Stable for the life of the assignment; ready-made alternative to the `dim_job.employment_type` join
