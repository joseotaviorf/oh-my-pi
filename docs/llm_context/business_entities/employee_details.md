...
- **Intern/apprentice effectivation / efetivação / conversion to CLT** → `is_effectivation_hire = TRUE` (the incoming permanent assignment when an intern/apprentice is effectivated) and `is_effectivation_termination = TRUE` (the closed intern/apprentice assignment); unlike a transfer, an effectivation brings a new person into the effective workforce, so `is_effectivation_hire` **counts** as a new hire (`is_turnover_new_hire`)
- **Termination count / desligamentos (default)** → `is_turnover_termination = TRUE` — the default leaver count; already excludes internal transfers and interns/apprentices. To **include interns/apprentices** in a termination count, drop the effective-worker restriction but exclude conversions and moves: `is_effectivation_termination = FALSE` (an effectivation closure is a conversion to CLT, not an exit) and `is_transfer_termination = FALSE`
- **New hire / admission / genuine hire / admissão** → `is_turnover_new_hire = TRUE` (genuine company admission — first hire or rehire — by an effective worker; excludes internal transfer-ins and interns/apprentices; intern/apprentice effectivations count as new hires), true on the employee's monthly closing snapshot in the hire month. Count per month with `COUNT(DISTINCT person_number)`; rehires and effectivations count, transfers do not
- **Turnover flag / real exit / leaver flag** → `is_turnover_termination = TRUE` (real company exit — voluntary, involuntary, or layoff — by an effective worker; excludes internal transfers and interns/apprentices), true on the employee's monthly closing snapshot in the termination month; ready-made alternative to `employment_status = 'Terminated'` + the intern/JA filter. It **includes Layoffs**, so add `is_reorganization_termination = FALSE` to match the official layoff-excluded turnover
- **Effective worker / efetivo (excludes interns & apprentices)** → `is_effective_worker = TRUE` (permanent/effective employees; FALSE for Interns and Young Apprentices); ready-made alternative to the `dim_job.employment_type` join
- **Turnover-eligible base / headcount base / turnover denominator** → `is_eligible_to_turnover = TRUE` (effective workers already in the company this month; excludes interns/apprentices and this month's new hires, includes transfer-ins and terminated employees); the ready-made denominator base for turnover rates
