...
| **Offer by Agent / Oferta por Agente / Submissão por Corretor** | Feature que permite a um corretor (agente) submeter uma proposta de compra em nome do comprador. Identificada por `actor_type = 'AGENT'` e `uuid_actor IS NOT NULL` em `datalake_sales_flow_clean.offer`. Quando ambos são `NULL`, a oferta foi submetida diretamente pelo buyer. Não há atualmente uma flag dedicada em `dw_sale.fact_offers` — para segmentar, fazer join com `datalake_sales_flow_clean.offer` via `id` (equivalente ao `id_sales_flow`). | `datalake_sales_flow_clean.offer.uuid_actor`, `datalake_sales_flow_clean.offer.actor_type` |
...
| **TIC / Termo de Intermediação Compartilhada** | Documento contratual obrigatório nas transações FS Marketplace mediadas por imobiliárias parceiras (`payment_model = 'CLOSING_3P'` — programas TSC e CQA). Formaliza o papel da imobiliária como intermediária responsável pela due diligence, CCV e pós-venda. Gerado via contract-filler e assinado pelas partes. A criação e upload podem ser feitos pela imobiliária no CDI (Central da Imobiliária) ou, no fluxo legado, por um expert no Vendas. **Exclusivo a ofertas 3P** — nunca aparece em transações 1P (`CCV_ASSISTANCE_CASH` ou `CCV_ASSISTANCE_FINANCED`). | `datalake_sales_flow_clean.tic`, `datalake_sales_flow_raw.tic` |
| **TIC\_CLOSING\_CASH / TIC\_CLOSING\_FINANCED** | ClosingTypes criados para ofertas 3P com `payment_model = CLOSING_3P`. Determinam que o step de TIC é obrigatório no status list do sales flow. `TIC_CLOSING_CASH` para pagamento à vista; `TIC_CLOSING_FINANCED` para financiado. | `datalake_sales_flow_clean.sales_flow` (`closing_type`) |
| **CDI / Central da Imobiliária** | App usado pelas imobiliárias parceiras para gerenciar leads, visitas e ofertas. Canal principal para criação/upload do TIC no novo fluxo 3P. Contraponto ao Vendas (interno). | — |
...
| `tic` | `datalake_sales_flow_clean` | 1 linha por TIC por sales flow | **Exclusivo a ofertas 3P** (`payment_model = 'CLOSING_3P'`, closing types `TIC_CLOSING_CASH` / `TIC_CLOSING_FINANCED` — programas TSC e CQA). Cada linha representa o ciclo de vida de um Termo de Intermediação Compartilhada: geração via contract-filler, review pela imobiliária no CDI e upload final junto ao CCV. Colunas principais: `id_sales_flow` (FK → `datalake_sales_flow_clean.sales_flow`), `uuid_signed_document_reference` e `uuid_unsigned_document_reference` (BlobVault objectIds no bucket `sales-flow-tic`), `status` (`CREATED` → `SIGNED`), `signed_document_origin` (`CDI` = upload direto pela imobiliária no CDI; `VENDAS` = upload via backoffice Vendas, seja pelo fluxo legado de expert ou por solicitação da imobiliária via canais de atendimento), `template_version`. Alinha com o macrostatus `TIC` e microstatuses `TIC_CREATED` / `TIC_SIGNED` do sales flow. Para histórico de mudanças, usar `datalake_sales_flow_clean.tic_aud`. |
| `tic_aud` | `datalake_sales_flow_clean` | 1 linha por revisão Envers do TIC | Histórico de auditoria Hibernate Envers da entidade TIC. Cada linha é uma revisão de um registro `tic`, rastreando transições de `status` (CREATED → SIGNED), mudanças em referências de documento e origem do upload. Join `rev` → `datalake_sales_flow_clean.revinfo.id` para obter timestamp e autor da revisão. Colunas `mod_*` indicam se o campo mudou nessa revisão. Usar para análise point-in-time do ciclo de vida do documento TIC. |
| `offer` | `datalake_sales_flow_clean` | 1 linha por oferta | Réplica CDC da tabela `offer` do banco Sales Flow. Use `uuid_actor` e `actor_type` para identificar se a oferta foi submetida diretamente pelo buyer (`NULL`) ou por um corretor via a feature **Offer by Agent** (`actor_type = 'AGENT'`, `uuid_actor` = UUID do corretor). Preferir `dw_sale.fact_offers` para análises EoF; consultar esta tabela diretamente apenas para os campos `uuid_actor`/`actor_type` enquanto não surfaçados no DW. Join com `fact_offers` via `id` → `id_sales_flow`. |
| `offer_aud` | `datalake_sales_flow_clean` | 1 linha por revisão Envers da oferta | Histórico de auditoria Hibernate Envers da entidade `offer`. Cada linha é uma revisão de um registro `offer`, rastreando mudanças em status, preços e atores. Colunas `mod_*` indicam se o campo mudou nessa revisão. Use `uuid_actor`/`actor_type` e `mod_uuid_actor`/`mod_actor_type` para rastrear mudanças no ator da oferta. Join `rev` → `datalake_sales_flow_clean.rev_info.id` para obter timestamp da revisão. |
