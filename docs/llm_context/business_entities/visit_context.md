...
| Post-visit qualitative feedback from demand | `dw_visit.dim_post_visit_demand` (owned by **visits**) — one row per visit; join on `sk_visit`; includes `channel`, ratings, and `is_visit_completed_by_demand` |
