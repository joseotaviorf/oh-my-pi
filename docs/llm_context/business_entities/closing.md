...
| CC2CS funnel timestamps and pre-computed SLA | `fact_listing_rent_flows` (`flrf`) — **owned by [`fr_transact.md`](fr_transact.md)** (the funnel/SLA fact). Closing joins to it via `flrf.sk_contract = dc.sk_contract` for `cc2cs_working_minutes` (working-time leadtime from creation to signature) and the contract-created/signed anchors. Referenced, not owned here. |
...
| Offer attributes (one row per offer on a property) | `dw_rent.dim_offer` (`do`) — **owned by [`fr_transact.md`](fr_transact.md)**. Closing references it only for Early Demand detection: `dt_first_sent` (the "offer date"), `sk_offer` (PK), `id_property`, `id_user`, `status` (`Aprovada`/`EmNegociacao`/`Rejeitada`), `dt_analysis` (owner decision). Does not carry `sk_contract` — walk offer → contract via `fact_contracts`. Referenced, not owned here. |
| House listing publication status | `dw_rent.dim_house_listing` — `status = 'PUBLISHED'` indicates the house is actively listed. |
| Contract-stage event volume/timing (contract created `CC`, contract signed `CS`) | `dw_rent.fact_rent_demand_events` (`fde`) + `dw_rent.dim_rent_event_type` (`det`) — the **enriched** event funnel (owned by [`fr_transact.md`](fr_transact.md)); already applies the business rules to derive clean contract events. **Prefer this** over the raw audit trail for counting or timing contract-stage events. |
| Transition *reasons*, draft-approval step, cancellation timing, point-in-time status | `datalake_ebdb_clean.contract_aud` (`ca`) — raw Hibernate-Envers revision log (one row per contract revision). The **authoritative source** for detail the enriched funnel does not carry: the transition *reason* (e.g. auto Offer Express vs CRM), the intermediate `Minuta → PreAssinaturas` draft-approval step, cancellation-transition timing, and field-level point-in-time status history. It is raw (needs business rules to turn revisions into events), so for plain contract-stage volume/timing (`CC`/`CS`) prefer the enriched funnel above — but reach for `contract_aud` whenever the question is about *why*, *when it changed*, or *the prior state*. Columns: `status`, `mod_status` (true when `status` changed in that revision), `ts_database_transaction`, `rev`. JOIN the revision metadata `user_revision_entity` (`ure`, owned by [`fr_transact.md`](fr_transact.md)) on `ca.rev = ure.id` for `reason` / `id_user`. |
...
- **Enriched funnel vs. raw audit — mind the hierarchy:** for contract-stage volume/timing (`CC`/`CS`), prefer the enriched `dw_rent.fact_rent_demand_events` + `dw_rent.dim_rent_event_type` funnel (see [`fr_transact.md`](fr_transact.md)) — it already applies the business rules. `contract_aud` / `user_revision_entity` are the raw Hibernate-Envers logs and remain the **right source** when you need the transition *reason*, the `Minuta → PreAssinaturas` draft-approval step, cancellation-transition timing, or point-in-time field-level history — the funnel does not surface those. Prefer enriched for volume; use the audit for *why / when it changed / prior state*.
...
### FR Transact (closing is the final sub-stage of the For Rent pre-contract funnel)

- Closing (contract created → sent → signed) is the last block of the broader **FR Transact** pre-contract funnel (visit → offer → proposal + credit → contract). For the upstream stages (offer `dim_offer`, proposal + credit `dim_proposal`, event-native funnel volume/conversion, CA2CC/CA2CS lead times, and drop reasons by funnel step), see [`fr_transact.md`](fr_transact.md). This doc owns the closing-specific detail (signatories, legal representatives, CC2CS leadtime, cancellation reasons).
...
- Don't reach for the raw `datalake_ebdb_clean.contract_aud` audit trail to *count or time* contract-stage events (`CC`/`CS`) when the enriched `dw_rent.fact_rent_demand_events` + `dw_rent.dim_rent_event_type` funnel already provides them as clean events — but do use `contract_aud` when the question needs the transition *reason* (`user_revision_entity.reason`), the `Minuta → PreAssinaturas` draft-approval step, cancellation timing, or the prior state, none of which the funnel exposes
