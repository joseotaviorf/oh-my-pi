...
- `business_entities/alias.md` — Alias B2B conversational AI agent for partner agencies: lead → session → resolution funnel, AI agent performance (LLM cost/latency), inventory ingestion, and portfolio listings (agente do Alias / imobiliárias parceiras / produto 40)
...
- `business_entities/cdp.md` — Customer Data Platform (CDP / Event Gateway): governed behavioral events (`datalake_cdp_clean`), active personas (`datalake_cdp`), historical personas (`datalake_cdp_personas`), business objects for AI (Domi, chatbots — `datalake_transactional_entities.entities`)
...
- `business_entities/fr_transact.md` — FR Transact — For Rent pre-contract transaction funnel: visit → offer (OS/OA) → proposal + credit (ES/EP/DS/CA) → contract created (CC) → sent for signature (Minuta/PreAssinaturas) → signed (CS) → active; funnel volumes & conversion, lead times (CA2CC/CA2CS/CC2CS), drop reasons by stage, Minuta→PreAssinaturas audit transitions via `contract_aud`, and the optional **advance payment (sinal)** product (`datalake_rental_transact_clean.advance_payment` + `event_log`; `ADVANCE_PAYMENT`-stage funnel events) (funil pré-contrato de locação / fluxo de locação / oferta / proposta / análise de crédito / minuta / pré-assinatura / sinal / pagamento antecipado). The CC→signed closing sub-stage is detailed in `closing.md`.
...
- `business_entities/jira_bugs.md` — Data & AI incidents/bugs from the Jira "Form S&S Dados" (request type 9228): line/squad ownership, SLA status, and the `incident_responsible` team mapping (bug / incidente de dados / chamado de dados)
...
- `business_entities/roas_ecm_credit_impact.md` — ROAS / ECM credit impact: measuring whether new eCM (ELTV) model versions improve acquired TP credit quality (mix, OA2CA, ES2EP) via ROAS API calls on VB/OS stored in EMLIO (ROAS / eCM / mix de crédito / EMLIO / eltv)
...
- `metric_entities/escalation_rate_fr_offboarding.md` — Escalation Rate FR Offboarding: share of `ForRentOffboardingAgentV1` (wall_e-tagged) sessions escalated to a human agent, per day, segmented by `is_escalated` on `datalake_chatbot.sessions` (taxa de escalonamento / escalation rate / offboarding FR). Builds on `business_entities/chatbot_sessions.md`.
