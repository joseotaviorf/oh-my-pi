with all_dates as (
    ...
    dense_rank() over (partition by coalesce(dr.long_region_name, ''),
    	...
    	+ dense_rank() over (partition by coalesce(dr.long_region_name, ''),
			...
			- 1 as monthly_count,
    dense_rank() over (partition by coalesce(dr.long_region_name, ''),
                                    date_part('year', dof.dt_analysis) order by f.sk_client asc)
    	+ dense_rank() over (partition by coalesce(dr.long_region_name, ''),
    	                                  date_part('year', dof.dt_analysis) order by f.sk_client desc)
			- 1 as yearly_count
	from fact_demand f
	join dim_offer dof
  ...
  order by coalesce(dr.long_region_name, ''), date_part('year', dof.dt_analysis), date_part('month', dof.dt_analysis), date_part('week', dof.dt_analysis), date_part('day', dof.dt_analysis)
),
...
all_dates_last_month as (
	...
	where date_part('year', dof.dt_analysis) = date_part('year', add_months(current_date, -1))
 	...
 	group by coalesce(dr.long_region_name, ''), date_part('year', dof.dt_analysis), date_part('month', dof.dt_analysis)
  order by coalesce(dr.long_region_name, ''), date_part('year', dof.dt_analysis), date_part('month', dof.dt_analysis)
),
all_dates_last_year as (
  ...
  left join dim_region dr
	...
	group by coalesce(dr.long_region_name, ''), date_part('year', dof.dt_analysis)
	order by coalesce(dr.long_region_name, ''), date_part('year', dof.dt_analysis)
),
