  ...
  select distinct
    ...
    dense_rank() over (partition by date_part('year', dp.dt_tenant_first_document_sent),
    	...
    	+ dense_rank() over (partition by date_part('year', dp.dt_tenant_first_document_sent),
			...
			- 1 as monthly_count,
    dense_rank() over (partition by date_part('year', dp.dt_tenant_first_document_sent) order by dp.sk_proposal asc)
    	+ dense_rank() over (partition by date_part('year', dp.dt_tenant_first_document_sent) order by dp.sk_proposal desc)
			- 1 as yearly_count
