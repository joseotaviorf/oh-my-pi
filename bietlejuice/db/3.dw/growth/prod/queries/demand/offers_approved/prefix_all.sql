  ...
  select distinct
    ...
    dense_rank() over (partition by date_part('year', dof.dt_approved),
    	...
    	+ dense_rank() over (partition by date_part('year', dof.dt_approved),
			...
			- 1 as monthly_count,
    dense_rank() over (partition by date_part('year', dof.dt_approved) order by dof.sk_offer asc)
    	+ dense_rank() over (partition by date_part('year', dof.dt_approved) order by dof.sk_offer desc)
			- 1 as yearly_count
	from fact_demand f
	join dim_offer dof
