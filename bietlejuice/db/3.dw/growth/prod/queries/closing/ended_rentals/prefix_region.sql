...
all_dates_last_month as (
	...
	where date_part('year', dc.dt_contract_annulment) = date_part('year', add_months(current_date, -1))
 	...
 	group by coalesce(dr.long_region_name, ''), date_part('year', dc.dt_contract_annulment), date_part('month', dc.dt_contract_annulment)
  order by coalesce(dr.long_region_name, ''), date_part('year', dc.dt_contract_annulment), date_part('month', dc.dt_contract_annulment)
),
all_dates_last_year as (
  ...
  left join dim_region dr
	...
	group by coalesce(dr.long_region_name, ''), date_part('year', dc.dt_contract_annulment)
	order by coalesce(dr.long_region_name, ''), date_part('year', dc.dt_contract_annulment)
),
