with all_dates_prev as (
    ...
    rank() over (partition by coalesce(dr.long_region_name, ''),
                                    date_part('year', f.dt_opportunity),
    																...
    																date_part('day', f.dt_opportunity) order by f.sk_lead asc)
    	+ rank() over (partition by coalesce(dr.long_region_name, ''),
    	                                  date_part('year', f.dt_opportunity),
    																		date_part('month', f.dt_opportunity),
    																		date_part('week', f.dt_opportunity),
    																		date_part('day', f.dt_opportunity) order by f.sk_lead desc)
			- 1 as daily_count,
    rank() over (partition by coalesce(dr.long_region_name, ''),
                                    date_part('year', f.dt_opportunity),
    																date_part('week', f.dt_opportunity) order by f.sk_lead asc)
    	+ rank() over (partition by coalesce(dr.long_region_name, ''),
    	                                  date_part('year', f.dt_opportunity),
    																		date_part('week', f.dt_opportunity) order by f.sk_lead desc)
			- 1 as weekly_count,
    rank() over (partition by coalesce(dr.long_region_name, ''),
                                    date_part('year', f.dt_opportunity),
    																date_part('month', f.dt_opportunity) order by f.sk_lead asc)
    	+ rank() over (partition by coalesce(dr.long_region_name, ''),
    	                                  date_part('year', f.dt_opportunity),
    																		date_part('month', f.dt_opportunity) order by f.sk_lead desc)
			- 1 as monthly_count,
    rank() over (partition by coalesce(dr.long_region_name, ''),
                                    date_part('year', f.dt_opportunity) order by f.sk_lead asc)
    	+ rank() over (partition by coalesce(dr.long_region_name, ''),
    	                                  date_part('year', f.dt_opportunity) order by f.sk_lead desc)
			- 1 as yearly_count
	from fact_supply f
	join dim_lead dl
		on f.sk_lead = dl.sk_lead
		  and f.dt_opportunity >= '2017-01-01' and f.dt_opportunity < current_date
		  and f.sk_lead != -1
	join dim_property dpr
		on f.sk_property = dpr.sk_property
	left join dim_region dr
		on dpr.regiao_id = dr.id
  order by coalesce(dr.long_region_name, ''), date_part('year', f.dt_opportunity), date_part('month', f.dt_opportunity), date_part('week', f.dt_opportunity), date_part('day', f.dt_opportunity)
),
all_dates as (
  select distinct
    _year,
...
all_dates_last_month as (
	...
	where date_part('year', f.dt_opportunity) = date_part('year', add_months(current_date, -1))
 	...
 	group by coalesce(dr.long_region_name, ''), date_part('year', f.dt_opportunity), date_part('month', f.dt_opportunity)
  order by coalesce(dr.long_region_name, ''), date_part('year', f.dt_opportunity), date_part('month', f.dt_opportunity)
),
  ...
  left join dim_region dr
	...
	where date_part('year', f.dt_opportunity) = date_part('year', add_months(current_date, -12))
  		and ((date_part('month', f.dt_opportunity) = date_part('month', add_months(current_date, -12))
  		  ...
  		  or date_part('month', f.dt_opportunity) < date_part('month', add_months(current_date, -12))
  		  )
