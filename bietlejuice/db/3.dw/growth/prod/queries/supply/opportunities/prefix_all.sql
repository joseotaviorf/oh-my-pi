  ...
  select distinct
    ...
    rank() over (partition by date_part('year', f.dt_opportunity),
    	...
    	+ rank() over (partition by date_part('year', f.dt_opportunity),
			...
			- 1 as monthly_count,
    rank() over (partition by date_part('year', f.dt_opportunity) order by f.cap_id asc)
    	+ rank() over (partition by date_part('year', f.dt_opportunity) order by f.cap_id desc)
			- 1 as yearly_count
