with all_dates_prev as (
    ...
    rank() over (partition by 5, 1, 2, 3, 4 order by f.sk_lead asc)
    	+ rank() over (partition by 5, 1, 2, 3, 4 order by f.sk_lead desc)
			- 1 as daily_count,
    rank() over (partition by 5, 1, 3 order by f.sk_lead asc)
    	+ rank() over (partition by 5, 1, 3 order by f.sk_lead desc)
			- 1 as weekly_count,
    rank() over (partition by 5, 1, 2 order by f.sk_lead asc)
    	+ rank() over (partition by 5, 1, 2 order by f.sk_lead desc)
			- 1 as monthly_count,
    rank() over (partition by 5, 1 order by f.sk_lead asc)
    	+ rank() over (partition by 5, 1 order by f.sk_lead desc)
			- 1 as yearly_count
	from fact_supply f
