with all_dates_prev as (
  select distinct
    ...
    rank() over (partition by date_part('year', f.dt_qualified),
    ...
    rank() over (partition by date_part('year', f.dt_qualified),
    	...
    	+ rank() over (partition by date_part('year', f.dt_qualified),
			...
			- 1 as monthly_count,
    rank() over (partition by date_part('year', f.dt_qualified) order by f.sk_lead asc)
    	+ rank() over (partition by date_part('year', f.dt_qualified) order by f.sk_lead desc)
			- 1 as yearly_count
	from fact_supply f
	join dim_lead dl
