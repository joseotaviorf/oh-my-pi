select
  ...
  dt_contract_annulment
			...
			from (
				...
				from (
					select
					  ...
					  if(a.fupVisita is null, false, true) as visit_performed,
	...
	select distinct
			...
			o.id_rent_flow,
			o.dt_rent_flow_created,
			(if(pp.id_pre_proposal is null, o.id_offer, null)) as id_offer,
		  	pp.id_pre_proposal,
		  	o.id_contract as o,
		  	pp.id_contract as pp,
		  	coalesce(pp.id_proposal, o.id_proposal) as id_proposal,
		  	coalesce(pp.dt_proposal_approved, o.dt_proposal_approved) as dt_proposal_approved,
		  	pp.id_contract as id_contract,
		  	pp.dt_contract_created as dt_contract_created,
		  	pp.dt_contract_signed as dt_contract_signed,
		  	pp.dt_contract_annulment as dt_contract_annulment
			from (
				select distinct
				  id_house,
				  dt_house_first_listing,
		...
		on pp.id_house = o.id_house
			and (if(pp.id_booking is not null, pp.id_booking = o.id_booking, true))
			and pp.id_rent_flow = o.id_rent_flow
		WHERE pp.id_contract IS NOT NULL
	)
	union
	(
		select
