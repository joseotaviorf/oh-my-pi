select
	...
	coalesce(max(rl.label = 'Pontualidade'), 0) as flg_punctuality,
	coalesce(max(rl.label = 'Corretor bem informado'), 0) as flg_agent_well_informed,
	coalesce(max(rl.label = 'Gentileza'), 0) as flg_kindness,
	coalesce(max(rl.label = 'Corretor não foi gentil'), 0) as flg_no_kindness,
	coalesce(max(rl.label = 'Imóvel igual ao anúncio'), 0) as flg_house_as_listing,
	coalesce(max(rl.label = 'Imóvel não corresponde ao anúncio'), 0) as flg_house_not_as_listing,
	coalesce(max(rl.label = 'Outro motivo (por favor, conte abaixo)'
		and rl.ratingReference >= 4), 0) as flg_other_reason_positive,
	coalesce(max(rl.label = 'Outro motivo (por favor, conte abaixo)'
