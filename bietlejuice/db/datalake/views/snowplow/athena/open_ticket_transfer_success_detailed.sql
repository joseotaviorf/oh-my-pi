DROP VIEW IF EXISTS datalake_snowplow_raw_prod.open_ticket_transfer_success_detailed;
CREATE OR REPLACE VIEW datalake_snowplow_raw_prod.open_ticket_transfer_success_detailed AS
WITH event_open_ticket_transfer_success AS (
    SELECT 
    ...
    WHERE 
        vendor = 'magiclink'
        AND event = 'open_ticket_transfer_success'
...
SELECT 
    ...
    json_extract_scalar(event_json, '$.data.data.event.channel_type') AS channel_type,
    ...
    day
FROM 
    event_open_ticket_transfer_success
;