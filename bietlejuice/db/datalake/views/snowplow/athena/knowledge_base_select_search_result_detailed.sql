DROP VIEW IF EXISTS datalake_snowplow_raw_prod.knowledge_base_select_search_result_detailed;
CREATE OR REPLACE VIEW datalake_snowplow_raw_prod.knowledge_base_select_search_result_detailed AS
WITH event_knowledge_base_select_search_result AS (
    SELECT 
    ...
    WHERE 
        vendor = 'magiclink'
        AND event = 'knowledge_base_select_search_result'
...
SELECT 
    ...
    json_extract_scalar(event_json, '$.data.data.event.current_document_code') AS current_document_code,
    json_extract_scalar(event_json, '$.data.data.event.selected_document_code') AS selected_document_code,
    ...
    day
FROM 
    event_knowledge_base_select_search_result
;