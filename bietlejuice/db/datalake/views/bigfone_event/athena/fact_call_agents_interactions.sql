...
******************************************************************************************************************/
drop view if exists dw_teravoz_prod.fact_call_agents_interactions;
create or replace view  dw_teravoz_prod.fact_call_agents_interactions as
with call_events as (
    select *
...
agent as (
    select
        cast(u.id as bigint) as id,
        ac.id_call,
        u.email,
        (u.id is null) as outsourcing_company
    from agents_in_call ac
    left join datalake_ebdb_raw_prod.usuario u 
...
select
    ...
    cast(date_format(tt.ts_created_entered_event, '%Y%m%d') as bigint) as sk_agent_answered_date,
    cast(date_format(tt.ts_created_entered_event_local, '%Y%m%d') as bigint) as sk_agent_answered_date_local,
    coalesce(a.email, tt.agent_email) email_agent,
    tt.extension_number as extension_number,
    date_diff('second', rti.ts_created_ring_event, rti.ts_created_next_ring_event) as seconds_call_ringing_time,
    date_diff('second', tt.ts_created_entered_event, tt.ts_created_left_event) as seconds_talk_time_agent,
