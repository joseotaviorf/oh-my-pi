...
ids_calls as (
    select
        min(regexp_extract(content,'\[(.+)\] VERBOSE\[[0-9]+\]\[C-(\w+)\].+Set\("(\w+)', 1)) as ts_time_ocurred,
...
event_start_call as (
	select
	    ...
	    case
	    	...
	    	when regexp_extract(content,'\[(.+)\] VERBOSE\[[0-9]+\]\[C-(\w+)\].+Set\("(\w+)', 3)='PJSIP' then 'call_made'
	    	when regexp_extract(content,'\[(.+)\] VERBOSE\[[0-9]+\]\[C-(\w+)\].+Set\("(\w+)', 3)='Local' then 'call_direct_received'
	    end as params,
		min(regexp_extract(content,'\[(.+)\] VERBOSE\[[0-9]+\]\[C-(\w+)\].+Set\("(\w+)', 1)) as ts_time_ocurred    
	from asterisk_data
	right join ids_calls id_c
	on (id_call=id_c.id_call and ts_time_ocurred=id_c.ts_time_ocurred)
	group by 1,2,3,4,5
),
event_end_call as (
	select
		...
		regexp_extract(content,'\[(.+)\] VERBOSE\[[0-9]+\]\[C-(\w+)\].+NoOp\("(PJSIP|SIP|Local)/\d+(@from-queue)?-(\w+)", "(HANGUP CAUSE: \d+)"\)', 1) as ts_time_ocurred
	from asterisk_data
...
event_start_ura as (
	select
		...
		regexp_extract(content,'\[(.+)\] VERBOSE\[[0-9]+\]\[C-(\w+)\].+Set\("SIP/\d+-(\w+)", "(__CRM_SOURCE=\d+)"', 1) as ts_time_ocurred
	from asterisk_data
...
event_start_queue as (
	select
		...
		regexp_extract(content,'\[(.+)\] VERBOSE\[[0-9]+\]\[C-(\w+)\].+Set\("Local/\d+@from-queue-(\w+);\d", "(QAGENT=\d+)"', 1) as ts_time_ocurred
	from asterisk_data
...
event_set_queue_num as (
	select
		...
		regexp_extract(content,'\[(.+)\] VERBOSE\[[0-9]+\]\[C-(\w+)\].+Set\("Local/\d+@from-queue-(\w+);\d", "(QUEUENUM=\d+)"', 1) as ts_time_ocurred
	from asterisk_data
...
event_set_destination as ( -- only to calls made by agents
    select
		...
		regexp_extract(content, '\[(.+)\] VERBOSE\[[0-9]+\]\[C-(\w+)\].+Set\("PJSIP/\d+-(\w+)", "(__CRM_DESTINATION=\d+)"\) in new stack', 1) as ts_time_ocurred
    from asterisk_data
    group by 1,2,3,4,5,6   
),
events_attendance as (
	select 
		...
		regexp_extract(content,'\[(.+)\] VERBOSE\[[0-9]+\]\[C-(\w+)\] app_dial.c: PJSIP/(\d+)-(\w+) answered Local/\d+@from-queue-(\w+);\d',1) as ts_time_ocurred	    
    from asterisk_data   
    group by 1,2,3,4,5
),
...
event_key_typed as (
  select 
        ...
        regexp_extract(content,'\[(.+)\] DTMF\[[0-9]+\]\[C-(\w+)\] channel.c: DTMF begin .(\d). received on (SIP|PJSIP|Local)/\d+(@from-queue)?-(\w+)', 1) as ts_time_ocurred
  from asterisk_data
  group by 1,2,3,4,5,6
),
event_audio_message as (
  select
    ...
    regexp_extract(content,'\[(.+)\] VERBOSE\[[0-9]+\]\[C-(\w+)\].+<(SIP|PJSIP)/\d+-(\w+)> Playing .?(.custom/)?(.+)\.(ulaw.?|gsm).*', 1) as ts_time_ocurred
  from asterisk_data
  group by 1,2,3,4,5,6
)
select * from event_start_call where id_call is not null 
union select * from event_end_call where id_call is not null  
union select * from event_start_ura where id_call is not null and id_phase is not null 
union select * from event_start_queue where id_call is not null and id_phase is not null
union select * from event_set_queue_num where id_call is not null and id_phase is not null and value_event is not null
union select * from event_start_attendance where id_call is not null and id_phase is not null 
union select * from event_answer_agent where id_call is not null and id_phase is not null
union select * from event_key_typed where id_call is not null and id_phase is not null
union select * from event_set_destination where id_call is not null and id_phase is not null and value_event is not null
union select * from event_audio_message where id_call is not null and id_phase is not null and value_event is not null
;