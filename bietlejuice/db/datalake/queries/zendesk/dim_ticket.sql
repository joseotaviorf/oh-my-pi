...
select
   ...
   t.recipient,
   t.tags,
   replace(cast(json_extract(t.satisfaction_rating, '$.score') as varchar), '"') as score,
   replace(cast(json_extract(t.satisfaction_rating, '$.reason') as varchar), '"') as reason,
   replace(cast(json_extract(t.satisfaction_rating, '$.comment') as varchar), '"') as comment,
