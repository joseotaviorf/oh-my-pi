...
select
    ...
    regexp_extract(first_resolution_time_in_minutes, '."calendar":"(\d+)".+', 1) as minutes_first_calendar_resolution,
    regexp_extract(first_resolution_time_in_minutes, '."business":"(\d+)".', 1) as minutes_first_business_resolution,
    ...
    reopens,
    regexp_extract(full_resolution_time_in_minutes, '."calendar":"(\d+)".+', 1) as minutes_full_calendar_resolution,
    regexp_extract(full_resolution_time_in_minutes, '."business":"(\d+)".', 1) as minutes_full_business_resolution,
    ticket_id as id_ticket,
    replies,
    regexp_extract(requester_wait_time_in_minutes, '."calendar":"(\d+)".+', 1) as minutes_calendar_requester_wait,
    regexp_extract(requester_wait_time_in_minutes, '."business":"(\d+)".', 1) as minutes_business_requester_wait,
    regexp_extract(agent_wait_time_in_minutes, '."calendar":"(\d+)".+', 1) as minutes_calendar_agent_wait,
    regexp_extract(agent_wait_time_in_minutes, '."business":"(\d+)".', 1) as minutes_business_agent_wait,
    regexp_extract(on_hold_time_in_minutes, '."business":"(\d+)".', 1) as minutes_calendar_on_hold,
    regexp_extract(on_hold_time_in_minutes, '."business":"(\d+)".', 1) as minutes_business_on_hold,
    group_stations,
