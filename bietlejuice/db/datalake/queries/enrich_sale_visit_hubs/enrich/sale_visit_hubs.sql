...
, business_units AS (
    SELECT
        bu1.*
    FROM 
        datalake_hub_services_clean.business_unit AS bu1 
    JOIN 
        (
            SELECT 
                id, 
            ...
            ON bu1.id = bu2.id 
            AND bu1.version = bu2.version
)
, users AS (
    SELECT
        u1.*
    FROM 
        datalake_hub_services_clean.users AS u1 
    JOIN 
        (
        SELECT 
            id, 
