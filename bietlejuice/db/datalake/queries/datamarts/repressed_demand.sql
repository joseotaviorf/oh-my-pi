...
dimensions as (
    select
        ...
        ds.week_start,
        ss.slot,
        case when ss.slot between 0 and 3 then 8
        ...
        case when ds.week_day between 1 and 5 and ss.slot between 0 and 3 then '1) Weekday 8-9h'
...
blocked_houses as (
--Filter status=blocked after define 'init' and 'end'
      select *
        from (
            select
...
suspended_houses as (
    ...
    from (
        ...
        join datalake_ebdb_raw_prod.usuariorevisionentity r on i.rev = r.id and status_mod = true
        ) 
...
encaixes_clean as (
    select
        ...
        slot,
        slot_share_encaixe,
        case when encaixe_realizado = 0 and (t.target_date between bh.init and bh."end") and bh.status = 'BLOCKED'
                then slot_share_encaixe end as slot_share_nao_realizados_por_bloqueio,
        case when encaixe_realizado = 0 and (t.target_date between sh.init and sh."end") and sh.status = 'suspenso'
            then slot_share_encaixe end as slot_share_nao_realizados_por_suspensao,
        case when encaixe_realizado = 0 and
        (((t.target_date between sh.init and sh."end") and sh.status = 'suspenso') or ((t.target_date between bh.init and bh."end") and bh.status = 'BLOCKED'))
                then slot_share_encaixe end as slot_share_nao_realizados_por_bloqueio_suspensao,
        ...
        case when encaixe_realizado = 0 and
        (((t.target_date between sh.init and sh."end") and sh.status = 'suspenso')
        or ((t.target_date between bh.init and bh."end") and bh.status = 'BLOCKED')
    ...
    left join suspended_houses sh 
        ...
        and t.target_date between sh.init and sh."end")
),
encaixes_agg as (
  select region_id,
           target_date,
