with events as (
	...
	select
		...
		rank() over (partition by evt.id_amplitude, evt.id_session order by evt.ts_event asc) as rnk_event_of_session
	from datalake_amplitude_clean_prod.events_repartitioned evt
	where evt.year = 2019
...
conversions as (
	...
	select
	    evt.event_time,
	    evt.id_amplitude,
	    rank() over (partition by id_amplitude order by event_time) as rnk_conversion
	from events evt
...
touchpoints as (
	...
	from events evt
	join conversions conv on conv.id_amplitude = evt.id_amplitude and conv.event_time >= evt.event_time
