...
encaixes_temp as (
  ...
  join datalake_ebdb_raw_prod.imovel i on
    i.id = cast(enc.house_id as bigint)
  join regions r on
    i.regiao_id = cast(r.region_id as bigint)
  where
    enc.rank_enc = 1
    and enc.user_id != ''
...
select
  ...
  case when encaixe_realizado = 0 and (((t.event_date between sh.init and sh."end") and sh.status = 'suspenso') or ((t.event_date between bh.init and bh."end")and bh.status = 'BLOCKED')) then slot_share_encaixe
  ...
  case
    when encaixe_realizado = 0 and (((t.event_date between sh.init and sh."end") and sh.status = 'suspenso') or ((t.event_date between bh.init and bh."end") and bh.status = 'BLOCKED')
