with house_available_hours as (
      ...
      encaixes_clean as (
          select
              ...
              slot,
              slot_share_encaixe,
              case when encaixe_realizado = 0 and (t.event_date between bh.init and bh."end") and bh.status = 'BLOCKED'
                      then slot_share_encaixe end as slot_share_nao_realizados_por_bloqueio,
              case when encaixe_realizado = 0 and (t.event_date between sh.init and sh."end") and sh.status = 'suspenso'
                  then slot_share_encaixe end as slot_share_nao_realizados_por_suspensao,
              case when encaixe_realizado = 0 and
              (((t.event_date between sh.init and sh."end") and sh.status = 'suspenso') or ((t.event_date between bh.init and bh."end") and bh.status = 'BLOCKED'))
