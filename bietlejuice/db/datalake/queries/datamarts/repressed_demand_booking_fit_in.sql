...
date_series as (
  select
    ...
    weekday_name,
    week_start,
    case when week_day = '6' then 'Saturday' else 'Weekday'
    end as week_day_type
  from
    datalake_clean.ods_dim_date dd
  where
    date(week_start) between date_trunc('week',current_date - interval '2' month) and current_date
    and date != ''
),
regions as (
  select
    distinct dr.id as region_id,
    dr.region_code,
    dr.city_group,
    dr.city_name
  from
...
encaixes_temp as (
  ...
  from
    encaixes_raw enc
  left join slot_series ss on
    ss.slot between enc.alert_slot_from and enc.alert_slot_to
  join datalake_raw.ebdb_imovel i on
    i.id = enc.house_id
  join regions r on
...
encaixes_clean as (
  select
    ...
    slot,
    slot_share_encaixe,
    case when encaixe_realizado = 0 and (t.target_date between bh.init and bh."end") and bh.status = 'BLOCKED' then slot_share_encaixe end as slot_share_nao_realizados_por_bloqueio,
    case when encaixe_realizado = 0 and (t.target_date between sh.init and sh."end") and sh.status = 'suspenso' then slot_share_encaixe end as slot_share_nao_realizados_por_suspensao,
    case when encaixe_realizado = 0 and (((t.target_date between sh.init and sh."end") and sh.status = 'suspenso') or ((t.target_date between bh.init and bh."end")and bh.status = 'BLOCKED')) then slot_share_encaixe
    ...
    case
      when encaixe_realizado = 0 and (((t.target_date between sh.init and sh."end") and sh.status = 'suspenso') or ((t.target_date between bh.init and bh."end") and bh.status = 'BLOCKED')
  ...
  left join house_available_hours hs 
    on cast(t.house_id as bigint) = hs.id_house
      ...
      and target_date between hs.available_started_date
      and coalesce(hs.available_ended_date, current_date)
