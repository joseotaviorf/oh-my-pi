...
select
    ...
    bl.lead_origin,
    case
        when lfet.id_lead is not null then lfet.tracking_source
        else bl.utm_source
    end as utm_source,
    case
        when lfet.id_lead is not null then lfet.tracking_medium
        else bl.utm_medium
    end as utm_medium,
...
left join
    sales_acquisitions a
        on a.id = f.id
left join
    base_lead_tasks_first btf
        on btf.lead_id = f.lead_id
left join
    base_lead_tasks_last btl
        on btl.lead_id = f.lead_id
left join
    sales_base_photo_tasks bpt
...
left join
    sales_leads_b2b l_b2b
        on l_b2b.id_lead = f.lead_id
left join
    public.partner_agent pa_b2b_prime
