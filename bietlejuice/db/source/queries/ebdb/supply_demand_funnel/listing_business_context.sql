...
LEFT JOIN
    (SELECT
        lbcaud.imovelid AS id_house,
        MAX(CASE WHEN businessContext = 'RENT' THEN COALESCE(from_unixtime(ure.timestamp/1000), NULL) ELSE NULL END) AS ts_opt_out_rent,
        MAX(CASE WHEN businessContext = 'SALE' THEN COALESCE(from_unixtime(ure.timestamp/1000), NULL) ELSE NULL END) AS ts_opt_out_sale
    FROM
    ...
    GROUP BY 1
        ...
        ON lbc.imovelid = opt_out.id_house
LEFT JOIN
    (SELECT
        lbc.imovelid,
    ...
    LEFT JOIN
        UsuarioRevisionEntity AS ure_rent
            ON ure_rent.id = rev.first_rev_rent
    LEFT JOIN
        UsuarioRevisionEntity AS ure_sale
            ON ure_sale.id = rev.first_rev_sale
    GROUP BY 1
    ) AS registrant
        ON lbc.imovelid = registrant.imovelid
