        ...
        select
            ...
            (
              ...
              where ua_aud_int.id = u.id
                and from_unixtime(ua_ure_int.timestamp/1000) <= occ.dataOperacao
            ) as u_max_rev,
            sum(occ.valor) as value_brl
        from OperacaoContaCorrente occ
        left join Usuario u
          on u.ContaCorrente_id = occ.ContaCorrente_id
        join Imovel i
          on i.id = occ.imovel_id
        where occ.tipo in ('valorFixoPorIndicacaoDeImovel', 'porcentagemPorIndicacaoDeImovel')
          and occ.dataOperacao = date('{str_date}')
        group by 1,2,3,4,5,6,7
    ) res
    join DadosAfiliado_AUD da_aud
      on da_aud.id = res.dadosAfiliado_id
      and da_aud.REV = res.da_max_rev
    join Usuario_AUD u_aud
      on u_aud.id = res.user_id
      and u_aud.REV = res.u_max_rev
...
from (
    ...
    resf.date_operation,
      ...
      and from_unixtime(da_ure_int.timestamp/1000) <= resf.date_operation
    ) as da_max_rev,
    (
      select max(ua_aud_int.REV)
      from Usuario_AUD ua_aud_int
     ...
     and from_unixtime(ua_ure_int.timestamp/1000) <= resf.date_operation
    ) as u_max_rev,
