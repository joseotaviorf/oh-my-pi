...
supply_daily_targets as (
	select
		date(str.date) as date,
		str.city_group,
		str.mkt_origin,
		str.mkt_channel,
		sum(coalesce(budget::float,0)) as budget,
		sum(coalesce(prospects_target::float,0)) as prospects_target,
		sum(coalesce(qualifieds_target::float,0)) as qualifieds_target,
		sum(coalesce(opportunities_target::float,0)) as opportunities_target,
		sum(coalesce(first_listings_target::float,0)) as first_listings_target
	from
...
supply_daily_spent as (
	...
	where
		co.mkt_origin in ('Price Calculator', 'Owner PWA')
		and co.funnel_side = 'supply'
	group by 1, 2, 3, 4
	)
select
	coalesce(sdt.date, sdc.date, sdf.date) as date,
	coalesce(sdt.city_group, sdc.city_group, sdf.city_group) as city_group,
	coalesce(sdt.mkt_origin, sdc.mkt_origin, sdf.mkt_origin) as mkt_origin,
	coalesce(sdt.mkt_channel, sdc.mkt_channel, sdf.mkt_channel) as mkt_channel,
	...
	sum(coalesce(sdt.budget, 0)) as budget,
	sum(coalesce(sdt.prospects_target,0)) as prospects_target,
	sum(coalesce(sdt.qualifieds_target,0))as qualifieds_target,
	sum(coalesce(sdt.opportunities_target,0)) as opportunities_target,
	sum(coalesce(sdt.first_listings_target,0)) as first_listings_target
from
	supply_daily_targets sdt
	full outer join supply_daily_spent sdc
		on  sdt."date" = sdc."date"
		and sdt.city_group = sdc.city_group
		and sdt.mkt_channel = sdc.mkt_channel
		and sdt.mkt_origin = sdc.mkt_origin
	full outer join supply_daily_funnel_counts sdf
		on  sdt."date" = sdf."date"
		and sdt.city_group = sdf.city_group
		and sdt.mkt_channel = sdf.mkt_channel
		and sdt.mkt_origin = sdf.mkt_origin
group by 1,2,3,4