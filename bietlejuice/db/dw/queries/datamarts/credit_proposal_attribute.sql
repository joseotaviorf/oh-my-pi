...
proponent AS (
    SELECT
        ...
        income_nature
    FROM
        datalake_sorting_hat_clean_prod.proponent
    ),
...
credit_analysis AS (
	...
	FROM datalake_sorting_hat_clean_prod.credit_analysis AS ca
	INNER JOIN 
	    last_status AS ls
...
LEFT JOIN
    datalake_sorting_hat_clean_prod.screening_result AS sr
...
LEFT JOIN
    tenants AS t
        ON rfb.sk_proposal = t.proposta_id
