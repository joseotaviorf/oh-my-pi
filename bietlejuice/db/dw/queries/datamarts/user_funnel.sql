...
-- order of events
	row_number() over(partition by sk_client order by nullif(sk_booking_created_date,-1)) as order_booking_created,
	row_number() over(partition by sk_client order by nullif(sk_visit_date,-1) asc, flg_visit_completed desc) as order_visit,
	row_number() over(partition by sk_client order by nullif(sk_reservation_created_date,-1)) as order_reservation,
	row_number() over(partition by sk_client order by nullif(sk_offer_submitted_date,-1)) as order_offer_submitted,
	row_number() over(partition by sk_client order by nullif(sk_offer_approved_date,-1)) as order_offer_approved,
	row_number() over(partition by sk_client order by nullif(sk_tenant_first_doc_sent_date,-1)) as order_tenant_first_doc_sent,
	row_number() over(partition by sk_client order by coalesce(nullif(sk_tenant_doc_complete_date,-1),nullif(sk_credit_analysis_init_date,-1))) as order_tenant_doc_complete,
	row_number() over(partition by sk_client order by nullif(sk_credit_analysis_init_date,-1)) as order_credit_analysis_init,
	row_number() over(partition by sk_client order by nullif(sk_credit_analysis_approved_date,-1)) as order_credit_analysis_approved,
	row_number() over(partition by sk_client order by nullif(sk_credit_analysis_end_date,-1)) as order_credit_analysis_end,
	row_number() over(partition by sk_client order by nullif(sk_contract_created_date,-1)) as order_contract_created,
	row_number() over(partition by sk_client order by nullif(sk_contract_signed_date,-1)) as order_contract_signed,
-- count of events
	sum(count(distinct case when sk_booking_created_date > 0 then sk_booking end)) over(partition by sk_client) as bookings_created,
	sum(count(distinct case when sk_visit_date > 0 and flg_visit_completed = true then sk_visit end)) over(partition by sk_client) as visits_completed,
	sum(count(distinct case when sk_offer_submitted_date > 0 then sk_offer end)) over(partition by sk_client) as offers_submitted,
	sum(count(distinct case when sk_offer_approved_date > 0 then sk_offer end)) over(partition by sk_client) as offers_approved,
	sum(count(distinct case when sk_reservation_created_date > 0 then sk_reservation end)) over(partition by sk_client) as reservations_created,
	sum(count(distinct case when sk_contract_signed_date > 0 then sk_contract end)) over(partition by sk_client) as contracts_signed
...
select
	...
	reservations_created,
	contracts_signed,
	coalesce(coalesce(nullif(first_city_group_booking,-1),nullif(first_city_group_offer,-1)),'-1') as first_city_group,
    case when nullif(first_booking_created_date,-1) is null and nullif(first_offer_submitted_date,-1) is not null then 'Offer'
         when nullif(first_booking_created_date,-1) is not null and nullif(first_offer_submitted_date,-1) is null then 'Booking'
         when date(nullif(first_booking_created_date,-1)) <= date(nullif(first_booking_created_date,-1)) then 'Booking'
         when date(nullif(first_booking_created_date,-1)) > date(nullif(first_booking_created_date,-1)) then 'Offer'
