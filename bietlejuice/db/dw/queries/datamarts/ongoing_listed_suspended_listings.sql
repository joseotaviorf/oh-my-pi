...
select
    fhls.*,
    rank() over(partition by dd.week_start, fhls.sk_house_listing order by coalesce(fhls.status_start_date, to_char(current_date -1, 'YYYYMMDD')::bigint) desc) as rk,
    dd.date,
    ...
    dd.week_start,
    dh.is_last_version
from fact_house_listing_status_dates fhls
join dim_date dd
  on dd.sk_date between fhls.status_start_date and coalesce(to_char(to_date(fhls.status_end_date, 'YYYYMMDD') - 1, 'YYYYMMDD')::bigint, to_char(current_date -1, 'YYYYMMDD')::bigint)
left join dim_house_listing dh
  on fhls.sk_house_listing = dh.sk_house_listing
