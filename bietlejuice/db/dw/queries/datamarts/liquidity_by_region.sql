...
listings_metrics AS (
  ...
  LEFT JOIN listings_regions r ON hl.sk_house_listing = r.sk_house_listing
  JOIN public.dim_region dr ON r.sk_region = dr.sk_region
  LEFT JOIN days_published dp ON hl.sk_house_listing = dp.sk_house_listing
  WHERE r.sk_region != -1
),
liquidity_metrics_region_code AS (
  SELECT
    m.city_group,
    m.region_code,
    NULL AS sk_region,
...
liquidity_metrics_sk_region AS (
  SELECT
    m.city_group,
    m.region_code,
    m.sk_region,
...
all_regions AS (
  WITH
  regions AS (
    SELECT *, 'sk_region' AS geographic_scale FROM all_sk_regions
    UNION ALL
    SELECT *, 'region_code' AS geographic_scale FROM all_region_codes
  )
  SELECT * FROM regions
  CROSS JOIN (
