...
select
 tx1.*,
 coalesce(ls1.total_monthly_active_users, 0) as total_monthly_active_users
from
 sessions tx1
...
select
	 ...
	 supply.utm_content,
	 supply.utm_term,
	 sum(coalesce(supply.total_daily_leads, 0)) as total_monthly_leads,
	 sum(coalesce(supply.total_daily_listings, 0)) as total_monthly_listings,
	 sum(coalesce(us.total_monthly_sessions, 0)) as total_monthly_sessions,
	 sum(coalesce(us.total_monthly_active_users, 0)) as total_monthly_active_users
from
	supply supply
left join users us
