with all_dates_prev as (
	...
	l.utm_content,
	l.utm_term,
    rank() over (partition by coalesce(dr.city_group, ''),
...
all_dates as (
  select distinct
	...
	utm_content,
	utm_term,
    max(daily_count) over (partition by city_group, _year, _month, _week, _day) as daily_count,
    max(weekly_count) over (partition by city_group, _year, _week) as weekly_count,
    max(monthly_count) over (partition by city_group, _year, _month) as monthly_count,
    max(yearly_count) over (partition by city_group, _year) as yearly_count
  from all_dates_prev
),