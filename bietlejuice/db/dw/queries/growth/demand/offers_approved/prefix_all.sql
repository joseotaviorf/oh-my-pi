  ...
  select distinct
    ...
    dense_rank() over (partition by date_part('year', dof.dt_analysis),
    	...
    	+ dense_rank() over (partition by date_part('year', dof.dt_analysis),
			...
			- 1 as monthly_count,
    dense_rank() over (partition by date_part('year', dof.dt_analysis) order by dof.sk_offer asc)
    	+ dense_rank() over (partition by date_part('year', dof.dt_analysis) order by dof.sk_offer desc)
			- 1 as yearly_count
