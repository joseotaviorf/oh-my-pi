with all_dates as (
  ...
  order by coalesce(dr.region_code, ''), date_part('year', dof.dt_analysis), date_part('month', dof.dt_analysis), date_part('week', dof.dt_analysis), date_part('day', dof.dt_analysis)
),
...
all_dates_last_month as (
	...
	where date_part('year', dof.dt_analysis) = date_part('year', add_months(current_date, -1))
 	...
 	group by coalesce(dr.region_code, ''), date_part('year', dof.dt_analysis), date_part('month', dof.dt_analysis)
  order by coalesce(dr.region_code, ''), date_part('year', dof.dt_analysis), date_part('month', dof.dt_analysis)
),
all_dates_last_year as (
  ...
  left join dim_region dr
	...
	group by coalesce(dr.region_code, ''), date_part('year', dof.dt_analysis)
	order by coalesce(dr.region_code, ''), date_part('year', dof.dt_analysis)
),
