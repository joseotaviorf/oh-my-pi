with all_dates as (
  ...
  order by coalesce(dr.city_name, ''), date_part('year', dof.dt_analysis), date_part('month', dof.dt_analysis), date_part('week', dof.dt_analysis), date_part('day', dof.dt_analysis)
),
...
all_dates_last_month as (
	...
	where date_part('year', dof.dt_analysis) = date_part('year', add_months(current_date, -1))
 	...
 	group by coalesce(dr.city_name, ''), date_part('year', dof.dt_analysis), date_part('month', dof.dt_analysis)
  order by coalesce(dr.city_name, ''), date_part('year', dof.dt_analysis), date_part('month', dof.dt_analysis)
),
all_dates_last_year as (
  ...
  join fact_house_listings fhl
	...
	group by coalesce(dr.city_name, ''), date_part('year', dof.dt_analysis)
	order by coalesce(dr.city_name, ''), date_part('year', dof.dt_analysis)
),
