with all_dates as (
  ...
  order by coalesce(dr.city_name, ''), date_part('year', dc.ts_signature), date_part('month', dc.ts_signature), date_part('week', dc.ts_signature), date_part('day', dc.ts_signature)
),
...
all_dates_last_month as (
	...
	where date_part('year', dc.ts_signature) = date_part('year', add_months(current_date, -1))
 	...
 	group by coalesce(dr.city_name, ''), date_part('year', dc.ts_signature), date_part('month', dc.ts_signature)
  order by coalesce(dr.city_name, ''), date_part('year', dc.ts_signature), date_part('month', dc.ts_signature)
),
all_dates_last_year as (
  ...
  join fact_house_listings fhl
	...
	group by coalesce(dr.city_name, ''), date_part('year', dc.ts_signature)
	order by coalesce(dr.city_name, ''), date_part('year', dc.ts_signature)
),
