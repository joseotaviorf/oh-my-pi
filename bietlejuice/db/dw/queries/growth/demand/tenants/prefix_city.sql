...
all_dates_last_month as (
	...
	where date_part('year', dc.dt_signature) = date_part('year', add_months(current_date, -1))
 	...
 	group by coalesce(dr.city_name, ''), date_part('year', dc.dt_signature), date_part('month', dc.dt_signature)
  order by coalesce(dr.city_name, ''), date_part('year', dc.dt_signature), date_part('month', dc.dt_signature)
),
all_dates_last_year as (
  ...
  left join dim_region dr
	...
	group by coalesce(dr.city_name, ''), date_part('year', dc.dt_signature)
	order by coalesce(dr.city_name, ''), date_part('year', dc.dt_signature)
),
