...
fact as (
    SELECT distinct min(ads.id) over (PARTITION BY ads.ad_id, ads.account_id, ads.campaign_id, ads.adset_id) as id,
        ...
        (coalesce(android_smartphone.spend, 0) + coalesce(android_tablet.spend, 0) + coalesce(iphone.spend, 0) + coalesce(ipad.spend, 0) + coalesce(ipod.spend, 0)) as mobile_spend,
        coalesce(desktop.spend, 0) as desktop_spend,
        coalesce(other.spend, 0) as other_spend,
