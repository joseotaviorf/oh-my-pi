...
fact as (
    SELECT distinct min(ads.id) over (PARTITION BY ads.ad_id, ads.account_id, ads.campaign_id, ads.adset_id) as id,
        ...
        coalesce(android_smartphone.total_clicks, 0) as android_smartphone_total_clicks,
        ...
        (coalesce(android_smartphone.total_clicks, 0) + coalesce(android_tablet.total_clicks, 0) + coalesce(desktop.total_clicks, 0) + coalesce(iphone.total_clicks, 0) + coalesce(ipad.total_clicks, 0) + coalesce(ipod.total_clicks, 0) + coalesce(other.total_clicks, 0)) as total_clicks,
...
SELECT distinct
    ...
    fact.android_smartphone_total_clicks,
    fact.android_tablet_impressions,
    fact.android_tablet_reach,
    fact.android_tablet_total_clicks,
    fact.desktop_impressions,
    fact.desktop_reach,
    fact.desktop_total_clicks,
    fact.iphone_impressions,
    fact.iphone_reach,
    fact.iphone_total_clicks,
    fact.ipad_impressions,
    fact.ipad_reach,
    fact.ipad_total_clicks,
    fact.ipod_impressions,
    fact.ipod_reach,
    fact.ipod_total_clicks,
    fact.other_impressions,
    fact.other_reach,
    fact.other_total_clicks,
    fact.total_reach,
