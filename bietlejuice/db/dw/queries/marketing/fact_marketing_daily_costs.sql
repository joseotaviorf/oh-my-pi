with manual_google_costs as (
    with t_prep as (
        ...
        from datalake_raw.gsheet_marketing_manual_costs_google g
    )
    select
...
,demand_tax as (
	...
	from campaigns_full cf
		left join datalake_raw.gsheet_marketing_cost_campaign_city as mccc
			on lower(mccc.campaign_name) = cf.campaign_name_l
		left join datalake_raw.gsheet_taxonomy_mkt_cost as tx
