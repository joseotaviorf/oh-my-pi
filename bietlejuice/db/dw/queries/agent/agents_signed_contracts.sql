select distinct
  ...
  coalesce(ecp.name, nullif(dprop.nome, '')) as owner_name,
...
left join datalake_ebdb_clean_prod.contract_person ecp
	on ecp.id_contract = cast(f.sk_contract as bigint)
		and ecp.type = 'Proprietario'
