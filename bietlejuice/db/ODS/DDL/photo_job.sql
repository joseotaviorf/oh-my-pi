DROP TABLE IF EXISTS public.photo_job;
create table public.photo_job (
	id bigint,
	imovel_id bigint,
	job_status varchar(255),
	creation_origin varchar(20),
	flexible_schedule int,
	dt_photographer_accepted datetime,
	dt_job_created datetime,
	dt_job_issued datetime,
	dt_shoot_started datetime,
	dt_job_scheduled datetime,
	dt_photos_uploaded datetime,
	dt_updated datetime,
	scheduling_instructions text,
	photo_shoot_contact_name varchar(255),
	photo_shoot_email varchar(255),
	photo_shoot_phone varchar(20),
	photo_shoot_second_phone varchar(20),
	approved int,
	confirmed int,
	lockbox int,
	key_withdraw varchar(255),
	key_comments text,
	photographer_id bigint,
	photographer_name varchar(255),
	photographer_email varchar(255),
	dt_photographer_start datetime,
	photographer_contract_type varchar(255),
	job_problem_reason varchar(255),
	cancel_reason text,
	user_cancel_dt datetime,
	user_cancel_id bigint,
	user_cancel_name varchar(255),
	user_cancel_email varchar(255),
	user_cancel_type varchar(255),
	rep_id bigint,
	creation_to_scheduling_diff_minutes int,
	creation_to_scheduling_diff_hours int,
	creation_to_scheduling_diff_days int
)