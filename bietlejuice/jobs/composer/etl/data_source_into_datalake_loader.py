...
class DataSourceIntoDataLakeLoader:
    ...
    CREATE_QUERY_TEMPLATE = """CREATE EXTERNAL TABLE IF NOT EXISTS
                            ...
                            LOCATION '{path}'
                            ;"""
    CREATE_QUERY_RAW_FORMAT = (
        "ROW FORMAT serde 'org.apache.hive.hcatalog.data.JsonSerDe'"
    )

    @staticmethod
    @logger
    def load_full_table_into_datalake_raw(
        table_name, consumer, query=None, partition_by=None, concurrency=1
    ):
        ...
        logger.info(
            "m=load_full_table_into_datalake_raw, table={}.{}, msg=Getting  data...".format(
        ...
        write_df.saveAsTable(
            "{}.{}".format(DataSourceIntoDataLakeLoader.DATALAKE_RAW_DB, new_table_name)
        )
        logger.info(
            "m=load_full_table_into_datalake_raw, table={}.{},"
            "msg=Copied table into datalake ({}).".format(
                DataSourceIntoDataLakeLoader.DATALAKE_RAW_DB, new_table_name, final_path
            )
        )

    @staticmethod
    @logger
    def load_incremental_partitioned_table_into_datalake_raw(
        table_name, consumer, query, partition_by
    ):
        if not partition_by:
            raise RuntimeError(
        ...
        df.write.mode("overwrite").option(
            "compression", DataSourceIntoDataLakeLoader.RAW_CODEC
        ).format(DataSourceIntoDataLakeLoader.RAW_FORMAT).save(final_path)
        ...
        spark.sql(
            "REFRESH TABLE {}.{}".format(
                DataSourceIntoDataLakeLoader.DATALAKE_RAW_DB, new_table_name
            )
        )
        spark.sql(
    ...
    def create_athena_external_table(
        consumer, table_name, db_source, partition_by=None
    ):
        ...
        drop_query = DataSourceIntoDataLakeLoader.DROP_QUERY_TEMPLATE.format(
            database=DataSourceIntoDataLakeLoader.DATALAKE_RAW_DB, table=table_name
        )
        AthenaClient.execute_athena_query(
            drop_query, DataSourceIntoDataLakeLoader.DATALAKE_RAW_DB
        )
        logger.info(
            "m=create_athena_external_table, table={}.{}, msg=Dropped "
            "table in Athena successfully".format(
        ...
        AthenaClient.execute_athena_query(
            create_query, DataSourceIntoDataLakeLoader.DATALAKE_RAW_DB
        )
        if partition_by:
            AthenaClient.execute_athena_query(
                "MSCK REPAIR TABLE `{}`.`{}`;".format(
