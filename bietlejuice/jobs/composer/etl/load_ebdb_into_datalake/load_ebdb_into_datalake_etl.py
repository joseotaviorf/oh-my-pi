...
logger = QuintoAndarLogger('LoadEBDBIntoDatalakeETL')

QUERY_OUT_PATH = 's3://5a-datalake/temp/databricks_output/'


class LoadEBDBIntoDatalakeETL:
    RAW_PATH = 's3://5a-datalake/temp/ebdb'
    ATHENA_RAW_SCHEMA = 'datalake_raw'
    RAW_FORMAT = 'json'
    ...
    CREATE_QUERY_TEMPLATE = """CREATE EXTERNAL TABLE IF NOT EXISTS
                            ...
                            LOCATION '{path}'
                            ;"""
    RAW_FORMAT_CREATE_QUERY = 'ROW FORMAT serde org.apache.hive.hcatalog.data.JsonSerDe'
    ...
    def load_full_table_into_datalake_raw(self, table, consumer, query=None, partition_by=None, read_concurrency=1):
        ...
        wr_df = df.write.mode("overwrite").format(self.RAW_FORMAT).option('path', final_path)
        if partition_by:
            for col in partition_by:
                wr_df = wr_df.partitionBy(col)
        wr_df.saveAsTable(db + '.' + table)
        logger.info(
            'm=load_full_table_into_datalake_raw, msg=Copied table {}.{} into datalake ({}).'.format(db,
                                                                                                     table,
                                                                                                     final_path))

        self._create_athena_external_table(DatabricksConsumer(db), self.ATHENA_RAW_SCHEMA,
                                           self.RAW_FORMAT_CREATE_QUERY, final_path, partition_by)
        logger.info(
            'm=load_full_table_into_datalake_raw, msg=Created external table {}.{} in datalake.'.format(
                self.ATHENA_RAW_SCHEMA, table))
    ...
    def load_incremental_partitioned_table_into_datalake_raw(self, table, consumer, query, partition_by):
        ...
        # refresh table in spark metastore and in athena
        spark = SparkSession.builder.getOrCreate()
        spark.sql('REFRESH TABLE {}.{}'.format(db, table))
        spark.sql('MSCK REPAIR TABLE {}.{}'.format(db, table))
        execute_athena_query(get_athena_client(), 'MSCK REPAIR TABLE {}.{}'.format(
            self.ATHENA_RAW_SCHEMA, table), self.ATHENA_RAW_SCHEMA)
    ...
    def get_table_names_and_sizes_from_consumer(self, consumer):
        consumer.get_table_names_and_sizes()

    @logger
    def _create_athena_external_table(self, consumer, table, athena_db, file_format, path, partition_by=None):
        ...
        drop_query = self.DROP_QUERY_TEMPLATE.format(database=athena_db,
                                                     table=table)
        execute_athena_query(get_athena_client(), drop_query, athena_db)
        logger.info('m=_create_athena_external_table, msg=Dropped table {}.{} in Athena successfully'.format(
            athena_db,
            table))
        ...
        create_query = self.CREATE_QUERY_TEMPLATE.format(database=athena_db,
                                                         table=table,
        ...
        execute_athena_query(get_athena_client(), create_query, athena_db)
        execute_athena_query(get_athena_client, 'MSCK REPAIR TABLE {}.{};'.format(athena_db, table))
