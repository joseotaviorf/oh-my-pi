...
for app_group in APP_GROUPS:
    for endpoint in ENDPOINTS:
        load_to_raw_task = QuintoAndarDatabricksSubmitRunOperator(
            task_id=f"load-canvases-{endpoint}-{app_group}-to-raw",
            dag=dag,
            json={
                "spark_python_task": {
                    "python_file": f"{SPARK_JOBS_PATH}/load_braze_canvases_into_datalake.py",
                    "parameters": [
                        ENV,
                        SOURCE,
                        DATALAKE_BUCKET,
        ...
        create_cluster_task >> load_to_raw_task >> clean_sub_dags.pop(
            f"canvases_{endpoint}_{app_group}"
