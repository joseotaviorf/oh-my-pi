...
for app_group in APP_GROUPS:
    for identifier in IDENTIFIERS:
        load_to_raw_task = QuintoAndarDatabricksSubmitRunOperator(
            task_id=f"load-{identifier}-details-{app_group}-to-raw",
            dag=dag,
            json={
                "spark_python_task": {
                    "python_file": f"{SPARK_JOBS_PATH}/load_braze_details_into_datalake.py",
                    "parameters": [
                        ENV,
                        SOURCE,
                        DATALAKE_BUCKET,
        ...
        sync_metastore_table_task = QuintoAndarDatabricksSubmitRunOperator(
            task_id=f"sync-hive-metastore-{identifier}-details-{app_group}",
            dag=dag,
            json={
                "spark_python_task": {
                    ...
                    "parameters": [
                        ...
                        SOURCE,
                        "--table-name",
                        f"{identifier}_details_{app_group}",
                    ],
                }
            },
        )
        ...
        validate_sync_metastore_table_task = QuintoAndarDatabricksSubmitRunOperator(
            dag=dag,
            task_id=f"validate-sync-hive-metastore-{identifier}-details-{app_group}",
            json={
                "spark_python_task": {
        ...
        airflow_helpers.chain(
            create_cluster_task,
            load_to_raw_task,
            clean_sub_dags.pop(f"{identifier}_details_{app_group}"),
            terminate_cluster_task,
        )
