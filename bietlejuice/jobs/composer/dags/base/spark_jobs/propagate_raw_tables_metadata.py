...
class MetadataPropagator:
    def __init__(
        self, metadata_propagator_host, layer, metadata_type, database_name, source_name
    ) -> None:
...
if __name__ == "__main__":
    layer_value, metadata_type_value, source_name, relative_file_path, table_name, all_tables = (
        parse_args()
    )
    ...
    logger.info(
        f"m={JOB_NAME}, "
        f"layer={layer}, metadata_type={metadata_type}, source_name={source_name}, "
        f"table_name={table_name}, all_tables={all_tables}, msg=Job execution started."
    )

    spark_ms = SparkMetastoreHelper("", layer, source_name, table_name, all_tables)
