...
from bietlejuice.jobs.composer.builders import CypressRawBuilder


JOB_NAME = "cypress_load_to_raw"
SOURCE = "cypress"
...
if __name__ == "__main__":
    ...
    s3_source_bucket = s3.Bucket(s3_file_path_source)
    s3_consumer = S3Consumer(spark_client)
    s3_loader = S3Loader()
    spark_metastore_loader = SparkMetastoreLoader(spark_metastore_service)

    cypress_raw_builder = CypressRawBuilder(s3_consumer, s3_service, execution_date)
    s3_source_file_paths = cypress_raw_builder.build_file_paths(s3_source_bucket)

    db_info = DatalakeMetastoreService.get_db_info(env, SOURCE, datalake_bucket)
    metastore_database_name = db_info["db_raw_databricks"]
    database_location = db_info["db_raw_path"]
    partition_cols = ["pwa", "dt"]

    for s3_source_file_path in s3_source_file_paths:
        try:
            json_data = cypress_raw_builder.get_json_data(s3_source_file_path)
            suites, tests = cypress_raw_builder.parse_json_data(json_data)
            for table_name, df in (("tests", tests), ("suites", suites)):
                if bool(df):
                    (
                        s3_target_database_location,
                        s3_target_table_name,
                    ) = cypress_raw_builder.build_target_file_path(
                        table_name, s3_file_path_target
                    )
                    ...
                    spark_metastore_service.refresh_table(
                        metastore_database_name, table_name
                    )
        except Exception as e:
            logger.info(f"m={JOB_NAME}, path={s3_source_file_path} error={e}")
