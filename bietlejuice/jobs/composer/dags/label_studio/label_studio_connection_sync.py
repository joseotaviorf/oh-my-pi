...
class LabelStudioConnectionSync:
    ...
    def __delete_connection_s3_storage(self, storage_id):
        ...
        if not response.ok:
            raise Exception(
                f"""m=__delete_connection_s3_storage, storage_id={storage_id},
                msg=There seems to be some problem with the s3 storage connection settings."""
            )
    ...
    def sync_data_s3_storage(self, project_id, project_name):
        ...
        self.__delete_connection_s3_storage(storage_id)
