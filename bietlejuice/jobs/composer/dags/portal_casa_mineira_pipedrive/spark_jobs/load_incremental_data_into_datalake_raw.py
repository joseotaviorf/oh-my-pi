...
def get_deals_flow(client, deals_id):
    ...
    response = consumer.sync(ids_list=deals_id, extra_params={"api_token": client.api_token})
...
def filter_deals(df):
    return df.filter("update_time >= '2021-01-01'").filter("pipeline_id == 6 or pipeline_id == 8 or pipeline_id == 9")
...
if __name__ == "__main__":
    ...
    parser.add_argument("full_load_execution_date", help="full load execution date date in str format")
    ...
    deals_df = deals_df.withColumn("update_time", to_date(to_timestamp(unix_timestamp("update_time", date_time_format))))            
    ...
    for table_name, table_config in config_service.get_config("tables_configurations").items():
        ...
        if deals_normalized and total_deals > 0: 
            ...
            if table_name == 'deals':
                ...
                df = spark_client.create_dataframe(data=selected_deals_flow, schema=deals_flow_schema)
                df = df.withColumn(column_create_date, to_date(to_timestamp(unix_timestamp(column_create_date, date_time_format))))            
                ...
                df = spark_client.create_dataframe(data=all_stages, schema=all_stages_schema)
                df = df.withColumn(column_create_date, to_date(to_timestamp(unix_timestamp(column_create_date, date_time_format))))            
