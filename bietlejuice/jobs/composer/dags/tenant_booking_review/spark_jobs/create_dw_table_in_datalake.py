...
if __name__ == "__main__":
    ...
    prefixed_table_name = f"dim_{table_name}"

    if schema == CUSTOM_SQL_QUERY_SCHEMA:
        query = f"SELECT * FROM dw_staging.{prefixed_table_name}"
    else:
        query_path = f"{QUERIES_DATALAKE_PATH}{table_name}/{prefixed_table_name}.sql"
    ...
    metastore_service.create_database(db_info["dw_schema_databricks"])
