...
def fetch_onetrust_organizations(auth_headers):
    client = OnetrustClient(auth_headers=auth_headers)

    organizations = CONSUMERS["Organizations"](client=client)

    results = organizations.sync()

    return results


def create_df_from_organizations(organization, spark_client):

    rows = []
    rows.append(
        Row(
            organization_id=str(organization.get("organizationId")),
            external_id=str(organization.get("externalId")),
            parent_organization_id=str(organization.get("parentOrganizationId")),
            parent_external_id=str(organization.get("parentExternalId")),
            name=str(organization.get("name")),
            default_approver=str(organization.get("defaultApprover")),
            default_language_code=str(organization.get("defaultLanguageCode")),
            description=str(organization.get("description")),
            children=str(organization.get("children")),
        )
    )
...
if __name__ == "__main__":
    ...
    organizations_response = fetch_onetrust_organizations(credentials["auth_headers"])

    spark_client = SparkClient()
    df = create_df_from_organizations(organizations_response, spark_client)
