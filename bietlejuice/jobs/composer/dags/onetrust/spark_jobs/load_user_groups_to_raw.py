...
def fetch_onetrust_user_groups(auth_headers):
    client = OnetrustClient(auth_headers=auth_headers)

    user_groups = CONSUMERS["UserGroups"](client=client)

    results = user_groups.sync()

    return results


def create_df_from_user_groups(results, spark_client):
    rows = []
    for user_groups in results:
        for record in user_groups:
            rows.append(
                Row(
                    user_group_id=str(record.get("userGroupId")),
                    name=str(record.get("name")),
                    description=str(record.get("description")),
                    created_by=str(record.get("createdBy")),
                    created_date=str(record.get("createdDate")),
                    last_modified_by=str(record.get("lastModifiedBy")),
...
if __name__ == "__main__":
    ...
    user_groups_response = fetch_onetrust_user_groups(credentials["auth_headers"])

    spark_client = SparkClient()
    df = create_df_from_user_groups(user_groups_response, spark_client)
