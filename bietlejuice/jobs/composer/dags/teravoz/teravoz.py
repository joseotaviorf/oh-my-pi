...
from bietlejuice.jobs.composer.base import BaseSubDAG
...
def sub_dag(sub_dag_name, **kwargs):
    ...
    request_api_and_load_to_raw_task = QuintoAndarDatabricksSubmitRunOperator(
        ...
        dag=local_dag,
        kwargs={
            'api_request': sub_dag_name,
            'execution_date': ''
        },
        json={
            "existing_cluster_id": '{{task_instance.xcom_pull(task_ids="create-cluster", key="cluster_id")}}',
            "spark_python_task": {
                "python_file": "{}/load_teravoz_into_datalake.py".format(SPARK_JOBS_PATH),
                "parameters": ['{{ ds }}', 'raw', sub_dag_name]
            },
    ...
    create_raw_partition_task = DummyOperator(
        task_id='create_raw_partition',
        dag=local_dag
    )

    load_to_clean_task = DummyOperator(
        task_id='load_to_clean',
        dag=local_dag
    )

    create_clean_partition_task = DummyOperator(
        task_id='create_clean_partition',
        dag=local_dag
    )
