...
RAW_PATH = 's3://5a-datalake/raw_spark/ebdb'
...
if __name__ == '__main__':
    ...
    connection_json = dbutils.secrets.get(
        ...
        key=DatabaseEnum.EBDB
    )
    connection = json.loads(connection_json)
    mysql_consumer = MySQLConsumer(connection)

    tables_sizes = dict(DataSourceIntoDataLakeLoader.get_table_names_and_sizes(mysql_consumer).collect())
