...
class TeravozCallsLoader(TeravozLoader):
    ...
    def __init__(self, api_user, api_pwd, execution_date):
        ...
        self.PARTITIONS["year"] = self.PARTITIONS["year"].format(year=execution_date.year)
        self.PARTITIONS["month"] = self.PARTITIONS["month"].format(month=execution_date.month)
        self.PARTITIONS["day"] = self.PARTITIONS["day"].format(day=execution_date.day)
        
        strdate = datetime.strftime(execution_date, "%d-%m-%Y")
