...
class TeravozLoader:
    ...
    def __init__(self, api_user, api_pwd):
        # api_instance
        self.api_instance = TeravozClient(
            api_user=api_user,
            api_pwd=api_pwd
        )
    ...
    def _build_s3_path_to_load(self, df, datalake_layer, endpoint, partitions=None):
        ...
        if partitions:
          ...
          self._create_partition_table(endpoint, (s3_path + partitions_path), list_partitions)
    ...
    @logger
    def __write_data_to_s3(self, df, s3_path, endpoint, partitions=None, partitions_path=None):
        ...
        # dataframe to json
        df_write = df.write.mode("overwrite")\
        ...
        if endpoint not in self.sqlContext.tableNames(dbName=self.SOURCE):
            logger.info('m=__write_data_to_s3, db={}, table_name={},'.format(self.SOURCE, endpoint)
