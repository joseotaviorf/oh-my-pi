...
ebdb_user_phone as (
    select 
        ...
        replace(telefoneprincipal,'+55') as telefoneprincipal,
        replace(telefonesecundario,'+55') as telefonesecundario,
        replace(telefonecomercial, '+55') as telefonecomercial,
        replace(oldphone, '+55') as oldphone
    from datalake_ebdb_raw_prod.usuario
    group by 1,2,3,4,5
),
ebdb_phones as (
    select
        id,
        -- concat all phone column values in a string. 
        array_join((array [telefoneprincipal] ||
                    array [telefonecomercial] ||
                    array [oldphone] || 
                    array [telefonesecundario]),',') as phones
    from ebdb_user_phone
    group by 1,2
),
call_new_events as (
    select
    ...
    from datalake_bigfone_clean_prod.call_new_events
    where 
        length(outside_phone_number)>=8
        and dt_event >= date('2019-09-01')
    group by 1,2
),
outside_user_phone as (
    select
        coalesce(tp.id, ts.id, tc.id, old.id) as id_user,
        new.id_call
    from call_new_events new
    ...
    left join (select max(id) as id, telefoneprincipal from ebdb_user_phone where telefoneprincipal is not null group by 2) tp
    on new.outside_phone_number=tp.telefoneprincipal
    left join (select max(id) as id, telefonesecundario from ebdb_user_phone where telefonesecundario is not null group by 2) ts
    on new.outside_phone_number=ts.telefonesecundario
    left join (select max(id) as id, telefonecomercial from ebdb_user_phone where telefonecomercial is not null group by 2) tc
    on new.outside_phone_number=tc.telefonecomercial
    left join (select max(id) as id, oldphone from ebdb_user_phone where oldphone is not null group by 2) old
    on new.outside_phone_number=old.oldphone
    where coalesce(tp.id, ts.id, tc.id, old.id) is not null
    group by 1,2
),
...
dialed_user_phone as (
    select
        id_call,
        max(id) as id_user -- filter for different users with the same phone (returns the most recent user)
    from (
        select
            d.id_call,
            ebdb.id
        from dialed_phone d
        inner join ebdb_phones ebdb
        -- if position>0, the dialed phone is a substring of the ebdb.phones
        on position(d.phone IN ebdb.phones)>0
    )
    group by 1
),
...
/*
    wait_time_interactions = time diff between the next event immediately after 'call_waiting' event and 'call_waiting' event
    A call can have multiple call_waiting event. So we find the pair (call_waiting, next_event_after_call_waiting) when:
    1. the events compared are different  
    2. min(ts_next_event) >= ts_call_waiting, and ts_next_event is the closest to ts_call_waiting
    3. id_call is the same
*/
wait_time_interactions as (
    select
        ...
        min(e.ts_created) as ts_created_next_wait_event
    from waiting_events wait
    inner join call_events e
    on wait.id_call=e.id_call 
    and wait.ts_created <= e.ts_created 
    and e.id<>wait.id
    group by 1,2
),
wait_time as (
...
left join (select distinct id_call from agent_ringing_events) ring
