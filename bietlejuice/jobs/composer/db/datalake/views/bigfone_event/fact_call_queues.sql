...
******************************************************************************************************************/
drop view if exists dw_teravoz_prod.fact_call_queues;
create or replace view dw_teravoz_prod.fact_call_queues as
with 
    queues as (
        select * from datalake_teravoz_clean_prod.queues
    ...
    */  
    blind_transfer as (
        ...
        from datalake_bigfone_clean_prod.called_blind_transfer_events bt
        inner join  wait_time_interactions wti 
            on wti.id_call = bt.id_call 
            and wti.queue_number = try_cast(destination_called_number as smallint)
    ...
    call_finished as (
        select
            ...
            ts_created,
            ts_created_local
        from datalake_bigfone_clean_prod.call_finished_events
        group by 2,3,4
    ),
    outside_phone as (
        select
...
select
    ...
    date_diff('second', wti.ts_created_wait_event, wti.ts_created_next_wait_event) as seconds_queue_waiting_duration,
