WITH task_resolution_ranking AS (
  SELECT
    ...
    type,
    ROW_NUMBER() OVER (PARTITION BY id_task, id_workgroup, type ORDER BY DATE(CONCAT(year,'-', month,'-', day)) DESC, ts_action DESC) AS ranking,
...
min_analyst_started AS (
  SELECT
    id_task,
    MIN(ts_action) AS ts_analyst_started
  FROM
