SELECT
    ...
    COALESCE(BIGINT(DATE_FORMAT(b.ts_created_local_tz, 'yyyyMMdd')), -1) AS sk_booking_created_date,
    COALESCE(BIGINT(DATE_FORMAT(b.ts_booking_local_tz, 'yyyyMMdd')), -1) AS sk_visit_date,
    COALESCE(BIGINT(DATE_FORMAT(b.ts_first_canceled_local_tz, 'yyyyMMdd')), -1) AS sk_visit_canceled_date,
    IF(b.is_visit_completed, BIGINT(DATE_FORMAT(b.ts_booking_local_tz, 'yyyyMMdd')), -1) AS sk_visit_completed_date,
