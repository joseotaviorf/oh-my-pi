...
united_deal_stage_pipeline AS (
    SELECT
        s.id_deal,
        s.stage_struct.value ::BIGINT AS id_stage,
        LAST(p.pipeline_struct.value::BIGINT, TRUE) OVER (PARTITION BY s.id_deal ORDER BY s.stage_struct.timestamp) AS id_pipeline,
        s.stage_struct.timestamp AS ts_stage_started
    FROM
        exploded_stages AS s
    LEFT JOIN
        exploded_pipelines AS p
            ON s.id_deal = p.id_deal
            AND s.stage_struct.timestamp = p.pipeline_struct.timestamp
),
calculated_end_timestamps AS (
    SELECT *,
        LEAD(ts_stage_started) OVER (PARTITION BY id_deal ORDER BY ts_stage_started) AS ts_stage_ended
    FROM
        united_deal_stage_pipeline
)
SELECT
    id_deal,
    id_stage,
