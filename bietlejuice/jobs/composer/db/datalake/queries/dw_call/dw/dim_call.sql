WITH ivr_events AS (
	...
	FROM datalake_bigfone.call_ivr_events
	GROUP BY 1,2,3
),
flex_events AS (
	SELECT
...
call_locations AS (
	SELECT
		...
		from_state,
		from_country
	FROM datalake_bigfone.inbound_call_locations
	GROUP BY 1,2,3,4
),
csat_events AS (
	SELECT
	...
	FROM datalake_bigfone.call_ivr_events
	WHERE csat_2 IS NOT NULL
	GROUP BY 1
)
SELECT
	COALESCE(ie.id_call,fe.id_task) AS sk_call, -- only inbound calls have id_call, but all calls have id_task
	COALESCE(ie.from_number,fe.from_number) AS from_phone_number,
	COALESCE(ie.to_number,fe.to_number) AS to_phone_number,
	COALESCE(fe.direction,'inbound') AS direction,
	cl.from_city,
	cl.from_state,
	cl.from_country,
...
FROM ivr_events ie
FULL JOIN flex_events fe
	ON fe.id_call = ie.id_call
LEFT JOIN csat_events ce
	ON ce.id_call = ie.id_call
LEFT JOIN call_locations cl
