...
first_booking_city AS (
	SELECT
		nb.*
	FROM next_bookings nb 
	INNER JOIN first_booking fb 
...
distinct_talk_to_agent AS (
	...
	FROM dw_datamarts_cross.talk_to_agent tta 
...
first_tta_city AS (
	SELECT
		nb.*
	FROM next_tta nb 
	INNER JOIN first_talk_to_agent fb 
