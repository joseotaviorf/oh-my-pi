select 
  ifnull(pe.peer, st.agent) as id_agent,
  ifnull(pe.queue, st.queue) as queue_number,
  ifnull(pe.name, st.fullName) as name,
  pe.answered as calls_answered,
  pe.notAnswered as calls_missed,
...
on pe.peer=st.agent and pe.queue=st.queue