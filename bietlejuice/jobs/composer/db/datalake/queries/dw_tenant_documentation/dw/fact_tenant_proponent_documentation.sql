...
SELECT
  ...
  CAST(DATE_FORMAT(ds.ts_first_documents_sent, 'yyyyMMdd') AS INTEGER) AS sk_first_document_sent_date,
  CAST(DATE_FORMAT(dr.ts_first_document_resent, 'yyyyMMdd') AS INTEGER) AS sk_first_document_resent_date,
  CAST(DATE_FORMAT(dr.ts_last_document_resent, 'yyyyMMdd') AS INTEGER) AS sk_last_document_resent_date,
  ds.amount_documents_needed,
  ds.amount_documents_sent + COALESCE(dr.amount_documents_resent, 0) AS amount_documents_sent,
