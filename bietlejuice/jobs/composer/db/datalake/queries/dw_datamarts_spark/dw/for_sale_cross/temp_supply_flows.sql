...
discard_by_context AS (
    ...
    LEFT JOIN
        datalake_wololo_clean.prospect AS p
            ON p.id_reference = dl.sk_lead
    LEFT JOIN
        wololo_discard AS w
            ON w.id_prospect = p.id
    LEFT JOIN
        conversions_prospect AS cp
