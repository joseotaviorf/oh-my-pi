...
house_date_series AS (
  ...
  JOIN datalake_ebdb_clean.access_type AS at
    ON at.id_house = h.id
...
key_type_base AS (
  ...
  key_type_ordered AS (
    ...
    FROM key_type_aud
    LEFT JOIN datalake_ebdb_clean.key_type AS kt
...
key_location_base AS (
  ...
  key_location_ordered AS (
    ...
    LEFT JOIN datalake_ebdb_clean.access_authorization_type AS at
