...
SELECT
    ...
    CASE
        ...
        ELSE hl.mkt_origin
    END AS mkt_origin
FROM dw_public.fact_house_listing_flows hl
INNER JOIN dw_public.dim_house_listing dhl
            ON hl.sk_house_listing = dhl.sk_house_listing
LEFT JOIN dw_datamarts_cross.quintoandar_consultant_listings ciq
...
source_ops_rent AS (
	...
	photo_job AS (
	    ...
	    FROM dw_public.fact_photo_job fpj
	    GROUP BY 1
	)
	SELECT
...
SELECT
    ...
    CASE
        ...
        ELSE hl.mkt_origin
    END AS mkt_origin
FROM dw_sale.fact_listing_flows hl
LEFT JOIN dw_datamarts_cross.quintoandar_consultant_listings ciq
...
source_ops_sale AS (
	...
	FROM sale_fact_listing_flows_adjust ssf
	    ...
	    LEFT JOIN dw_public.fact_house_listing_flows hlf
	        ON hlf.sk_house_listing_flow = ssf.sk_house_listing_flow
