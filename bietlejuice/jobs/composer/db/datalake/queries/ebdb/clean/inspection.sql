WITH
    comments AS (
        SELECT
            vistoria_id,
            max(comentario IS NOT NULL) AS has_inspector_comment,
            max(comentarioInquilino IS NOT NULL) AS has_tenant_comment,
            max(comentarioProprietario IS NOT NULL) AS has_owner_comment
        FROM
