...
select
  ...
  regexp_extract(get_json_object(events_json, '$.eventDate'), '(\\d{{4}}-\\d{{2}}-\\d{{2}}\\w{{1}}\\d{{2}}:\\d{{2}}:\\d{{2}})', 1) as event_date,
  regexp_extract(get_json_object(events_json, '$.snoozed'), '(\\d{{4}}-\\d{{2}}-\\d{{2}}\\w{{1}}\\d{{2}}:\\d{{2}}:\\d{{2}})', 1) as snoozed,
  ts_created,
  ts_updated,
  year,
  month,
