...
SELECT -- [ODS] This table was migrated FROM ODS flow and needs a future refactoring to remove castings and renamings
    ...
    CAST(u.id_google AS VARCHAR(255)) AS google_id,
    ...
    CAST(u.is_tenant AS VARCHAR(10)) AS inquilino,
    ag.is_sale_agent,
    ag.is_rent_agent,
    CAST(u.bank_another_holder AS INTEGER) AS dadosbancarios_outro_titular,
    CAST(u.has_house AS VARCHAR(10)) AS tem_imovel,
    CAST(u.has_tenant_app AS VARCHAR(10)) AS tem_app_inquilino,
    CAST(u.has_active_contract AS VARCHAR(10)) AS tem_contrato_ativo,
    CAST(u.has_accepted_sms AS INTEGER) AS aceita_sms,
    CAST(LEFT(u.name, 200) AS VARCHAR(255)) AS nome,
    u.cpf,
    u.rg,
    u.gender AS sexo,
    u.email,
