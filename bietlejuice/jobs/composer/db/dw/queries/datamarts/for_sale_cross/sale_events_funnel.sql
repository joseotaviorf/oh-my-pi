...
data_deal_quali AS (
    ...
    FROM
        datalake_sales_flow_clean_prod.sales_flow_aud
    WHERE
...
SELECT
    ...
    DATE(eso.dt_offer_accepted) AS dt_offer_accepted,
    DATE(eso.dt_sale_agreement_signed) AS dt_ccv_signed,
    ...
    DATE(eso.dt_sale_transacton_paid) AS dt_payment_concluded,
    ...
    DATE(eso.dt_house_registry_started) AS dt_matricula_inicio,
    DATE(eso.dt_house_registry_ended) AS dt_matricula_atualizada,
    ...
    ROUND(((dl.price - eso.first_price_offered_by_buyer)/dl.price), 3) AS offer_discount
FROM
	datalake_offer_prod.sale_offer AS eso
LEFT JOIN 
    sale.dim_listing AS dl 
      ON eso.id_house = dl.sk_house
LEFT JOIN
    datalake_sale_offer_flows_prod.sale_offer_flows AS sof 
      ON eso.id_offer = sof.id_offer
LEFT JOIN 
    data_deal_quali AS ddq 
      ON sof.id_sales_flow = ddq.id
LEFT JOIN 
    datalake_firestore_prod.monday AS m 
