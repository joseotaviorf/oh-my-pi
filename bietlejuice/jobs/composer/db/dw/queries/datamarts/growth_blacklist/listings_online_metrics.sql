...
pre_online_metrics AS (
    ...
    metrics AS (
        ...
        WHERE event_type IN ('listing_page_viewed', 'schedule_page_viewed', 'tips_page_viewed')
			...
			AND CAST(CAST(year AS VARCHAR) || CAST(month AS VARCHAR) || CAST(day AS VARCHAR) AS DATE) > DATE_ADD('week', -52, CURRENT_DATE)
  )
    SELECT
        event_date,
        house_id,
...
online_metrics AS (
    ...
    ON 
        m.house_id = dp.sk_house 
        AND m.event_date BETWEEN dp.min_status_date AND dp.max_status_date
    WHERE (
...
FROM 
    online_metrics om
LEFT JOIN 
    days_published_in_period pp
ON 
    pp.sk_house_listing = om.sk_house_listing 
    AND pp.date_period = om.date_period
