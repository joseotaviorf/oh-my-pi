...
affiliates_aquisiton AS (
    ...
    SELECT 
        ...
        cost_funnel_side,
    	SUM(events_quantity_fr) AS sum_events_quantity_fr,
    	SUM(events_quantity_fs) AS sum_events_quantity_fs,
    	sum_events_quantity_fr + sum_events_quantity_fs as sum_events_total,
    	SUM(cost_fr) as sum_cost_fr,
    	SUM(cost_fs) as sum_cost_fs,
    	sum_cost_fr + sum_cost_fs as sum_cost_total
    FROM datamarts.marketing_campaigns_costs_and_volumes mcc 
    LEFT JOIN dim_date d on d.sk_date = mcc.sk_date
    WHERE d.date BETWEEN CURRENT_DATE - interval '31 days' AND CURRENT_DATE
       AND event_type in ('Marketing Investment', 'Affiliates') AND (cost_funnel_side in ('affiliates') or cost_funnel_side is NULL)
       AND mkt_origin in ('Indica Aí - General', 'Indica Aí - Agents', 'Doorman')
       AND utm_campaign_normalized ~* '(acq)'
    ...
    SELECT 
    	...
    	b.mkt_medium,
    	b.mkt_source,
        utm_campaign_normalized as utm_campaign,
        b.cost_funnel_side as funnel_side,
        case when (sum_events_quantity_fr + (cast(sum_events_quantity_fs as float)/cast(relative_value_fr_fs as float))) > 0 
            then cast(sum_events_quantity_fr as float) / (sum_events_quantity_fr + (cast(sum_events_quantity_fs as float)/cast(relative_value_fr_fs as float))) * sum_cost_total 
            else sum_cost_fr end as "allocated_cost_fr",
        case when (sum_events_quantity_fr + (cast(sum_events_quantity_fs as float)/cast(relative_value_fr_fs as float))) > 0 
...
affiliates_engagement AS (
    ...
    LEFT JOIN dim_date d on d.sk_date = mcc.sk_date
    WHERE d.date BETWEEN CURRENT_DATE - interval '31 days' AND CURRENT_DATE
       AND event_type in ('Marketing Investment', 'qualifieds') AND ((cost_funnel_side in ('affiliates')  AND coalesce(mkt_channel,'') not like '%Commission%') or cost_funnel_side is NULL)
       AND mkt_origin in ('Indica Aí - General', 'Indica Aí - Agents', 'Doorman')
       AND  coalesce(utm_campaign_normalized,'') !~* '(acq)'
    GROUP BY 1,2,3,4,5,6,7,8,9
    )
    SELECT 
        sk_date,
...
affiliates_comission AS (
    ...
    SELECT 
    	...
    	city_group,
    	mkt_origin,
        'Commission Listing' as mkt_channel,
        mkt_medium,
    	mkt_source,
        utm_campaign_normalized,
        cost_funnel_side,
    ...
    LEFT JOIN dim_date d on d.sk_date = mcc.sk_date
    WHERE d.date BETWEEN CURRENT_DATE - interval '31 days' AND CURRENT_DATE
       AND event_type in ('Marketing Investment', 'listings') AND ((cost_funnel_side in ('affiliates') AND mkt_channel = 'Commission Listing') or cost_funnel_side is NULL)
       AND mkt_origin in ('Indica Aí - General', 'Indica Aí - Agents', 'Doorman')
    GROUP BY 1,2,3,4,5,6,7,8,9
    )
    SELECT 
...
demand AS (
    ...
    LEFT JOIN dim_date d on d.sk_date = mcc.sk_date
    WHERE d.date BETWEEN CURRENT_DATE - interval '31 days' AND CURRENT_DATE
       AND event_type in ('Marketing Investment', 'New Buyer Prospect', 'New Tenant Prospects') AND (cost_funnel_side in ('demand') or cost_funnel_side is NULL)
       AND mkt_origin in ('Tenants PWA')
    GROUP BY 1,2,3,4,5,6,7,8,9
    )
    SELECT 
        sk_date,
...
supply AS (
    ...
    FROM datamarts.marketing_campaigns_costs_and_volumes mcc 
    LEFT JOIN dim_date d on d.sk_date = mcc.sk_date
    WHERE d.date BETWEEN CURRENT_DATE - interval '180 days' AND CURRENT_DATE
       AND event_type in ('Marketing Investment', 'qualifieds') AND (cost_funnel_side in ('supply') or cost_funnel_side is NULL)
       AND mkt_origin in ('Owner PWA', 'Price Calculator')
    GROUP BY 1,2,3,4,5,6,7,8,9
    )
    SELECT 
        sk_date,
...
SELECT 
    ...
    allocated_cost_fr AS allocated_cost
FROM 
    affiliates_aquisiton
WHERE 
    allocated_cost_fr > 0
