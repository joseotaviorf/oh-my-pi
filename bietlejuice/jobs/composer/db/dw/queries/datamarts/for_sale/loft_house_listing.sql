WITH loft_listings_raw AS (
    SELECT
        CONCAT(id, 'Loft', CAST(ROW_NUMBER() OVER (PARTITION BY id ORDER BY CAST(CONCAT(CAST(year AS VARCHAR), '-', CAST(month AS VARCHAR), '-', CAST(day AS VARCHAR)) AS DATE)) AS VARCHAR)) AS sk_house,
        ...
        REGEXP_REPLACE(LOWER(CAST(address.street AS VARCHAR)), 'avenida|rua|av.|#|&|1|2|3|4|5|6|7|8|9| da | de | do | das | dos ') AS loft_address_for_join,
        CAST(address.number AS VARCHAR) AS st_number,
        CAST(address.zip_code AS VARCHAR) AS zip_code,
        CAST(geolocation.latitude AS VARCHAR) AS lat,
        CAST(geolocation.longitude AS VARCHAR) AS lng,
        CAST(house_info.floor AS INTEGER) AS floors,
        CAST(house_info.num_floors AS VARCHAR) AS num_floors,
        CAST(house_info.total_area AS INTEGER) AS total_area,
        ...
        CAST(house_info.year_built AS INTEGER) AS year_built,
        ...
        CONCAT(UPPER(SUBSTR(ELEMENT_AT(house_info.usage_type, 1), 1, 1)), LOWER(SUBSTR(ELEMENT_AT(house_info.usage_type, 1), 2, 15))) AS usage_type,
        CAST(price.sale.price AS DOUBLE) AS price,
        (CAST(price.sale.price AS DOUBLE)/CAST(house_info.total_area AS INTEGER)) AS price_m2,
        CAST(price.sale.condo_fee AS DOUBLE) AS condo_fee,
        CAST(price.sale.iptu AS DOUBLE) AS iptu,
        CONCAT(UPPER(SUBSTR(CAST(metadata.loft_property AS VARCHAR), 1, 1)), LOWER(SUBSTR(CAST(metadata.loft_property AS VARCHAR), 2, 15))) AS is_platform_property,
        CASE
