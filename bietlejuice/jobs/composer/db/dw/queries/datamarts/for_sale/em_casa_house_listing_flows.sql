WITH listings_raw AS (
    SELECT
        ...
        CONCAT(id, 'Emcasa', CAST(ROW_NUMBER() OVER (PARTITION BY CONCAT(id, 'Emcasa') ORDER BY CAST(CONCAT(CAST(year AS VARCHAR), '-', CAST(month AS VARCHAR), '-', CAST(day AS VARCHAR)) AS DATE)) AS VARCHAR)) AS id_status,
