WITH raw AS (
    SELECT 
        ...
        rev_type AS REVTYPE
    FROM
        datalake_ebdb_clean_prod.agent_data_aud
),
base AS (
    SELECT
        raw.*,
        ...
        DATE_FORMAT(from_unixtime(revision.ts_revision/1000),'%Y-%m-%dT%H:%i:%sZ') AS date,
        CASE
    ...
    FROM
        raw
    INNER JOIN
        datalake_ebdb_clean_prod.user_revision_entity revision 
            ON raw.rev=revision.id
    LEFT JOIN
        datalake_ebdb_clean_prod.user user 
            ON revision.id_user=user.id
    LEFT JOIN
        datalake_ebdb_clean_prod.user agent 
            ON raw.id=agent.id_agent
    LEFT JOIN
        datalake_ebdb_clean_prod.work_contract cwc 
            ON raw.workcontract_id=cwc.id
    LEFT JOIN
        datalake_ebdb_clean_prod.work_contract pwc 
            ON raw.previous_workcontract_id=pwc.id
    LEFT JOIN
        datalake_ebdb_clean_prod.agent_data_types at 
