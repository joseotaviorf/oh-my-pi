WITH termination_aud AS (
  SELECT
    ...
    ts_updated
  FROM 
    datalake_terminator_clean_prod.termination_aud
),
...
terminations_modified AS (
  ...
  FROM 
    termination_aud AS ta1
  INNER JOIN 
    termination_aud AS ta2
...
keys AS (
  SELECT
    ...
    owner_keys_location
  FROM 
    datalake_terminator_clean_prod.termination
),
completed_utility_attachments AS (
  SELECT
...
pending_utility_attachments AS (
  SELECT
    DISTINCT id_termination,
...
total_nps AS (
  ...
  FROM 
    datalake_terminator_clean_prod.termination AS t
  JOIN 
    tracksale.fact_nps_dispatches AS fnd
...
iq_nps AS (
  ...
  FROM 
    datalake_terminator_clean_prod.termination t
  JOIN 
    tracksale.fact_nps_dispatches fnd
      ON fnd.sk_contract = t.id_contract
  LEFT JOIN  
    tracksale.dim_nps_campaign dnc
      ON dnc.sk_nps_campaign = fnd.sk_nps_campaign
  WHERE 
    t.status <> 'CANCELED'
...
last_inspection_synch AS(
  ...
  FROM 
    datalake_terminator_clean_prod.termination AS t
  JOIN 
    fact_inspection_bookings AS fib
...
termination_finished_user AS (
SELECT
  id AS id_termination,
  min(rev) AS min_rev,
  min(ta.ts_updated) AS ts_termination_finished
FROM 
  datalake_terminator_clean_prod.termination_aud AS ta  
WHERE 
  ta.status = 'DONE'
GROUP BY 1
), 
...
SELECT
  ...
  au.email
FROM 
  termination_finished_user AS tfu 
JOIN 
  datalake_terminator_clean_prod.rev_info AS ri
    ON ri.rev = tfu.min_rev
LEFT JOIN 
  datalake_terminator_clean_prod.application_user AS au
    ON au.id = ri.id_user
...
last_negotiation AS (
  ...
  LEFT JOIN 
    datalake_terminator_clean_prod.termination_fee AS tf
