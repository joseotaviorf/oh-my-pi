...
class BaseSubDAG(object):
    ...
    def _build(self, entity):
        ...
        entity, staging_dim_entity, load_entity = self._build_data_tasks(
            entity_dag, entity
        )
    ...
    def _build_with_tests(self, entity, source_command, tests, table_name=None):
        ...
        entity_dag = self._build_local_dag()
        entity, staging_dim_entity, load_entity = self._build_data_tasks(
            entity_dag, entity, source_command, table_name
        )
    ...
    def _build_tests_tasks(dag, tests):
        ...
        for _test in tests:
            test_task = BaseDAG.build_python_operator(
                dag=dag, task_id="TEST_{}".format(_test[0]), python_callable=_test[1]
            )
