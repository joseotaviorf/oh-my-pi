    ...
    def get_dag_doc(dag_name):
        ...
        try:
            if len(file_path) == 1:
                ...
                raise Exception(f"The amount of files discovered for {dag_name} is {len(file_path)} and we expected to get 1")
        except Exception as e:
            logger.info(
                f"m=get_dag_doc, msg=we found an issue for {dag_name}, e={e}"
            )
        finally:
