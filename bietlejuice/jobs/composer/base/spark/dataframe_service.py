...
class DataFrameService:
    ...
    def make_schema_merging(table_name, db, file_format, path, partition_by_list, df):
        ...
        ddl = "create table {}.{} ({}) using {} partitioned by ({}) location '{}'".format(
            db, table_name, columns_ddl, file_format, partitions_ddl, path
        )
        logger.info(
    ...
    def incremental_write(
        ...
        if table_name not in sqlContext.tableNames(dbName=db):
            logger.info(
                "m=incremental_write, db={}, table_name={}, ".format(db, table_name)
                + "msg=table does not exist in db, creating new..."
            )
            write_df.option("path", path).saveAsTable(db + "." + table_name)
        else:
            if schema_merging:
                DataFrameService.make_schema_merging(
                    table_name, db, file_format, path, partition_by_list, df
                )
            logger.info(
                "m=incremental_write, db={}, table_name={}, ".format(db, table_name)
