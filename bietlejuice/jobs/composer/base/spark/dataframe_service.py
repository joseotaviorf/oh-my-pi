...
class DataFrameService():
    ...
    def make_schema_merging(table_name, db, file_format, path, partition_by_list, df):
        ...
        if not new_columns:
            logger.info('m=make_schema_merging, msg=the schema is compatible no need to recreate table')
            return None

        logger.info('m=make_schema_merging, msg=the schema is incompatible, creating new columns: {}'
    ...
    def df_create_year_month_day_columns(df, date_column_name):
        if not df:
            raise AttributeError('m=df_create_year_month_day_columns, msg=input df is None')
