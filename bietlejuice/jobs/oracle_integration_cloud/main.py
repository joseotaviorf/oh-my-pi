...
from bietlejuice.jobs.oracle_integration_cloud.oic_processing_api import (
    OICprocessingAPI,
)
...
class OICPipeline:
    ...
    def __init__(
        ...
        self.sftp_handler: SFTPHandler = None
    ...
    def _initialize_job(self) -> Tuple[SparkSession, "SFTPHandler", "PGPHandler", Any]:
        ...
        return spark, self.sftp_handler, pgp_handler, s3_resource

    def _process_file_raw(
        self, local_decrypted_path: str, args: dict, spark: SparkSession
    ) -> None:
        """
    ...
    def run(self):
        ...
        try:
            ...
            self.logger.info(f"Starting cleanup of old files for table '{table_name}'.")
            try:
                old_files = sftp_handler.list_files(hcm_tablename=table_name)
                for file in old_files:
                    sftp_handler.delete_file(file)
                self.logger.info(f"Finished cleanup of {len(old_files)} old file(s).")
            except Exception:
                self.logger.warning(
                    "Non-critical error during old file cleanup. The job will continue.",
                    exc_info=True,
                )

            oic_api = OICprocessingAPI(environment=args["environment"])
            self.logger.info(
                f"Triggering OIC API to generate report for '{table_name}'. This is a blocking call."
            )
            oic_api.send_process_request(report_name=table_name.upper())
            self.logger.info(
                "OIC API process finished. Checking for new files on SFTP server."
            )

            new_files = sftp_handler.list_files(hcm_tablename=table_name)
            if not new_files:
                raise FileNotFoundError(
                    f"OIC API reported success, but no new files were found on SFTP for table '{table_name}'."
                )

            self.logger.info(f"Found {len(new_files)} new file(s) to process.")
            ...
            for file in new_files:
                try:
                    ...
                    sftp_handler.delete_file(file)
                    self.logger.info(
                        f"Successfully processed and cleaned up file: {file}"
                    )

                except Exception:
                    self.logger.error(
                        f"A critical error occurred while processing file {file}. Halting job.",
                        exc_info=True,
                    )
                    raise
            ...
            self.logger.error(
                f"An unexpected error occurred in the main function: {e}", exc_info=True
            )
            sys.exit(1)
        finally:
