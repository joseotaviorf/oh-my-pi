...
logger = QuintoAndarLogger('ZendeskFactTicketTags')


class ZendeskFactTicketTags(Zendesk):
    CLASS_ENUM = ZendeskTableEnum.FACT_TICKET_TAGS
    SK_COLUMN = 'sk_ticket'

    @logger
    def __init__(self, s3_bucket, execution_date):
        super(ZendeskFactTicketTags, self).__init__(
            s3_bucket=s3_bucket,
