...
logger = QuintoAndarLogger('ZendeskFactTicketContactTypes')


class ZendeskFactTicketContactTypes(Zendesk):
    CLASS_ENUM = ZendeskTableEnum.FACT_TICKET_CONTACT_TYPES
    SK_COLUMN = 'sk_ticket'

    @logger
    def __init__(self, s3_bucket, execution_date):
        super(ZendeskFactTicketContactTypes, self).__init__(
            s3_bucket=s3_bucket,
