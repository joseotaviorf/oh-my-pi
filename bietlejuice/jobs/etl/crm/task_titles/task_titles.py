...
logger = QuintoAndarLogger('CRMTaskTitles')
...
class CRMTaskTitles(object):
    BUCKET_FOLDER_SUFFIX = 'crm/task_titles'

    TABLE_NAME = 'crm_task_titles'
    ...
    def extract_and_load_data(self):
        ...
        collection_gen = db.tasktitles.find().batch_size(10000)  # reduces the number of trips to the server
    ...
    def __delete_old_files(self):
        key = 'raw/{}/'.format(CRMTaskTitles.BUCKET_FOLDER_SUFFIX)
    ...
    def __save_to_s3(self, json_list, total_count):
        ...
        file_suffix = 'raw/{}/{}.gz'.format(CRMTaskTitles.BUCKET_FOLDER_SUFFIX, CRMTaskTitles.S3_FILE_NAME)
        self.__obj_to_s3(
            obj_io=gz_body,
    ...
    @logger
    def move_task_titles_to_clean(self):
        r_cols = OrderedDict([
            ('id', str),
            ('description', str),
            ('title', str),
        ...
        self.__move_to_clean(
            bucket_folder_suffix=CRMTaskTitles.BUCKET_FOLDER_SUFFIX,
            sql_file_name='create_task_titles_table.sql',
            r_cols=r_cols,
            c_cols=c_cols
        )
    ...
    def __move_to_clean(self, bucket_folder_suffix, sql_file_name, r_cols, c_cols, queries_folder_suffix=None):
        key = 'clean/{}/{}.parq'.format(bucket_folder_suffix, CRMTaskTitles.S3_FILE_NAME)
