...
logger = QuintoAndarLogger('HouseStatusHistoryCheck')


class HouseStatusHistoryCheck(object):
    TABLE_NAME = 'imovel_status_history_check'
    ...
    def __get_max_loaded_date():
        max_date = BaseETL.from_db_query(
            ...
            query="select max(date_status_changed) from {}".format(HouseStatusHistoryCheck.TABLE_NAME)
        )
    ...
    def load_data_into_ods_stg(self):
        max_loaded_date = HouseStatusHistoryCheck.__get_max_loaded_date()
        ...
        logger.info(
            'm=load_data_into_ods, table_name={}, msg=bulk inserting into ODS'.format(
                HouseStatusHistoryCheck.TABLE_NAME))
        BaseETL.bulk_insert(
            table=houses,
            table_name='stg.{}'.format(HouseStatusHistoryCheck.TABLE_NAME),
            db_enum=EnumDB.BI_ODS,
            encoding='utf-8',
            append=False,
            commit=True
    ...
    def delete_duplicated_entries(self):
        ...
        BaseETL.execute_command(
            command=delete_query.format(table_name=HouseStatusHistoryCheck.TABLE_NAME),
            db_enum=EnumDB.BI_ODS,
            encoding='utf-8',
            commit=True
        )
    ...
    def load_data_into_ods(self):
        ...
        BaseETL.execute_command(
            command='insert into public.{0} select * from stg.{0}'.format(HouseStatusHistoryCheck.TABLE_NAME),
            db_enum=EnumDB.BI_ODS,
            encoding='utf-8',
            commit=True
        )
