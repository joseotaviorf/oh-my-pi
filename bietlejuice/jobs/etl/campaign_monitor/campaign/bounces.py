...
logger = QuintoAndarLogger('CampaignBounces')


class CampaignBounces(CampaignMonitorCampaign):
    @logger(exclude='cm_auth')
    def __init__(self, s3_bucket, cm_auth, _type, execution_date):
        super(CampaignBounces, self).__init__(s3_bucket, cm_auth, _type, execution_date)
    ...
    def get_json_fields(self):
        return ['Date', 'EmailAddress', 'ListID', 'BounceType', 'Reason']
