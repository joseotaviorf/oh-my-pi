...
class QuintoAndarAWSBatchSensor(BaseSensorOperator):
    ...
    @apply_defaults
    def __init__(self, job_id=None, xcom_task_id=None, mode='reschedule', *args, **kwargs):
        if not job_id and not xcom_task_id:
            raise RuntimeError('m=__init__, msg=job_id and xcom_task_id cannot be both None!')
    ...
    def poke(self, context):
        ...
        if not self.job_id:
            self.job_id = xcom.xcom_pull(task_instance=context['ti'], task_id=self.xcom_task_id)

        job_status = self.batch_client.get_job_status_by_id(job_id=self.job_id)
        logger.info('m=poke, job_id={}, job_status={}, msg=poked job'.format(self.job_id, job_status))

        if job_status is None:
            raise RuntimeError('m=poke, job_id={}, msg=Job not found'.format(self.job_id))
        elif job_status in self.INTERMEDIATE_STATES:
            return False
        elif job_status in self.SUCCESS_STATES:
            return True
        elif job_status in self.FAILURE_STATES:
            raise RuntimeError('m=poke, job_id={}, msg=Job has failed!'.format(self.job_id))
        else:
            raise RuntimeError('m=poke, job_id={}, status={}, msg=status not expected!'.format(self.job_id, job_status))
