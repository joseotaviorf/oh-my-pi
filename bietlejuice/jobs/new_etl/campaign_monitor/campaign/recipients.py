...
logger = QuintoAndarLogger('CampaignRecipients')


class CampaignRecipients(CampaignMonitorCampaign):
    @logger(exclude='cm_auth')
    def __init__(self, s3_bucket, cm_auth, _type, execution_date):
        super(CampaignRecipients, self).__init__(s3_bucket, cm_auth, _type, execution_date)
