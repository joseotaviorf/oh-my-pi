...
class StitchTransferRaw(object):
    @logger
    def __init__(self, bucket, date, integration):
        self.s3_client = boto3.resource('s3')
        self.bucket = self.s3_client.Bucket(bucket)
        self.yesterday = date - timedelta(1)
    ...
    def copy_files(self):
        for key in self.__fetch_files():
            ...
            _logger.debug('m=copy_files, msg=creating new copy_source with origin bucket and origin key')
            copy_source = dict(
                Bucket=self.bucket.name,
                Key=key
            )
            _logger.info('m=copy_files, msg=copying files to raw bucket')
    ...
    def __build_key(self, key):
        ...
        account = tokens[2].replace('facebook_ads_', '') if self.integration == 'facebook_ads' else tokens[2]
        table = tokens[3]
        file = tokens[-1]

        _logger.info('m=move_files_from_stitch_to_raw, msg=bulding new path to copy files')
        return 'raw/facebook_ads/{table}/acc={account}/dt={extract_date}/{file}'.format(
            table=table,
            account=account,
