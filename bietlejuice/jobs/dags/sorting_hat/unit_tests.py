...
from bietlejuice.jobs.dags import SOURCE_QUERIES_TESTS_DIR, DATALAKE_TEST_QUERIES_DIR, ODS_TEST_QUERIES_DIR
...
@logger
def build(sub_dag_name, dag_name, schedule_interval, start_date, entity, use_query_param=False):
    """ Main method for building the entire Subdag """

    local_dag = __build_local_dag(sub_dag_name, dag_name, schedule_interval, start_date)
    __build_test_tasks(local_dag, entity, use_query_param)

    return local_dag


def __test_count(**kwargs):
    sh_query = BaseETL.get_query_from_file_name(kwargs['sh_file_path']).format(
        execution_date=kwargs.get('execution_date') if kwargs['use_query_param'] else None)
...
@logger
def __build_test_tasks(local_dag, entity, use_query_param):
    return BaseDAG.build_python_operator(
        ...
        op_kwargs={
            'sh_file_path': '{}/sorting_hat/{}_{}'.format(SOURCE_QUERIES_TESTS_DIR, entity, COUNT_CHECK_SQL_SUFFIX),
