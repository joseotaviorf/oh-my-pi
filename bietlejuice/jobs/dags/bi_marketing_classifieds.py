...
# Demand
demand_classifieds_raw_subdag = BaseSubDag.get_sub_dag_operator(
    dag=main_dag,
    sub_dag_func=raw_sub_dag,
    sub_dag_name='demand-classifieds-costs-load-to-raw',
    class_=MarketingEnum.CLASSIFIEDS_COSTS,
    auth=AUTH,
    extra_configs={'side': SIDE_DEMAND, 'sheet_id': MANUAL_COST_SHEETS['demand_costs']}
)

demand_classifieds_clean_subdag = BaseSubDag.get_sub_dag_operator(
    dag=main_dag,
    sub_dag_func=clean_sub_dag,
    sub_dag_name='demand-classifieds-costs-raw-to-clean',
    class_=MarketingEnum.CLASSIFIEDS_COSTS,
    auth=AUTH,
    extra_configs={'side': SIDE_DEMAND}
)
...
airflow_helpers.chain(demand_classifieds_raw_subdag, demand_classifieds_clean_subdag)
airflow_helpers.chain(supply_classifieds_raw_subdag, supply_classifieds_clean_subdag)

supply_demand_classifieds_load_to_staging_subdag.set_upstream(
    [demand_classifieds_clean_subdag, supply_classifieds_clean_subdag])
