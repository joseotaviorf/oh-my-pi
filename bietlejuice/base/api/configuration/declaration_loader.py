...
2. Installed package (Databricks - *_api_declaration.y*ml bundled via MANIFEST.in)
3. S3/volume (legacy fallback)

Convention: api_ingestion DAGs are named with _api suffix (e.g. currency_api).
The declaration file is {dag_name}_declaration.yml. MANIFEST.in includes only
*_api_declaration.y*ml so only these DAGs are bundled in the package.
"""
...
def _load_from_installed_package(dag_name: str) -> Optional[Dict[str, Any]]:
    ...
    Declarations are bundled in the whl via MANIFEST.in (same as gsheets
    declarations and ``*_conf.yml``). On Databricks, the whl is installed as a
    cluster library, so declarations are available from the package.
